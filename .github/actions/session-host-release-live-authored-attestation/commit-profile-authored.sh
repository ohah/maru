...
if ! valid_path "$evidence_bundle" || ! valid_path "$manifest_bundle"; then
  commit failed
  exit 1
fi
case "$timing_required" in
  false) [[ -z "$timing_bundle" ]] ;;
  true) valid_path "$timing_bundle" ;;
  *) false ;;
esac || {
  commit failed
  exit 1
...
printf 'evidence-bundle-path=%s\nmanifest-bundle-path=%s\ntiming-bundle-path=%s\n' \
  "$evidence_bundle" "$manifest_bundle" "$timing_bundle" >> "$output_file"
