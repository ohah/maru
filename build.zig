...
/// 빌드 헬퍼의 단일 출처. `build/` 아래 게이트 파일들도 같은 함수를 써야 하는데 그쪽에서
/// `build.zig` 를 import 하면 순환이라, 헬퍼만 `build/support.zig` 로 내리고 여기서는
/// 예전 이름 그대로 별칭을 둔다 — `build()` 본문은 한 글자도 바뀌지 않는다.
...
pub fn build(b: *std.Build) void {
    ...
    // **왜 4인가**: CI 실측이 4샤드 172초, 3샤드 249~272초다(인덱스 mod 3 은 무거운 판정자가 한 샤드에 몰려 균형이 나쁘다).
    // 한때 3으로 줄였던 이유는 코어(3 vCPU)보다 프로세스가 많으면 반복 횟수로 워커를 기다리는 판정자(`spins < 200_000`)가
    // 부하에서 흔들렸기 때문인데(2026-09-06 IG14 플레이크), #3327 이 그 대기를 벽시계 마감으로 바꿔 원인을 없앴다.
    //
    ...
    run_macos_editor_merge_tests.addArg("--maru-expect-tests=43");
    // ⚠️ **그리고 실제로 돌았는가** — MRG2 이후는 macOS 가 아니면 `SkipZigTest` 다.
    run_macos_editor_merge_tests.addArg("--maru-expect-passed=43");
    ...
    const macos_editor_untitled_tests = addProjectTest(b, .{
        .root_module = macos_app_host_abi_tests.root_module,
        .filters = &.{ "U1", "U2", "C0", "C1a", "C1b", "U3-", "U4a-", "U4b-", "U4c-" }, // `SYNU1` 도 걸린다(부분 일치) — 아래 개수가 그것을 포함한다
    });
    ...
    //       **U4a-1~U4a-13 열셋 + U4b-1~U4b-9 아홉 + U4c-1~U4c-5·U4c-8 여섯**을 더한 값이다(U4a 백업 · U4b 복원 · U4c 이름 없는 문서, §3.10).
    run_macos_editor_untitled_tests.addArg("--maru-expect-tests=118");
    // ⚠️ **그리고 실제로 돌았는가** — 전부 macOS 가 아니면 `SkipZigTest` 다.
    run_macos_editor_untitled_tests.addArg("--maru-expect-passed=118");
    ...
    const untitled_rule_tests = addProjectTest(b, .{
        ...
        .filters = &.{ "UT1", "UT2", "UT3", "UT4", "UB", "U4c-" }, // UB = 백업 레코드 규칙(§3.10), U4c- = workspace 영속
    });
    ...
    // 37 = UT1~UT4 넷 + **UB1~UB10 열**(백업 레코드 규칙 — §3.10) + **U4c-6·U4c-7 둘**(workspace 영속 —
    //      `untitled-term` 왕복·검증) + 이 그래프의 이름 없는 test 블록들(필터와 무관하게 컴파일된다).
    run_untitled_rule_tests.addArg("--maru-expect-tests=37");
    // ⚠️ **필터가 0 개를 골랐어도 「돌았다」로 읽히지 않게** 통과 수까지 못박는다(순수라 SKIP 이 없다).
    run_untitled_rule_tests.addArg("--maru-expect-passed=37");
    ...
    const provider_no_mutation_tests = addProjectTest(b, .{
        ...
        .filters = &.{
            ...
            // AK1 의 짝. AK1 은 리셋 함수를 직접 부르므로 `pollAgentKinds` 안의 **호출**은 안 잠긴다.
            "AK2: `pollAgentKinds` 가 종류 전환에서 리셋을 **실제로 부른다** (제품 경로)",
    ...
    if (target.result.os.tag == .macos) {
        ...
        // A local TCC grant is tied to the exact ad-hoc code requirement. This compile-only
        // escape hatch rebuilds the fixture harness without rebuilding, copying, or resigning
        // the already-staged product app that the user just approved.
        const install_session_host_cr6c_appkit_harness = b.addInstallArtifact(
            session_host_cr6c_appkit_harness,
            .{},
        );
        install_session_host_cr6c_appkit_harness.step.dependOn(&session_host_cr6c_appkit_harness.step);
        const build_session_host_cr6c_appkit_harness = b.step(
            "build-macos-session-host-cr6c-appkit-smoke-harness",
            "Build only the CR6c/CR6d AppKit harness without touching the staged app",
        );
        build_session_host_cr6c_appkit_harness.dependOn(&install_session_host_cr6c_appkit_harness.step);
        ...
        run_session_host_cr6d_appkit.setEnvironmentVariable(
            "MARU_SESSION_HOST_CR6C_ARTIFACT_ROOT",
            session_host_cr6d_home,
        );
        run_session_host_cr6d_appkit.setEnvironmentVariable("MARU_APP_SUMMARY_PATH", "/tmp/maru-macos-app/session-host-cr6d-home/app.summary.txt");
        run_session_host_cr6d_appkit.setEnvironmentVariable("MARU_MACOS_APP_SMOKE_MS", "60000");
        ...
        run_session_host_cr6d_appkit.setEnvironmentVariable(
            "MARU_CONFIG",
            "/tmp/maru-macos-app/session-host-cr6d-home/.config/maru/config",
        );
        ...
        session_host_cr6d_fixture.step.dependOn(&macos_app_bundle.step);
        ...
        run_session_host_cr6d_pixel_verify.addArgs(&.{
            "/tmp/maru-macos-app/session-host-cr6d-home/session-host-cr6d-ime-pixel-receipt.json",
            "/tmp/maru-macos-app/session-host-cr6d-home/captures/session-host-recovery-before-ime.ppm",
            "/tmp/maru-macos-app/session-host-cr6d-home/captures/session-host-recovery-first-marked.ppm",
        });
    ...
    if (builtin.os.tag == .macos) {
        ...
        run_remote_activity_vertical.addArg("--maru-expect-tests=30");
        ...
        run_turn_capture_wiring.addArg("--maru-expect-tests=66");
    ...
    if (builtin.os.tag != .windows) {
        ...
        run_activity_roundtrip.addArg("--maru-expect-tests=15");
        run_activity_roundtrip.addArg("--maru-expect-passed=15"); // env 가 빠지면 조용히 초록이 된다
    ...
    const editor_tests = addProjectTest(b, .{
        ...
        .filters = &.{ "MC", "EDIT", "UNDO", "SAVE", "EDOC", "FIND", "FOLD", "MOV", "CRT", "MM", "DGS", "DGP", "DGC", "LSF", "LSJ", "LSP", "LST", "LSI", "HVT", "HOVB", "GOTO", "SIG", "TXE", "FMT", "WSE", "RNM", "CPL", "CMP", "SGB", "SEM", "FRG", "FLD", "SAV", "RFP", "REF", "INL", "CAX", "CA", "DIRTY", "COPY", "PASTE", "CUT", "CLIP", "SEL", "DEL", "CUR", "TAB", "ADV", "AID", "PAIR", "CMT", "LANG", "EF", "IME", "ES", "NAV", "SP", "NS", "DFF", "LN", "CS", "ETX", "BR", "AC", "COL", "OPT", "OW", "EMK", "TIG", "FKB", "SBL", "DCARET", "DCOL", "DSB", "DHS", "CRUMB", "LOOP", "app_session.editor.", "app_session.editor_diff.", "session.editor." },
    });
    ...
    const editor_core_tests = addProjectTest(b, .{
        ...
        .filters = &.{ "LANG", "MOT", "CLIP", "PAIR", "DLT", "BUF", "OCC", "FND", "HL", "DGC", "DGT", "LSF", "LSJ", "LSP", "LST", "LSI", "HVT", "TXE", "WSE", "CPL", "SEM", "FRG", "INL", "CAX", "CT", "CASE", "ETX", "BR", "AC", "COL", "OPT", "OW", "EMK", "TIG", "FKB", "SBL", "DCARET", "DCOL", "DSB", "DHS", "CRUMB", "LOOP", "session.editor.", "session.syntax_theme.", "platform.cell_text.", "session.repo_path." },
    });
    ...
    const run_macos_app_host_abi_shards_boundary_tests = b.addRunArtifact(macos_app_host_abi_shards_boundary_tests);
    ...
    const run_ci_cache_generations_boundary_tests = b.addRunArtifact(ci_cache_generations_boundary_tests);
    ...
    const run_debug_trace_alloc_wiring_tests = b.addRunArtifact(debug_trace_alloc_wiring_tests);
    run_debug_trace_alloc_wiring_tests.addArg("--maru-expect-tests=2");
    run_debug_trace_alloc_wiring_tests.setCwd(b.path("."));
    boundary_step.dependOn(&run_debug_trace_alloc_wiring_tests.step);
    ...
    run_remote_watch_channel.addArg("--maru-expect-tests=7");
    run_remote_watch_channel.addArg("--maru-expect-passed=7");
    ...
    const digest_site_step = b.step(
        ...
        "Every digest production site is counted under a distinct name with bytes",
    );
    const digest_site_tests = addProjectTest(b, .{
        .root_module = b.createModule(.{
            .root_source_file = b.path("tests/digest_site_attribution_boundary.zig"),
            .target = target,
            .optimize = optimize,
        }),
    });
    ...
    const run_digest_site = b.addRunArtifact(digest_site_tests);
    run_digest_site.addArg("--maru-expect-tests=1");
    run_digest_site.addArg("--maru-expect-passed=1");
    run_digest_site.setCwd(b.path("."));
    ...
    const session_host_release_authored_attestation_action_step = b.step(
        "test-session-host-release-authored-attestation-action",
        "Check the authored evidence and manifest pair attestation action",
    );
    session_host_release_authored_attestation_action_step.dependOn(&session_host_release_authored_attestation_action_contract.step);
    if (posix_host_tests) test_step.dependOn(&session_host_release_authored_attestation_action_contract.step);

    const session_host_release_profile_authored_payload_action_contract = b.addSystemCommand(&.{ "bash", "tools/test-session-host-release-profile-authored-payload-action.sh" });
    session_host_release_profile_authored_payload_action_contract.setCwd(b.path("."));
    session_host_release_profile_authored_payload_action_contract.stdio = .inherit;
    const session_host_release_profile_authored_payload_action_step = b.step(
        "test-session-host-release-profile-authored-payload-action",
        "Check the profile-aware authored attestation payload fan-out",
    );
    session_host_release_profile_authored_payload_action_step.dependOn(&session_host_release_profile_authored_payload_action_contract.step);
    if (posix_host_tests) test_step.dependOn(&session_host_release_profile_authored_payload_action_contract.step);
    ...
    // session-host 판정자 등록은 `build/session_host_gates.zig` 가 소유한다 —
    // 그 6,545줄이 여기 있었다. 위에서 만든 값 중 그쪽이 읽는 것만 Context 로 넘긴다.
    session_host_gates.register(b, .{
        .target = target,
        .optimize = optimize,
        .shutdown_wire_contract_mod = shutdown_wire_contract_mod,
        .syntax_mod = syntax_mod,
        .maru_mod = maru_mod,
        .build_options_test_mod = build_options_test_mod,
        .exe = exe,
        .session_host_tests = session_host_tests,
        .run_core_tests = run_core_tests,
        .run_exe_tests = run_exe_tests,
        .run_session_host_tests = run_session_host_tests,
    ...
    // 릴리스 어댑터 판정자 등록은 `build/session_host_release_gates.zig` 가 소유한다 —
    // 그 5,967줄이 여기 있었고, 바뀌는 이유(릴리스 절차·GitHub 표면)가 이 파일의 나머지와 다르다.
    session_host_release_gates.register(b, .{
        .target = target,
        .optimize = optimize,
        .syntax_mod = syntax_mod,
        .maru_mod = maru_mod,
        .exe = exe,
        .run_session_host_tests = run_session_host_tests,
        .run_session_host_ssh_upload_boundary_tests = run_session_host_ssh_upload_boundary_tests,
        .run_session_host_ssh_reconnect_isolation_boundary_tests = run_session_host_ssh_reconnect_isolation_boundary_tests,
        .macos_app_bundle_command = macos_app_bundle_command,
        .macos_host_tests = macos_host_tests,
        .posix_host_tests = posix_host_tests,
        .test_step = test_step,
