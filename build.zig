...
pub fn build(b: *std.Build) void {
    ...
    if (target.result.os.tag == .macos) {
        for ([_]std.builtin.OptimizeMode{ .Debug, .ReleaseFast }) |attestation_optimize| {
            ...
            const tests = addProjectTest(b, .{ .root_module = b.createModule(.{ .root_source_file = b.path("tests/session_host_release_adapter_github_manifest_attestation.zig"), .target = target, .optimize = attestation_optimize, .link_libc = true, .imports = &.{ .{ .name = "release_manifest", .module = release_manifest_mod }, .{ .name = "release_adapter_deadline", .module = deadline_mod }, .{ .name = "release_adapter_github_manifest_file", .module = file_mod }, .{ .name = "release_adapter_github_manifest_attestation", .module = composition_mod } } }) });
