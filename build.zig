...
pub fn build(b: *std.Build) void {
    ...
    // 45 = U1a~U1r 열여덟 + U2a~U2l 열둘 + U2r·U2s 둘 + SYNU1 하나(필터 부분 일치)
    //      + `U2` 가 부분 일치하는 기존 판정자 일곱 + 각 모듈이 자동 생성하는 `test_0` 다섯.
    run_macos_editor_untitled_tests.addArg("--maru-expect-tests=45");
    // ⚠️ **그리고 실제로 돌았는가** — 전부 macOS 가 아니면 `SkipZigTest` 다.
    run_macos_editor_untitled_tests.addArg("--maru-expect-passed=45");
