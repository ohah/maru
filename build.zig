...
pub fn build(b: *std.Build) void {
    ...
    run_macos_editor_untitled_tests.addArg("--maru-expect-tests=122");
    // ⚠️ **그리고 실제로 돌았는가** — 전부 macOS 가 아니면 `SkipZigTest` 다.
    run_macos_editor_untitled_tests.addArg("--maru-expect-passed=122");
    ...
    const untitled_rule_tests = addProjectTest(b, .{
        ...
        .filters = &.{ "UT1", "UT2", "UT3", "UT4", "UB", "U4c-", "U4d-" }, // UB = 백업 레코드 규칙(§3.10), U4c- = workspace 영속
    });
    ...
    // 37 = UT1~UT4 넷 + **UB1~UB10 열**(백업 레코드 규칙 — §3.10) + **U4c-6·U4c-7 둘**(`untitled-term` 왕복·검증) + **U4d-1·U4d-2 둘**(`remote-doc-term`) + 이 그래프의 이름 없는 test 블록들(필터와 무관하게 컴파일된다).
    run_untitled_rule_tests.addArg("--maru-expect-tests=39");
    // ⚠️ **필터가 0 개를 골랐어도 「돌았다」로 읽히지 않게** 통과 수까지 못박는다(순수라 SKIP 이 없다).
    run_untitled_rule_tests.addArg("--maru-expect-passed=39");
    ...
    const editor_tests = addProjectTest(b, .{
        ...
        .filters = &.{ "MC", "EDIT", "UNDO", "SAVE", "EDOC", "FIND", "FOLD", "MOV", "CRT", "MM", "DGS", "DGP", "DGC", "LSF", "LSJ", "LSP", "LST", "LSI", "HVT", "HOVB", "GOTO", "SIG", "TXE", "FMT", "WSE", "RNM", "CPL", "CMP", "SGB", "SEM", "FRG", "FLD", "SAV", "RFP", "REF", "INL", "DSY", "OCH", "SSEL", "CAX", "CA", "DIRTY", "COPY", "PASTE", "CUT", "CLIP", "SEL", "DEL", "CUR", "TAB", "ADV", "AID", "PAIR", "CMT", "LANG", "EF", "IME", "ES", "NAV", "SP", "NS", "DFF", "LN", "CS", "ETX", "BR", "AC", "COL", "OPT", "OW", "EMK", "TIG", "FKB", "SBL", "DCARET", "DCOL", "DSB", "DHS", "CRUMB", "LOOP", "app_session.editor.", "app_session.editor_diff.", "session.editor." },
    });
    ...
    const editor_core_tests = addProjectTest(b, .{
        ...
        .filters = &.{ "LANG", "MOT", "CLIP", "PAIR", "DLT", "BUF", "OCC", "FND", "HL", "DGC", "DGT", "LSF", "LSJ", "LSP", "LST", "LSI", "HVT", "TXE", "WSE", "CPL", "SEM", "FRG", "INL", "DSY", "OCH", "SSEL", "CAX", "CT", "CASE", "ETX", "BR", "AC", "COL", "OPT", "OW", "EMK", "TIG", "FKB", "SBL", "DCARET", "DCOL", "DSB", "DHS", "CRUMB", "LOOP", "session.editor.", "session.syntax_theme.", "platform.cell_text.", "session.repo_path." },
    });
