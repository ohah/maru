...
**카드는 두 줄이다 — 제목 + 메타(같은 줄, 오른쪽) / 요약**(2026-08-25 사용자 결정). 예전에는 메타가
자기 줄을 따로 썼는데, 기본 폭(640pt)에서 **제목 줄이 60%, 메타 줄이 44%** 를 비운 채 세로로 쌓여
있었다(실측). 한 줄로 합치면 그 여백이 정보가 되고 카드가 **102 → 78pt** 로 낮아져 같은 높이에 세션이
33% 더 들어온다.

- **제목이 먼저다.** 메타는 제목이 자기 폭을 가져간 뒤 남는 만큼만 쓴다 — 반대로 하면 짧은 제목까지
  잘린다. 다만 **provider + 개수**까지는 지킨다(그것마저 밀리면 "무엇이 얼마나 돌았는가"가 사라진다).
  그 최소치를 위해서는 제목이 바닥(80pt)까지 줄어드는 것을 허용한다.
- **좁아지면 글자를 자르지 않고 값을 통째로 내려놓는다**: 서브에이전트 → 모델 → 시각 순. 자르면
  `claude-o` 처럼 잘린 이름이 남아 정보가 아니라 잡음이 된다(실측).
- 폭 계산은 **role 크기**(제목 14pt·메타 12pt)로 한다. 셀 폭으로 재면 메타를 25% 넘게 과대평가해 함께
  놓을 수 있는 것도 못 놓는다 — 그 비율(`advance_milli_per_point`)은 platform 이 넘긴다.
