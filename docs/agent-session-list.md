...
**좁은 폭의 열화 규칙 — "제목은 마지막까지 남는다"**(2026-08-25). 카드가 좁아지면 **disclosure 예약 →
메타데이터 줄 → 요약 줄 → 카드 여백(16 → 8pt)** 순으로 버리고 제목을 남긴다. 세 뷰가 같은 규칙을 쓴다
([file-explorer.md](file-explorer.md) §3이 원본, 소스 컨트롤은 editor-surface-dock.md §3.5).

이 규칙 전에는 도크 하한(카드 폭 104pt)에서 카드가 **통째로 비었다** — chevron 하나만 남고 제목·요약·
메타가 전부 사라졌다(Lab 실측). `textAtY` 가 예산 0 이하면 조용히 돌아가고, disclosure 예약(48pt)이 그
폭을 먼저 먹었기 때문이다.
...
메타 줄(`메시지 N개 · 시각 · 모델 · 서브에이전트 N`)은 **문장 하나가 아니라 세그먼트 목록**이다
(`session_dock.types.CardMetadata`). platform 은 지역화된 세그먼트를 넘기고, **구분자와 색은 컴포넌트가
소유한다** — 셋을 한 문자열로 뭉치면 줄 전체가 한 색이 되어 개수·시각·모델이 같은 무게로 읽히고, 빈
세그먼트(서브에이전트 0)가 생길 때 구분자 손질을 문구가 떠안는다. 색은 세 위계다: **개수는 본문색**으로
먼저 읽히고, **시각은 muted** 로 물러나며, **모델은 카드 좌상단 provider 라벨과 같은 색**이라 "무엇이
돌렸는가"가 한 색으로 묶인다(2026-08-18).
...
- resume은 **사용자 로그인 셸을 거쳐** provider를 실행한다. `/usr/bin/env <provider>`를 직접 exec하면
  ...
  - **인용은 셸로부터만 지켜 준다.** transcript 내용이 실행 인자가 되는 자리는 둘(세션 id·모델)이고, 둘 다
    같은 위험을 진다 — `'-x'`는 셸에 안전하게 도착해도 **provider의 인자 파서가 플래그로 읽는다.** 그래서
    두 값 모두 같은 토큰 규칙을 지난다(`isSafeArgvToken`: `[A-Za-z0-9._-]`, 첫 글자는 `-`가 아님, 길이 상한은
    쓰는 쪽이 준다). 규칙이 둘이면 한쪽이 낡으므로 자리는 하나다.
  - 두 값의 **처분이 다르다**. 모델은 못 쓰면 플래그를 안 붙이고 기본 모델로 열리지만, **세션 id는 그 세션을
    가리키는 유일한 손잡이**라 못 쓰면 재개도 exact-live 대조도 성립하지 않는다 — 그래서 그 세션을 목록에
    넣지 않는다. 파서는 이미 빈 id를 같은 이유로 떨어뜨리고 있었고, 이것은 그 규율을 "빈 값"에서 "못
    가리키는 값"으로 넓힌 것이다.
  - 상한은 세션 id가 128바이트다. 실측은 전부 36바이트 UUID지만(2026-09-15, Claude 300·Codex 229 표본에서
    예외 0) Codex `resume`이 UUID 말고 **세션 이름**도 받으므로 UUID 모양을 강제하지는 않는다.
  - **세션 id는 표시용 정제(`displayCopy`)를 안 거치고 그대로 복사된다.** 제어문자가 섞인 id를 여기서 막지
    않으면 막는 자리가 어디에도 없다.
...
- resume은 **모델도 그대로 되살린다**. 두 provider 다 플래그가 있다(`claude --model <이름|별칭>`,
  ...
  - **모델 값은 거른다.** Claude Code는 합성 assistant 줄에 `"model":"<synthetic>"`를 적는다(실측
    ...
    아닌 값을 아예 기록하지 않는다**(`isResumableModel` — 위 §6의 공용 토큰 규칙에 1~64바이트 상한). 표시와 재개가
