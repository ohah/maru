...
- 끝나면 같은 자리가 `✻ Worked for 26s · done 오후 3:41 · 1 shell still running` 으로 바뀐다.
  **`…`(U+2026) 유무가 진행/완료를 가른다** — 7 pane × 30 회 관측에서 진행 줄은 **전부** `…` 로 끝났고
  완료 줄에는 **하나도** 없었다.
- 스피너 프레임은 회전한다 — 0.3 초 간격 900 회(≈5 분) 관측에서 여섯이 나왔다: `✽` 932 · `·` 896 ·
  `✢` 817 · `✻` 741 · `✳` 591 · `✶` 564. **그 표본에서 목록 밖 프레임은 없었다.**
- ⚠️ **표본 크기가 결론을 바꿨다.** 앞선 150 회 관측에서는 `·` 가 **0 회**라 목록에서 빠졌는데, 6 배로
  늘리니 **두 번째로 흔한 프레임**이었다. 드물어서 놓친 게 아니라 표본이 모자랐던 것이다 — 「관측에서
  안 나왔으니 없다」는 판단은 표본이 충분할 때만 선다.
- ⚠️ **관측 방법이 한 부류를 구조적으로 못 봤다.** 위 수집기는 줄 첫 문자를 `ord > 0x00A0` 으로 걸렀다.
  비ASCII 기호만 세려던 것인데, 그 조건이 **ASCII 프레임을 애초에 배제**한다. 표본을 6 배로 늘려도
  안 나오는 것이 당연했다 — 표본 크기가 아니라 **필터가 문제**였던 경우다.
- **`*`(U+002A) 는 넣지 않는다.** herdr(Apache-2.0) 의 claude 규칙에는 그 프레임이 있는데, 조건을
  ASCII 까지 넓혀 600 회를 다시 재도 이 환경(2.1.251~2.1.260)에서는 **0 회**였다. 넣지 않는 이유는 둘이다:
  ⑴ 우리 실측 근거가 없다 — 같은 이유로 추정 프레임 다섯(`✷✸✹✺✴`)을 이미 뺐고, 레퍼런스에 있다는
  이유만으로 넣으면 그 기준이 무너진다(그리고 목록을 그대로 옮기는 것은 clean-room 규율 위반이다).
  ⑵ **ASCII 라 오탐 비용이 크다** — `*` 는 마크다운 강조·리스트 마커로 흔해서 `* 무언가…` 한 줄이
  running 으로 둔갑한다. `⏺`(답변 블록 마커)를 뺀 것과 같은 판단이다.
  다른 환경에서 그 프레임이 실제로 관측되면 그때 **실측 근거로** 더한다.
- ⚠️ **답변 블록 마커 `⏺`(U+23FA)는 같은 관측에서 155 회로 최다지만 진행 신호가 아니다.** 완료된 답변
  줄이 화면에 계속 남고 그 안에 `…` 가 흔해서(실측: 3 줄 중 1 줄), 넣으면 배지가 영영 running 에 묶인다.
...
순간 한 번만** 찍는다(`Term.hook.turn_opened_wall_ns`) — 지금 필요한 질문이 그것 하나이고, 일반 지도는
