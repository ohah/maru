...
- **상태줄(진행중 표시 + 답변 미리보기 겸용)**: running이면 **`⠋ 진행중`**(회전 스피너), idle이면
...
- **running 스피너 + 솔리드 아이콘**(사용자 피드백, 펄스 폐기): 옛 "아이콘 밝기 펄스"(`blink_visible` 500ms 변조)는 작은
  ...
  **작업 중 애니메이션은 상태줄 회전 스피너**가 담당한다 — `● 진행중` 대신 **`⠋ 진행중`(브라유 8프레임 ⠋⠙⠹⠸⠼⠴⠦⠧)**을
  `agent_spin_frame`(running일 때 `advanceBlinkPhase`가 4틱≈130ms마다 +1, 사이드바 재투영)으로 회전. **스피너는 브랜드색**
