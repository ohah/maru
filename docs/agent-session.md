...
### 두 모드 (2026-08-20 개정 — 사용자 결정)

**이 문서 전체는 «관측 모드»의 계약이다.** provider 훅이 설치된 Term은 «훅 모드»로 돌고, 그 Term의 상태·알림은
훅 payload에서만 온다 — 이 문서의 화면 규칙·OSC 알림 경로를 **쓰지 않는다**. 모드 판정, 이벤트 세트, 전달
채널, 훅 모드에서 끄는 입력의 정확한 범위는 [에이전트 훅 통합](agent-hooks.md)이 단일 출처다.
...
필요하고 claude·codex에만 있으므로, 관측 모드는 **폐기하지 않고 폴백으로 유지**한다.

이 관측 계층은 다음을 의도적으로 제공하지 않는다.

- **관측 모드에서는** provider 파일·훅에 의존하지 않는다 — kind·state를 화면과 process tree만으로 정한다
  (훅 모드는 정반대이고, 그것이 두 모드를 섞지 않는 이유다)
...
훅이 남기는 것은 provider payload **전문**이고, 그중 파서가 쓰는 것은 일부다. 차지하는 비율로 재면
`tool_input` 이 **41%** (782KB) 인데 파서는 그 안의 `file_path`·`description`·`command` **세 개만** 본다.
나머지(파일 전문·패치 본문·긴 인자)는 쓰이지 않으면서 디스크에 남고, 그 내용이 **셸 명령과 프롬프트 원문**
이다 — 로그에 개수만 찍고 내용을 안 찍는 규율(§7)이 정작 파일에는 적용되지 않는 셈이다.

**그럼에도 지금 걸러내지 않는다.** 훅은 `printf` 한 줄이라 JSON 을 이해하지 못한다. 필터링하려면 `jq` 같은
파서가 필요한데 그것은 §4.1 이 의도적으로 피한 프로세스다 — 훅은 **도구 호출마다** 실행되므로(한 세션에서
`PreToolUse` 2,037 건 관측) 프로세스 하나가 그만큼 곱해진다. **시각을 넣는 것과 값이 다르다**: 시각은 셸
포맷 문자열 한 칸이라 프로세스가 안 늘지만, 필터링은 반드시 는다.
...
**메울 자리는 쓰는 쪽이다.** 훅 명령이 우리 것이므로(셸 한 줄의 `printf`) 거기서 시각을 찍으면 훅이 발화한
그 순간을 얻는다. **읽는 쪽에서 찍으면 안 된다** — 앱이 꺼져 있던 동안 쌓인 백로그를 나중에 한꺼번에 읽으면
몇 시간 전 일이 전부 «지금» 으로 둔갑한다(이 문서가 «backlog 는 상태만 세우고 알리지 않는다» 를 두는 이유와
같은 사고다). 훅이 fire-and-forget 이라 병렬로 도는 것도 읽기 순서를 발생 순서로 못 쓰게 만든다.
