...
- **처음 만나는 파일을 셸이 고치면** before가 없다 → 그 파일만 «변경됨(비교 불가)». `Read` 캡처가
  이 경우를 크게 줄인다.
- **관측 모드**(훅 없음)에는 캡처 트리거가 없다. §4.3의 세션 파일 파싱이 폴백인데, Claude transcript의
  `toolUseResult`에는 `originalFile`이 실려 **before까지 얻을 수 있다**(§2.2 실측). 다만 셸 편집은
  그 기록에도 없으므로 관측 모드의 셸 변경은 **턴 단위**까지만 간다. 모드에 따라 정확도가 갈리는 것을
  숨기지 않는다.
...
3. **관측 모드는 정확도가 낮다.** 훅이 없으면 캡처 트리거도 없어 세션 파일 파싱에 기댄다 — 셸 변경은
   턴 단위까지만 간다(§4.4). 모드에 따라 화면이 달라지는 것을 숨기지 않는다.
