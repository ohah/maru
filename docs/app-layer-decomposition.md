...
**추천: A** — 가장 적은 변경으로 위상이 정합한다. layering §2 4층 표에 "L4 = OS-중립 런타임(`src/app`) + OS 어댑터(`platform/*`·`pty/*`)"로 명문화하면 충분하고, `src/app`은 그대로 두되 "L4 공통 런타임"으로 규정한다. (B/C는 이름이 더 또렷하지만 변경량 대비 이득이 작아 보류 권고 — 사용자가 "런타임을 독립 층으로 보고 싶다"면 B 재검토.)
