...
>
> - **다른 기능의 설계 상한을 밀어 올렸다.** [editor-surface.md](editor-surface-tooling.md) §6이 파일 뷰어의 바이트 상한을 1 MiB → 8 MiB로 올린 직접 원인이 이 파일이다("이 저장소의 `app_session.zig`가 4.0 MB(60,965줄)라 못 열렸다"). 즉 한 파일의 크기가 이미 제품 다른 축의 결정을 바꿨다.
