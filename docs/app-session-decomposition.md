...
> ## test 전용 공용 헬퍼를 `test_support.zig`로 뺐다(2026-08-09)
>
> `app_session.zig`의 파일 레벨 선언 376개 중 **제품 경로가 한 번도 참조하지 않는 47개(705줄)**를
> `app_session/test_support.zig`로 옮겼다. 같은 트리의 `session_host/client_external_rx_turn_test_support.zig`와
> 같은 성격이다 — 제품 코드와 test 전용 harness를 가른다.
>
> 그 자체로도 값이 있지만, 재는 김에 **test 이동 비용이 얼마나 내려가는지**도 실측했다.
>
> | | pub화 필요 |
> |---|---|
> | 이전: 단일 그룹 test 313개(13,211줄) 이동 | **70개** |
> | `test_support.zig` 분리 후 같은 이동 | **16개** |
>
> 즉 §2-c-3이 "test 동반 이동은 pub화 6배"라고 잰 값의 **대부분은 도메인 결합이 아니라 공용 헬퍼가
> 파일 레벨에 흩어져 있던 탓**이었다. 원인을 잘못 짚은 채 결론을 낸 것이므로 그 근거를 여기서 갱신한다.
> (다만 §2-c-3의 **결론**인 "분해 시 test는 원본에 남긴다"는 `core.zig` 선례와 여전히 일치한다 —
> 아래 참조.)
>
> **경계 계산에서 결함 셋을 잡았다.** 이 문서의 이전 실측치들이 흔들릴 수 있는 지점이라 남긴다.
>   1. test 블록을 중괄호 균형으로 잡으면 문자열·주석 속 `{`에 깨져 **제품 코드를 test로 오분류**한다.
>      컬럼 0의 `}`를 종료로 삼아야 한다.
>   2. struct 값을 가진 `const`(`const X = struct {`)를 첫 `;`에서 자르면 선언이 반토막 난다.
>      컬럼 0의 `};`까지 봐야 한다.
>   3. 참조를 `app_session.zig` 안에서만 세면, 그룹 파일이 쓰는 선언이 "test 전용"으로 오판된다
>      (`sidebar_scroll_ids`가 그랬다). 그룹 파일 10개와 `app_host_abi.zig`까지 봐야 한다.
>
> ### test를 그룹 파일로 옮기지 않는다 — 시범 이동 실측으로 결론
>
> 저장소 전체로는 `src/**/*.zig` 381개 중 339개(88%)가 인라인 test를 가진다. 그러나 **분해 산출물로
> 한정하면 선례는 반대다** — `core.zig`(8,111줄·test 358개)에서 갈라져 나온 `parser.zig`(1,073줄)·
> `osc.zig`(325줄)·`types.zig`(403줄)·`kitty.zig`(517줄)는 **test 0개**, `screen.zig`(2,098줄)·
> `selection.zig`(929줄)는 1개다. 이 저장소는 **처음 쓰는 파일에는 test를 넣고, 분해 산출물에는 넣지
> 않는다.**
>
> 그래도 "pub이 안 닫히는 원인이 test 잔류 아닌가"라는 가설이 남아 `pane` 한 그룹으로 시범 이동을
> 실측했다(test 101개·3,833줄을 `pane.zig`로 옮겨 컴파일·`test-macos-app-host-abi` 통과까지 확인).
>
> | | 결과 |
> |---|---|
> | `app_session.zig` | 58,381 → 54,521 (−3,860) |
> | pub 신규 | **+35** (예측 12 — 3배 빗나갔다) |
> | pub 닫힘 | **0** |
> | 그중 `pub var` | `app_remote_client`·`host_connect_failed` — §2-c-2가 경고한 **되돌릴 수 없는** 종류 |
> | 예측이 놓친 것 | `extern "c" fn usleep`·`mkfifo` 같은 C interop 선언, `AppSession` 내부 선언 |
>
> **가설은 기각됐다.** 옮겨도 닫히는 것이 없고 오히려 는다. 원인은 비대칭이다.
>
> - test가 **허브**에 있으면 그룹 코드 접근은 공짜다 — 그룹 파일은 허브가 부를 수 있도록 어차피 전부
>   `pub fn`으로 내보낸다.
> - test가 **그룹 파일**에 있으면 허브 코드 접근이 전부 파일 경계를 넘어 pub화를 강제한다.
>
> `pane` test는 pane 코드와 허브 코드를 **둘 다** 만지므로, 어느 쪽에 두든 한쪽은 경계를 넘는다.
> 그리고 **허브에 두는 쪽이 항상 싸다.** 따라서 §2-c-3의 결론(분해 시 test는 원본에 남긴다)은
> `core.zig` 선례와도, 이 실측과도 일치한다. 시범 이동은 되돌렸다.
>
> 부수로 얻은 것: 이 실측 과정에서 `test_support.zig` 분리가 나왔고(위), 그것만으로 705줄이 줄었다.
> test 이동 비용도 70 → 16으로 줄어 있지만, **그 16을 치를 이유가 없다는 것이 이 실측의 결론**이다.
...
> 헬퍼를 부르니 pub이 못 닫힌다"고 적었으나, `pane` test 101개를 실제로 `pane.zig`로 옮겨 보니
> pub이 **+35 늘고 0개 닫혔다**(위 절). 원인은 더 단순하다 — **파일을 가르면 경계를 넘는 참조가
> 생기고, Zig의 privacy는 파일 경계 단위다.** 그룹 파일이 존재하는 것 자체의 비용이지 test 배치
> 문제가 아니다.
>
> 이것을 감추지 않고 남긴다 — 이후 누가 "분해했는데 왜 캡슐화가 나아지지 않았나"를 물으면 답은
> **"줄 수는 목표였고 pub 표면은 목표가 아니었다"**이다. pub까지 줄이려면 파일을 가르는 대신
> 도메인 상태를 별도 struct로 캡슐화해야 하고(그러면 필드 접근이 메서드가 된다), 그건 F 시리즈와
> 다른 작업이다.
