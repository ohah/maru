...
6. **(§9.6) 헤더·git chrome 아이콘 = 빌드타임 SVG→coverage 합성.** 폰트에 없는 단색 아이콘(설정 ⚙·검색 🔍·알림 🔔·새 워크스페이스 +·접기 ◧·브랜치 줄 GitHub octocat(mark-github, 0xF0009)·폴더 줄 file-directory-fill(folder, 0xF000A))을 SVG→coverage 마스터로 만들고(개발 시 `tools/svg_to_coverage.py` + `rsvg-convert`[librsvg] → PIL alpha → `src/renderer/icon_coverage_data.zig`로 **커밋**), `renderer/icon_glyph.zig`가 `synthesizeGlyph` 경로(box/braille 합성과 동형)로 슬롯 크기에 area-average 다운스케일해 그린다. codepoint는 **Plane 15 PUA(0xF0000~0xF00FF)** — Zig `isIcon`과 `coretext_smoke.m`의 C 게이트(`maru_is_synthesized_glyph`)가 **동일 범위**여야 한다(어긋나면 그 글리프가 blank). 단색이라 셰이더가 coverage×전경색으로 칠해 **테마색 자동**, 합성이라 slot-stretch blur가 없고, **빌드·CI·런타임 의존성 0**(커밋된 데이터만 — `nerd_font_attributes`와 같은 생성-파일 패턴). 아이콘 폰트+PUA 폰트 라우팅 인프라를 새로 만드는 대신 기존 합성 경로를 재사용한 결정(maru "런타임 의존성 0·네이티브 UI 비사용" 정합). **주의(Nerd Fonts v3 겹침)**: Nerd Fonts v3가 Material Design Icons를 Plane-15 PUA(U+F0001~)로 옮겨 이 범위와 겹친다 — synthesizeGlyph가 전역이라 터미널 콘텐츠가 0xF0000~0xF00FF의 MDI 글리프를 내보내면 폰트 글리프 대신 maru 아이콘(등록됨)/빈 글리프(미등록)가 그려진다(알려진 트레이드오프; 그리드 라우팅을 등록-전용으로 좁히려면 C 게이트가 Zig 등록 목록을 알아야 함 — 후속). 카드 보조줄 아이콘(브랜치 줄 octocat·폴더 줄 folder)은 `coretext_frame_builder.titleCellWidth`가 **width-2(2칸, ~16px)**로 렌더한다 — 1칸(~8px)이면 octocat이 동그란 링처럼 뭉개졌다(사용자 피드백 "깃 아이콘이 너무 작다"). 에이전트 gutter 아이콘(별도 셀 width 2)과 같은 크기로 통일. 폭 확대는 **등록된 아이콘만**(`icon_glyph.isRegisteredIcon`, 범위 전체 isIcon이 아니라) — 위 겹침 때문에 신뢰 불가 OSC 0/2 제목에 미등록 MDI 글리프가 와도 폭을 안 키운다. 헤더 줄0(◧⚙+🔔)은 §7대로 목표 px 1.7×. 브랜치 줄이 **GitHub octocat(0xF0009)**을 쓰는 이유 — 예전 git-branch(0xF0001, 얇은 선+링 3개)는 카드 셀 크기(~8~12px)로 다운스케일되면 내부 구조가 뭉개져 ├(U+251C)처럼 보였고(사용자 피드백), 꽉 찬 단색 실루엣 octocat은 작은 크기에서도 외곽이 살아 GitHub 마크로 읽힌다. git-branch 마스터는 데이터에 남겨 두되(0xF0001) 현재 미사용.
