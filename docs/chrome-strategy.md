...
> **구현 이력:** 다음 C/U 단계 서술과 날짜·완료·후속 표기는 chrome 구조가 형성된 판단 순서를 보존하는 연대기이며 현재 진행 상태가 아니다. 현재 단계와 gate는 [Metal UI 구현 계획](plans/metal-ui-layout.md), [chrome polish 계획](plans/chrome-polish.md), [검증 매트릭스](verification-matrix.md)가 소유한다.
>
...
`chrome/components/confirm.zig`는 **재사용 가능한 예/아니오 확인 다이얼로그** 컴포넌트다 — host가 메시지 + 버튼 라벨을 주입하면(`show(message, .{ .confirm = "닫기", .cancel = "취소" })`) 경계선 패널 + accent 기본 버튼 + 보조 버튼 + 키 안내(Enter/Esc)를 그린다. 닫기 확인뿐 아니라 삭제·저장 등 어떤 확인에도 쓴다(컴포넌트는 용도를 모르고 host가 라벨·의미를 정함 — 경계). 의도는 notice의 1개(dismissed)가 아니라 **최대 4개**(`confirmed`/`alternate`/`extra`/`cancelled`): host가 confirmed면 보류한 동작을 실행, cancelled면 버린다(예: 실행 중 명령이 있는 터미널/창 닫기 — [macos-app-host-boundary.md](macos-app-host-boundary.md) "닫기 확인"). 라우팅 우선순위는 **최우선**(파괴적 게이트라 notice보다 앞).

⚠️ **버튼 자리는 최대 넷이고 `cancel` 은 자리이면서 «Esc·바깥 클릭»이다.** `show`(둘) · `showChoices`(셋, `alternate` 추가) · `showChoices` + `extra`(넷)로 늘어난다. 마커는 자리 순으로 `[Y]`·`[D]`·`[R]`·`[N]` 이고 **뜻이 아니라 자리**다(←/→ 순회는 없는 자리를 건너뛴다). **세 번째 «행동»이 필요하면 `extra` 를 쓴다 — `cancel` 에 놓으면 Esc 가 그 행동을 실행한다.** 그리고 **`primary` 에는 아무것도 버리지 않는 선택만 둔다**: 상자는 열 때 `primary` 에 포커스를 두므로 Enter 가 그것을 실행한다(저장 충돌의 「비교」가 그 자리에 선 이유 — [editor-surface.md](editor-surface.md) §4).
...
**`below_clamp` 을 그림으로 묻는 자리는 `dropdown-bottom-clamp` 하나다**(2026-09-15). ⚠️ **점 앵커에서는 `below_clamp` 와 `at_anchor` 가 수학적으로 같다**(`popup_box` A44) — 그래서 평범한 캡처로는 두 정책이 구분되지 않는다. 앵커에 **두께가 있고**(control 한 줄) 아래 공간이 목록 높이에 모자랄 때만 갈린다. 기존 컨텍스트 메뉴 캡처는 전부 점 앵커라 그 조건을 만들지 못했고, 실제로 `below_clamp` 를 `below_flip_up` 으로 바꾸는 뮤테이션은 **그 시나리오만** 빨갛게 한다(1,446 px — 같은 뮤테이션에서 `dropdown-open` 은 통과한다).

⚠️ **드롭다운 골든이 못 보는 것 둘.** Lab 의 lowering 은 `.quad` 만 내리고 드롭다운은 행 칠을 `.fill` 로 내므로, **상자 폭**(`@max(box_w, anchor.w)`)과 **선택 강조**는 그림에 아예 없다 — 둘 다 뮤테이션으로 실측해 확인했다(지우거나 옮겨도 골든이 통과한다). 그 계약들은 `dropdown.zig` 의 `popupRect`·`viewPopup` 판정자가 지킨다. **랩의 한계이지 제품 결함이 아니다.**
...
- **draw 출력 모델은 둘이다 — 고정 슬라이스에 쓰는 컴포넌트는 몫을 방출 코드가 센다**(2026-09-24 사용자 결정).
  - **호출자 arena 의 목록에 덧붙이기**(`out.append(arena, …)`) — chrome 컴포넌트의 **다수**(context_menu·palette·sidebar·find·dropdown·modal_box·settings 등 20개)가 이렇다. 개수가 얼마든 목록이 늘어나므로 **셀 필요가 없다.** 새 컴포넌트는 이유가 없으면 이쪽을 쓴다.
  - **호출자가 준 고정 슬라이스에 쓰기** — 목록형 도크(`file_tree`·`scm_dock`·`session_dock`·`archive_detail`)와 편집기(`editor_view`). 이 모델에서만 아래 규칙이 필수다.

  호출자가 준 고정 저장소(op·run·글자 바이트)에 쓰는 컴포넌트는, 그 저장소가 얼마나 필요한지를 **방출 코드와 같은 파일**의 함수(`bufferSizes`)로 답한다. 값은 추정 상수가 아니라 **입력이나 기하에서 유도한 상한**이고, 호출자는 그 값만큼 잡는다(프레임 arena든 프레임을 넘어 유지하는 버퍼든).
  - **답하는 방식은 방출의 모양이 정한다 — 둘이다.**
    - **입력에서 센다** — 행 하나가 **고정 개수**를 내는 목록(`file_tree`·`scm_dock`: "행마다 op 5개 + 라벨 길이"). 공식이 방출과 거의 같은 모양이라 낡기 어렵다.
    - **기하에서 묶는다** — 개수가 **내용에 따라** 정해지는 방출(편집기 본문의 run은 구문 토큰 수를 따른다). 이때 입력에서 세는 공식은 "어디서 가르는가"를 한 번 더 적은 **사본**이 되어, 가르는 규칙이 늘 때마다(괄호 쌍 색·공백 표시처럼) 조용히 낡는다. 대신 **개수를 셀 수(행 × 열)로 묶는 불변식**이 있으면 그것으로 답한다 — 가르는 규칙과 무관해서 사본이 없다. 선례: Neovim UI 프로토콜은 격자(`grid_resize`)와 **셀마다** 강조 id(`grid_line`)를 주고, xterm.js WebGL은 `cols × rows × 셀당 고정 개수`, Ghostty는 행마다 `열 × 3`을 격자로 미리 잡는다.
