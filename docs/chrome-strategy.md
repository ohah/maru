...
- **draw 저장소의 몫은 방출하는 컴포넌트가 센다 — 필수**(2026-09-24 사용자 결정). 호출자가 준 저장소(op·run·글자 바이트)에 쓰는 컴포넌트는, 그 저장소가 얼마나 필요한지를 **방출 코드와 같은 파일**의 함수(`bufferSizes`)로 답한다. 값은 추정 상수가 아니라 **입력이나 기하에서 유도한 상한**이고, 호출자는 그 값만큼 잡는다(프레임 arena든 프레임을 넘어 유지하는 버퍼든).
