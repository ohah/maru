...
커맨드 팝업(Cmd+Shift+P) **"Reset All Settings to Defaults"**(action `reset_settings`)와 메뉴 **"Reset to Defaults"**(ABI `reset_defaults`)는 **같은 동작**이다. 둘 다 `requestResetAll` 확인 뒤 config를 내장 기본값으로 되돌린다. **현재 구현**은 `session.keep-alive-after-quit` 값을 메모리에서 보존하지만 현재 기본값(`false`)과 다를 때만 override를 다시 쓰므로 explicit `false`의 provenance는 사라진다. release A의 G2는 이 키를 PTY 소유권 예외로 바꿔 값과 explicit override를 기본값과 같아도 항상 보존한다. 별도 persisted migration marker나 만료 정책은 두지 않고, G2 이후에는 사용자가 Workspace 토글을 직접 바꿀 때만 explicit true/false를 교체한다. `resetAllSettings`은 ① session 값을 보존한 뒤 `loaded_config.config = Config{}`로 갈고 값을 복원하고 ② `reapplyLoadedConfig`한 뒤 ③ config 파일을 안내 주석 + session override만 남긴 기본 상태로 덮어쓴다. schema 키만 부분 초기화하면 비-schema 키(`theme.preset`·`palette.N`·`env.*`·`cursor.color`·`shell.args`)가 남으므로 나머지는 전체 덮어쓰기를 유지한다. `config_dirty_keys`를 비워 Swift 부분 write-back이 끼지 않게 하고, 이후 변경은 `serializeConfig`가 채운다. 파일 덮어쓰기는 파괴적이므로 메뉴·팝업 모두 확인 모달을 사용한다.
...
- **리셋 = override 줄 제거(값 덮어쓰기 아님)**: 현재 일반 항목은 기본값 setter 뒤 `markConfigKeyRemoved(key)`로 override를 삭제한다. release A G2부터 `session.keep-alive-after-quit`만 이 규칙의 소유권 예외가 된다.
...
- **행 종류별 리셋 규칙**:
  - `session.keep-alive-after-quit` → **G2 계획(미구현)**: 값과 explicit override를 바꾸지 않고 "Workspace 토글을 직접 변경" notice. `reset_field`/Backspace 효과 0. G2 전에는 generic row reset이 override를 제거하므로 release B gate로 인정하지 않는다.
