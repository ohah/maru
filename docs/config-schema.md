...
| **CS-3** ✅ | **doc-drift 가드** — 모든 스키마 키가 `configuration.md` 키 표에 행으로 문서화됐는지 `@embedFile`로 컴파일 타임 검증(`schema.zig` 테스트). 표 *통째 생성*은 풍부한 비고 주석을 잃으므로 안 함 — 대신 **누락을 CI가 잡는** 가드로(주석 표 유지 + drift 차단). build.zig가 `configuration.md`를 익명 import로 등록 | ✅ 머지. 가드가 미문서 4키(chrome.theme·text.blink·notifications.agent-complete·shell-integration.ssh) 발견·문서화. 전체 마커-블록 생성(주석을 meta.doc로 이전)은 별도 CS-3b(필요 시) |
