...
- **편집기 전용 action**: `toggle_editor_wrap`(그 뷰의 줄 바꿈 토글)·`fold_all`·`unfold_all`·
  ...
  `transform_to_uppercase`·`transform_to_lowercase`·`add_next_occurrence`·`jump_to_bracket`·`add_cursor_above`·`add_cursor_below`·
  ...
  `undo_workspace_edit`(기본 chord 없음 — 팔레트 「Editor: Undo Last Rename」; 마지막 rename 의 모든 파일이 그대로일 때만 되돌린다) — **줄 조작
...
- **메뉴 keyEquivalent 층을 지나야 산다.** 이 다섯은 팔레트 카탈로그에만 있고 메뉴 항목이 아니므로
  ...
  **「Option 단독」 부류는 편집기 Term 컨텍스트가 푼다**([키 입력과 단축키](key-input-and-shortcuts.md)
  ...
  `move_lines_up`/`_down` 은 **편집기 컨텍스트 기본키**로 설 수 있다. **접기 다섯도 섰다**(2026-09-07 — 위 「접기 다섯의
  chord」). 이 줄은 한동안 *"여전히 못 선다"* 로 남아 **같은 문서 안에서 갈려 있었다**(위 표는
  이미 「풀렸다」로 적고 있었다) — 2026-09-08 재실측이 고쳤다.
