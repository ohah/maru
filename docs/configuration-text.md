...
적용 대상은 **두 가지**다 — 둘은 서로 다른 축이라 나눠 읽어야 한다.

1. **폭 승격**: VS16 이모지 표현(❤️)과 키캡(2️⃣)의 base 를 폭 2 로 올린다.
2. **cluster 묶기**: 스킨톤(👍🏽)·국기(🇰🇷)·ZWJ 시퀀스(👨‍👩‍👧‍👦)를 **한 셀**로 묶는다. `wide` 면 mode 2027 합의
   없이도 묶고, `narrow` 면 2027 에서만 묶는다.

②가 없으면 같은 설정에서 ❤️ 는 2칸인데 👨‍👩‍👧‍👦 는 구성 이모지마다 셀을 먹어 **10칸**이 된다(2026-08-20 실측:
`|👍🏽|`=3.8칸 · `|👨‍👩‍👧‍👦|`=10.4칸, 같은 줄의 `|❤️|`·`|🇰🇷|`·`|人|`은 2칸). 그 상태로 Claude Code 같은 TUI 를
띄우면 표 테두리가 그만큼 밀린다 — "이모지를 항상 2칸으로 본다"는 `wide` 의 약속과 어긋나므로 둘을 같은
게이트에 둔다.

> 베이스/결정: **두 축의 방향이 다르다.**
>
> **cluster 묶기(②)는 레퍼런스와 같은 쪽이다.** Ghostty 는 `grapheme-width-method` 가 **기본 `unicode`**
> (`Config.zig`)라 mode 2027 없이도 grapheme cluster 단위로 센다. maru 도 `wide` 에서 같게 맞춘다.
>
> **폭 승격(①)에서는 레퍼런스와 반대로 간다.** 소스로 확인한 바로는 Ghostty 도 xterm.js 도 **합의 전에는
