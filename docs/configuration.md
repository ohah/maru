...
# theme.min-contrast = 3.0   # 라이트 배경 가독성 하한(WCAG 명암비) — ANSI 팔레트 + 256색·truecolor 전경. 0=끔. 다크는 무동작
...
| `theme.min-contrast` | 실수(0.0~21.0) | `3.0` | **ANSI 팔레트 자동 대비 게이트** — ANSI 16색(0~15)이 배경 대비 이 명암비(WCAG contrast ratio)에 못 미치면 **색상(hue)은 보존한 채 검은색 방향으로 최소한만 어둡게** 보정해, 밝은 배경에서 안 읽히는 색(밝은 노랑·초록·흰색 등)을 읽히게 한다. 프리셋·`theme.palette.N`·프리셋 없이 배경만 밝힌 경우의 xterm 기본색까지 모두 적용된다(**explicit `theme.palette.N`도 포함** — 프리셋과 같은 필드라 분리 불가; 원색 그대로 원하면 `0`으로 끈다). **충분히 어두운 배경(모든 번들 다크 프리셋)에선 자동 무동작** — 검정으로도 목표 대비에 못 미치면 원본을 유지하며, 그 경계는 배경 luminance ≈ 0.05×(목표−1)(기본 3.0이면 ≈0.10; 다크 프리셋 배경은 `#101010`≈0.005 등으로 훨씬 낮다). 중간 밝기(회색) 배경은 목표 도달이 가능하면 보정될 수 있다(그 배경에서도 약한 색을 읽히게 함). `0`(또는 `1` 이하)=끔(업스트림 원색 그대로). 기본 `3.0`=WCAG 대형 텍스트/UI 컴포넌트 기준(테마 정체성 보존 + 안 보이는 색 교정), 더 강한 대비는 `4.5`(WCAG 일반 텍스트 AA)로 올린다. 적용 범위는 **두 겹**이다: ① **ANSI 16색 팔레트**는 config resolve 시점에 선보정되고 OSC 4 팔레트 질의 응답도 이 보정값을 보고한다(화면과 보고 일치). ② **256색(16~255)·truecolor 전경**은 프로그램이 escape로 직접 고르는 색이라 팔레트 선보정이 못 미치므로, **렌더가 셀 단위로 같은 하한을 강제**한다(자기 셀 배경 기준·reverse 반영 — Claude Code처럼 truecolor 밝은 회백색을 본문에 쓰는 프로그램이 라이트 테마에서 안 보이던 것을 교정). per-cell 보정은 렌더 전용이라 OSC 4/10/11 질의 응답엔 반영되지 않고, conceal(SGR 8)·blink-off(의도적 비표시)와 powerline/box/block 도형 글리프(이웃 셀과 색 이어붙임 이음매)는 제외한다. theme의 배경·전경(default)·커서·선택색은 안 바꾼다(그건 프리셋이 이미 조정). 범위 밖/비실수는 무시(기본 유지) |
...
>   마찬가지다. 이를 런타임에 읽히게 하는 안전장치가 [`theme.min-contrast`](#키)(기본 `3.0`)다 — 색이 배경
>   대비 그 명암비에 못 미치면 색상을 보존한 채 최소한만 어둡게 보정한다(ANSI 16색은 resolve 선보정, 그 밖은 렌더 per-cell). **프리셋 상수(파일의 원색 세트)는 손대지 않고
