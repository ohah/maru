...
| `editor.render-whitespace` | `none`·`boundary`·`selection`·`trailing`·`all` | `selection` | **공백 표시**([시각 매핑](native-editor-visual-mapping.md) §5.1e) — 공백은 `·`, 탭은 `→` 로 옅게 보인다. `selection` 은 선택한 글 안만, `boundary` 는 단어 사이의 홀로 선 공백을 뺀 전부, `trailing` 은 줄 끝 공백만. VS Code `editor.renderWhitespace` 와 같은 기본 |
