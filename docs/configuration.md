...
| `session.keep-alive-after-quit` | `true`\|`false` | `false` | **영속 터미널 세션**(실험적 opt-in) — GUI를 종료해도 터미널 runtime(PTY·자식·화면)을 별도 host 프로세스(`maru-sessiond`)에서 유지해 GUI 종료·크래시에도 살아남고 재실행 시 재접속하게 한다(tmux식, `docs/persistent-session-host.md` §10). **기본 `false`**(현행 — 터미널이 GUI 프로세스 소유라 종료 시 함께 죽음). `true`면 새 Workspace/Term/split/quick terminal을 host에 생성한다. **아직 미완성 경로라 opt-in**(P4 종료 gate 완성 전엔 기본값을 바꾸지 않는다). host 연결 실패 시 조용히 in-process로 폴백. 설정 GUI(**workspace** 섹션 토글)에서 켜고 끌 수 있다(config 파일로도 편집 가능). 토글은 **재실행 시 적용**된다(backend 셋업이 app init에서 일어나므로 실행 중 기존 터미널은 소급 변경 안 됨) |
