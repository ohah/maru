...
| `sidebar.agent-hooks` | `true`\|`false` | `false` | provider 훅(claude `settings.json`, codex `hooks.json`)을 설치해 에이전트 **상태·알림·턴 변경분**을 훅에서 받는다 — 켜면 그 터미널은 화면·OSC를 읽는 관측 모드를 쓰지 않는다([agent-hooks.md](agent-hooks.md) §1: 두 모드는 섞이지 않는다). **끔은 «설치하지 않음»이지 «제거»가 아니다** — 인스턴스 둘이 게이트 값을 다르게 가지면 설치·삭제가 무한히 왕복하기 때문이다(§5). 제거는 명시적 사용자 액션에서만 하는데 **그 액션이 아직 없다** — 지금 되돌리려면 `settings.json`에서 `MARU_HOOK_V3` 표식이 붙은 항목을 손으로 지운다. 우리가 넣은 항목은 커맨드 안의 표식으로 식별되고, 표식이 없는 사용자 항목은 순서까지 보존한다. 위 `sidebar.agent-transcript-hook`(상태줄 훅)과는 별개 키이고, 이 키를 켜도 저 키가 자동으로 꺼지지는 않는다 |
