...
| `theme.follow-system` | `true`\|`false` | `false` | **시스템 라이트/다크 외관을 따라 테마 색을 자동 전환**한다. `true`면 macOS가 라이트면 `theme.preset-light`, 다크면 `theme.preset-dark` 색 세트로 라이브 교체하고 외관이 바뀌면 즉시 따라간다. **켜져 있는 동안 `theme.preset`·개별 `theme.*` 색은 무시**되고 시스템이 색을 정한다(끄면 파일의 그 값으로 복귀). 시스템 외관 교체는 config 파일에 영속하지 않는다. 기본 `false`(현행 — 수동 테마) 전환 순간 `CSI ? 2031 h` 로 구독한 앱(Claude Code 등)에 `CSI ? 997 ; 1\|2 n` 색 구성 통지를 보내 앱이 테마를 따라온다(DECSET 2031 · `CSI ? 996 n` 질의) |
...
| `theme.syntax.keyword`·`theme.syntax.string`·`theme.syntax.number`·`theme.syntax.comment`·`theme.syntax.property`·`theme.syntax.type`·`theme.syntax.function`·`theme.syntax.punctuation`·`theme.syntax.tag`·`theme.syntax.attribute`·`theme.syntax.invalid` | `#RRGGBB` | (파생) | **구문 강조 색을 역할별로 직접 정한다**(편집기·diff·파일 패널 편집기가 함께 쓴다). 안 정한 역할은 터미널 팔레트에서 파생한다 — keyword=밝은 magenta, attribute=magenta(ANSI 5), string=밝은 green, number=밝은 yellow, function=밝은 blue, property=white(ANSI 7), type=밝은 cyan, tag·invalid=밝은 red, comment·punctuation=전경색을 배경 쪽으로 흐린 색(property·attribute 는 2026-09-21 에 function·keyword 와 갈렸다 — visual-mapping §5.3). 그래서 **아무것도 안 정하면 지금과 같다**. **명시한 색은 대비 보정을 받지 않는다**(`theme.background`·`theme.foreground`와 같은 규율 — 보정은 프로그램이 고르는 ANSI 팔레트에만 건다). `theme.follow-system`이 켜져 있으면 다른 개별 `theme.*` 색과 함께 무시되고, `theme.preset`이 **뒤 줄**에 오면 지워진다(순차 적용). 형식 오류 색·모르는 역할 이름은 무시(그 역할은 파생 유지) |
...
| `session.keep-alive-after-quit` | `true`\|`false` | `true` | **영속 터미널 세션**(기본 켜짐) — 정상 GUI Quit 뒤에도 일반 Window의 terminal runtime을 `maru-sessiond`가 유지하고 재실행 시 같은 `host_id:runtime_id`에 재접속한다([전체 P4 종료 gate](persistent-session-host.md#p4--일반-window-default-readinessbackground-알림)). **기본값은 `true`다**(2026-09-03 전환). 끄려면 Workspace 설정 토글을 직접 바꾼다. 새 일반 Workspace/Term/split부터 토글 값을 적용하고 열린 Term은 process 사이로 이동하지 않는다. quick은 항상 in-process이고 Quit 때 종료된다. 현재 host 실패는 one-shot notice 뒤 local fallback하고 handle을 쓰지 않으며, P4 전에 persistent `not preserved` 상태를 추가한다. Workspace 설정에서 편집하고 모든 Window가 앱 전역 snapshot을 공유한다. global Reset은 `absent`를 absent로 유지하고 explicit valid/invalid intent는 기본값과 같아도 canonical bool override로 보존하며, row Reset/Backspace는 값·override·파일을 바꾸지 않고 Workspace 토글을 직접 바꾸라는 notice를 낸다. 사용자가 토글을 직접 바꿀 때만 true/false를 교체한다. 전환은 **첫 릴리스 전**에 했으므로 G3 이관 계약(materialize·exact predecessor 고정·롤백 왕복)은 **타지 않았다** — 그 계약은 「기본값이 바뀌기 전에 설치한 사용자」를 옮기기 위한 것인데 릴리스가 0개라 그런 사용자가 없었다. **출하 뒤에** 이 값을 다시 바꾼다면 그때는 G3 계약이 단일 출처다. 업데이트 호환성의 A runtime→B exact adapter attach는 이 기본값 전환과 별도로 검증한다. 외부 CLI(P5)와 자동 migration(U5) 전체는 opt-in 사용의 선결이 아니고 나머지 조건은 링크된 P4 gate가 단일 출처다 |
...
| `browser.engine` | `webkit`\|`chromium` | `webkit` | 웹 브라우저 탭 엔진. `chromium` 은 따로 설치하는 Chromium 엔진(`brew install maru-chromium`)이 있을 때만 켜지고, 없으면 안내 뒤 WebKit 으로 연다. **maru 를 다시 시작해야 적용된다**(열린 탭이 엔진을 옮기며 로그인·입력을 잃지 않게 — 바꾸면 안내한다). 마크다운·파일 패널은 늘 WebKit 이다. 개발용 환경변수 `MARU_WEB_OSR_DIR`(sidecar 빌드 디렉터리)가 있으면 그것이 먼저다 |
...
| `editor.wrap` | `true`\|`false` | `false` | 네이티브 파일 편집기에서 본문 폭을 넘는 줄을 **다음 시각 행으로 접을지**([세로 축 매핑](native-editor-visual-mapping.md) §4). 접힌 행에는 줄 번호를 비운다(VSCode 관례). **기본 `false`** — 한동안 `true`였고 이유는 하나였다: 가로 스크롤이 없어 끄면 본문 폭에서 잘린 뒤를 볼 수단이 없었다. 그 조건이 해소돼(단일 편집기·비교 뷰 **양쪽**에 가로 스크롤이 붙었다) 2026-08-16 에 방침대로 되돌렸다. 덤으로 랩+비교의 좌우 어긋남이 기본 상태에서 사라진다(Vim 이 diff 에서 `nowrap` 을 전제하는 것과 같은 자리). 터미널에는 영향이 없다(그쪽 줄바꿈은 PTY 와 앱이 정한다). **설정 GUI 에도 뜬다** — 값이 렌더에 닿는 경로가 서면서 `hidden` 을 벗겼다 |
| `editor.max-columns` | `0`~`1000000` | `100000` | 네이티브 편집기에서 **가로로 볼 수 있는 최대 열**([설정](native-editor-ui.md) §9 · [시각 매핑](native-editor-visual-mapping.md) §4.1c). `0` = 무제한. 가장 긴 줄을 셀 때 여기서 멈추고 가로 위치의 상한이 `max_cols - 보이는 열`이므로 **이 값이 곧 갈 수 있는 끝**이다 — 그 너머 글자는 안 그려지고, 걸리면 **상태바가 알린다**(조용히 안 움직이면 버그로 읽힌다). **선례**: VSCode·Monaco 의 `editor.stopRenderingLineAfter`(기본 10,000)가 같은 자리이고 그쪽도 *"Rendering paused for long line for performance reasons"* 로 알린다(`-1` 이면 안 멈춘다 — 우리 `u32` 에서는 `0` 이 그 자리다). **기본 100,000 의 근거**: 이 저장소에서 편집기가 열 수 있는(UTF-8) 파일의 가장 긴 줄이 33,061자라 그 세 배이고 VSCode 기본의 열 배다. **비용이 이 값에 선형이다** — 여는 경로가 줄마다 여기까지 세고, 가로로 밀면 줄마다 여기까지 체크포인트를 세운다(실측: 560만 열 한 줄에서 각각 약 71ms 이므로 10만이면 1~2ms). 바꾸면 열려 있는 편집기에 **즉시 반영**된다 |
...
| `editor.scroll-beyond-last-column` | `0`~`64` | `5` | 네이티브 파일 편집기에서 **가장 긴 줄 너머로 몇 열까지 더 밀 수 있는가**([시각 매핑](native-editor-visual-mapping.md) §4 「가로도 caret 을 따라간다」). **왜 필요한가**: caret 은 마지막 글자보다 **한 칸 뒤**에 서므로, 상한이 내용 폭이면 가장 긴 줄 끝에서 `⌘→` 를 눌렀을 때 글자 끝은 보이는데 **커서만 안 그려진다**. `0` 이면 그 옛 동작 그대로다. **기본 5 는 VSCode `editor.scrollBeyondLastColumn` 과 같은 값·같은 뜻**이고, 불리언이 아니라 열 수인 것도 그쪽을 따랐다 — 그쪽도 이 값을 가장 긴 줄 폭에 **더해** scroll width 를 만들므로 caret 뿐 아니라 **휠·가로 막대도** 그만큼 더 간다. 그래서 가장 긴 줄이 화면 폭에 가까우면(폭 − 이 값 이내) 이제 가로 막대가 뜬다. **랩(`editor.wrap = true`)이면 뜻이 없다** — 그때는 가로 축 자체가 없다. 비교 뷰는 좌우가 **각자** 자기 상한에 더한다. **상한 64 의 근거**: 좁게 뒀다 넓히는 것은 안전하고 그 반대는 남의 설정을 깨므로 되돌리기 쉬운 쪽으로 잡았다 |
| `editor.cursor-surrounding-lines` | `0`~`64` | `5` | 이동으로 caret 이 옮겨진 뒤 뷰가 따라갈 때 **caret 위·아래로 남겨 두는 줄 수**([시각 매핑](native-editor-visual-mapping.md) §4 「caret 여백」). `0` 이면 가장자리에 딱 붙는 옛 동작이다. **기본 5 의 근거는 관례가 그 값으로 수렴한다는 것**이다 — Vim `scrolloff` 는 표면상 `0` 이지만 **`defaults.vim` 이 5 로 켜므로** vimrc 없는 사용자가 실제로 받는 값이 5 이고, Helix `scrolloff` 도 5 다(Zed `vertical_scroll_margin` 은 3, VSCode `editor.cursorSurroundingLines` 와 Emacs `scroll-margin` 은 0). **걸리는 자리는 키보드 이동 뒤 노출뿐**이고 휠·막대 드래그는 안 건드린다 — VSCode 의 `cursorSurroundingLinesStyle` 기본값 `"default"` 와 Zed 의 *"with the keyboard"* 가 같은 선택이다. **화면 절반을 넘는 값은 절반으로 묶인다**(그래서 큰 값은 「늘 가운데」가 되며, Vim 에서 큰 `scrolloff` 가 그렇게 동작한다). 문서 끝에서는 여백 때문에 **문서 밖을 보여 주지 않는다**. 비교 뷰도 같은 값을 쓴다(좌우가 함께 구른다) |
| `editor.cursor-surrounding-columns` | `0`~`64` | `0` | 세로의 짝 — **caret 좌·우로 남겨 두는 열 수**. **기본이 `0` 인 이유는 「선례가 없다」이다**: 가로 여백을 가진 것은 Zed `horizontal_scroll_margin`(5) 하나인데 **그 문서가 「마우스 스크롤용」이라고 적어** 우리가 여는 자리(키보드 이동 뒤 노출)와 축이 다르고, Vim `sidescrolloff` 는 0 이며 VSCode·Emacs·Sublime 에는 그 설정이 아예 없다. 그래서 **값을 지어내지 않고 자리만 연다** — `0` 이면 이 설정이 생기기 전과 동작이 **완전히 같다**. **랩(`editor.wrap = true`)이면 뜻이 없다**(가로 축 자체가 없다). 비교 뷰는 §3.5 *"가로는 각자다"* 그대로 **caret 이 선 열만** 민다 |
| `editor.minimap` | `true`\|`false` | `true` | 네이티브 파일 편집기의 **미니맵**([시각 매핑](native-editor-visual-mapping.md) §6.1) — 본문 오른쪽·세로 막대 왼쪽에 문서의 «모양» 스트립(줄 하나 2px, 글자 하나 1px 의 색 블록)을 그린다. 문서가 스트립보다 길면 본문을 따라 **비례**로 굴러가고, 보이는 구간이 반투명 슬라이더로 선다. 클릭·드래그하면 그 줄이 화면 **가운데** 오게 굴린다. **기본 켬은 VS Code `minimap.enabled` 와 같다.** 비교 뷰·병합 화면에는 아직 없다(N5b). 바꾸면 열려 있는 편집기에 **즉시 반영**된다(본문 폭이 그만큼 늘거나 준다) |
| `editor.minimap-width` | `4`~`60` | `15` | 미니맵의 폭(**셀** — gutter·본문이 셀 격자라 px 로 두면 본문 끝이 셀 경계에서 안 끝난다). 기본 15 셀 ≈ 120px(8px 셀) 로 VS Code 의 기본 폭 120px 과 같다. 본문이 **40 열보다 좁아지면** 미니맵이 접혀 0px 이 된다(좁은 분할에서 본문보다 미니맵이 넓은 화면은 뜻이 없다). `editor.minimap = false` 면 뜻이 없다 |
| `editor.sticky-scroll` | `true`·`false` | `true` | **sticky scroll**([시각 매핑](native-editor-visual-mapping.md) §4.1i) — 스크롤하면 지금 화면 맨 위 줄이 속한 함수·클래스·블록의 머리줄을 본문 위쪽에 고정한다(바깥부터). 스코프는 심볼(언어 서버 → tree-sitter)에서, 없으면 접힘 범위에서 온다. VS Code `editor.stickyScroll.enabled` 와 같은 기본(켬) |
| `editor.sticky-scroll-max-lines` | `1`~`10` | `5` | 고정할 줄의 상한. 편집기 높이의 25% 도 넘지 않는다(VS Code `editor.stickyScroll.maxLineCount` 와 같은 기본) |
| `editor.render-line-highlight` | `none`\|`gutter`\|`line`\|`all` | `line` | **현재 줄 강조**([시각 매핑](native-editor-visual-mapping.md) §5.1b) — 커서가 선 행을 2px 테두리 상자로 두른다(VS Code 기본 테마의 모양). `line` 은 본문(선택이 전부 비었을 때만), `gutter` 는 줄 번호 칸(선택과 무관), `all` 은 둘 다. 커서가 여럿이면 전부. 줄 번호는 이 값과 무관하게 primary 커서의 줄이 더 밝다. VS Code `editor.renderLineHighlight` 와 같은 값·기본 |
| `editor.match-brackets` | `never`\|`near`\|`always` | `always` | **짝 괄호 강조**(§5.1b) — caret 에 닿은 괄호와 그 짝을 테두리 상자로 두른다(둘 다 닿으면 뒤의 괄호). `always` 는 닿은 괄호가 없으면 caret 을 품는 가장 안쪽 쌍을 두른다. 구문 트리가 있으면 문자열·주석 속 괄호는 세지 않는다. 포커스가 있는 편집기만. VS Code `editor.matchBrackets` 와 같은 값·기본 |
| `editor.guides-indentation` | `true`·`false` | `true` | **들여쓰기 안내선**([시각 매핑](native-editor-visual-mapping.md) §5.1c) — 들여쓰기 단계마다 1px 세로선. 간격은 **파일에서 추정한 들여쓰기 폭**이다(2 칸 파일이면 2 열마다 — VS Code `detectIndentation` 과 같은 규칙; 탭 파일은 `editor.tab-width`). 추정은 선 간격에만 쓰고 `Tab` 키는 그대로다. 공백만인 줄은 위·아래 블록으로 잇는다. 랩된 줄은 첫 조각에만. VS Code `editor.guides.indentation` 과 같은 기본 |
| `editor.guides-highlight-active-indentation` | `true`·`false` | `true` | caret 이 든 블록의 안내선을 더 밝게(§5.1c). VS Code `editor.guides.highlightActiveIndentation` 과 같은 기본 |
| `editor.bracket-pair-colorization` | `true`·`false` | `true` | **괄호 쌍 색**([시각 매핑](native-editor-visual-mapping.md) §5.1d) — 짝 괄호를 바깥 쌍의 수(단계)마다 세 색으로 돌려 칠하고, 짝 없는 괄호는 빨갛게. 문자열·주석 속 괄호는 글자다. VS Code `editor.bracketPairColorization.enabled` 와 같은 기본 |
| `editor.bracket-pair-colorization-independent-pools` | `true`·`false` | `false` | 괄호 **종류마다** 단계를 따로 센다(`(` 안의 `[` 도 첫 단계 색). VS Code `editor.bracketPairColorization.independentColorPoolPerBracketType` 와 같은 기본 |
| `editor.diagnostics` | `true`·`false` | `true` | **진단 표시**([시각 매핑](native-editor-visual-mapping.md) §5.4) — 물결 밑줄·gutter 글리프(`✖`)·막대/미니맵 마커·`F8`/`⇧F8` 이동을 한꺼번에 켜고 끈다. 지금의 출처는 **구문 오류**(tree-sitter `ERROR`/`MISSING`)라 타이핑 중에도 밑줄이 뜬다(JetBrains 의 즉시 표시) — 거슬리면 끈다. LSP·린터가 오면 같은 표시를 쓴다 |
| `editor.hover` | `true`·`false` | `true` | **호버 박스**([툴링](editor-surface-tooling.md) §8.2b) — 편집기 본문에서 포인터가 낱말 위에 머물면 그 자리를 덮는 진단의 문장(구문 오류·서버 진단)과 언어 서버의 hover 를 낱말 아래 상자로 낸다. 끄면 포인터 트리거만 꺼지고 `show_hover` 명령은 남는다 |
| `editor.hover-delay` | `0`~`5000` | `300` | 호버가 뜨기까지 포인터가 머무는 시간(ms). VS Code·Zed 기본 300 |
| `editor.parameter-hints` | `true`·`false` | `true` | **시그니처 힌트**([툴링](editor-surface-tooling.md) §8.2d) — `(`·`,` 같은 서버의 트리거 글자를 치면 활성 시그니처(매개변수 강조·설명)를 caret 아래 상자로 낸다. 끄면 타이핑 트리거만 꺼지고 `trigger_parameter_hints`(`⇧⌘Space`)는 남는다 |
| `editor.quick-suggestions` | `true`·`false` | `true` | **자동완성 팝업의 타이핑 트리거**([툴링](editor-surface-tooling.md) §8.2g) — 식별자 글자를 치면 언어 서버의 완성 목록을 낱말 아래에 낸다(`↑↓` 고르기·`Enter`/`Tab` 확정·`Esc`). 끄면 서버 트리거 글자(`.`)와 `trigger_suggest`(`⌃Space`·`⌥Esc`)만 남는다(VS Code `editor.quickSuggestions`) |
| `lsp.enabled` | `true`·`false` | `true` | **언어 서버 사용**([툴링](editor-surface-tooling.md) §8.2a). 파일을 열면 언어에 맞는 서버(`zls`·`clangd`·`typescript-language-server`·`rust-analyzer`·`pyright-langserver`·`gopls`)를 PATH 에서 찾아 **워크스페이스마다 한 번 묻고**(허용/거부를 `~/.config/maru/lsp-trust` 에 기억) 실행한다. 없으면 상태바에 「‹서버› 없음 — 설치」가 뜨고 누르면 새 탭에 설치 명령을 입력만 한다(Enter 는 사용자). 서버의 진단은 `editor.diagnostics` 의 같은 자리에 선다. 끄면 찾지도 묻지도 않는다 |
