...
| `file-panel.max-live-views` | 정수(1~256) | `8` | 파일 도크에서 동시에 유지할 WKWebView 상한. **FP16에서 제거 예정** — 파일 패널이 워크스페이스 Term으로 옮겨가면서 LRU eviction 자체가 사라진다([file-panel.md](file-panel.md) §1). 제거 뒤 이 키는 loader의 forgiving 규칙대로 무시 + diagnostic이라 기존 config 파일은 깨지지 않는다. |
