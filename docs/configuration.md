...
| `sidebar.session-icon-gutter` | `true`\|`false` | `false` | 카드 하위 **세션 목록** 행의 종류 아이콘(✶ claude·◆ codex·web·파일)을 어디에 둘지. 기본(`false`)은 **이름줄 선두 인라인** — 아이콘이 첫 줄에만 자리를 먹고 폴더·브랜치·응답 줄은 전폭을 쓴다. `true`면 **왼쪽 독립 gutter**(옛 배치)로, 아이콘이 행 세로 중앙의 별도 열에 서고 그만큼 행의 **모든 줄**이 밀린다. 좁은 사이드바에서 gutter는 글리프 하나에 3칸(폭의 ~7%)을 내주므로 기본은 인라인이다. view options(⚙) 메뉴에서 토글·양방향 |
