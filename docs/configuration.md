...
| `chrome.tab-style` | `connected`\|`underline` | `connected` | 활성 탭 룩(직교 축). `connected`(기본)=터미널 본문색 cutout + 앰버 언더바(아래 본문과 이어짐), `underline`=언더바만(미니멀, 배경 박스 없음). `chrome.theme`(tui\|rich)·`theme.preset`(색)과 직교. rich 경로에서만 의미(tui는 셀 밴드). 자세히는 [Chrome 전략 §7](chrome-strategy.md) |
