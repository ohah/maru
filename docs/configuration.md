...
| `font.ligatures` | 불리언 | `false` | **프로그래밍 합자**(`=>`·`!=`·`//` 등). 켜면 폰트가 정의한 `liga`/`clig`/`calt`를 그대로 두고, 끄면 셋을 모두 꺼 글자 그대로 그린다. **기본 끔(2026-08-17 정정)** — 켠 상태의 렌더가 깨져 있다: JetBrains Mono 실측에서 `//`·`::`·`~~` 가 모두 **첫 칸에 빈 글리프(1742)**, 둘째 칸에 왼쪽으로 넘치는 전체 합자를 받는다. maru 의 래스터는 셀 단위라 그 오버행이 잘려 **첫 글자가 사라진 것처럼 보인다**(사용자 제보: "`//` 를 입력하면 하나만 보인다"). 다시 켜려면 글리프 오버행 렌더(셀보다 큰 비트맵을 atlas 에 담고 quad 를 셀 밖으로 확장)가 선행돼야 한다. 근거·실측 표는 `src/config/theme.zig` 의 이 필드 주석이 소유한다 |
