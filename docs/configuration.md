...
- **파일 변경 자동 감지 reload**: 파일 watcher로 변경을 감지해 자동 재-resolve(자동 감지만 후속). 메뉴의 수동 **Reload Config**(파일 재로드해 재시작 없이 적용)·**Reset to Defaults**(모든 config를 내장 기본값으로 되돌리고 config 파일을 삭제 — 커맨드 팝업 "Reset All Settings to Defaults"와 같은 통합 리셋에 위임)는 구현됨.
