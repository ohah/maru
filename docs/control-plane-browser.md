...
> **구현 이력 안내:** 이 문서에서 5a~5f 번호와 함께 쓰는 완료·후속·구현 상태 표기는 browser 코어와 CLI가 형성된 순서를 보존하는 연대기이며 현재 진행 상태가 아니다. 현재 범위와 남은 gate는 [검증 매트릭스](verification-matrix.md)가 소유한다.
