...
채널**을 열고 원격에서 `maru control --stdio`(가칭)를 실행한다. 그 프로세스가 로컬 컨트롤
...
- **`maru` 를 PATH 에서 못 찾는 것과 "없는 것" 은 다르다.** 비대화형 `exec` 은 로그인 셸의 rc 를
  ...
  사용자는 없는 문제를 고치러 간다. S10c 가 절대경로·힌트를 정한다.
- **프레임 상한과 채널 배압이 만나는 자리다.** wire 는 프레임당 ≈1MiB
  ([프로토콜](control-plane-protocol.md))이고 우리 채널 패킷은 32KiB·윈도 2MiB 다 — 큰 답은
  여러 패킷에 걸쳐 오므로 **중계도 모바일도 줄 단위로 이어 붙여야** 한다(패킷 경계 = 줄 경계가
  아니다).

### 우리 쪽에서 막히는 자리 (적대적 검증 3회차)

- **`exec` 이 아직 없다.** 우리 채널 코드는 `pty-req`·`shell` 만 낸다(`channel.zig` — `"exec"`
  문자열이 없다). 이 절의 그림 전체가 `exec` 위에 서 있으므로, **S10b 가 그것부터 만든다**.
  (`Options.pty = false` 로 pty 를 건너뛰고 shell 로 가는 길은 이미 있다 — `exec` 은 그 자리에
  들어간다.)
- **모바일 ABI 가 채널을 모른다.** `maru_mobile_ssh_feed/write/screen_ptr` 은 **핸들 하나**를
  받고 화면 바이트도 한 흐름이다 — 채널이 둘이 되면 "어느 채널의 바이트인가" 를 말할 수가
  없다. 그래서 S10b 는 **코어만이 아니라 ABI·펌프까지** 넓히는 일이다.
...
- **재접속하면 채널도 다시 열어야 한다.** 지금 요청 모델(`take_server_connect`)에는 채널 개념이
...
**시한 안에 `hello` 가 없으면 축을 끄고**, 원인은 아는 만큼만 말한다 — `exit-status` 가 `127`
이면 "maru 가 없다", 다른 코드면 "그 서버가 다른 프로그램을 실행했다", 아무것도 안 오면
"응답이 없다". **셋 다 터미널은 그대로 쓸 수 있다**고 함께 말한다.

**채널 둘은 독립이어야 한다 — 그런데 지금 코어는 그렇지 않다.** RFC 4254 는 채널을 독립으로
두지만, 우리 `client.zig` 는 **채널이 닫히면 세션을 닫는다**(`.closed => state = .closed`).
그대로 두면 중계 프로세스가 끝나는 순간 **터미널까지 죽는다** — 목록을 한 번 본 대가로 셸이
끊기는 셈이다. 그래서 S10b 는 다중 채널을 여는 일이면서 **"마지막 채널이 닫힐 때만 세션이
닫힌다" 로 그 규칙을 고치는 일**이다(적대적 검증이 이 자리를 잡았다).
