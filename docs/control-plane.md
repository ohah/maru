...
- **되돌리는 데는 시한이 있다 — 5초**(2026-09-03 개정). 예전에는 "닫힘을 기다리는 데 시한은
  없다(연결이 끊기면 축이 처음부터 다시 선다)" 였는데, 그러면 닫힘이 끝내 안 오는 동안 화면이
  **조용히** "받는 중" 에 머문다 — §4a 가 정한 「실패하면 그 화면에서 말한다」를 그 경우에만
  어긴다. 그래서 되돌리기를 무한히 하지 않고, 컨트롤 시한과 **같은 5초**를 넘기면 딱딱한 실패로
  접어 화면이 말하게 한다. 그 뒤의 회복은 사용자가 그 자리를 벗어났다 다시 들어오는 것이다
  (같은 것을 다시 원하면 아무 일도 안 일어난다는 위 규칙 때문이다).
