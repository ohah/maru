...
- **peer-cred 가 원격에서 그대로 성립한다.** 중계 프로세스는 **그 PC 에서 그 사용자로** 도는
  로컬 클라이언트라, §1 의 보안 모델이 깨지지 않는다. 소켓을 네트워크에 여는 것이 아니다.
...
낸다), 화면은 **"이 서버에는 maru 가 없다 — 터미널만 쓸 수 있다"** 고 말한다. 터미널 채널은
그것과 무관하게 계속 산다(둘은 독립한 채널이다).
...
폰과 PC 의 maru 는 따로 갱신된다. 첫 메시지에서 **스키마 버전을 주고받고**, 모르는 필드는
무시하고 모르는 메서드는 **이름 있는 오류**로 답한다(조용히 성공으로 접지 않는다 — §5 의
"모르는 것은 모른다고 답한다" 와 같은 규율).
