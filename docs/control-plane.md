...
**슬라이스 분해(1e-confirm — 전부 구현 완료 2026-07-13. 계획 시 coarse 0/1/2였으나 안전 위해 아래처럼 세분 구현):**
  ...
  - **후속(1e-confirm 완료 후)**: grant dedup(같은 grant 중복 모달)·target 창에 모달(현재 `firstAppSession`)·grant revoke UX·`maru browser` CLI/MCP(에이전트가 실제 쓸 클라이언트 — 현재 드라이버는 스모크 클라뿐).
