...
- collector·소켓·디스패처(L4): `src/platform/macos/MaruAppHost.swift`, `src/platform/macos/control_socket.zig`(1b 존재), `control_collector.zig`(**아직 미존재 — Phase 1 실 collector**), `src/platform/macos/app_host_abi.{zig,h}`. `.m` 동반은 현재 불요(peer-cred/bind가 Zig로 충분 — 1b 노트).
