...
**9.5.2 — 이벤트 채널**. **구독**: 전용 메서드 **`browser.subscribe {surface_id, events?}`**(§9.2 5g `panel.navigated`를 흡수·대체 — 이름 `browser.navigated`로 통일; §6 표·§11에 반영). 요구 scope=**`browser`**(base; 민감 이벤트 console/network는 미룸 버킷이라 착수 시 sub-scope 판정). **이벤트 흐름**: 메인서 KVO/delegate 발화→매칭 구독 조회→notification 직렬화→그 연결 outbound 큐 enqueue. **백프레셔(§5 subscribeOutput 규범)**: 상태-스냅샷(navigated·loadState)=**coalesce**, 이산(dialog)=bounded drop+gap 마커, 하드 초과=`subscriber-lagged`로 강제 해제. **수명·revocation(§8.5:205 규범)**: surface close→`browser.closed`+구독 제거+**그 surface의 큐 잔여 프레임 purge**, 연결 drop→구독 제거, cap revoke→구독 제거+**큐 purge**(9.5.6 purge API). **순수 코어 분리(필수)**: 구독 레지스트리+필터+coalesce/drop 정책을 **L2 순수 `EventBroker`**(`control_capture.zig` 선례 — import=std+control_plane만, 강제=tests/boundary/imports.zig)로. **seam**: broker는 L4 `connection`을 모르므로 **opaque `subscriber_id: u64`** 로 추상화(L4가 id→OutboundQueue 매핑 소유). coalesce는 outbound 큐의 `coalesce_key`(=`(surface,kind)`)로 수행, L4가 pump. WKWebView 소스는 L4가 broker에 이벤트 주입만 → fake 소스로 헤드리스 테스트. **구현(5f-0b-3a)**: L4 `SubscriberRegistry`(`control_server.zig`, **메인 전용·락 없음**)가 순수 `EventBroker` + `subscriber_id→*OutboundChannel` 매핑(연결당 1:1·재구독 id 재사용) + id 발급기를 소유해 `subscribe`(→id)·`pushEvent`(매칭→구독자별 `serializeEvent`(cross_gpa)→coalesce_key로 outbound push)·`purgeOutbound`(연결 drop)·`purgeSurface`(surface close)를 제공한다. 헤드리스 유닛(fan-out·필터·coalesce vs 이산 누적·재구독·purge). **미배선(후속)**: ControlServer 임베드 + 연결 수명(연결-drop 시 `purgeOutbound`, 5f-0b-3a-2)·`browser.subscribe` dispatch(3b)·KVO 주입(3c).
