...
### 4.3 프레이밍 견고성
- max frame size(≈ 1 MiB) 정의. 초과 시 `payload-too-large` + 연결 종료. 부분 읽기는 누적 버퍼. `payload-too-large`의 JSON-RPC error code는 **-32001**이다 — JSON-RPC 2.0이 이 조건의 코드를 정하지 않으므로, impl-defined server-error 범위(-32000~-32099)에서 maru가 택한 값이다(구현: `src/session/control_plane.zig` `ErrorCode`). 표준 코드(parse -32700·invalid request -32600·method not found -32601·invalid params -32602·internal -32603)는 명세 그대로.
