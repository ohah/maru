...
- **D3 — 네트워크 티어 (결정 필요, 권장 Tier 1)**. WKWebView엔 CDP 전량 네트워크가 없다. **Tier 1(얕음·권장)**=주입 JS `fetch`/`XHR` 후킹 → 페이지 API 요청/응답(url·method·status·body); 미커버=서브리소스·리다이렉트·저수준 타이밍이나 대부분 시나리오 충족. **⚠️ Tier 1은 "공짜/재사용"이 아니다(감사 정정)**: fetch/XHR 후킹은 `.atDocumentStart` **주입 스크립트**를 요구하는데, 현 코드는 **비신뢰 browser 패널에 주입이 0**(브리지는 신뢰 markdown 패널 전용 — §8.1 origin 격리, 의도적). `browser.executeScript`는 page-world·1회성이라 `.atDocumentStart`에 고정 불가(라이브 경로는 on-demand로 didFinish 전후 아무 때나 발화하나 문서-시작 시점 주입은 안 됨) → 페이지 스크립트보다 먼저 fetch/XHR 참조를 선점 못 한다. 따라서 Tier 1 = **비신뢰 임의 콘텐츠에 read-only 관측 world를 새로 주입하는 보안심사 대상 신규 기능**(safaridriver는 이 `addinitscript`가 아예 불가라 maru의 직접 소유가 **능력상 우위**이긴 하나, 비용은 보안 결정이다 — D5 신뢰 게이트와 연동). **Tier 2(내비)**=`WKNavigationDelegate`(이미 있음)로 메인 프레임 요청/응답/status/리다이렉트 — 주입 불요라 **가장 싼 관측 시작점**. **Tier 3(전량·깊음)**=Web Inspector로 전 요청·타이밍·**interception/mock**; Passkeys·network mock 필요 시만. → **Tier 1(주입 관측 world)+Tier 2(delegate)를 5f, Tier 3 후속.** interception(변조/mock)은 Tier 3 전용. **Web Inspector viability 스파이크**(임베드·프로덕션서 되는지) 뒤 Tier 3 승격 여부 결정 — Tier 1 관측 훅은 Web Inspector가 관측을 대체해도 interception은 못 오므로 완전 폐기는 아님(에스컬레이션 사다리: 주입 관측 → Web Inspector 관측 → CEF interception).
- **D4 — 스토리지 (결정 필요, 권장 cookies+localStorage+clear)**. cookies=`WKHTTPCookieStore`(get/set/delete/observe) — **greenfield(감사)**: 현 코드는 쿠키 API 미사용, 격리를 `nonPersistent` 데이터스토어로만 함(agent-browser의 safari 경로는 W3C WebDriver 쿠키 엔드포인트로 **읽기**는 됨, maru는 아직 0). localStorage/sessionStorage=eval(page-world, `browser.executeScript`로 이미 가능 — D5 세탁 구멍 주의). 관리(clear/enumerate origins)=`WKWebsiteDataStore`. 비신뢰 패널은 **ephemeral store**(7e-0)라 종료 시 소멸 — clear 범위·의미 주의. IndexedDB=후속(async·복잡).
- **D5 — capability 서브스코프 (보안 결정 필요, 권장 분리)**. 현재 `browser` 단일 scope가 browser.* 전부 인가한다. 그러나 **storage-read(쿠키·localStorage=토큰)·network-read(요청 body=인증 헤더)는 navigate/click보다 훨씬 민감**하다(§9.2 세션 하이재킹). → `browser`(act/perceive/observe/perf) + 민감 read를 `browser:storage`·`browser:network` 서브스코프로 분리. **구현(감사 확정 — 모델 정합)**: metadata의 payload(self|window|all) 패턴이 **아니라** `read_output`/`write`/`lifecycle`처럼 **payload 없는 peer `ScopeClass` 변형**(`browser_storage`/`browser_network`)으로 추가한다 — metadata sub-scope는 응답 **필터**(authorize서 미검사)지만 browser sub-scope는 메서드 **게이트**(authorize의 class-equality가 그대로 거름)라 성격이 반대. `authorize`/`resolve`/`CapabilityStore` **무변경**, `methodRequiredScope`만 `browser.getCookies→browser_storage` 식 rest-dispatch + `class()`/`allowedViaInheritedFd()` switch에 arm 추가. §8.5가 이미 이 재조임을 예고("Phase 5 자체 발급 UX서 더 조일지 재검토 — 코드는 `allowedViaInheritedFd` 한 곳", §8.5). 1e-confirm 모달이 "쿠키 읽기 허용?"을 구분해 물음. **⚠️ 세탁 구멍(감사+적대적 재검증 — D5의 실 가치 한계, 확대 확인)**: (1) **executeScript**가 base `browser`면 page-world eval로 `browser_storage`를 **구조적으로 우회**한다 — `localStorage`/`sessionStorage`/IndexedDB는 **네이티브 읽기 경로가 없어 eval이 유일 통로**인데 그게 base `browser`라, `browser_storage`가 이걸 덮을 방법 자체가 없다. 게다가 `executeScript("document.cookie")`로 **non-HttpOnly 쿠키**·DOM 렌더된 토큰도 샌다 → "쿠키는 `WKHTTPCookieStore` 네이티브라 격리"는 **native getCookies 경로 + HttpOnly 쿠키에만** 참. (2) **screenshot**(base `browser`)이 렌더된 토큰/세션 화면을 캡처해 부분 우회. → **결론: `browser_storage` 서브스코프는 executeScript를 base로 두는 한 native getCookies·HttpOnly만 의미 있게 보호**한다. D5 착수 시 결정 필요: (a) executeScript를 `browser_storage`로 elevate(단 click/findElement/sendKeys가 전부 eval 기반이라 perceive/act 과도 제약 — control-plane §9 매핑) / (b) 서브스코프 유지하되 "eval-도달 비밀은 base `browser` cap이면 노출"을 명시(정직한 한계) / (c) executeScript에 별도 `browser:eval` scope. 서브스코프는 per-method 매핑만큼만 촘촘하다.
...
- **5f-3 observe**: **read-only 관측 주입 world**(신뢰 브리지와 분리·비신뢰 콘텐츠 주입 보안심사 — D2/D3) 위에 console·pageError·network Tier 1(fetch/XHR) + dialog(`WKUIDelegate` 핸들러 신규) + network Tier 2(nav delegate·주입 불요).
- **5f-4 state+perf**: cookies·localStorage·clear(+D5 서브스코프) + performance(Timing/Vitals/memory).
