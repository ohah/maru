...
tmux(`list-panes`/`send-keys`/`capture-pane`) 같은 도구가 푸는 문제를 다루되, maru는 **하나의 wire 프로토콜을 CLI와 웹뷰가 공유**하게 해서 두 번 설계하지 않는다.
...
### 닿기는 우리 몫이 아니다 — 터널은 사용자가 준비한다 (사용자 확정 2026-09-02)

위 전송은 **「SSH 로 그 PC 에 닿을 수 있다」를 전제**한다. 그 전제를 누가 세우는가가 남는데,
**maru 는 그것을 안 맡는다.** 사용자가 준비한 터널 위에 붙기만 한다 — **권장은 Tailscale** 이고,
SSH 리버스 터널·Cloudflare Tunnel 도 같은 자리다.

**왜 릴레이를 안 만드는가.** 릴레이는 NAT 를 피하려고 양쪽이 각자 밖으로 걸어 나가 만나는
장소라, 「연결만 붙이고 빠지는」 서버가 아니라 **세션 내내 트래픽이 지나가는 상시 인프라**다.
그것을 운영하는 순간 사용자 터미널이 우리 서버를 지나가므로 「우리는 못 본다」를 암호로 증명해야
하고(레퍼런스 앱이 실제로 그 길을 갔다), 샤드·자격증명 회전·고아 정리가 클라이언트 코드에까지
드러난다. **그 복잡도 전부를 안 지는 대신 「설치하면 바로 폰에서 됨」을 포기한다** — 사용자가
터널을 준비해야 한다. 이것이 이 결정의 대가이고 숨기지 않는다.

**Tailscale 을 써도 SSH 는 그대로다.** 둘은 다른 문제를 푼다:

| | Tailscale | SSH |
|---|---|---|
| 닿기(NAT·주소) | **준다** — `100.x` 고정 주소 | 못 준다 |
| 전송 암호화 | 준다(WireGuard) | 준다 |
| **기기** 신원 | 준다 — 내 tailnet 안인가 | — |
| **사용자** 신원 | **안 준다** — 「어느 OS 사용자로 도는가」가 아니다 | 준다 |
| 셸·PTY·다중 채널 | **안 준다** — 네트워크 계층이다 | 준다 |

tailnet 소속은 「이 기기는 내 것」이지 「`yoonhb` 로 셸을 열어라」가 아니다. 그리고 §1 의 보안
모델은 **「SSH 로 로그인한 사용자 == maru 앱을 돌리는 사용자」** 위에 서 있다 — 그 등식을 세우는
것이 SSH 사용자 인증이다. tailnet 소속만으로 문을 열면 그 등식이 사라지고, 폰 하나가 털리면
셸이 통째로 열린다. 그래서 **닿기는 터널이, 신원과 세션은 SSH 가** 맡는다.

키 관리가 부담이면 **Tailscale SSH**(테일스케일이 SSH 를 종단해 tailnet 신원으로 인증)를 쓸 수
있다 — 사용자가 키를 안 만들어도 되고, **maru 쪽은 여전히 평범한 SSH 라 코드가 그대로다**.

**코드가 지켜야 할 것은 하나다: 닿는 방법을 전제하지 않는다.** `ssh.server.<n>.host` 는
「이름 또는 IP」이므로 `100.x` 주소든 MagicDNS 이름이든 그대로 들어간다
([모바일 config §4.3](mobile-config.md)). LAN 탐색·브로드캐스트·특정 대역 가정 같은 것을 넣지
않는다 — 넣는 순간 터널 위에서 안 도는 코드가 된다.
...
- **그 상태 검사만으로는 모자란다**(실기 2026-09-03, PR#3164). 채널 상태는 **닫기를 «요청한»
  순간** `closed` 가 되는데 전송은 아직 오가는 중이라, 그 창에서 열면 코어가
  `MARU_SSH_ERR_NOT_READY`(-7 — `control_closing`·`ssh_rekeying`)를 낸다. **host 는 그것을 실패로
  접으면 안 된다** — 뜻은 이미 가져간 뒤라 사라지고, 그 화면은 영영 "받는 중" 이 된다(세션 화면
  **재진입**이 정확히 그렇게 죽었다). 「아직 때가 아니다」는 화면에 아무 말도 하지 않고 **뜻만
  되돌린다**(`maru_mobile_control_open_retry`). 그 밖의 코드만 실패다.
...
- **되돌리는 데는 시한이 있다 — 5초**(2026-09-04 확정). 되돌리기를 무한히 하지 않고, 이 축이
  이미 쓰는 컨트롤 시한과 **같은 5초**를 넘기면 딱딱한 실패로 접어 화면이 말하게 한다. 그 뒤의
  회복은 사용자가 그 자리를 벗어났다 다시 들어오는 것이다(같은 것을 다시 원하면 아무 일도 안
  일어난다는 위 규칙 때문이다).

  **왜 바꿨나.** 여기에는 원래 "닫힘을 기다리는 데 시한은 없다 — 상대가 `close` 에 끝내 답하지
  않으면 연결 자체가 끊기고 축이 처음부터 다시 선다" 가 적혀 있었다. 적대적 검증에서 그 근거가
  **코드에 없다**는 것이 드러났다 — 컨트롤 채널의 닫힘이 안 끝나도 세션을 접는 자리도 시한도
  없고(터미널 채널이 멀쩡하면 연결은 안 끊긴다), 그 문장은 결정을 저울질한 것이 아니라 그때의
  코드를 **서술한 것**이었다(2026-08-28 fix 커밋이 다른 두 결함을 고치며 함께 적었다).
  반대로 이 저장소가 **명시적으로 택한 정책은 무한 재시도 금지**다 —
  [네이티브 편집기 계획](plans/native-editor.md)의 *"무한 재시도는 하지 않는다"*(`120ms × 50회`,
  초과하면 화면이 「보여 줄 수 없음」을 말한다)과
  [영속 세션 호스트](persistent-session-host.md)의 permanent rejection 규칙이 같은 모양이다.
  무한 대기는 §4a 가 정한 「실패하면 그 화면에서 말한다」를 **그 경우에만** 어긴다.

  **후속(더 나은 형태).** 지금은 5초 뒤 「실패했다」고 말한다. 더 정확한 것은 계속 되돌리면서
  화면이 「아직 정리 중」이라고 말하는 것이다 — 거짓 실패도 침묵도 없다. 그것은 화면 상태를
  하나 더 두는 일이라 이 절의 이분법(실패 표시)을 넓히는 결정이고, 별도로 정한다.
...
**그 화면은 읽기 전용이다.** `--stream` 은 observer 로 붙으므로(세션 호스트 §8) 폰도 그렇게
보인다 — 키보드를 안 올리고 입력을 안 받는다. 뜨는데 안 들어가면 사용자가 쳐 놓고 잃는다.
상단에 "보는 중" 을 적어 조종하는 화면과 가른다.

**세 상태를 가른다**: 아직 안 왔다(받는 중) · 끊겼다 · 화면이 왔다. 특히 앞의 둘을 뭉치면
**끊긴 화면에 "받는 중" 이 영원히 떠서** 사용자가 오지 않을 것을 기다린다. 판정은 "아직도 그
화면을 원하는가" 로 한다 — 연결이 새로 서면 원하는 것이 비므로 그때가 끊긴 것이다.

**누를 수 있는 줄만 눌린다.** `runtime_id` 가 없는 줄(§3)은 누름 표시도 안 준다 — 눌리는 것처럼
보이고 아무 일도 안 일어나면 사용자는 고장으로 읽는다. 그리고 **누를 때 잡아 둔 줄을 연다**:
뗄 때 좌표로 다시 찾으면 그 사이 목록이 갱신됐을 때 다른 세션을 연다.
...
**그래서 셀렉터 없는 연결의 metadata scope 는 `all` 이다**(2026-08-29 — 단일 출처는
[보안 §8.4](control-plane-security.md)). 이 문단은 처음부터 "앵커가 필요 없는 것은 된다" 를
계약으로 적었지만 **코드는 그렇지 않았다**: 셀렉터가 없으면 anchor 가 0 이 되고 scope 가 `self`
라 목록이 언제나 비었다 — 폰에서는 그것이 "세션이 하나도 없다" 로만 보였다(실측). 셀렉터를 댄
연결은 종전대로 그 surface 하나만 본다. **넓어진 것은 metadata 뿐이고**, `session.capture`
(read-output)·write·`browser.*` 는 그대로 각자의 cap·앵커를 요구한다.
