...
**9.5.1 — 연결 I/O 모델 (직렬 블로킹 폐기)**. **결정: 연결당 스레드 + 통합 outbound 큐**. `serveConnection` 인라인 직렬 호출 대신 **연결마다 스레드 spawn**(bounded `max_connections` — 제어 플레인은 클라 소수라 스레드-per-conn이 poll-멀티플렉스보다 추론 단순). 각 연결 스레드는 **`waitResolved` 블로킹을 버리고** `poll([socket_fd, wakeup])` 루프: socket readable→프레임 조립→요청을 `ControlRequestQueue`에 **`tryPush`(포화면 `server-busy` 응답, 블로킹 아님 — §5:93 규범)**·다시 poll. 메인 drain이 응답을 만들면 **그 연결의 outbound 큐**에 enqueue+wakeup. 스레드는 outbound 큐(=**응답+이벤트 통합**)를 socket에 write. §8.8 lock-order([io-render-threading.md]): **outbound 큐=leaf lock**, core_mutex 보유 중 미접근(이벤트=KVO=core_mutex 밖, 응답=`ControlRequestQueue` drain 후 push). 수명·소유권·포화는 9.5.6이 소유(초판 gap 해소).
