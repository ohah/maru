...
**능력 택소노미 — WKWebView 소스·티어별** (base=agent-browser의 WebDriver-형 명령 + 브라우저 devtools 관측 표면; CDP 아님):

| 축 | 능력 | WKWebView 소스 | 티어 |
|---|---|---|---|
| **act** 제어 | navigate·back/forward/reload·click·sendKeys/type·scroll·hover·select | `load`/`goBack`… + `evaluateJavaScript` | 얕음(5f) |
| **perceive** 지각 | snapshot(a11y/DOM+안정 노드 ref)·screenshot(`takeSnapshot`, 전체/요소)·PDF(`createPDF`)·getUrl/title/favicon/readyState/viewport/UA | 네이티브 + eval | 얕음(5f) |
| **observe** 이벤트 | nav 생애주기·console(+uncaught error·unhandledrejection)·dialog·loadState·**웹 프로세스 crash**·CSP 위반 | KVO/navigationDelegate/uiDelegate + 주입 JS + `webViewWebContentProcessDidTerminate` | 얕음(5f, D2) |
...
| **performance** 성능 | Navigation/Resource/Paint Timing·Web Vitals(LCP/CLS/FCP/INP)·`performance.memory`·프로세스 메모리 | 주입 JS `performance.*` + 프로세스 통계 | 얕음(5f) |
| **synchronize** 대기 | wait-for(selector·navigation·network-idle·load·timeout) | events + 폴링 | 얕음(5f, perceive-act 필수) |
| **deep debug** 깊은 디버깅 | JS 디버거(breakpoint/step)·CPU/heap 프로파일·coverage·DOM inspector | Web Inspector 원격 프로토콜(`isInspectable`) | 깊음(후속) |

**핵심 조직 원리 — 티어 경계**: 축 대부분(성능 Timing·Web Vitals·wait·crash·errors 포함)은 **주입 JS + 네이티브 delegate = 얕은 티어**로 커버된다. 오직 **전량 네트워크 interception·프로파일러·디버거·coverage**만 **Web Inspector 원격 프로토콜(깊은 티어)**이 필요하다(web-panel.md §12 "얕은 host-mediated vs 깊은 Web Inspector" 분기). → 얕은 티어를 5f에, 깊은 티어를 명시적 후속으로. 성능·디버깅이 "다 Web Inspector라 무겁다"는 오해를 깬다: 개발자가 흔히 보는 값(로드 타이밍·Web Vitals·메모리·콘솔 에러·crash)은 대부분 주입 JS로 얕게 나온다.
...
### 9.5 5f-0 구현 설계 — 지속 세션·이벤트·대용량 결과 (doc-first, §9.4 A1~A5 해소)

**목적**: §9.4 프로토콜 설계 아래 **구현·테스트 층**의 문제 5개(감사 후속 비판)를 5f-0 코딩 **전에** 해소한다. §9.3이 5e를 앞선 것과 같은 doc-first — A1/A2는 단순 배선이 아니라 **연결 모델 재아키텍처**라 설계를 먼저 못박는다.

**전제(감사 확인 사실)**: (i) `acceptLoop`이 `serveConnection`을 인라인으로 하나 다 처리 후 다음 accept = **직렬**, (ii) accept 스레드가 `waitResolved`에서 **블로킹**, (iii) `default_max_frame=1MB` 초과 시 **연결 abandon**(재조립 불가), (iv) 이벤트 소스=메인 스레드(WKWebView KVO/delegate), (v) 구독·레지스트리는 §8.8대로 **메인 전용**.

**9.5.1 — 연결 I/O 모델 (A1: 직렬 블로킹 폐기)**. **결정: 연결당 스레드 + 통합 outbound 큐**. `acceptOne` 후 `serveConnection`을 인라인 호출하는 대신 **연결마다 스레드를 spawn**(bounded `max_connections` — 제어 플레인은 클라 소수라 스레드-per-conn이 poll-멀티플렉스 단일루프보다 추론 단순; 후자는 연결 급증 시 대안). 각 연결 스레드는 **`waitResolved` 블로킹을 버리고** `poll([socket_fd, wakeup])` 루프: socket readable→프레임 조립→요청을 기존 `ControlRequestQueue`에 push(**논블로킹, 응답을 기다리지 않음**)·다시 poll. 메인 drain이 응답을 만들면 **그 연결의 outbound 큐**에 enqueue + wakeup. 스레드는 outbound 큐(=**응답 + 이벤트 통합**)를 socket에 write. `PendingRequest`에 `*OutboundQueue`(연결 back-ref) 추가해 메인이 올바른 연결로 응답 라우팅. §8.8 lock-order: **outbound 큐 = leaf lock**(그 아래 다른 락 없음), core_mutex 보유 중엔 절대 안 잡음(이벤트는 KVO=core_mutex 밖 발생, 응답은 `ControlRequestQueue` drain 후 push). 이것이 A1(직렬)+A2(이벤트 write 못 끼움)를 **한 outbound 큐로 동시 해소**.

**9.5.2 — 이벤트 채널 (A2·B3)**. **구독**: `browser.subscribe {surface_id, events?}` → 메인이 `(connection, surface_id, filter, capability)` 구독 등록. **이벤트 흐름**: 메인서 KVO/delegate 발화 → 매칭 구독 조회 → notification 직렬화 → 그 연결 outbound 큐 enqueue(포화 시 백프레셔). **백프레셔(§4.3/§5 규범 재사용)**: 상태-스냅샷 이벤트(navigated·loadState)=**coalesce**(최신만), 이산 이벤트(console·dialog)=bounded drop + gap 마커, 하드 초과=`subscriber-lagged`로 **구독 강제 해제**. **수명·revocation(§8 재검증)**: surface close→`browser.closed`+그 surface 구독 제거, 연결 drop→그 연결 전 구독 제거, cap revoke→해당 cap 구독 제거. **B3 순수 코어 분리(필수)**: 구독 레지스트리+필터+coalesce/drop 정책을 **L2 순수 모듈**(`control_capture.zig` 상태기계 선례 — `EventBroker`)로 두고 **fake 이벤트 소스 주입으로 헤드리스 테스트**. WKWebView 소스(KVO/delegate)는 L4가 순수 코어에 이벤트를 넣기만. 안 그러면 이벤트 전체가 스모크-only=약함.

**9.5.3 — 대용량 결과 (A3: 스크린샷 1MB 초과)**. **결정: 바이너리 결과는 파일 side-channel**. 스크린샷/PDF는 `<control-dir>/artifacts/<nonce>.png`에 **0600으로 쓰고 결과엔 경로만** 반환(에이전트가 읽음). 근거: unix 소켓=same-host라 파일 공유 가능, chunk 스트리밍(§4.3 미배선)을 스크린샷 하나 때문에 배선하는 것보다 단순, trace-inspector 파일 기반 힌트와 정합. 수명=TTL/read-후-삭제(누수 방지). **chunk 스트리밍은 `subscribeOutput`(고처리량 텍스트)용으로 남김** — 바이너리=파일, 스트림 텍스트=chunk. (원격 maru ssh는 파일 전송 필요=후속.)

**9.5.4 — snapshot fork (A4: 노드-ref 출처 = 주입 함정)**. click/type가 참조할 snapshot 노드-ref를 **네이티브 WebKit a11y(NSAccessibility)** 로 뽑으면 주입 불요(5f-1 유지 가능)지만 **안정 ref가 어렵다**(NSAccessibility 요소는 안정 id 없음, CDP Accessibility backend-node-id는 CDP 전용). **주입 JS DOM walk**로 뽑으면 안정 ref 쉬우나 **비신뢰 콘텐츠 주입=D3와 같은 보안 표면→console/network처럼 미룸**. **결정: 5f-0에 "네이티브 a11y snapshot viability 스파이크" 추가**(Web Inspector 스파이크와 병렬) — 성공=네이티브 snapshot을 5f-1에, 실패=snapshot을 **주입-gated 버킷으로 미룸**(network·console과 함께). 그때까지 **5f-1 perceive = screenshot(takeSnapshot·네이티브·주입불요) + metadata(getUrl/title 네이티브·1회성 eval)만 확정**, snapshot은 스파이크-gated.

**9.5.5 — 테스트 경계 (B1·B2·B3)**. **헤드리스(CI 게이트)**: EventBroker(fake 소스로 구독·필터·coalesce/drop·revocation)·지속세션 멀티요청 왕복(기존 `control_server` 왕복 테스트 확장)·outbound 큐 백프레셔·파일-경로 결과 직렬화·D5 서브스코프 게이트(`control_capability` 순수). **스모크(display·비-CI)**: 실 WKWebView 이벤트(navigate→`navigated`, `alert()`→`dialog`, `console.log`→`console`)·스크린샷 파일 존재+PNG 헤더·쿠키/localStorage 실 read. **fixture 페이지**(maru-app:// 신뢰 테스트 페이지 or data:)에 클릭 버튼·로그/alert/fetch JS·결정적 네비 + **scripted-agent**(5e 인-프로세스 소켓 클라 확장)가 subscribe→op 구동→이벤트/결과 단언. **손 테스트 only**: crash 이벤트(결정적 트리거 불가 — private API/known-crash 필요), 실 에이전트+실 로그인.

**5f 재슬라이싱(§9.5 반영)**: **5f-0**=연결당 스레드+통합 outbound 큐(9.5.1)+EventBroker 순수 코어(9.5.2)+`browser.subscribe`/`navigated`(KVO 재사용)+**두 스파이크(Web Inspector·네이티브 a11y snapshot)**. **5f-1**=screenshot(파일)+metadata (snapshot은 a11y 스파이크 gated). **5f-2**=act(click/type/scroll)+wait. **5f-3(주입불요)**=dialog·loadState·crash(손 테스트). **5f-4**=cookies+localStorage+clear+performance. **5f-5**=fidelity. **미룸(주입-world 보안심사+스파이크 후)**: network·console·pageError·(a11y 실패 시)snapshot.

**베이스·의사결정(clean-room)**: 연결 모델 base=기존 `ControlRequestQueue`/`PtyEventQueue`(bounded MPSC) + poll 패턴; outbound 큐 base=§4.3 per-connection bounded outbound; 이벤트 백프레셔 base=§5 subscribeOutput 규범; 순수 코어 base=`control_capture.zig` L2 상태기계. **결정 요약**: 연결당 스레드+통합 outbound 큐(A1/A2), 바이너리=파일 side-channel(A3), snapshot=a11y 스파이크 gated(A4), 이벤트 코어=순수 헤드리스+소스 주입(B3). **후속 스파이크 2개(5f-0)**: Web Inspector 임베드 viability(네트워크/성능 딥 티어·CEF), 네이티브 a11y snapshot viability(snapshot 주입 회피 가부).
