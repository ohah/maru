...
- **모바일 ABI 가 아직 채널을 모른다.** 코어는 채널 둘을 들지만(S10b-1),
  `maru_mobile_ssh_feed/write/screen_ptr` 은 **핸들 하나**를 받고 화면 바이트도 한 흐름이라
  "어느 채널의 바이트인가" 를 말할 수가 없다 — **S10b-2** 가 ABI·펌프를 넓힌다.
