...
- **⚠️ 에이전트 상태(사이드바 카드) 검증 시 자식 세션 금지**: `MARU_DEBUG=1 ./zig-out/Maru.app/...`를 **Claude Code 세션 안에서** 띄우면 팬 셸이 `CLAUDE_CODE_CHILD_SESSION=1`을 상속해, 그 안에서 실행하는 claude가 트랜스크립트를 persist하지 않아 훅 매핑이 phantom이 되고 **상태줄이 안 뜬다**(환경 오염, 기능 결함 아님 — 단일 출처 [agent-session.md "검증"](agent-session.md)). 일반 터미널에서 띄우거나 `env -u CLAUDE_CODE_CHILD_SESSION -u CLAUDE_CODE_SESSION_ID …`로 벗기고 실행한다.
...
- macOS Swift app host app shell smoke 실행: `mise run macos-app-smoke` (`MARU_MACOS_APP_SMOKE_MS=6000`으로 실제 AppKit/CAMetalLayer+WKWebView를 띄우고 controlled PTY와 파일 패널 fixture를 함께 태워 `zig-out/maru-macos-app/app.summary.txt`를 남긴다. 기존 frame/input/resize/close gate에 더해 FP4는 `file_viewer_ready=true`, fixture 본문, 로컬 SVG 1/1 로드, renderer bridge/message-handler `undefined`, `renderer_parent_accessible=false`를 확인한다. 파일 패널은 `MARU_FILE_PANEL=<절대 markdown>`·`MARU_WEB_APP_ROOT=<web/dist>`로 재현할 수 있다.)
