...
- bounded browser.executeScript WKWebView/socket smoke 실행: `mise run macos-browser-bounded-smoke` (`MARU_WEB_PANEL=1`·test browser cap으로 실 WKWebView에 소켓 요청을 보내 구조화 strict JSON, persistent intrinsic 변조 방어, UTF-8 byte cap, typed error, depth 경계를 검증한다. `zig-out/maru-macos-app/app.summary.txt`의 7개 `browser_ctl_bounded_*` 필드가 모두 `true`가 아니면 명령이 실패한다. GUI 눈검사는 필요 없다)
