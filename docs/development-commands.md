...
- **세션 trace 레코딩(replay용)**: `MARU_TRACE=/tmp/session.trace mise run macos-app`로 켠다. 세션의 base kind 이벤트(원시 PTY 출력·resize·process-exit)를 `maru.trace.v1`로 그 파일에 **이벤트마다 증분 append**한다(`app/trace_recorder.zig`의 `TraceRecorder` — `SurfaceRuntime` 훅, host file writer에 write+flush). 굳힌 trace는 `observability.replay.replayTrace`로 GUI 없이 화면을 byte-for-byte 재생한다(회귀 fixture·오프라인 분석 — 단일 출처 [trace-replay.md](trace-replay.md)). opt-in(미설정이면 오버헤드 0), 8 MB 상한(초과 시 앞부분만 유효). **크래시가 나도 직전까지 디스크에 남아** 재생된다(clean 종료 불필요). 캡처한 trace를 **git fixture로 승격**하려면 먼저 `maru trace anonymize /tmp/s.trace out.trace`로 PII(홈 경로·IP·`user@host`·유저명, env HOME/USER 기준)를 일반화한다(구조 보존 → 여전히 재생 가능; 남은 `TOKEN=…` secret은 경고). 예: `MARU_TRACE=/tmp/s.trace mise run macos-app` → `maru trace anonymize /tmp/s.trace tests/fixtures/traces/x.trace.txt` → `MARU_UPDATE_GOLDEN=1 zig build test-replay`로 golden 생성.
