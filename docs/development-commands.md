...
- macOS Swift app host app shell smoke 실행: `mise run macos-app-smoke` (`MARU_MACOS_APP_SMOKE_MS=6000`으로 실제 AppKit/CAMetalLayer+WKWebView를 띄운다. FP4 Markdown fixture에서 본문·로컬 SVG·renderer bridge/message-handler 부재·부모 DOM 접근 거부를 확인한 뒤, FP5 HTML fixture를 순차 실행해 별도 ephemeral store·브리지/스킴 핸들러 부재·부모 read scope·내부 SVG 성공·scope 밖 SVG 차단·top-level pin을 확인한다. 각 실행은 `zig-out/maru-macos-app/app.summary.txt`를 남긴다. `MARU_FILE_PANEL=<절대 .md|.html>`·`MARU_WEB_APP_ROOT=<web/dist>`로 재현할 수 있다.)
