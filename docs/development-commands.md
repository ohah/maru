...
- **세션 trace 레코딩(replay용)**: `MARU_TRACE=/tmp/session.trace mise run macos-app`로 켠다. 세션의 base kind 이벤트(원시 PTY 출력·resize·process-exit)를 `maru.trace.v1` 텍스트로 누적하다가 세션 종료 시 그 경로로 굳힌다(`app/trace_recorder.zig`의 `TraceRecorder` — `SurfaceRuntime` 훅에서 append, `AppSession.deinit`에서 파일 write). 굳힌 trace는 `observability.replay.replayTrace`로 GUI 없이 화면을 byte-for-byte 재생한다(회귀 fixture·오프라인 분석 — 단일 출처 [trace-replay.md](trace-replay.md)). opt-in(미설정이면 오버헤드 0), in-memory 누적이라 핫패스에 파일 I/O가 없고 8 MB 상한(초과 시 앞부분만 유효). 한계: 정상 종료(`deinit`)에서만 파일이 쓰인다(크래시 시 미기록 — 증분 flush는 후속).
