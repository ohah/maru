...
- macOS live PTY Metal smoke 실행: `mise run macos-app-pty-metal-smoke` (controlled PTY command output과 AppKit synthetic `keyDown:`에서 얻은 key event가 app host keybinding resolver를 통과하는 roundtrip을 실제 AppKit/CAMetalLayer 창의 CoreText glyph atlas shader sampling까지 태우고 `zig-out/maru-macos-app-pty-metal-smoke/app-pty-metal-frame.ppm` screenshot artifact를 남긴다. 마지막 visible frame 뒤에는 작은 AppKit close delegate smoke가 Zig callback을 통해 `FrameLoop.closeActiveLivePty -> LivePtyRegistry.closeActive -> LivePtySession.closeAndDetach` 경로를 호출하고, native close request/callback/window close와 registry mapping 제거 gate도 summary에 남긴다. 창이 너무 빨리 닫히면 `MARU_APP_PTY_METAL_SMOKE_MS`로 조절한다. 아직 물리 키보드 입력을 받는 지속 interactive shell loop나 제품 tab/window close button 검증은 아니다)
- macOS live PTY Metal manual keyDown smoke 실행: `MARU_APP_PTY_METAL_KEYDOWN_SOURCE=manual MARU_APP_PTY_METAL_KEYDOWN_MS=15000 mise run macos-app-pty-metal-smoke` (별도 key capture 창이 뜨면 사용자가 `Cmd+B`를 직접 누른다. 이 경로는 물리 키보드 -> AppKit `keyDown:` -> Zig `terminal.KeyEvent` -> app host resolver -> PTY write -> Metal screenshot까지 확인하지만, 여전히 한 번의 smoke이고 지속 실행되는 제품 shell loop는 아니다)
