...
- macOS Mermaid helper lifecycle smoke: `mise run macos-mermaid-helper-smoke` (실제 별도 helper process로 Hello/HelloAck, duplicate/result-flood 거부, 2초 hang kill/restart 3회 failure latch와 exact termination ACK, stdin 미소비 max request의 bounded 종료, stdout/stderr EOF 1회 종료, executor deadline 지연 시 physical spawn 0, A→B→A path ABA에서 actual PID mismatch와 capability frame/Result commit 0, running=nil 종료 barrier 뒤 late callback/control 0, 100개 요청 bounded admission을 검증하고 `zig-out/maru-macos-mermaid-helper-smoke/mermaid-helper.summary.json`을 남긴다. 실제 editor dirty 보존 E2E는 helper가 제품 editor와 연결되는 FP10d gate다)
