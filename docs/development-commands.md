...
- **sync(2026) 게이트 관측**: `MARU_DEBUG=1`이면 `app_session.zig`의 `logSyncGateDiag`가 `std.log.scoped(.sync)`로 tick별 sync hold/투영 결정을 한 줄씩 stderr에 찍는다(`active`·`hold/timeout`·`gproj`·`cproj`·`bsu/esu`=리더 처리 BSU/ESU 누적·`voff` 등). 폴링 렌더 루프가 라이브 프레임을 언제 붙잡고 언제 flush하는지, `maru ssh` 원격 sync 어긋남이 어디서 나는지 본다(단일 출처 [io-render-threading.md §11.6](io-render-threading.md)). 캡처한 로그는 `python3 tools/sync/analyze_sync_log.py <log>`로 half-frame(active 중 grid 투영) 원인(esu_edge/timeout/scroll/force)을 자동 분해한다. 예: `MARU_DEBUG=1 ./zig-out/Maru.app/Contents/MacOS/maru-macos-app 2> /tmp/maru-sync.log` → `python3 tools/sync/analyze_sync_log.py /tmp/maru-sync.log`.
