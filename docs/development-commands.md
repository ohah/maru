...
- macOS Mermaid 격리 helper 빌드: `mise run macos-mermaid-helper-build` (`zig-out/bin/maru-mermaid-renderer`를 만들며 parent/helper 모두 Zig `mermaid_protocol.zig` codec ABI를 링크한다. FP10c1에서는 Mermaid 실행은 inert다)
- macOS Mermaid helper lifecycle smoke: `mise run macos-mermaid-helper-smoke` (실제 별도 helper process로 Hello/HelloAck, duplicate/result-flood 거부, 2초 hang kill/restart 3회 failure latch와 exact termination ACK, stdin 미소비 max request의 bounded 종료, stdout/stderr EOF 1회 종료, executor deadline 지연 시 physical spawn 0, A→B→A path ABA에서 actual PID mismatch와 capability frame/Result commit 0, running=nil 종료 barrier 뒤 late callback/control 0, 100개 요청 bounded admission과 helper가 소비하는 공용 inert 문서 base URL의 nil 계약을 검증하고 `zig-out/maru-macos-mermaid-helper-smoke/mermaid-helper.summary.json`을 남긴다. Launch Services 경고 UI 부재는 제품 macOS 환경에서 수동 확인하며, 실제 editor dirty 보존 E2E는 helper가 제품 editor와 연결되는 FP10d gate다)
...
- macOS Swift app host app shell smoke 실행: `mise run macos-app-smoke` (`MARU_MACOS_APP_SMOKE_MS=9000`으로 실제 AppKit/CAMetalLayer+WKWebView를 띄운다. 빌드 target은 원본 fixture를 `zig-out/`으로 복사하고 `MARU_FILE_PANEL_EDIT_SMOKE=1`을 명시한 뒤, FP4 Markdown 본문·로컬 SVG·renderer 격리뿐 아니라 새 파일의 기본 live mode, 초기 hydration과 경쟁한 dirty-sync의 완료형 recovery, FP11b `EditableProjection` running·decoration>0·일반 fragment iframe=0, 실제 AppKit 글자 입력→CM6 dirty→실제 `⌘S`의 Zig `WebKeyRoute.web_editor` 판정→pathless atomic write의 disk marker까지 확인한다. 이어 FP5 HTML fixture를 별도로 실행해 ephemeral store·브리지/스킴 핸들러 부재·부모 read scope·내부 SVG 성공·scope 밖 SVG 차단·top-level pin을 확인한다. 각 실행은 `zig-out/maru-macos-app/app.summary.txt`를 남긴다. 수동 `MARU_FILE_PANEL=<절대 .md|.html>`·`MARU_WEB_APP_ROOT=<web/dist>` 재현은 파일을 읽기만 하며, **복사한 테스트 Markdown을 실제로 수정·저장하려는 경우에만** `MARU_FILE_PANEL_EDIT_SMOKE=1`을 함께 준다.)
