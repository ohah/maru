...
- bounded browser.executeScript WKWebView/socket smoke 실행: `mise run macos-browser-bounded-smoke` (`ReleaseSafe`; Debug의 byte-by-byte base64/JSON 비용이 16 MiB 운영 deadline을 왜곡하지 않게 제품 최적화에 가까운 빌드로 잰다. `MARU_WEB_PANEL=1`·test browser cap으로 실 WKWebView에 소켓 요청을 보내 구조화 strict JSON, persistent intrinsic 변조 방어, UTF-8 byte cap, typed error, depth 경계와 512 KiB inline/초과 chunk 전환·16 MiB 32-chunk 완료를 검증한다. `zig-out/maru-macos-app/app.summary.txt`의 8개 `browser_ctl_bounded_*` 필드가 모두 `true`이고 pump action>0·p95≤0.5ms·max≤1.0ms가 아니면 명령이 실패한다. GUI 눈검사는 필요 없다)
