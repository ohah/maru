...
- macOS Swift app host dev shell 실행: `mise run macos-app-dev` (지속 실행되는 `NSApplication` placeholder window를 띄우고 Zig 쪽 interactive shell surface와 `FrameLoop`를 30Hz timer로 tick한다. placeholder view의 `keyDown`, window resize, window close는 dev session ABI로 내려간다. 아직 Metal terminal view가 아니므로 화면은 placeholder지만 summary에는 `terminal_surface=true`와 frame/output/input/resize/close 통계가 남는다)
