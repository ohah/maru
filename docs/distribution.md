...
**그림의 단일 출처는 `assets/icon/render.py` 다.** 모티프는 「마루」의 **「마」**를 터미널이 쓰는
두 커서 모양으로 쓴 것이다 — `ㅁ` 자리에 블록 커서, `ㅏ` 의 세로획 자리에 바 커서, 거기 삐침
하나(사용자 확정 2026-09-10). 글자로도 읽히고 터미널로도 읽힌다. 색은 새로 만들지 않는다:
앰버는 브랜드 강조색(`accent_default` = `#dda15e`), 바탕은 기본 다크 배경 계열(`#1e1e2e`)이다.

**`ㅁ` 은 터미널 셀 비율이 아니다.** 블록 커서는 1:1.9 로 길쭉해서 그대로 두면 「마」가 아니라
「미」로 읽힌다 — 1:1.25 로 눕혀야 글자가 된다.
...
**「작아도 안 뭉친다」는 눈으로 하는 말이라 «재어» 본다.** `--selftest` 가 두 가지를 잰다:
가운데 가로줄에서 바탕이 이어지는 토막이 **셋**인가(둘이면 `ㅁ` 과 `ㅏ` 가 붙은 것이다) — 32px
이상 모든 크기에서. 그리고 적응형 앞면의 잉크가 **안전 원 안에 드는가**. `ㅁ` 을 눕히자 제일
작은 앞면이 원을 0.6px 넘은 것을 이것이 잡았다(눈으로는 안 보였다). 32px 아래는 옛 모티프도
뭉갰으니 재지 않는다.
