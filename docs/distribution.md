...
**왜 그림 파일이 아니라 생성기인가.** 크기가 서른 몇 개다(macOS `.icns` 열 · iOS 다섯 ·
Android 밀도 다섯 × **셋** — 런처 · 적응형 앞면 · 알림). 손으로 맞추면 한 자리만 낡고, 그 한
자리는 **그 크기에서만** 드러난다. 한 번 그리고 전부 뽑는다.

**알림 아이콘만 잉크가 다르다.** Android 상태바는 small icon 의 **알파만 읽어** 자기 색으로
칠하므로(API 21+), 앰버도 바탕도 사라진다 — 런처 아이콘을 그대로 주면 **흰 네모**가 된다.
그래서 같은 모티프를 **흰 잉크 · 투명 배경**으로 따로 뽑고(`NOTIFICATION`, 24~96 px), **24 px
에서도 두 덩이가 붙지 않는지** `--selftest` 가 잰다: 색이 사라진 실루엣에서 붙으면 그냥 얼룩이다.
