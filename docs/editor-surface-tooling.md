...
| **하지 않는 것** | 여러 결과의 목록·peek(`⌥F12`) · ~~declaration/typeDefinition/implementation~~(**§8.2m 에서 섰다** — 2026-09-22, 같은 피커) · ~~references~~(**§8.2l 에서 섰다** — 2026-09-21, 참조 피커) · `⌘` 호버 밑줄(링크처럼 보이기) · 「정의로 이동 → 선택」(caret 만 놓는다 — §5.2) · stale 위치 보정 | 다음 조각들 |
...
| **하지 않는 것** | 편집기 안 인라인 peek 창 · 참조 뷰 패널(`⌥⇧F12`) · ~~declaration/typeDefinition/implementation~~(§8.2m) · 미리보기 캐시의 지속(피커 수명) · 다른 파일의 낡은 위치 보정 | 다음 |
...
### 8.2m LSP 2단 ⑫ — 구현·타입 정의·선언으로 (2026-09-22, 계획 공격 뒤의 결정)

**계획 공격이 드러낸 것.** ① §8.2l 의 피커가 「위치 목록 → 0 은 알림 · 1 은 이동 · 2+ 는 피커」를 이미 세웠으므로 세 명령은 **method 만 다르다** —
표면·이동·되묻기·root 밖 규율을 그대로 쓰고 새 판정 축을 만들지 않는다. ② **실측(2026-09-22, JSON-RPC 프로브)**: rust-analyzer 는 셋 다 provider 를
내고 `implementation` 이 trait → impl **둘**(피커의 첫 실사례), `typeDefinition` 은 `Box<dyn Shape>` 에서 **툴체인 소스**(`~/.rustup/…/boxed.rs`) 셋을 낸다 —
root 밖 행이 실제로 선다(§8.2l 「루트 밖」 제목·고르면 알림) · tsgo 는 `declarationProvider` 가 **없고** 그래도 물으면 `-32600 InvalidRequest` → **capability 로
막는다**(없으면 무동작 + 알림 「이 서버는 지원하지 않습니다」) · clangd 는 `declaration` 이 헤더(`shape.h`)로 간다. `LocationLink[]`(tsgo)·`Location[]`
(둘) 다 온다 — `locationsFromResult` 가 이미 받는다. ③ **키**: VS Code 의 `⌘F12`(Go to Implementation)는 편집기 컨텍스트 표에서 ⑵ `⌘` 예외인데 `ETX4` 가
*"예외는 반드시 편집 가능한 문서를 요구한다"*(비교 뷰에서 전역을 가로채지 않게) 고 정해 두었고 이동은 읽기 전용에서도 되어야 한다 — 모순이라 **키를 안 맨다**.
셋 다 팔레트 명령만(「Editor: Go to Implementation / Type Definition / Declaration」). 키가 필요해지면 그 절의 규율을 먼저 고친다.

| 축 | 결정 | 근거 |
| --- | --- | --- |
| **트리거** | `goto_implementation`·`goto_type_definition`·`goto_declaration` — 팔레트만. caret 자리 offset | ③ |
| **capability** | `textDocument.{implementation,typeDefinition,declaration}{linkSupport: true}`. 서버의 `{implementation,typeDefinition,declaration}Provider`(bool·객체)를 읽어 **없으면 요청하지 않고** 알림 `nav_unsupported`(어느 명령인지 이름을 싣는다) | ② tsgo 의 `-32600` |
| **요청** | id 칸 `13e8`(구현) · `14e8`(타입 정의) · `15e8`(선언) — §8.2a 표에 셋 더. 응답은 지금 기다리는 seq 일 때만; 되묻기(`content modified`)·0/1/2+ 처리는 §8.2l 의 `editor_references` 가 **그대로**(요청 종류만 든다) | ① |
| **피커** | 같은 `reference_picker`·같은 오버레이. 프롬프트는 명령별(「구현 N개」·「타입 정의 N개」·「선언 N개」) | ① |
| **하지 않는 것** | `⌘F12` 키(③) · 세 결과의 병합 뷰 · peek | 다음 |

**구현이 계약에 되먹인 것.** ① 캡처 실측(rust-analyzer): 구현 → 「구현 2개」 피커(`impl Shape for Circle {` :8 · `Square` :12), 타입 정의 →
「타입 정의 3개」에 **루트 밖 둘**(`…/library/alloc/src/boxed.rs`·`alloc.rs`)이 §8.2l 규율대로 제목 「루트 밖」으로 선다 — 그 행을 고르면 알림. ② 판정자
픽스처(`SmtFixture`)가 `git_repo` 에 `open()` 의 스택 버퍼를 넘기고 있었다 — 이동을 안 하는 SMT·FLD·SAV 에선 안 보이다가 `REF2` 의 `withinNavRoot`
가 죽은 메모리를 「루트 밖」으로 읽었다 → 픽스처가 root 를 소유하고 `ready()` 가 자기 주소로 다시 건다(값 반환이라 `open` 안에서 잡으면 사본의 주소다).

**관측점**: `LSJ19`(순수: capability 셋·요청 셋·id 칸 13~15e8·classify·provider 파싱) · `REF2`(제품 경계: 가짜 서버 — 구현 둘 → 피커(프롬프트 「구현 2개」) ·
타입 정의 하나 → 이동 · 선언 provider 없음 → 요청 0·알림 · 종류가 갈린 응답은 서로 안 섞인다 · 구현 0 → 구현의 문구 · 서버 없음은 무동작) · `REF3`(provider 를
내는 서버에선 선언 요청이 나간다). 캡처 훅 `MARU_FORCE_REFERENCES=<implementation|type_definition|declaration>`.

**적대적 검증(2026-09-22, 1~5회차 · 변이 18)**: 1회차 순수 8 → 0 · 2회차 제품 6 → 1(+컴파일 오류로 죽은 둘은 의미 변이로) · 3회차 배선 4 → 1 · 4회차 3 → 1 ·
5회차 1 → 0. 판정자 보강 둘, 등가 하나:
- **B3** 서버 없음도 「미지원」으로 알림 — `lsp.enabled = false` 에서 알림 0·요청 0 을 잰다(다른 LSP 명령과 같은 「무동작」).
- **B6** 없음 알림이 종류를 모름(참조 문구) — 같은 **수**로는 안 갈려 `notice.message == impl_none` 을 잰다.
- **C1** 종류별 seq 를 references 의 것과 나눠 씀 — 응답 대조는 `(칸, seq)`+`waiting_kind` 라 관측이 같다 → 등가, id 표의 뜻으로 두고 주석.

**적대적 검증 6~12회차(사용자 요청 「5회 반복」 · 새 변이 28 — rpc 가장자리 6 · 되묻기·종류 결합 6 · 배선 6 · 픽스처·훅·가짜 서버 5 · 응답 상태·프롬프트 5)**:
6회차 6 → 0 · 7회차 6 → 3 · 8회차 6 → 2 · 9회차 5 → 3 · 10회차 5 → 1 · 11회차 재실행 9 → 6 · 12회차 재실행 6 → 3(등가 셋). 판정자 보강 여섯:
- **E4** 미지원 알림의 명령 이름 — 알림 열림만 재고 문구를 안 봤다 → `nav_unsupported` 에 「Go to Declaration」이 든 문구를 비교.
- **E6** 구현에서의 되묻기 — `REFBUSY` 가 참조에만 걸려 있었다 → 가짜가 세 method 에도 `-32801` 을 내고, 구현이 상한(8)까지 되묻은 뒤 구현의 문구.
- **E2** 되묻기 예약이 익었는데 요청을 못 보내는 갈래(서버가 꺼짐) — 상한 경로와 다른 문구 자리 → `lsp.enabled = false` 뒤 tick 으로 그 알림이 구현의 문구.
- **F2** implementation provider 를 typeDefinition 키로 읽음 — 가짜가 둘 다 내어 안 갈렸다 → `REF4`(`MARU_FAKE_LSP_NOTYPEDEFCAP=1`: 타입 정의만 없는 서버).
- **F5** `typedef_none` 이 `impl_none` 문구 — 영문 표만 비교해 **한국어 표**를 바꾼 변이가 살았다 → `tIn(.en/.ko)` 로 두 언어 표 모두, 참조·선언과도 갈림.
...
**적대적 검증(2026-09-23, 1~5회차 · 변이 42)**: 1회차 순수 12 → 생존 2 · 2회차 제품 9 → 4 · 3회차 배선 11 → 3 · 4회차 재실행 10 → 2 · 5회차 재실행 2 → 0.
**보강한 판정자 일곱**: `DSY7`(같은 byte 시작이면 넓은 것이 먼저 — 뒤집히면 체인이 거꾸로) · `DSY8`(유효한 `selectionRange` 가 「범위 안 첫 등장」을
이긴다 — 주석에 같은 이름이 앞서 있는 자리) · **`LSJ20`**(`initialize` JSON 의 **모양** — `valueSet` 이 정수 배열, `initialized.params` 가 객체: 세션을
두 번 죽인 그 결함을 판정자가 잡게 했다) · `DSY4` ⑶⑷(두 층이 **다른 답을 내는 자리**로 체인을 잰다 — 가짜의 범위는 그 줄, 1층 함수 범위는 본문 전체) ·
`DSY4` ⑹(대기 중 편집 → 낡은 응답 버림) · `DSY5` ⑷(가짜 `DSYERR` — 오류 뒤 조용 시계) · `DSY9`(문서 둘: 응답은 그 seq 를 기다린 Term 에만) ·
`DSY10`(1층이 비는 문서에서도 피커가 선다). **등가 셋**(코드 주석): `fresh` 의 version 검사와 `onEdit` 의 `version = 0`(편집이 목록을 비우므로 이중
방어) · 가짜가 순서를 지켜 내는 변이(정렬은 순수 판정자가 잰다). 컴파일 오류 꼴 다섯(미사용 변수)은 의미 변이로 바꿔 다시 돌렸다.

**관측점**: `DSY1~DSY3`·`DSY6~DSY8`(순수: 평탄화·포함 depth·정렬·자기 검산·빈 `selectionRange` 회복·provider·평탄 꼴·상한) · `DSY4·DSY5·DSY9·DSY10`(제품 경계: ready 뒤 한 번 묻고 밴드 체인·피커가 2층으로 서며,
...
**구현이 계약에 되먹인 것(2026-09-23).** ① **caret 이 낱말에 «닿아» 있으면 그 낱말이다** — 실서버 캡처에서 `scale|`(낱말 끝 바로 뒤, 타이핑 뒤의 보통
자리)에 강조가 안 섰다: `wordRangeAt` 은 더블클릭 규칙이라 그 자리의 **뒤** 글자를 본다. 뒤가 낱말이 아니면 앞 글자를 본다(`wordAtCaret`). ② **낱말 밖으로
나가도 비우지 않는다** — 처음엔 비웠는데 `version`·낱말 키가 남아 같은 낱말로 돌아오면 「이미 답이 있다」로 판정돼 **영영 안 떴다**(적대적 B9 가 드러냈다).
그리기는 `spans()` 의 낱말 대조가 이미 가리므로 캐시를 남기고, 돌아오면 요청 없이 곧바로 선다. ③ 캡처 실측: 파라미터 `scale` 과 사용처 둘 다 배경이
`(32,32,32)`(바탕 `(16,16,16)`), 다른 낱말은 그대로 — 가장 약한 배경이라는 계약이 화면에서도 성립한다.

**적대적 검증(2026-09-23, 1~4회차 · 변이 38)**: 1회차 순수 6 → 생존 2 · 2회차 제품 10 → 7(컴파일 셋 포함) · 3회차 배선·렌더 9 → 4 · 4회차 재실행 13 → 2.
**결함 하나**(위 ②). **판정자 보강**: `OCH1` 시작 순 ≠ 끝 순인 쌍(A6) · `OCH2` 서로 다른 범위로 상한(A5 — 같은 범위만 주면 중복 제거가 가렸다) ·
`OCH3` 편집 직전 값과 비교한 비움 수·저장소 자체가 비었는가(B7 — 앞 갈래가 카운터를 이미 채웠다) · 낱말 밖→복귀는 요청 없이 다시 선다(B9) ·
**그려진 quad** 의 `occurrence` 색·알파를 센다(C5·C8 — 마크 빌더를 직접 부르면 배선이 안 잡혔다) · `OCH4` 비교 뷰 · **utf-16 협상에서 비ASCII 앞의
낱말**(C4 — 가짜가 협상한 인코딩을 따르게 고쳤다: 기본은 utf-8 이라 ASCII 픽스처로는 byte = 글자였다). **등가 둘**(코드 주석): 비교 뷰 가드(`readyClientFor`
도 거절 — 이중 방어) · 선택과의 그리기 순서(선택이 있으면 강조를 안 그려 둘이 공존하지 않는다). 판정자 이름은 `OCH` 다 — `DHL` 은 비교 뷰 판정자가 이미 쓴다.

**되먹임(2026-09-23, sticky scroll 작업 중 발견).** 프레임의 op 합계에서 **같은 낱말 강조 몫(`occ_ops`)이 빠져 있었다** — 뒤 층의 자리는 그 몫을
세는데 반환 개수는 안 세서, 강조가 N 개면 **끝 op N 개(세로·가로 막대·미니맵의 끝)가 잘렸다** — 강조가 선 동안 막대가 사라졌다. `OCH3` 는 강조 quad
만 세고 막대는 안 봤다. `OCH5`(표식 하나가 op 를 정확히 하나 늘리고 마지막 op 는 막대다)가 고치기 전 빨갰다(`expected 25, found 24`).

**관측점**: `OCH1~`(순수: 응답 → 범위 목록, 정렬·중복 제거·상한·utf-16·provider) · `OCH3~`(제품 경계: caret 이 낱말에 멈추면 묻고 마크가 서며,
...
**구현이 계약에 되먹인 것(2026-09-23).** ① **1층의 트리 조회는 구현 전에 반증했다** — 저장소가 한 번도 부르지 않은 `ts_node_parent`·
`ts_node_descendant_for_byte_range` 가 zig·TS·C 에서 사슬을 세우는지 `SYN42` 로 먼저 쟀다(초록). ② 계획 공격 중 「트리는 최신이거나 없다」를
공격하다 **여는 파싱이 끊긴 채 편집이 오면 트리가 틀어지는 기존 결함**이 드러나 #3886 으로 먼저 고쳤다([layering](native-editor-layering.md)
§2.1a 되먹임) — 이 절의 「`pending` 이 아닐 때만」은 그 뒤에도 유지한다(끊긴 동안 트리는 없다). ③ **실서버 캡처(tsgo)**: `this.r` 의 `r` 에서 세 번 →
`this.r * this.r`, 다섯 번 → `Math.PI * (this.r * this.r)`; `lsp.enabled = false` 로 세 번 → 같은 `this.r * this.r`(1층). 파일 계측으로 tsgo 가
13 단 사슬을 주고 세 걸음이 차례로 서는 것을 확인했다. ④ **판정자가 조용히 SKIP 이었다** — 여는 파싱 중 1층을 재는 판정자가 세 줄 문서라 1ns
예산으로도 안 끊겨 건너뛰었다(`SSEL15` — 문서를 늘리고 끊김을 단언한다). ⑤ 변이 전 판정자 대조에서 **못 잡을 갈래 일곱**을 찾아 먼저 세웠다:
낱말 끝 caret 의 물을 자리(`SSEL16`) · utf-16 비ASCII(`SSEL17`) · undo 묶음(`SSEL18`) · caret 드러내기(`SSEL19`) · primary 유지(`SSEL13`) · 사슬·조상
상한(`SSEL20`·`SYN43`) · 비교 뷰의 팔레트 경로(`SSEL21`).

**적대적 검증(2026-09-23, 1~3회차 · 변이 39 + 다시 쓴 6, 판정 `zig build test-editor`).** 변이 전 대조에서 판정자 일곱을 먼저 세웠다(위 ⑤).

| 회차 | 변이 | 결과 |
|---|---|---|
| 0 | 러너 | **무효** — 150 변이 선례처럼 `.zig-cache/o` 가 디스크를 채워 둘째 변이에서 멈췄고, 파일을 비우고 쓰는 되돌리기가 변이 하나를 트리에 남겼다 → `git checkout` 되돌리기 · 여유 25 GB 밑이면 캐시를 비우는 러너로 다시 |
| 1 | 사슬 11 · 해독 4 · rpc 2 · 요청 2 · 상태 15 · 키 1 · 트리 3 | **죽음 27** · 컴파일 거절 6(변이가 변수를 안 쓰게 만들었다 → 2회차에 의미 변이로) · **생존 7** |
| 1 | **D20** 상태 없는 축소가 사슬을 세움 | 판정자 구멍 — 축소가 서버에 묻고 답이 오면 **넓힌다**, 답이 비동기라 선택만 보면 제자리였다 → `SSEL10` 이 「축소는 묻지 않는다」를 단언 |
| 1 | **D21** 시간 초과 500 → 5000 ms | 판정자 구멍 — 판정자가 상수를 읽어 기다렸다 → `SSEL11` 이 계약 값(580 ms 에 선다 · 300 ms 에는 아직)으로 잰다 |
| 1 | A1 기준 포함 거름 · A10 첫 루프 상한 · T3 조상 포함 방어 · D3 `pending` 가드 · D9 합친 뒤 상태 | **등가 다섯**(코드 주석) — 사슬은 앞 단계를 품는 것만 잇는다 · 줄 단계 루프가 같은 상한으로 다시 자른다 · 조상은 늘 품는다 · #3886 뒤로 「`pending` 이면 트리가 없다」 · 「사슬 수 = 커서 수」 대조가 다시 거른다 |
| 2 | D20 · D21 · 다시 쓴 A5·A9·B1·B3·D17·T1 | 죽음 — D20 \`SSEL10\` · D21 \`SSEL11\` · A5 \`SSEL3\` · B1 \`SSEL7\`·\`SSEL11\` · B3 \`SSEL6\`·\`SSEL17\` · D17 \`SSEL13\`. **A9 등가**(공백뿐인 줄 — 줄 단계의 「줄 전체」가 같은 범위를 끼운다). **T1 생존** — 조상 사슬의 같은 범위 거름을 빼도 초록: 픽스처에 부모·자식이 같은 범위인 쌍이 없었다 → \`SYN42\` 에 끝 개행 없는 문서(뿌리 = 선언) — 그 쌍이 있는지 먼저 단언한다 |
| 3 | 기준선 · T1 | 죽음(\`SYN42\`) — **전부 죽거나 등가, 등가 여섯**(A1·A9·A10·T3·D3·D9, 코드 주석) |

**관측점**: `SSEL1~5`·`SSEL20`(순수 사슬: 기준·안쪽부터·교차·줄 단계·낱말 단계·하위 낱말·상한) · `SSEL6~8`(해독·provider·길이 불일치·깊이·utf-16·
id 칸) · `SYN42`·`SYN43`(tree-sitter 조상 사슬·상한) · `SSEL14`(키 표 — 비교 뷰는 컨텍스트가 진다) · `SSEL9~13`·`SSEL15~19`·`SSEL21`(제품 경계 —
키 이벤트 경로: 한 요청·걸음·축소·이동/편집 무효화·1층 여섯 갈래·대기 순 걸음·낡은 답·멀티 커서 합치기·primary·낱말 끝 caret·utf-16·undo 묶음·caret
드러내기·비교 뷰 팔레트). 카운터 `editor_smart_select.{sent, applied, dropped_stale, dropped_error, timeout_fallback, layer1_only}`·
`editor_lsp.{sent_selection_range, received_selection_range}`. 가짜 서버 표식 `SSRNONE`·`SSRSTALL`·`SSRERR`·`SSRSHORT`, `MARU_FAKE_LSP_NOSRCAP=1`.
캡처 훅 `MARU_FORCE_SMART_SELECT=<N>`(서버가 문서를 연 뒤 N 번 — `lsp.enabled = false` 면 1층을 곧바로).
