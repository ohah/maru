...
- native는 write 직전에 disk fingerprint를 다시 비교한다. 다르면 **쓰지 않고** conflict를 반환한다.
  ...
  - **그래서 지금 이 상자에 선 것은 둘이다** — `primary`=덮어쓰기 · `alternate`=다시 읽기 ·
    `cancel`(Esc·닫기)=계속 편집. 둘 다 무언가를 버리므로 **열 때 포커스를 `cancel` 에 둔다**(그
    상자만의 예외이고, 컴포넌트 기본값은 `primary` 그대로다). **비교가 더해지는 날** 그것이 `primary`
    로 올라가고 그 예외는 사라진다 — 그때 필요한 것이 위의 네 번째 자리다.
