...
이 문서는 Maru에 코드 에디터 surface를 추가하는 설계의 단일 출처다(단계와 진행은 [구현 계획](plans/editor-surface.md)이 소유한다). 첫 제품 가치는 에이전트가 만든 변경을 사용자가 Maru 안에서 검토하는 것이고, 이후 같은 문서 모델 위에 편집·저장·포맷/린트·LSP를 얹는다.
...
- native는 write 직전에 disk fingerprint를 다시 비교한다. 다르면 **쓰지 않고** conflict를 반환한다.
  ...
  - ⚠️ **다만 «두 쪽을 채우는 길」은 새것이다**(적대 검증에서 확인). 지금 제품 경로에서
    ...
    **(메모리 버퍼, 디스크 바이트)** 라 git 이 관여하지 않으므로, 채우는 갈래와 **소유·해제 규칙**을
    ...
    이 갈래가 없다.
    - **소유 규칙은 «하나로 남긴다»**(2026-09-22 코드 대조). 해제하는 쪽(`freeDiffContent`)이 쓰는
      `git_backend.worker_allocator` 는 **전역 `std.heap.smp_allocator`** 다 — 워커 스레드 전용 arena 가
      아니라 어느 스레드에서도 쓸 수 있는 할당기다. 그러므로 새 갈래는 **그 할당기로 두 쪽을 만든다**:
      소유자 표시 필드를 새로 더하면 해제 규칙이 둘이 되고, 둘이 되면 한쪽이 낡는다.
    - ⚠️ **새로 고치는 자리가 이미 둘 있고, 둘 다 git 을 부른다.** 파일이 또 바뀌면
      `fileChanged` 가 그 entry 에 `requestDiffContent` 를 걸고, tick 폴링도 아직 못 건 요청을 다시
      건다 — 저장 충돌 비교가 **git 내용으로 덮인다.** 그 두 자리는 이 갈래를 알아보고 **자기 방식으로
      (디스크 다시 읽기 + 버퍼 다시 복사)** 새로 고쳐야 한다.
    - ⚠️ **이미 열린 비교는 내용을 갱신하지 않는다.** `openDiffTerm` 은 같은 `(경로, kind, base)` Term 이
      있으면 **활성화만 하고 돌아간다** — 두 번째 저장 시도가 첫 번째의 비교를 보여 준다. 이 갈래는
      열 때마다 두 쪽을 다시 채운다.
    - **새로 고치는 자리는 실은 하나다**(2026-09-22 코드 대조). 위의 두 자리(`fileChanged`·tick 폴링)가
      전부 `requestDiffContent` 를 부르므로, 그 함수 **머리에서** 이 갈래를 가르면 둘이 함께 닫힌다.
      그 함수는 지금 `diff_repo` 가 비면 실패로 표시하는데, 이 비교에는 저장소가 없다 — 그래서 갈라야
      한다는 사실 자체가 그 검사에 이미 적혀 있다.
    - **두 쪽은 「그 채움의 순간」이다**(열었을 때가 아니다). 채울 때 디스크를 읽고 버퍼를 복사하므로,
      파일이 또 바뀌면 그 두 쪽이 함께 새로워진다 — 한쪽만 새로워지면 화면이 두 시점을 섞어 보여 준다.
    - ⚠️ **버퍼 쪽의 주인을 «경로»로 찾지 않는다.** 문서 Term 은 `entry` 가 없을 수도 있고
      (`openPathInActivePane` 로 연 것), 같은 경로를 다시 열면 다른 Term 이 된다. 비교 entry 가 **그
      문서의 `surface_id`** 를 들고, 그 Term 이 없으면 **비교를 실패로 표시한다** — 주제가 사라졌다.
    - **왼쪽이 디스크, 오른쪽이 내 편집이다** — git 비교와 같은 방향(왼쪽=바깥, 오른쪽=내 것)이라
      사용자가 이미 배운 방향이 유지된다.
    - **비교는 화면이 둘이어도 «채우는 자리」는 하나다.** 네이티브 비교 뷰가 기본이지만
      (`MARU_NATIVE_DIFF=0` 이면 CM6 웹 Term) **둘 다 같은 entry 두 쪽을 읽는다** — 그래서 이 갈래는
      두 화면에서 함께 선다.
    - **비교 Term 은 영속되지 않는다**(저장 시퀀스가 `.diff` 를 빼 둔다) — 재시작 뒤 되살아나 없는
      버퍼를 가리키는 일이 없다.
    - ⚠️ **문서 Term 이 사라지는 «그 순간» 비교를 실패로 만든다**(2026-09-22 적대적 2회차). 새로 고침을
      기다릴 수 없다: tick 폴링은 `diff_ready` 인 entry 를 건너뛰고, 파일이 또 바뀌지 않으면
      `fileChanged` 도 오지 않는다 — 그러면 **사라진 편집이 「내 편집」으로 영영 남는다**.
    - ⚠️ **실패하는 갈래에서도 두 쪽을 비운다.** 낡은 내용을 들고 「실패」라고만 말하면 화면은 있지도
      않은 비교를 계속 보여 준다(파일이 지워진 경우가 그 자리다).
  ...
  - 아래 줄의 `reload/keep/compare` 는 **watcher 가 변경을 알렸을 때**의 선택이고 이것은 **저장을 누른
    순간**의 선택이다 — 두 자리가 다르지만 「사용자가 고른다」는 같다.
  - ⚠️ **셋을 담으려면 확인 상자에 «네 번째 자리»가 필요하다**(2026-09-22 코드 대조). 지금 확인
    컴포넌트의 슬롯은 셋(`primary`·`alternate`·`cancel`)인데, **`cancel` 은 버튼이면서 동시에 Esc·바깥
    클릭의 갈래다**(`.confirm_cancel` 하나로 온다). 그 자리에 행동을 놓으면 **Esc 가 그 행동을
    실행한다** — 위의 「창을 닫으면 아무 일도 일어나지 않는다」와 정면으로 어긋난다. 그래서 선택 셋은
    「행동 세 자리 + 닫기」이고, 그 네 번째 자리를 만드는 일이 이 개정에 포함된다.
  - ⚠️ **Enter 는 `primary` 를 실행한다**(상자는 열 때마다 `primary` 에 포커스를 둔다 — 컴포넌트 계약).
    그러므로 **`primary` 에는 아무것도 버리지 않는 선택만 둔다** — 셋 중 그것은 **비교**뿐이다.
    무심한 Enter 가 바깥 변경이나 방금 친 것을 지우면 그것은 확인 상자가 아니라 함정이다.
  - ⚠️ **비교를 골라도 답은 아직 «안 한» 것이다.** 상자를 닫고 비교 탭을 열 뿐 **디스크도 버퍼도
    그대로**이고 문서는 dirty 로 남는다 — 사용자는 보고 나서 다시 `⌘S` 를 눌러 고른다. 상자를 비교
    위에 띄워 둘 수는 없다: 모달이 그 화면을 읽지 못하게 막는다.
  - **그래서 이 상자는 네 자리다** — `primary`=**비교**(Enter 가 실행하므로 아무것도 버리지 않는 것이
    그 자리에 선다) · `alternate`=덮어쓰기 · **`extra`=다시 읽기**(컴포넌트의 네 번째 자리, 마커 `R`) ·
    `cancel`(Esc·닫기)=계속 편집. 포커스는 컴포넌트 기본값(`primary`)이고 **예외가 없다**.
  - ⚠️ **「다시 로드」는 «되돌릴 수 있는 편집»으로 만든다.** 문서 버퍼를 통째로 갈아치우면 위의
    「통짜 model 교체로 undo/cursor 를 깨지 않는다」를 이 절이 스스로 어긴다 — 전체 선택 + 삽입과 같은
    **평범한 편집 하나**로 넣으면 `⌘Z` 가 사용자의 편집을 되살린다. 그리고 그 편집 뒤 저장 상태는
    **clean 이다**(디스크와 같은 내용이므로 `saved_hash`·디스크 지문을 그 내용으로 맞춘다) — 편집으로
    넣었다는 이유로 dirty 로 남으면 사용자가 방금 「디스크를 받아들였다」고 한 것과 화면이 어긋난다.
  - **고른 순간의 내용으로 쓴다.** 상자가 떠 있는 동안에도 사용자는 계속 칠 수 있으므로, 덮어쓰기는
    **누른 시점의 버퍼**를 읽는다(충돌이 난 시점의 사본을 들고 있지 않는다). 들고 있으면 그 사이 친
    글자가 조용히 사라진다.
  - **대상이 사라졌으면 상자도 사라진다.** 그 Term 이 닫히면 보류한 선택을 함께 버린다 — 안 버리면
    보이지 않는 모달이 키를 먹고, 되살아난 선택이 **남의 문서**를 덮어쓴다(U2 에서 같은 부류의 결함을
    겪었다).
...
10. **유지 확인(2026-08-14 2차 결정) — 섹션은 그대로 비교 기준이다.** 같은 날 오전 판정(추적 여부 2섹션 + 체크박스)은
    **하루 안에 뒤집혔다.** 스테이징을 체크박스가 아니라 **행 호버의 `+`**로 하기로 하면서(VS Code 목업), "누르면 그
    행이 위 그룹으로 올라간다"가 곧 피드백이 되어 그룹이 기준으로 갈려 있어야 하기 때문이다. 그래서 2판의 그룹은
    `스테이지된 변경`(`HEAD↔index`)·`변경 사항`(`index↔작업트리`, 추적되지 않은 파일 포함) 둘이고, 아래 원문이
    그대로 유효하다([§3.5.2](editor-surface-dock.md)). 잠시 존재했던 `DiffBase.head_worktree`는 함께 제거했다.

    **섹션이 곧 기준이다 — 전역 기본 base도, base 선택기도 두지 않는다(2026-07-31).** 행이 속한 섹션이 비교 기준을
...
15. **도크 소스 컨트롤 2판 = 탭 3개 + 쓰기 포함(2026-08-14 사용자 결정, 옛 §10.14).** 스테이징 UI를 "언제 켤지"를
    묻던 항목이 닫혔다 — **지금 켠다**. 결정된 내용은 넷이다. ⑴ 화면은 `변경 사항 · 히스토리 · 에이전트` **탭 3개**,
    ⑵ 변경 사항 탭의 시각 기준은 **VS Code 목업**이다 — 그룹은 `스테이지된 변경`/`변경 사항`이고 스테이징은
    **행 호버의 `+`**다(체크박스가 아니다. 같은 날 오전의 체크박스 판정은 2차 결정으로 뒤집혔다), ⑶ stage/unstage/commit/fetch를
    포함하되 `push`/`pull`은 터미널에 명령을 넣어 주는 방식, ⑷ 렌더 기반을 터미널 셀 그리드에서 `chrome/ui` typed
    tree로 옮긴다. 시각 계약은 [§3.5](editor-surface-dock.md), 쓰기·원격 안전 계약은
    [§3.6](editor-surface-dock-write.md), 단계는 [plans/scm-dock.md](plans/scm-dock.md)가 소유한다.

    **`git_write` grant는 `git_read`와 분리해 유지한다**(옛 질문의 나머지 절반). 다만 도크는 native chrome이라 어느
    grant의 대상도 아니고, 그 플래그는 **웹 브리지 축**의 것이다 — 지금 어떤 웹 표면도 쓰기 명령에 닿지 않는다.
...
14. → **10.A로 이동(2026-08-14 결정).** 아래 §10.15를 본다.
