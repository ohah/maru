...
10. **개정(2026-08-14) — 섹션은 더 이상 기준이 아니다.** 도크 2판이 섹션을 **추적 여부**로 바꾸고 스테이지 여부를
    **체크박스**로 옮기면서, 아래 규칙의 전제(같은 파일이 기준마다 다른 행에 선다)가 사라졌다. 2판의 기본 비교는
    `HEAD ↔ 작업트리` 하나이고, 스테이지 쪽·미스테이지 쪽 비교는 컨텍스트 메뉴의 부가 항목이다
    ([§3.5.2](editor-surface-dock.md)). **전역 base 선택기를 두지 않는다는 결론은 그대로다** — 바뀐 것은 기준을
    정하는 주체가 섹션에서 "기본값 + 부가 항목"으로 옮겨 갔다는 점이다. 아래는 초판 원문이다.
...
15. **도크 소스 컨트롤 2판 = 탭 3개 + 쓰기 포함(2026-08-14 사용자 결정, 옛 §10.14).** 스테이징 UI를 "언제 켤지"를
    ...
    ⑵ 변경 사항 탭의 섹션은 **추적됨/추적되지 않음**이고 스테이지 여부는 체크박스, ⑶ stage/unstage/commit/fetch를
