...
6. **상한은 바이트로만 건다(2026-08-01 실측 후 갱신).** 한쪽 1 MiB를 넘으면 typed `too_large`와 external-open
   fallback을 준다(§6). **"diff 페이지 500행"은 페이지네이션 단위이지 표시 상한이 아니다** — 초판 구현이 그걸 상한으로
   써서 500줄 넘는 파일(대부분)이 안 열렸다. 제품 WKWebView 실측: 2만 줄 문서에서 10%가 다른 최악의 경우도 마운트
   **229 ms**, 1천·5천 줄은 ~100 ms다(CM6가 화면 밖을 그리지 않는다). 바이트 상한이 1 MiB면 보통 코드로 2만 줄
   남짓이라 그 측정 범위 안에 든다.
