...
- **`.gitignore` 흐림**(2026-08-18 사용자 결정): 방금 열거한 디렉터리의 항목들을
  ...
  **B 의 행들을 지운다** — 트리를 펼치면 형제들의 흐림이 풀린다. 「모르면 흐리게 하지 않는다」가
  ...
  ⚠️ **묻는 것도 탐색기를 보고 있을 때만이다 — 안 볼 때는 «적어 둔다».** 스캔 결과 드레인은 뷰와
  무관하게 매 tick 도므로, 게이트가 없으면 소스 컨트롤 탭에 있거나 도크를 닫아 둔 동안에도 디렉터리
  스캔마다 git 프로세스가 뜬다. **그리고 반대쪽에 결함이 있었다**: 백엔드는 탐색기 진입에서만 생기니,
  도크를 소스 컨트롤로 두고 쓰다가 탐색기로 들어오면 트리는 이미 다 읽혀 있고 새 스캔이 없어
  **아무도 묻지 않는다** — 흐림이 영영 안 뜬다. 안 볼 때 적어 두면 들어올 때 tick 이 그 디렉터리를
  다시 걸고, 그 스캔의 드레인이 그때 묻는다.
  ...
  거절된 디렉터리는 적어 두고, **탐색기를 보고 있는 동안** tick 이 백엔드가 한가해졌을 때 **하나씩**
  다시 스캔을 건다(`retryPendingIgnore`). ⚠️ **가시성 게이트가 계약의 일부다** — 이 재시도는 디스크
  스캔을 거는 일이고 흐림은 탐색기 뷰에서만 쓰이므로, 다른 뷰이거나 도크가 닫혀 있으면 **안 돈다**
  (목록은 그대로 남아 돌아오면 이어 건다). 목록 읽기를 `dock.view == .source_control` 로 거는 것과
  같은 규율이다. 「재요청은 tick 이 소유한다」는 오래된 규율이고 diff 재요청이 같은 자리에
