...
- **detached worker 수명 — 제품과 판정자가 다르다(2026-09-08, CI 가 잡은 것)**: 위 경계의 「backend 교체/세션 종료는 느린 FS worker 를 main actor 에서 기다리지 않는다」는 **제품 계약이고 그대로 둔다**(멈춘 I/O 로 창 닫기가 굳는 것이 훨씬 나쁘다 — `Backend.deinit` 과 그것을 재는 판정자 「retirement releases owner without waiting for worker generation」이 단일 출처다). 그런데 **판정자에서는 그 대가를 다른 판정자가 문다**: 스캔을 던져 놓고 세션이 끝나면 아직 도는 job 의 경로 사본이 그 순간 **누수로 잡히고**, 그 워커가 뒤이어 해제된 자리를 만지면 누수 트레이스를 찍는 도중에 죽는다. 실제로 CI 샤드가 `dupe` 누수 한 줄 → `writeTrace` 에서 Segmentation fault → 종료 코드 134 로 abort 했다.
  ...
  - 처방은 **`detached_worker_wait.quiet`**(참조수가 1 로 떨어질 때까지 벽시계 상한 30 초 대기)를 **`AppSession.deinit` 한 자리**에서 세션이 든 backend 전부에 부르는 것이다(파일 트리·트리 변경·git·에이전트 세션 아카이브 셋). 이 backend 들을 쓰는 판정자가 수십 개인데 규율을 그만큼 나눠 두면 새로 쓰는 사람이 반드시 빠뜨린다. workspace 교체는 물러난 backend 가 세션보다 오래 남을 수 있는 두 번째 자리라 거기서도 부른다. 이미지 스캔·디코드 backend 는 **제품 `deinit` 에서 이미** 워커를 거두므로 목록에 없다.
  ...
  - 상한을 넘기면 그냥 지나간다 — 그때는 누수가 **정직하게** 잡힌다(조용한 통과를 만들지 않는다). 벽시계로 재는 이유는 반복 상한(`spins < N`)이 부하 걸린 기계에서 먼저 끊어져, 정작 느려서 생기는 이 결함을 못 잡기 때문이다(IG14 와 같은 실패다).
  - 메커니즘 자체는 판정자 「판정자는 워커보다 오래 살면 안 된다」가 **기계 속도에 기대지 않고** 잰다: 항목 4,000 개짜리 디렉터리로 느린 스캔을 강제하고, **자기 `DebugAllocator` 로 그 자리에서 결산**한다(테스트 할당자를 쓰면 누수가 샤드 끝에서 이름 없이 보고되는 — 우리를 헤매게 한 바로 그 — 모양이 된다). 대기를 빼면 그 판정자가 자기 줄에서 죽는다(실측).
  - **새 backend 를 세션에 달면 `quietDetachedWorkersForTest` 에 한 줄 더한다.** 빠뜨리면 빠른 기계에서는 아무 일도 안 일어나고, 느린 CI 에서만, 그것도 엉뚱한 판정자 이름으로 터진다.
