...
창 레벨 도크의 **탐색기 view**(프로젝트 트리)의 단일 출처 문서다. 트리의 root 모델·스캔·감시·선택·키보드 탐색·파일 변경 명령을 소유한다. **도크는 view 를 셋 담고**(§3.5) 이 문서가 소유하는 것은 그중 하나다 — 소스 컨트롤은 [editor-surface-dock.md](editor-surface-dock.md) §3.5 가, AI 세션은 [에이전트 세션 기록 도크](agent-session-list.md)가 소유한다.
...
>
> **실제 앱에서 확인했다(2026-08-31).** 네 경로를 눈으로 봤다 — 시작 cwd 가 root, `cd src` 가 저장소
> 루트로 올라가지 않고 그 폴더를 root 로 세움, `cd ~` 가 홈을 root 로 세움, 그리고 **홈에서 하위로
> `cd` 하면 빠져나옴**(앞 계약이 갇히던 자리다). 이 축에는 GUI 자동 E2E 가 없어 1 회 관측이며, 무엇을
> 어떻게 확인했고 무엇이 여전히 안 재졌는지는 [검증 매트릭스](verification-matrix.md)의 "파일 패널 FP7"
> 행이 소유한다.
>
...
| 비-로컬 cwd | OSC 7을 안 쏘는 셸은 관측이 비고, `maru ssh` 원격 세션은 **로컬에 없는 경로**를 준다 | 검증 파이프라인이 거른다(정책 3) — 실패는 조용하고 root는 그대로다. **이 칸이 「미개발」의 자리다** — 원격 축은 [원격 파일 트리(RF)](plans/remote-file-tree.md)가 연다 |
...
- **슬롯 폭도 터미널 폰트와 무관하다 — 높이와 같은 계약의 나머지 절반이다.** 슬롯 하나는 `4 × 터미널 셀폭`
  고정이었고, 그래서 폰트를 키우면 요구 폭만 커지는데 도크 폭(pt 고정)은 그대로라 **어느 순간 스위처가 통째로
  사라졌다** — `slotRect`가 "폭이 모자라면 일부만 그리지 않는다"는 정책대로 null을 내고, 그러면 사용자는 도크 뷰를
  **아예 바꿀 수 없다**. 실측(2026-09-15): font 24pt에서 요구 224px > 바 180px(1×), 464px > 360px(2×) — **배율과
  무관하게** 재현되며, `macos-agent-session-archive-smoke`가 `timeout_enterAgentSessions`로 죽어 그 뒤 시나리오
  넷이 통째로 안 돌고 있었다.

  이제 슬롯의 셀 수를 `space.dock_view_slot_width_pt`(rich 32pt)에서 **반올림 환산**한다
  (`dock_view_bar.Grid`, `AppSession.dockViewBarGrid`). 값은 기존 외형을 보존하도록 골랐다 — 기본 폰트 14pt는
  1×·2× 모두 4칸이라 이 변경 전과 픽셀이 같고, 24pt에서만 2칸으로 줄어 스위처가 산다. tui는 token 0이라 셀
  파생 4칸으로 떨어진다(높이와 같은 규율).

  **셀 격자를 버리는 것이 아니다.** 아이콘은 여전히 열 번호로 놓이므로(`buildDockViewBarDrawList`) 슬롯도 셀
  배수여야 그린 자리와 눌리는 자리가 안 갈린다. 바뀌는 것은 "몇 칸인가"뿐이고, 그 칸 수를 렌더·hover·클릭·smoke
  probe가 **같은 `Grid` 하나**에서 받는다. 아이콘 시작 열도 상수가 아니라 슬롯 폭에서 파생한다
  (`Grid.iconOffsetCols`) — 상수 1로 두었더니 2칸 슬롯에서 `1+2=3 > 2`로 **아이콘이 이웃 슬롯을 침범했다**
  (적대적 검증이 셀 폭 1~64 전수에서 잡았다).
...
| AI 세션 | 코드 | 창의 **모든 탭**을 가로지르는 Codex·Claude 과거 세션 기록 | [agent-session-list.md](agent-session-list.md) |

**AI 세션 뷰 — 이 문서가 소유하지 않는다.** 뷰가 도크 안에 산다는 사실(뷰 바에 자리가 있고 §3.5 의 기하를 쓴다)만 여기
적고, 그 안의 계약은 [에이전트 세션 기록 도크](agent-session-list.md)가 **단독으로** 소유한다 — 목록의 발견·검색·재개·
보안, 카드 레이아웃, 선택·확장 geometry가 전부 그쪽이다. 두 문서가 같은 것을 각자 적으면 반드시 갈린다.

여기 남기는 것은 **왜 사이드바와 별개인가** 하나다: 사이드바가 워크스페이스 카드별로 나누는 것과 달리 이 뷰는 **창 전체를
한 목록**으로 낸다. 행 문자열 자체는 사이드바 에이전트 행과 **같은 출처**(마지막 사용자 프롬프트 우선 + 상태 마커)를 쓰고
그 표시 형태의 단일 출처는 [사이드바 에이전트 목록](sidebar-agent-list.md)이다 — 같은 것을 두 곳에서 다르게 부르지 않는다.
...
- **`.gitignore` 흐림**(2026-08-18 사용자 결정): 방금 열거한 디렉터리의 항목들을
  ...
  ⚠️ **한 배치는 «개수»가 아니라 «보낼 수 있는 바이트»가 정한다.** 경로는 stdin 으로 나가므로 실제
  상한은 페이로드 크기(`max_check_ignore_stdin_bytes`)이고, 개수 상한은 그 위에 얹는 안전판이다.
  담는 자리에서 바이트를 세어 끊지 않으면 **보낸 적 없는 경로가 「물어본 목록」에 섞이고**, 드레인이
  그 행들의 표시를 지운다(아무도 안 물어봤는데 흐림이 풀린다). 경로가 길고 항목이 많은 깊은
  디렉터리에서 실제로 닿는다 — 512 × 128 B 면 이미 넘는다.

  **그리고 한 배치에 안 들어가면 «이어서» 묻는다.** 한 디렉터리마다 **커서**(다음에 물을 자식 순번)를
  들고, tick 마다 백엔드가 한가할 때 다음 배치를 보낸다. 예전에는 첫 배치만 묻고 나머지를 버려서,
  항목이 많은 디렉터리는 **뒤쪽이 영영 판정 없이** 남았다.

  ⚠️ **비용은 줄 머리 점유다**(실측 기준, 512 개/배치·30Hz): 항목 1,000 개면 2 배치(0.1 초),
  10,000 개면 20 배치(0.7 초), 100,000 개면 196 배치(6.5 초) 동안 그 디렉터리가 줄 머리를 잡는다.
  그동안 뒤에 선 디렉터리는 **흐림 없음**으로 남는데, 그것은 「모름」이지 「틀림」이 아니다 —
  「모르면 흐리게 하지 않는다」가 그 자리를 지킨다. 상한을 더 두는 것은 **정책**이라 여기서 임의로
  정하지 않는다(그런 디렉터리를 펼치는 것은 사용자의 행동이다).

  ⚠️ **«물어본 목록»도 답이 들고 온다**(`IgnoreResult.asked`). 출력에는 무시된 것만 오므로, 무시가
  풀린 항목의 표시를 지우려면 물어본 전체가 필요한데 — 그것을 세션 버퍼로 읽으면 안 된다. 그 버퍼는
  디렉터리를 읽을 때마다 비워지고 덮이는 반면 백엔드는 답이 걸려 있는 동안 새 요청을 **거절**한다.
  그래서 「A 를 물어 둔 채 B 를 스캔」하면 버퍼는 B 것이 되고 요청은 안 나가며, 뒤늦게 온 A 의 답이
  **B 의 행들을 지운다** — 트리를 펼치면 형제들의 흐림이 풀린다. 「모르면 흐리게 하지 않는다」가
  아니라 **알던 것을 잊는** 모양이라, 같은 조항이 두 번 필요했다.
  ...
  실패하면 그 화면은 **판정 없이 남는다** — 모르면 흐리게 하지 않는다.

  ⚠️ **묻는 대상은 «트리가 들고 있는 그 디렉터리의 자식»이다 — 스캔 결과가 아니다.** 스캔 완료는
  「이 디렉터리를 다시 봐야 한다」는 **신호**일 뿐이고, 물어볼 목록은 트리가 소유한다(스캔이 방금
  `applySnapshot` 으로 넣어 준 바로 그것이다). 출처를 하나로 두면 네 가지가 따라온다:

  1. **이어 묻기가 가능하다.** 커서만 들면 되고, 배치 사이에 목록을 어디 보관할 필요가 없다.
  2. **거절되어도 다시 스캔하지 않는다.** 백엔드 자리가 차서 거절되면 다음 tick 에 **같은 커서로**
     다시 보낸다 — 디스크 I/O 0 이다. 예전에는 목록이 스캔 결과에만 있어서 그 디렉터리를 **다시
     읽는** 것 말고는 되살릴 방법이 없었다.
  3. **안 볼 때는 커서만 남는다.** 소스 컨트롤 탭에 있거나 도크가 닫혀 있으면 묻지 않고, 탐색기로
     들어오면 **그 자리에서** 묻는다(재스캔 없음). 백엔드가 탐색기 진입에서만 생기므로, 이 커서가
     없으면 「이미 다 읽어 둔 트리」에서는 아무도 묻지 않아 흐림이 영영 안 뜬다.
  4. **스냅샷이 안 실린 갈래는 자연히 안 묻는다.** 부모가 사라졌거나(`NotFound`) 신원이 어긋나면
     (`IdentityMismatch`) 트리에 그 자식이 없다 — 예전에는 **답이 닿을 곳도 없는데** 프로세스를 띄웠다.

  ⚠️ 자식 슬라이스는 **트리 소유**라 다음 스냅샷까지만 유효하다. 커서는 순번이므로 그 사이 목록이
  바뀌면 엉뚱한 자리를 가리킬 수 있는데, 새 스캔이 오면 그 디렉터리의 커서를 **0 으로 되돌리므로**
  (같은 신호가 같은 자리를 리셋한다) 갈리지 않는다.

  ⚠️ **`.gitignore` 가 바뀌면 «펼쳐 둔 것 전부»를 다시 읽는다.** 이 판정의 입력은 셋이다 — 디렉터리
  목록, 저장소, 그리고 **규칙 파일**. 앞의 둘은 바뀌면 다시 읽는 길이 있는데 규칙 파일에는 없었다:
  변경 알림은 `invalidatePath` 로 가고 그것은 **그 경로를 담은 가장 깊은 펼친 디렉터리 하나**만 다시
  읽는다. 규칙 한 줄은 이미 읽어 둔 **다른 디렉터리 전부**의 판정을 바꾸는데도 그랬다 — `web/dist/`
  를 더해도 펼쳐 둔 `web/` 은 밝은 채이고, 규칙을 지워도 흐림이 남는다. 접었다 펴기 전까지 **틀린
  것을 계속 말한다**(「모르면 흐리게 하지 않는다」와 다른 잘못이다).

  이름이 `.gitignore` 인 변경만 `invalidateExpanded` 로 접는다 — 중첩 규칙 파일도 같은 갈래를 타고,
  범위는 **펼친 것**으로 유계다. `.git/info/exclude` 는 여기 안 온다(`.git` 안쪽은 앞에서 git 신호로
  갈라진다). 전역 `core.excludesFile` 은 감시하는 나무 밖이라 애초에 알림이 없다.

  ⚠️ **단, 무시된 디렉터리 «안» 의 규칙 파일은 건너뛴다.** git 은 무시된 디렉터리로 **내려가지
  않으므로** 그 아래의 `.gitignore` 는 `!` 되살리기 규칙을 써도 화면의 판정을 하나도 못 바꾼다
  (실측: `vendor/` 를 무시한 저장소에서 `vendor/pkg/.gitignore` 에 `!keep.txt` 를 넣어도 여전히
  무시됨). 이 구분이 없으면 비용이 엉뚱한 곳에서 터진다 — 이 저장소만 해도 `.gitignore` 가 **68 개**
  이고 대부분이 `zig-pkg/` 안인데 그 디렉터리가 무시되므로, `zig build` 한 번이 전체 재스캔을 수십 번
  부른다. **아직 안 물어본 조상은 「무시 아님」으로 본다** — 모르면 다시 읽는 쪽으로 기울어야 틀린
  화면이 남지 않는다.

  ⚠️ **거절은 다르다 — 잊지 않는다.** 백엔드는 `check-ignore` 자리가 하나라 도는 작업이 있으면 새
  요청을 거절하는데, 이 질의를 부르는 자리는 **스캔 결과 드레인 하나뿐**이다. 그래서 옛 계약의
  「다음 스캔이 다시 묻는다」는 **다시 스캔할 이유가 없으면 일어나지 않는 일**이었다 — 폴더를 펼쳐
  하위가 한꺼번에 스캔되면 **첫 하나만 묻고 나머지는 전부 잃는다**(어떤 폴더는 흐려지고 어떤 폴더는
  안 흐려지며, 어느 쪽이 될지는 스캔 도착 순서에 달렸다).

  질의는 **탐색기를 보고 있는 동안** tick 이 백엔드가 한가해졌을 때 **한 배치씩** 나간다
  (`pumpIgnoreQueries`). ⚠️ **가시성 게이트가 계약의 일부다** — 흐림은 탐색기 뷰에서만 쓰이므로 다른
  뷰이거나 도크가 닫혀 있으면 **안 나간다**(커서는 그대로 남아 돌아오면 이어 묻는다). 목록 읽기를
  `dock.view == .source_control` 로 거는 것과 같은 규율이다. 「재요청은 tick 이 소유한다」는 오래된 규율이고 diff 재요청이 같은 자리에
  있다. 여기서 **직접 묻지 않는 이유**는 그 디렉터리의 항목 목록이 스캔에만 있기 때문이다(트리는
  화면용 행만 들고 있다). 한가한지 안 보고 걸면 다시 거절당해 **스캔만 도는 헛바퀴**가 된다.

  ⚠️ **그 재시도 목록에서는 «건 뒤에» 뺀다 — 그리고 「걸었다」의 기준이 성공 반환이 아니다.**
  `requeueScan` 은 스캔 줄이 꽉 차면 세부 요청을 전부 버리고 root 만 다시 예약한 뒤 **성공으로**
  돌아온다(`Tree.hasScanRequest` 주석). 그 성공을 「들어갔다」로 읽으면 디렉터리는 안 들어갔는데
  목록에서는 빠진다 — **거절을 잊던 바로 그 모양이 한 층 위에서 되살아난다.** 줄에 실제로 들어간
  것을 확인하고 빼며, 그 사이 탐색기가 다른 root 를 보게 됐으면 그때는 **버린다**(화면에 없는 경로를
  영원히 들고 있지 않는다).
  ...
  2. **백엔드를 만드는 자리가 사용자 행동 쪽에 없었다.** 만드는 자리가 전부 소스 컨트롤 경로라,
     탐색기만 쓰는 동안에는 질의가 첫 줄에서 되돌아갔다. 이제 **탐색기 뷰
  ...
  `noteIgnoredScan` → `pumpIgnoreQueries` → 진짜 `check-ignore` → `drainIgnoreResults` → **행 투영**까지 태워,
...
- **detached worker 수명 — 제품과 판정자가 다르다(2026-09-08, CI 가 잡은 것)**: 위 경계의 「backend 교체/세션 종료는 느린 FS worker 를 main actor 에서 기다리지 않는다」는 **제품 계약이고 그대로 둔다**(멈춘 I/O 로 창 닫기가 굳는 것이 훨씬 나쁘다 — `Backend.deinit` 과 그것을 재는 판정자 「retirement releases owner without waiting for worker generation」이 단일 출처다). 그런데 **판정자에서는 그 대가를 다른 판정자가 문다**: 스캔을 던져 놓고 세션이 끝나면 아직 도는 job 의 할당이 그 순간 **누수로 잡히고**, 그 워커가 뒤이어 해제된 자리를 만지면 누수 트레이스를 찍는 도중에 죽는다. 실제로 CI 샤드가 `dupe` 누수 한 줄 → `writeTrace` 에서 Segmentation fault → 종료 코드 134 로 abort 했다.
  - **가장 나쁜 성질은 오귀속이다.** 누수 결산은 매 판정자 끝에 도는데 워커를 남긴 판정자와 결산이 걸린 판정자가 **다르다**. 게다가 트레이스를 찍다 죽어 **이름조차 안 남는다** — 우리는 이 실패를 무관한 커밋(session_host 파일만 만진 커밋)의 회귀로 읽고 시간을 버렸다. 빠른 기계에서는 워커가 먼저 끝나 안 보인다: 로컬 4 샤드 전부 초록인데 CI 만 빨간 모양이 그것이다.
  - **거두는 자리는 「게이트가 있는가」로 갈린다** — 이 갈림이 이 항목의 본체다.
    - **워커가 `shutting_down` 을 봐야 나오는 backend**(에이전트 세션 아카이브 본체·상세)는 **자기 `deinit` 안에서, 취소한 뒤에** 거둔다. 그 플래그를 세우는 것이 `deinit` 자신이라, 그 **앞**에서 기다리면 영원히 안 끝난다 — 처음 판이 정확히 그 실수였고 상한까지 헛도는 것을 실측했다(적대적 검증 1 회차: 도크 판정자 셋이 매번 30 초씩 멈췄고, 그 부하가 무관한 signal 판정자 셋까지 함께 넘어뜨렸다).
    - **게이트가 없는 backend**(파일 트리·트리 변경·아카이브 scope·git)는 `AppSession.deinit` **한 자리**에서 세션이 물러나며 거둔다. 이 backend 들을 쓰는 판정자가 수십 개인데 규율을 그만큼 나눠 두면 새로 쓰는 사람이 반드시 빠뜨린다. workspace 교체는 물러난 backend 가 세션보다 오래 남을 수 있는 두 번째 자리라 거기서도 부른다.
    - 이미지 스캔·디코드는 **제품 `deinit` 에서 항상** 스레드를 join 한다 — 대기 상한이 작아(디코드 한 장 실측 평균 4.4 ms) 제품에서도 기다리는 편이 낫다고 이미 결정한 자리다.
  - `inflight == 0` 이 아니라 **참조수**를 본다. 워커는 자기 카운터를 줄인 **뒤에도** `state.release()` 를 한 번 더 만지므로, 카운터만 보고 나가면 그 사이에 판정자가 끝나 state 자체가 누수로 잡히는 좁은 창이 남는다.
  - 상한을 넘기면 그냥 지나간다 — 그때는 누수가 **정직하게** 잡힌다(조용한 통과를 만들지 않는다). 벽시계로 재는 이유는 반복 상한(`spins < N`)이 부하 걸린 기계에서 먼저 끊어져, 정작 느려서 생기는 이 결함을 못 잡기 때문이다(IG14 와 같은 실패다). 고친 뒤 전체 판정자를 상한 3 초로 낮춰 재계측했다: **타임아웃 0**, 가장 긴 대기 153 ms.
  - 메커니즘은 판정자 둘이 **기계 속도에 기대지 않고** 잰다. ⑴ 「판정자는 워커보다 오래 살면 안 된다」 — 항목 4,000 개짜리 디렉터리로 느린 스캔을 강제한다. ⑵ 「상세 backend: 게이트에 세워 둔 워커는 deinit 이 거두고 나간다」 — **게이트로 워커를 세워 두고** 부르므로 아예 결정적이다. 둘 다 **자기 `DebugAllocator` 로 그 자리에서 결산**한다(테스트 할당자를 쓰면 누수가 샤드 끝에서 이름 없이 보고되는 — 우리를 헤매게 한 바로 그 — 모양이 된다). 대기를 빼면 각자 자기 줄에서 죽는다(실측).
  - **새 backend 를 달 때 빠뜨리는 것은 게이트가 막는다** — `tests/boundary/detached_worker_quiesce_axis.zig` 가 `src/platform/macos` 에서 detached worker 를 띄우는 refcount backend 를 **전수로** 찾아, 자기 `deinit` 이 거두거나 세션 소유자 함수가 부르거나 둘 중 하나를 요구한다. 이름을 재고에 적는 것으로는 통과하지 않는다 — **거두는가**를 센다.
