...
> **2026-08-11 사용자 결정 — root를 갈아끼운다(아래 "왜 root를 갈지 않는가"를 뒤집는다).** cwd가 **root 밖**이면 이제
> 그 자리를 root로 세운다. 계기는 [editor-surface-dock.md §3.5](editor-surface-dock.md)가 소스 컨트롤 뷰를 **어느 저장소로든**
> 따라가게 바꾼 것이다 — 탐색기만 root 안에서만 reveal하면 `cd` 한 번에 **목록은 저장소 B, 트리는 저장소 A**가 되어
> 두 뷰가 갈린다. 그 불일치가 reveal 정책이 지키려던 것들보다 크다는 것이 사용자 판단이다.
>
> **무엇을 root로 세우나**: cwd가 아니라 **그 cwd를 품은 저장소 루트**다. 소스 컨트롤 뷰가 고르는 값과 같아야 두 뷰가
> 같은 것을 보고, `~/work/repo/src/deep`에서 `deep`을 root로 세우면 형제 디렉터리가 안 보여 쓸모도 없다. 저장소가
> 아니면 cwd 폴더 자체다.
>
> **아래 기각 표의 대가를 실제로 치른다**(지우지 않고 남긴다 — 무엇을 포기했는지가 기록이다): `cd`마다 접힘 상태가
> 사라지고 watcher가 재등록되며, `cd ~`면 홈이 root가 되고, `dock-tree-roots` 영속이 자동 전환으로 덮인다.
> 완화한 것은 둘뿐이다 — ⑴ **root 안에서는 예전대로 reveal만** 한다(같은 저장소 안을 오갈 때는 아무것도 안 버린다),
> ⑵ **사용자 조작이 항상 우선**이다(아래 §1.1).
>
> 아래 원문은 그 결정에 이른 기록이며, **"root 밖 no-op" 정책(3번)만 무효**다.
...
3. ~~**root 밖이면 아무것도 하지 않는다**~~ → **root 밖이면 root를 갈아끼운다**(2026-08-11 결정, 위 박스). 대상은 그 cwd를 품은 **저장소 루트**(없으면 cwd 폴더)이고, 전환은 사용자가 "폴더 열기…"로 고를 때와 **같은 검증 파이프라인**(`provideFileTreeRootPick` + `.replace`)을 탄다 — realpath·identity pinning·watcher union 재구성·영속이 그 배관에 이미 있어서, 여기서 `replaceExplicitRoots`를 직접 부르면 반쪽 전환이 된다. root **안**이면 예전대로 reveal만 한다.
...
- **탐색기를 보고 있을 때만 바꾼다.** 도크가 보여도 view가 소스 컨트롤·에이전트면 트리는 화면에 없다. 그때 root를 갈면 보이지도 않는 트리의 접힘·스크롤을 버리고 영속까지 덮는다. reveal은 비파괴적이라 상관없었지만 **교체는 다르다.**
...
- **전환이 커밋되면 한 번 더 펼친다.** 전환은 비동기라 요청 시점엔 새 root가 없어 그때의 reveal이 거절됐다. 재시도하지 않으면 깊은 하위 디렉터리에서 `cd` 했을 때 트리가 저장소 루트에 접힌 채 멈춘다. **one-shot이고 그 cwd의 것일 때만 유효**하다 — 도크가 숨어 있어 소비되지 않은 채 남은 플래그를 다른 cwd가 먹으면 그 cwd의 전환이 삼켜지고, 새 root가 realpath로 정규화돼 cwd를 안 품는 경우(심볼릭 링크)엔 재시도가 루프가 된다.
