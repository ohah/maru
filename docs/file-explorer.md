...
- **사이드바 헤더도 같은 두 밴드를 쓴다(창 전폭 정렬).** 창 상단은 좌우가 한 줄로 읽혀야 하므로, 왼쪽 사이드바 헤더의
  ...
  **남은 결함과 그 이관 계획.** 위 밴드는 *위치*만 정한다. 밴드 안 텍스트는 여전히 terminal 셀 그리드로 그려지므로,
  ...
  2. **입력 줄의 오버플로 규칙을 어느 도메인에 둘지 정한다.** 사이드바 검색 줄의 tail 창은
     `chrome.components.overlay_input.inputLineView`로 **find·palette와 단일 출처를 공유**한다(셀 col 기반). 사이드바만
     픽셀로 옮기면 세 소비처 중 하나만 다른 규칙을 쓰게 되므로, 그 함수를 픽셀 도메인으로 함께 올리거나 세 소비처를
     한 단계에서 같이 옮겨야 한다. 탭 제목도 rename 중 tail 앵커가 같은 성질이다.
  ...
  이관 순서는 **캐시 확장 → 입력 줄 3종(검색·find·palette) → 탭 제목**이다(사용자 결정: 단계적으로 나눠 각 단계를
  독립 PR로). 캐시는 나머지 둘의 선행 조건이고, 입력 줄 셋을 한 단계에 묶는 이유는 위 (2)의 오버플로 규칙 공유
  때문이다. 탭 제목은 hit-test(`segCols`)가 셀 칸을 유지해도 되어(칸→`colPx()` 변환 구조) 파급이 가장 작으므로
  마지막에 둔다.
  ...
  **정렬을 terminal 식으로 맞추지 않는다.** 예전에는 탐색기·소스 컨트롤만 `paneBarHeightPx`를 그대로 받았고, 그러면 **같은
  ...
  ([agent-session-list.md](agent-session-list.md) §2.1.3이 예고한 "두 chrome이 공유하는 logical token" 해법이다).
