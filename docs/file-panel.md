...
- **리치의 HTML 블록 미리보기 = 신뢰 shell DOM 삽입의 단일 예외(2026-08-02, 사용자 결정).** 원칙(§2.1 ②)은
  ...
  - **④-1a 로컬 이미지는 리치에서 보이지 않는다(알려진 결함, 2026-08-02).** `data-maru-asset-path`를 실제
    바이트로 바꾸는 코드가 **읽기의 격리 renderer 안에만** 있어서, 리치는 본문의 마크다운 이미지도 미리보기
    안의 이미지도 그리지 못한다. 미리보기가 만든 문제가 아니라 리치 모드 전체의 결함이며 별도로 고친다 —
    shell에는 `readAsset` 브리지가 이미 있으므로 배관을 잇는 일이다. **원격 이미지가 안 보이는 것은 결함이
    아니라 정책이다**(`src`는 어떤 scheme도 허용하지 않는다 — §2.1).
