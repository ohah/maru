...
- **왕복 실측 결과(2026-07-29)와 그에 따른 정책.** 요소별로 직접 재서 세 등급으로 나눴다.
  ...
  | **표현 불가** | **원시 HTML · 각주** | **그 문서는 리치에서 편집을 잠근다** |
  ...
  해결했다 — **문서 맨 앞이라는 위치**로 떼어 낼 수 있었기 때문이다. 반면 원시 HTML과 각주는 본문 **안에 섞여**
  있어 경계로 가를 수 없다. **그래서 이 둘 중 하나라도 있는 문서는 리치를 읽기 전용으로 열고 배너로 알린다**
  (`unsupportedRichSyntax`). 저장 경로가 닫히면 원문은 안전하고, 사용자는 소스 모드에서 손실 없이 고칠 수 있다.
  코드펜스 안의 HTML은 내용일 뿐이라 검사에서 제외한다.
