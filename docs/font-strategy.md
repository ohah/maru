...
- **메모리 문제가 아니다.** 한때 "프레임마다 새 CTFont 를 만들면 CoreText 전역 캐시가 상한 없이 자란다"고
  진단했으나 격리 하네스로 기각됐다 — 참조만 맞으면 인스턴스 churn 은 **14.7 B/iter** 로 사실상 공짜다.
  실제 원인은 위에 적은 빠진 `CFRelease` 였다.
- **남는 이득은 CPU 뿐이고 그 수치는 낡았다.** 재해석 95µs vs 재사용 25µs(호출당 약 70µs, 3.8배)로 쟀지만
  그건 style 슬롯이 **4 개**이던 구조다. 지금은 8 개다(face 4 종 × 커서/비커서 — 커서 칸은 합자를 푼다).
  새 구조의 이득은 측정된 적이 없다. **착수한다면 재측정이 첫 단계다.**
- **가장 먼저 풀 것은 소유권이다.** 커서 슬롯은 face 슬롯의 폰트를 **빌려 쓴다.** 캐시가 소유권을 가지면
  슬롯마다 "소유 vs 대여"를 다시 모델링해야 하고, 그 자리는 이미 한 번 over-release 로 죽은 이력이 있다.
- **참고**: 그 구현은 적대적 리뷰 9 라운드에서 실증 결함 32 건을 냈다(같은 기간 `CFRelease` 한 줄에서는
  0 건). 캐시 자체가 나쁘다는 뜻은 아니고, **얻는 것이 CPU 1~2% 일 때 감수할 표면인지**를 그 숫자로 재라는
  뜻이다. chrome 경로(`maru_chrome_font_for`)가 같은 캐시를 이미 갖고 있으므로 구조 자체는 선례가 있다.
...
ligature는 `font.ligatures`(불리언, **기본 켬**)로 사용자가 정한다.

**베이스/결정(사실상 표준)**: 단일 표준이 없어 터미널마다 갈린다 — Ghostty(`font-feature`에 `-calt`를 넣어야 꺼짐)·kitty(`disable_ligatures none`)·WezTerm은 기본 켬이고, iTerm2만 "Use ligatures"가 기본 해제다. maru는 다수 관례를 따라 **기본 켬**을 택한다(사용자 결정 2026-08). 합자를 쓰려고 코딩 폰트를 고른 사용자가 별도 설정 없이 기대한 모양을 보는 쪽이 놀람이 적다는 판단이다.

**등폭 격자는 유지된다 — 단 슬롯을 넓혀야 한다.** 코딩 폰트의 합자는 **글자 수를 유지**한다: CoreText가 `//`에 glyph를 둘 주는데(첫째는 빈 글리프, 둘째가 두 글자를 합친 모양) 칸 수가 그대로라 뒤 글자가 밀리지 않는다(A/B 실측: 켬 `//`→glyph 12137·11977, 끔→19·19, 양쪽 다 record 2개).

문제는 **그 합자 글리프의 ink가 자기 자리보다 왼쪽에서 시작**한다는 것이다(실측: advance 8.4px, ink 12.6px, 왼쪽으로 6.3px). 슬롯을 advance 폭으로만 잡으면 넘친 부분이 잘려 `//`가 `/` 하나로 보였다([#2123](https://github.com/ohah/maru/issues/2123) — 2026-08-18 실측으로 편집기 본문에서 재현). 두 경로가 같은 사실을 각자의 좌표로 받아 슬롯을 넓힌다:

- **터미널(셀 격자)**: `left_overhang_cells`(칸 수) — 시작 칸을 당기고 `cell_width`를 늘려 CJK와 같은 wide 경로를 탄다.
- **chrome 텍스트(편집기 본문 포함)**: `left_overhang_px` — 셀이 없으므로 px로 받아 래스터 슬롯을 넓히고 그린 자리를 그만큼 당긴다.

래스터는 왼쪽으로 넘치는 글리프를 슬롯 오른쪽에 붙여 그리므로, 넓힌 슬롯 안에 ink가 정확히 들어온다. **합자를 끄지 않고 모양을 유지한 채 격자를 지킨다**(Ghostty와 같은 동작 — 사용자 결정 2026-08-18).

**칸 수는 문턱을 뺀 올림이다(2026-09-19 정정).** 처음엔 반올림이었다(일반 글자의 작은 음수 bearing 을 거르려고). 그런데 JetBrains Mono 의 `...` 은 넘침이 **1.46 칸**(ink_x −11.4, advance 7.8)이라 반올림하면 1 이 되어 첫 점이 잘렸다 — 자동완성 목록의 `printf(const char *, ...)` 가 `*,  ..)` 로 보인 것이 그것이다(사용자 제보; 본문도 같았다). `//`(0.74)·`<!--`(2.87)는 반올림이 우연히 맞았을 뿐이다. 넘침은 덮어야 하는 칸 수이므로 `ceil(넘침/advance − 0.2)` — 1/5 칸 아래의 넘침은 bearing 으로 보고 0. 판정자 `LIG3`(`...` → 3칸).

**터미널 경로의 칸 수에는 상한이 있다.** 한 글리프가 덮는 칸 수는 `max_glyph_cell_span`(`renderer/glyph_layout.zig`)이 소유하고, 그 안에서 왼쪽 오버항은 최대 3칸이다(상한을 소유하는 코드는 `coretext_smoke.m`의 `maru_left_overhang_cells`). 3이라는 값은 **폰트 실측에서 나왔다** — 설치된 252 패밀리와 번들 5종의 *모든 글리프*를 순회해 ink가 advance 왼쪽으로 몇 칸 넘치는지 쟀다(2026-08-20):

| 최대 오버항 | 폰트 | 해당 글리프 |
|---|---|---|
| 3칸 | JetBrains Mono | `<!--` `<#--` `<==>` `####` `{{--` `{!--` `--}}` |
| 3칸 | Jetendard(번들 기본) | 위와 같은 7종 |
| 3칸 | Fira Code | `<!--` |
| 2칸 이하 | Cascadia Code·Hack, 한글/CJK/이모지 fallback 폰트 전부 | — |

4칸이 필요한 폰트도 있지만(Mishafi·Diwan Thuluth·Mishafi Gold 아랍어 서예체, Zapfino·Trattatello swash) 상한을 거기 맞추지 않는다: 터미널 폰트로 쓰이지 않고, 앞 칸이 실제 잉크를 가져 아래 "앞 칸이 비었을 때만" 조건에서 오버항이 부여되지 않으며, 걸려도 잘려 보일 뿐이다. 상한을 키우면 쓰이지 않을 넓은 슬롯을 아틀라스가 떠안는다.

**오버항은 앞 칸이 비어 있을 때만 준다.** 합자는 폰트가 앞 칸을 빈 글리프로 치환해 덮어도 지울 것이 없지만, left bearing이 큰 일반 글리프에 오버항을 주면 앞 글자를 덮어쓴다. 위 순회에서 JetBrains Mono의 오버항 글리프 224개 중 72개가 합자가 아닌 **결합 문자**(U+0336 combining long stroke overlay, 세디유·오고넥 등)였다 — 이들은 앞 글자 위에 겹쳐 그리라고 설계돼 ink가 왼쪽에서 시작한다. 이 조건이 그 둘을 가른다.

끄면 `liga`/`clig`(`kCTLigatureAttributeName=0`)와 `calt`(폰트 feature 사본)를 **모두** 꺼야 한다. `liga`만 끄면 `calt`가 남아 여전히 대체가 일어난다.

**셰이핑 문자열에서 빈 칸은 공백이다.** 한 번도 쓰인 적 없는 칸(codepoint 0)을 `U+0000` 그대로 run 문자열에 실으면 CoreText의 `calt`가 NUL 몇 개 뒤부터 죽는다 — 같은 줄에서 **앞쪽 합자만 붙고 뒤쪽은 원본 글자로 남거나, 합자에 삼켜진 글자가 아예 사라진다**(2026-08-20 실측, JetBrains Mono 13pt: `== \0 !== \0×5 =`가 `!`+빈 글리프 2개로 셰이핑되고 5칸 뒤 단독 `=`에 `===` 합자 글리프가 붙었다). 그래서 셰이퍼는 빈 칸을 `U+0020`으로 바꿔 넘긴다. 렌더 결과는 바뀌지 않는다 — 공백은 glyph record를 만들지 않는다는 계약이 그대로다.

이 칸이 생기는 경로는 셸이 아니라 **화면을 부분 갱신하는 TUI**다. Ink 기반 앱(Claude Code 입력창)은 다시 그릴 때 글자 칸만 하나씩 쓰고 사이 칸은 `ESC[K`로 지운 채 둔다. 셸은 공백을 `0x20`으로 실제로 쓰기 때문에 이 차이가 셸에서는 드러나지 않는다. Windows 경로(`win32_text.zig`)는 같은 치환을 이미 하고 있었고("빈 칸이 전부 지원 못 하는 글자로 잡힌다"가 그쪽 근거다), 동작 비교 오라클 Ghostty도 셰이퍼 run을 만들 때 같은 치환을 한다.

cursor/selection이 합자 위에 있을 때의 표시 규칙과 DrawList fixture·screenshot smoke는 이 계약 밖(별도 이니셔티브)이다.
...
native CoreText DrawList shaper는 **run 단위**로 셰이핑한다: 같은 행에서 열이 이어지고 face(bold/italic)가 같은 cell들을 한 문자열로 묶어 `CTLine` 하나를 만든다. cell마다 `CTLine`을 만들던 이전 방식은 호출당 고정비(속성 조회·typesetter 생성·폰트 캐스케이드 준비)가 지배적이라, glyph 수가 같아도 훨씬 느렸다(135x72·문자 25% 화면 실측: cell 단위 3.56ms → run 단위 0.20ms/frame). 공백 cell도 run 문자열에는 넣어 run이 단어마다 끊기지 않게 하되 glyph record는 만들지 않고, 행 끝 여백은 잘라내 헛된 glyph를 만들지 않는다. glyph를 다시 cell로 되돌리는 것은 `CTRunGetStringIndices`와 유닛→cell 표이며, 그 표가 run 셰이핑 정확성의 핵심이다.

**리가처는 끈다**(`kCTLigatureAttributeName = 0`). run 단위로 묶으면 문맥이 생겨 CoreText가 합자를 적용하는데, 터미널은 등폭 격자라 glyph 수가 cell 수보다 적어지면 `//`가 한 칸으로 합쳐지며 격자가 깨진다([#2123](https://github.com/ohah/maru/issues/2123)). 합자를 v1 품질 기준에서 제외한다는 위 결정과 같은 방향이다.

남은 한계는 둘이다. 첫째, 한 cell의 cluster가 여러 glyph(base + combining mark)로 분해되면 그 glyph들이 여전히 모두 같은 `(row, col)`에 매핑되어 겹쳐 그려진다 — run 단위 셰이핑은 cell 사이 문맥을 살렸지만 cell **안**의 cluster 위치 보정은 별도 작업이다. 둘째, 한 cell이라도 `.notdef`(glyph 0)로 매핑되면 native가 실패 status로 닫아 `CoreTextDrawListShapeFailed`로 frame 전체가 실패한다. 즉 폰트+fallback 체인에 없는 코드포인트는 아직 해당 cell만 tofu(`.notdef` placeholder)로 그리는 게 아니라 frame 전체를 떨어뜨린다. 임의의 셸 출력을 받는 단계에서는 이 두 한계를 line-level shaper와 per-cell tofu 대체로 풀어야 한다.
