...
- 제품 `GlyphRasterFrame.uploads/pixels`를 Metal atlas texture에 업로드하고 blit readback으로 source bytes와 일치하는지 확인하는 smoke. 이 smoke는 제품 raster byte 계약이 native GPU texture 경계까지 도달하는지 증명하지만, 아직 그 texture를 terminal cell shader가 샘플링했다는 뜻은 아니다.
