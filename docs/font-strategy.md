...
- CoreText glyph record가 renderer 중립 `ShapedGlyphRecord -> GlyphRunList -> RendererState -> RenderFrame` 경로를 거쳐 Maru `GlyphAtlas` cache key, UV, smoke 전용 CoreText raster bytes로 들어가는 smoke.
