...
- 제품 `GlyphRasterFrame.uploads/pixels`를 Metal atlas texture에 업로드하고 blit readback으로 source bytes와 일치하는지 확인한 뒤, 같은 atlas texture를 terminal cell shader가 샘플링해 drawable readback 픽셀이 source raster의 non-clear texel과 일치하는지 확인하는 smoke. 이 smoke는 CoreText shaper/rasterizer 기반이라 실제 glyph bitmap이 Metal atlas shader까지 이어졌는지 검증한다. 다만 아직 실제 `TerminalCore -> DrawList` 입력이 아니라 probe-derived surface를 쓴다.
