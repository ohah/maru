...
**등폭 격자는 어느 쪽이든 유지된다.** 예전에 합자를 배제한 이유는 cell 격자와 충돌한다는 것이었는데, 실제로 격자를 깨는 것은 셀 수를 줄이는 `liga`뿐이고([#2123](https://github.com/ohah/maru/issues/2123)) 코딩 폰트의 합자는 대개 `calt`(문맥 대체)로 구현돼 **글자 수를 유지**하며 칸마다 조각을 하나씩 놓는다 — 등폭을 지키려고 그렇게 만들어진 기능이다. 셰이핑이 run 단위로 바뀌면서 CoreText가 각 칸에 glyph를 배정하므로 켠 상태에서도 셀이 비지 않는다(A/B 실측: 켬 `//`→glyph 12137·11977, 끔→19·19, 양쪽 다 record 2개).
