...
- link과 **같이 dedup**한다(`grapheme_ids` 해시맵) — 같은 cluster는 한 번만 저장한다. 처음엔 "grapheme은 셀마다 고유해 dedup 이득이 없다"고 봤으나, 실제 흔한 cluster(악센트·NFD 음절·키캡)는 **반복적**이라 dedup이 store를 distinct cluster 수로 bound한다(셀 수와 무관). flat `cells: []Cell`이라 `memcpy`가 `grapheme_id`를 값 복사해 한 항목을 여러 셀이 공유하지만, append-only(dedup)면 dangling이 없다(link과 동일). 화면에서 사라진 cluster의 **구조적 회수**는 grapheme 저장을 Screen/page 수명에 귀속시키는 page-aligned storage와 함께 들어온다(§5 HG2a-후속) — 전역 refcount/GC는 위험·임시품이라 쓰지 않는다.
...
- **수명 관리**: `grapheme_store`를 `link_store`와 동형의 **dedup append-only**로 둔다(`grapheme_ids` 해시맵 — 같은 cluster 1 entry). store가 distinct cluster 수로 bounded돼 반복 cluster의 per-cell 증가가 없고, reset(RIS)·deinit에서 일괄 free한다. 화면에서 사라진 cluster를 더 회수하려면(구조적 회수) grapheme 저장을 Screen/page 수명에 귀속시켜야 하며, 이는 page-aligned storage 작업과 함께 들어온다(§5).
...
- **HG2a-후속 — store 증가 방지(dedup, 완료) + 구조적 회수(page-aligned storage와 함께)**: 메모리 목표는 store 무한 증가 방지다. **장기적으로 가장 탄탄한 구조는 전역 refcount/GC가 아니라 grapheme 저장을 셀의 수명 단위(Screen/page)에 귀속**시키는 것이다 — page가 eviction으로 free될 때 그 page의 grapheme도 함께 사라져 **회수가 구조적**이고, id 네임스페이스가 page-local이라 page 내부 `memcpy`는 안전하고 page끼리 안 섞인다(Ghostty 모델, 아키텍처 §메모리 전략의 page-aligned storage 방향). 전역 store에 refcount/GC를 다는 것은 flat `cells:[]Cell`+`memcpy` 위에서 모든 셀 수명 이벤트/모든 root를 정확히 추적해야 해 **위험하고**, page-aligned storage가 들어오면 버려지는 **임시품**이라 도입하지 않는다. 그 전 브리지로 **`grapheme_store`를 `link_store`처럼 dedup**(같은 cluster 한 번만 저장)해 셀당 증가를 막는다 — store가 distinct cluster 수만큼만 커지고(악센트·NFD 음절·키캡 반복은 1 entry), append-only라 dangling이 없으며, page로 옮길 때 깔끔히 흡수된다. (`link_store`는 이미 dedup이라 추가 누수 정리가 불필요하다 — 둘 다 distinct 키 수로 bounded.) **구조적 회수의 결정·구체 플랜은 [terminal-core-decomposition.md §11.8](terminal-core-decomposition.md)에 박아 두었다** — 활성 grid 페이징(§11 B) 단계에서 grapheme을 page-local로 귀속(page free 시 동반 소멸). 그 단계의 한 step으로 진행하고, 미리 별도 구현하지 않는다(같은 스토리지 레이어 동시 리팩터 충돌 회피).
