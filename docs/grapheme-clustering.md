...
- **HG3 — 렌더·셰이핑 통합 + ObjC ABI + hack/`combining` 제거**: `NativeDrawCell`/`coretext_smoke.m`가 cluster 전체를 CTLine에 넘기고, `draw_list→glyph_layout→shaped_records→coretext` 체인이 `combining`(단일) 대신 store cluster를 읽도록 이전한다. 그 과정에서 단일-combining 보정 hack 3곳(`isColorGlyph` 색판정·셰이퍼 VS16 재주입·`selection` 복사 — 전부 `isKeycapCombining` 경유)과 `Cell.combining`을 제거한다(마지막 reader가 셰이퍼라 자연스럽다). atlas cache key 정합. CoreText 글리프 출력은 폰트 의존이라 macOS 수동 캡처로 검증(자동 CI 제외).
