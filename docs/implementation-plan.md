...
- app host smoke: 실제 UI 없이 `AppWindow -> SurfaceRuntime -> RuntimeEventPump -> RendererState -> DrawList -> GlyphFrame` frame 조립, resize, focused input artifact를 `mise run app-smoke`로 남긴다. `RendererState`는 frame 사이에 살아남는 `GlyphAtlas`를 소유하므로, app host가 매 frame atlas를 새로 만들지 않는다는 계약도 여기서 고정한다.
...
- macOS Metal smoke: `mise run test-macos-metal-smoke`로 summary와 fixture 계약을 검증하고, `mise run macos-metal-smoke`로 AppKit 창 위 CAMetalLayer에 `RendererState -> GlyphFrame -> GlyphQuadFrame` 기반 placeholder 셀을 present한 뒤 셀 중심 픽셀을 readback한다. `terminal_grid=true`는 입력 cell count가 아니라 샘플한 셀 중심 픽셀이 모두 clear 색이 아닐 때만 기록한다. summary에는 `renderer_input=renderer_state_glyph_frame`, `renderer_frame_prepared=true`, `renderer_atlas_slot_placement=true`, `renderer_glyph_uv_ready=true`, glyph/atlas 통계를 남긴다. 이 단계는 glyph bitmap text renderer 검증은 아니지만, `GlyphFrame -> GlyphQuadFrame -> Metal placeholder` 소비 경로와 atlas placement/UV 준비의 첫 실제 UI 검증이다.
...
- `mise run macos-metal-smoke`가 macOS에서 실제 Metal drawable을 한 frame 이상 present하고, `RendererState -> GlyphFrame -> GlyphQuadFrame` placeholder 셀 중심 픽셀을 readback해 비-clear 픽셀을 확인한다. native bridge가 atlas slot id뿐 아니라 `x_px/y_px/width_px/height_px` placement 후보도 받으면 summary에 `renderer_atlas_slot_placement=true`를 남기고, slot pixel rect가 shader UV로 변환됐으면 `renderer_glyph_uv_ready=true`를 남긴다. 아직 glyph bitmap text renderer가 없다는 한계는 summary에 `glyph_text=false`로 적는다.
...
- `mise run macos-glyph-text-smoke`가 macOS에서 실제 창을 띄워 shader sampling된 glyph texture와 PPM screenshot artifact를 남기고, 동시에 제품 renderer frame probe 통계를 summary에 남긴다. probe의 shaper는 아직 `FakeFontBackend`이므로 `renderer_shaper=fake_font_backend`도 summary에 남긴다. 아직 제품 `GlyphFrame`을 Metal backend가 직접 소비해 terminal cell text를 그리는 단계는 아니므로 이 한계도 summary와 PR 설명에 적는다.
