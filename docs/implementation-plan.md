...
- 후속: **PR3a 세로 사이드바 탭바(레이아웃 토대)** — drawable에 사이드바 strip 예약, 터미널 grid가 줄어든 너비 사용(헤드리스 레이아웃 단위 + macOS smoke) → PR3b(탭 엔트리 렌더 + cwd/✓✗ 메타데이터) → PR3c(클릭/Cmd+Shift+]/[ 전환) → PR3d(탭 드래그 재정렬) → PR4(탭 close, active_tab clamp). 이후 split 단계(SplitTree + 멀티-panel 렌더, 큼)·tmux-CC 드라이버(control-mode 파서, 큼)는 별도. quick terminal/global shortcut은 직교라 별도.
