...
- [웹 OSR 백엔드 구현 계획(W1~W8 — `.browser` 를 CEF 창 없는 렌더링 sidecar 로)](plans/web-osr-backend.md) — **계획, D1(브라우저는 항상 로컬)·D2(선택형, 기본 WKWebView)·D6(로그인 유지)·D7(mock keychain) 확정, 사용자 결정 다섯(D3~D5·D8·D9)이 열려 있다**
