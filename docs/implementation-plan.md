...
- 후속: PR3(Swift NSWindow 탭 UI + 탭 전환 키/클릭 — ABI 쿼리 `tab_count`/`active_tab` 노출 + 탭바 렌더, **인터랙티브 E2E**), PR4(탭 close PTY 수명 엣지 — `reselectAfterClose`로 active_tab clamp해 `active().?` 패닉 방지). quick terminal/global shortcut은 직교라 별도.
