...
- printable 출력이 마지막 열을 넘으면 다음 줄로 넘어가는 **autowrap(DECAWM deferred/pending wrap)**을 구현했다(완료). 마지막 칸을 채운 직후 커서는 그 칸에 머물고 pending_wrap만 서며, 다음 printable 글자가 먼저 다음 줄로 넘어간 뒤 그려진다(끝 글자마다 빈 줄이 끼지 않게). 명시적 커서 이동(CR/LF/backspace/커서 위치 지정/resize)은 pending_wrap을 무효화하고, wide glyph가 줄 끝에 1칸만 남으면 통째로 다음 줄로 넘긴다. autowrap은 zsh PROMPT_SP 등 거의 모든 셸/프로그램이 의존하는 기본 동작이라 우선 구현했다. 그 위에 행별 soft-wrap 플래그(wrapped)를 추적해 **resize reflow를 스크롤백 위에서 구현했다(완료)**: 활성 화면의 논리 줄을 합쳐 새 폭에 다시 wrap하고 넘치는 위쪽 행은 스크롤백으로 민다. 커서는 내용을 늘리지 않고 좌표로만 환산해 따라가며, hard 줄끝(프롬프트)은 절대 soft가 되지 않아 합쳐지지 않는다. (한 번은 스크롤백 없이 reflow했다가 되돌렸다 — zsh SIGWINCH redraw와 피드백 루프로 프롬프트가 중복되고 perf 예산(`core_resize_loop`)을 넘겼다. 핵심 버그는 "커서까지 빈 칸 보존"이 hard 프롬프트 줄을 soft로 만든 것이었고, 지금은 커서가 wrap 여부를 절대 안 바꾼다. Ghostty처럼 스크롤백 위에서 reflow하고 Ghostty reflow 오라클로 그리드+커서를 검증한다. perf는 활성 화면만 reflow해 O(활성 행)으로 유지한다 — 기존 스크롤백 행 재-wrap은 이후 단계.)
...
- 아직: reverse(`7`/`27`)는 Style에 필드가 없어 소비만 한다. 배경색 erase(BCE — 현재 pen 배경으로 지우기)는 아직 없다. scroll region/insert/delete line 등 나머지 CSI는 소비만 한다.
