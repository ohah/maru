...
- 후속: PR1c(`DevSession`을 동적 탭 컨테이너로 — `ArrayList(*Tab)` heap-pin + create/switch + `activeSurface()`를 `app_window.active()`로 라우팅 + 결정적 테스트), PR1d(멀티-PTY drain pump + 탭별 PTY spawn, macOS 게이트), PR2(additive ABI: create/select/close_tab + app_action summary), PR3(Swift NSWindow 탭 UI + keybinding 액션 분기, GUI 수동), PR4(탭 close PTY 수명 엣지). quick terminal/global shortcut은 직교라 별도.
