...
- 아직: SGR가 정한 cell 배경색은 cell.style에만 담기고 Metal renderer가 배경 fill로 적용하는 것은 별도(전경색은 적용됨). reverse(`7`/`27`)는 Style에 필드가 없어 소비만 한다. `:` sub-parameter는 `;`처럼 다뤄 colon direct-color(`38:2::r:g:b`)는 아직 정확하지 않다. scroll region/insert/delete line 등 나머지 CSI는 소비만 한다.
...
- 실제 Swift window에 terminal glyph를 그리는 제품 앱은 다음 단계들로 나눠 시작한다.
  ...
  - 그 다음 PR은 Swift placeholder view를 `MaruMetalTerminalView`(CAMetalLayer)로 바꿔, 매 tick `maru_macos_app_dev_session_metal_frame`을 lean renderer로 그린다(완료). 여기서 dev session의 shell glyph가 Swift 창에 처음 보인다. `macos-app-dev-smoke`가 `metal_renderer_created=true`/`metal_frames_drawn>0`로 렌더 경로를 gate한다. 아직 resize는 placeholder cell 추정값이고, NDC grid는 inset 매핑이라 glyph가 창에 맞춰 늘어난다. 실제 font-metrics 기반 fixed-cell layout, 스크롤백/탭/선택 같은 제품 UX는 이후 단계다.
...
- `mise run macos-app-dev-smoke`가 실제 Swift `NSApplication` placeholder window를 띄우고 `app-dev.summary.txt`에 `visible_ui=true`, `swift_host=true`, `abi_ready=true`, `terminal_surface=true`, `terminal_surface_note=zig_runtime_attached_placeholder_view_no_metal_surface`, `frame_loop_ticks`, `output_events>0`, `exit_events=1`, `key_events=2`, `terminal_input_events=2`, `resize_events=1`, `close_events=1`, `frame_prepared=true`, `final_frame_ended=true`를 남긴다. 이 명령은 Swift/Zig ABI 링크, 앱 lifecycle, Zig-owned shell surface/frame loop, key/resize/close event ABI를 검증하지만, Swift window 안에 terminal glyph를 그리지는 않는다.
