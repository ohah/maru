...
- **② zsh 통합 스크립트가 OSC 133 마커를 emit한다(완료)**: 위 zsh 통합 `.zshenv`가 `precmd`로 직전 명령 끝(`D;$?`)+새 프롬프트 시작(`A`)을, `preexec`로 출력 시작(`C`)을, PS1 끝에 입력 시작(`B`)을 emit한다(`print -rn`·`%{%}`). osc133 precmd를 사용자 .zshrc 훅 '전에'(.zshenv가 먼저 실행) 등록해 `precmd_functions` 맨 앞에서 직전 `$?`를 정확히 캡처한다 — 편집키 one-shot precmd가 뒤에 와도 종료코드가 안 틀어진다(실측 캡처로 확인). PS1은 프레임워크가 매 프롬프트 재생성해도 없으면 B를 다시 붙인다. **실측 검증**: 실제 `/bin/zsh -i`(vi-mode .zshrc 포함)를 PTY로 띄워 `A→B→C→D;0`/`D;1` 순서·종료코드를 캡처로 확인. core 측 end-to-end 단위 테스트(zsh emit 형태 → 행 분류)도 추가. `MARU_DEBUG=1`이면 dev session 화면 덤프가 행별 분류(P/I/C/·)+`last_exit`를 찍어 거터 PR 전에 눈으로 확인 가능. clean-room: zsh 매뉴얼 + semantic-prompts.md에서 직접 작성(Ghostty·kitty GPL 스크립트 미참조).

이어서 ③ 커서 줄 reflow를 prompt 경계로 정확화(위 `reflowCursorLine=false` workaround 대체 — reflow 태그 carry 포함) ④ 거터 마크(✓/✗)·프롬프트 점프 네비게이션 ⑤ OSC 7 cwd 보고(8·9단계 토대) ⑥ trace 의미 이벤트. clean-room: freedesktop semantic-prompts.md 명세 + Ghostty 동작 비교(코드 미복사). **현재 한계**: reflow는 태그를 carry하지 않고 reflow된 행을 `.unknown`으로 둔다(누수/크래시 없음, resize 후 태그만 부정확 — ③에서 해결).
