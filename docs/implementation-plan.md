...
- 후속: PR1d-2b(`createTab`을 2번째 탭에 호출 + `switchTab`(`app_window.selectTab`) + tick이 모든 탭 drain + **2탭 macOS smoke**로 "둘 다 출력" 검증 = 반-E2E), PR2(additive ABI: create/select/close_tab + app_action summary), PR3(Swift NSWindow 탭 UI + keybinding 액션 분기, GUI 수동), PR4(탭 close PTY 수명 엣지 — `reselectAfterClose`로 active_tab clamp해 `active().?` 패닉 방지). quick terminal/global shortcut은 직교라 별도.
