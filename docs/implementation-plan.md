...
- macOS Metal smoke: `mise run test-macos-metal-smoke`로 summary와 fixture 계약을 검증하고, `mise run macos-metal-smoke`로 AppKit 창 위 CAMetalLayer에 `RendererState -> GlyphFrame` 기반 placeholder 셀을 present한 뒤 셀 중심 픽셀을 readback한다. `terminal_grid=true`는 입력 cell count가 아니라 샘플한 셀 중심 픽셀이 모두 clear 색이 아닐 때만 기록한다. summary에는 `renderer_input=renderer_state_glyph_frame`, `renderer_frame_prepared=true`, `renderer_atlas_slot_placement=true`, glyph/atlas 통계를 남긴다. 이 단계는 glyph bitmap text renderer 검증은 아니지만, `GlyphFrame -> Metal placeholder` 소비 경로와 atlas placement ABI 전달의 첫 실제 UI 검증이다.
...
- `mise run macos-metal-smoke`가 macOS에서 실제 Metal drawable을 한 frame 이상 present하고, `RendererState -> GlyphFrame` placeholder 셀 중심 픽셀을 readback해 비-clear 픽셀을 확인한다. native bridge가 atlas slot id뿐 아니라 `x_px/y_px/width_px/height_px` placement 후보도 받으면 summary에 `renderer_atlas_slot_placement=true`를 남긴다. 아직 glyph bitmap text renderer가 없다는 한계는 summary에 `glyph_text=false`로 적는다.
