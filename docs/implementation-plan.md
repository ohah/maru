...
- [웹 OSR 백엔드 구현 계획(W1~W9 — `.browser` 를 CEF 창 없는 렌더링 sidecar 로)](plans/web-osr-backend.md) — **W1a~W2 구현됨(codec·sidecar·브라우저·픽셀 링), D1(브라우저는 항상 로컬)·D2(선택형, 기본 WKWebView)·D6(로그인 유지 — WKWebView 는 macOS 14+)·D7(mock keychain)·D9(GPU 안 되면 거부) 확정, 사용자 결정 넷(D3~D5·D8)이 열려 있다**
...
- [Session host kernel cwd parity 구현 계획](plans/session-host-kernel-cwd.md)
...
5. **CR3 — shared Client 세대:** CR3a는 두 merge slice로 닫았고 CR3b R1까지 완료했다. **CR3a-1(완료)**은 현
   ...
   **2c3b-3은 response-side execution/ownership**을 구현한다. client-slot-internal
   ...
   9. **2c3d one-shot event facade (doc-first):** generation event는 raw `BufferedEvent` 값 반환 대신 caller-final-address
      ...
      **C3-3b1~b6 event settlement와 비동기 close는 구현 완료했다.** 아래 순서는 각 slice를 닫을 당시의 구현 기록이다. b2는
...
8. **CR6 — 제품 gate:** CR6c에서 actual daemon/manifest → 일반 launch discovery → primary recovery row → 실제
   ...
   **비릴리스 잔여 범위:** restore-incomplete 실행은 현재 손상된 manifest를 덮지 않도록 background/final checkpoint를
   건너뛴다. 완전 복원 성공 뒤 stale `.bak`을 해제하고 다음 첫 저장에서 backup을 다시 무장하는 R7-5는
   [검증 매트릭스](verification-matrix.md)의 `Workspace restore checkpoint backup re-arm` 행대로 구현·검증 완료했다.
   따라서 current-current 세션 호스트 제품 경로의 비릴리스 기능 구현 항목은 없다.
