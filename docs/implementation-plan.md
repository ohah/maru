...
- **N0b — 시각 기준 수령(코드 변경 없음).** [file-panel.md](file-panel.md) §1·[editor-surface.md](editor-surface.md)
  ...
  - **남은 것은 셀 환산 하나**다. VSCode는 px로 계산하는데 우리 gutter는 셀 격자에 서므로 decorations 폭
    (`10px` + 접기 `16px`)을 셀 수로 정해야 한다. §4.1이 3셀을 기본 후보로 두었고 **N1에서 실제 화면을 보고
    확정**한다 — 목업 수령이 아니라 구현 중 판단이다.
