...
- 후속: PR1d(`DevSession`을 동적 탭 컨테이너로 — `ArrayList(*Tab)` heap-pin + create/switch + 멀티-PTY drain pump + 탭별 PTY spawn; PTY spawn·pump는 macOS 게이트, surface/active 모델은 헤드리스 결정적 테스트), PR2(additive ABI: create/select/close_tab + app_action summary), PR3(Swift NSWindow 탭 UI + keybinding 액션 분기, GUI 수동), PR4(탭 close PTY 수명 엣지). quick terminal/global shortcut은 직교라 별도.
