...
- `mise run macos-coretext-smoke`가 macOS에서 창/GPU 없이 default `ResolvedAppearance` font 요청 전달, CoreText font resolve, `ShapedGlyphRecord -> GlyphRunList -> RendererState -> RenderFrame` 준비, smoke 전용 CoreText `GlyphRasterFrame` bytes, CPU bitmap rasterization을 확인한다. 아직 설정 파일/설정 UI/runtime reload, Metal texture upload와 실제 화면 text draw가 없고, CoreText rasterizer가 재사용 가능한 제품 macOS font backend가 아니며, surface가 실제 `TerminalCore -> DrawList`가 아닌 probe-derived bounds라는 한계는 summary와 PR 설명에 적는다.
