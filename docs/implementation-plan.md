...
- glyph raster test: GPU 없이 `GlyphFrame -> GlyphRasterFrame`을 만들고, upload 후보가 contiguous RGBA bytes, byte offset, row bytes, zero-ink 진단값으로 바뀌는지 검증한다. 이 단계는 실제 CoreText 제품 rasterizer가 아니라 backend input byte 계약을 고정하는 fake rasterizer 경로다.
...
- `mise run macos-metal-smoke`가 macOS에서 실제 Metal drawable을 한 frame 이상 present하고, `RendererState -> GlyphFrame -> GlyphQuadFrame` placeholder 셀 중심 픽셀을 readback해 비-clear 픽셀을 확인한다. native bridge가 atlas slot id뿐 아니라 `x_px/y_px/width_px/height_px` placement 후보도 받으면 summary에 `renderer_atlas_slot_placement=true`를 남기고, slot pixel rect가 shader UV로 변환됐으면 `renderer_glyph_uv_ready=true`를 남기며, renderer upload byte 계약이 준비됐으면 `renderer_glyph_raster_ready=true`를 남긴다. 아직 glyph bitmap text renderer가 없다는 한계는 summary에 `glyph_text=false`로 적는다.
