...
- macOS Metal smoke: `mise run test-macos-metal-smoke`로 summary와 fixture 계약을 검증하고, `mise run macos-metal-smoke`로 AppKit 창 위 CAMetalLayer에 `RendererState -> GlyphFrame -> GlyphQuadFrame` 기반 placeholder 셀을 present한 뒤 셀 중심 픽셀을 readback한다. `terminal_grid=true`는 입력 cell count가 아니라 샘플한 셀 중심 픽셀이 모두 clear 색이 아닐 때만 기록한다. 또한 같은 제품 `GlyphRasterFrame`의 upload bytes를 Metal `RGBA8Unorm` atlas texture에 올리고 blit readback으로 source bytes와 일치하는지 확인한다(`product_atlas_uploaded=true`). summary에는 `renderer_input=renderer_state_glyph_frame`, `renderer_frame_prepared=true`, `renderer_atlas_slot_placement=true`, `renderer_glyph_uv_ready=true`, `renderer_glyph_raster_ready=true`, `renderer_glyph_raster_skipped_count`, `product_atlas_uploaded`, atlas upload/readback 통계를 남긴다. 이 단계는 glyph bitmap text renderer 검증은 아니지만, `GlyphFrame -> GlyphQuadFrame/GlyphRasterFrame -> Metal placeholder/product atlas upload` 소비 경로와 atlas placement/UV/upload byte/skip 준비의 첫 실제 UI 검증이다.
...
- `mise run macos-metal-smoke`가 macOS에서 실제 Metal drawable을 한 frame 이상 present하고, `RendererState -> GlyphFrame -> GlyphQuadFrame` placeholder 셀 중심 픽셀을 readback해 비-clear 픽셀을 확인한다. native bridge가 atlas slot id뿐 아니라 `x_px/y_px/width_px/height_px` placement 후보도 받으면 summary에 `renderer_atlas_slot_placement=true`를 남기고, slot pixel rect가 shader UV로 변환됐으면 `renderer_glyph_uv_ready=true`를 남기며, renderer upload byte와 skip 회계 계약이 준비됐으면 `renderer_glyph_raster_ready=true`와 skip count를 남긴다. 같은 smoke에서 `GlyphRasterFrame.uploads/pixels`를 Metal atlas texture에 업로드하고 readback byte 비교가 성공하면 `product_atlas_uploaded=true`를 남긴다. 아직 glyph bitmap text renderer가 없다는 한계는 summary에 `glyph_text=false`로 적는다.
