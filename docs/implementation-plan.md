...
- 후속: PR3b(사이드바에 탭 엔트리 텍스트 + 활성 하이라이트 + cwd/✓✗ 메타데이터) → PR3c(사이드바 클릭→탭 히트테스트→switchTab, Cmd+Shift+]/[ 전환) → PR3d(탭 드래그 재정렬) → PR4(탭 close, active_tab clamp). 이후 split 단계(SplitTree + 멀티-panel 렌더 — 같은 origin-offset 메커니즘 확장, 큼)·tmux-CC 드라이버(control-mode 파서, 큼)는 별도. quick terminal/global shortcut은 직교라 별도.
