...
- macOS live PTY Metal smoke: `mise run test-macos-app-pty-metal-smoke`로 summary 계약을 검증하고, `mise run macos-app-pty-metal-smoke`로 controlled command의 실제 PTY output을 `PtyReader -> PtyEventQueue -> RuntimeEventPump -> SurfaceRuntime -> AppWindow active surface -> TerminalCore -> DrawList -> CoreTextDrawListShaper -> RendererState -> GlyphQuadFrame/GlyphRasterFrame -> Metal atlas shader sampling -> screenshot`까지 통과시킨다. summary에는 `renderer_input=surface_runtime_live_pty_draw_list`, `screen_contains_expected`, `output_events`, `exit_events`, `termination`, `product_atlas_uploaded`, `product_atlas_sampled`, `screenshot_artifact`, raw/screen/snapshot/screenshot artifact 경로를 남긴다. 이 단계는 실제 PTY bytes가 보이는 Metal UI까지 도달하는 첫 검증이지만, prompt/dotfile/ANSI escape에 좌우되는 interactive shell과 사용자 키 입력 event loop는 아직 아니다.
