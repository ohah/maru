...
- macOS CoreText smoke: `mise run test-macos-coretext-smoke`로 summary 계약을 검증하고, `mise run macos-coretext-smoke`로 창/GPU 없이 macOS CoreText가 기본 고정폭 폰트를 찾고 ASCII/CJK/emoji probe를 glyph run으로 shape하는지 확인한다. 이 단계는 glyph atlas/bitmap 검증은 아니지만, text renderer 전에 font stack 실패를 분리하는 검증이다.
...
- `mise run macos-coretext-smoke`가 macOS에서 창/GPU 없이 CoreText font resolve와 glyph run 생성을 확인하고, 아직 glyph atlas/bitmap이 없다는 한계는 summary에 `glyph_rasterized=false`로 적는다.
