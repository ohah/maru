...
- 미정 폴리시: floating 탭 말줄임. "+" 슬롯 hover 밴드(현재 글리프+커서만). 터미널 레벨 키 config·기본 바인딩 unbind. 호버 탭 hit-test는 마우스 이동마다 leaf rect 레이아웃(작은 트리라 cheap, 필요 시 캐시). tmux-CC 드라이버(control-mode 파서, 큼)는 split 후 별도. quick terminal/global shortcut은 직교라 별도.
