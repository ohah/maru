...
- 후속: PR2(additive ABI: create/select/close_tab + app_action summary — Swift가 Cmd+T 등으로 createTab/switchTab을 호출), PR3(Swift NSWindow 탭 UI + keybinding 액션 분기, GUI 수동 — 인터랙티브 E2E), PR4(탭 close PTY 수명 엣지 — `reselectAfterClose`로 active_tab clamp해 `active().?` 패닉 방지). quick terminal/global shortcut은 직교라 별도.
