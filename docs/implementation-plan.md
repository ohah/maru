...
- **split-pane chrome 예정(cmux식 per-pane 상단 UI)**: 각 panel 위에 작은 헤더 바(제목 + 호버 ✕ 닫기 + 드래그로 재배치)를 그린다 — 사이드바 탭 chrome(밴드/제목 glyph/✕/드래그)의 panel 버전. cmux 화면을 시각 비교(동작/레이아웃만, GPL 코드 미참고)해 레이아웃을 맞춘다. close는 PR5a의 `closeActivePane`/`closePane`을 ✕ 클릭에 연결. (드래그 재배치는 트리 위 노드 이동이라 별도 검토.)
- 미정 폴리시: 사이드바 글자색 테마화(`sidebar_foreground`)·활성 탭 글자 강조·긴 제목 말줄임·드래그 중 floating 탭 시각. 비활성 panel 위 마우스 휠 라우팅. tmux-CC 드라이버(control-mode 파서, 큼)는 split 후 별도. quick terminal/global shortcut은 직교라 별도.
