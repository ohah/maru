...
- macOS live PTY Metal smoke: `mise run test-macos-app-pty-metal-smoke`로 summary 계약을 검증하고, `mise run macos-app-pty-metal-smoke`로 controlled command의 실제 PTY output과 scripted input roundtrip을 `PtyReader -> PtyEventQueue -> RuntimeEventPump -> SurfaceRuntime -> AppWindow active surface -> TerminalCore -> DrawList -> CoreTextDrawListShaper -> RendererState -> GlyphQuadFrame/GlyphRasterFrame -> Metal atlas shader sampling -> screenshot`까지 통과시킨다. 입력은 ready marker를 PTY output에서 먼저 관측한 뒤 `SurfaceRuntime.writeInput` 경로로 보낸다. summary에는 `renderer_input=surface_runtime_live_pty_draw_list`, `input_source=surface_runtime_write_input`, `scripted_input_sent`, `screen_contains_expected`, `output_events`, `exit_events`, `termination`, `product_atlas_uploaded`, `product_atlas_sampled`, `screenshot_artifact`, raw/screen/snapshot/screenshot artifact 경로를 남긴다. 이 단계는 실제 PTY bytes와 terminal input bytes가 보이는 Metal UI까지 도달하는 첫 검증이지만, prompt/dotfile/ANSI escape에 좌우되는 interactive shell과 AppKit 키 이벤트 loop는 아직 아니다.
