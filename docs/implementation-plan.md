...
- font layout test: fake font backend로 `DrawList -> GlyphRunList` 계약을 검증한다. native shaper가 이미 font id/glyph id 후보를 만든 경로는 `ShapedGlyphRecord -> GlyphRunList` adapter test로 별도 검증해 CoreText smoke 전용 변환 로직이 제품 frame 준비 로직과 갈라지지 않게 한다.
...
- `mise run macos-coretext-smoke`가 macOS에서 창/GPU 없이 default `ResolvedAppearance` font 요청 전달, CoreText font resolve, `ShapedGlyphRecord -> GlyphRunList` adapter, `GlyphFrame` 준비, CPU bitmap rasterization을 확인한다. 아직 설정 파일/설정 UI/runtime reload, Metal texture upload와 실제 화면 text draw가 없다는 한계는 summary와 PR 설명에 적는다.
