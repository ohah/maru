...
8. **CR6 — 제품 gate:** CR6c에서 actual daemon/manifest → 일반 launch discovery → primary recovery row → 실제
   ...
   **현재 판정(2026-09-11 코드·gate 대조): P1~P5의 로컬/일반 CI 구현과 ad-hoc 제품 gate는 완료.** P4의 actual
   ...
   CR6e-c3c key→screen hard cap 확정이다. **CR6e-c3c-sample-set은 구현·1차 실측 완료**했으며, 같은
   ...
   임의로 만들지 않는다. 2026-09-11 첫 20회 sample-set은 19.869~22.612ms, median 20.792ms였다.
   반복 sample-set 실측 뒤 별도 doc-first 슬라이스가 baseline을 환경별 예산으로
