...
- 후속: 미정 — 사이드바 글자색 테마화(`sidebar_foreground`, 기본=foreground)·활성 탭 글자 강조·긴 제목 말줄임·드래그 중 floating 탭 시각 피드백. 이후 split 단계(SplitTree + 멀티-panel 렌더 — 같은 origin-offset/cell-배열 메커니즘 확장, 큼)·tmux-CC 드라이버(control-mode 파서, 큼)는 별도. quick terminal/global shortcut은 직교라 별도.
