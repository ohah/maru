...
- **split PR5b 예정(pane exit 자동 collapse)**: split 안의 한 panel에서 셸이 `exit`하면 그 panel을 자동으로 닫고(collapse) 형제가 차지하게 — 지금은 죽은 panel이 트리에 남는다(단일 panel/마지막 panel은 기존대로 세션 종료 latch). 활성/배경 탭·비활성 panel 모두 다뤄야 하므로 `closePane(tab, index)` 일반화가 필요.
