...
- 후속: PR3b-2(탭 번호·제목 glyph — 합성 DrawList + 공유 atlas 두 번째 패스 + 업로드 머지) → PR3c(사이드바 클릭→탭 히트테스트→switchTab, Cmd+Shift+]/[ 전환) → PR3d(탭 드래그 재정렬) → PR4(탭 close, active_tab clamp). 이후 split 단계(SplitTree + 멀티-panel 렌더 — 같은 origin-offset/cell-배열 메커니즘 확장, 큼)·tmux-CC 드라이버(control-mode 파서, 큼)는 별도. quick terminal/global shortcut은 직교라 별도.
