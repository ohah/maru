...
- **N0b — 시각 기준 수령(코드 변경 없음).** [file-panel.md](file-panel.md) §1·[editor-surface.md](editor-surface.md)
  §3.5가 목업을 받아 배치를 확정한 것과 같은 절차다(native-editor.md §1.1). **N1 착수 전에 필요한 것은
  gutter 요소 순서와 폭·본문 여백·상태바 우측 항목 순서**뿐이고, diff 좌우 배치는 N1.5 전에, 호버 박스와
  미니맵 폭은 N4~N5 전에 받으면 된다. 색은 이미 확정이라(`syntax_theme.zig` 파생) 목업 대상이 아니다.
- **N0 — 성능 baseline(코드 변경 없음).** 현행 CM6 경로에서 대형 파일 open·스크롤 프레임타임 p50/p99·타이핑→반영
  지연을 빌드 모드·기기와 함께 기록한다. **이관 근거 중 하나가 성능인데 측정 기록이 없으므로 이것이 선행한다**
  (native-editor.md §1.0·§10). 다른 단계와 병행 가능하지만 N5 이전에 끝나야 비교가 성립한다.
- **N1 — 읽기 전용 등폭 뷰.** 화면에 파일이 뜬다. **여기까지가 diff 본문의 전제**이고, 편집·IME를 건드리지 않아
...
- **N2 — 편집.** 버퍼(rope)와 undo(delta)는 이미 확정돼 있으므로(§3.0) 여기서는 **구현과 프로파일 검증**이다.
  계약 절과의 대응:
...
| [performance-budget.md](performance-budget.md) | 편집기 프레임 예산 항목(native-editor §10) | N0 |
...
**N0 결과가 결정을 되돌릴 수 있는 범위**: baseline에서 **현행 CM6가 이미 충분히 빠른 것으로 나오면** 이관 근거
셋 중 성능 하나가 무너진다(native-editor.md §1.0). 그래도 룩 일관성·역량/통제력이 남으므로 **이관을 취소하지는
않지만**, 그 경우 ⑴ N1.5까지만 하고 편집 이관은 뒤로 미루는 선택, ⑵ 성능을 근거로 든 PR 서술을 정정하는 것,
둘을 그 시점에 사용자와 결정한다. **측정 결과가 어느 쪽이든 그것을 문서에 반영하지 않고 진행하지 않는다.**
