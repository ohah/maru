...
- 아직: reverse(`7`/`27`)는 Style에 필드가 없어 소비만 한다. 배경색 erase(BCE — 현재 pen 배경으로 지우기)는 아직 없다. insert/delete line(IL/DL)·DECOM(origin mode)·커서 표시(?25) 등 나머지 CSI/모드는 소비만 한다.
