...
- 후속: **PR3b-2b(라이브 제목 렌더)** — tick에서 사이드바 frame을 빌드해 `MetalFrameBuffer`가 터미널+사이드바 cells/uploads/pixels를 머지(사이드바 upload offset += 터미널 pixels 길이), 렌더러는 사이드바 glyph 셀을 슬롯 안 세로 중앙 ch-높이로(밴드는 슬롯 전체) 그린다 → **cmux식 탭 슬롯 나머지**(호버 X 닫기 아이콘 — 호버 hit-test는 PR3c와, X 글리프는 PR3b-2b와 묶임) → PR3c(사이드바 클릭/호버→탭 히트테스트→switchTab, Cmd+Shift+]/[ 전환) → PR3d(탭 드래그 재정렬) → PR4(탭 close, active_tab clamp). 이후 split 단계(SplitTree + 멀티-panel 렌더 — 같은 origin-offset/cell-배열 메커니즘 확장, 큼)·tmux-CC 드라이버(control-mode 파서, 큼)는 별도. quick terminal/global shortcut은 직교라 별도.
