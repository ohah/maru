...
- 후속: **cmux식 탭 슬롯**(슬롯 높이 ~2.5× + 호버 X 닫기 아이콘 — 사이드바를 cell-grid 밴드에서 **픽셀 슬롯 레이아웃**으로 확장; 호버 hit-test는 PR3c와, X 글리프는 PR3b-2와 묶임) → PR3b-2(탭 번호·제목 glyph — 합성 DrawList + 공유 atlas 두 번째 패스 + 업로드 머지) → PR3c(사이드바 클릭/호버→탭 히트테스트→switchTab, Cmd+Shift+]/[ 전환) → PR3d(탭 드래그 재정렬) → PR4(탭 close, active_tab clamp). 이후 split 단계(SplitTree + 멀티-panel 렌더 — 같은 origin-offset/cell-배열 메커니즘 확장, 큼)·tmux-CC 드라이버(control-mode 파서, 큼)는 별도. quick terminal/global shortcut은 직교라 별도.
