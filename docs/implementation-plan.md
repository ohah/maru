   ...
   **2c3b-3은 response-side execution/ownership**을 구현한다. client-slot-internal
   ...
   9. **2c3d one-shot event facade (doc-first):** generation event는 raw `BufferedEvent` 값 반환 대신 caller-final-address
      ...
      **C3-3a3 product activation**이 기존 take/release에 a1을, 현재 존재하는 모든 generation mutation final admission에 a2를 동시에
      ...
      closed 7-row의 `Busy|AdminBusy|false|observer success no-op` 및 owner retention을 전수 증명한다. public nonblocking input은 `0`,
...
- **N1 — 읽기 전용 등폭 뷰.** 화면에 파일이 뜬다.
  ...
  **진행 상황(2026-08-10)**: 골격이 섰고 **계약의 절반 이하가 구현됐다.** "N1 완료"가 아니다.
  ...
  | 구현됨 | 남음 |
  |---|---|
  | `line_index`(byte↔논리행) · `geometry`(§4.1 영역 분할) · `gutter`(줄 번호) · `content`(본문·탭 전개) · `viewport`(스크롤·컬링) | **§3.8 적대적 입력 전부**(BiDi·제어 문자 가시화, 초장문 줄 축소, 중첩 상한) · 랩과 랩 토글 · 접힘과 전체 접기/펼치기 · 줄별 폭 합(L2 캐시) · 스크롤바 · 상태바 커서 위치(`ItemId` 확장) · 선택 하이라이트 · 파일 입출력 실제 배선 |

  **§3.8이 가장 시급하다.** 그 절이 "**읽기 단계부터 필요하다**"고 명시한 이유가 첫 제품 가치와 직결된다 — 검토 대상이 에이전트가 만든 코드이므로 "보이는 것과 실제가 다르면 안 된다"가 N1의 요구이고, BiDi 제어 문자 가시화는 Trojan Source 방어다. **지금 상태로 파일을 열면 그 방어가 없다.**
