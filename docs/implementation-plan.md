...
- **SV2 — 파일 탐색기 이관.** 행 단위 좌표를 backing pixel로 옮기는 것이 실제 변경이다. 부분적으로
  ...
  **코드를 읽고 확인한 것**(SV1d 직후 조사). 이 셋이 단계 나눔을 정한다.
  ...
  - **SV2-0 — 판정자.** 탐색기 행·스크롤바를 실제로 보는 게이트를 만든다. 판정 기준은 하나다 —
     ...
     **A로 간다**(2026-08-07 결정). 아래 셋 중 A만이 뒤 단계를 막지 않으면서 탐색기 렌더를 통째로
     바꾸지 않는다. 렌더러에 **같은 패턴이 이미 둘** 있다 — 사이드바 스크롤 scissor(셀 버퍼의 연속
     ...
     **앞서 이 자리에 적었던 "함정"은 틀렸다(2026-08-07 정정).** "pane 구간 index를 먼저 기록하면
     ...
     - **A(선택). 셀 pane이 자기 scissor를 갖도록 렌더러·ABI를 넓힌다.** `modal_clip_*`을 넣을 때의
       주석이 "부분 카드 픽셀 스크롤(알림 패널 등) 재사용 인프라, 컴포넌트 적용은 후속"이라 원래 이
       방향을 염두에 둔 자리다. 프레임당 하나인 것을 pane별로 넓힌다. ABI는 끝에 필드를 더해 기존
       offset을 보존하고, quad 소비자에는 영향이 없다. 덤으로 모달 clip 경로에 첫 소비자가 생긴다.
     - **B. 탐색기 텍스트를 measured 경로로 옮긴다**(도크와 같게). 셀 정렬 전제·아이콘 2패스·성능
       예산을 전부 다시 봐야 한다.
     - **C. 부분 행을 포기한다.** SV2는 스크롤 상태와 스크롤바만 통일한다. SV3~SV5가 기대는 픽셀
       경로가 안 생기므로 뒤 단계가 막힌다.
