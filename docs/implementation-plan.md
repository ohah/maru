...
- 미정 폴리시: 사이드바 글자색 테마화(`sidebar_foreground`)·활성 탭 글자 강조·긴 제목 말줄임·드래그 중 floating 탭 시각. 비활성 panel 위 마우스 휠 라우팅. 워크스페이스 생성을 사이드바 "+" 버튼으로(현재 ⌘⇧T 임시). 탭 제목에 번호 prefix(현재 제목만). tmux-CC 드라이버(control-mode 파서, 큼)는 split 후 별도. quick terminal/global shortcut은 직교라 별도.
