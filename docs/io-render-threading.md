...
- **PR5 — 문서 갱신 + 후속 테스트**: 이 문서·`pty-operating-model.md`(interactive 처리 위치 변경) 갱신 ✅. "렌더 tick 없이 응답 전달" 결정론 통합 테스트(§6-1)·폭주 중 응답 지연 상한(§6-2) ✅(`test-pty`). **남은 후속**: 동시성 스트레스(§6-3)·close race(§6-4)(현재는 setProcessing 게이트 단위 테스트 + §6-1·§6-2 + 실 codex 실측 + 기존 close 테스트로 커버).
