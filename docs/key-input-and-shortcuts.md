...
- **「`resolveEditor` 가 `.app_action` 을 내나」로 물으면 안 된다**(2026-09-06 적대적 검증 7회차).
  ...
  *"비교 뷰에서 그 키는 앱의 것"* 이 깨진다(좌우 분할이 죽는다).
  ...
  - **답은 provenance 를 내는 것이고, 그 선례가 이미 있다**: `resolveWeb` 이 `resolveWebDetailed` 를
    감싸 *"provenance 를 보존한다"*. 같은 모양으로 `resolveEditorDetailed` 를 두고 `resolveEditor` 가
    그것을 감싼다 — **호출자 스무 곳을 안 건드리고**(실측) 순서의 출처도 하나로 남는다.
