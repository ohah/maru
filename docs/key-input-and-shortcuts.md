...
- 한 transaction에서 앞 음절이 확정되어 PTY admission되고 다음 음절의 marked text가 시작되면, `Surface`의 client-local preedit는 아직 base cursor가 인수하지 않은 확정 셀과 그 IME callback bytes를 bounded carry로 보존한다. carry는 직전까지 preedit로 이미 표시됐던 내용을 host echo 전의 같은 화면 위치에만 일반 스타일로 유지하고, 다음 marked text는 그 뒤에 반전 스타일로 합성한다. 단일 셀 글자가 다음 행으로 넘어가는 경우에도 carry를 양쪽 행에 그린다. wide glyph가 행 경계를 가로질러 정확한 셀 위치를 증명할 수 없으면 carry를 폐기한다. 이후 base cursor 전진이 carry 폭을 부분 또는 전부 인수할 때 앞 codepoint부터 폐기하며, cursor의 비순차 이동·geometry 변경·조합 clear/take·surface 소멸에는 전부 폐기한다. 따라서 `한` 확정 직후 `글` 조합에서 앞 음절이 host echo까지 빈칸으로 사라지지 않는다. carry는 고정 256-byte 상한이고, 초과·제어문자·잘못된 UTF-8은 문자열 carry 없이 기존 폭 보정도 폐기해 fail-closed한다. canonical grid·PTY admission·MRSH wire·host runtime은 바꾸지 않는다.
