...
**`F7`·`⇧F7` — 다음/이전 충돌 구간(2026-09-15, [병합 충돌 해결](editor-merge-conflicts.md) S5).** 이 컨텍스트에
처음 들어온 기능키다. 전수 대조: 기능키는 Swift 가 IME 를 거치지 않고 `handleKeyDown` 으로 보내고(위
「기능키」 문단), ①ʹ 선-가로채기 목록에도 ② 메뉴 keyEquivalent 에도 없으며, ③ʹ 의 편집기 앞 갈래는 파일
트리가 초점일 때의 `F2` 뿐이다 — 그래서 편집기 Term 에서는 `resolveEditor` 까지 온다. 터미널 Term 에서는
컨텍스트가 아니라 그대로 `CSI 18~` 로 나간다(이 표는 편집기 Term 에서만 읽힌다). VS Code 병합 편집기에는
이 키가 없고(버튼만 — 실측) **비교** 편집기의 「다음 차이」 관례를 빌린 것이다. 판정자 `ETX4` 는 컨텍스트
표의 항목을 ⑴ `⌘` 없는 `⌥` · ⑵ 근거가 적힌 전역 예외 · ⑶ **수식키 없는 기능키** 셋으로 가르고, ⑶ 은
...
**`⌘F12`(Go to Implementation)는 안 맨다(2026-09-22, [툴링](editor-surface-tooling.md) §8.2m ③).** ⑵ `⌘` 예외는 `ETX4` 가 *"반드시 편집 가능한
문서를 요구한다"* 고 정해 두었고(비교 뷰에서 전역을 가로채지 않게) 이동은 읽기 전용에서도 되어야 한다 — 모순이라 팔레트 명령만 둔다. 키가 필요해지면
그 규율(⑵ 의 `needs_editable`)을 먼저 고친다.
...
**`⌃⇧⌘→`·`⌃⇧⌘←` — 선택 확장·축소(2026-09-23 사용자 결정, [툴링](editor-surface-tooling.md) §8.2q).** VS Code mac 주 키(`editor.action.smartSelect.expand/shrink`).
**이것이 컨텍스트 표에 들어온 첫 `⌃⌘` 조합이라 `ETX4` ⑵ 를 연다**: ⑵ 는 *"`⌘` 는 반드시 끼고 `⌃` 는 안 낀다 — `⌃` 는 제어문자를 뺏는다"* 였는데, 그 근거는
⑷ 가 이미 적었듯 **터미널 Term 의 것**이고 이 표는 편집기 Term 에서만 읽힌다. 그래서 ⑵ 에 `⌃` 가 낄 수 있되 **항목마다 `allowed_control_command`
목록에 근거와 함께** 선다(⑷ 의 `allowed_control` 과 같은 관문). 전수: 전역 표·터미널 매크로 표에 `⌃`+화살표는 없고(`grep "arrow_left\|arrow_right"`
— `⌘`·`⌥`·`⌥⌘`·`⇧⌥⌘` 뿐), 카탈로그 밖 메뉴 키는 `⌃⌘f` 하나(화살표 아님), AppKit `StandardKeyBinding.dict` 에 `⌃⇧⌘` 화살표가 없고, 시스템 단축키
(SkyLight 읽기 전용 스캔, id 0~69999)에 `⌘`+화살표가 없다. 편집기에서 지금 이 chord 는 이동 표가 `⌃` 를 무시해 `⇧⌘→`(줄 끝까지 선택)의 중복이다 — 뺏는
것이 없다. `needs_editable = true` — 비교 뷰에서는 컨텍스트가 지고 그 중복 동작으로 떨어진다(비교 뷰는 축이 둘이라 이 기능 밖이다).
**VS Code 보조 키 `⌃⇧→`·`⌃⇧←` 는 안 맨다.** 시스템 단축키 82·249 가 그 chord 에 켜져 있는데, 같은 표에 맨 `←/→`·`⌥←/→` 도 켜진 채 앱에 닿으므로
「켜져 있다 ≠ 가로챈다」이고 합성 이벤트로 가리면 사용자의 스페이스가 넘어갈 수 있어 실측하지 않았다. 필요해지면 실기에서 가린 뒤 ⑷ 로 넣는다.
...
**남은 충돌은 ~~넷~~ 여섯이다**(2026-09-07 적대적 검증 13·14회차가 둘을 더 찾았다). §9.1 의 경계
기준 — *"그 키가 **문서에** 작용하는가"* — 으로 여섯 다 **편집기 것**인데 터미널 동작이 이기고 있다.

**층이 넷이었다 — resolver 는 맨 마지막이다.** 위 표에 ③ʹ 를 더한다:

| 층 | 무엇 | 이 여섯에 대해 |
|---|---|---|
| ① 뷰 `performKeyEquivalent` | 메뉴 양보 판정 | 안 걸린다(아래) |
| ①ʹ Swift `handleKeyDown` 선-가로채기 | `⌘C`·`⌘V`·`⇧Page*`·`⌘↑`·`⌘↓` | **넷을 먹는다** |
| ② 메뉴바 keyEquivalent | 카탈로그 항목 | **안 먹는다** — 이 여섯은 전역 표에도 액션에도 없다(5·6회차 실측). 그래서 ①ʹ 게이트가 실제로 돈다 |
| ③ʹ **Zig `handleKeyEvent` 의 편집기 분기 앞** | 오버레이·갤러리·도크·파일트리 + **페이지 스크롤** | **맨 `PageUp`/`PageDown` 을 먹는다** |
| ③ `resolveEditor` | 컨텍스트·전역 표 | 여기까지 **안 온다** |

**③ʹ 가 이번에 드러난 자리다**(13·14회차). 편집기 분기 앞에 소비 지점이 **스물일곱**인데, 그중 스물여섯은
**자기 컨텍스트 게이트를 가진 주인**(검색창·이미지 갤러리·에이전트 도크·파일 트리·모달)이고 정상이다.
**하나만 터미널 기능인데 Term 종류를 안 본다** — 페이지 스크롤 블록이다:

- `pageScrollDelta(page_keys_scroll, alt_active, key)` 가 0이 아니면 `scrollPage` 를 부르고
  **`keyConsumedByApp` 로 끝낸다**. `scrollPage` 는 편집기를 거절하므로 **스크롤도 안 되고 편집기에도
  안 온다** — 키가 통째로 사라진다.
- **기본 설정에서 그렇다**(`input.page_keys` 기본이 `.scroll` — 실측).

**그리고 `⇧` 쪽 피해는 두 chord 보다 넓다**(8·9회차). Swift 의 스크롤 게이트가
`event.modifierFlags.contains(.shift)` 라 **정확 일치가 아니다** — `⇧⌘PageUp` 같은 조합도 함께
먹는다. 바로 위 프롬프트 점프 게이트는 `chordMods == .command` 로 **정확**하고, 이 저장소가 브라우저
nav 에서 *"`.contains(.command)` 은 `⌘⇧R` 까지 삼킨다(리뷰 [8])"* 로 이미 배운 그 실수다. 전역 표에
`Page*` 바인딩이 **하나도 없어**(9회차) 지금 터미널 쪽 피해는 없지만, 편집기 쪽은 **지금 살아 있다**.
...
- **판정은 Zig 가 한다.** `term_ops.activeTermIsTerminal` 이 이미 그 답을 들고 있다(`scrollPage` 가
  ...
  같은 부류의 부작용 없는 질의다.
- **③ʹ 의 페이지 스크롤 블록에도 같은 게이트가 필요하다.** Swift 만 고치면 맨 `PageUp` 은 여전히
  죽는다 — 그 블록은 Swift 를 안 거치고 Zig 안에서 소비한다. **`scrollPage` 가 거절하는 것으로는
  부족하다**: 거절해도 `keyConsumedByApp` 가 이미 끝을 낸다. **`page_delta != 0` 판정 자체를 터미널일
  때만** 하게 한다.
- **Zig 쪽 가드의 대칭은 「도달 가능한가」로 갈린다**(11·12회차 실측). `scrollPage` 는 제품 호출자가
  **둘**(ABI·`handleKeyEvent`)이라 그 가드가 살아 있다. **`jumpToPrompt` 는 ABI 하나뿐**이라 Swift 를
  게이트하면 거기 더한 가드는 **어떤 판정자도 지킬 수 없는 줄**이 된다. 그래도 두되 **등가임을 주석에
  적는다** — 근거는 대칭이 아니라 *"Swift 를 안 거치는 호출자가 생기는 날"* 이고, 그것은 이 저장소가
  `toggleCursorAt` 의 「마지막 커서는 안 지운다」 가드에서 이미 쓴 규율이다.
...
- **고쳐지면 §3.9 가 그 키들을 받는다**(실측 — 편집 키 갈래가 `⌘↑ → .doc_start`, `⌘↓ → .doc_end`,
  ...
  않는다** — 이미 있는 것이 닿게만 한다.
- **터미널은 안 바뀐다.** 활성 Term 이 터미널이면 여섯 다 지금 그대로다.
  - **`contains(.shift)` 를 정확 일치로 좁혀도 그렇다**(2026-09-07 — 처음엔 *"`⇧⌘PageUp` 이 이제
    PTY 로 간다"* 고 적었는데 **거짓이었다**). `pageScrollDelta` 는 **수식자를 아예 안 본다**(실측 —
    인자가 `(scroll_mode, alt_active, key)` 뿐이다). 그래서 Swift 가 안 먹은 `⇧⌘PageUp` 은 **Zig
    페이지 블록이 먹어** 터미널에서 여전히 스크롤한다. **바뀌는 것은 어느 층이 먹느냐뿐**이다.
  - **그럼에도 좁히는 이유는 편집기 쪽이다.** `contains` 로 두면 편집기에서 `⇧` 낀 Page 조합이 Swift
    단계에서 **통째로 사라진다**(Zig 블록까지 못 간다). 좁혀야 그 조합이 게이트를 지나 편집기에 닿는다.
    이 저장소가 브라우저 nav 에서 *"`.contains(.command)` 은 `⌘⇧R` 까지 삼킨다(리뷰 [8])"* 로 내린
    결론과 같은 부류이고, 여기서는 **터미널 쪽 대가가 0** 이다.
  - **그래서 편집기 피해는 「여섯 chord」가 아니라 「Page 키 전부 + `⌘↑`·`⌘↓`」다.** Zig 블록이
    수식자를 안 보므로 `⌥PageUp`·`⌃PageDown` 같은 것도 편집기에서 죽어 있었다.
- **고친 뒤 편집기에서 Page 키의 뜻은 「수식자 무관 페이지 이동」이다**(2026-09-07 D회차). 편집 키
  갈래의 이동표가 `.page_up => .page_up` 으로 **수식자를 안 보고**(`⇧` 만 `extend` 로 쓴다), 이 조각은
  그 표를 **안 바꾼다** — 키가 거기 닿게만 한다. `⌘PageUp` 이 페이지 이동이 되는 것이 macOS 관례와
  정확히 같지는 않지만, 그 판단은 §3.9 이동 일습의 몫이지 이 절의 몫이 아니다.
- **웹·파일 Term 에서 사용자가 보는 결과는 안 바뀐다**(E회차). 전에는 Swift 가 먹고 `scrollPage` 가
  거절해 아무 일도 안 났고, 뒤에는 게이트가 비켜서 `frame_loop` 로 흐르는데 **PTY 가 없어** 역시 아무
  일도 안 난다. **바뀌는 것은 층뿐**이다 — 다만 `⌘↑` 는 개선이다(전에는 엉뚱한 큐를 넣었다).
- **`page_keys = .passthrough` 에서는 전·후가 완전히 같다**(F회차). `pageScrollDelta` 가 0 을 내므로
  블록이 애초에 소비하지 않는다 — 게이트가 있든 없든 터미널은 PTY 로, 편집기는 편집기가 받는다.
- **웹·파일 Term 에서는 안 돈다.** 그것도 터미널이 아니므로 같은 게이트에 걸린다 — 지금은
  `scrollPage` 가 조용히 거절하고 `jumpToPrompt` 는 엉뚱한 큐를 넣는 **갈린 상태**인데, 그 갈림이
  사라진다.
  - **그 키가 셸로 새지 않는다**(2026-09-07 적대적 검증 2회차 — 위험해 보여 확인했다). 게이트를
    지난 키는 `frame_loop.handleKeyEvent` 로 흘러가는데, 이 파일이 이미 *"편집기 Term 에 PTY 가 없어
    **바이트가 갈 곳이 없다**"* 고 적어 둔 그 성질 때문에 무해하다. 웹 Term 도 PTY 가 없는 갈래
    셋 중 하나다.
- **비교 뷰에서는 그 넷이 죽은 키가 된다**(1회차). 편집 키 갈래가 `ed == .editor **and !is_diff**`
  이므로 `⌘↑` 가 문서 처음으로 **안 간다** — 비교에는 문서 offset 축이 없기 때문이다(§4.1g). 지금은
  프롬프트로 튀므로 **틀린 동작에서 아무 동작으로** 바뀌는 것이고, 그 편이 낫다. 비교 뷰의 이동
  일습은 이 조각 밖이다.
- **`activeTermIsTerminal` 을 재는 판정자가 하나도 없다**(10회차 — 이 조각이 세운다). `scrollPage` 의
  거절조차 안 재고 있어 그 가드를 지워도 초록이다. 앞 조각의 「선례에 판정자가 없다」와 같은 자리다.
- **게이트 값은 `scrollPage` 가 쓰는 그 함수 하나에서 온다**(3회차). `activeTermIsTerminal` 을 새로
  구현하면 출처가 둘이 되고, 그 둘은 반드시 갈린다 — 지금 `jumpToPrompt` 가 가드를 안 가진 것이
  바로 그 갈림이다.
  - **그 함수는 fail-open 이다** — `surface_initialized` 가 거짓이거나 탭이 없으면 **참**을 낸다.
    이 게이트에서는 그것이 안전한 방향이다(시작 중에는 선-가로채기가 그대로 돈다). 반대로 fail-close
    였다면 창이 뜨는 동안 `⌘C` 계열이 조용히 죽었을 것이다.

#### 비교 뷰에 **caret 을 세운다** (2026-09-08)

바로 위 「선-가로채기」 조각이 **비교 뷰를 죽은 키로 남겼다** — 편집 키 갈래가
`ed == .editor and !is_diff` 라 `⌘↑`·`PageUp` 이 아무 일도 안 한다. 그 조각은 *"틀린 동작에서
아무 동작으로 바뀌는 것이고 그 편이 낫다"* 로 한계를 적었고, 이 절이 그 한계를 닫는다.

**처음에는 「caret 없이 뷰만 굴린다」로 적었다가 뒤집었다**(2026-09-07 → 09-08). 그때 근거가
*"비교는 읽기 전용이니 caret 이 없다"* 였는데, **읽기 전용과 caret 없음은 다른 것**이다.

- **JetBrains** 는 비교 뷰의 열 전환을 *"Switch between the panes of the Diff Viewer. **The active
  pane has the caret.**"* 로 정의한다. 같은 문서가 *"You can change text only in the right part of
  the side-by-side viewer"*·*"You cannot edit files that have read-only status"* 라고도 적는다 —
  **편집할 수 없는 쪽에도 caret 이 있고**, caret 이 오히려 「어느 열이 활성인가」의 표식이다.
- **VS Code** 는 읽기 전용 파일에서 `⌘D`(다음 일치 선택)가 **동작하고**, `insertCursorAbove/Below`
  가 안 되는 것은 **확인된 결함**으로 열려 있다(`microsoft/vscode#204183`).
- 그러니 caret 은 **편집 권한이 아니라 읽는 도구**다. 우리 비교 뷰가 읽기 전용인 것은 caret 을 안
  만들 근거가 못 된다. 읽기 전용이라는 사실 자체는 실측이다 — `insertText`·`deleteBy`·`pasteText`·
  `cutSelection`·`saveDocument`·`lineOpDoc`·`toggleLineComment`·`stepHistory`(undo)가 저마다
  `editor_diff != null` 을 보고 거절한다.

**축은 이미 있다 — 새로 짓지 않는다.**

- **caret 은 선택의 focus 다.** `RowSelection` 이 `anchor_start`·`anchor_end`·**`focus`** 를 이미
  들고(`session/editor/selection.zig`), 비교 선택이 그 타입을 그대로 쓴다. caret 자리를 **따로 든
  필드로 두지 않는다** — 두면 선택과 caret 이 두 출처가 되고, 그 둘은 반드시 갈린다.
- **비어 있으면 caret 뿐이라는 뜻도 이미 계약이다.** `buildDiffSelectionMarks` 가
  *"`if (sel.sel.isEmpty()) return null;` — caret뿐, 그릴 띠가 없다"* 로 그 상태에 이미 이름을
  붙여 놨다. 지금은 그 상태를 **아무도 그리지 않을** 뿐이다.
- **어느 열인가도 이미 있다.** `editor_diff_selection.side` 가 그 값이고 마우스가 세운다.
  `diff_frame.Shared.carets` 주석이 *"비교 뷰는 안 쓴다 — 커서가 어느 쪽 것인지 판정이 선행하고,
  그 판정은 아직 없다. (…) 자리만 뚫어 둔다"* 고 적은 그 판정이 **그것**이다. 자리도 뚫려 있고
  판정도 있는데 **이어져 있지 않다.**
- **좌우 행 배열은 길이가 같다** — `scrollLines` 가 *"비교 Term 은 좌우 **행** 배열이 문서다(좌우
  길이가 같다)"* 로 그 축에 이미 기대고 있다. **다만 타입이 그것을 강제하지 않는다**(`materialize`
  가 `rows.left.len`·`rows.right.len` 으로 따로 잡는다) — 그러니 caret 의 행 첨자는 **자기 열의
  배열 길이로** 자른다. 반대 열 길이로 자르면 그 불변식이 깨지는 날 범위 밖을 훑는다.
- **`carets` 는 `Shared` 에서 `Side` 로 옮긴다.** 지금 자리는 좌우 **공통** 값 묶음이라 한 열에만
  그릴 수가 없다. 옮기면 `selection_marks`·`search_marks` 와 같은 축이 되고(그 둘도 열마다 `null`
  이 될 수 있다), **비활성 열은 `null`** 이 그 열에 caret 이 없다는 뜻이 된다. `caret_visible`·
  `caret_shape` 는 좌우 공통이 계약이므로 `Shared` 에 남는다.

**caret 은 열자마자 오른쪽 `(행 0, byte 0)` 에 선다.**

- **오른쪽인 이유**: 왼쪽은 *"git 이 준 HEAD 판이라 열려 있는 문서가 아니다"*(문서 모델 §비교 뷰).
  오른쪽이 「지금 내 파일」이고, JetBrains 가 편집 가능한 쪽으로 오른쪽을 두는 것과 같다.
- **부수 결과를 하나 받는다.** `diffSearchSide` 는 선택이 없으면 `.left` 로 떨어지는데, caret 이 늘
  서 있으면 **비교 뷰 검색의 기본 열이 오른쪽이 된다**. 사고가 아니라 **의도한 변경**이다 — 검색도
  「내 파일」에서 시작하는 편이 맞다. 판정자가 이 값을 고정한다.
- **비교 내용을 다시 계산하면 caret 도 다시 선다.** 선택을 버리는 것은 이미 계약이고(옛 행 인덱스로
  훑으면 죽는다), 그 뒤에 caret 을 **오른쪽 (0, 0)** 으로 되세운다 — 「늘 있다」를 지키면서 옛
  인덱스를 안 쓰는 값은 그것뿐이다. 세로 스크롤 위치는 안 건드린다.

**키 표** — 단일 편집기 §3.9 의 축을 `(행, 행 안 byte)` 로 옮긴 것이고 뜻은 같다.

| 키 | 비교 뷰에서 | `⇧` 를 얹으면 |
|---|---|---|
| `←`·`→` | 글자 하나(`motion.prevCharBoundary` 와 같은 경계), 행 경계를 넘는다 | 선택 확장 |
| `↑`·`↓` | 한 행 — **목표 열을 유지한다** | 선택 확장 |
| `⌥←`·`⌥→` | 낱말 하나(`motion.wordLeft`/`wordRight`) | 선택 확장 |
| `⌘←`·`⌘→` | 행의 첫 글자(smart)·행 끝 | 선택 확장 |
| `⌘↑`·`⌘↓` | 맨 위·맨 아래 행 | 선택 확장 |
| `PageUp`·`PageDown` | 한 화면 | 선택 확장 |

- **여기 「행」은 랩된 시각 행이 아니다** — 비교 뷰는 한 행이 곧 한 줄이라(랩이 없다) 두 축이
  같다. 단일 편집기에서는 랩이 켜지면 갈리고, `⌘←`/`⌘→` 가 **행 → 줄로 단계 확장**한다
  ([문서 모델](native-editor-document-model.md) §3.9). `^A`/`^E` 는 거기서 **언제나 논리 줄**이다.
- **`⇧` 가 답을 바꾼다 — 그것이 앞 결정과 갈리는 자리다.** caret 이 없던 표에서 `⇧` 는 아무 뜻도
  없었다. caret 이 서면 **anchor 를 두고 focus 만 옮기는** 표준 뜻이 그대로 성립하고, 나중에 뜻이
  바뀌어 손버릇이 깨지는 일도 없다.
- **목표 열은 `RowSelection` 이 든다** — 단일 편집기의 `Selection` 이 `goal`·`anchor_goal` 을 자기
  안에 든 것과 같은 자리다. 밖에 두면 선택을 버릴 때 목표 열만 살아남아 다음 `↑` 가 엉뚱한 열로
  간다.
- **caret 이 보이게 굴린다.** 이동이 화면 밖으로 나가면 `editor_first_line` 이 따라간다. 세로
  상한은 `scrollLines` 가 이미 쓰는 출처 하나를 쓴다 — 두 곳에서 세면 비교 뷰만 한 행씩 어긋난다.
- **`moveCarets` 를 재사용하지 않는다.** 그 함수도 위 목록과 같이 `editor_diff != null` 에서 거절한다
  — 문서 offset 축을 전제하기 때문이고, 그 전제는 비교 뷰에서 참이 아니다. 비교 쪽은 **자기 축의
  이동 함수**를 따로 세우고, 공유하는 것은 그 아래의 `motion.*`(바이트 경계·낱말·행 끝) 하나다.
  **위로 합치지 않는다** — 좌표를 추상화해야 하고, 그 값이 이 조각에 없다(문서 모델이 멀티 커서
  조각에서 같은 판단을 이미 했다).
- **편집은 여전히 거절한다.** caret 이 생겨도 비교 뷰는 읽기 전용이다 — 위 실측이 준 결론의 나머지
  절반이 이것이다. 글자·Backspace·Enter 는 `editor_diff != null` 에서 계속 막힌다.
- **`!is_diff` 게이트를 지우지 않는다 — 옆에 비교 갈래를 세운다.** 그 자리 주석이 *"이 `!is_diff`
  를 지워도 판정자가 안 잡는다(그 변이가 살아남는 것이 정상이다) — 안쪽 함수들이 전부 비교를 스스로
  거절하기 때문"*, 그리고 *"그럼에도 여기서 막는 이유는 **심층 방어**다(§6): 안쪽 거절이 하나라도
  빠지는 날, 비교 Term 의 selection 을 단일 편집기 좌표로 훑는 경로가 열린다"* 라고 적어 둔다.
  그 방어를 걷어내면 이 조각이 **자기가 막으려는 결함을 스스로 연다**. 비교 갈래는 `ed == .editor and
  is_diff` 로 **따로** 열고, 단일 쪽 갈래는 손대지 않는다.
- **caret 을 되세우는 자리는 `invalidate` 가 아니다.** 그 함수는 **Term 이 죽을 때도** 불린다
  (`release` 가 부른다) — 거기서 세우면 죽는 Term 에 caret 을 심는다. 행 배열이 실제로 서는
  `materialize` 뒤가 그 자리다.
- **터미널은 안 바뀐다** — 바로 위 게이트가 활성 Term 이 터미널일 때만 선-가로채기로 보낸다.

**이 조각 밖으로 미루는 것**(빚으로 적는다).

- **열 전환 chord — 이 절 아래 「비교 뷰의 열을 키로 넘긴다」가 갚는다.**
- **랩을 켠 비교 — 이것은 「할 일」이 아니라 닫힌 결정이다.** 랩이 켜지면 좌우가 **각자** 접혀
  그 아래 행부터 어긋나고(실측 — 왼쪽 3행 / 오른쪽 13행), §3.5 가 요구하는 「같은 행이 같은
  높이에 선다」가 그때 깨진다. 판정자가 그 상태를 못 박아 둔다(`알려진 구멍: 랩을 켠 비교는
  좌우가 어긋난다`). **그러나 2026-08-16 사용자 결정이 「고치지 않고 적는다」이다** — Vim 이
  diff 정렬이 깨지는 첫 조건으로 `'wrap' is on` 을 명시하고 고치지 않은 선례를 따랐고, 대신
  완화책 둘을 갔다: **`editor.wrap` 기본값 `false` 되돌림**(`99f79786`)과 **비교 뷰 가로
  스크롤**(2026-08-17·18). **둘 다 이미 끝났다.** 그래서 지금 이 구멍은 랩을 **켠** 사람만
  본다. 이 조각은 그것을 넓히지도 좁히지도 않는다.
- **상태바 `줄:열`.** 계획이 *"비교 뷰는 축이 달라 답하지 않는다"* 로 적어 둔 그대로 둔다. caret 이
  생겨 답할 수는 있게 됐지만, 그 값의 뜻(어느 열의 몇 번째 행인가)은 따로 정할 일이다.



#### 비교 뷰의 열을 키로 넘긴다 — `⌃⇧Tab` (2026-09-08)

바로 위 조각이 **키보드로 읽을 수 있게 만들어 놓고 왼쪽 열에는 못 가게** 남겼다. 실측으로 확정한
구멍이다 — `editor_diff_selection` 을 쓰는 비-테스트 자리가 넷뿐이고(**마우스 둘**·씨앗·이동),
이동은 열을 **유지**하므로 **키로 열을 바꿀 길이 없다**. 검색의 「열 넘기기」는 `find.diff_side`
만 세우고 caret 은 안 건드린다.

**chord 는 `⌃⇧Tab` 이다.**

- **JetBrains 관례와 같다.** 도움말이 `Ctrl+Shift+Tab`(macOS keymap `⌃⇧⇥`)을 *"Switch between the
  panes of the Diff Viewer. The active pane has the caret."* 로 적는다. **`⌃Tab` 이 아니다** — 이 문서가
  한때 그렇게 적었다가 고쳤고, macOS keymap 에서 `⌃⇥` 는 **Switcher**(열린 탭 이동)다.
- **`Tab` 은 resolver 표에 없다**(탭 전환은 `⌘⇧]`/`⌘⇧[`). 메뉴바에도 `Tab` keyEquivalent 가 없고,
  창 탭(`NSWindow` tabbing)을 안 쓰므로 OS 가 가로채지도 않는다.
- **그러나 「임자가 없다」는 아니다.** 편집 키 갈래의 `.tab` 갈래는 **수식자를 안 본다** —
  `selectionSpansLines` 면 들여쓰기/내어쓰기, 아니면 탭 문자다. 그래서 **단일 편집기에서 `⌃⇧Tab` 은
  이미 내어쓰기**이고, 비교 갈래가 그 앞에 서기 때문에 비교에서만 새 뜻을 가진다. 같은 키가 포커스에
  따라 다른 일을 하는 대가는 §9.1 이 이미 받아들인 것이고, 여기서는 **JetBrains 도 같은 짝**이다
  (`⇥`=Indent · `⇧⇥`=Unindent · `⌃⇧⇥`=pane 전환).

**넘어갈 때 무엇을 들고 가나.**

| 값 | 규칙 | 근거 |
|---|---|---|
| 행 | **그대로** | 좌우 행 배열이 짝을 맞춰 정렬돼 있다 — 같은 첨자가 같은 높이다(§3.5) |
| 열 | **표시 열을 유지한다** | byte 를 그대로 옮기면 탭·CJK 가 든 행에서 눈에 보이는 자리가 튄다. 되돌리는 것은 `offsetForGoal` 의 `.col` 갈래 하나를 쓴다 — **다만 `goalAt` 은 안 쓴다**: 그 함수의 「줄 끝 고정」은 **세로 이동**의 성질이라, 열 넘기기에 적용하면 짝맞춤 빈 행에서 **왕복이 깨진다**(실측 — 열 0 이 곧 줄 끝이라 돌아올 때 반대 행의 끝에 선다) |
| 선택 | **접힌다 — caret 만 넘어간다** | *"좌우를 걸치는 선택은 만들지 않는다"* 가 이미 계약이다. 늘린 선택을 들고 넘어가면 그 계약이 깨진다 |
| 세로 스크롤 | **안 움직인다** | 행이 그대로이므로 caret 이 화면 밖으로 나가지 않는다 |
| 검색 열 | **따라가고, 목록도 다시 센다** | `diffSearchSide` 가 명시값이 없으면 caret 열을 본다. **그것만으로는 모자라다** — 강조·막대 마커는 그 답을 **live** 로 읽는데 매치 목록은 `editor_find_matches` 에 **캐시**돼서, 안 세면 §5.1 이 경고한 *「화면은 왼쪽인데 결과는 오른쪽 것」* 이 된다(실측으로 재현했다 — DCOL8). `find_ops.diffCaretSideChanged` 가 그 자리다 |

- **짝맞춤 빈 행으로도 간다.** 반대 열의 그 자리가 빈 행이면 caret 은 그 행의 byte 0 에 선다. 거절하면
  «어떤 행에서는 키가 죽는다»가 되고, 그것이 더 나쁘다 — 화면에는 그 빈 행이 실제로 그려져 있다.
- **왕복은 셀 폭이 같을 때만 제자리다.** 좌우의 폭이 다르면(탭 4열 · CJK 2열) 반대 열에 **그 열이
  없을 수 있고**, 그때 `byteAtPoint` 가 cluster 경계로 스냅한다 — 실측: 오른쪽 `"가나다"` 의 byte 3
  (2열)은 왼쪽 `"\tAB"` 에서 탭 **안**이라 byte 1(4열)로 스냅되고, 돌아오면 byte 6 이다. **지켜지는
  것은 「한 번 왕복한 뒤로는 안 흐른다」**이고, 그것이 없으면 키를 누를수록 caret 이 계속 밀린다.
- **검색 목록 다시 세기는 「열이 실제로 바뀌었을 때」만이다.** 같은 열 안에서 caret 을 옮길 때도
  다시 세면 `current` 가 0 으로 튀어 **사용자가 보던 매치를 잃는다**. 판정은 `diffSearchSide` 하나로
  한다 — 그 답을 읽는 셋이 같은 기준을 써야 한다. **명시로 고른 열이 있으면 아무 일도 안 한다**
  (그때는 caret 이 옮겨져도 검색 열이 안 바뀐다).
- **같은 결함이 마우스에도 있었다.** 반대 열을 **클릭**해도 목록을 안 세고 있었다 — 이 조각 이전부터다.
  열이 바뀌는 세 자리(마우스 두 곳·`⌃⇧Tab`)가 모두 같은 헬퍼를 지난다.
- **caret 이 없으면 아무 일도 안 한다.** 비교가 아직 안 섰거나 행 배열이 비면 거절한다 — 이동 갈래와
  같은 규율이다.

**이 조각 밖으로 미루는 것.**

- **명령 팔레트·재바인딩.** 이동 일습과 같은 자리(키 갈래 안)에 둔다 — resolver 표에 넣으려면 「비교
  에서만」 게이트가 필요한데 그런 축이 없고(`needs_editable` 의 대칭이 아니다), 만들면 게이트가 둘이
  된다. 카탈로그에 올리는 것은 그 축을 세울 때 함께 한다.
- **AppKit 위층은 판정자가 못 잰다.** `⌃⇧Tab` 이 `keyDown` 까지 오는지는 원리상 판정자의 답이
  아니다 — **캡처로 메운다**(위 조각이 같은 자리를 그렇게 메웠다).
...
- 각 `setMarkedText` callback은 Zig preedit를 바꾼 뒤 AppKit main run-loop의 다음 turn에 frame tick 하나를 예약한다. 같은 key transaction의 여러 callback은 한 tick으로 coalesce한다. 이는 `ㅎ → 하 → 한`처럼 같은 셀을 교체하는 조합을 다음 주기 타이머까지 미루지 않기 위한 입력-시각 wake이며, 별도 반복 타이머나 유휴 polling을 만들지 않는다. 로컬·host-backed surface는 같은 경로를 사용한다.
- 한 transaction에서 앞 음절이 확정되어 PTY admission되고 다음 음절의 marked text가 시작되면, `Surface`의 client-local preedit는 아직 base cursor가 인수하지 않은 확정 셀과 그 IME callback bytes를 bounded carry로 보존한다. carry는 직전까지 preedit로 이미 표시됐던 내용을 host echo 전의 같은 화면 위치에만 일반 스타일로 유지하고, 다음 marked text는 그 뒤에 반전 스타일로 합성한다. 단일 셀 글자가 다음 행으로 넘어가는 경우에도 carry를 양쪽 행에 그리며, 덮인 wide base 셀의 앞/뒤 반쪽은 scratch에서 함께 정리한다. wide glyph가 행 경계를 가로질러 정확한 셀 위치를 증명할 수 없으면 carry를 폐기한다. 이후 base cursor 전진이 carry 폭을 부분 또는 전부 인수할 때 앞 codepoint부터 폐기하며, cursor의 비순차 이동·geometry 변경·조합 clear/take·surface 소멸에는 전부 폐기한다. 따라서 `한` 확정 직후 `글` 조합에서 앞 음절이 host echo까지 빈칸으로 사라지지 않는다. carry는 고정 256-byte 상한이고, 초과·제어문자·잘못된 UTF-8은 문자열 carry 없이 기존 폭 보정도 폐기해 fail-closed한다. canonical grid·PTY admission·MRSH wire·host runtime은 바꾸지 않는다.
