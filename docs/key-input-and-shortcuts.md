...
- 조합 중(preedit) 글자는 커서 위치에 반전 스타일로 합성 표시된다(core.setPreedit + renderSnapshot 합성 — 셀 그리드는 오염되지 않고, 조합 갱신/확정/취소·포커스 이탈 시 즉시 정리). 아직: 입력기 후보창의 커서 위치 정밀 배치, preedit 밑줄 스타일(현재 반전).
