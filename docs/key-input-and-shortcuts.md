...
- 한 transaction에서 앞 음절이 확정되어 PTY admission되고 다음 음절의 marked text가 시작되면, `Surface`의 client-local preedit는 아직 base cursor가 인수하지 않은 확정 셀 폭을 anchor advance로 보존한다. 표시 위치는 `base cursor + 미인수 폭`이며, 이후 base cursor 전진이 그 폭을 부분 또는 전부 인수할 때 보정을 감소시킨다. 확정 문자열 자체를 복제해 그리지 않고 폭만 보존하므로 무반향 입력 내용을 노출하지 않는다. 조합 clear/take, cursor의 비순차 이동, surface 소멸에는 보정을 함께 폐기한다. 이 상태는 MRSH wire나 host runtime으로 보내지 않는다.
