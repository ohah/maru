...
- 조합 중(preedit) 글자는 커서 위치부터 반전 스타일로 합성 표시된다(`TerminalCore.snapshotWithPreedit`). 줄 가운데에서 조합하면 커서 뒤 글자들을 조합 폭만큼 오른쪽으로 밀고 그 자리에 조합 글자를 넣어, 확정 후 셸이 그릴 모습(`가나다`의 `나` 앞 조합 → `가[라]나다`)을 조합 중에도 미리 보여준다(삽입형 미리보기). 합성 버퍼(`viewport_cells`)에만 미는 것이라 실제 셀 그리드·셸 상태는 불변이고, 확정 순간 셸이 동일 배치를 그려 화면이 자연스럽게 이어진다. 단 밀면 행 밖으로 콘텐츠가 잘리는 경우(줄 끝 근처)나 조합이 행에 안 들어가는 경우는 기존 오버레이로 폴백한다(뒤 글자 가림 — 잘려 사라지는 것보다 덜 혼란스럽다). **베이스/결정**: 사실상 표준(Ghostty·iTerm2·Terminal.app)은 조합 글자를 커서 칸에 오버레이만 해 뒤 글자를 가리지만(`가라다`), Maru는 이를 한글 입력 결함으로 보고 의도적으로 삽입형을 택한다 — preedit은 셸 미전송 텍스트라 GUI 입력창처럼 자체 버퍼 가운데 삽입이 원칙적으론 불가하지만, 합성 버퍼에만 미리 그려 시각만 흉내낸다. 조합 중 블록 커서는 숨긴다(반전 preedit이 커서 역할 — 안 숨기면 preedit 끝에 블록 커서가 또 그려져 커서가 둘로 보인다). 깜빡임도 멈춘다(보이는 위상 고정 — Terminal.app/Ghostty 동일). 포커스 이탈(view resign + 창 key 상실 — 앱 전환 포함)에는 조합 중 텍스트를 버리지 않고 확정(커밋)해 입력기 상태와 화면이 어긋나지 않게 한다. 아직: 입력기 후보창의 커서 위치 정밀 배치, preedit 밑줄 스타일(현재 반전).
