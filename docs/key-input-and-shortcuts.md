...
- **열 전환 chord.** 열은 **클릭으로만** 바뀐다. JetBrains 는 **`Ctrl+Shift+Tab`** 을 쓴다
  ...
  **`Tab` 키에 걸린 chord 는 지금 하나도 없고**(탭 전환은 `⌘⇧]`/`⌘⇧[` 이다), 다만 편집 갈래가
  맨몸 `Tab` 을 들여쓰기로 쓴다.
- **랩을 켠 비교.** 여기는 **판정자가 없는 자리가 아니라 판정자가 못 박아 둔 결함**이다
  (`알려진 구멍: 랩을 켠 비교는 좌우가 어긋난다 — 조각 단위 정렬 대기`). 랩이 켜지면 좌우가
  **각자** 접혀, 한쪽 줄이 3조각이고 반대쪽이 1조각이면 그 아래 행부터 어긋난다 — §3.5 가
  요구하는 「같은 행이 같은 높이에 선다」가 그때 깨진다. 고치려면 세로 정렬을 **조각 단위**로
  올려야 한다. 이 조각은 그 구멍을 넓히지도 좁히지도 않는다.
