...
- 위 DECCKM/kitty/기능키 인코딩의 mode 단일 출처는 현재 in-process runtime에 한정된다. host-backed 일반 key는
  attachment placeholder의 `EncodeOptions`를 읽으므로 host가 켠 DECCKM/DECKPAM/kitty mode parity는 후속이다.
