...
- **웹·파일 Term 에서는 안 돈다.** 그것도 터미널이 아니므로 같은 게이트에 걸린다 — 지금은
  ...
  - **그 키가 셸로 새지 않는다**(2026-09-07 적대적 검증 2회차 — 위험해 보여 확인했다). 게이트를
    ...
    **바이트가 갈 곳이 없다**"* 고 적어 둔 그 성질 때문에 무해하다. 웹 Term 도 PTY 가 없는 갈래
    셋 중 하나다.
#### 비교 뷰에서는 그 키들이 **뷰를 굴린다** (2026-09-07)

바로 아래 「선-가로채기」 조각이 **비교 뷰를 죽은 키로 남겼다** — 편집 키 갈래가
`ed == .editor **and !is_diff**` 라 `⌘↑`·`PageUp` 이 아무 일도 안 한다. 그 조각은 *"틀린 동작에서
아무 동작으로 바뀌는 것이고 그 편이 낫다"* 로 한계를 적었고, 이 절이 그 한계를 닫는다.

**caret 을 만들지 않는다 — 뷰를 굴린다.**

- **비교 뷰에는 caret 이 없다**(실측 — `editor_diff_selection` 은 마우스로 서는 **선택**이고 이동
  개념이 없다). 그것을 만드는 것은 §4.1g 가 *"그 축은 문서 offset 을 전제하고 비교에는 그 축이
  없다"* 고 적은 그 축을 `(row, byte)` 로 다시 세우는 일이고, **이 조각의 밖**이다.
- **읽는 뷰가 실제로 원하는 것은 스크롤이다.** 비교는 검토용이라(§3.5) 커서를 옮기는 것보다 **화면을
  넘기는 것**이 그 화면의 일이다.
- **기계는 이미 있다**(실측). `scrollLines` 가 비교 Term 을 **명시적으로 다룬다** — *"비교 Term 은
  좌우 **행** 배열이 문서다"* 로 상한을 그쪽에서 잡는다. 판정자들이 이미 그 함수로 비교 뷰를 굴린다.
  **키만 안 닿았다.**

| 키 | 비교 뷰에서 |
|---|---|
| `PageUp`·`PageDown` | 한 화면씩 굴린다 |
| `⇧PageUp`·`⇧PageDown` | 같다 — **선택 확장이 아니다**(늘릴 caret 이 없다) |
| `⌘↑`·`⌘↓` | 맨 위·맨 아래로 |

- **`⇧` 가 답을 안 바꾸는 것이 이 표의 요점이다.** 단일 편집기에서 `⇧` 는 **선택을 늘리는데**, 비교
  뷰에는 늘릴 것이 없다. 같은 키가 두 뷰에서 다른 일을 하는 대가는 §9.1 이 *"같은 키가 포커스에 따라
  다른 일을 하는 대가를 수용한다"* 로 이미 받아들인 것이고, 여기서는 **더 약한 형태**다(둘 다 세로
  이동이고 단위만 다르다).
- **한 화면의 크기는 보이는 행 수다** — 단일 편집기의 `pageRows` 와 같은 근거이고, 값의 출처도 렌더가
  굳힌 행 수 하나여야 한다. 두 곳에서 세면 비교 뷰만 한 줄씩 어긋난다.
- **터미널은 안 바뀐다** — 바로 위 「선-가로채기」 게이트가 활성 Term 이 터미널일 때만 그쪽으로 보낸다.
