...
- **열 전환 chord.** 열은 **클릭으로만** 바뀐다. JetBrains 는 **`Ctrl+Shift+Tab`** 을 쓴다
  (도움말 원문 — *"Switch between the panes of the Diff Viewer. The active pane has the caret."*).
  이 절은 한때 그것을 `⌃Tab` 이라 적었는데 **틀렸다**(2026-09-08 재확인) — 이름만 보고 선례를
  적으면 틀린다는 그 규율이 여기서 또 깨졌다. 우리 chord 를 고를 때는 §9.1 전수 대조를 먼저 한다 —
  **`Tab` 키에 걸린 chord 는 지금 하나도 없고**(탭 전환은 `⌘⇧]`/`⌘⇧[` 이다), 다만 편집 갈래가
  맨몸 `Tab` 을 들여쓰기로 쓴다.
