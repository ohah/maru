...
6. **B(병행)** — renderer **backend-neutrality 가드 테스트** ✅ **완료**(`tests/boundary/imports.zig`의 `scanForbiddenIdentifiers` — 중립 레이어 **terminal·renderer·session·chrome**에 OS 타입명(NativeMetalCell·MetalFrame·CT*·CG*·NS*·MTL*)이 식별자로 새면 빌드 실패; `@import` 금지 1차에 더한 2차 re-export 가드). `metal_frame`(중립 투영) **중립 위치 재배치는 후속**(§8 topological note — 아직 platform/macos에 갇힌 중립 코드).
