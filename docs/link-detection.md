...
- **maru가 다르게 한 점**(레퍼런스와 갈린 지점 — project-rules.md §"단일 표준 없음" 의무):
  ...
  4. **U+2026 말줄임표가 들어간 휴리스틱 토큰은 자동 링크로 보지 않는다.** 터미널 폭이나 UI 말줄임 때문에
     화면 텍스트 자체가 `…`로 잘린 경우 원본 URL/경로는 복원할 수 없고, 잘린 문자열을 여는 편이 더 위험하다.
     원본 URI가 따로 있는 경우는 OSC 8 명시 링크가 항상 우선하므로, 보이는 텍스트가 `https://…`처럼 말줄임되어도
     저장된 URI를 그대로 연다.
...
- **말줄임표 차단**: 토큰 안에 U+2026(`…`)이 있으면 자동 감지하지 않는다. 이건 휴리스틱에만 적용되며,
  OSC 8 명시 링크는 셀의 link id로 저장 URI를 먼저 회수하므로 영향을 받지 않는다.
