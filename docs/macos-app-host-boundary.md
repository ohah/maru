...
| 로그 | 뜻 |
| --- | --- |
| `=== maru app exit pid=N signal=M code=C from=P ===` | 잡을 수 있는 시그널로 죽었다. `from` 은 **보낸 쪽 PID**(`SA_SIGINFO` 의 `siginfo_t.pid`) |
...
- **`from` 이 핵심이다.** `signal=15` 만으로는 "SIGTERM 을 받았다"까지고 **누가 보냈는지**를 모른다 —
  조용한 종료를 추적할 때 정작 알아야 하는 것이 그것이다. 그래서 `SA_SIGINFO` 로 `siginfo_t` 를 받아
  보낸 쪽 PID 를 남긴다(실측: 다른 프로세스의 `kill -TERM` 이 그 셸의 PID 로 정확히 찍혔다).
  하드웨어 폴트(SEGV·BUS)에는 보낸 쪽이 없어 `from` 이 의미 없으므로 커널의 `si_code` 를 함께 남겨
  둘을 구분한다. `si_code` 는 음수 값을 쓰므로 부호를 보존해 적는다 — u32 로 접으면 `-1` 이
  `4294967295` 로 보여 커널 상수와 대조가 안 된다.
...
- **스택 오버플로 SEGV 는 마커를 못 남긴다.** 핸들러가 이미 바닥난 스택 위에서 돌기 때문이고,
  막으려면 `sigaltstack` + `SA_ONSTACK` 이 필요하다. 실측으로 일반 SEGV(널 참조)는 마커가 남고
  스택 오버플로는 안 남는 것을 확인했다 — 다만 **그 경우 macOS 가 크래시 리포트를 쓰므로** 진단이
  통째로 사라지지는 않는다. 이 기능의 고유한 값은 리포트가 **안 나오는** 종료 계열
  (`TERM`·`HUP`·`INT`·`QUIT`)에 있다.
...
app shell은 제품 앱 loop와 Zig shell surface/frame loop를 함께 실행하고, `MaruMetalTerminalView`가 AppKit/Metal UI를 그리며 key/resize/close event를 Zig app session ABI로 내려보낸다. `zig build macos-app-bundle`은 표준 메뉴바와 `.app` bundle에 Info.plist·CLI·web/font resources를 패키징하며 Developer ID 배포는 [distribution.md]의 서명·공증 경로를 쓴다. `install_cli`는 command catalog·팔릿에서 `~/.local/bin/maru` symlink를 설치한다. 자동 smoke가 보장하는 범위와 실제 사용자 조작·firstResponder·Gatekeeper gate는 [검증 매트릭스](verification-matrix.md)가 소유한다.
