...
- **줄 간격으로 늘어난 여백은 위아래로 나눈다**(CSS 의 half-leading). 전부 baseline 아래에 두면 글자가 셀 위쪽에 붙고 아래에 패딩이 있는 것처럼 보인다 — 선택 배경이 셀 전체를 칠하므로 그 치우침이 그대로 드러난다(14px·`lineHeight` 1.2 에서 위 1px·아래 6px → 위 3px·아래 4px). 본체는 배수 대신 폰트가 정한 `line_gap` 을 쓰므로 이 문제가 없다.
