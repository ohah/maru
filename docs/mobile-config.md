...
| `ssh.server.<n>.fingerprint` | 호스트키 지문(`SHA256:...`). **필수** — 자동 승인이 없다([SSH 계약](ssh-client.md) §4) |
...
- **주소·이름·지문이 다 있어야 접속 대상이다.** 하나라도 비면 목록에는 보이되 접속은 거절한다
  — 반쯤 적은 줄로 붙으러 가면 실패 이유가 "네트워크" 처럼 보인다.
