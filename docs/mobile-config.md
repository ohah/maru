...
| `editor.wrap`·`editor.tab-width`·`editor.max-columns`·`editor.cursor-shape`·`editor.scroll-beyond-last-column`·`editor.cursor-surrounding-lines`·`editor.cursor-surrounding-columns`·`editor.minimap`·`editor.minimap-width`·`editor.diagnostics`·`editor.hover`·`editor.hover-delay`·`lsp.enabled`·`file-panel.external-link-target`·`theme.syntax.keyword` 외 구문 색 열하나(`theme.syntax.<역할>`) | 그 화면이 모바일에 없다. 구문 색은 코드 편집기 전용이고 폰에는 그 편집기가 없다([native-editor-ui.md](native-editor-ui.md) §9.0) |
