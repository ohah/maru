...
데스크톱 키 94개를 훑으면 왜 나눠야 하는지가 드러난다. `quick-terminal.*`·`shell.command`·
...
| 부분 갱신(주석·모르는 키 보존) | **공유** — `serialize.updateForKeys` |
| 어떤 키가 있고 기본값이 무엇인가 | **모바일 것** — 이 문서 §4 |
| 파일 위치 | **모바일 것** — 위 §2 |

**엔진을 공유하려면 한 자리를 열어야 한다.** `schema.tryParse` 가 `config: *theme.Config` 로
데스크톱 구조체에 박혀 있다. 본문은 이미 `@typeInfo`·`@hasDecl` 로 도는 comptime 코드라 받는
타입만 `anytype` 으로 열면 그대로 쓴다 — 모바일 구조체가 자기 `schema` 메타를 들고 같은 엔진을
탄다. **데스크톱 동작은 안 바뀐다**(같은 함수에 같은 타입을 계속 넘긴다).
...
- **부분 갱신이다**(`serialize.updateForKeys`). 통째로 다시 쓰면 주석과 **모르는 키**가 사라진다.
