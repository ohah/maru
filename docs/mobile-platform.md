...
**합성 글리프 경로를 아직 안 탄다.** `renderer.synthesizeGlyph` 는 박스 드로잉·블록·브라유·
파워라인을 폰트 대신 절차 합성해 셀에 꽉 채운다(그게 계약이다 — "rasterizer 는 폰트 경로
전에 이걸 한 번 호출한다"). 모바일 host 는 그냥 폰트로 굽는다. 화면에서 박스가 끊기고 블록에
이음매가 보인다.

호출만 추가해서는 안 되는데, **글리프 기하가 격자와 안 맞기 때문**이다 — 격자 한 칸은
5×22 논리 px 인데 그리는 quad 는 11×15 다(아틀라스 셀 24×32 전체가 quad 에 매핑된다). 폰트
글리프는 잉크가 안쪽에 앉아 안 보이지만, 셀을 가장자리까지 채우는 합성 글리프는 곧바로
드러난다. 기하 정합이 선행 조건이다(계획 M4a3).
