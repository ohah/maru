...
**아직 남은 구멍 셋**(전부 화면으로 확인했다):

| | 무엇 | 왜 |
|---|---|---|
| VS16 결합(`❤️`) | 컬러가 아니라 **단색**으로 그려진다 | 브리지가 셀의 **base 코드포인트만** 넘긴다 — 클러스터 본체(`grapheme_id`)를 안 실어 `❤`(U+2764)와 `❤️`(U+2764 U+FE0F)가 host 에게 **같아 보인다** |
| 국기(`🇰🇷`) | 태극기가 아니라 **글리프 두 개** | 지역 표시자 두 개가 **각각** 미스로 올라와 따로 구워진다 |
| 스킨톤·ZWJ 조합 | 미검증 | 같은 뿌리(클러스터를 안 넘긴다) |

셋 다 **브리지가 host 에 클러스터 본체를 넘기면** 풀린다. 컬러 판정도 base 만 보므로
(`isEmojiPresentation(U+2764)` 는 거짓) 같은 자리에서 함께 고쳐야 한다.
