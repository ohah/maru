...
**재는 방식도 같아야 한다.** 판정 기준만 같고 표본을 다르게 모으면 두 수를 나란히 놓는 것
자체가 무의미하다 — 실제로 Android 만 첫 프레임들을 버리고 iOS 는 안 버려, **같은 이름의
로그가 다른 것을 재고 있었다**. 목표·PASS 창·표본 수·워밍업은 전부 ABI 헤더가 소유한다
(`MARU_FRAME_*`). 워밍업이 필요한 이유는 아틀라스를 굽고 스왑체인이 서는 동안의 간격이
정상 주기가 아니기 때문이다.
