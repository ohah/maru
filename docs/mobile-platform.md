...
**셀 속성이 전경색 하나뿐이다.** 코어는 SGR 을 전부 파싱해 셀에 담는다(배경·reverse·굵기·
dim·italic·밑줄·이중밑줄·취소선·윗줄·blink·conceal·밑줄색). 모바일 렌더는 `style.foreground`
**하나만** 읽는다 — 화면으로 확인했다: `bg`·`rev`·`under`·`bold` 네 개가 전부 똑같은 평범한
글자로 나온다. 배경색은 상태줄·선택·`ls` 색·에이전트 카드가 전부 쓰는 것이라 공백이 크다.

이것이 **테마의 선행 조건**이기도 하다. 터미널이 `xterm-maru` terminfo 로 truecolor 를
광고해도(모바일은 원격 전용이라 `TERM` 은 원격 PC 의 maru 세션이 정한다 — 모바일이 들고 갈
것은 없다), 그 광고를 **렌더가 뒷받침하지 않으면** 앱이 낸 색이 화면에 안 나온다. 테마를
config 로 여는 일(M10)은 그 다음이다.
