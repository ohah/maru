...
**BMP 밖 글자는 아직 못 굽는다.** 두 host 다 코드포인트를 16비트 한 단위로 넘긴다
(iOS `unichar`, Android `jchar`) — 서러게이트 쌍을 안 만들어서 **U+FFFF 위가 통째로
빈칸**이다(화면으로 확인: 😀🚀 는 자리만 두 칸 차지하고 아무것도 안 그려진다. ✓ █ 같은
BMP 글자는 정상). 터미널 출력에 이모지가 흔하므로 실제로 보이는 구멍이다. 고치려면 서러게이트
쌍·폰트 폴백·**컬러 아틀라스**가 함께 필요해서 별도 슬라이스로 둔다([M4a6](plans/mobile-platform.md)).

**폰트에 없는 글자를 다루는 방식이 두 플랫폼에서 다르다.** Android 는 `Canvas.drawText` 라
시스템 폴백 체인이 붙지만, iOS 는 특정 `CTFont` 에 글리프를 조회해 없으면 **안 그린 채 등록**
한다 — 등록부에 "있음" 으로 박혀 다시 굽지도 않아 **영구 공백**이 된다. 같은 슬라이스에서
`CTFontCreateForString` 폴백으로 맞춘다.
