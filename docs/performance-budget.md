...
| render scheduling | entry worker in-flight≤1/latest≤1/source≤8 MiB, app-global active worker≤8/source≤64 MiB/result≤16 MiB, pending inserted bytes≤8 MiB, request structured-clone bytes≤8 MiB+64 KiB, result HTML+metadata≤2 MiB, stale generation retained bytes 0 | hidden/non-focused worker는 suspend하고 focused/visible을 우선한다. pending metadata/insert cap 초과는 request 0 + `resync_required` 뒤 최신 Seed 1회로 복구한다. |
| fragment projection | mounted renderer iframe ≤ 8, animation frame당 create≤2/destroy≤2, 한 기본 chunk는 source ≤ 64 KiB 또는 top-level block ≤ 16 | 2-viewport hysteresis를 쓰고 cap 예상 시 인접 inactive chunk를 더 합친다. 단일 64 KiB 초과 block은 source-preserving limit 상태다. |
| rich block | KaTeX source≤32 KiB, Prism fence≤256 KiB, Mermaid source≤32 KiB·line≤512·fragment당≤4, helper app-global in-flight≤1·pending jobs≤32·pending source≤1 MiB·SVG≤512 KiB/job·accepted 합계≤2 MiB·response deadline≤2s·60초 실패≤3 | Mermaid는 번들된 별도 `maru-mermaid-renderer` process에서만 실행한다. cap+1은 allocation/enqueue 0, timeout/crash는 helper를 terminate/restart하고 CM6·후속 job을 보존한다. 60초 안 3회 실패면 앱 수명 동안 disabled latch로 helper churn을 멈춘다. `MermaidJobCapability`가 late/duplicate result의 DOM mutation을 0으로 만든다. WebKit service CPU의 정확한 2초 정지는 주장하지 않는다. |
...
| atomic projection | main-thread hashed/copied source bytes 0, worker hashed bytes≤kind cap(KaTeX/Mermaid 32 KiB, Prism 256 KiB), 일반 Markdown iframe 0, atomic iframe≤8, frame당 mount/unmount≤2, viewport+overscan+2-viewport retention window 밖 신규 HTML 생성·전송 0 | main은 exact range/nonce만 보내고 delta-synced worker가 queue에서 hash한다. exact cap은 처리하고 cap+1은 hash/enqueue 0이다. 이미지·수식·비활성 fenced code·Mermaid만 `AtomicProjection`으로 보내며 desired/mounted/retained-but-not-visible/generated HTML을 별도 counter로 둔다. cap/timeout/asset 거부는 해당 range만 원문으로 강등한다. |
