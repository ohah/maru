...
- 파일 생성·rename은 단일 worker, request queue 64, in-flight 1, completion queue 16으로 제한한다. macOS 휴지통 adapter는 queue 16, in-flight 1이다. cap+1은 명시적으로 거부하고 pending/capacity를 같은 tick에 회수한다. user mutation은 coalesce하지 않으며 watcher/coarse rescan만 `{root, deepest expanded ancestor}`로 합친다. retired backend를 main actor가 기다리지 않고, frame tick은 completion을 최대 1개만 처리한다.
