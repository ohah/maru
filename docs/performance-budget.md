...
- mutation completion은 worker가 검증한 kind·project root·path metadata와 필요한 snapshot을 소유한다. tick은 동기 `openFilePanelPath`/`rebuildFileTreeFromDock`를 재사용하지 않고 메모리 apply만 하므로 생성 자동 열기·directory rename을 포함해 `open/stat/readdir` 0이다. submit 실패·worker 실패·completion/adaptor queue overflow 뒤에도 pending을 해제하고 notice+재시도가 가능해야 한다.
...
| workspace focus boundary 보정 | 기존 frame의 `leaf_rects` layout·active-leaf 순회 대비 증분 layout build = 0, leaf visit = 0, allocator call = 0. border quad = 4, overlay presence sentinel = 1, focus 왕복의 WebView transition = 0 | frame build가 active leaf에서 `PaneGeometry.body`와 `.grid`를 함께 얻어야 한다. border target을 찾으려고 `activeTabLeafRects`를 다시 호출하거나 별도 body cache/invalidation을 추가하면 실패한다. 단일 pane과 `workspace.max_panes_per_tab` fixture에서 1,000 projected frame의 변경 전/후 layout·visit·allocation delta를 계측한다. |
...
| 경로 | hard gate | 실패 조건·구현 제약 |
| --- | --- | --- |
| CM6 input/IME transaction | unified/KaTeX/Mermaid 실행 0, iframe create/destroy 0, native bridge write 0, full-document `Text.toString()` 0, document-size 비례 copied bytes/allocation 0 | dirty는 CM6 `Text.eq`/revision으로 판정하고 worker에는 초기 snapshot 뒤 `ChangeSet` delta만 보낸다. composition 중 block 교체를 금지한다. |
...
FP10c2 구현 PR은 `mise run web:live-preview-perf`와 `mise run macos-live-preview-perf` task·CI required job을 함께 추가한다. 전자는 `tests/artifacts/perf/live-preview.json`에 8 MiB short-paragraph fixture의 1,000회 단일-character/IME transaction, worker queue/retained source·HTML·structured-clone bytes, offscreen HTML count와 iframe churn을 기록한다. 후자는 `tests/artifacts/perf/live-preview-macos.json`에 모든 창의 worker token/bytes, Mermaid helper queue bytes/concurrency/timeout/restart/failure latch, tick별 FP10 FS I/O·WebView create/destroy·worker wait뿐 아니라 synchronous process spawn/terminate·pipe setup/read/write·blocking wait를 기록하고 명시적 save bridge I/O는 별도 필드로 귀속한다. 1,000 tick cold start/restart fixture에서 process/pipe/wait tick counter는 모두 0, completion drain≤8이어야 한다. task나 artifact/필수 field 누락·비영 tick counter·상한 초과는 실패하며 Web/macOS required job이 각각 업로드한다. FP10c2 전에는 두 명령이 아직 존재하지 않으므로 문서 PR의 실행 가능한 현재 gate로 간주하지 않는다. WebContent RSS, Mermaid WebKit service CPU의 정확한 deadline 종료와 실제 key-to-paint p95 수치 실패 gate는 공개 process 귀속/입력 지연 하니스가 없어 주장하지 않는다.
