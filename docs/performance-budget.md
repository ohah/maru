...
FP11a는 제품 projection을 바꾸기 전에 `mise run web:live-preview-perf`와 결정적 `tests/artifacts/perf/live-preview.json` schema/validator를 필수로 추가한다. 필수 counter는 `visited_code_units`, `visited_syntax_nodes`, `emitted_decorations`, `diffed_decorations`, `copied_bytes`, `source_transactions`, `projection_transactions`, `dom_mutations`, `iframe_create`, `iframe_destroy`, `retained_html_bytes`, `generated_outside_retention`, `projection_fallback_counts`다. 일회성 `InteractionRejectReason`은 이 artifact의 projection count에 합치지 않는다. 명령·artifact·필수 field 누락, threshold 초과, 같은 fixture 반복의 비결정성은 nonzero exit다. FP11b 이후 각 PR은 해당 행의 bounded fixture와 exact-cap/cap+1·stale·composition 회귀를 먼저 추가하고 이 task를 필수 실행한다. FP11f가 Mermaid 제품 pump를 활성화할 때는 아래 native task·artifact도 같은 PR의 필수 gate다.

FP11f/FP10c2 구현 PR은 기존 web task를 Mermaid counter로 확장하고 `mise run macos-live-preview-perf` task·CI required job을 추가한다. web artifact는 8 MiB short-paragraph fixture의 1,000회 단일-character/IME transaction, worker queue/retained source·HTML·structured-clone bytes, retention-window 밖 HTML 생성과 iframe churn을 기록한다. native `tests/artifacts/perf/live-preview-macos.json`은 모든 창의 worker token/bytes, Mermaid helper queue bytes/concurrency/timeout/restart/failure latch, tick별 FP10 FS I/O·WebView create/destroy·worker wait뿐 아니라 synchronous process spawn/terminate·pipe setup/read/write·blocking wait를 기록하고 명시적 save/link bridge I/O는 별도 필드로 귀속한다. 1,000 tick cold start/restart fixture에서 process/pipe/wait tick counter는 모두 0, completion drain≤8이어야 한다. task나 artifact/필수 field 누락·비영 tick counter·상한 초과는 실패하며 Web/macOS required job이 각각 업로드한다. 현재 문서 PR에는 두 명령이 아직 없고 실행 가능한 gate로 간주하지 않는다. FP11a는 web task 없이, FP11f는 native task 없이 머지할 수 없다. WebContent RSS, Mermaid WebKit service CPU의 정확한 deadline 종료와 실제 key-to-paint p95 수치 실패 gate는 공개 process 귀속/입력 지연 하니스가 없어 주장하지 않는다.
