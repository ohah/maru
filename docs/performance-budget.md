...
- **`zig build test` 안의 벽시계 단언은 "재앙 감지선"이고, 선은 실측 두 값 사이에 둔다** — 지금 값과, 그것이 잡겠다는 회귀의 값. 둘 다 주석에 적는다. 한쪽만 보면 양쪽으로 어긋난다(2026-08-18에 둘 다 겪었다):
  ...
  - **적용 사례(2026-09-15) — 같은 함정 셋을 한 번에 걷었다.** 세 판정자가 각각 다른 이유로 「기계를
    재고」 있었고, 셋 다 무관한 PR 을 빨갛게 만들었다. 처방이 셋 다 다르다는 것이 요점이다.
    - **마감이 시험 대상이 아닌데 선이 빡빡했다** — `session_host_release_adapter_github_attestation`
      의 실제 자식 판정자들이 `1s` 마감을 넘겼다. 그 판정자가 보는 것은 argv·경로·토큰 부재이고 마감은
      자식이 영영 안 끝나는 재앙만 막는 선이다. 이름 있는 `real_child_budget_ns = 60s` 로 옮겼다 —
      마감 **자체**의 계약(0 은 `InvalidBudget`, 초과는 실패)은 가짜 실행기를 쓰는 판정자가 결정적으로
      보므로 이 값을 키워도 그 판정은 안 흔들린다.
    - **다른 기계 종류에서 잰 선을 썼다** — `CoreHandoff` 기아 판정자의 상한 `3ms` 는 주석이 근거를
      「M-series 실측」이라 적어 뒀는데 `check` 잡은 ubuntu 에서 돈다. **벽시계를 버리고 「요구자가
      대기하는 동안 리더가 몇 번 더 이겼는가」를 센다** — 그 수는 기계 속도와 무관하고(빠른 기계는 같은
      시간에 더 돌 뿐이다) 계약을 직접 센다. 실측 양보 있음 worst 0~1 회 · 양보 없음 1559~2632 회라
      상한 16 은 위 기준(실측의 4~5 배 · 회귀 값 아래)을 세 자릿수 여유로 만족한다.
    - **실패 주입을 «첨자»로 더듬었다** — `MAXC2` 가 할당 첨자를 훑어 목표를 찾는데, 그 첨자는 앞선
      준비 단계의 할당 수를 따라 움직인다. 같은 코드가 기계마다 다른 답을 냈다(`FailurePathNotReached`
      가 CI 2 회·로컬 1 회). 겨냥하는 할당이 하나뿐이면 **모양(크기·정렬)으로** 고르는 편이 결정적이고,
      못 맞히면 「범위가 좁았다」가 아니라 **「그 할당이 사라졌다」**로 읽힌다.
    - **그 뒤 전수 조사(2026-09-15).** 저장소 전체에서 「테스트 안에서 실제 시계를 읽는」 블록 57 개를
      뽑고, 그중 잰 값을 단언하는 21 개를 손으로 읽어 **주입한 시각에 대한 결정적 단언을 걷어낸 8 자리**를
      남겼다. 결과는 **대부분 이미 건전**했다 — `code_signature` 는 주석이 「1 초는 증명이 아니라 기계 속도
      측정이었다」며 스스로 고쳐 뒀고, `system_text` 는 아예 「timing 은 판정에 쓰지 않는다」고 적고 hit/miss
      **개수**로 판정한다. 나머지도 회귀 값을 주석에 적고 그 아래에 선을 뒀다. 남은 둘은 둘 다
      `core_handoff` 였다:
      - **선으로 가를 수 없던 자리** — 「요구자가 없으면 안 잔다」를 `dt < 0.5 ms` 로 봤다. 「잤다」가
        1 ms 라 신호가 스케줄러 잡음보다 작다. **선을 옮기는 대신 신호를 키웠다**: 기다릴 상한을 인자로
        받는 갈래(`yieldToDemandFor`)를 두고 판정자가 5 초를 넘겨, 「잤다」를 초 단위로 만든 뒤 1 초에서
        가른다. 제품은 그대로 상수를 넘긴다.
      - **아무것도 안 가르던 자리** — 「signal 이 오면 타임아웃 전에 깬다」가 `dt < 10 ms` 였는데, 깬 경우
        (200 µs)와 못 깬 경우(1 ms)가 **둘 다 그 아래**다. 판정은 이미 세대 증가가 하고 있었으므로 재는
        것 자체를 걷었다. **재는 값이 판정에 안 쓰이면 남는 것은 빨개질 여지뿐이다.**
...
**`AppKit smokes macOS`는 required가 아니다.** 실제 AppKit 앱을 띄우는 스모크(keep-alive 복구·R1·R7·C4 Quit, 에이전트 세션 기록 도크, 편집기 저장 충돌)를 한 잡에서 돈다. 이 표는 required 컨텍스트의 단일 출처이므로 여기 없다는 것이 곧 branch protection 에도 없다는 뜻이다. 필수로 올리지 않은 이유는 둘이다 — ① required 인데 실행 조건이 어긋나면 PR 이 영영 pending 으로 막힌다(위 `changes` 가드 절), ② 실제 AppKit 프로세스 수십 개를 띄우는 잡이라 러너 부하에 취약한 부류다(`session host macOS (Debug)` 를 PR 에서 뺀 것과 같은 성질). **그래도 게이트로 작동한다** — 이 저장소는 required 여부와 무관하게 «전부 초록일 때만» 머지한다. 기록 도크 스모크가 CI 에 들어온 이유는 그 반대쪽 실패다: 2026-09-15 까지 `mise` 태스크로만 있어 손으로 안 돌리면 안 돌았고, 그 사이 시나리오 넷이 조용히 죽어 있었다([verification-matrix.md](verification-matrix.md)).

**세 잡을 하나로 합쳤다(2026-09-24).** 예전에는 `keep-alive recovered session macOS` · `agent session archive macOS` · `editor save conflict macOS` 세 잡이 **각자 같은 앱 번들을 콜드로 빌드했다**(잡마다 약 4분, 스모크 자체는 30초~2분 — 두 잡의 캐시 복원은 저장하는 쪽이 없어 늘 빈손이었다). 병목은 실행 시간이 아니라 **macOS 러너 대기**다: 공개 저장소의 macOS 동시 실행은 5개이고, 최근 PR 15회의 CI 벽시계 중앙값 29분 중 macOS 잡 대기가 중앙값 15분이었다(main push 하나가 macOS 잡 8개를 띄운다). 합쳐서 PR 의 macOS 잡이 4 → 2, main push 가 8 → 6, 러너 시간이 PR 당 약 17 → 11분이 된다. 이 잡(약 11~12분)은 `file explorer`(약 17분)보다 짧아 임계경로는 그대로다. 따로 두던 이유(「잡 이름이 곧 귀책」)는 중립 이름과 스텝마다 `!cancelled()` 로 지킨다 — 앞 스모크가 실패해도 뒤 스모크가 돌고, 실패는 스텝 이름으로 남는다(ci.yml 주석).
...
**발견(2026-09-24) — 안 보이는 탭의 출력이 화면 전체를 다시 그리고 있었다(재투영의 88 %).** 드레인은 모든 탭의 출력을
합쳐 `metal_dirty` 를 세웠다. 활성 32 세션(2 Hz)에서 30 초 동안 재투영 710회 중 627회가 **안 보이는 탭 출력만으로**
생겼고(tick 중앙값 374 µs), 그 프레임은 사실상 아무것도 안 바꿨다 — 사이드바 DrawList 는 1,200번 중 1,199번이 직전과
같았다. 사이드바만 다시 그리는 경로로 좁히는 것만으로는 효과가 없었다(A/B −3.5 %, 잡음): 그 경로도 1회 약 325 µs 로
전체와 비슷했다 — 프레임 비용의 대부분이 보이는 터미널이 아니라 사이드바의 만들기·셰이핑·배치·교체였다. 답은 **올려 둔
것과 같으면 아무것도 안 올리는 것**이다. 올리지 않으면 generation 이 그대로라 Swift 도 다시 그리지 않는다
(`MaruAppHost.swift` 의 generation 게이트).

처방: ① 출력 게이트(`AppSession.outputRedrawFor`) — 보이는 탭(모든 분할 pane)의 출력만 전체 재투영, 나머지는 사이드바
경로. ② `MetalFrameBuffer` 가 사이드바 셀을 만든 입력의 지문을 든다(`sidebar_ops.sidebarSourceKey` — DrawList·전경색·
chrome 기하·행·메트릭을 필드 단위로 직렬화한 바이트, 해시 아님). **사이드바를 쓰는 모든 호출이 지문을 먼저 죽이고**
성공한 호출자만 `adoptSidebarSource` 로 살린다 — adopt 를 빠뜨린 새 경로는 최적화만 꺼진다. ③ 상태바 에이전트 집계는
매 tick 값 비교(`noteAgentTallyForStatusBar`). ④ `syncRemoteCellMetrics` 는 매 tick(전체 재투영 분기 안에 있을 때는
안 보이는 새 runtime 이 셀 크기를 못 받을 수 있었다).

**규칙: 안 보이는 탭의 상태를 사이드바 밖에 보이는 표시는 스스로 dirty 를 세운다.** 그 탭의 출력은 더 이상 전체
재투영을 일으키지 않는다. 전수 확인(드레인이 바꾸는 상태 × 그것을 읽는 모든 표시)에서 그런 표시는 상태바 에이전트
집계 하나였고(③), 그 빈틈은 안 보이는 탭 출력의 재투영이 가리고 있었다(사이드바를 접으면 막힘 개수가 늦게 바뀐다).

결과(8쌍 A/B, 부하 15–29 로 평소보다 잡음 큼): 기본 출력 환경 앱 CPU **−31.1 %**(8/8), 가짜 에이전트 32개가 모두 제목
스피너를 돌리는 환경 **−7.2 %**(7/8 — 사이드바가 실제로 계속 바뀌어 교체가 남는다). host 는 두 번 모두 변화 없음.
정확성: 매 tick 끝에 사이드바를 새로 만들어 마지막으로 올린 것과 비교한 «낡은 tick» 비율이 main 과 같았다(활성 탭을
조용히 둔 에이전트 환경 3회씩: main 4.6·5.2·5.6 % / 제품 4.9·4.5·4.3 %, 교체 횟수는 약 1/4). 9 tick 을 넘는 드문
낡음은 터미널 제목(OSC) 캐시가 dirty 를 안 세우는 **기존** 구조였다(`syncAutoTitles`) — 이 변경이 만든 종류가 아니다.
...
[control-plane-protocol.md §4.4](control-plane-protocol.md)가 채택된 전송 계약을 소유한다. 5f-5c 경로는 strict-CSP `callAsyncJavaScript` expression+args+await, raw strict-JSON ≤512 KiB inline, 그 초과~16 MiB progressive JSON-RPC chunk, screenshot 공통 pump, connection 4 MiB/process 32 MiB queued+writer-owned 회계, Swift `Data` pin/pull/release와 CLI atomic spool을 사용한다. hello 16 MiB capability의 공개 여부는 아래 Track 5 성능 gate와 [검증 매트릭스](verification-matrix.md)가 소유한다.
