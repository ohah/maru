...
| font/glyph atlas | 실제 raster atlas와 GPU texture upload는 아직 없다. 현재는 GlyphCacheKey -> AtlasSlot 도메인 cache 계약만 있다. 세부 정책은 [폰트 전략](font-strategy.md)을 따른다. | first glyph resolve, frame당 atlas miss, atlas upload bytes, font size 변경 후 첫 frame |
