...
**`agent session archive macOS`는 required가 아니다(2026-09-15 신설).** 이 표는 required 컨텍스트의 단일 출처이므로 여기 없다는 것이 곧 branch protection 에도 없다는 뜻이다. 필수로 올리지 않은 이유는 둘이다 — ① required 인데 실행 조건이 어긋나면 PR 이 영영 pending 으로 막힌다(위 `changes` 가드 절), ② 실제 AppKit 프로세스 15개를 띄우는 잡이라 러너 부하에 취약한 부류다(`session host macOS (Debug)` 를 PR 에서 뺀 것과 같은 성질). **그래도 게이트로 작동한다** — 이 저장소는 required 여부와 무관하게 «전부 초록일 때만» 머지한다. 이 잡이 생긴 이유는 그 반대쪽 실패다: 2026-09-15 까지 이 스모크는 `mise` 태스크로만 있어 손으로 안 돌리면 안 돌았고, 그 사이 시나리오 넷이 조용히 죽어 있었다([verification-matrix.md](verification-matrix.md)). 비용은 macOS 러너 약 5분(`runtime` 변경 PR 한정)이고 **wall-clock 은 늘지 않는다** — macOS 잡은 병렬이고 임계경로가 `file explorer`(실측 15분 41초)다(스모크 자체는 실측 40.5초).
