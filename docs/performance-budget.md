...
| direct interaction | pointer/key event당 CM6 transaction≤1, 외부 action≤1, 두 효과 동시 발생 0, document-wide table parse 0 | task/table/plain click은 native bridge write 0이다. trusted `⌘`/`⌘⇧` link activation만 명시적 사용자 gesture당 bounded bridge action 최대 1회를 허용한다. `LivePreviewIntent`가 현재 guard/revision/range를 재검증하며 stale/readonly/close lock/composition/synthetic event는 transaction·bridge 모두 0이다. |
...
FP11b의 `mise run web:live-preview-perf`는 schema v2 `fp11b-8mib-1000-editable-projection`을 두 번 실행해 동일한 `tests/artifacts/perf/live-preview.json`인지 확인한다. short-paragraph 정확히 8 MiB 문서의 실제 jsdom `EditorView`→`EditableProjectionController.handleUpdate`→`buildEditableProjection` 경로에서 heading text 한 글자를 같은 길이로 1,000회 교체하고, 별도 product `EditorView`의 `"$1".repeat(8192)` dense-dollar parse를 함께 실행한다. source transaction은 정확히 1,000회여야 하고 `Text.toString`·`Text.sliceString`·`TextEncoder.encode`의 64 Ki UTF-16 초과 document-scale copy는 0이다. 공용 상한은 실제 tree/cursor visit, selection range 비교, parser-site dollar scan, 생성한 Decoration 수와 정렬 merge 비교 수를 각각 센다. 단위 fixture는 node/entry/table/dollar exact-cap과 cap+1, 1,024 multi-cursor의 logarithmic 비교, 4,096 entry 중 하나 변경 시 4,095 Decoration identity 재사용을 검증한다. 실제 counter는 `visited_code_units`, `visited_syntax_nodes`, `selection_range_checks`, 일반 fixture의 `math_scanned_code_units`, 단일 dense context를 정확히 32 Ki 이하로 직접 gate하는 `dense_math_scanned_code_units`, `emitted_decorations`, `diffed_decorations`, `copied_bytes`, `source_transactions`, `projection_transactions`, projection-class `dom_mutations`, `iframe_create`, `iframe_destroy`, `retained_html_bytes`, `generated_outside_retention`, `projection_fallback_counts`다. 같은 source geometry의 1,000회 입력 구간은 최초 decoration commit 하나만 유지해 `projection_transactions=1`, 구간 시작 전 observer record를 버린 뒤 `dom_mutations=0`이어야 한다. 일반 iframe/retained HTML은 0이며 fixture의 image atomic 후보 하나만 `atomic_not_enabled=1`이다. 명령·artifact·필수 field 누락, threshold 초과, 같은 fixture 반복의 비결정성은 nonzero exit다. FP11c 이후 각 PR은 관련 counter와 bounded/stale/composition 회귀를 먼저 추가해 이 task를 필수 실행한다. FP11f가 Mermaid 제품 pump를 활성화할 때는 아래 native task·artifact도 같은 PR의 필수 gate다.
