...
| rich block | KaTeX source≤32 KiB, fenced source≤256 KiB, Prism token render≤32 KiB·그 이상은 escaped plain code, worker result HTML+metadata 합계≤2 MiB, Mermaid source≤32 KiB·line≤512·fragment당≤4, helper app-global in-flight≤1·pending jobs≤32·pending source≤1 MiB·SVG≤512 KiB/job·accepted 합계≤2 MiB·response deadline≤2s·60초 실패≤3 | worker result cap 초과는 해당 nonce만 `rich_source_limit`으로 끝내고 worker를 재시작하지 않는다. Mermaid는 번들된 별도 `maru-mermaid-renderer` process에서만 실행한다. cap+1은 allocation/enqueue 0, timeout/crash는 helper를 terminate/restart하고 CM6·후속 job을 보존한다. 60초 안 3회 실패면 앱 수명 동안 disabled latch로 helper churn을 멈춘다. `MermaidJobCapability`가 late/duplicate result의 DOM mutation을 0으로 만든다. WebKit service CPU의 정확한 2초 정지는 주장하지 않는다. |
...
| atomic projection | main-thread hashed/copied source bytes 0, worker request별 hashed bytes≤kind cap(KaTeX/Mermaid 32 KiB, Prism 256 KiB)·8-request batch aggregate≤2 MiB, 일반 Markdown iframe 0, atomic iframe≤8, frame당 mount/unmount≤2, viewport+overscan+2-viewport retention window 밖 신규 HTML 생성·전송 0 | main은 exact range/nonce만 보내고 delta-synced worker가 queue에서 hash한다. exact cap은 처리하고 cap+1은 hash/enqueue 0이다. 이미지·수식·비활성 fenced code·Mermaid만 `AtomicProjection`으로 보내며 desired/mounted/retained-but-not-visible/generated HTML을 별도 counter로 둔다. cap/timeout/asset 거부는 해당 range만 원문으로 강등한다. |
...
FP11f/FP10c2 구현 PR은 기존 web task를 Mermaid counter로 확장하고 아직 없는 `mise run macos-live-preview-perf` task·CI required job을 추가한다. web artifact는 8 MiB short-paragraph fixture의 1,000회 단일-character/IME transaction, worker queue/retained source·HTML·structured-clone bytes, retention-window 밖 HTML 생성과 iframe churn을 기록한다. native `tests/artifacts/perf/live-preview-macos.json`은 모든 창의 worker token/bytes, Mermaid helper queue bytes/concurrency/timeout/restart/failure latch, tick별 FP10 FS I/O·WebView create/destroy·worker wait뿐 아니라 synchronous process spawn/terminate·pipe setup/read/write·blocking wait를 기록하고 명시적 save/link bridge I/O는 별도 필드로 귀속한다. 1,000 tick cold start/restart fixture에서 process/pipe/wait tick counter는 모두 0, completion drain≤8이어야 한다. task나 artifact/필수 field 누락·비영 tick counter·상한 초과는 실패하며 Web/macOS required job이 각각 업로드한다. FP11a부터 Web required job은 `web:live-preview-perf`와 JSON artifact upload를 실행하고, FP11f는 native task 없이 머지할 수 없다. WebContent RSS, Mermaid WebKit service CPU의 정확한 deadline 종료와 실제 key-to-paint p95 수치 실패 gate는 공개 process 귀속/입력 지연 하니스가 없어 주장하지 않는다.
