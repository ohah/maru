...
| 경로 | hard gate | 실패 조건·구현 제약 |
| --- | --- | --- |
| `EditableProjection` discovery | 한 transaction에서 방문하는 source는 visible range+앞뒤 1 viewport이고 합계 ≤ 64 Ki UTF-16 code unit, syntax node visit≤8,192, emitted `ProjectionEntry`≤4,096, full-document scan/copy 0, remark/unified 호출 0 | 현재 CM6 incremental Markdown tree만 읽는다. node/entry exact cap에서 중단하고 남은 최소 불확정 range를 source-preserving `projection_limit`으로 둔다. syntax tree가 incomplete하거나 table cell 경계를 bounded range 안에서 확정할 수 없어도 해당 범위를 원문으로 둔다. Select All도 discovery 범위를 넓히지 않는다. |
...
FP11a가 `mise run web:live-preview-perf`와 결정적 `tests/artifacts/perf/live-preview.json` schema/validator를 구현했다. short-paragraph 8 MiB 문서의 1,000회 input을 실제 jsdom `EditorView`→`LivePreviewEditorController.handleUpdate`→`LivePreviewWorkerClient.submitChanges`→controlled worker apply 경로로 두 번 실행해 동일 JSON인지 확인한다. source transaction과 worker apply가 각각 정확히 1,000회가 아니면 artifact 생성 전에 실패하고, `Text.toString`과 `Text.sliceString`의 64 Ki UTF-16 초과 document-scale copy를 계측한다. 필수 counter는 `visited_code_units`, `visited_syntax_nodes`, `emitted_decorations`, `diffed_decorations`, `copied_bytes`, `source_transactions`, `projection_transactions`, `dom_mutations`, `iframe_create`, `iframe_destroy`, `retained_html_bytes`, `generated_outside_retention`, `projection_fallback_counts`다. FP11a는 새 projection을 활성화하지 않으므로 projection discovery/diff/transaction/iframe/retention/fallback counter는 실제 제품 경로에서 정확히 0이어야 한다. FP11b에서 실제 projection을 활성화할 때 scenario/schema를 명시적으로 승격하고 `maxLivePreviewProjectionCodeUnits`, `maxLivePreviewSyntaxNodes`, `maxLivePreviewProjectionEntries` 공용 상한의 exact-cap/cap+1 검증으로 바꾼다. 일회성 `InteractionRejectReason`은 projection count에 합치지 않는다. 명령·artifact·필수 field 누락, threshold 초과, 같은 fixture 반복의 비결정성은 nonzero exit다. FP11b 이후 각 PR은 실제 projection probe로 관련 0 counter를 대체하고 해당 행의 bounded fixture와 exact-cap/cap+1·stale·composition 회귀를 먼저 추가해 이 task를 필수 실행한다. FP11f가 Mermaid 제품 pump를 활성화할 때는 아래 native task·artifact도 같은 PR의 필수 gate다.
