...
| font/glyph atlas | smoke 수준의 CoreText CPU raster와 Metal texture upload 검증은 있지만, 제품 renderer의 atlas packing/eviction/shader sampling은 아직 없다. 현재 제품 경계는 `GlyphCacheKey -> AtlasSlot -> GlyphFrame -> GlyphQuadFrame -> GlyphRasterFrame` 도메인 계약이다. `GlyphRasterFrame`은 fake rasterizer로 upload byte/skip contract를 고정한 단계라 제품 CoreText raster 성능을 아직 측정하지 않는다. 경계 밖 slot은 byte buffer를 만들지 않고 skip해 oversized 입력의 메모리 증폭을 막는다. 세부 정책은 [폰트 전략](font-strategy.md)을 따른다. | first glyph resolve, frame당 atlas miss, atlas upload bytes, raster upload bytes, raster skip count, font size 변경 후 첫 frame |
