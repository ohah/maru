...
| 경로 | hard gate | 실패 조건·구현 제약 |
| --- | --- | --- |
| CM6 input/IME transaction | unified/KaTeX/Mermaid 실행 0, iframe create/destroy 0, native bridge write 0, full-document `Text.toString()` 0, document-size 비례 copied bytes/allocation 0 | dirty는 CM6 `Text.eq`/revision으로 판정하고 worker에는 초기 snapshot 뒤 `ChangeSet` delta만 보낸다. composition 중 projection decoration·selection 재작성을 금지한다. |
| render scheduling | entry worker in-flight≤1/latest≤1/source≤8 MiB, app-global active worker≤8/source≤64 MiB/result≤16 MiB, pending inserted bytes≤8 MiB, request structured-clone bytes≤8 MiB+64 KiB, result 전체 envelope+success HTML/grant+rejection metadata≤2 MiB, stale generation retained bytes 0 | hidden/non-focused worker는 suspend하고 focused/visible을 우선한다. pending metadata/insert cap 초과는 request 0 + `resync_required` 뒤 최신 Seed 1회로 복구한다. |
| atomic projection | mounted renderer iframe≤8, animation frame당 create≤2/destroy≤2, result application in-flight≤1/latest≤1, sealed widget retention≤2 viewport | 동일 document revision·kind·range의 terminal-rendered widget만 selection generation 사이 재사용한다. 미렌더·stale widget은 revoke-first로 제거하고 range source를 보존한다. |
| rich block | KaTeX source≤32 KiB, fenced source≤256 KiB, Prism token render≤32 KiB·그 이상은 escaped plain code, worker result HTML+metadata 합계≤2 MiB, Mermaid source≤32 KiB·Web 줄 개수≤512(`maxMermaidSourceLines`)·Zig 한 줄 바이트≤512(`max_line_bytes`)·atomic widget batch≤8(`maxAtomicProjectionRequests`), helper app-global in-flight≤1·pending jobs≤32·pending source≤1 MiB·exact terminal≤98·SVG≤512 KiB/job·accepted 합계≤2 MiB·cold response deadline≤5s·warm≤2s·60초 실패≤3 | worker result cap 초과는 해당 nonce만 `rich_source_limit`으로 끝내고 worker를 재시작하지 않는다. Mermaid는 번들된 별도 `maru-mermaid-renderer` process에서만 실행한다. terminal queue는 backlog와 모든 pending/in-flight/accepted live job을 동시에 담는 98개 fixed storage이고 admission은 `terminal+live+growth` cap+1을 source copy와 기존 pending mutation 0으로 거부한다. coalesce·deadline·transient·integrity·invalid result·accepted capacity·failure latch는 모두 exact terminal을 내보낸다. frame tick은 terminal+accepted completion 합계≤8이다. timeout/crash는 helper를 terminate/restart하고 CM6·후속 job을 보존한다. navigation/widget revoke는 실행 중 결과만 stale 폐기해 helper를 재사용하며 100회 연속 in-flight 편집 revoke에서도 helper start=1·마지막 결과 수락=1이어야 한다. revoked hang은 해당 action의 cold/warm deadline과 failure budget으로 종료한다. 60초 안 3회 실패면 앱 수명 동안 disabled latch로 helper churn을 멈춘다. `MermaidJobCapability`가 late/duplicate result의 DOM mutation을 0으로 만든다. WebKit service CPU의 정확한 deadline 정지는 주장하지 않는다. |
| renderer capability | shell DOM에 Markdown 파생 HTML/SVG insertion 0, renderer bridge/message handler/asset/link action 0, Mermaid external request 0 | sanitize 성공만으로 shell insertion을 허용하지 않고 load-scoped MessagePort와 `RendererCapability { editor_epoch, document_revision, projection_generation, widget_id, widget_generation, renderer_instance }` 6-field 공용 alias를 모두 검증한다. FP10 구현 설명의 옛 5-field 표기는 FP11a 이후 현재 계약이 아니다. |
| native frame tick | FP10 귀속 FS I/O 0, WebView create/destroy 0, worker wait 0 | tick은 mode/action scalar와 기존 surface layout만 소비한다. 저장 FS I/O는 명시적 bridge command 경로다. |
...
### FP11 Obsidian형 편집 projection 추가 gate

FP11d 표 입력 probe는 수동 full-range projection을 제품 controller의 viewport projection과 섞지 않는다. 알려진 fixture source offset으로 selection만 두고 실제 `EditableProjectionController.tableKeyCapture` → `LivePreviewIntentCoordinator` → `interactionContextForIntent` → `dispatchLivePreviewIntent` 경로를 통과하며, 3,840 non-table record 뒤 exact 256-cell fixture의 cell 수와 표 context도 controller-owned `tableGroups`에서만 얻는다. 초기 CM6 background parse를 jsdom에서 결정적으로 대신하는 `forceParsing` prewarm은 controller enable과 측정 probe 전에만 실행하며 interaction 경로에는 포함하지 않는다. capture/context lookup 구간은 table/group·cell 이진 탐색과 cached readonly cell slice만 소비해 이벤트별 record check≤32, 새 cell 배열 할당 0이다. 성공한 selection transaction 뒤 projection reconciliation은 entry≤4,096의 별도 bounded commit이고, table entry identity가 모두 같으면 기존 group/cell slice를 재사용해 group build scan과 cell 배열 할당이 모두 0이다. 문서 변경이나 table identity 변경 때만 group build가 record≤4,096을 한 번 순회한다. 64 Ki UTF-16보다 큰 같은 제품 문서에서 시작한 copy probe와 두 editor host의 실제 `MutationObserver`가 hot path를 감싼다. schema v4는 `table_intent_events=10`, `table_cm6_transactions=8`, `table_context_record_checks>0`(전체≤384), `table_event_record_checks_max<=32`, exact `table_projection_record_count=4096`, `table_non_table_record_count=3840`, `table_projection_record_checks_max=4096`, fixture의 `table_group_build_record_checks_max<=4096`, `table_group_cell_arrays_created_max=1`, `table_context_cells_retained_max=256`, `table_queue_max_retained=1`, `table_external_actions=0`, `table_bridge_calls=0`, `table_iframe_create=0`, `table_iframe_destroy=0`, `table_copied_bytes=0`을 고정한다. exact-cap 표의 `Shift-Tab`/`Tab` 성공 왕복은 unchanged group build/allocation 0을 scenario 내부에서 별도로 단언한다.

| 경로 | hard gate | 실패 조건·구현 제약 |
| --- | --- | --- |
| `EditableProjection` discovery | 한 transaction에서 방문하는 source는 visible range+앞뒤 1 viewport이고 합계 ≤ 64 Ki UTF-16 code unit, tree iterate와 보조 cursor를 합친 syntax node visit≤8,192, emitted `ProjectionEntry`≤4,096, table cell≤256, 동일 inline parse context의 모든 dollar 후보가 공유하는 delimiter scan≤32 Ki UTF-16, full-document scan/copy 0, remark/unified 호출 0 | 현재 CM6 incremental Markdown tree만 읽는다. node/entry exact cap을 넘으면 이미 만든 entry를 버리고 bounded window 전체를 source-preserving `projection_limit`으로 둔다. table cell cap+1은 table 전체를 `table_limit` 원문으로 둔다. syntax tree incomplete/ambiguous도 해당 범위를 원문으로 둔다. Select All도 discovery 범위를 넓히지 않고, multi-selection 교차는 정렬 range 이진 탐색을 쓴다. |
| decoration apply | canonical `(from,to,variant_tag,닫힌 enum/scalar fields)`의 O(N+M) merge diff, document-size 비례 allocation 0, 동일 fingerprint DOM mutation 0, composition 중 apply 0 | 정렬된 bounded projection만 diff하고 per-entry `find`/`some` 중첩 탐색을 금지한다. syntax marker를 숨기기 위해 text 자체를 복제하거나 별도 rich document를 만들면 실패한다. |
| direct interaction | pointer/key event당 CM6 transaction≤1, 외부 action≤1, 두 효과 동시 발생 0, document-wide table parse 0, retained intent running+pending≤8 | task/table/plain click은 native bridge write 0이다. trusted `⌘`/`⌘⇧` link activation만 명시적 사용자 gesture당 bounded bridge action 최대 1회를 허용한다. capture-time composition/repeat/synthetic event는 enqueue 0이고 `LivePreviewIntent`가 실행 시 현재 guard/revision/range를 다시 검증한다. native link completion은 문서 mutation queue 밖 bounded lane에서 기다리며 close lock·mode/document 전환은 pending을 폐기한다. destination cap+1은 CM6 source slice 0이다. |
| table edit | 현재 table source ≤ 64 Ki UTF-16 code unit, cell≤256, 행 추가 transaction≤1, view 진입 normalization 0 | malformed/ragged/escaped-pipe 경계가 모호하거나 cap+1이면 table 전체를 source-preserving으로 둔다. 별도 contenteditable/table document를 금지한다. |
| atomic projection | document-size 비례 Web main-thread hash/copy 0, Mermaid exact source Web main 수신≤32 KiB/job·Web main 재hash 0, native Zig admission 보안 재hash≤32 KiB/job·frame tick hash 0, worker request별 hashed bytes≤kind cap(KaTeX/Mermaid 32 KiB, Prism 256 KiB)·8-request batch aggregate≤2 MiB, 일반 Markdown iframe 0, atomic iframe≤8, frame당 mount/unmount≤2, viewport+overscan+2-viewport retention window 밖 신규 HTML 생성·전송 0 | Web main은 exact range/nonce만 보내고 delta-synced worker가 hash한다. Zig admission은 MainActor bridge 요청에서 exact bounded source를 한 번 재hash해 worker digest를 보안상 재검증하며, 이는 frame tick이 아니다. exact cap은 처리하고 cap+1은 hash/enqueue 0이다. 이미지·수식·비활성 fenced code·Mermaid만 `AtomicProjection`으로 보내며 desired/mounted/retained-but-not-visible/generated HTML을 별도 counter로 둔다. 승인된 Mermaid SVG cache는 editor당 최대 8개·entry당 512 Ki UTF-16 code unit이며 SHA-256+exact source 일치만 재사용한다. cap/timeout/asset 거부는 해당 range만 원문으로 강등한다. |
| diagnostics | 제품 직렬화 active source offset 0, test-only range 저장≤16, content/path/URL bytes 0, frame tick snapshot 0, snapshot 추가 allocation 0 | applied projection set+widget registry의 fixed storage에서 한 함수가 파생한다. 제품 snapshot은 전체 range count와 truncated bit만 내고 `{from,to}`는 bridge/log/artifact에 없는 test-only in-memory accessor로 분리한다. 테스트·smoke의 count/reason은 제품과 같은 snapshot을 읽는다. |

FP11d의 `mise run web:live-preview-perf`는 schema v4 `fp11d-8mib-1000-editable-projection-table-interactions`를 두 번 실행해 동일한 `tests/artifacts/perf/live-preview.json`인지 확인한다. short-paragraph 정확히 8 MiB 문서의 실제 jsdom `EditorView`→`EditableProjectionController.handleUpdate`→`buildEditableProjection` 경로에서 heading text 한 글자를 같은 길이로 1,000회 교체하고, 별도 product `EditorView`의 `"$1".repeat(8192)` dense-dollar parse를 함께 실행한다. source transaction은 정확히 1,000회여야 하고 `Text.toString`·`Text.sliceString`·`TextEncoder.encode`의 64 Ki UTF-16 초과 document-scale copy는 0이다. 공용 상한은 실제 tree/cursor visit, selection range 비교, parser-site dollar scan, 생성한 Decoration 수와 정렬 merge 비교 수를 각각 센다. 단위 fixture는 node/entry/table/dollar/link destination exact-cap과 cap+1, 1,024 multi-cursor의 logarithmic 비교, 4,096 entry 중 하나 변경 시 4,095 Decoration identity 재사용을 검증한다. 실제 counter는 `visited_code_units`, `visited_syntax_nodes`, `selection_range_checks`, 일반 fixture의 `math_scanned_code_units`, 단일 dense context를 정확히 32 Ki 이하로 직접 gate하는 `dense_math_scanned_code_units`, `emitted_decorations`, `diffed_decorations`, `copied_bytes`, `source_transactions`, `projection_transactions`, projection-class `dom_mutations`, `iframe_create`, `iframe_destroy`, `retained_html_bytes`, `generated_outside_retention`, `projection_fallback_counts`다. FP11c interaction probe는 제품 `LivePreviewIntentCoordinator`를 통과해 `intent_events=6`, `intent_cm6_transactions=1`, `intent_external_actions=1`, `intent_dual_effects=0`, `intent_zero_effect_rejections=4`, DOM target lookup과 실행 시 entry 재검증을 합친 `intent_range_checks>0`, `intent_queue_capacity=8`, `intent_queue_max_retained=8`, cap+1의 `intent_queue_dropped=1`, baseline과 stalled-cap probe의 실제 DOM mailbox `intent_bridge_calls=2`를 추가한다. task commit과 link action을 각각 한 번 실행하고 duplicate nonce·stale revision·composition·untrusted intent가 모두 효과 0인지 같은 coordinator 결과로 센다. FP11d table probe는 `table_intent_events=10`, `table_cm6_transactions=8`, `table_source_transactions=2`, `table_appended_rows=2`, `table_zero_effect_rejections=2`, `table_cell_cap=256`, cap+1과 multi-range의 transaction 0을 추가해 다음/이전/아래 이동, 마지막 Tab/Enter의 한 행 추가, exact-cap 표의 성공 `Shift-Tab`/`Tab` 왕복을 실제 dispatcher에서 결정적으로 센다. exact fixture가 4,096 record·그중 non-table 3,840인지, 성공 이벤트의 reconciliation 비교가 4,096인지, unchanged table group build/allocation이 0인지도 controller metric으로 검증한다. 같은 source geometry의 1,000회 입력 구간은 최초 decoration commit 하나만 유지해 `projection_transactions=1`, 구간 시작 전 observer record를 버린 뒤 `dom_mutations=0`이어야 한다. 일반 iframe/retained HTML은 0이며 fixture의 image atomic 후보 하나만 `atomic_not_enabled=1`이다. 명령·artifact·필수 field 누락, threshold 초과, 같은 fixture 반복의 비결정성은 nonzero exit다. FP11e 이후 각 PR은 관련 counter와 bounded/stale/composition 회귀를 먼저 추가해 이 task를 필수 실행한다. FP11f가 Mermaid 제품 pump를 활성화할 때는 아래 native task·artifact도 같은 PR의 필수 gate다.

FP11f/FP10c2는 기존 web task를 Mermaid counter로 확장하고 `mise run macos-live-preview-perf` task·CI required job을 추가했다. web artifact는 8 MiB short-paragraph fixture의 1,000회 입력과 함께 Mermaid main bridge가 실제 받은 source bytes, native request 수, bounded cache와 disable 뒤 0을 기록한다. native `tests/artifacts/perf/live-preview-macos.json`은 제품과 같은 `MermaidProductTickAdapter`, helper queue/concurrency/timeout/restart/failure latch와 synchronous process spawn/terminate·pipe setup/read/write·blocking wait를 기록한다. 정상 max-SVG 경로 1,000 tick과 별도로 세 번의 5초 deadline→terminate ACK→restart→latch 전 구간도 같은 product tick adapter로 구동하며 `failure_latch_product_tick_calls>0`, completion drain≤8, final pending/source 0을 요구한다. 2.6초 지연한 cold result가 restart 없이 accepted되는 실제 helper smoke도 2초→5초 계약 회귀를 고정한다. 이때 smoke harness의 pump outer timeout은 제품 cold deadline(5.0s) + fallback grace(→ 5.25s)보다 커야 한다 — cold helper의 첫 결과를 기다리는 pump는 7.0s로 둔다. 그렇지 않으면(옛 4.5/4.0/3.0s) 느린 runner에서 제품이 아직 정상 accept할 결과를 harness가 먼저 잘라 false-fail이 난다. 성공·실패 판정은 여전히 Zig deadline(`deadline_expirations`)이 소유하고 harness는 그 판정을 관측할 시간 여유만 준다. bundle-seal 전 거부·integrity latch처럼 deadline-bounded가 아닌 pump는 짧은 timeout을 유지한다. 두 시나리오 모두 process/pipe/wait tick counter는 0이어야 한다. task나 artifact/필수 field 누락·비영 tick counter·상한 초과는 실패하며 Web/macOS required job이 각각 업로드한다. FP11a부터 Web required job은 `web:live-preview-perf`와 JSON artifact upload를 실행하고, FP11f는 native task 없이 머지할 수 없다. WebContent RSS, Mermaid WebKit service CPU의 정확한 deadline 종료와 실제 key-to-paint p95 수치 실패 gate는 공개 process 귀속/입력 지연 하니스가 없어 주장하지 않는다.
...
현재 gate는 schema v7 `fp11f-8mib-1000-editable-projection-mermaid`다. 기존 8 MiB·1,000 input·intent·table counter를 보존하면서 실제 `EditorView → EditableProjectionController commit → AtomicProjectionController.handleUpdate/submitEntries → LivePreviewWorkerClient → protocol-valid worker` 입력 경로를 연결한다. 1,000개 CM6 delta는 최초 base부터 `Apply 0→1000` 하나로 합성되고 Seed 추가 0이어야 하며, input 구간의 실제 `Text.toString`/큰 slice/encode copy probe와 worker hash probe로 `atomic_main_copied_bytes=0`, `atomic_main_hashed_bytes=0`을 산출한다. 별도 제품 facade probe는 실제 CM6 selection transaction 100회와 결정적 viewport getter를 사용해 `EditableProjectionController` commit을 atomic controller에 연결한다. 동일 terminal widget에서 worker request·asset read·iframe identity 변화 0, discovery 밖이지만 2-viewport hysteresis 안에서는 identity 유지, retention 밖 이동은 worker render/transmit 0, 재진입은 scroll/viewport commit을 포함해 request≤8의 bounded 생성이어야 한다. 8개 widget 전량 교체의 실제 `MutationObserver` 값으로 `atomic_mounted_max=8`, frame당 create/destroy≤2를 산출한다. worker probe는 `atomic_requests=4`, `atomic_results=4`, `atomic_asset_grants=1`, request별 `atomic_worker_hashed_bytes_max=262144`, exact 8-request aggregate `atomic_worker_hashed_bytes_batch_max=2097152`, `atomic_cap_plus_one_hashed_bytes=0`을 고정한다. exact 256 KiB fence는 bounded escaped plain code로 처리하고 cap+1은 hash/render 전에 거부한다. token-dense Prism과 전체 result envelope+success HTML/grant+rejection metadata 합계 2 MiB도 공용 `atomicProjectionBatchWireBytes`로 range-local gate하며, exact cap·cap+1과 success+rejection 혼합을 producer/validator 양쪽에서 검증한다. 같은 scenario를 두 번 실행한 JSON bytes가 달라지면 실패한다. 위 schema v4 문단은 FP11d 당시 counter의 역사적 baseline이다.
...
| session-host slow observer | P5b2b1은 매 owner turn의 logical queue/base/global ledger가 compile-time hard cap을 넘지 않아야 한다. P5b2b2는 독립 ReleaseFast host의 warm baseline 대비 반복-sample peak와 post-drain RSS를 `session-host-slow-observer-macos.json`에 기록하며, projector/encoded transient와 allocator slack을 포함한 분석 상한을 validator가 검사한다. | in-process test runner RSS, `ru_maxrss`, FakeRuntimeOps input 성공만으로 PTY/RSS isolation을 완료 처리하지 않는다. |
...
Web schema v7은 기존 counter에 `mermaid_requests=1`, `0 < mermaid_worker_hashed_bytes <= 32 KiB`, `mermaid_cap_plus_one_hashed_bytes=0`, `mermaid_main_hashed_bytes=0`을 추가한다. 여기서 `main`은 Web main thread만 뜻하며, source를 받지 않는다는 뜻이 아니라 Web에서 hash를 반복하지 않는다는 뜻이다. 실제 bridge source copy는 `0 < mermaid_main_received_source_bytes <= 32 KiB`로 기록한다. native MainActor의 JSON/UTF-8 변환과 보안상 필요한 Zig exact-source SHA-256은 이 Web artifact에 귀속하지 않으며 job당 32 KiB로 닫혀 있고 frame tick 밖이다. 현재 native artifact는 이 admission wall-time을 별도 필드로 측정하지 않는 한계를 남기되, unrelated edit 전후 `mermaid_native_requests=1`, cache entries≤8·SVG≤512 Ki code unit/entry, disable 뒤 cache 0을 함께 gate한다. native schema `maru.live-preview-macos.v1`은 제품과 같은 `MermaidProductTickAdapter`를 1,000번 호출하고 `completion_drain_max<=8`, helper physical start, 3회 deadline failure latch를 기록한다.
