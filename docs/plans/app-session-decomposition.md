...
| **F17** ✅ | 상태바 + 그 안의 리소스 표시(CPU·메모리 표본·메뉴) | 892(메서드) | 낮음 | 2026-09-22 완료 → `app_session/status_bar.zig` 1,057줄 |
| **F18** ✅ | 터미널 마커 프리뷰(열기·배치·그리기·번호/히트 수집) | 약 480(메서드) | 낮음 | 2026-09-22 완료 → `app_session/marker_preview_view.zig` 579줄 |
| **F19** ✅ | 원격 에이전트 채널(host 수명·Term 연결·줄 공급·정리) | 약 400(메서드) | 낮음 | 2026-09-22 완료 → `app_session/remote_agent.zig` 458줄 |
...
>
> **pub 순증은 셋 합쳐 39** 다(AppSession pub 메서드 +5 · 최상위 pub 선언 +29 · AppSession pub const +5).
> F2(50)·F7(44)·F4(43) 보다 낮고 F6(41) 수준이다. 세 그룹 모두 그룹 «밖의 비공개 메서드» 는 거의
> 안 부르고(상태바는 0 건), 비용의 본체는 **파일 스코프 상수·헬퍼**였다
