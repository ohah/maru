...
| 2026-08-10 (호출 그래프 귀속) | **52,637** | 이름이 아니라 호출로 소유를 정해 46개 이동. **F 시리즈 착수(72,317) 이후의 최저점** |
| 2026-08-18 | **59,690** | 분해 뒤 **재성장**(+7,053). 아래 "분해가 끝난 뒤에도 자란다" |
...
> **분해가 끝난 뒤에도 자란다(2026-08-18 실측).** 바로 위 블록은 F10 시점(59,124줄)의 서술이다. 그 뒤
> 표의 최저점 **52,637**까지 내려갔다가 지금은 다시 **59,690줄**이다. 최저점까지도 단조 감소는 아니었다 —
> 파일 레벨 헬퍼 동반 이동이 줄 수를 55,435로 되레 올렸고(그 작업의 목적은 줄 수가 아니라 pub 표면이었다),
> F6 보정·디버그 픽스처 분리가 다시 내렸다. 최저점 이후 무엇이 자랐는지 test 블록을 갈라 보면:
>
> | 구획 | `d79ae3f1`(2026-08-11) | 2026-08-18 | 증가 | 증가분 비중 |
> |---|---|---|---|---|
> | test | 34,558 | 38,611 | +4,053 | 65% |
> | 제품 | 18,936 | 21,079 | +2,143 | 35% |
> | 전체 | 53,494 | 59,690 | +6,196 | 100% |
>
> 비교 기준은 커밋 `d79ae3f1` 시점의 파일이고 그 뒤 이 파일을 건드린 커밋은 **146개**다(52,637을 적은
> 호출 그래프 절과는 하루 차이가 있어 시작값이 소폭 다르다). 줄 수는 `wc -l` 기준이고, test 구획은
> **컬럼 0 `}` 종료**와 **리터럴·주석 제거 후 괄호 depth** 두 파서로 양쪽 시점 모두 따로 세어 0.00%
> 차이로 일치시킨 값이다(한 방법만 쓰면 문자열 속 중괄호에 깨진다 — 아래 "경계 계산에서 결함 셋을 잡았다").
>
> **읽는 법 둘.** ⑴ test가 증가분의 3분의 2다 — §2-c-3이 "의도적으로 남긴다"고 한 몫이 계속 커진다.
> ⑵ 그런데 **제품 코드도 2,143줄 늘었다.** F 시리즈가 그룹으로 빼낸 것보다 새로 허브에 들어오는 것이 있다는
> 뜻이고, 이것은 파일을 한 번 더 가른다고 해결되지 않는다(무엇을 어디에 쓰는가의 문제다). 다음에 이 파일의
> 크기를 논할 때는 "분해가 덜 됐다"가 아니라 이 두 흐름을 먼저 본다.
>
> **그룹 파일의 test는 0이 아니다(같은 실측에서 정정).** `app_session/` 20개를 세면 test 블록이 있는 것이
> 다섯이다 — `editor.zig` 85개·`editor_diff.zig` 40개·`scm_dock.zig` 6개·`settings.zig` 4개·`pane.zig` 1개.
> 앞의 셋은 **분해 산물이 아니라** 처음부터 그 폴더에 쓴 새 기능이고, 뒤의 둘은 분해 뒤에 붙은 소수다
> (`pane.zig`는 F4 생성 시 test 0이었다). 즉 분해로 떼어낸 17개 중 **15개가 test 0**이고, 나머지 둘이
> 가진 test도 합쳐 5개뿐이다 — §2-c-3의 규칙("분해 시 test는 원본에 남기고, 새 파일을 처음 쓸 때만 그 파일에
> test를 쓴다")이 실제로 지켜지고 있다는 뜻이다.
>
> **거대 메서드도 같이 다시 잰다**(아래 2026-08-08 목록과 같은 자리): `tick`(1,425)·`mouse`(838)·
> `deinit`(381)·`handleKeyEvent`(351)·`hoverCursor`(287)·`init`(214).
> `maybeDebugOpenSettings`는 디버그 픽스처 분리로 목록에서 빠졌다.
> **허브 소함수 추출도 되감겼다** — §6이 기록한 추출 직후 값(`tick` 1,367·`mouse` 794·`handleKeyEvent` 340)에서
> 각각 +58·+44·+11 늘어 2026-08-08 값에 가까워졌고, `deinit`은 322 → 381로 그때보다도 커졌다.
> 파일 전체와 같은 방향이다(위 표) — 한 번 줄인 것이 그대로 있지 않다.
...
| **F17** ✅ | 상태바 + 그 안의 리소스 표시(CPU·메모리 표본·메뉴) | 892(메서드) | 낮음 | 2026-09-22 완료 → `app_session/status_bar.zig` 1,057줄, pub 순증 38 |
| **F18** ✅ | 터미널 마커 프리뷰(열기·배치·그리기·번호/히트 수집) | 약 480(메서드) | 낮음 | 2026-09-22 완료 → `app_session/marker_preview_view.zig` 579줄, pub 순증 17 |
| **F19** ✅ | 원격 에이전트 채널(host 수명·Term 연결·줄 공급·정리) | 약 400(메서드) | 낮음 | 2026-09-22 완료 → `app_session/remote_agent.zig` 458줄, pub 순증 19 |

> **F17~F19 를 F16 까지 못 찾은 이유 — 이름이 동사로 시작한다(2026-09-22).**
>
> §4.1 은 "F10 으로 **이름 기준** 그룹은 전부"라고 적었다. 그 「이름 기준」이 **앞부분**이었다.
> 그런데 주제가 이름 «가운데» 있는 메서드가 많다:
>
> ```
> collectStatusBarItems · appendStatusBarBackground · buildStatusBarItem
> appendMarkerPreviewImage · toggleMarkerPreviewAt · pollMarkerPreview
> drainRemoteAgentHost · feedRemoteAgentTerms · closeRemoteAgentHost
> ```
>
> 남은 589 개 메서드를 첫 낱말로 묶으면 `collect…` 798 줄 · `build…` 437 · `append…` 297 처럼
> **동사 덩어리**만 나와 「주제가 없다」로 읽힌다. **동사를 걷어내고 명사로 묶으면** `StatusBar` 513 ·
> `RemoteAgent` 311 · `MarkerPreview` 294 가 드러난다. 다음 그룹을 찾을 때는 이 방식으로 본다.
>
> **경계는 여전히 호출 관계로 확인했다.** StatusBar 와 Resource 는 이름으로는 둘인데 서로 부른다
> (각 2 건 — 리소스 표본이 상태바 «항목» 이 되고 상태바 클릭이 리소스 «메뉴» 를 연다). F1 이
> archive 와 agent dock 을 합친 것과 같은 판정이라 한 파일로 뒀다.
>
> **pub 순증은 셋 합쳐 74** 다 — 그룹별로 **status_bar 38 · remote_agent 19 · marker_preview_view 17**.
>
> ⚠️ **이 열의 과거 값과 그대로 비교하면 안 된다 — 행마다 정의가 다르다(2026-09-22 검산).**
> F16(`term.zig`)을 같은 정의로 다시 재 보면 문서의 **6** 과 아무 데서도 안 맞는다:
>
> | F16 을 어떻게 재나 | 값 |
> | --- | --- |
> | 남은 쪽(`app_session.zig`)이 연 것 — 전체 | 21 |
> | 남은 쪽이 연 것 — **fn 만** | **7** ← 문서의 6 에 가장 가깝다 |
> | 나간 쪽(`term.zig`)의 새 pub | 17 |
> | **양쪽 합계(위 74 가 쓰는 정의)** | **38** |
>
> 즉 옛 행들은 대체로 **「남은 쪽에서 pub 화가 강제된 메서드 수」** 를 적었고, 그것은 이 열의
> 일부일 뿐이다. **같은 정의로 재면 F16 term 38 · F17 status_bar 38 로 사실상 같다.**
> settings(60)·F2(50) 같은 값은 정의가 확인되기 전까지 나란히 놓지 않는다.
>
> **앞으로는 이 정의로 잰다**: 「비공개였다가 공개가 된 선언의 수, **양쪽 파일 합쳐**,
> fn·const·var 전부」. 그룹을 여럿 떼면 **그룹별로 귀속**해서 적는다.
>
> ⚠️ **이 수를 처음에 39 로 잘못 적었다**(PR #3856 최초 본문·커밋 메시지). 적대적 검증이 잡았다.
> 39 는 **그룹 파일 쪽만** 센 수다 — 파일을 나누면 `app_session.zig` **에 남는 쪽도** 문을 열어
> 줘야 하는데(그룹이 쓰려면) 그 46 개를 빠뜨렸다: 최상위 const 23 · 최상위 fn 6 · 멤버 fn 12 ·
> 멤버 const 5. **분리 비용을 세려면 «나간 쪽» 과 «남은 쪽» 을 다 세야 한다.** 다음 슬라이스는
> 이 정의로 잰다 — 「비공개였다가 공개가 된 선언의 수, 양쪽 파일 합쳐」.
>
> 세 그룹 모두 그룹 «밖의 비공개 메서드» 는 거의 안 부르고(상태바는 0 건), 비용의 본체는
> **파일 스코프 상수·헬퍼**였다
> (`status_bar_metrics`·`resource_poll_interval_ms`·`truncateColsInto`·`MarkerPreviewOwner` 등).
> 열린 74 개는 **전부 실제 소비자가 있다** — 그룹 파일의 pub fn 35 개는 별칭 호출로, `app_session.zig`
> 가 연 46 개는 `self.f(...)`·`AppSession.f(...)` 로 실제로 쓰인다(적대적 검증에서 전수 확인).
>
> **파일 이름 충돌에 주의한다.** `app_session/marker_preview.zig` 가 이미 있다(프리뷰의 상태 기계,
> MP1). 여기서 뗀 것은 그 상태를 macOS 세션에 붙이는 자리라 `marker_preview_view.zig` 로 나눴다.
>
> 결과: `app_session.zig` **92,467 → 90,509 줄**(−1,958). 그룹 셋 합계 2,094 줄.
>
> **⚠️ 판정자를 고칠 때 «감시 대상만» 옮기면 원래 파일 쪽에 구멍이 난다(2026-09-22, 적대적 검증).**
> MP1 판정자는 `app_session.zig` 안의 `dock_ops.openDockTo(self, .agent_activity)` 가 **정확히 하나**
> 임을 요구했다. F18 이 그 본문을 `marker_preview_view.zig` 로 옮기자 게이트가 빨개졌고, 나는 그 단언을
> 새 파일로 «옮겨» 초록을 만들었다. 그 순간 `app_session.zig` 는 **무제한**이 됐다 — 추출 전이라면
> 둘째 호출이 개수 2 로 잡혔을 자리가 이제 아무 말도 안 한다. 뮤테이션으로 실제 확인했다(다른 자리에
> 심은 호출이 안 잡혔다). `app_session.zig` 쪽 **0** 단언을 더해 메웠다.
>
> **규칙**: 추출로 판정자가 빨개지면 단언을 옮기지 말고 **쪼갠다** — 새 파일에 「여기 하나」,
> 원래 파일에 「여기 0」. 개수 판정은 «어느 파일을 보는가» 가 판정의 절반이라, 대상만 바꾸면
> 판정이 조용히 반쪽이 된다.
...
>
> **후속: 의존 방향 정리 — 2026-08-10 실측으로 보류한다.** 그룹 파일 17개 사이에 양방향 쌍이 **39개** 있고
> tree↔panel은 그중 하나일 뿐이다(아래 "그룹 파일 17개는 독립 모듈이 아니다"). 한 쌍만 고치면 나머지
> 38쌍이 그대로여서 일관성이 없다. 아래 원안은 방법으로는 유효하므로 남긴다.
>
...
> 추정했다. 후보 55개를 **적대적 검증 2회**로 걸러 46개를 옮겼다.
>
> | | |
> |---|---|
> | `app_session.zig` | 54,321 → **52,637** (−1,684) |
> | 옮긴 함수 | 46개 (file_panel 18 · settings 12 · tab 8 · term 5 · sidebar 3) |
> | 동반 이동 파일 레벨 | 7개 |
> | ABI facade 신설 | 17개 |
> | 허브 pub | 558 → 560 (**+2**) |
>
> ### 검증 1회차 — 내 반증 근거가 무너졌다
>
> 처음 "호출 신호와 필드 신호가 34건 어긋난다"고 쟀는데 그 측정 자체가 결함이었다.
>   - **정규식이 `self.buildX()` 같은 메서드 호출을 필드로 셌다.** settings 12개가 "필드 없음"으로 나온
>     원인이다. 진짜 필드는 `dropdown_snapshot_*` 등 전부 settings 상태였다.
>   - **file_panel이 만지는 `dock_*` 필드는 `dock_initialized` 하나뿐**(9회)이었다. "도크가 떠 있나"
>     가드일 뿐 도크 로직이 아니다.
>
> ### 검증 2회차 — 교정 후 불일치 17 → 10
>
> | | 교정 전 | 교정 후 |
> |---|---|---|
> | 일치 | 21 | 21 |
...
> | **불일치** | **17** | **10** |
>
> 남은 10건 중 **6건은 교차 도메인이라 제외**했다 — `copyText`(주소창+터미널)·`applyMetricsPipeline`
> (폰트 변경 파생 파이프라인)·`handleDroppedFiles`·`handleDroppedImage`(원격 업로드+paste)·
> `metalFrame`(프레임 조립)·`collectSessionInto`(세션 전체 순회)다. 어느 그룹에 넣어도 틀렸을 것이다.
> 2건은 재배치했다(`computeDropTarget`→tab, `refreshAfterReap`→term).
>
> ### pub 예측이 세 번째로 빗나갔다
>
> **−21을 예측했는데 실제는 +9였다**(동반 이동 7개를 붙여 +2로 낮췄다).
>
> | | 예측 | 실제 |
> |---|---|---|
> | F14 agent | +3 | **+24** |
> | F16 test 시범 이동 | +12 | **+35** |
> | 호출 그래프 귀속 | **−21** | **+9** |
>
> 원인은 매번 같다 — **"옮기는 것"만 세고 "옮긴 것이 부르는 것"을 덜 센다.** 앞으로 pub을 예측할 때는
> 이동 대상이 부르는 허브 비공개 선언의 **폐포**까지 세야 한다.
>
> ### 닫기 클러스터는 옮기지 않는다(기록만)
>
> `requestClose` → `resolveCloseScope`·`scopeHasRunningJob`, 그리고 F10이 허브에 남긴
> `closeTargetHasRunningJob`도 같은 둘을 부른다. 폐포를 구하면 **7개·137줄이 자족적**이고 허브 비공개
> 호출이 0개다 — **이름이 아니라 호출로만 드러나는 그룹**이다.
>
> 그럼에도 별도 파일로 빼지 않는다. 양방향 순환 쌍이 이미 39개이고(아래 절), 137줄을 얻자고 간선을 둘
> 늘리는 것은 "순환은 이 구조의 성질"이라는 결론과 어긋난다. 다음 사람이 알 수 있도록 기록만 남긴다.

> ## 그룹 파일 17개는 독립 모듈이 아니다 — 결합 실측(2026-08-10)
>
> F2에서 "tree↔panel이 순환하므로 한 파일로 둔다"고 결정했다. 그 기준을 **전체에 적용해 보면
> 일관되지 않는다**는 것이 드러나 여기 기록한다.
>
> | | |
> |---|---|
> | 그룹 파일 | 17개 |
> | 그룹 간 `@import` 간선 | **118개** |
> | **양방향 쌍** | **39쌍** |
> | import 밀도 | 118/272 = **43%** |
>
> 즉 F2의 tree↔panel은 **예외가 아니라 전형**이었다. 39쌍 중 하나만 특별 취급한 셈이다.
>
> ### 층위를 정할 수 있는가 — 11쌍이 막는다
>
> 양방향 쌍을 방향별 호출 수로 나누면:
>
> | | 쌍 |
> |---|---|
> | 약한 방향이 2회 이하(끊기 쉬움) | 28 |
> | **양쪽 다 3회 이상(진짜 양방향)** | **11** |
>
> `pane ↔ tab`(30/30) · `sidebar ↔ tab`(21/19) · `pane ↔ term`(16/18) · `dock ↔ file_panel`(14/19) ·
> `agent_dock ↔ dock`(22/11)이 그렇다. 탭이 pane을 담고 pane이 자기 탭을 아는 것은 **도메인의 실제
> 모양**이라 끊으려면 중간 자료구조나 이벤트 역전이 필요하다 — 리팩터가 아니라 재설계다.
>
> ### 끊어도 얻는 것이 없다
>
> | 기대 이득 | 실제 |
> |---|---|
> | 빌드 시간 | **없음** — Zig는 모듈 단위 컴파일이라 파일 간 순환을 끊어도 증분이 빨라지지 않는다(C 헤더와 다르다) |
> | 독립 테스트 | **없음** — 17개 파일의 함수가 전부 `*AppSession`을 받는다. 한 파일만 따로 세울 수 없다 |
> | 우발적 결합 방지 | 있으나 작음 — 118개 중 어느 것이 "실수"인지 판별할 근거가 지금 없다 |
> | 나중에 떼어내기 | 있으나 계획 없음 — `term`을 app_session 밖으로 뺄 목표가 문서에 없다 |
>
> ### 결론 — 순환은 결함이 아니라 이 구조의 성질이다
>
> **17개 파일은 독립 모듈이 아니라 한 모듈(`AppSession`)의 조각이다.** 필드를 모두가 공유하므로 서로를
> 부르는 것이 자연스럽다. F 시리즈가 실제로 얻은 것과 못 얻은 것을 정직하게 적으면:
>
> | 얻었다 | 못 얻었다 |
> |---|---|
> | **탐색성** — 사이드바를 고치려면 `sidebar.zig`만 연다 | **모듈 경계** — 여전히 서로 다 부른다 |
> | 파일당 크기(73,658 → 54,000줄대) | 캡슐화(pub 93 → 558) |
>
> 순환은 **원인이 아니라 증상**이다. 원인은 "하나의 큰 상태 객체를 파일로만 나눴다"이고, 그것을 고치려면
> 상태를 나눠야 하는데 실측상 함수의 **25~32%만** 한 도메인 필드만 만진다(sidebar 27/83, tab 25/76,
> scroll 10/39, notification 7/29). 나머지는 여러 도메인을 함께 읽는 것이 그 함수의 일이다.
>
> **그러므로 층위 정하기는 "term을 app_session 밖으로 뺀다" 같은 목표가 생겼을 때 한다.** 지금은 그
> 목표가 없고, 하면 28쌍을 끊는 수고만 남는다. F2의 후속(tree→panel 의도 반환)도 같은 이유로
> **그 목표가 생길 때까지 보류**한다 — 한 쌍만 고쳐도 나머지 38쌍이 그대로이므로 일관성이 없다.
...
## 6a. F 시리즈 이후 재측정 (2026-09-22)

F1~F16 이 끝난 지 6 주가 지나 다시 쟀다. **측정은 전부 `std.zig.Ast` 로 했다** — 중괄호를
정규식으로 맞추면 주석·문자열 안의 `{`·`}` 에 걸려 구조체 끝을 지나친다(실제로 한 번 그렇게 재서
메서드가 8,330 줄로 나왔다). 아래 수치는 다음 결정의 재료이고, **결론을 담지 않는다.**

### 무엇이 늘었나

| 날짜 | 전체 | test | 제품 | test 증가 | 제품 증가 |
|---|---|---|---|---|---|
| 2026-08-11 (F 직후) | 53,494 | 35,345 | 18,149 | — | — |
| 2026-08-20 | 60,814 | 40,302 | 20,512 | +4,957 | +2,363 |
| 2026-09-01 | 77,055 | 52,115 | 24,940 | +11,813 | +4,428 |
| 2026-09-10 | 84,894 | 58,541 | 26,353 | +6,426 | +1,413 |
| 2026-09-22 | **92,454** | 63,057 | 29,397 | +4,516 | +3,044 |

**6 주에 38,960 줄이 붙었고 그중 71%(27,712)가 test 다.** F 시리즈가 걷어낸 것은 17,382 줄이었으니
증가 속도가 분해 속도의 두 배다. 분해가 실패한 것이 아니라, **분해가 다루는 축(메서드 그룹)이
아닌 축(새 기능 + 새 판정자)으로 자랐다.**

이 수치는 「줄 수 상한」류 도구의 표적이 틀렸다는 뜻이기도 하다 — 총 줄 수를 막으면 주로
**판정자 추가**가 막히고, 계획이 「test 는 잔류」로 정한 이상 개발자에게 합법적 탈출구가 없다.

### 총량과 조각

```
허브 92,467 + 그룹 42 개 97,198 = 189,665 줄
```

§6 「파일 수 증가」가 경고한 *"총 읽을 양이 주는 게 아니다"* 가 그대로다. 그리고 **떼어낸 조각이
다시 비대해진다**: `app_session/editor.zig` 가 **37,955 줄**(test 70%)로 두 번째 `app_session.zig` 가
됐다. 분해는 한 번인데 증가는 계속이라 같은 일이 한 단계 아래서 반복된다.

### 양방향 의존 73 쌍 (§4.1 의 39 쌍에서)

그룹 42 개 사이의 의존 간선 262 중 **양방향이 73 쌍**이다(AST 토큰으로 셌다 — 별칭 `alias.` 를
정규식으로 세면 주석 안 언급까지 센다). 2026-08-10 에 39 쌍을 세고 「한 쌍만 고치면 나머지가
그대로여서 일관성이 없다」며 보류했는데, 보류하는 동안 두 배가 됐다.

**양방향이면 두 파일은 독립 모듈이 아니다** — 파일 경계는 있고 의존 경계는 없다.

### `pub` 표면

`AppSession` pub 메서드 **350 개 중 165 개(47%)를 그룹 파일이 부른다.** §6 이 「되돌릴 수 없다」고
경고한 항목이고, 이제 절반이다. 한편 test 가 부르는 **비공개** 메서드는 82 개다(픽스처 아홉을
파일 스코프로 내린 뒤 91 → 82).

### test 를 파일 밖으로 옮기는 비용 (재측정)

§2-c-3 은 F1 한 그룹에서 「동반 이동 78 vs 잔류 13」을 쟀다. **파일 전체를 한 덩어리로 옮기는
경우는 그 실험과 조건이 다르다**(그때 비용의 본체였던 「다른 그룹 메서드 호출」이 같이 옮겨 가면
사라진다). 다시 재면:

| | 개수 |
|---|---|
| test 가 이름으로 닿는 비공개 선언 | 262 |
| — 그중 **test 안에서만 쓰이는 장치** (함께 옮기면 됨) | 89 |
| — `AppSession` 제품 비공개 메서드 | 82 |
| — 최상위 비공개인데 제품 본문도 쓰는 것 | 82 |
| **실제 `pub` 순증** | **164** (347 → 511, +47%) |

그리고 **일관성 문제가 따로 있다**: `src/` 880 파일 중 **585 개(66%)가 인라인 test 를 품고**
있고 그 합이 339,058 줄이다. `app_session.zig` 는 그중 18% 다. 한 파일만 밖으로 빼면 저장소에서
유일하게 다른 파일이 된다.

> **Zig 에는 Rust 의 탈출구가 없다.** Rust 는 `#[path]` 하위 모듈로 **파일을 나누면서 비공개
> 접근을 유지**할 수 있어 같은 문제를 값싸게 푼다. Zig 의 privacy 는 파일 경계라 파일을 나누면
> `pub` 이 강제된다(필드는 예외 — 항상 공개다). 즉 test 잔류는 설계 취향이 아니라 **언어 제약**이다.

### 아직 안 해 본 방향 — 데이터를 쪼갠다

**F 시리즈는 함수를 옮겼지만 데이터는 평평하게 뒀다.** `AppSession` 은 지금도 **필드 757 개**를
한 덩어리로 들고 있고, 모든 그룹이 그 하나를 공유한다. 위의 「양방향 73 쌍」은 그 결과로 보인다 —
그룹이 서로를 부를 수 있는 이유는 전부 같은 `*AppSession` 을 받기 때문이다.

그런데 **그 757 개는 주제별로 뭉친다**(AST 실측):

```
 80  scm_      65  file_     55  agent_    47  ft_
 26  debug_    26  remote_   24  editor_   22  sidebar_
 17  pending_  17  dock_     16  git_      12  web_        …
상위 20 접두가 필드의 64% 를 덮는다 (접두 126 가지, 1 회뿐인 것 44 가지)
```

접두가 **이미 그룹 파일이 있는 바로 그 주제들**이다. 그러면 타입을 이렇게 바꿀 수 있다:

```zig
// 지금                          // 가능한 모양
AppSession = struct {            AppSession = struct {
    scm_a: …, scm_b: …,              scm: ScmState,
    file_x: …, file_y: …,            file_panel: FilePanelState,
    agent_p: …, …757개                agent: AgentState, …
};                               };
```

그룹 함수가 `*AppSession` 대신 `*ScmState` 를 받으면 **그룹이 다른 그룹의 함수에 닿을 길이
없어진다** — 양방향 쌍이 구조적으로 사라지고, `pub` 압력도 준다(Zig 에서 **필드는 항상 공개**라
상태 접근에는 `pub` 이 필요 없다). 격리 단위 test 도 가능해진다.

**비용은 재지 않았다.** 757 개를 옮기면 그 필드를 읽는 모든 자리(`self.scm_a` → `self.scm.a`)가
바뀌고, 그 수가 얼마인지·허브 셋(`tick`·`mouse`·`handleKeyEvent`)이 얼마나 흔들리는지 모른다.
착수한다면 §2-c-2 의 규율대로 **한 주제를 실제로 옮겨 컴파일러에게 묻는 것**이 먼저다
(사전 추정은 이 문서에서 세 번 빗나갔다).

#### `git_` 한 주제로 실제 비용을 쟀다 (2026-09-22, 되돌림)

규율대로 한 주제를 **실제로 옮겨** 컴파일러에게 물었다. 결과만 남기고 변경은 되돌렸다 —
방향을 정하기 전에 제품 478 줄을 바꿔 두면 되돌릴 이유가 생겼을 때 비용이 두 배가 된다.

**① 접두는 경계가 아니다.** `git_` 16 개를 참조로 갈라 보니 **두 덩어리**였다.

| | 필드 | 참조 | 쓰는 곳 |
|---|---|---|---|
| `git_ignore_*` | 4 | 69 | app_session · git · **file_tree_dock** — gitignore 판정은 트리 항목의 성질 |
| `git_repo`/status 계열 | 12 | 486 | scm_dock · editor · editor_merge 등 9 파일 — SCM 상태 |

선언 위치가 이미 그렇게 말하고 있었다 — `git_ignore_query_paths` 가 `file_tree_rows` 와
`file_tree_entry_inputs` **사이**에 있다. F1(F1+F3 합침)·F2(tree↔panel 순환)가 배운
**「이름이 아니라 호출 관계로 경계를 잡는다」가 필드에도 그대로 적용된다.**

**② 기계적 비용은 작다.** 응집된 12 개를 `GitState` 하위 struct 로 묶고 `self.git_x` → `self.git.x`
로 바꾸니 **478 줄 / 9 파일**이고 **컴파일 오류 0 · `pub` 순증 0** 이었다(Zig 에서 필드는 항상
공개라 하위 struct 접근에 `pub` 이 필요 없다). 필드당 40 줄 꼴이다.

**③ 그런데 컴파일러가 못 잡는 비용이 있다 — 소스를 문자열로 보는 판정자.**

```
quiesce axis: src/platform/macos/git_backend.zig 는 detached worker 를 띄우는데
              아무도 거두지 않는다.
```

`tests/boundary/detached_worker_quiesce_axis.zig` 는 「`git_backend.zig` 가 워커를 띄우면
`quietDetachedWorkersForTest` 본문에 **`git_backend` 라는 문자열이 있어야 한다**」로 판정한다.
거두는 동작은 그대로인데(`self.git.backend` 로 여전히 거둔다) 그 문자열이 사라져 빨개졌다.
**오탐이 아니라, 판정자가 필드 이름을 대리 지표로 쓰고 있었고 그 대리가 끊어진 것이다.**

이 저장소는 그런 판정자가 본체다. 757 필드를 다 옮기면 몇 개가 깨지는지 **모른다** —
`git_` 12 개에서 하나였으니 단순 외삽은 60 여 개지만, 이 문서에서 사전 추정은 세 번 빗나갔다.

**정리.** 데이터 분할은 `pub` 을 안 늘리고 양방향 의존을 구조적으로 끊는다는 점에서 유일하게
근본을 건드리는 방향이지만, **비용이 「기계적 치환 + 판정자 재작성」이고 후자를 못 센다.**
착수한다면 접두가 아니라 참조로 경계를 먼저 가르고, 주제 하나마다 깨지는 판정자를 세어
누적치를 기록해야 한다.
