...
| 호버 | 전폭 사각 | 선택과 같은 형상, 약한 배경 | 두 상태의 형상이 다르면 호버가 선택처럼 읽힌다. **밴드는 이 둘뿐이다** — 열린 파일에 밴드를 주면 그 색이 곧 호버 색이라 포인터 자리를 거짓으로 말한다(2026-08-25 실측·수정, `view.bandRole`) |
...
> 않는다.** 그 자리를 `list_row`로 옮기는 것은 **그 화면의 크기가 커지는 별개 결정**이다.
>
> **그 판단을 했다(2026-08-23, 사용자 요청).** 둘의 답이 갈렸다.
>
> - **소스 컨트롤 파일 행 → 옮겼다.** 그것은 진짜 "목록 행"이다. 이름은 `list_row`(14pt regular),
>   전경은 `list_secondary_fg`, 경로 꼬리는 한 단 아래(`list_disabled_fg`), 아이콘은 자기 라벨과 같은
>   단. 실측으로 탐색기와 같은 값이 됐다(아이콘·라벨 169 / 꼬리 130 / 섹션 제목 255).
> - **AI 세션 목록 → 옮기지 않았다.** 그것은 행 목록이 아니라 **카드 목록**이다(`card.title` =
>   `card_heading`, `card.summary` = `body`). 카드 제목에 `list_row`를 씌우면 성격이 다른 role을
>   강제하는 것이고, 그것은 위 ⑴이 막으려는 것과 같은 종류의 오류다.
...
**"언젠가 지운다"는 더 이상 목표가 아니다(2026-08-24 갱신).** 이 절을 쓸 때는 소비자가 Windows 스모크
하나여서 "그 스모크가 컴포넌트 경로로 가면 지운다"고 적었다. 그 사이 **Windows 제품이 이 투영으로 도크
트리를 그리기 시작했다**(W8.7a2 — `runWin32Terminal` → `rebuildDockAll` → `buildDockTreeFrame`). 즉 이
함수는 삭제 대기 중인 잔여물이 아니라 **한 플랫폼의 렌더 경로**다.

사라진다면 그것은 Windows가 `ChromeDraw` 낮추기 층을 갖고 컴포넌트 경로로 갈아탈 때이고, 그것은 이
계획의 후속이 아니라 **Windows 쪽 이니셔티브**다. 이 계획이 남기는 것은 그때까지의 방어뿐이다 —
경계 게이트가 정의 위치와 호출 수를 세고, 수가 바뀌면 사람이 보게 멈춰 세운다(실제로 W8.7a2가 그
게이트에서 멈춰 1 → 2로 고쳐졌다 — 설계대로 돌았다).
...
- ~~**Chrome Lab에 탐색기 scenario가 없다.**~~ **FT-LAB에서 해소됐다.** `file-tree-rows`·
  `file-tree-row-hover` 두 시나리오와 crop 여섯(선택 밴드·안내선·종류 아이콘·dirty 점·무시된 행·
  호버 대 선택)이 들어갔다. FT1이 추가하기로 했던 것을 FT1은 하지 않았고(그 사실을 이 줄이 계속
  "미해결"로 적고 있었다), 그동안 트리 픽셀에는 자동 그물이 없었다. **첫 캡처를 눈으로 본 자리에서
  스펙 위반이 하나 나왔다** — 호버 밴드가 선택 밴드보다 밝아(rgb 140 대 80) 무엇을 고른 상태인지
  화면에서 사라졌다(§3 은 "호버: 약한 배경"). 값 위계 테스트와 골든이 함께 그것을 잡는다.
- **Windows 의존**(FT3 박스). 셀 경로는 macOS만의 것이 아니다.
- ~~**도크 폭이 좁을 때의 열화 규칙**을 다시 정해야 한다.~~ **정했다(2026-08-25): "이름은 마지막까지
  남는다".** 좁아지면 **들여쓰기 → 상태 슬롯 → chevron** 순으로 버리고, 좌우 패딩과 종류 아이콘은 못
  버린다. 결정은 **폭만 본다**(깊이·dirty 로 갈리면 행마다 x 가 달라져 목록이 흔들린다) — `Metrics.rowLayout`
  하나가 그 값을 내고 밴드·선·chevron·아이콘·라벨·상태 점이 전부 그것을 되읽는다.

  **왜 하한을 올리지 않았는가**: 사이드바는 하한을 내용에서 파생하지만(신호등이라는 못 버리는 OS 요소가
  있다) 도크에는 그런 것이 없다. 게다가 AI 세션 도크는 이미 반대 관용구를 쓴다 — 헤더가 280pt 미만이면
  정렬 토글을 버려 제목에 자리를 준다. 즉 사다리는 **어차피 필요하고**, 하한을 올려도 그것을 없애지
  못한다(실측: 그 토글의 버리는 구간이 120~280pt 로 어떤 하한보다 넓다). 사다리가 있으면 지금 하한
  120pt 에서도 이름이 82pt(약 10자)를 받는다.
...
| FT-LAB — Chrome Lab 시나리오와 시각 골든 | **완료** — §6 의 "탐색기 scenario 없음"을 닫는다. 호버/선택 세기 뒤집힘을 그 자리에서 잡았다 |
