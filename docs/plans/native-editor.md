...
- **N1 — 읽기 전용 등폭 뷰.** 화면에 파일이 뜬다.
  ...
  **진행 상황(2026-08-10)**: 골격이 섰고 **계약의 절반 이하가 구현됐다.** "N1 완료"가 아니다.
  ...
  | 구현됨 | 남음 |
  |---|---|
  | `line_index`(byte↔논리행) · `geometry`(§4.1 영역 분할) · `gutter`(줄 번호) · `content`(본문·탭 전개) · `viewport`(스크롤·컬링) · **`hazard`(§3.8 가시화 — BiDi·제어·폭 0·비표준 공백)** · **`display_width`(§4.2 표시 폭 — 열 계산과 셀 배치의 단일 출처)** · **`visual_map`(§4 세로 축 — 랩 분할)** · **§3.8 초장문 줄 축소** | 접힘과 전체 접기/펼치기 · §3.8 깊은 중첩 상한 · 가로 스크롤 clamp·입력 · 줄별 폭 합(L2 캐시) · 스크롤바 · **랩 재계산 스레딩**(§2.1) · 상태바 커서 위치(`ItemId` 확장) · 선택 하이라이트 · **파일 읽기는 됐고 pane 표시가 남았다**(아래) |
  ...
  **캡처가 하나 붙었다(2026-08-12).** Chrome Lab의 `editor-real-file` 시나리오가 파일을 써서 `openPath`로 읽고 그 줄들을 그린다 — §3.5의 BOM·CRLF·탭 규칙은 *문자가 안 보이는 것*이 정답이라 문자열 비교로는 "제대로 뗐다"와 "못 읽었다"가 갈리지 않는다. 그 캡처가 **폰트 리가처로 `//`가 한 칸으로 합쳐지는 것**도 함께 드러냈다([#2123](https://github.com/ohah/maru/issues/2123) · visual-mapping §4.2 미해결) — 이 슬라이스가 만든 것이 아니라 렌더 경로의 성질이라 골든이 현재 상태를 잡아 두는 선에서 남긴다.

  **남은 것은 화면에 그리는 절반이다.** 제품의 chrome 텍스트가 measured 캐시 경로를 타므로(도크·사이드바가 그렇다) 편집기의 셀 격자 텍스트를 그 경로에 어떻게 얹을지가 다음 슬라이스의 첫 질문이다. 지금은 `MARU_NATIVE_EDITOR=<path>` 디버그 훅이 열어 보고 결과를 notice로 알린다 — 그 훅이 "열어서 그린다"로 바뀌면 N1의 "화면에 파일이 뜬다"가 닫힌다.
  ...
  **"화면에 파일이 뜬다"가 N1의 정의인데 그 마지막 조각이 표에서 "파일 입출력"으로만 보였다.** 지금 편집기를 부르는 곳은 Chrome Lab뿐이고 제품 pane에는 배선이 없다 — 그것까지가 N1이므로 표현을 맞춘다.
  ...
  **랩은 구현됐다(2026-08-11).** 그 과정에서 §4의 `ceil(폭/열수)` O(1) 주장이 **2칸 글자에서 틀린다**는 것이 실측으로 드러나 계약을 고쳤다(cluster를 쪼갤 수 없어 행 끝에 한 칸이 남고 누적된다). 남는 배선이 하나 있다:
  ...
  - **`editor.wrap` 설정은 파싱만 되고 렌더에 닿지 않는다.** 편집기가 제품 화면에 없어 연결할 곳이 없으므로 설정 GUI에서도 가렸다(`Meta.hidden`). **편집기를 제품에 배선하는 슬라이스에서** `hidden`을 벗기고 값을 `content.Props.wrap`에 잇고 `⌥Z` 토글 명령을 넣는다 — 그때 `config/schema.zig`의 "아직 UI에 뜨지 않는다" 테스트가 실패하므로 잊을 수 없다.
