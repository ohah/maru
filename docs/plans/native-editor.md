...
- **N1 — 읽기 전용 등폭 뷰.** 화면에 파일이 뜬다.
  ...
  **진행 상황(2026-08-14)**: **"화면에 파일이 뜬다"가 닫혔다** — `MARU_NATIVE_EDITOR=<경로>` 훅이 파일을 편집기 Term으로 열고
  ...
  | 구현됨 | 남음 |
  |---|---|
  | `line_index`(byte↔논리행) · `geometry`(§4.1 영역 분할) · `gutter`(줄 번호) · `content`(본문·탭 전개) · `viewport`(스크롤·컬링) · **`hazard`(§3.8 가시화 — BiDi·제어·폭 0·비표준 공백)** · **`display_width`(§4.2 표시 폭)** · **`visual_map`(§4 세로 축 — 랩 분할)** · **§3.8 초장문 줄 축소** · **`scrollbar`(§4.1a — 제품 화면에 뜬다)** · **파일 읽기(`open.zig`)와 제품 pane 표시(`appendPaneFrame`)** · **`editor.wrap` 배선과 랩 토글 명령** · **세로 스크롤 입력(휠 — 편집기 pane이 통째로 소유, 논리 줄 단위, 마지막 화면이 비지 않게 clamp)** · **가로 스크롤 입력(2026-08-16 — 본문이 **넘칠 때만** 그 축을 가져간다. 탭 바 가로 축이 편집기 pane 위에서도 살아 있어야 한다는 결정을 뒤집지 않는 유일한 규칙이다. 상한은 문서에서 **가장 긴 줄**이 정하고(보이는 줄만 보면 세로로 굴릴 때마다 상한이 출렁인다) 그 값은 캐시한다. 랩이 켜지면 넘칠 것이 없어 축 자체가 없다. **비교 뷰는 뺐다** — §3.5의 "가로는 각자다"를 지키려면 포인터가 어느 열 위인지 알아야 하고, `split_x`는 나오지만 소비할 히트테스트가 없다)** | 접힘과 전체 접기/펼치기(gutter 자리만 잡혀 있고 `content`가 폭 0으로 낸다) · §3.8 깊은 중첩 상한 · **비교 뷰의 가로 스크롤**(열 히트테스트가 선행)  · **조각 단위 좌우 정렬**(랩을 켠 비교가 어긋난다 — 실측 왼쪽 3행/오른쪽 13행, 같은 내용이 2행 대 12행. `editor.wrap` 기본이 `true`라 기본 상태다. [visual-mapping §4.1d](../native-editor-visual-mapping.md)) · **열↔byte 인덱스**(Zed·xi 계열 rope 집계 — 들어오면 `frame.max_first_col` 상한이 없어진다. 폭 합 캐시와 **같은 슬라이스**다, [visual-mapping §4.1c](../native-editor-visual-mapping.md)) · 줄별 폭 합(L2 캐시 — 접힘·선택 하이라이트와 함께 **바이트↔열 매핑 구조를 재평가**한다, ../native-editor-visual-mapping.md §4.1c) · **랩 재계산 스레딩**(§2.1) · 상태바 커서 위치(`ItemId` 확장) · 선택 하이라이트(`selection.zig`는 섰고 소비처 없음) |
