...
- **N1.5 — git diff 본문(읽기 전용, ../native-editor-ui.md §7).** 좌우 배치·세로 동기·추가/삭제 배경(`diffFromTheme`)·
  ...
  **c는 이렇게 섰다(2026-08-14).** 조합을 **컴포넌트 층**(`chrome/components/editor_view/diff_frame.zig`)이
  ...
  - **출하 판정을 고쳤다(2026-08-14 적대적 검증).** 초판 표가 c를 "출하 가능"이라 적었는데 사실이
    아니다. 둘이 남았다: ① **세로 스크롤 입력이 없다** — `editor_first_line`을 쓰는 입력 경로가
    아직 없어(N1 잔여) pane보다 긴 비교는 **첫 화면만 보이고**, 막대는 그려지지만 움직일 수 없다.
    ② **색이 없어** 추가·삭제가 위치로만 구분된다(d). 그래서 기본 경로 전환은 최소한 d와 세로
    스크롤이 붙은 뒤다 — 지금은 `MARU_NATIVE_DIFF=1` 훅으로만 열린다.
  - **이 한계는 문서 편집기(N1)와 같은 뿌리다.** 스크롤 입력이 붙으면 두 화면이 함께 풀린다.
