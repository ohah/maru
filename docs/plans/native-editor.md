...
- **N1 — 읽기 전용 등폭 뷰.** 화면에 파일이 뜬다.
  ...
  **캡처가 하나 붙었다(2026-08-12).** Chrome Lab의 `editor-real-file` 시나리오가 파일을 써서 `openPath`로 읽고 그 줄들을 그린다 — §3.5의 BOM·CRLF·탭 규칙은 *문자가 안 보이는 것*이 정답이라 문자열 비교로는 "제대로 뗐다"와 "못 읽었다"가 갈리지 않는다. 그 캡처가 **폰트 리가처로 `//`가 한 칸으로 합쳐지는 것**도 함께 드러냈다(visual-mapping §4.2 미해결) — 이 슬라이스가 만든 것이 아니라 렌더 경로의 성질이라 골든이 현재 상태를 잡아 두는 선에서 남긴다.
