...
- **N1.5 — git diff 본문(읽기 전용, ../native-editor-ui.md §7).** 좌우 배치·세로 동기·추가/삭제 배경(`diffFromTheme`)·
  ...
  **남은 결정 하나**: diff 색을 chrome `ColorRole`로 올릴지(`terminal_bg`에서 한 방식). 올리면 제품과 Chrome Lab이 같은 값을 읽어 캡처가 제품을 예고한다 — 안 올리면 이번 슬라이스에서 겪은 "Lab이 색 역할을 구분 못 함"이 재발한다. **d 슬라이스에서 실측하며 정한다.**
