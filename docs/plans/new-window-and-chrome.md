...
- **(W5) glyph atlas 공유 + #5~7 kitty 이미지 캐시 정합 — 설계 분해(2026-06 조사·측정, 구현 보류)**:
  ...
  - **#5~7 kitty 이미지 캐시(별개 축, 멀티 pane 이미지 렌더 전제)**: `image_id`가 surface-local opaque counter라 두 surface의 `image_id=1`이 다른 픽셀 — 공유 키가 **틀림**(atlas의 content-derived 키와 반대). fix는 ref-count가 아니라 **namespacing**: `(surface_id, image_id)` 복합 키를 4곳(`kitty_uploaded`·`planImageUploads`/`buildGpuImages`·Swift `imageTextures`·evict live-set)에 적용. ~~현재는 **활성 surface만 이미지 렌더**(`app_session` active-only)라 충돌이 잠재 — 비활성 pane 이미지 렌더(별도 기능)가 충돌을 노출하므로 그 기능과 함께 구현한다.~~ **구현(2026-09-23, 비활성 pane 렌더와 함께)**: 원칙(namespacing)은 그대로, **메커니즘은 바꿨다** — 4 곳의 키 타입을 복합 키로 넓히지 않고, 입구에서 `(surface, 로컬 id)` 를 겹치지 않는 u32 로 번역한다(`renderer/kitty_image_ids.zig`). 렌더러 ABI 무변경, 원래 id 우선이라 단일 pane 동작 불변. 상세·함정·판정자는 [terminal-input-and-protocols.md](terminal-input-and-protocols.md) K2e.
  - **순서·전제**: W5(atlas)와 #5~7(이미지)은 disjoint 캐시·다른 키 원리(content-shared vs identity-namespaced)라 독립. 공통 토대는 "per-surface id"(#5~7이 도입, W5b가 ref-count에 개념 재사용). W5는 프로파일/WebGPU 게이트, #5~7은 멀티 pane 렌더 게이트 — W5 는 여전히 미트리거(단일 윈도우)라 설계만 고정하고, #5~7 은 K2e 로 구현됐다.
