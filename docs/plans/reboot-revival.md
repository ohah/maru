...
## 머지 뒤 코드 리뷰 반영 (2026-09-24)

머지 뒤 적대적 검증과 `/code-review max` 에서 확인된 결함들. 계약 문서(「재부팅 뒤 부활(RB)」)를 먼저 고쳤다.

| 결함 | 처리 |
|---|---|
| in-process 칸은 에이전트가 적혀 있어도 부활 경로를 안 탔다(계약의 「한 규칙」 위반, 판정자가 틀린 동작을 고정) | #3902 로 머지 |
| 복원이 불완전하면 저장이 막혀 같은 부팅의 다음 실행이 또 부활 | 「이번 부팅에 부활함」 표식 + 판정 프로세스당 한 번 |
| claude 폴더 이름(slug) 규칙이 claude 와 다름(영숫자 아닌 문자 전부·200자 해시) | 세션 id 로 `projects/*` 를 훑는다. 같은 흉내를 쓰던 사이드바 대화 줄도 같은 찾기로 옮겼다(`SB-1`, [사이드바 에이전트 목록](../sidebar-agent-list.md) §7.3) |
| 신원이 에이전트보다 오래 산다 · 거둬들인 상태줄 훅이 남긴 옛 파일 | 종류가 끝나거나 바뀌면 신원을 버린다 · 이 실행 뒤에 쓰인 파일만 |
| 재접속·부활 직후 checkpoint 가 파일이 알던 `agent-resume` 을 지운다 | 보류 신원 |
| `CODEX_HOME` 무시 | 훅 설치와 같은 `configDir` |
| 원격 훅 종류 래치가 ssh 를 빠져나와도 안 풀림 | 채널을 놓을 때 함께 푼다 |
| 부활 칸 spawn 실패 하나가 창 전체를 잃는다 | 그 칸만 묘비 |
| 끝 1 MiB 밖으로 밀린 모드 → 기록보다 넓은 기본 권한 | 1→4→16 MiB 로 넓히고 값 줄만 연다 |
| 알림이 창마다 뜨고 host 실패 알림을 덮는다 | checkpoint 무장 뒤 한 번, host 실패면 생략 |
| tcsh·csh 에서 `-l -i -c` 가 죽는다 | 받는 셸에서만 이어간다 |
| codex cwd 무시 · cwd 가 사라지면 「대화 없음」으로 셈 · 읽기 권한까지 요구 | 대화 기록 cwd → 저장 cwd → 기본, `access(X_OK)` |
| codex 파일 이름 느슨한 일치 | id 앞 구분자 검사 |

판정자 RB1-9~11, RB2-12~17(+RB2-8·RB2-11·RB1-4·RB2-9 강화, 원격 래치 판정자 단언), 변이는 PR 본문.
