...
##### 7a 와 §1.1 권위표가 맞물리는 자리

합성한 훅 상태가 §1.1 권위표의 `hook` 입력이 된다. **`hook_child_count` 는 합치지 않는다** — pane A 의
자식 둘과 pane B 의 자식 없음을 더해 「자식 2」로 넘기면 D1(자식이 있으면 running)이 pane 사이에서 서게
되는데, 그 규칙의 근거(「자식이 도는 동안 lead 의 `Stop` 은 턴 끝이 아니다」)는 **한 pane 안에서만**
성립한다. 자식 셈은 pane 별 `advance` 안에서 이미 그 pane 의 상태에 반영되므로, 접은 값에는 다시 실을
필요가 없다.

원격 Term 은 화면 축이 없어(§1.1 의 원격 주) 권위표에서 **B 만 탄다** — 즉 합성값이 그대로 배지가 된다.
`hook_child_count` 를 0 으로 넘겨도 결과가 같은 이유다.
...
⚠️ **상수를 빌리면 두 쓰임이 한 값에 묶인다.** `turn_open_warn_ms` 를 그대로 쓰면 「진단에 한 줄 남기는
기준」과 「합성에서 빼는 기준」이 같이 움직인다 — 나중에 한쪽만 바꾸고 싶을 때 그 사실이 드러나지 않으면
다른 쪽이 조용히 따라 바뀐다. 그래서 **이름을 따로 두고 값만 그것으로 시작한다**
(`remote_pane_running_max_age_ms = turn_open_warn_ms`). 갈라야 할 때 갈 자리가 이미 있게 한다.
...
**차이는 「원격에 무엇을 띄우나」가 아니라 「누가 pane 을 소유하나」다**(2026-09-03 정정 — 처음에 「우리는
원격에 terminfo 와 훅만 심는다」고 적었는데 **틀렸다**). 우리도 원격에서 `maru` 를 돌린다:
`command -v maru` 로 존재를 확인하고, 훅 설치도 스트리밍(`maru agent-events --stdio`)도 **원격 maru 가**
한다. 그 점에서는 herdr 와 같다.

갈리는 자리는 **PTY 소유권**이다.

    herdr   원격 herdr 가 PTY 와 pane 을 **소유**한다 (thin client 가 붙는다)
    maru    원격 maru 는 **관측만** 한다 — PTY 는 사용자의 `ssh` 와 그 안의 tmux 것이다

`maru ssh` 가 *「사용자의 평소 `ssh` 는 건드리지 않는다」* 를 계약으로 삼기 때문이고(§1), 그래서 사용자가
쓰는 tmux 가 그대로 축에 들어와 RA7 이 그것을 다뤄야 한다.

**herdr 방식이 대안으로 존재한다**: 원격 `maru` 가 PTY 까지 소유하면 pane 관리가 우리 것이 되어 이 문제가
사라진다. 택하지 않는 이유는 **사용자의 tmux 워크플로를 버리게** 하기 때문이고, 그 대가가 이 기능이 주는
값보다 크다. 다만 「우리가 어려운 길을 골랐다」는 사실은 알고 있어야 다음 사람이 같은 저울질을 다시 하지
않는다.

⚠️ **herdr 도 tmux 를 다루긴 한다 — 축이 다를 뿐이다.** `wrap_tmux_passthrough`(알림을 `\x1bPtmux;` 로
감싼다) · OSC 파서의 passthrough 처리 · `detect` 의 `"tmux" => None`(tmux 를 에이전트로 분류하지 않는다).
전부 **herdr 자신이 tmux 안에서 돌 때**의 것이지, 「원격 tmux 안의 에이전트 여럿을 가른다」는 아니다.

#### RA7.6 ⚠️ PoC 실측이 RA7 의 전제를 반증했다 (2026-09-03)

**구현 전에 재 봤고, 전제가 틀렸다.** 실제 스풀을 복사해 원격 CLI 를 돌리고 tmux 를 직접 물었다.

| 확인한 것 | 결과 |
| --- | --- |
| 한 tmux 세션에 pane 여럿인가 | ❌ **아니다.** 세션이 **9개**이고 각각 클라이언트가 **1개씩**이다(중복 0) |
| 그래서 nonce 가 접히는가 | ❌ **안 접힌다.** wire 의 nonce 가 `host_<같은 instance>_<서로 다른 pane>` 로 **이미 갈려 나온다** |
| detached 세션이 원인인가 | ❌ 하나 있지만(`zts-codex`) 도는 것이 `node` 이고 에이전트가 아니다 — 스풀 파일도 없다 |

즉 **「역조회가 pane 을 접어 하나만 남는다」는 RA7 의 출발점이 사실이 아니었다.** 사용자가 「한 세션에
pane 여럿」이라고 인식한 것과 실제 구성이 달랐고, 나는 그 말을 **코드로 확인하지 않고** 설계를 세웠다.

**진짜 원인은 채널이다.** 같은 회차에 건 감시가 잡았다 —

    05:42:52  streamer=50787  sshd_sessions=9    스트리머 뜸
    06:00:49  streamer=none   sshd_sessions=9    18분 뒤 죽음 — **sshd 는 그대로 9**
    06:01:15  streamer=11697  sshd_sessions=9    재시도가 살림

**SSH 연결은 멀쩡한데 스트리머 프로세스만 죽는다.** 재시도가 살리지만 `retry_max`(6)를 넘기면 굳고,
그때부터 그 목적지는 **앱 수명 동안 안 돌아온다**(RA5-c) — 화면에는 「배지가 안 뜬다」로만 보인다.
사용자가 말한 「하나만 감지」는 여러 Term 중 일부가 그렇게 굳은 모양으로 읽는 것이 맞다.

**그래서 RA7 은 보류한다.** 이미 머지된 wire 의 `pane` 필드는 해가 없고(없으면 예전과 같이 동작한다)
뒷날 「한 세션에 pane 여럿」이 실제로 나오면 그때 쓸 수 있다. **7a 이후는 짓지 않는다** — 없는 문제를
위해 자료구조·귀속·합성·판정자를 만들 뻔했다.

**교훈**: 사용자가 말한 구성을 **코드가 아니라 그 기계에 물어** 확인했어야 했다. RA7.1~RA7.5 는 전부 그
확인 없이 쌓은 것이고, PoC 하나가 그 전부를 뒤집었다.
