...
| 4 | ~~**v1 은 닫힌 pane 을 접지 않는다**~~ → **뒤집었다**(2026-09-24, 아래 §RA7.5) | 원래 근거는 *「스풀 파일이 있으면 행이 있고, 7일 회수(`isStale`)가 지우면 사라진다 — 대가는 죽은 pane 행이 최대 7일」* 이었다. **그 대가 계산이 틀렸다**: 행을 만드는 것은 스풀 파일이 아니라 **로컬 테이블 항목**이고, `isStale` 은 **원격 스트리머가 파일을 건너뛰는** 규칙이라 로컬 테이블을 지우지 않는다. 실제 대가는 「최대 7일」이 아니라 **「앱이 사는 동안 무한」**. 「시간 규칙을 넣지 않는다」는 판단 자체는 지금도 옳다 — 그래서 시간이 아니라 **원격이 말해 주는 사실**로 접는다 |
...
- ~~사용자가 pane 을 닫은 뒤 목록에서 사라지기까지의 시간(위 4번의 접는 규칙과 같은 문제).~~ → **답이 생겼다**(§RA7.5): 그 호스트에 **다음 훅 이벤트가 올 때**다. 하한은 역조회 TTL(`route_ttl_ms` 5초)이고, 상한은 「그 기계의 어떤 pane 이든 다음에 말할 때까지」다.

#### RA7.5 죽은 pane 을 지운다 — 살아 있는 목록을 역조회에 얹는다 (2026-09-24)

**증상**: 사이드바에서 원격 세션 하나만 pane 하위 목록이 서 있었다. 실측은 달랐다.

```
$ ssh <host> tmux list-panes -a -F '#{session_name}|#{pane_id}'
allthatnba|%7        ← pane 은 하나뿐
현재 pane id 전부:  %0 %1 %2 %3 %4 %5 %6 %7 %8     ← %20 은 없다
tmux 서버 기동:     2026-09-24 09:52:25
maru 앱 기동:       2026-09-23 15:09:40            ← 앱이 18 시간 먼저 떴다
```

`%20` 은 **tmux 서버 재시작 전 세대**의 pane 이다. tmux id 는 서버가 사는 동안만 단조 증가하고 재시작하면
`%0` 으로 리셋되는데, 로컬 테이블은 그 리셋을 모른다.

**잔재 하나가 «없을 구조» 를 만든다.** pane 행은 슬롯이 **둘 이상**일 때만 선다(`countFor >= 2`). 살아 있는
pane 이 `%7` 하나뿐이면 pane 행은 **아예 없어야** 하는데, 죽은 `%20` 이 count 를 2 로 만들어 하위 목록
전체를 세웠다. 잔재는 행 하나를 더하는 게 아니라 **구조를 만든다.**

**그리고 화면만의 문제가 아니다.** `hookSlotsAggregate` 가 pane 슬롯을 `strongerState` 로 합쳐
`arbitrateAgentState`(Term 상태)·`ScanSkip`(화면 스캔 생략)·`child_count`(턴 종료)에 먹인다. 죽은 pane 이
`running`/`blocked` 로 굳으면 **배지가 영영 안 풀리고 턴이 안 끝난 것으로 읽힌다.** 이번엔 `%20` 이 `idle`
이라 조용했을 뿐이다 — 그래서 「나이로 행만 감추기」 같은 표시층 처방은 **고치는 척만 한다.**

**처방**: 원격이 **지금 있는 pane 목록**을 알리고, 로컬이 그 목록에 없는 슬롯을 버린다.

- wire: `{"panes":"%0 %7 %8"}` 한 줄. **버전은 안 올린다** — 소비자가 `hello` 의 `v` 를 애초에 안 읽고
  (접두만 본다), 안전은 「프레임이 올 때만 지운다」에서 나온다. 결정 3 과 같은 규율이다.
- **새 폴링을 만들지 않는다.** 하트비트 주기로 묻는 안은 기각했다 — 역조회는 **로그가 자랐을 때만** 돌아
  (`main.zig` 의 «새 바이트 없으면 건너뛴다») 조용한 호스트는 지금 tmux 프로세스를 **하나도 안 띄우는데**,
  주기 조회는 그 0 을 깬다. 대신 그 역조회 스크립트에 `@maru-panes` 구분자와 `list-panes` 를 붙여
  **이미 뜬 `/bin/sh` 안에서** 묻는다 — 스트리머의 fork 가 늘지 않는다.
- **잔재의 협조가 필요 없다**: `%20` 은 죽어서 조용하지만 `%7` 이 살아서 이벤트를 내므로, `%7` 의 조회가
  목록을 실어 와 `%20` 을 지운다.
- **fail-safe — 침묵은 「모른다」이지 「없다」가 아니다**: tmux 가 없거나(`!maru-no-tmux`), 조회가
  실패·시한 초과이거나, 구획이 비었으면 **프레임을 아예 안 보낸다**. 소비자도 빈 목록은 무시한다.
  한 번의 잘못된 «없음» 이 살아 있는 행을 **전부** 지우는 것이 이 기능의 유일한 파괴적 실패다.
- **밀림으로 세지 않는다**: 「자리가 모자라 밀렸다」(조각 5 의 «+N pane 밀림»)와 「원격에서 없어졌다」는
  다른 사실이다. 섞으면 사용자가 「상한을 올리면 보이겠지」로 읽는데, 없어진 pane 은 상한과 무관하다.

**한계**: 그 호스트의 **모든** pane 이 조용하면 갱신이 없다. 그때는 새 정보가 실제로 없는 것이다.
즉시성이 필요하면 `tmux set-hook -g pane-exited` 로 이벤트 구동도 되지만, 사용자의 **전역 tmux 훅**을
건드리고 정리·충돌이 생겨 이 단계에서는 안 한다.
