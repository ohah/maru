...
#### RA5 후속의 사각지대 — **EOF 가 안 오는 사망**(2026-09-07 실측)

위 재접속은 사망 신호를 **EOF 하나로** 잡았다. 그런데 EOF 가 안 오는 사망이 있다.

    로컬  ssh 브리지        살아 있음 5h11m, read() 는 EAGAIN 만 낸다
    원격  maru agent-events  없음 — ControlMaster(pid 39815) 가 교체되며 죽었다
    원격  t15.ndjson         계속 갱신 — 이벤트는 쌓이는데 나르는 쪽만 죽었다
    로컬  사이드바           마지막 수신 값에 고정

브리지는 `-S` **mux 클라이언트**다. ControlMaster 가 교체되면 원격 명령은 죽지만 클라이언트는 EOF 를
내지 않고 `read` 가 계속 `EAGAIN` 을 낸다. 그래서 `drainRemoteAgentHost` 의 `eof` 갈래에 **영영 못
들어가고**, 재생성이 그 갈래에만 있었으므로 죽은 전송을 붙든 채 굳는다.

`Channel` 의 침묵 시한(15 초)은 제대로 돌았다 — 채널은 닫혔다. 문제는 그 닫힘이 **전송 재생성으로
이어지지 않은 것**이다. 감지와 복구가 다른 갈래에 묶여 있었다.

**고침**: 침묵을 EOF 와 같은 길로 보낸다. host 가 `last_rx_ms` 를 들고(스트리머 기동 때 켜고 줄이 올
때마다 되돌린다 — `retries` 를 되돌리는 그 자리다), 시한을 넘기면 자식을 거두고 같은
`scheduleStreamerRetry` 로 예약한다. 시한은 `Channel` 과 같은 상수를 쓴다.

**`ServerAliveInterval` 은 안 붙였다.** keepalive 는 mux master 의 일이라 클라이언트에 붙여도 이 사망을
잡는다는 근거를 못 만들었다. 침묵 판정은 사망 방식을 **열거하지 않으므로** 그 근거 없이도 옳다 —
다음에 또 다른 방식으로 죽어도 같은 길로 복구된다.
