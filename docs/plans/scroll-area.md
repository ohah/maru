...
2. **selection follow(§4.5) — 흡수하지 않기로 했다(결정 2026-08-30).** 아래는 그 근거다.

   **값 비교는 게으름이 아니라 두 번 실패한 뒤의 답이다.** SV5d가 기록한 대로 ⑴ `.selection_changed`
   액션으로 잡으려다 실패했고(선택은 쿼리 필터·섹션 전환 등 여러 경로에서 바뀌어, host가 그 목록을
   열거하면 새 경로가 생길 때 조용히 빠진다) ⑵ "setter로 모으면 된다"는 답도 검증에서 무너졌다.

   **그런데 SV5d 자신이 "장기 답은 아니다"라고 적었다** — 소비처마다 파생 필드가 하나씩 늘고, 「무엇이
   바뀌었나」를 값으로 재구성하는 것은 이 저장소가 반복해 밟은 함정과 같은 계열이라는 이유다.

   **그래서 «ScrollArea로 옮기기»는 답이 아니다.** 옮겨도 그 형태는 그대로이고, 오히려 **비판받은 방식이
   계약으로 굳는다**. `settings.ensureSelectedVisible`이 이미 컴포넌트 안에서 같은 값 비교를 하고 있는데,
   그것은 "검증된 선례"가 아니라 **같은 타협이 두 곳에 퍼졌다는 신호**로 읽어야 한다.

   **결정: 지금 형태를 유지한다.** 동작은 맞고, 옮기는 이득(파생 필드가 한 곳으로)이 잘못된 형태를
   계약으로 굳히는 대가보다 작다. **근본 해법(선택 변경을 사실로 다루는 구조)은 별도 설계 슬라이스**이고,
   그때까지 이 항목은 「알면서 안 하는 것」이다 — 다음 사람이 계약 문구만 보고 착수했다가 SV5d가 무너진
   자리를 다시 밟지 않게 여기 적어 둔다.
...
3. ~~**fade 축이 선언에 없다(§4.1·§7).**~~ → **닫혔다(2026-08-30). 계약안(paint 시점)으로 구현했다.**

   최종 형태는 `paint.paintWithAlphaOverrides`(노드 id로 alpha를 덮어쓰는 축) + 도크 컴포넌트의 **`view`가**
   스크롤바 두 id에만 host의 `dockScrollAreaAlpha`를 얹는 것이다. **`build`는 alpha를 모르므로 tree가
   fade 내내 불변**이고, 그 불변을 판정자가 못 박는다(alpha만 다른 두 build가 같은 entry·action을 내는지 —
   A안 방식으로 되돌리는 변이를 걸면 정확히 빨개진다). fade 상태는 도크 공용 하나(`dock_scroll_area_idle_ticks`)이고,
   탐색기 뷰는 이 갈래가 세지 않으며(host가 자기 타이머를 든다), **잡고 있는 동안은 full로 핀한다**.

   문서에 적어 둔 계약안의 비용 둘 중 **하나는 없었다** — "paint가 노드를 식별 못 한다"고 봤지만 `paint`는
   순회하며 이미 `entry.id`를 안다. 남은 하나(공용 함수 인자)는 래퍼로 흡수했다.

   <details><summary>되돌린 A안과 그 근거(보존)</summary>

   **fade 축이 선언에 없다 — 「전부 흐려지게」로 결정났고, A안 구현은 되돌렸다.**

   **결정**: 스크롤바는 전부 흐려진다(사용자 결정 — SV2가 SV2b로 미뤘다가 기록 없이 흘러간 그 결정이다).

   **현황 서술부터 틀렸었다.** 이 항목은 "도크 스크롤바만 fade가 없다"고 적었지만, host가 직접 그리는
   셋(터미널 pane·탐색기 목록·사이드바)은 **이미 다 흐려지고 있다**(`pane.scrollbar_idle_ticks` ·
   `dock_list_scrollbar_idle_ticks` · `sidebar_scrollbar_idle_ticks`). 안 흐려지는 것은 **ScrollArea
   컴포넌트가 그리는 쪽**(에이전트 세션 도크·소스 컨트롤 도크)이고, 그 선언은 색을 역할 토큰으로만 적어
   alpha 자리가 없다. 이미지 갤러리는 뷰는 생겼지만 `tree.scrollArea` 를 안 써 대상이 아니다.

   **한 번 만들었다가 되돌렸다 — 계약이 그 방식을 이름 붙여 금지하고 있었다.** props 에
   `scrollbar_alpha` 를 더해 host 가 계산한 값을 `PaintStyle.opacity` 로 싣는 판(A안)을 구현하고
   판정자·변이까지 통과시켰는데, [ScrollArea](../scroll-area.md) §7 이 정확히 그것을 막는다:

   > **fade alpha는 published tree에 넣지 않는다.** … tree가 프레임마다 달라져 frame 동등 비교가 매번
   > 실패한다. … 정확성 문제가 아니라 churn이며, **carry 조건이 나중에 조금이라도 좁아지면 곧바로
   > 정확성 문제가 되는 자리**다. alpha는 tree 값이 아니라 **paint 시점에 얹는 값**으로 둔다.

   **측정으로 그 금지가 옳다는 것을 확인했다(2026-08-30).** 「churn 이 실제로 문제인가」를 재려고 코드를
   따라갔더니 근거가 나왔다 — `publishAgentSessionDockFrame` 이 `agentSessionDockFrameEql` 로 entry·action 을
   **`std.meta.eql` 로 통째 비교**해 같으면 그 자리에서 return 한다. 그것이 이 경로의 **유일한 절약
   장치**인데 `RectEntry` 에 `visual.paint` 가 들어 있어 **alpha 가 그 비교에 든다**. 즉 fade 구간(≈0.45s,
   60Hz 면 ~27프레임) 내내 early return 이 무산되고 capacity 확보·배열 복사·carry 판정·`reconcile` 이
   매 프레임 다시 돈다. 게다가 **그 드래그 carry 판정이 바로 같은 함수 안에 있다** — 계약이 경고한
   「좁아지면 정확성 문제」가 추상적 우려가 아니라 그 자리에 붙어 있다.

   그래서 **계약을 갱신하고 A안을 남기는 길은 닫혔다.** 구현은 통째로 되돌렸다(문서만 남는다).

   **다음 슬라이스가 여기서 시작한다 — 계약안(paint 시점)의 실제 비용.** 지금 그 자리가 없다:
   `paint.zig` 가 하는 일은 `style.opacity` 를 `Quad.alpha` 로 옮기는 것뿐이고 **host 가 그 시점에 값을
   주입하는 경로가 없다**. 만들려면 ⑴ `paint` 는 순수 함수라 인자를 늘려야 하는데 그 층은 **모든 컴포넌트
   공용**이고 ⑵ paint 는 노드가 스크롤바인지 몰라 **식별 개념을 새로 넣어야** 한다. 이 둘이 이 항목의
   진짜 비용이고, A안이 싸 보였던 이유이자 함정이었다.

   </details>

   **fade 상태 자체는 도크 공용 하나면 된다**(되돌린 판에서 확인). 도크는 하나이고 뷰만 갈아 끼우므로
   뷰가 바뀌면 offset 이 달라져 자연히 리셋된다. 탐색기 뷰는 그 갈래가 세면 안 된다 — host 가 직접 그리고
   자기 타이머를 들어 한 스크롤이 두 타이머를 흔든다. 그리고 **잡고 있는 동안은 full 로 핀해야 한다**
   (적대적 검증 1회차가 잡았다 — offset 이 안 변하니 「굴리면 리셋」 갈래가 그 상황을 못 본다).
