...
4. **키보드 스크롤이 도크에만 있다(§4.5).** `scroll_area.pageStepPx`의 소비처는 Session Dock 하나다.
   §4.5는 "이관하면 탐색기·소스 컨트롤·사이드바가 함께 얻는다 — 동작이 늘어나는 변경이므로 각 이관
   단계에서 그 사실을 적는다"고 했는데, SV2·SV3·SV4의 "보이는 변화"에 PageUp/PageDown/Home/End가 없다.
   즉 이관은 했고 그 부수 효과만 안 따라왔다.
