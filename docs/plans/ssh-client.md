...
| S9 | **모바일 배선** — host 소켓 ABI(브리지엔 OS 호출 0), 키 저장은 Keychain/Keystore. **S9-1**: 세션 상태기계(`client.zig`)를 도구에서 승격 — 검증 도구와 모바일이 같은 것을 쓴다. **S9-2**: C ABI + 브리지(`maru_mobile_ssh_*` — 핸들·세대·양방향 배압, 난수는 host 가 준다). **S9-3**: 안드로이드 포그라운드 서비스(소켓 소유)·iOS host. **거기서 정할 것 둘**: (1) 붙었을 때
