...
| S7d | **재키잉** — 서버가 다시 보낸 `KEXINIT` 에 답하고 키를 간다. `session_id` 는 **초기 `H`** 로 고정하고 strict 표시자는 초기값을 잇는다([계약 §3](../ssh-client.md)). **안 하면 한 시간 넘는 세션이 멈춘다**(실측: `RekeyLimit 1M` 서버에서 917,504바이트 뒤 교착) | `RekeyLimit` 을 낮춘 실서버로 왕복 |
