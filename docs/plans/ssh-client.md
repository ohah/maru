...
| S9 | **모바일 배선** — host 소켓 ABI(브리지엔 OS 호출 0), 키 저장은 Keychain/Keystore. **S9-1**: 세션 상태기계(`client.zig`)를 도구에서 승격 — 검증 도구와 모바일이 같은 것을 쓴다. **S9-2**: C ABI + 브리지. **S9-3**: 안드로이드 포그라운드 서비스(소켓 소유)·iOS host | S9-1 은 실서버 6 회차, 나머지는 기기 |
