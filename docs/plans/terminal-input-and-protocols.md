...
- **매체 전송**(`t=f`/`t=t`/`t=s` + `S`/`O` 키) — 아래 절에서 근거와 함께 보류 중이다.
- `N`(사용 힌트, `N=1` = transient) 미파싱 — 힌트라 무시가 합법이고, 섞어 보내도 명령은 `OK` 다.

## kitty graphics 전송 매체 백로그 — `t=f`/`t=t`/`t=s` (보류, 근거 실측 2026-09-14)

현황: `t=d`(inline base64)만 받고 나머지 셋은 **명시 ENOTSUPP** 다. 앱은 그 응답을 보고 곧바로 inline
...
`TerminalCore` 는 파일을 열지 않는다(경로 **문자열** 조작만 한다). 부수효과는 클립보드처럼 「코어가
pending 을 기록하고 플랫폼이 나중에 처리」로 빼 왔다. 그런데 매체 전송은 그 자리에서 바이트를 읽어야
`OK`/`ENOENT` 를 답할 수 있다.

- **(A) 동기 reader 이음매** — 코어에 nullable 콜백(`ctx` + `read fn`)을 두고 host 가 주입한다.
  wasm(`platform/wasm/wasm_bridge.zig` 가 같은 코어를 브라우저로 내보낸다)·헤드리스 판정자는 null 이라
  지금처럼 ENOTSUPP. 순서가 정확하고 구현이 작다. 대가는 **코어에 I/O 능력이 생기는 첫 선례**다.
  (검증: 「코어 순수성」을 강제하는 판정자는 **없다** — 관례이지 잠긴 계약이 아니다.)
- **(B) 비동기 2단계** — 코어가 요청만 기록하고 플랫폼이 읽어 되먹인다. 순수성은 지키지만 상태 기계가
  커진다. 「`a=T,t=f` 직후 `a=p` 가 오면 ENOENT」 같은 순서 위험을 우려했는데 **근거가 없다** — 실제
  앱이 보내는 바이트를 확인해야 하고 이 기계엔 kitty/icat/chafa 가 없다(추측이었음을 명시해 둔다).
...
### 현재 결정(사용자 합의, 2026-09-14)

**C(현행 유지) — 보류.** 최적화이고 폴백이 동작하므로 급하지 않다. 진행하게 되면 **(A) 동기 이음매 +
`S`/`O` 파싱 + `t=t` 삭제는 임시 경로로만 + 크기 한도(기존 이미지 총량 한계에 합산)** 로 가고, 소유자
검사는 넣지 않는다(위 근거). 판정자는 삭제 경로 제한·크기 한도·`S`/`O` 경계(파일보다 큰 `S`, 파일 끝을
넘는 `O`)를 겨눈다.
