...
- **미구현 프로토콜 audit을 진행했다(focus/mouse/synchronized output/kitty keyboard/kitty graphics)**: Ghostty 레퍼런스와 대조해 빠진 VT 프로토콜을 순서대로 메웠다(전부 머지·`verification-matrix.md` 반영). 각 프로토콜은 progressive enhancement(앱이 DECSET/CSI로 켤 때만 동작, legacy 공존)이고, 거짓 지원을 피하려 "지원 응답 + 실제 동작"을 한 PR로 묶었다.
  ...
  - **OSC 66(kitty 텍스트 사이징) / OSC 5522(kitty 클립보드) — 로드맵(미착수)**: maru가 목표하는 kitty는 **graphics(APC)·keyboard(CSI u)**이고 이 둘은 완료다 — 66/5522는 별개의 kitty 확장이다. **접두 충돌 없음**을 확인(`"5522;"`는 `"52;"`로 오배치 안 됨).

    **이 줄은 「Ghostty 패리티 후속 — Ghostty는 이 둘도 지원(동적 버퍼 사용)」이라고 적고 있었다. 틀렸다**(2026-09-24 명세·업스트림 확인). 둘 다 **아무도 안 끝냈다**:

    | | 업스트림 실제(2026-09-24 확인) |
    |---|---|
    | OSC 66 | Ghostty 1.3.0(2026-03)이 **파싱만** 한다(파서는 PR #10315). 렌더가 없어 ucs-detect 지원 표시가 `no` 이고 [ghostty#10333](https://github.com/ghostty-org/ghostty/issues/10333)이 열려 있다 |
    | OSC 5522 | [ghostty#10549](https://github.com/ghostty-org/ghostty/issues/10549) **열림**(2026-02-03 생성, PR #14061 진행 중) — 릴리스에 안 들어갔다 |

    그래서 **「패리티」는 착수 명분이 못 된다** — 뒤처져서 따라가는 일이 아니라 아무도 안 한 것을 먼저 하는 일이다. 명분은 기능 자체의 값으로 서야 한다.

    **「핸들러 없는 OSC 버퍼를 키우면 megabyte를 받아 버릴 뿐」도 이 둘엔 안 맞는다.** 명세가 **한 시퀀스당 4096바이트**로 스스로 자른다 — 66은 텍스트가, 5522는 base64 **전** 청크가 그 상한이다. 큰 데이터는 청크 **재조립**이라 `osc_buffer` 가 아니라 별개 구조가 받을 문제다. 지금 `max_osc_small_bytes` 가 2048이므로 **66은 그 소형 상한만 4096+헤더로 올리면 되고, 52급 동적 버퍼가 필요 없다**(그래서 `oscMayGrow` 대용량 허용 목록에 안 넣는 것은 지금도 맞지만, 이유는 「megabyte 방어」가 아니라 「그 크기가 필요 없다」이다). 판단:
    - **OSC 5522(kitty 클립보드) — 낮음. 단 「거의 공짜」는 과소평가였다**: OSC 52(범용 표준)를 이미 지원하고 프로그램이 폴백하므로 실기능 공백이 아니다 — 이 판단은 유지한다. 다만 스트리밍 싱크 위에서 공짜로 따라오는 것은 **청크뿐**이다. 명세([kitty clipboard](https://sw.kovidgoyal.net/kitty/clipboard/))가 함께 요구하는 것: 쓰기 3단(`type=write` → `type=wdata:mime=` 반복 → 빈 `wdata` 로 종료), 읽기 응답 3단(`status=OK` → `DATA` 반복 → `DONE`), 읽기 전 **사용자 권한 프롬프트**, 프롬프트를 건너뛰는 비밀번호(`pw`/`name`), MIME 별칭(`type=walias`), 붙여넣기 이벤트 모드(`CSI ? 5522 h`/`l`), 능력 질의(`CSI ? 5522 $ p`), 멀티플렉서가 응답을 라우팅하도록 에코하는 `id`, 오류 코드 6종(`ENOSYS`·`EPERM`·`EBUSY`·`EIO`·`EINVAL`·`EFBIG` — 터미널은 최소 64MB를 수용해야 한다). **부산물이 아니라 프로토콜 한 벌이다.**
    - **OSC 66(텍스트 사이징) — 진짜 후보(큰 렌더러 작업)**: 형식은 `ESC ] 66 ; <metadata> ; <text> ST` 이고 metadata 는 **콜론(`:`) 구분** `key=value` 다([kitty text sizing](https://sw.kovidgoyal.net/kitty/text-sizing-protocol/), 2026-09-24 확인) — `s` 배율 1~7(`s×w` × `s` 칸에 그린다), `w` 칸 너비 0~7(0이면 터미널이 계산), `n`/`d` 분수 배율 0~15(`d > n`), `v` 세로 정렬 0~2(top/bottom/center), `h` 가로 정렬 0~2(left/right/center). 그린 뒤 커서는 **같은 행에서 `s×w` 칸 오른쪽**으로 가고, 블록이 화면보다 크면 그 문자를 **버린다**. 글자를 셀 배수/분수로 그리는 렌더 기능이라 파싱이 아니라 **fixed-cell 렌더러 확장**(kitty graphics급)이 본체다. 착수 시 파서를 **번호-인식 + 스트리밍 싱크**로 전환하는 트리거로 삼는다(단일 트리거 — [terminal-compatibility-policy.md §OSC52 "장기 방향"](../terminal-compatibility-policy.md) 단일 출처). ⚠️ 이 리팩터링 대부분은 **사용자에게 안 보이는 유지보수·미래 대비**다(피크 메모리·파서 내부 불가시) — UX 명분으로 팔지 않는다. 사용자에게 실제 나은 건 이미 구현한 "가시적 상한"(무음 폐기 제거)뿐이다.
...
- **K2d — Swift/ObjC Metal 렌더(완료)**: `maru_metal_renderer.m`가 image_id→`MTLTexture` 캐시(generation 바뀐 것만 업로드, RGB→RGBA 확장), `maru_image_*` 셰이더로 textured-quad, z-pass(maru는 기본 셀 배경 alpha=0이라 셀 패스 전=텍스트 뒤·후=텍스트 앞)로 그린다. AppSession이 매 frame 활성 surface placement를 수집해 ABI로 전달(K2e 부터는 화면에 보이는 pane 전부). 컴파일/계약/단위 전부 green, 화면 픽셀은 GUI 육안 수동.

- **K2e — 화면에 보이는 비활성 panel 이미지 + 뷰포트 crop(완료, 2026-09-23)**: K2d 는 **활성 surface 하나**의 placement 만 모았고 live 집합도 그 surface 기준이라, split 으로 함께 보이는 비활성 pane 의 이미지는 안 그려졌고 텍스처까지 evict 됐다(다시 활성화해야 재업로드). 이제 `AppSession.tick` 이 **화면에 펼쳐진 탭의 split 트리 leaf**(`activeTabLeafRects`) 중 activeTerm 이 terminal 인 pane 을 모두 모은다 — 활성 pane 먼저, 그다음 비활성 pane 을 각자 `paneTermRect` origin 으로. **안 보이는 pane**(다른 워크스페이스 탭·다른 워크스페이스·pane 안 뒤쪽 Term)은 옛처럼 모으지 않아 텍스처가 evict 된다. 비용: GPU 이미지 메모리가 「활성 1 개」에서 「**동시에 보이는 pane 수**」로 묶이고, 보이는 비활성 애니메이션도 업로드한다(창 전체 상한은 측정 후 후속).
  - **id namespacing — 입구 번역(u32 매핑)**: kitty image id 는 앱이 정하는 **surface-local** 값이라 두 pane 이 같은 id 를 쓸 수 있는데(terminal-browser 는 4207 고정), GPU 쪽 키(Swift `imageTextures`·`kitty_uploaded`·live 집합·`GpuImage`/`GpuImageUpload.image_id`)는 창 하나에 맨 u32 다. [new-window-and-chrome.md](new-window-and-chrome.md) #5~7 은 키 타입을 `(surface_id, image_id)` 복합 키로 넓히는 설계였는데, **키 타입은 u32 그대로 두고 입구에서 (surface, 로컬 id) → 겹치지 않는 u32 로 번역**하는 쪽으로 구현했다(`renderer/kitty_image_ids.zig`). 근거: ① 렌더러 ABI 무변경 ② **원래 id 우선** — 다른 surface 가 안 쓰면 원래 id 를 그대로 줘서 pane 하나일 때 옛 동작과 같다. 겹칠 때만 `0xF000_0000~` 대역에서 새 id(예약 id·이미 쓰인 새 id 는 건너뜀). 매핑은 프레임을 넘어 안정하고(바뀌면 매 프레임 재업로드), 이번 프레임에 안 보인 매핑은 놓아준다. **함정**: 놓아준 전역 id 의 업로드 기록을 안 지우면, 그 id 가 다른 이미지에 재배정됐을 때 generation 이 우연히 같으면(generation 은 코어마다 1 부터) 업로드를 건너뛰어 옛 그림이 보인다 — live 집합으로 prune 해 막는다. 활성 pane 을 먼저 resolve 해 원래 id 우선권을 준다.
  - **픽셀 복사 락**: `planImageUploads` 가 코어 메모리에서 픽셀을 복사하므로 비활성 surface 도 **그 surface 락 안에서** 계획하고, 임시 버퍼를 기존 버퍼 뒤에 이어 offset 을 민다(배경 이미지 합류와 같은 패턴). 합친 뒤 `sortGpuImages` 로 (pass, z) 순을 다시 맞춘다 — 렌더러가 그 순서로 텍스트-앞 패스 경계를 자른다.
