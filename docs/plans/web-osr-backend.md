...
선택은 열기 창을 **maru 창에 붙여** 띄우고 답을 콜백으로 돌려준다. 권한은 요청마다 묻고 기억하지 않는다(출처별 기억은
후속). 카메라·마이크·위치는 W5 에서 지원한다(Maru.app Info.plist 사용 목적 — sidecar 는 maru 가 띄운 자식이라 macOS 가 권한을
**Maru 앱에 귀속**한다. 셸에서 띄운 개발 실행은 터미널에 귀속된다). **웹 알림도 W5 에서 maru 가 띄운다**(사용자 결정 2026-09-25) —
...
| `on_show_permission_prompt`·`on_request_media_access_permission` | maru 창에 붙는 네이티브 경고창으로 요청마다 묻는다(기억하지 않음). 카메라·마이크의 macOS 권한 귀속(Maru.app Info.plist)은 장치 있는 기계에서 확인(W5) |
