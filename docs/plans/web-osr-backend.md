...
| 배포·배치 | **별도 Homebrew formula `maru-chromium`**(사용자 결정 2026-09-24, 이름은 2026-09-25 — `brew install maru-chromium`. maru formula 와 따로, OSR 을 쓰는 사용자만 설치한다. OSR 은 선택형이라(D2) 안 쓰는 사용자가 CEF 약 323MB 를 받을 이유가 없다), `.app` 번들 없이 `libexec/` 에 sidecar·helper·프레임워크를 **실제 파일로** 함께 둔다 | 번들 없는 배치로 154 의 모든 실측이 섰다. 샌드박스는 **별도 helper + 프레임워크를 실행 파일 아래에 둔** 배치에서만 동작했고(§13.1 「남은 미해결」 1), PoC 의 메모리·N 개·링 실측은 샌드박스 없이 잰 값이다 — N 개는 W1c 가 샌드박스를 켠 채 다시 쟀고(렌더러 포함 helper 모두 샌드박스), 메모리·링은 W2 이후 다시 잰다 |
...
| **W3 노출** | W4(입력) 전까지 OSR 백엔드는 **개발용 환경변수로만** 켠다 — config·문서에 키를 내지 않는다. 설정 키는 입력까지 되는 W4 에서 연다(사용자 결정 2026-09-24) | 입력 없는 W3 만으로는 웹을 보기만 할 수 있다 → **W4d① 에서 열었다**: `browser.engine = webkit(기본) | chromium` 하나(사용자 결정 2026-09-25 — 경로 키는 두지 않고 `maru-chromium` 설치 위치 `$(brew --prefix)/opt/maru-chromium/libexec` 를 찾는다), 앱을 다시 시작해야 적용(열린 탭이 엔진을 옮기며 로그인·입력을 잃지 않게), 개발용 환경변수가 먼저 |
...
핸들러가 없으면 `alert`·`confirm` 에서 페이지가 멈추고 권한 요청은 응답이 오지 않는다(§13.1 「남은 미해결」 9).

**결정(사용자 2026-09-25): maru 창에 붙는 네이티브 대화상자(sheet)로 받는다** — JS 대화상자·권한은 macOS 경고창을, 파일
선택은 열기 창을 **maru 창에 붙여** 띄우고 답을 콜백으로 돌려준다. 권한은 Chromium 이 묻는 때(처음 요청) 묻고, **답은
Chromium 기본대로 출처별로 영구 기억한다**(사용자 결정 2026-09-25 — 처음의 「요청마다 묻고 기억하지 않는다」를 아래 W5b 실측
뒤 바꿨다. 기억을 지우는 화면은 후속). 카메라·마이크·위치·알림에 더해 **그 밖의 권한(MIDI sysex·창 관리·로컬 글꼴·화면
공유 등)도 같은 모달로 묻는다**(사용자 결정 2026-09-25). 카메라·마이크는 Maru.app Info.plist 사용 목적으로 macOS 권한을 받는다
(TCC 는 maru 가 띄운 자식의 요청을 **Maru 앱에 귀속**한다 — 셸에서 띄운 개발 실행은 터미널). **위치는 다르다** — 아래 실측대로
Chromium 의 위치 공급자가 CEF 에서 돌지 않으므로 **maru 가 CoreLocation 으로 위치를 받아 그 탭에 DevTools
`Emulation.setGeolocationOverride` 로 넣는다**(사용자 결정 2026-09-25, CEF 메인테이너 권고와 같다 — CEF #3881). **웹 알림도 W5 에서 maru 가 띄운다**(사용자 결정 2026-09-25) —
CEF 는 웹 알림을 OS 로 보내지 않고 가로챌 API 도 없으므로(아래 실측), CDP(`Page.addScriptToEvaluateOnNewDocument`)로 넣은
대리 스크립트가 `Notification`·`registration.showNotification` 을 받아, helper 의 렌더러 처리기가 숨겨 둔 네이티브 함수와 CEF
프로세스 메시지로 sidecar 에 넘기고 sidecar 가 maru 알림으로 넘긴다(W5c — `Runtime` 도메인·바인딩은 쓰지 않는다).
페이지가 닫혀 있을 때의 서비스 워커 푸시 알림은 덮지 못한다. 처음 적은 「maru chrome
모달」에서 바꿨다 — 구현이 가볍고 포커스가 maru 를 떠나지 않는다. 대가는 **창 전체를 막는 것**이다(크롬은 그 탭만 막는다).

**W5 착수 전 실측(2026-09-25) — Chromium 기본 동작을 그대로 두면**(대화상자 억제를 뺀 sidecar, 창 목록·맨 앞 앱 관찰):

| 항목 | Chromium 기본 동작 |
|---|---|
| `alert`·`confirm`·`prompt` | sidecar 프로세스가 보통 층 창(260×218)을 **화면** 가운데 위쪽에 띄운다 — 창 순서가 **maru 창 뒤**라 안 보이고 페이지만 멈춘다(사용자에게는 이유 없는 먹통). 앱이 끝나면 창·프로세스가 남지 않는다 |
| 파일 선택 | sidecar 가 열기 창(880×448)을 **화면** 가운데, 모달 층(8)에 띄운다 — 보이지만 맨 앞 앱은 maru 라 한 번 눌러야 키를 받는다. 페이지는 계속 돈다 |
| 권한 | 창이 없다 — 위치는 답 없이 시한 초과, 알림은 끝까지 답 없음, 카메라·마이크는 이 기계에 장치가 없어 `NotFoundError`. 어느 방식이든 maru 가 받아야 한다 |
| 파일(우리 핸들러) | `on_file_dialog` 에 1 을 돌려주고 경로를 `cont` 로 넘기자 페이지가 파일을 받고 **샌드박스 렌더러가 내용(1234 바이트)을 읽었다** |
| 권한(우리 핸들러) | 위치(`0x100`)·알림(`0x8000`)이 `on_show_permission_prompt` 로 온다. 위치는 허용해도 시한 초과(셸 실행이라 책임 앱이 터미널 — 위치 권한 없음), 알림은 허용하면 페이지가 `granted`·`onshow` 까지 받지만 **화면에 아무것도 안 뜬다** — 알림 데몬(usernoted) 로그에 maru 기록이 0 줄이라 OS 까지 가지도 않는다(사용자 확인). 같은 세션 안의 두 번째 요청부터는 Chromium 이 허용을 기억해 다시 묻지 않는다 |

**끌어올림 실험**(대화상자가 뜰 때 sidecar 를 액세서리 앱으로 바꾸고 스스로 활성화): 대화상자가 모달 층으로 maru 위에 보이고
alert·confirm·prompt 의 답이 페이지로 돌아갔다(한글 prompt 포함 — 사용자 확인). 그러나 ⑴ 닫은 뒤 창 없는 sidecar 가 약 6 초
맨 앞 앱으로 남아 키가 어디로도 가지 않았고(사용자가 maru 를 누를 때까지), ⑵ 조건 없이 올리면 뒤쪽 탭이 다른 앱을 쓰던
사용자의 포커스를 빼앗고, ⑶ 위치가 화면 가운데로 고정된다(Chromium 창이라 maru 탭 위에 못 붙인다). **실제 Chrome 과의 차이**:
Chrome 의 JS 대화상자는 **탭 모달**이다 — 그 탭 웹 내용 위쪽 가운데에 붙고, 그 탭만 막고, 탭을 옮기면 숨는다. 그래서 Chrome
경험에 가장 가까운 것은 maru 가 그리는 탭 모달(pane 위쪽 가운데)이다 — 구현이 가장 커서 이번에는 네이티브 sheet 로 가고,
**탭 모달은 후속 선택지로 남긴다**. 못 잰 것: 대화상자가 떠 있을 때 탭을 닫는 경우(시험 환경에서 브라우저 탭이 워크스페이스의
유일한 탭이라 닫기가 세션 닫기로 간다), 파일을 실제로 골랐을 때 샌드박스 렌더러가 내용을 읽는지.

**W5b 착수 전 실측(2026-09-25)** — 판정자로 실제 host 를 돌리고(권한 핸들러를 실험용으로 달아 답을 바꿔 가며), 위치는 Maru.app
을 LaunchServices 로 띄워 쟀다:

| 항목 | 실측 |
|---|---|
| 오는 곳 | 위치 `0x100`·알림 `0x8000`·MIDI sysex `0x2000`·창 관리 `0x800000`·로컬 글꼴 `0x80` 이 `on_show_permission_prompt` 로 온다. 답하지 않으면(0) 페이지는 **끝없이 기다린다**. 카메라·마이크는 장치가 없는 이 기계에서 **콜백 전에** `NotFoundError` — 장치 있는 기계에서 확인할 몫. 클립보드 읽기·`requestPointerLock` 은 프롬프트 없이 거절 |
| **기억** | `ACCEPT`·`DENY` 가 프로필 `Preferences` 의 content setting 예외(허용 1·차단 2)로 **저장되어 재시작 뒤에도** 다시 묻지 않는다(알림·MIDI). 닫기(`DISMISS`) 셋이면 embargo — 묻지 않고 `denied`. CEF 는 `ACCEPT` 를 Chromium 의 영구 허용에 잇고 「이번만 허용」은 드러내지 않는다 |
| 되돌리기 | `set_content_setting(DEFAULT)` 뒤 다음 요청은 다시 묻는다 — 그러나 **그 순간 페이지도 권한을 잃는다**(`granted` → `default`). 환경설정 `profile.content_settings.exceptions.<종류>` 를 비우면(`set_preference(null)`) 저장된 예외·embargo 가 지워지고 재시작 뒤에도 유지된다 |
| 위치(Chromium) | 공급자는 `kHybridPlatform`(CoreLocation 먼저)으로 만들어지지만 **시작하지 않는다** — Maru.app 이 macOS 위치 권한을 받아도(`authorizationStatus` 3) 18 번 모두 시한 초과. 같은 순간 **sidecar 프로세스 안의 상태는 0**(정해지지 않음) — 위치 권한은 책임 앱에서 물려받지 않고 프로세스마다 매긴다. Chromium 은 자기 CLLocationManager 가 허용일 때만 공급자를 시작하고 그 권한을 청하는 길은 Chrome 권한 UI 에만 있다(CEF 는 그 UI 를 `on_show_permission_prompt` 로 바꾼다). Info.plist 없는 sidecar 가 스스로 청하면 macOS 가 무시하고, 실행 파일에 Info.plist 를 넣으면 Chromium 이 macOS 알림 부품을 띄우려다 우리 샌드박스 검사에 막혀 페이지가 준비되지 않았다(그 길은 macOS 위치 창이 둘 — Maru·sidecar — 이 되는 대가도 있다). 위치는 저장된 허용이 있어도 매번 다시 물었다 |
| 위치(덮어쓰기) | `Emulation.setGeolocationOverride` 를 보내면 권한을 거친 페이지가 그 좌표를 곧바로 받는다(`37.5665,126.978 acc=25`). `watchPosition` 은 덮어쓰기를 다시 보낼 때만 새 값을 받는다 |
| 위치(W5b2 착수 전) | 허용하면 저장값은 허용(1)이 되지만 **Chromium 은 위치를 부를 때마다 다시 묻는다**. 차단은 저장되어 묻지 않고 거절. 덮어쓰기가 없으면 시간 초과, 빈 덮어쓰기면 「위치를 알 수 없음」(오류 2). **덮어쓰기를 다시 걸면 기다리던 요청이 먼저 오류 2 를 받는다** — `getCurrentPosition` 은 그 오류로 끝나고 `watchPosition` 은 오류 뒤 새 좌표. 페이지의 시한은 권한을 묻는 동안에도 흐른다(7 초 늦춘 답에 `timeout: 6000` 이 시간 초과). 덮어쓰기는 이동 뒤에도 그 탭에 남는다(다른 출처는 권한을 거쳐야 읽는다). **덮어쓰기는 걸 때 한 번만 새 좌표로 전해진다** — 같은 문서의 다음 `getCurrentPosition`(`maximumAge` 0)은 묻지도 않고 새 좌표를 기다리다 시간 초과(시한이 없으면 끝없이), `maximumAge` 가 있으면 받아 둔 좌표를 곧바로 받는다 |
| 비교 | Electron 은 권한을 저장하지 않고 매번 앱의 핸들러에 묻는다(핸들러가 없으면 모두 허용). Suji(같은 저자의 CEF 앱)는 앱에 위임하고 저장을 다루지 않으며 위치를 따로 처리하지 않는다 |

| CEF 콜백 | maru 쪽 |
|---|---|
| `on_jsdialog`(`alert`·`confirm`·`prompt`)·`on_before_unload_dialog` | maru 창에 붙는 네이티브 경고창(sheet), 응답을 콜백으로 |
...
| `on_file_dialog` | maru 창에 붙는 열기 창(sheet)을 띄우고 고른 경로를 콜백으로(샌드박스 안 렌더러가 내용을 읽는 것까지 실측). **폴더는 안의 파일들로 펼쳐 답한다** — 폴더 경로 자체로 답하면 페이지에 아무것도 오지 않았다(W5a 판정자 실측 — Chrome 의 폴더 업로드 확인 화면이 창 없는 CEF 에 없어 멈추는 것으로 본다). 그래서 페이지의 `webkitRelativePath` 가 빈다(후속) |
| `on_show_permission_prompt`·`on_request_media_access_permission` | maru 창에 붙는 네이티브 경고창으로 묻는다(W5b). 답은 Chromium 이 출처별로 기억한다. 카메라·마이크는 허용할 때 maru 가 macOS 권한(`AVCaptureDevice`)을 먼저 받는다 — 실제 장치 확인은 장치 있는 기계의 몫. 위치는 허용하면 maru 가 CoreLocation 좌표를 DevTools 덮어쓰기로 넣는다 |
...
| **W4c** 키보드 | C5 의 키 라우트(`web_key_route` — 메뉴 먼저·`app_action` 은 `dispatch_web_app_action`)·모달 에지(capture lost·조합 마침·포커스), 키 이벤트(raw_down·char·up), IME(`NSTextInputClient` 목적지 분기 — 기존 `ime_insert`·`ime_marked` 를 Zig 가 OSR 로 돌린다)·후보창 위치(`ime_range`), 메뉴 편집 액션. W4a 가 넘긴 것: 16 KiB 를 넘는 `insertText` 는 글자 경계에서 나눈다(바꿀 범위는 첫 조각에만), 키 이벤트의 글자는 UTF-16 하나라 BMP 밖 글자는 IME 경로로, 조합 사각형은 maru 가 조합을 보내는 동안에만 쓴다(끝 알림이 없다) | 라우팅 판정은 Zig 순수 시험, 앱 경로는 스모크 — **구현됨**: 키 대상은 Zig 가 답한다(`osrKeyTarget` — 창 키·입력 초점이 터미널 자리·오버레이 없음·활성 Term 이 OSR browser). Swift 는 NSEvent 를 옮기기만 한다(ABI v193 `osr_keyboard_active`·`osr_key`·`osr_edit`): ⌘·⌃ chord 와 기능키는 곧바로 raw_down, 그 밖은 입력기 트랜잭션(ime_begin → interpretKeyEvents → ime_end)을 거쳐 Zig 가 판정한다(`web_osr_input.imeOutcome` — 모든 키 raw_down, 평문 한 글자·Enter 만 char, 여러 글자·BMP 밖은 확정 글, 입력기가 가져간 키는 키 이벤트 없음). 조합은 `ime_set_composition`, 확정은 `ime_commit_text`(16 KiB 씩 글자 경계에서 나눔), 조합 취소(확정 없이 비움)는 `ime_cancel_composition`, 후보창은 `ime_range` 사각형. key equivalent 는 WKWebView 패널과 같은 판정(⌘←/→/R 탐색·`web_key_route`) — ⌘A·⌘Z·⌘⇧Z 는 앱 액션에서, ⌘C·⌘V·⌘X 는 메뉴에서 페이지 편집 명령으로. 모달 에지: 키 대상이 바뀌면(탭·pane 전환·오버레이·창 키) tick 이 옛 탭의 조합을 확정하고 포커스를 옮긴다(탭을 활성으로 올리는 누름 **직전**에도). 남은 WKWebView 포커스는 회수한다. **착수 전 실측**(판정자 진단): Backspace·Tab·화살표·Home·Esc·⌥⌫ 는 raw_down 만으로 동작하고 char 를 더하면 쓸데없는 keypress 가 생긴다, Enter 는 char `\r` 이 있어야 textarea 줄바꿈, ⌘← 는 줄 처음으로 가지 않는다(maru 는 ⌘← 를 뒤로 가기로 쓴다), **⌃A·⌃E·⌃K 같은 macOS 텍스트 편집 단축키는 키 이벤트로 동작하지 않는다** — Chrome 은 편집 명령을 키 이벤트에 실어 보내는데 창 없는 CEF 에서는 CDP(`Input.dispatchKeyEvent` commands)로만 된다 → W9 에서 붙인다(사용자 결정 2026-09-24). 판정자 `input-special-keys` 가 이 규칙을 잡는다. **착수 뒤 실측이 고친 것**: Swift `insertText` 는 확정 전에 늘 빈 조합을 보낸다 — 그것을 「입력기가 가져감」으로 세면 평문 타이핑이 통째로 사라졌다, 첫 포커스 동기화가 sidecar 기록 전에 돌면 대상만 바뀌고 포커스는 영영 안 갔다(키는 닿아도 페이지 `focus` 가 안 오고 입력기 조합이 버려졌다 — 기록이 생길 때까지 미룬다). 입력기 트랜잭션 판정은 순수 `web_osr_input.imeOutcome` 이다 — 시작 때 조합의 확정은 트랜잭션 끝까지 쥐고(마지막 자모 Backspace 는 「확정 + deleteBackward」로 오는데 그때는 **취소**다), 새 조합이 뒤따르면 그 전에 보낸다. 조합을 끝낸 키는 뒤따른 입력기 명령(`doCommand` → ABI `ime_command`)이 있을 때만 그 동작으로 간다(한글 Space·Enter·화살표), 명령 없이 확정만 했으면 입력기가 키를 삼킨 것이다(일본어 변환 확정 Enter), 조합을 취소한 키(Esc·마지막 자모 Backspace)는 키 이벤트가 없다. 조합이 열렸다는 믿음은 `web_osr` 가 브라우저마다 든다(보낸 조합으로 세우고 확정·마침·취소·이동·포커스 잃음·재생성에서 푼다) — **조합이 없을 때의 조합 취소는 선택한 글을 지우므로**(Chromium — 판정자 `input-ime-cancel-idle` 실측, CEF 는 조합이 끝날 때 알려 주지 않는다) 취소는 이 값이 참일 때만 보낸다. 판정: 순수 시험(`imeOutcome` 셋 포함), 판정자 입력 19 판정(`input-special-keys`·`input-ime-cancel-idle` 추가), 앱 스모크 W4c 19 판정(대본의 `ime keyCode 단계…` 는 입력기 콜백을 한 트랜잭션 안에서 직접 부른다 — 사용자 입력 소스에 따라 갈리지 않게, `key` 는 performKeyEquivalent → 메뉴 → keyDown 순서): 클릭으로 textarea 초점, 평문이 keydown 뒤 글자, Enter 줄바꿈, Backspace, 한글 조합 ㅇ→아→안·확정, **keydown 순서 전체**(입력기가 가져간 키는 없다), ⌃E 가 key e·ctrlKey, ⌘A → Backspace 비움·⌘Z 되돌림, 조합 중 오버레이가 조합을 확정하고 페이지가 초점을 잃었다 되찾음, ⌘R 새로고침, 한글 Space(확정 + 공백)·Enter(확정 + 줄바꿈), 확정만 하는 Enter 는 줄바꿈 없음, 마지막 자모 Backspace·Esc 는 조합 취소, 같은 키에서 확정 뒤 새 조합이 서도 앞 글이 남음. **적대 검증 두 차례(2026-09-24~25)로 고친 것**: 1 차 — 조합을 끝낸 키가 사라짐(한글 Space·Enter·문장부호), 조합 중 ⌘ 단축키로 대상이 바뀌면 입력기 세션의 조합이 남아 글자가 겹침(소비하는 key equivalent·메뉴 편집 명령 앞에서 확정, Zig 가 끝낸 조합은 Swift 가 버림 — tick 과 keyDown 초입), notice 토스트가 페이지 포커스를 빼앗음(`terminalOwnsInput` 으로 — WKWebView 탭 규칙), 마지막 자모 Backspace 가 자모를 확정함, 키로 탭을 바꾼 직후 한 프레임 동안 대상이 어긋남(키·편집·확정 직전에 맞춤), ⌘A 메뉴 순서, ABI 값 검사, 입력기가 가져간 키의 keyup; 2 차 — 조합 없는 트랜잭션에서 확정 뒤 새 조합이 오면 확정 글이 사라짐, 터미널이 쥔 조합(pin)을 OSR 이 가로챔, 조합 없을 때의 취소가 선택을 지움(위), Esc 조합 취소가 Escape 키도 보냄, 창을 옮긴 탭의 포커스를 옛 창이 늦게 빼앗음(포커스 준 창만 놓을 수 있다), 옛 조합 사각형을 후보창에 씀, 제어 문자가 든 확정이 통째로 사라짐(뺀다), 키 변환 실패로 트랜잭션이 안 닫힘, ⌘ chord 의 keyup 짝, 트랜잭션 밖 unmarkText(Apple 의미는 확정 — 곧 오는 확정 글이 대신하지 않으면 tick 에 확정), 스모크 거짓 통과(keydown 판정이 느슨했다). 변이: 1 차(첫 코드) 10 개 중 8 개 FAIL — 살아남은 둘은 이 시나리오에서 결과가 같다(⌃ chord 를 입력기로 보내도 macOS 가 ⌃E 를 편집 명령으로 바꿔 같은 키 누름이 나간다, 대상이 바뀔 때 조합 확정을 빼도 Chromium 이 포커스를 잃으며 스스로 확정한다), 2 차(중간 코드) 유효 10 개 모두 FAIL, 3 차(최종 코드 — 2 차 리뷰 고침 겨냥) 5 개 모두 FAIL. 「주소가 바뀌면 포커스를 다시 준다」는 빼면 조합이 깨져 필요하다는 증거가 됐다. **남은 것**: macOS 텍스트 편집 단축키(⌃A·⌃E·⌃K)는 W9(CDP), 조합 믿음은 페이지 스크립트가 몰래 조합을 끝내면 어긋날 수 있다(그때 조합 없는 취소가 선택을 지울 수 있다 — 드묾), 진짜 입력기·NSEvent 변환·입력기 세션 버리기는 스모크 밖(W4d 시험기), 수식키 단독 keydown(Shift 등)은 안 간다, 텍스트 칸 안의 ⌘←/→ 는 WKWebView 처럼 뒤로·앞으로다, `replacementRange` 는 쓰지 않는다 |
| **W4d①** 설정 키 | `browser.engine`(webkit·chromium, 재시작 후 적용), `maru-chromium` 설치 찾기, 설치가 없으면 안내 뒤 WebKit, 설정이 바뀌면 「재시작하면 적용」 안내 | 설정 파싱 시험, 엔진 결정 규칙 시험, 설정 문서 게이트 — **구현됨**: 엔진은 첫 창이 설정을 읽은 뒤 프로세스에 한 번 정한다(`web_osr.decide` — 순수 규칙 `decideFrom`: 개발용 환경변수 → 설정이 chromium 이고 설치가 있으면 그 설치 → 청했는데 설치가 없으면 WebKit + 안내). 설정 파일 reload 로 바뀌면 적용 중인 값과 다를 때 한 번 「재시작하면 적용」(`engineChangeNeedsNotice` — 되돌리면 풀린다, 청했지만 설치가 없어 이미 WebKit 인데 webkit 으로 되돌리면 알리지 않는다). 설정 화면은 작업 공간 섹션 드롭다운이고 행 이름에 「maru 를 다시 시작하면 적용」이 있다. 판정: 설정 파싱 시험, 엔진 결정 규칙·첫 결정 고정·안내 시험, 앱 스모크(개발용 환경변수 없이 설정만으로 — 가짜 `HOMEBREW_PREFIX` 에 sidecar 를 링크한 「설치됨」은 Chromium 으로 열리고 자식 sidecar 가 보이며, 빈 prefix 의 「설치 없음」은 sidecar 를 띄우지 않고 안내를 남긴다). **적대 검증(2026-09-25)으로 고친 것**: 설정 화면에서 엔진을 바꾸면 안내가 설정 창을 닫아 드롭다운 미리보기만 해도 창이 닫히고 값이 저장되지 않았다(안내는 파일 reload 에서만 — 설정 화면은 행 이름이 알린다), 설치가 없어 WebKit 인데 되돌리면 헛안내, 개발용 환경변수 경로를 512 바이트에서 말없이 잘랐다(복사하지 않는다), 「시작 실패」 안내가 개발용 환경변수를 가리켰다, 스모크 「설치 없음」이 사실상 빈 판정이었다(자식 sidecar 없음과 양성 대조를 더했다), 첫 결정 고정 시험이 없었다. 변이: 첫 판 4 개 중 3 개 FAIL — 살아남은 하나(설치가 없는데 Chromium 으로 정함)는 스모크 「설치 없음」이 WebKit 으로 실제로 열렸는지 안 봐서였다 → WKWebView 수(`web_panel_count`)를 보게 고친 뒤 FAIL(한 번은 부하로 W3c 애니메이션 빈도 판정이 먼저 떨어져 다시 돌렸다). 설치 prefix 무시·설정 무시·첫 결정 고정 없음은 FAIL |
| **W4d②** 앱 안 시험기 | 앱 안 시험기(25 항목 — §13.1 「pane 안 실측」)를 opt-in smoke 로 저장소에. 키는 `postToPid` 로 **진짜 NSEvent 경로**를 탄다(앱이 비활성이어도 닿는다 — W4d 착수 전 실측), 커서·후보창 위치는 앱 안에서 잰다. **진짜 macOS 입력기(한글)는 maru 가 맨 앞 앱일 때만 조합한다**(실측 — 입력 문맥 입력기를 바꿔도 비활성 앱에서는 ASCII 로 들어왔다) → 자리를 비울 때 돌리는 별도 opt-in 명령으로 maru 를 앞으로 올리고 시스템 키보드 이벤트로 친다(사용자 결정 2026-09-25) | 시험기 항목(`<select>` 두 항목은 W6) — **구현됨**: `tools/test-macos-web-osr-tester.sh`(`mise run web-osr-tester`, CI 밖). W4b·W4c 대본 실행기(`MARU_WEB_OSR_TEST_INPUT`)에 Swift 한 겹을 타는 명령을 더했다 — `post`(진짜 키 이벤트를 앱 자기 프로세스에만 — 창 서버 → NSApp.sendEvent → keyDown·interpretKeyEvents, 사적 이벤트 원천), `view`(터미널 view 의 마우스 메서드를 NSEvent 로 — 오른쪽 이상 버튼은 CGEvent 로 만들어 버튼 번호를 싣는다), `cursor`·`imerect`(firstRect)·`overlay`·`menu`·`config`·`newwindow`(창을 만들고 키 창이 바뀔 때의 delegate 를 부른 뒤 그 창을 대상으로)·`mark`(페이지 이벤트를 단계로 가르는 시각), 결과는 `MARU_WEB_OSR_TEST_REPORT` 파일. 자동 판정 33 개 — 앱을 앞으로 올리지 않고 시스템 입력 소스·클립보드·포인터를 건드리지 않는다: 클릭 초점, 진짜 키 순서·Enter·Backspace·⌃E, ⌘A(메뉴)·⌘Z, 후보창이 조합 자리(폴백인 페이지 원점과 갈린다)·확정, 더블클릭·두 클릭 100 pt → 100 CSS px·우클릭·가운데 클릭·링크 이동·뒤로 버튼·본문 밖까지 끌기, hover 커서(링크 → 글 → 밖 — 앞 단계와 다른 것을 바라게), ⌘⇧P 팔레트가 막고 Esc 로 되돌림, 설정 reload 알림 토스트가 뜨고 첫 클릭만 닫음, 뗌을 잃은 누름 뒤 주소창 누름이 페이지로 새지 않음, 주소창에서 시작한 끌기가 페이지에 0 건, ⌘D 분할·⌘⌥← 복귀의 blur·focus, 새 창 같은 자리의 클릭·키가 첫 창 페이지에 0 건·새 창 탭에 닿음. 페이지마다 인스턴스 id·순번을 실어 창 둘·병렬 요청 순서를 가른다. `--live`(`mise run web-osr-tester-live` — 자리 비움 모드, **아직 돌리지 않았다**): LaunchServices 로 번들을 띄워 maru 를 앞으로 올리고, 앞이 된 것을 확인한 뒤 CR6d 의 입력 소스 트랜잭션(`SessionHostInputSourcePolicy` — 복원 기록을 먼저 쓴다)으로 한글 2벌식을 고르고, 진짜 입력기로 ㅇ→아→안·Space 확정·마지막 자모 Backspace 취소·Enter, 진짜 클릭·클립보드(⌘A ⌘C → ⌘V)·⌘Z·⌘⇧P(진짜 키 창의 performKeyEquivalent)·hover 커서를 본다. 입력기가 필요한 키와 포인터만 HID 로(보낼 때마다 활성·맨 앞·첫 응답자, 포인터는 그 자리의 맨 위 창이 maru 인지), ⌘ 조합은 앱 자기 프로세스에만. 끝에(앱이 죽어도 스크립트가) 입력 소스는 우리가 고른 소스일 때만, 클립보드는 시험 뒤 아무도 안 썼을 때만, 앞 앱은 maru 가 아직 앞일 때만 되돌린다. **실측으로 정한 것**: 비활성 앱에는 키 창이 없어 ⌘ 조합은 view 의 performKeyEquivalent 를 건너뛰고 메뉴로 간다(⌘A·⌘D·⌘⌥← 는 메뉴로 판정되고, view 가 먼저 받는 ⌘Z·⌘⇧P 는 AppKit 순서를 대본이 밟는 `key` 로 — 진짜 경로는 `--live`), `NSEvent.mouseEvent` 는 오른쪽·가운데 누름의 버튼 번호를 0 으로 만든다, 새 창을 만들면 stderr 가 앱 로그로 옮겨 간다(그래서 보고 파일), 스모크 모드는 New Window 메뉴를 막는다, 창 맨 위 28 pt 는 창 끌기 영역이라 AppKit 이 누름을 가져간다(본문 밖 시나리오는 주소창에서), 스크린샷(`MARU_SCREENSHOT`)과 대본은 함께 돌지 않는다, 백그라운드로 LaunchServices 가 띄운 앱에는 `postToPid` 키가 닿지 않았다. **적대 검증(2026-09-25)으로 고친 것**: 후보창 판정이 폴백 자리로도 통과했다(1 배 화면), 페이지 인스턴스·요청 순서 판정이 느슨했다, hover 커서 판정이 앞 단계가 기억시킨 커서로 통과했다, 우·가운데 클릭이 한 단계에 섞여 뒤바뀌어도 통과했다, 토스트가 떴는지 안 봤다, 뗌 유실 판정이 Chromium 스스로의 동작과 갈리지 않았다, 자리 비움 모드 — cleanup 이 앱을 못 죽였다, 활성화 확인 전에 입력 소스를 바꿨다, 포인터가 덮은 창에 떨어질 수 있었다, ⌘ 조합을 HID 로 보냈다, 클립보드를 앱과 스크립트가 이중으로·사용자 것까지 되돌렸다, 앞 앱을 무조건 활성화했다. **2 차 적대 검증(같은 날)으로 고친 것**: 자리 비움 모드가 새 트리에서 시작도 못 했다(입력 소스 복원 도구를 `macos-app-build` 가 만들지 않는다 — 시험기가 같은 두 소스로 직접 만든다), 이벤트 보내기 권한(TCC)이 없으면 HID 가 말없이 버려졌다(`begin` 이 CR6d 와 같이 확인), LaunchServices 로 띄운 앱이 문서 폴더의 sidecar 를 읽다 권한 창이 뜰 수 있었다(임시 뿌리로 복제), 앞 앱 복원이 죽은 코드였다(띄우는 순간 maru 가 앞이라 — 스크립트가 띄우기 전 앞 앱을 넘긴다), `begin`~`end` 밖에서도 HID 를 보낼 수 있었다, 다른 앱이 쓴 클립보드를 시험기 것으로 받아들였다(⌘C 뒤 정확히 한 번 바뀐 것만), 앱이 죽었을 때 스크립트 폴백이 그림 클립보드를 빈 글로 덮을 수 있었다, 전체 시간 제한이 없었다(감시 타이머), HID → 뒤 ⌘V 순서 경쟁, 누르지 않은 이동이 본문 밖 끌기 중 새도 못 잡았다, 첫 창 페이지가 다시 로드되면 새 창 페이지로 오인할 수 있었다(적재 정확히 둘). 변이: 1 차 10 개 중 8 개 FAIL — 살아남은 둘은 우·가운데 뒤바뀜(판정을 단계로 나눈 뒤 FAIL)과 backing 배율 무시(이 기계 화면 배율이 1.0 이라 같은 동작 — **배율 판정은 2 배 화면에서만 판별력이 있다**), 2 차 3 개(우·가운데 뒤바뀜·웹 제스처 중 왼쪽 누름이 옛 제스처를 안 끊음·후보창이 조합 자리를 무시) 모두 FAIL. **남은 것**: 셸에서 띄운 maru 도 맨 앞 앱이 시스템 알림 창·Finder 이면 곧바로 맨 앞이 된다(W5 착수 전 실측 — 0.27 초) — 자동 모드가 그때는 앞으로 나올 수 있다(방해 없음 주장의 예외, 백그라운드 실행 방법을 찾을 것), `--live` 실행(사용자가 자리를 비울 때), 휠의 Swift 한 겹(`handleScroll` — W4b 는 ABI 로만), `drainOsrCursor` 의 키 창·포인터 게이트는 자동에서 안 돈다(hover 가 커서를 바로 정한다), 창 hitTest 한 겹은 `--live` 에서만, `<select>` 는 W6 |
| **W5** 대화상자·파일·권한 | C6 전부 — maru 창에 붙는 네이티브 sheet(사용자 결정 2026-09-25, 착수 전 실측은 C6). 세 PR: **W5a** JS 대화상자·떠나기 확인·파일 선택, **W5b** 권한(카메라·마이크·위치 — Info.plist), **W5c** 웹 알림 중계(CDP 대리 스크립트 → maru 알림) | `alert`·`confirm`·`prompt`·파일 선택(내용 읽기까지)·권한 거부/허용. 카메라·마이크 권한 귀속을 장치 있는 기계에서 확인 — **W5a 구현됨**: 프로토콜 여섯 tag(`js_dialog`·`file_dialog`·`dialog_closed` → maru, `dialog_reply`·`file_dialog_path`·`file_dialog_reply` → sidecar — 대화상자 글은 줄바꿈·탭을 받고 4 KiB, 경로는 절대 경로, 출처는 `scheme://host[:port]` 255 바이트·위장 문자 거절, 요청 번호 0 금지). sidecar(`web_sidecar/dialogs.zig`·`dialog_table.zig`)가 CEF 콜백을 쥐고 번호를 매겨 보내고 답으로 콜백을 부른다 — maru 가 이동·뒤로·새로고침·파괴를 보내면 떠 있는 JS 대화상자를 먼저 취소하고(`dialog_closed` — CEF 는 대화상자가 떠 있으면 이동도 안 하고 기다렸다, 실측), 렌더러가 죽거나 페이지 상태가 비면 놓는다. 출처는 CEF 보안 표시 형식(`cef_format_url_for_security_display`). 같은 페이지의 둘째 대화상자부터 「이 페이지가 대화상자를 더 띄우지 못하게」를 청하고(Chrome 과 같다), 억제는 **maru 가 시킨 이동의 새 문서에서만** 푼다(페이지가 스스로 새로고침해도 이어 센다 — 창 전체를 막는 sheet 의 탈출구). maru(`web_osr` 대기열 — 탭마다 4, maru 토큰으로 짝을 찾아 sidecar 가 다시 떠도 옛 답이 새 요청에 안 붙는다)는 **키보드 초점을 가진** 탭의 요청만 띄운다 — 보이기만 하는 옆 pane 은 알림 목록(안 읽음)에 한 번 남기고 초점이 오면 띄운다. Swift 는 NSAlert·NSOpenPanel·NSSavePanel 을 maru 창에 붙는 sheet 로 띄운다(제목에 출처 — 「○○의 메시지」, 떠나기 확인은 페이지 글이 아닌 maru 문구, 뜬 뒤 0.5 초 단추·입력칸 막기, sheet 가 키면 편집 메뉴는 sheet 로·메뉴 단축키는 부모 창으로, quick terminal 은 sheet 가 붙으면 숨지 않음, 폴더는 「파일 N 개를 이 사이트에 올릴까요?」 확인·4096 개 넘으면 안내 뒤 취소·숨긴 파일·심볼릭 링크 제외, 받을 형식은 UTType 으로 — 모르면(동적 형식 포함) 제한하지 않음). ABI v194. 판정: 순수 시험(codec·대화상자 표·web_osr 대기열), 판정자 `--dialogs` 12(수락·취소·한글 여러 줄 답·묻는 동안 host 창 0 · 이동하면 `dialog_closed` 와 늦은 답 무시 · 떠나기 확인 머무르기 뒤 응답·떠나기 · 억제 · 스스로 새로고침해도 억제 · 출처 사용자 정보·`data:`·긴 호스트 · 파일 하나·여러 개·폴더·취소), 시험기 W5a 24(대화상자 셋이 maru 창에 붙는 sheet·출처 제목·한글 답·취소 · 입력 보호 · 파일 하나·여러 개·폴더 확인·거절·심볼릭 링크 · 실제 열기 창(형식 제한·폴더 전용)·취소 · 형식 변환 · 억제 · ⌘R 떠나기 확인 · 뒤쪽 탭·초점 없는 pane 은 기다림 · sidecar 가 죽으면 sheet 닫힘). **실측으로 정한 것**: CEF 는 대화상자를 하나 닫을 때마다 `on_reset_dialog_state` 를 부른다(억제 수는 주 프레임 `on_load_start` 에서만 푼다), alert 로 막힌 페이지는 이미지 요청도 못 보낸다(시험 신호는 `sendBeacon`), CEF 154 는 파일 선택 취소에 `cancel` 이벤트를 쏘지 않았다, NSAlert 단추는 sheet 를 세운 **뒤**에 꺼야 꺼진다(먼저 끄면 배치가 다시 켰다 — 시험기가 잡았다), 안내 토스트는 「다음 입력이 닫는다」라 그 pane 으로 옮기는 첫 키를 먹는다(알림 목록으로 바꿨다). **적대 검증 세 차례(2026-09-25)로 고친 것**: 1 차 — quick 창의 답이 버려짐, sheet 중 메뉴 단축키가 다른 창에, `while(1) alert()` 에서 빠져나갈 길 없음, 출처 위장(사용자 정보·`data:`), sidecar 재시작 뒤 요청 번호 재사용, 긴 prompt 답이 빈 글로, 폴더 일부만 몰래·확인 없음, 창 닫힌 뒤 sheet 기록, Esc 없음, 떠나기 머무르기·파일 취소·열기 창의 거짓 통과; 2 차 — 보이기만 하는 옆 pane 이 터미널에 치던 글자(비밀번호)를 가로챔, sheet 안 ⌘V 가 터미널로, 출처를 자른 뒤 검사, 새로고침 반복으로 억제 우회, 렌더러가 죽은 뒤 sheet 남음, 출처의 위장 문자, 판정의 거짓 통과; 3 차 — quick terminal 이 sheet 와 함께 숨음, 모르는 형식(동적)이 모든 파일을 막음, 저장 창에 올리기 문구, 초점 없는 pane 이 표시 없이 기다림·뒤로 버튼이 초점을 안 줌, 머무르기·멈춤이 억제 풀기 표시를 남김, 받을 형식을 자르면 제한이 엄격해짐, 주석 불일치. 변이: 1 차 묶음(2 차 반영 뒤 코드) 15 개 중 13 개 FAIL — 살아남은 둘은 같은 동작이다(심볼릭 링크 검사 — macOS 열거기가 이미 따라가지 않는다, 출처 길이 가드 — 뒤의 `checkOrigin` 이 스스로 거절한다), 2 차 묶음(3 차 반영 뒤) 3 개 중 2 개 FAIL(출처를 버퍼 앞 255 바이트만 읽기, 동적 형식 받기) — 살아남은 하나는 「머무르기가 억제 풀기 표시를 지운다」다: 시험이 없다. 머무른 뒤 10 초 안에 페이지가 **스스로** 이동할 때만 억제가 풀리는 차이라 판정에 넣지 않았다. **남은 것**: 열기·저장 창에는 입력 보호가 없다(Chrome 과 같다), `showDirectoryPicker` 도 폴더를 파일 목록으로 받는다, 사용자가 링크로 다른 사이트에 가도 억제가 이어진다(maru 가 시킨 이동만 푼다), quick terminal·알림 목록 표시·sheet 안 ⌘V 는 자동 시험 밖(비활성 앱에는 키 창이 없다 — `--live` 몫), 폴더 업로드의 `webkitRelativePath`, 모르는 형식을 넓게 여는 것 외에 「모든 파일」 전환은 없다, 창 전체를 막는 sheet 대신 Chrome 같은 탭 모달은 후속 선택지 — **W5b1 구현됨**(권한 — 위치 좌표는 W5b2): 프로토콜 tag 둘(`permission_request` → maru: 출처와 CEF 와 같은 비트의 `kinds`(프롬프트 0~28) 또는 `media`(카메라·마이크·화면) 중 하나만, `permission_reply` → sidecar: 허용·차단·닫기·**못 물음**(IGNORE — maru 가 묻지 못한 경우를 사용자 닫기 수에 섞지 않게 따로. Chromium 은 이것도 따로 세어 넷이면 한동안 묻지 않는다 — 실측, Chrome 에서 묻는 중 탭을 닫은 것과 같다. 미디어 요청은 어느 쪽도 세지 않는다), 닫힘은 W5a 의 `dialog_closed`). sidecar(`web_sidecar/permissions.zig`)는 W5a 요청 표를 같이 쓰고(한 브라우저가 권한 요청으로 표를 채우지 못하게 8 개까지), 묻지 못하면 프롬프트는 IGNORE·미디어는 거부, CEF 가 프롬프트를 닫으면(페이지 이동) `dialog_closed`, 미디어 요청은 CEF 가 닫힘을 알리지 않아 maru 의 이동·파괴와 **새 문서의 `on_load_start`**·실패한 이동의 `on_load_error` 에서 치운다. maru 는 W5a 대기열·초점 게이트·토큰을 그대로 쓰고(`DialogKind.permission`), 창이 닫히거나 대기열이 차면 못 물음. Swift sheet 는 허용·차단·닫기(Esc) 셋 — Return 에 걸린 단추가 없고 첫 초점은 닫기(전체 키보드 접근의 Space 가 허용·차단을 누르지 않게), 창에 다른 sheet 가 붙어 있으면 다음 요청을 가져오지 않는다(뒤에 줄 서면 입력 보호가 보이기 전에 끝났다). 본문은 청한 권한 목록(29 종 + 미디어 넷의 이름 — 같은 이름은 한 줄)과, 프롬프트면 「허용하거나 차단하면 이 사이트에 기억됩니다」(위치 — 저장된 허용이 있어도 매번 다시 묻는다 — 와 파일 편집만이면 붙이지 않는다). 카메라·마이크·화면을 허용하면 **Maru 의 macOS 권한을 먼저 받는다**(`AVCaptureDevice`·화면 기록) — macOS 가 막으면 페이지에는 못 물음으로 답하고 시스템 설정으로 안내(화면 기록은 다시 시작해야 반영된다는 문구), 기다리는 사이 요청이 사라졌으면 안내하지 않는다. Info.plist 에 카메라·마이크 사용 목적. ABI v195. 판정: 순수(codec·표 상한), 판정자 `--permissions` 10(알림 허용·출처·host 창 0 · MIDI 차단 · 글꼴 닫기는 다시 묻는다 · 묻는 동안 maru 이동 → `dialog_closed`·늦은 답 무시 · 위치 비트 · 카메라(장치가 없으면 요청 전 `NotFoundError`) · 화면 공유는 미디어로 오고 기억하지 않으며 허용은 거부와 다른 결과 · 페이지가 스스로 떠나면(닿지 않는 주소로 떠나 이동이 실패해도) 미디어 요청 `dialog_closed` · 못 물음 넷이면 다섯째는 묻지 않고 거절(Chromium 실측을 판정으로) · **host 를 다시 띄워도 허용·차단을 기억**), 시험기 W5b 17(권한 sheet 가 maru 창에 붙고 출처·목록·기억 안내·단추 셋 · 첫 초점은 닫기 · 입력 보호 · 허용 → granted · 다시 청하면 sheet 없이 같은 답 · 차단 → 거절·기억 · 닫기는 기억 안 함 · 화면 공유는 기억 안내 없음 · macOS 가 막으면 못 물음과 설정 안내 · 안내 뒤에 온 요청은 안내를 닫은 뒤 뜬다 · 묻는 동안 페이지가 떠나면 sheet 가 답 없이 닫힘). **실측으로 정한 것**: 프롬프트의 허용·차단은 프로필에 영구 저장되고, 미디어 답은 기억하지 않는다. `queryLocalFonts` 는 거절을 빈 목록으로 준다. 닫기 셋·못 물음 넷이면 그 종류를 한동안 묻지 않는다(따로 센다). 이 기계에는 카메라·마이크가 없어 미디어 요청이 콜백 전에 `NotFoundError` 로 끝난다. 셸에서 띄운 화면 공유 허용은 `NotReadableError`(화면 기록 권한 없음). **적대 검증(2026-09-25) 1 차로 고친 것**: macOS 안내 뒤에 줄 선 권한 sheet 의 입력 보호가 보이기 전에 끝나 안내를 닫는 클릭이 허용에 떨어짐, maru 가 스스로 보낸 닫기가 embargo 를 쌓음(→ IGNORE), 전체 키보드 접근의 Space, 페이지가 스스로 떠난 뒤 남는 미디어 요청, 요청이 사라진 뒤의 macOS 안내, 문구(화면 **전체** 공유 · 기억 안내 · 같은 이름 두 줄 · 장치 이름 · 화면 기록 재시작), 주석. 2 차 — 못 물음도 Chromium 이 세는데 「embargo 를 쌓지 않는다」고 적음(실측으로 판정 추가·문구 정정), 위치 요청에 기억 안내, 실패한 이동 뒤 남는 미디어 요청, 시험기 좌표가 두 단추 경계, 첫 초점을 배치 전에 둠, ABI 상수가 Zig 열거형에 묶이지 않음. 변이: 세 묶음 28 개(구현 뒤 · 1 차 반영 뒤 · 2 차 반영 뒤) — 1 묶음 13 개 중 9 개 FAIL(빌드 실패 1 은 변이를 고쳐 다음 묶음에서 FAIL), 2 묶음 10 개 중 6 개 FAIL(살아남은 넷 중 둘은 변이 스크립트가 새 단위 시험의 실패를 못 읽은 것 — 3 묶음에서 FAIL, 첫 초점은 시험을 더해 FAIL), 3 묶음 5 개 중 4 개 FAIL(둘은 변이가 컴파일되지 않아 손으로 고쳐 다시 돌려 FAIL 확인), 살아남은 「실패한 이동 뒤 미디어 요청 치우기」는 판정(닿지 않는 주소로 떠나기)을 더해 FAIL 확인. **끝내 살아남은 셋**: 모르는 권한 비트도 묻기(CEF 가 모르는 비트를 만들 길이 없다), Return 에 허용 단추가 걸림(비활성 앱에는 키 창이 없어 Return 을 보낼 수 없다), macOS 권한을 기다리는 사이 요청이 사라져도 안내(그 경합을 만드는 시험이 없다). **남은 것**: 카메라·마이크 실제 장치 확인(장치 있는 기계), 화면 공유는 고르는 단계 없이 화면 전체(CEF 기본) — 실제 공유는 Maru 에 화면 기록 권한이 있는 기계에서 확인, 기억한 권한을 지우는 화면(후속), 위치 좌표(W5b2), Return·Space·Esc 키 자체는 자동 시험 밖(비활성 앱에는 키 창이 없다 — 단추 클릭과 첫 초점으로 본다), iframe 이 청한 미디어 요청은 그 iframe 이 떠나도 주 프레임이 바뀔 때까지 남는다, 되풀이된 닫기·못 물음의 embargo 를 지우는 화면(후속) — **W5b2 구현됨**(위치): Chromium 의 위치 공급자는 CEF 에서 돌지 않아(macOS 위치 권한은 프로세스마다 매겨진다 — C6 실측) **Maru 가 CoreLocation 으로 좌표를 한 번 구해**(1 분 안의 좌표는 다시 쓴다, macOS 위치 창의 답은 1 분·좌표는 10 초까지 기다린다) 새 tag `geolocation`(maru → sidecar: 있음·위도·경도·정확도 — 범위 밖·NaN·무한대 거절)으로 보내면 sidecar 가 그 탭에 DevTools `Emulation.setGeolocationOverride` 를 걸고 **나서** 허용한다(허용 뒤에 걸면 기다리던 요청이 실패한다 — 실측). 좌표 없이 허용하면 sidecar 가 「없음」을 건다(옛 좌표가 나가거나 시한까지 기다리지 않게). Chromium 이 부를 때마다 다시 묻으므로 sidecar 는 저장된 허용이면 `remembered` 를 붙이고, maru 는 **이 실행 동안 자기 sheet 에서 허용받은 출처일 때만**(sidecar 의 표시만으로는 아니다 — 적대 검증) sheet 없이 좌표를 구해 답한다(보이지 않는 탭도 — 맡은 창이 닫히면 다른 창이 맡는다). 기록은 **(탭, 출처)** 에 묶고(장악된 sidecar 가 다른 탭 번호로 흉내 내도 sheet), 탭이 닫히면 지우고, 차단하면 그 출처를 모든 탭에서 빼며 좌표를 기다리던 같은 출처의 허용은 허용으로 답하지 않는다(늦은 허용이 나중의 차단을 덮지 않게). 위치 sheet 에는 「허용하면 Maru 를 다시 시작할 때까지 이 탭에서는 다시 묻지 않습니다」를 적는다. **보정 스크립트**(사용자 결정 2026-09-26): 같은 문서에서 위치를 다시 부르면 멈추므로(실측) 브라우저를 만들 때 DevTools `Page.addScriptToEvaluateOnNewDocument` 로, 위치를 허용할 때 `Runtime.evaluate` 로 지금 문서에도, `getCurrentPosition` 에 `maximumAge` 가 없으면 60 초를 채우는 작은 스크립트를 넣는다 — 그 문서의 좌표는 어차피 하나다. sidecar 는 같은 좌표를 다시 걸지 않고(다시 걸면 기다리던 요청이 오류를 먼저 받는다), 새 문서가 시작하면 「없음」으로 되돌린다(다른 출처로 옮겨 간 뒤 옛 좌표가 남지 않게). `watchPosition` 은 좌표 하나(갱신하지 않는다 — 사용자 결정 2026-09-26, 갱신하면 매번 오류가 먼저 간다). macOS 가 위치를 막으면 sheet 경로는 못 물음과 위치 서비스 안내, 기억된 요청은 허용과 「없음」(embargo 가 쌓이지 않게). sheet 에서 허용을 누르면 창의 대기열을 곧바로 푼다(macOS 창·좌표를 기다리는 동안 다른 탭의 대화상자가 멈추지 않게). Info.plist 위치 사용 목적, ABI v196. 판정: codec(좌표 범위·NaN·기억 표시는 위치만), `web_osr` 단위(기억 표시는 maru 가 허용한 출처만·좌표 먼저·맡은 창이 닫히면 맡음만 풀림·차단하면 기록에서 뺌), 판정자 위치 5(처음 허용에 좌표 · 다시 청하면 기억 표시와 새 좌표 — 다른 출처 `localhost` 는 표시 없음 · 같은 문서의 둘째 요청은 곧바로 같은 좌표(보정 스크립트) · 좌표 없이 허용·「없음」은 오류 2 · `watchPosition` 좌표 하나 · 차단은 기억), 시험기 위치 6(macOS 가 막으면 못 물음과 안내 · 위치 sheet 의 「다시 시작할 때까지 이 탭」 안내 · 허용하면 좌표 · 같은 페이지가 다시 청하면 sheet 없이 같은 좌표 · 새 문서에서 다시 청하면 sheet 없이 새 좌표 · 나중에 macOS 가 막으면 sheet·안내 없이 오류 2). **적대 검증(2026-09-26)**: 1 차 — sidecar 의 기억 표시 하나로 sheet 없이 위치가 넘어감(→ maru 가 스스로 기록한 출처만), macOS 위치 창에 시한이 없어 창의 sheet 가 멈춤, 덮어쓰기에 실패해도 걸었다고 표시(아주 작은 값이 형식 버퍼를 넘김), 시한이 지난 요청의 늦은 실패가 다음 요청을 끝냄, 위치 비트 판정이 양쪽에서 다름, 창을 닫으면 다른 창 탭의 기억된 요청까지 끝냄, 메인 스레드의 `locationServicesEnabled`, 다른 출처 음성 판정 없음. 2 차 — 늦게 온 허용이 나중의 차단을 덮음(→ 차단이 기다리던 허용을 거둔다), 동의 문구와 실제(재시작까지 기억)가 다름, 최고 정확도를 기다리다 10 초 시한과 겹침(→ 100 m·시한에 신선한 좌표면 그것), 한 번 허용한 출처를 다른 탭 번호로 흉내 내면 무제한 추적(→ (탭, 출처)), 넘침 경로가 maru 기록을 보지 않음, 같은 좌표를 다시 걸어 오류가 먼저 감, 시한 뒤 늦은 콜백(→ 매니저를 버린다), 이동 뒤 덮어쓰기가 남음, 주석. 그 뒤 실측 — 같은 문서의 둘째 요청이 멈춤(→ 보정 스크립트). 변이: 세 묶음 29 개(구현 뒤 · 1 차 반영 뒤 · 2 차 반영과 보정 스크립트 뒤) — 1 묶음 12 개 중 7 개 FAIL(빌드 실패 1 은 변이를 고쳐 2 묶음에서 FAIL, 살아남은 넷 중 「좌표 없는 허용에 「없음」 걸기」는 판정이 앞 단계의 남은 덮어쓰기에 기대 통과했다 — 순서를 바꿔 FAIL, 단위 시험 둘은 새 코드로 다시 돌려 FAIL), 2 묶음 9 개 중 7 개 FAIL, 3 묶음 8 개 중 5 개 FAIL(`web_osr` 단위 변이 셋은 시험 자체의 기대 프레임 수 실수로 원본에서도 실패하던 때라 무효 — 고친 뒤 다시 돌려 모두 FAIL, 하나는 변이 스크립트가 다음 줄의 FAIL 을 못 읽어 손으로 확인. 「새 문서마다 보정 스크립트 등록」은 주 프레임이 허용 때의 `Runtime.evaluate` 와 겹쳐 살아남았다 — iframe 판정을 더하자 그 등록이 `Page.enable` 없이 **아예 먹지 않았다**는 것이 드러나 고치고 FAIL 확인). **끝내 살아남은 넷**: 위치가 아닌 요청에 좌표를 걸기(페이지에 보이는 차이가 없다), sheet 에서 허용한 뒤 창 대기열을 곧바로 풀기(시험기의 `macos-location` 이 곧바로 답해 기다림이 없다), 새 문서에서 「없음」으로 되돌리기·같은 좌표 다시 걸지 않기(Chromium 이 같은 문서에서 다시 묻지 않고 탭마다 새 문서에서 되돌려져 다른 장치와 겹치는 방어 — 그 상황을 만들 수 없다). **남은 것**: CoreLocation 상태 기계(권한 창·시한·합치기·위임 콜백)는 자동 시험 밖(시험기는 `macos-location` 으로 대신 답한다 — 비활성 앱은 TCC 를 다룰 수 없다), sheet 에서 허용한 뒤 창의 대기열을 곧바로 푸는 것도 그 탓에 시험 밖, 앱을 다시 띄우면 maru 의 허용 기록이 비어 첫 위치 요청은 sheet 로 다시 묻는다, 실행 중에는 허용을 거둘 화면이 없다(후속), 판정자를 여러 번 돌리는 중 한 번 PASS 가 14 개로 셌다(틀림 없음 — 로그를 덮어써 원인을 못 봤다, CEF 로그가 같은 출력에 섞여 줄이 갈린 것으로 본다, 그 뒤 여섯 번은 15), 같은 문서의 좌표는 하나라 페이지를 다시 열어야 새 좌표, 첫 좌표 요청의 성공 콜백 **안에서** 곧바로 다시 부르면 보정 스크립트가 없는 원래 동작처럼 기다릴 수 있다, 릴리스 서명 entitlement(W7), 프로토콜 버전(W7 — 따로 설치될 때). **W5c 구현됨**(웹 알림 중계): Chromium 은 웹 알림을 OS 로 보내지 않는다(착수 전 실측 — 페이지는 `onshow` 까지 받지만 화면에 없다). 결정(사용자 2026-09-26): maru 알림을 누르면 그 탭으로 가고 페이지 `onclick` 도 부른다, 설정 `notifications.web`(기본 켜짐), 전달 경로는 DevTools 바인딩(`Runtime.addBinding`·`Runtime.enable`) 대신 **렌더러 처리기**(첫 구현의 적대 검증 — `Runtime.enable` 은 봇 탐지 신호이고, 실행 컨텍스트 표가 밀려나고, 옛 컨텍스트 번호로 누를 수 있었다). 구조: helper 가 CEF app 을 넘겨 렌더러 처리기를 둔다. `on_context_created`·그 microtask·렌더러 `on_load_start` 에서는 `Notification`·`ServiceWorkerRegistration` 이 아직 없다(실측 — Blink 가 secure context 인터페이스를 그 뒤에 설치한다) → 대리 스크립트는 sidecar 가 `Page.addScriptToEvaluateOnNewDocument` 로 넣고(Page 도메인은 위치 보정이 켠다), 렌더러는 네이티브 `send` 를 전역에 숨겨(열거되지 않게) 두며 대리 스크립트가 페이지 스크립트보다 먼저 꺼내 지운다. 숨기는 문서는 주 프레임까지 같은 프로세스로 이어지는 http(s)·불투명하지 않은 출처의 문서뿐이다(대리 스크립트가 돌지 않는 다른 사이트 iframe·그 안·`about:blank`·sandbox 에서 드러나지 않게). 대리 스크립트는 생성자·`showNotification` 을 Proxy 로 감싸고 판정·직렬화·누르기에 쓰는 내장(원래 `permission` getter 포함)을 먼저 쥔다. 알림은 CEF 프로세스 메시지로 sidecar 에 가고, sidecar 는 **그 프레임의 주소**로 출처를 정해 주 프레임과 같은 출처(Chrome 처럼 다른 출처 iframe 은 막는다)·Chromium 이 허용으로 기억한 출처만, 브라우저마다 10 초에 5 건, 제목 256·본문 1024 바이트로 넘긴다. maru 는 탭마다 8 건 대기, 제목 「출처 · 제목」, 방향·보이지 않는 문자를 빼고 원격 알림과 같은 정리, 보고 있는 탭이면 배너 없이 목록만. 누르면 sidecar 가 그 프레임에 보내고 렌더러는 문서 표식(무작위)이 같을 때만 한 번 누른다. sidecar 가 죽으면 대기 알림·누를 기록을 버린다. ABI v197. 판정: codec(왕복·방향·빈 출처·번호 0), `web_osr` 단위(탭 상한·다른 탭 이름으로 누르기·죽으면 버림), `notification` 단위(글자 경계 자르기·방향 문자·깨진 머리 바이트), 판정자 알림 9(중계와 누르면 `onclick` · 누르지 않고 둔 옛 문서의 알림을 새 문서에서 눌러도 무동작 · 두 번 눌러도 한 번 · 미리 그린 문서의 전역(이 CEF 는 미리 그리지 않았다 — `a0`, 회귀 감시) · 허용하지 않은 출처가 권한을 속여도 안 옴과 `maru` 전역 0(그 문서·`about:blank`·다른 사이트 iframe 과 그 안·sandbox·같은 출처 iframe) · 포트만 다른 다른 출처 맨 위 문서 안의 허용한 출처 iframe(iframe 은 `granted` 를 본다) · CSP sandbox 불투명 출처 · 열 개에 다섯 · 첫 문서), 시험기 3(maru 알림 제목 「출처 · 제목」·누르면 탭과 `onclick`·`notifications.web = false`). **적대 검증(2026-09-26)**: 재설계 1 차 — 누르기 중 페이지가 프레임을 떼면 렌더러 UAF(→ 컨텍스트·함수를 따로 잡는다), 숨긴 함수가 OOPIF·`about:blank` 에서 보임, 불투명 출처가 주소로 통과, 다른 출처 iframe 허용, 처리기 객체가 `Object.prototype` 을 이어 페이지가 트랩을 심음, 두 번 누르면 두 번, 히스토리 넣기 실패 때 앞 항목에 표식, 빈도 표 64 칸 고갈, 여러 바이트를 통째로 넘겨 방향 문자가 살아남음, 주석. 2 차 — 한 번만 누르기로 옛 알림 판정이 빈 판정이 됨(→ 누르지 않은 옛 알림으로), OOPIF 안의 같은 사이트 iframe 에 숨긴 함수(→ 주 프레임까지 부모를 따라감), 미리 그린 문서(→ 판정), 손으로 한 enter/exit(→ `execute_function_with_context`), 권한 getter 위조·`then`·`dispatchEvent` 등 페이지가 바꿀 수 있는 내장(→ 먼저 쥔다), `self.origin` 을 `eval` 로(→ 네이티브 getter), 해제된 프레임의 메시지. 실측 중: 판정 서버가 speculation rules 의 미리 연결(요청 없는 소켓)에 묶임(→ 받은 소켓 읽기 시한 1 초), WIP 의 `web_osr` 단위 시험 기대 프레임 수 실수(그때 돌려 보지 않았다). 변이: 세 묶음 — 1 묶음(재설계 뒤) 17 개 중 10 개 FAIL·넷 생존·빌드 실패 둘(쓰지 않는 변수 — 고쳐 2 묶음에서)·하나는 변이 스크립트가 시험 출력의 깨진 UTF-8 에 죽어 2 묶음에서 다시(FAIL), 2 묶음(1·2 차 반영 뒤) 14 개 중 8 개 FAIL, 3 묶음 — 살아남은 「주 프레임과 같은 출처만」에 판정을 더해 FAIL. **끝내 살아남은 여섯**: 렌더러의 문서 표식 비교(이 Chromium 은 문서마다 프레임 식별자가 바뀌어 옛 알림은 sidecar 에서 프레임을 못 찾는다 — 식별자가 남는 경우의 방어), 렌더러의 불투명 출처·주소 검사(sandbox iframe 은 다른 프로세스라 부모 검사가, 주 프레임 CSP sandbox 는 원래 권한 getter 가 먼저 막는다), sidecar 의 권한 확인과 대리 스크립트의 원래 권한 getter(서로 가린다 — 둘을 함께 없애면 FAIL), 렌더러의 초당 10 회(브라우저 쪽 5 건이 가린다), 렌더러의 컨텍스트 동일성(격리 세계가 없다). **남은 것**: 서비스 워커 안에서 띄운 알림·닫힌 페이지의 푸시, 다른 사이트 iframe 의 알림(Chrome 도 막는다), 스크립트로 만든 `about:blank` iframe·`blob:`·전용 워커의 원래 `Notification`, 첫 문서 경쟁(등록이 첫 커밋보다 늦으면 그 문서는 중계되지 않고 숨긴 함수가 보인다 — 판정은 매번 이겼다, 위치 보정과 같다), 드러나는 차이(`toString` 모양·`prototype.constructor`·누르기 `isTrusted` false·권한이 있을 때 `then` 의 species 조회), `tag`·`close()`·아이콘·액션 없음, 렌더러 문서 표 128 칸(프로세스 공유 — 차면 그 문서는 중계하지 않는다), 격리 세계가 생기면 다시 볼 것 |
| **W6** 팝업·툴팁·드래그·메뉴 | `<select>` 팝업(D4), 툴팁, 드래그 시작·드롭, 우클릭 메뉴(D5) — IME 후보창 위치는 C5·W4 | 팝업 표시·선택·닫힘 복원, 드래그 콜백 도착 |
| **W7** 배포 | formula, 매니페스트(formula 설치물의 버전·ABI 확인용), 설치 감지, 버전 올림 절차, dmg·Intel 공급(D8), CEF·Chromium 라이선스 동봉과 attribution([third-party 라이선스](../third-party-licenses.md)) | 깨끗한 기계에서 설치 → 실행 → 샌드박스 판정자 — **W4d① 이 넘긴 것**: maru 는 `HOMEBREW_PREFIX`·`/opt/homebrew`·`/usr/local` 의 `opt/maru-chromium/libexec/maru-web-host` 와 개발용 `MARU_WEB_OSR_DIR` 을 **실행한다** — 둘 다 환경변수로 실행 파일을 고를 수 있으니 서명 검증은 두 경로 모두에 건다(배포 빌드에서는 `MARU_WEB_OSR_DIR` 를 막는 것도 검토) — **W5b 가 넘긴 것**: 릴리스 서명은 hardened runtime 이라 Maru.app 에 `com.apple.security.device.camera`·`device.audio-input`·`personal-information.location`(W5b2) entitlement 가 있어야 macOS 가 카메라·마이크를 묻는다(없으면 `AVCaptureDevice.requestAccess` 가 묻지 않고 거절할 수 있다) — sidecar 를 설치할 때 실제로 장치에 닿는 프로세스(host·helper)의 entitlement 도 같이 본다 · 제어 채널 `wire.version` 을 올린다(W5a·W5b 가 tag 와 본문을 늘렸다 — maru 와 sidecar 가 따로 설치되면 hello 에서 갈라야 한다) |
...
프로세스 비용, Chromium 탭의 macOS 텍스트 편집 단축키(⌃A·⌃E·⌃K 등 — W9 의 CDP 편집 명령과 함께, 사용자 결정 2026-09-24), 추가 마우스 버튼(뒤로·앞으로)의 터미널 쪽 동작 — W4b 부터 Chromium 탭 본문 밖에서는 무시한다(전에는 왼쪽
