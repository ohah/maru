...
| **W4d②** 앱 안 시험기 | 앱 안 시험기(25 항목 — §13.1 「pane 안 실측」)를 opt-in smoke 로 저장소에. 키는 `postToPid` 로 **진짜 NSEvent 경로**(keyDown·performKeyEquivalent·메뉴)를 탄다(앱이 비활성이어도 닿는다 — W4d 착수 전 실측), 커서·후보창 위치는 앱 안에서 잰다. **진짜 macOS 입력기(한글)는 maru 가 맨 앞 앱일 때만 조합한다**(실측 — 입력 문맥 입력기를 바꿔도 비활성 앱에서는 ASCII 로 들어왔다) → 자리를 비울 때 돌리는 별도 opt-in 명령으로 maru 를 앞으로 올리고 시스템 키보드 이벤트로 친다(사용자 결정 2026-09-25) | 시험기 항목(`<select>` 두 항목은 W6) |
