...
- 첫 프레임 전 입력은 렌더러가 버린다(W4a 실측) — 새 탭은 첫 장이 보이기 전의 클릭을 잃는다.
...
| **W4a** 입력 메시지·sidecar | codec 입력(마우스·휠·키·IME 조합/확정/마침/취소·편집 명령·capture lost, sidecar → maru 알림: 커서·IME 조합 사각형), sidecar 의 CEF 입력 호출, 우클릭 네이티브 메뉴 억제(maru 가 그리는 메뉴는 W6) | 판정자로 실제 페이지에 닿는지 — **구현됨**: codec 입력 9 종 + 알림 2 종(닫힌 필드 — 수식자 예약 비트·좌표 절댓값 64K·클릭 수·범위·enum, 손상 바이트 전수), sidecar `input.zig`(닫히는 중이거나 없는 브라우저의 입력은 조용히 버린다 — 파괴와 입력의 정상 경합이라 실패로 알리면 마우스 이동마다 쏟아진다), `input_map.zig`(수식자 비트·사각형 합치기 — CEF 없이 기본 test, 비트 값은 sidecar 빌드에서 헤더와 comptime 대조), 우클릭 메뉴 억제(메뉴 항목을 비운다 — 없으면 sidecar 가 네이티브 메뉴 창을 띄운다, 변이로 확인). 판정: `mise run web-sidecar-judge` 에 입력 14 판정(전체 51 판정 통과, 입력만 도는 `--input` 3 회 연속 14/14) — 클릭 좌표·버튼·클릭 수, raw_down → char → up 타이핑, Ctrl+E, IME ㅇ→아→안·조합 사각형·확정, 전체 선택·지우기·되돌리기, blur, 우클릭(창 0)·가운데 클릭, 버튼 비트를 실은 끌기 선택, 커서 hand/ibeam, leave, 휠, 없는 브라우저 입력, 포커스를 가진 둘째 브라우저로 새지 않음. **실측**: 첫 프레임 전 입력은 렌더러가 버린다(페이지 준비 직후 클릭이 사라졌다 — 판정 페이지는 `requestAnimationFrame` 두 번 뒤에 준비를 알린다), DOM `code` 는 `native_key_code` 에서, `key` 는 `character`·`unmodified_character` 중 하나에서 오고 `windows_key_code` 는 macOS 에서 쓰이지 않는다. 변이 13 개 중 11 개 FAIL — 살아남은 둘(`unmodified_character` 만 뺌·`windows_key_code` 뺌)은 판정 빈틈이 아니라 macOS 에서 효과 없는 필드다(둘을 함께 뺀 변이와 `native_key_code` 를 뺀 변이는 FAIL) |
| **W4b** 포인터 | C5 의 마우스 게이트·제스처 주인·포커스 주인(클릭 → pane 활성화)·창, 휠, hover·leave, 커서(`cursor_changed` → NSCursor) — 라우팅은 Zig(`AppSession.mouse`·hover·휠)가 든다 | 라우팅 판정은 Zig 순수 시험, 앱 경로는 스모크 |
| **W4c** 키보드 | C5 의 키 라우트(`web_key_route` — 메뉴 먼저·`app_action` 은 `dispatch_web_app_action`)·모달 에지(capture lost·조합 마침·포커스), 키 이벤트(raw_down·char·up), IME(`NSTextInputClient` 목적지 분기 — 기존 `ime_insert`·`ime_marked` 를 Zig 가 OSR 로 돌린다)·후보창 위치(`ime_range`), 메뉴 편집 액션 | 라우팅 판정은 Zig 순수 시험, 앱 경로는 스모크 |
