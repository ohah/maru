...
| W5.5 | **Windows 상대 경로 링크 감지** — `filePathSpan`의 `home_path`·`dot_relative`·`bare_relative` 세 갈래도 `\`를 받게 한다(`.\x`·`..\x`·`src\x`·`~\x`). 절대 갈래만 고친 W1.5의 후속이고 **더 흔한 형태**다. `dot_relative`·`home_path` 두 갈래를 **닫았다** — `path_shape.detectableRelativePrefixFor`가 OS를 인자로 받고, 규칙은 오탐 스윕이 찾은 세그먼트 조건 하나다(계약 §5.2 ⒜). POSIX 철자는 회귀 0. `bare_relative`는 **구조적으로 규칙이 없어**(이스케이프 출력과 세그먼트 모양이 동일) 켜지 않았다 — 남은 갈래는 "Windows에서만 받고 오탐 감수" 뿐이고 그건 정책 결정이다 | **부분 완료** |
