...
| W7.2c-2 | **화면에 올린다** — `metal_frame.zig`의 `buildNativeCellsSplit`·`buildNativeRasterUploads` 투영을 소비해 `NativeMetalCell` → `d3d11_cells.Cell`로 옮기고, 아틀라스 **부분 업로드**(`UpdateSubresource`)와 eviction을 물린다. 그러면 실제 터미널 화면이 뜬다 | 미착수 |
