...
| W2.5 | **Windows 기본 셸 + config OS 분기** — `resolveInteractiveShellFor(kind)`가 OS 갈래를 갖는다(`MARU_INTERACTIVE_SHELL` → pwsh 7 → 5.1 → `%COMSPEC%` → cmd; 계약 §3.1a). 후보 목록은 **OS와 `%COMSPEC%`을 인자로 받는 순수 함수**라 테스트가 두 갈래를 모두 돈다. config에 **일반 OS 접미 메커니즘**(`shell.command.windows`·`font.size.macos` — 아무 키에나)과 **`shell.windows-shell`**(`powershell`|`cmd`, 종류 선택)을 넣었다. 이로써 계약 §8의 "셸 설정의 OS 분기"가 닫혔다. **config 값을 실제 spawn에 배선하는 것은 W7**(Windows 호스트) — 지금은 `main.zig`가 config를 읽지 않는다 | 완료 |
