...
| W5.5 | **Windows 상대 경로 링크 감지** — `filePathSpan`의 `home_path`·`dot_relative`·`bare_relative` 세 갈래도 `\`를 받게 한다(`.\x`·`..\x`·`src\x`·`~\x`). 절대 갈래만 고친 W1.5의 후속이고 **더 흔한 형태**다. **오탐 스윕을 끝냈다**(계약 §5.2 ⒜): `dot_relative`·`home_path`는 세그먼트 규칙으로 오탐 0·미검출 0이라 바로 갈 수 있고, `bare_relative`는 후보 네 계열이 전부 깨져 **정책 결정이 남는다**. 구현할 때는 제품 `linkSpanInWord`를 직접 몰아 같은 집합을 다시 돌린다 | 미착수 |
...
| W5 | **셸 통합 주입** — cmd `PROMPT`, PowerShell `prompt` 오버라이드(계약 §3.3). 사용자 프롬프트 보존. **메커니즘은 실측으로 다 확인됐다**(계약 §6): 두 셸 모두 OSC 9;9·133;A를 내고 cwd를 정확히 보고하며, pwsh는 사용자 프로필이 `prompt`를 정의해도 우리가 이긴다. **선행 결정은 닫혔다**(계약 §4.2a): seam이 갈리는 축은 OS가 아니라 메커니즘이라 `shell_integration`을 union(`assets_dir` / `inline_injection`)으로 바꾼다 — WSL·git-bash가 Windows 호스트인데 파일 기반이기 때문이다. **이 슬라이스가 그 전환까지 한다**: `SpawnRequest`와 RPC 짝(`server.zig`·`remote_runtime.zig`·`runtime_manager.zig`). wire 키 `"zdotdir"`는 파일 갈래에 그대로 남아 깨지지 않는다 | 미착수 |
...
| W7 | **Win32 호스트 + 렌더 백엔드** — 창·입력·IME·클립보드. **선행 정리 2건**(계약 §4): `live_pty.childPid()`가 내는 `std.c.pid_t`가 Windows에서 `*anyopaque`라 백엔드의 `u32`와 안 맞는다(중립 레이어의 POSIX 타입 누수), 그리고 `PreparedAdoption` 계열 exec-restore 표면이 Windows 백엔드에 없다. 둘 다 지금은 Windows에서 컴파일되지 않아 잠복 상태다. **선행 결정 2건**(GPU 백엔드, 웹뷰 합성 모델)이 계약 §8에 있다. 파일 트리 백엔드를 이식할 때 **루트 스트라이핑 두 곳**(`parent_path[root.len + 1 ..]`)을 `endsWithSep`로 함께 고친다 — 루트가 `/`나 `C:/`면 첫 세그먼트가 잘린다(계약 §5.2 ⒝, 이 커밋 이전부터 있던 버그) **config에서 온 경로를 정규화하는 자리도 여기서 정한다** — 로더가 파싱하며 할지 소비처마다 할지가 결정 사항이고, 지금은 `workspace.root = C:\proj` 같은 값이 역슬래시를 단 채 통과한다(계약 §5의 알려진 공백) | 미착수 |
