...
| W7.2c | **실제 앱 프레임 배선** — `app.host`의 프레임을 표시 경로에 연결한다. `metal_frame.zig`의 `buildNativeCellsFromGlyphQuads`·`buildNativeRasterUploads` 투영을 소비하고, `GlyphAtlas`의 업로드 delta·eviction을 물린다. 여기서 §2a가 걸어 둔 이식성 검증(중립 렌더러 계약이 Metal 한 구현에만 맞춰져 있지 않은가)의 답이 나온다 | 미착수 |
| W7 | **Win32 호스트 나머지** — W7.4(입력·IME·클립보드)·W7.5(파일 트리 루트 스트라이핑·config 경로 정규화). 선행 정리는 W7.0, 창은 W7.1, 표시 경로는 W7.2a, 셀 드로우는 W7.2b, 폰트는 W7.3에서 닫았다. **선행 결정 1건**(웹뷰 합성 모델)이 계약 §8에 있다 — GPU 백엔드는 **D3D11 + DXGI로 정해졌다**(계약 §2a). 파일 트리 백엔드를 이식할 때 **루트 스트라이핑 두 곳**(`parent_path[root.len + 1 ..]`)을 `endsWithSep`로 함께 고친다 — 루트가 `/`나 `C:/`면 첫 세그먼트가 잘린다(계약 §5.2 ⒝). **config에서 온 경로를 정규화하는 자리도 여기서 정한다** — 지금은 `workspace.root = C:\proj` 같은 값이 역슬래시를 단 채 통과한다 | 미착수 |
