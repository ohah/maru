...
| W5 | **셸 통합 주입** — cmd `PROMPT`, PowerShell `prompt` 오버라이드(계약 §3.3). 사용자 프롬프트 보존. **메커니즘은 실측으로 다 확인됐다**(계약 §6): 두 셸 모두 OSC 9;9·133;A를 내고 cwd를 정확히 보고하며, pwsh는 사용자 프로필이 `prompt`를 정의해도 우리가 이긴다. **선행 결정 1건** — `shell_integration_dir`이 Windows에서 디렉터리가 아니라 그 필드의 뜻을 먼저 정해야 한다(계약 §4.2의 미결 상자) | 미착수 |
