...
배열의 **자리**를 값으로 드는 필드(인덱스·오프셋)를 새로 만들었다면, 그 배열을 **움직이는 코드**를 전수로 찾아 각각에 판정자를 세웠는가? (아래 "합류 규율")
```

## 합류 규율 — 각자는 초록인데 합치면 깨지는 것 (2026-09-08 사용자 결정)

**두 갈래가 각자 판정자까지 갖추고도, 합치는 순간 깨지는 결함이 있다.** 2026-09-07~08 하루에 활동 뷰
한 축에서만 **넷**이 나왔고, 넷 다 각 PR 의 CI 는 초록이었다. 찾은 것은 언제나 **합쳐 놓고 읽어 본 사람**이다.

| 무엇이 부딪혔나 | 형태 |
|---|---|
| 퇴출이 배열을 압축하는데 다른 축이 그 **인덱스**를 들고 있었다 | 배열을 움직이는 코드 ↔ 자리를 값으로 드는 소비자 |
| 새 UI 줄이 **자리는 빼 가고 그리지는 않아** 클릭이 그 줄에서 샜다 | 자리 계산 ↔ 그리기 |
| 한쪽이 「본문 없음」으로 되돌아가는데 상대는 「있다」로 가정했다 | 실패 표현 ↔ 소비자의 전제 |
| 안내가 「필터의 **모양**」으로 갈려, 두 종류를 담는 필터에서 한쪽 잘림을 말하지 않았다 | 분류 축 ↔ 담는 내용 |

규율은 둘이다.

1. **자리를 값으로 드는 필드를 새로 만들면, 그 배열을 움직이는 코드(압축·정렬·뒤집기·퇴출)를 전수로
   찾아 각각에 판정자를 세운다.** 「소비자를 재고에 적는다」로는 부족하다 — 적는 것을 잊으면 그만이고
   검증되지 않는다. **찾는 방법**이 규율에 들어가야 실효가 있다.
2. **표본을 둘 이상 둔다.** 하나짜리 표본은 「자기 자신을 가리키는」 어긋남을 감춘다 — 항목이 하나면
   자리를 잘못 골라도 우연히 맞고, 뮤테이션이 살아서 빠져나간다. 실제로 하루에 **세 번**(칩 판정자,
   접기의 뒤집기 remap, 펼침의 명령 자리) 같은 형태로 걸렸다.

⚠️ **판정자가 깨졌다고 제품을 되돌리기 전에, 그 판정자의 기준선이 아직 참인지 먼저 묻는다.** 합류가
바꾸는 것은 코드만이 아니다 — 「닫힌 화면에서 잰 자리」처럼 판정자가 굳혀 둔 전제도 함께 낡는다.
그때 고칠 것은 제품이 아니라 기준선이고, 옮기고 나면 단언이 오히려 강해지는 경우가 많다.
