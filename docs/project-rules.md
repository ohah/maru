...
- **여러 커밋을 리베이스했으면 중간 커밋도 본다 — 문법이 아니라 게이트로 본다.** 앞 판은 여기에
  ...
  중간 커밋을 체크아웃해 **`zig build check-boundaries` 까지** 돌린다. **CI 가 그 절반을 대신한다** —
  `tools/ci/per-commit-boundaries.sh` 가 PR 의 중간 커밋 중 **원장이 참조하는 파일을 건드린 것**만 골라
  게이트를 돌리고, 빨간 커밋의 SHA 와 이유를 찍는다. 무관한 커밋은 건너뛰되 **건너뛴 수를 출력한다** —
  조용한 축소는 "전부 봤다" 로 읽힌다. 손으로 확인하는 규율이 없어지는 것은 아니다: CI 는 PR 범위만 보고,
