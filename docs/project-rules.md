...
- **`count` 와 `digest` 는 값어치가 다르다 — 전수로 쟀다.** 원장을 건드린 커밋 236 개 중 `count` 가 실제로
  ...
  **구현의 함정은 따로 있다** — "반사 자리" 를 무엇으로 정의하느냐가 **또 하나의 규칙**이 되고, 그 규칙이
  ...
  간다면 그 정의를 **한 곳이 소유**하는 것이 전제다 — 지금 그 규칙(`@field` 토큰 위치 + test 블록 마스킹)은
  `tests/boundary/imports.zig` 안에 인라인으로 박혀 있어, **규칙을 읽으려면 게이트 구현을 읽어야 한다.**
  꺼내는 일은 좁히기와 무관하게도 값어치가 있다.
