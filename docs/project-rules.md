...
- 코드가 문서화된 구조, 명령, 테스트 경로, artifact 포맷과 달라지면 같은 PR에서 문서를 함께 수정한다.
- 문서를 바꾸지 않아도 되는 PR이라면, PR 설명에 왜 문서 변경이 필요 없는지 적는다.
- 실제 구현이 문서와 다른 방향을 요구하면 먼저 사용자에게 보고하고, 합의된 뒤 문서와 구현을 같이 바꾼다.

## 전략 유지

- 모든 PR은 기존 전략에 지장이 없는지 평가해야 한다.
- GitHub PR 본문은 `.github/pull_request_template.md`를 사용한다.
