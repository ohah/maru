...
- 리베이스 뒤에는 **digest 원장을 다시 수렴시킨다**(`tests/boundary/external_source_digests.zig`). 안 하면 로컬은
  통과하고 CI 만 깨진다.
- **해시는 잘린 것을 쓰지 않는다.** 이 원장은 여러 세션이 같은 두세 항목(`src/i18n.zig`·`app_session.zig`)을
  번갈아 건드려 리베이스마다 충돌한다 — 하루에 다섯 번 난 날도 있다. 충돌을 손으로 풀 때는 게이트가 내는
  **전체 64자** 값을 쓴다. 도구 출력이 축약형을 보여 주던 시절에 잘린 해시를 커밋할 뻔한 사례가 있었고,
  그 경우 게이트가 다시 빨개지는데 원인이 "해시가 틀렸다" 로만 보여 찾기 어렵다.
