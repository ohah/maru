...
src/
  ...
  shutdown_wire_contract.zig  종료 backend와 앱 조합 계층이 함께 쓰는 포인터 없는 wire 권위 값
    ...
    app_session/        app_session.zig에서 목적별로 떼어낸 그룹 구현(docs/app-session-decomposition.md §4.1 F 시리즈).
                        ...
                        **서로 다른 이름**의 수다. 그 기준으로 2026-08-18에 58개, 2026-08-24에 73개다(전에 적힌
                        「57」은 세는 법을 안 적어 재현되지 않았고, 같은 기준으로 다시 잰 값으로 바꾼다. 그날
                        tick에서 마저 옮겨 온 상태바 캡처 게이트 넷은 그중 +4이고, 나머지 증가분은 그 사이 다른
                        작업이 늘린 것이다). 함수 이름은
