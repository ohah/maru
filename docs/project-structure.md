...
- **판정자 이름에 작업 슬라이스 ID 를 쓰지 않는다.** `session_host_2c3d_c3_3b2b3_boundary.zig`(옮겨져
  ...
  **규칙만 적어 두면 안 지켜지므로 원장을 둔다**: [`tests/boundary/judge_names.zig`](../tests/boundary/judge_names.zig)
  가 지금 있는 것을 목록으로 못 박고, 목록에 **없는** 파일이 슬라이스 ID 이름이면 실패한다.
  ...
  **한 번에 한 주제씩 옮긴다.** 108(최초) → 106(reconnect worker 둘) → 103(kernel cwd 셋) →
  99(release adapter P5d 넷) → 92(센티넬 일곱) → 68(CR3a-2c3 스물넷) → 63(CR3b 다섯) → 58(B3 다섯) →
  53(CR6e 재접속 다섯) → 47(P4 관측·유휴 비용 E2·E3 여섯) → 44(CR2e 재접속 단위 테스트 셋) →
  41(P5c3d 같은 메이저 호환성·attach 제품 E2E 셋) →
  37(CR4 재접속 Client 교체 넷) →
  29(CR5 호스트 전체 재접속 트랜잭션 여덟) →
  26(CR6a·b 복구된 세션 셋) →
  24(CR6f output wake · idle soak 둘) →
  22(CR3c 세대 승격 · Client 회수 둘) →
  18(CR3a-2d·2e 세대 해제 넷) →
  ...
  **예외 — 라벨이 «기계로 일하면» 옮긴다(2026-09-22, F3 계열).** 아래 규칙의 근거는 「거기서 그
  ID 는 **이력**이다」인데, 이력이 아니라 **선택 키**인 라벨이 있다. F3 계열이 그랬다:

  ```zig
  .filters = &.{"f3c1"},                                   // 이 게이트는 이 이름이 든 테스트«만» 돈다
  if (startsWith(literal, "f3c1 ")) behavior_tests += 1;   // 센티넬이 접두로 «센다»
  pub const f3c1_contract_version: u16 = 2;                // 제품 공개 선언
  // MARU_F3D_PRODUCT_ORCHESTRATION_BEGIN                  // 센티넬이 잘라내는 구간 마커
  ```

  이런 라벨은 주제 접두로 옮긴다. **단 한 번에** — 테스트 선언 이름 · 빌드 필터 · 센티넬/판정자가
  요구하는 문자열 · 제품 선언과 마커가 **함께** 움직여야 한다. 하나라도 어긋나면 필터가 덜 고르고
  게이트가 **조용히 얇아진다**(그래서 센티넬이 있다).

  | 단계 라벨 | 테스트 접두 | 그 게이트가 고르는 수 |
  | --- | --- | --- |
  | F3c0 | `typed control admission` | 12 |
  | F3c1 | `completed drain` | 11 |
  | F3c2 | `semantic take` | 25 |
  | F3d | `whole turn` | 6 |
  | F3e | `hostile revoke` | 6 |

  **접두는 저장소 전체 `test` 선언 12,803 개에 대해 충돌 0 임을 확인하고 골랐다** — 필터는 부분
  문자열이라 겹치면 게이트 «내용» 이 조용히 바뀐다. 처음 고른 `typed control` 이 CR3a 테스트
  하나와 겹쳐 `typed control admission` 으로 좁혔다.

  **일괄 치환이 원리상 못 잡는 것 넷**(실제로 밟았다):
  ⑴ 점 뒤 멤버 접근(`pump.f3c1_contract_version`) — 선언만 바뀌고 참조가 남아 **컴파일이 깨진다**(싸다).
  ⑵ **라벨이 둘 달린 이름**(`test "f3c0 f3c1 …"`) — 앞 라벨만 바뀌어 뒤 게이트에서 **조용히 빠진다**.
  실제로 11 → 10 이 됐고, **게이트가 고른 수를 전후 대조해서만** 잡혔다. 이 셋이 유일하게 조용한 실패다.
  ⑶ 판정자 이름 속 산문(`one f3d product callsite`). ⑷ 문서가 «지금 기계» 를 설명하는 자리
  (`compile-time \`f3c0\` filter`) — 이건 이력이 아니라 사용법이라 함께 고쳐야 한다.

  **검증은 「전후 개수가 같다」로 한다.** 컴파일도 테스트 실행도 「덜 고른 게이트」를 못 잡는다.
  옛 라벨과 새 접두를 **같은 잣대(부분 문자열)** 로 양쪽 트리에서 세어 맞춘다.

  **문서 산문과 제품 주석의 F3 라벨은 그대로 뒀다** — 거기서는 이력이 맞다. 위 표가 그 라벨에서
  테스트로 가는 길을 잇는다.

  **테스트 이름과 계획 문서의 슬라이스 ID 는 (그 밖에는) 건드리지 않는다.** 거기서 그 ID 는 이력이다 —
...
src/
  ...
  platform/             OS별 process/window/input bridge
    ...
    macos/web_sidecar/  웹 OSR sidecar(W1b — plans/web-osr-backend.md). `host_main.zig`(`maru-web-host` — CEF 브라우저 프로세스, 샌드박스 밖)·
                        ...
                        `profile.zig`(0700·소유자·ACL·링크 거절·백업 제외)·앱 쪽(W3b): `../web_osr.zig`(sidecar 띄우기·재시작 예산·브라우저 기록·알림 적용 — lsp_process 선례)·`session/web_osr_plan.zig`(창 배치 → sidecar 명령, 순수)·`session/web_osr_view.zig`(보일 링 고르기·GPU 소비자 규칙, 순수)·`session/web_osr_input.zig`(W4b — 본문 hit·divider 잡는 띠 제외·DIP·수식자·휠, 순수)·픽셀 링(W2): `ring_producer.zig`(sidecar — 복사·맞바꾸기·세대)·`ring_receiver.zig`(maru —
...
**이관은 진행 중이다** — 빌드 소스를 문자열로 재던 145자리 중 **61자리**가 뷰로 갔고 84자리가
남았다. 옮긴 결은 넷이다: 「이 스텝이 있는가」 31 · 매달기 13 · 실행 인자 8 · 필터 4 · 환경변수 5.
남은 것의 결은 이렇다: 파일 경로 13 · 제어 흐름과 선언 11 · 실행·인자 8 · 박힌 셸·정규식 5 ·
매달기 5 · 환경변수 3. **제어 흐름·선언과 박힌 셸·정규식 16자리는 안 옮긴다** — 그건 빌드
「그래프」가 아니라 스크립트 본문이라, 담으려면 AST 전체를 노출해야 한다.

**「뜻이 다를 것 같다」로 미루지 않는다 — 재 본다.** 환경변수 자리를 처음에는 「문자열은 이름이
어디서든 나온 수를 세니 뷰의 `envs` 와 뜻이 다르다」며 미뤘는데, 여덟을 실제로 재 보니 **일곱이
정확히 같았다**(그 이름이 빌드 소스에 나오는 자리가 전부 `setEnvironmentVariable` 이었다). 미룬
근거가 측정이 아니라 짐작이었다. 옮길 수 있는지는 짐작이 아니라 두 값을 나란히 찍어서 정한다.

**안 옮기는 이유가 하나 더 있다 — 건초더미가 «부분 구간»인 자리.** `between(build, "…", "…")` 로
잘라낸 조각을 세는 판정이 여럿이고(`cr2`·`cr6b`·`cr6d`), 뷰는 전역이라 그대로 바꾸면 **범위가
조용히 넓어진다**. 실제로 첫 일괄 치환이 그걸 놓쳐 세 자리를 잘못 바꿨고, 건초더미가 「직접 읽은
빌드 소스」인지 검사해서 걸러 냈다. 옮길 때는 **무엇을 세는지 먼저 실측하고** 값이 같은 것만 옮긴다.
