...
**IETF 인터넷 초안은 다시 받는다.** 초안은 리비전이 올라가며 문장이 바뀌고, 로컬 사본은 받은
날짜에 멈춰 있다 — 실측으로 `draft-ietf-sshm-strict-kex` 사본이 1년 묵은 `-00`이었다. 스크립트가
datatracker에 현재 리비전을 물어 받으므로, **초안에서 유도한 코드를 손대기 전에 한 번 돌린다**.
`| 명세 |` 표에 로컬 경로를 적었다면 그 파일은 **반드시 이 스크립트로 받아진다** — 받아지지 않는
경로를 적으면 그 인용은 아무도 확인할 수 없다(실제로 chacha20 초안이 그랬고, 그 탓에 K_1/K_2
명명을 두고 틀린 지적이 나왔다).
...
| RFC 4251~4254 (SSH) | SSH 아키텍처·인증·전송·연결 프로토콜 | `references/specs/rfc4251-ssh.txt` … `rfc4254-ssh.txt` | https://www.rfc-editor.org/rfc/rfc4253.txt |
