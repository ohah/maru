...
- native shaper 결과는 먼저 macOS `coretext_font.zig` 같은 platform adapter를 거쳐 `ShapedGlyphRecord`로 정규화한다. smoke native ABI record는 `coretext_probe.zig`가 `coretext_font.zig` 입력으로 바꾼다. macOS `coretext_shaper.zig`는 이 record 배열과 `ShapedGlyphSurface`를 받아 `GlyphRunList`를 만든다. 이 경계는 `.notdef`/space처럼 atlas 후보가 아닌 record를 걸러내고, CJK/emoji width, fallback 여부, color glyph kind, font size/scale cache key를 `GlyphRunList`로 전달하며, 제품 경로에서 `DrawList`의 size/cursor/dirty/overlay metadata가 사라지지 않게 한다.
- native shaper가 선택한 drawable glyph의 font face는 `FontIdentityRegistry`를 거쳐 안정적인 renderer `FontId`로 바꾼다. glyph id는 font-relative 값이라, shaping 때 쓴 face identity와 rasterizer가 그릴 face identity가 갈라지면 wrong-glyph 버그가 난다. 공백처럼 rasterizer에 도달하지 않는 record는 registry count에 넣지 않는다.
...
- 실제 Metal device 생성.
- 실제 AppKit 창 위 CAMetalLayer `coretext_probe.zig -> coretext_font.zig -> coretext_shaper.zig -> RendererState -> GlyphFrame -> GlyphQuadFrame/GlyphRasterFrame -> coretext_raster.zig` 제품 atlas sampling smoke(`mise run macos-metal-smoke`). 이 smoke는 native cell이 atlas slot id와 placement 후보를 받았는지 `renderer_atlas_slot_placement`로, shader UV 준비가 됐는지 `renderer_glyph_uv_ready`로, renderer upload byte/skip 계약이 준비됐는지 `renderer_glyph_raster_ready`와 raster skip count로 남긴다. 같은 run에서 CoreText 기반 제품 `GlyphRasterFrame.uploads/pixels`를 Metal atlas texture에 올리고 readback byte 비교를 통과해야 `product_atlas_uploaded=true`가 되며, source raster의 non-clear texel이 drawable readback에서 같은 색으로 보이고 `atlas_sample_missing_cells=0`일 때만 `product_atlas_sampled=true`와 `glyph_text=true`가 된다. upload가 0개인 all-skip frame은 Metal upload/readback 실패로 오기록하지 않고 visible cell의 sample source 누락으로 분리한다. 다만 이 경로는 아직 실제 `TerminalCore -> DrawList` 입력을 CoreText runtime으로 shape하지 않고, probe-derived surface를 쓴다.
- 실제 CoreText font resolve/glyph run/`coretext_probe.zig -> coretext_font.zig -> FontIdentityRegistry -> coretext_shaper.zig -> RendererState -> RenderFrame -> coretext_raster.zig` 준비/CPU glyph raster smoke(`mise run macos-coretext-smoke`). 이 smoke의 fixed probe shaper는 `coretext_shaped_records`로 기록되고, 제품 `GlyphRasterFrame`은 제품 후보 `coretext_glyph_rasterizer` wrapper가 smoke native bridge를 호출해 실제 CoreText glyph bytes를 만든다. 추가로 실제 `TerminalCore -> DrawList` fixture를 `CoreTextDrawListShaper` native bridge로 shape해 `drawlist_renderer_shaper=coretext_draw_list`, `drawlist_frame_prepared=true`, `drawlist_glyph_raster_ready=true`를 남긴다. rasterizer wrapper는 drawable shaping record의 PostScript name에서 만든 같은 renderer `FontId`를 조회해 CTFont 요청 이름을 native bridge로 넘기고, CoreText가 요청 이름 대신 대체 face를 돌려주면 실패로 닫는다. 다만 native CoreText raster 구현은 아직 smoke bridge에 있으며, Metal smoke는 아직 native record bounds로 만든 probe surface를 쓴다. `coretext_probe.zig`, `coretext_shaper.zig`, `coretext_raster.zig` 단위 테스트는 native record 변환, explicit surface metadata 보존, DrawList shaper bridge, FontId -> PostScript name bridge 계약을 검증한다.
