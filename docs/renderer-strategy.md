...
- native shaper 결과는 먼저 macOS `coretext_font.zig` 같은 platform adapter를 거쳐 `ShapedGlyphRecord`로 정규화한다. 이 adapter는 `.notdef`/space처럼 atlas 후보가 아닌 record를 걸러내고, CJK/emoji width, fallback 여부, color glyph kind, font size/scale cache key를 `GlyphRunList`로 전달한다.
