...
- 실제 CoreText font resolve/glyph run/`coretext_font.zig -> FontIdentityRegistry -> coretext_shaper.zig -> RendererState -> RenderFrame` 준비/CPU glyph raster smoke(`mise run macos-coretext-smoke`). 이 smoke의 shaper는 `coretext_shaped_records`로 기록되고, 제품 `GlyphRasterFrame`은 smoke 전용 `coretext_glyph_rasterizer`로 실제 CoreText glyph bytes를 만든다. smoke rasterizer는 drawable shaping record의 PostScript name에서 만든 같은 renderer `FontId`를 조회해 CTFont를 만들고, CoreText가 요청 이름 대신 대체 face를 돌려주면 실패로 닫는다. 다만 이 rasterizer는 아직 재사용 가능한 제품 macOS font backend가 아니며, surface는 native record bounds로 만든 probe surface다. `coretext_shaper.zig` 단위 테스트는 explicit surface metadata 보존을 검증하지만, 실제 `TerminalCore -> DrawList`를 native CoreText runtime으로 shape하는 제품 경로는 아직 아니다.
