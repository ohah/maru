...
- 실제 CoreText font resolve/glyph run/`coretext_probe.zig -> coretext_font.zig -> FontIdentityRegistry -> coretext_shaper.zig -> RendererState -> RenderFrame -> coretext_raster.zig` 준비/CPU glyph raster smoke(`mise run macos-coretext-smoke`). 이 smoke의 shaper는 `coretext_shaped_records`로 기록되고, 제품 `GlyphRasterFrame`은 제품 후보 `coretext_glyph_rasterizer` wrapper가 smoke native bridge를 호출해 실제 CoreText glyph bytes를 만든다. rasterizer wrapper는 drawable shaping record의 PostScript name에서 만든 같은 renderer `FontId`를 조회해 CTFont 요청 이름을 native bridge로 넘기고, CoreText가 요청 이름 대신 대체 face를 돌려주면 실패로 닫는다. 다만 native CoreText raster 구현은 아직 smoke bridge에 있으며, surface는 native record bounds로 만든 probe surface다. `coretext_probe.zig`, `coretext_shaper.zig`, `coretext_raster.zig` 단위 테스트는 native record 변환, explicit surface metadata 보존, FontId -> PostScript name bridge 계약을 검증하지만, 실제 `TerminalCore -> DrawList`를 native CoreText runtime으로 shape하는 제품 경로는 아직 아니다.
