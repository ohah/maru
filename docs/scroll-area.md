...
- **SV6b — 오버레이 발행 순서를 규약으로 적는다.** "배경 → 내용 → 장식(스크롤바)" 순서다. 지금은 각
  소비처가 손으로 지키고 있고(알림·팔레트·세팅 셋 모두), 그 규율이 다음 소비처에서 깨진다. 규약을
  적고 판정자로 고정할지, 발행 지점을 하나로 모을지는 SV6a 뒤에 남은 실패로 판단한다.
...
- **SV2 — 파일 탐색기.** 행 단위 좌표를 픽셀로 옮기는 것이 실제 변경이다. 부분적으로 보이는 행이 생기므로
  ...
  **판정자가 먼저다.** 지금 탐색기에는 시각 골든이 없어(Lab이 도크·detail만 그린다) 스크롤을 통째로
  ...
  [실제 구현 계획](implementation-plan.md)이 소유한다.
...
- **SV4 — 사이드바.** 스크롤바가 host의 GPU quad라 발행 경로가 없다. ScrollArea로 옮기면 사이드바도
  ...
  이관과 드래그 추가를 나눠 넣는다 — 슬라이스 나눔은 [실제 구현 계획](implementation-plan.md)이
  소유한다.
...
- **SV5 — 알림 패널·팔레트·세팅.** `overlay_input.windowStart`의 item-index windowing을 ScrollArea로
  ...
  **주의**: 셋 다 오버레이(모달) 셀 경로를 지난다. 스크롤바 quad의 layer와 clip은 사이드바에서 겪은 것
  ...
  여기서도 안전한 것이 아니다. 슬라이스 나눔은 [실제 구현 계획](implementation-plan.md)이 소유한다.
