...
**발행은 스크롤바와 같다.** 가상화 평행이동을 받지 않고(받으면 같이 흘러내린다), 컨테이너의 clip을
그대로 받는다(좌우로 넘치면 잘려야 한다). 스크롤바가 gutter를 컨테이너 안에서 예약해 같은 clip을 쓰는
것과 같은 자리다 — 여기서도 예외를 만들지 않는다. z만 다르다: sticky 헤더는 스크롤되는 자식보다 위여야
한다(카드가 헤더 밑으로 지나간다).
