...
- **죽은 것을 죽었다고 안다** — 서버 keepalive 를 기다리지 말고 우리가 주기적으로 확인한다.
  ...
  **어떻게**(2026-09-11): 조용한 동안 **채널 요청 `keepalive@openssh.com` 을 `want_reply`=1 로**
  보낸다. 상대가 그 요청을 몰라도 §5.4 가 **답을 요구**하므로(`CHANNEL_SUCCESS` 든 `FAILURE` 든)
  **답이 온다는 사실 자체**가 살아 있다는 증거다 — 내용은 안 본다. OpenSSH 가 우리에게 쓰는 것과
  **같은 수단**이고, 우리는 그 답을 이미 낸다(§5.4 의 `unknown_request`).

  **주기와 마감은 «부르는 쪽» 이 정한다.** 코어는 시계가 없고(호스트가 잰다) 무엇이 적당한
  간격인지도 모른다 — 폰과 데스크톱이 다르고, 같은 폰도 셀룰러와 Wi-Fi 가 다르다. 코어는
  **인코딩만** 하고, 언제 보낼지·언제 포기할지는 호출자의 값이다.

  모바일이 쓰는 값(사용자 확정 2026-09-11): **조용한 지 15초면 보내고, 답이 45초 없으면 죽은
  것으로 본다**(= 세 번 놓침). 한 번 놓쳤다고 안 죽이는 이유는 전환 직후나 깊은 잠에서 답이
  늦을 수 있어서다 — 재접속은 **새 세션**이라(아래) 오판의 대가가 크다.
