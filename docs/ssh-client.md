...
그냥 확장이라 답하고 넘어간다. **키 교환 중에는 예외로 끊는다**: strict KEX 는 그 사이의 예상
밖 패킷에 연결을 끊으라고 요구한다(§4).

`SSH_MSG_DISCONNECT` 는 **사유 코드와 설명을 같은 `Step` 에 실어** 올린다(§11.1) — 오류 이름
하나로는 사용자가 고칠 것이 없다. 설명은 서버가 고른 문자열이므로 배너와 같은 거름망을 거친다
(§4.4.2). 모르는 사유 코드가 와도 설명은 그대로 남긴다. **그리고 상태기계가 스스로 닫힌다** —
`Disconnected` 를 돌려주는 것만으로는 부족하다. 오류를 흘려 보낸 드라이버가 그 뒤에도 먹이고
쓸 수 있고, 그러면 이미 끝난 연결에 바이트를 짜 넣는다. 닫힌 뒤 들어온 것은 흘려보내고 쓰기는
`NotReady` 다(설명을 못 담을 만큼 길었을 때도 마찬가지로 닫는다 — 담기와 끝내기는 다른 일이다).
...
- **배너는 어느 상태에서나 받고 걸러서 준다**(RFC 4252 §5.4 — "at any time after this
  ...
  클립보드에 손댈 수 있다(§4.4.2).
- **끝은 상태기계가 소유한다.** 서버가 `DISCONNECT` 를 보내면 오류를 올리는 것으로 그치지 않고
  `closed` 로 옮긴다(§3.2). 오류 하나만 돌려주면 그것을 흘려 보낸 드라이버가 그 뒤에도 먹이고
  쓸 수 있다 — **호출자가 반쪽만 지킬 수 있는 신호로는 계약을 못 지킨다.**
