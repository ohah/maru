...
| 인증 | `publickey`(ed25519) · `password`(**와이어만** — 아래) | 키가 정답이고, 비밀번호는 서버가 그것만 열어 둔 경우의 탈출구 |
...
**`password` 는 아직 세션이 안 쓴다.** 요청 패킷을 만드는 자리(`userauth.writePasswordRequest`)와
응답 파싱은 있고 테스트도 있지만, 세션 상태기계(`client.zig`)가 고르는 방식은 `publickey`
하나뿐이고 ABI·펌프에 비밀번호를 실을 자리도 없다 — **지금은 키로만 붙는다.** 위 표의 이름만
보고 "된다" 고 읽으면 안 된다(계약이 반쪽만 참인 자리이므로 여기 적어 둔다).
