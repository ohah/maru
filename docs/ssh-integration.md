...
3. **드롭/paste 핸들러가 분기한다. ✅ 3·4단계 구현**
   ...
   - **원격(ssh) 업로드의 한계**: 원격 세션의 파일 드롭은 백그라운드 업로드가 끝난 **뒤** 경로를 paste하므로(아래 공통 업로드), 그 사이 사용자가 pane을 옮기면 결과 경로는 **완료 시점의 활성 pane**에 붙는다. 드롭 시점의 포커스 이동만으로는 이 비동기 구간을 묶지 못한다(업로드 결과에 대상 surface id를 실어 보내는 것은 후속).
