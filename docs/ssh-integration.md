...
3. **드롭/paste 핸들러가 분기한다. ✅ 3·4단계 구현**
   ...
   - **pane 라우팅(공통)**: Swift `handleDrop`이 내용 삽입 **직전에** 드롭 지점(backing px)을 ABI `maru_macos_app_session_focus_pane_at`(v115)로 넘겨 **떨어뜨린 pane을 활성으로** 만든다 → 뒤이은 삽입(드래그 경로는 `paste_text` 또는 `drop_files`)이 기존 경로 그대로 그 pane에 들어간다(예전엔 좌표를 버려 **어디에 떨어뜨려도 활성 pane**에만 삽입됐다). 드롭 처리 전체는 **그 뷰의 창** surface로 스코프한다(`withSurface(surfaceForView(view))`) — 안 그러면 forwarder가 key 창을 가리켜 백그라운드 창의 드롭이 다른 창에 붙는다. 대상 surface를 삽입 함수마다 넘기지 않고 포커스 이동으로 표현하는 이유: paste는 붙여넣기 보호 모달을 거쳐 **비동기로** 확정될 수 있어(`pasteNeedsConfirmation`) 대상을 모달 너머까지 들고 가면 확정 시점 대상이 흔들린다. pane rect는 탭 바를 포함한다. **"포커스 이동=라우팅"이 성립하지 않는 상태에서는 라우팅하지 않고 활성 pane으로 폴백**한다(Zig `focusPaneAtPoint`의 가드): 사이드바·pane 밖 / 붙여넣기 확인 모달 보류 중(그 모달은 *이전* 대상의 payload를 들고 있다) / 이전 paste 배수 중(`pendingPasteRetarget`이 FIFO 순서를 지키려 대상을 옛 surface에 고정한다) / 대상이 web pane(붙일 PTY가 없어 내용이 버려진다).
   - **비동기 구간까지 대상 고정**: 원격 세션의 파일 드롭은 백그라운드 업로드가 끝난 **뒤** 경로를 paste하므로(아래 공통 업로드), 드롭과 붙는 시점 사이에 비동기 구간이 있다. **드롭 시점의 surface id를 업로드 job에 실어**(`UploadJob.target_id` → `UploadResult.target_id`) 완료 시 `pasteTextTo(target_id, …)`로 되돌린다 — 그 사이 사용자가 pane/탭을 옮겨도 경로는 **드롭한 pane**에 붙는다(그 Term이 닫혔으면 no-op으로 버린다; 엉뚱한 pane에 붙이는 것보다 안전). 붙여넣기 확인 모달도 같은 규율이다(모달을 띄운 시점의 대상을 `pending_paste_confirm_target`에 고정 — 확인하는 동안 pane을 옮겨도 payload는 원래 pane으로). 이를 위해 미전송 paste 잔여 큐는 **surface별**(`pending_pastes`)이다: 단일 FIFO였을 땐 잔여가 다 빠지기 전까지 대상이 옛 surface에 고정돼, 다른 surface로 갈 바이트가 그 FIFO에 붙으면 엉뚱한 pane으로 갔다.
