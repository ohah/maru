...
- **로컬 hostname은 프로세스 수명 동안 캐시하지 않는다**(`app_session.localHostname`). hostname은 프로세스 수명 동안 바뀌므로(DHCP 도메인·Wi-Fi 전환·슬립 복귀, 구성 전에는 실패나 `localhost`) 첫 조회를 굳히면 그 뒤 이름이 바뀌는 순간 기준값이 낡아 로컬 세션이 원격으로 오판된다. "시작할 때 자리 잡았는지"만 보는 보정으로는 **시작 뒤에 바뀌는 경우**를 못 막는다.
  - **한 프레임 안에서는 재사용한다(2026-09-23).** 옛 문장은 "`gethostname` 382ns/call 이라 매 호출 조회해도 초당 0.5ms 미만" 이었는데, 그건 마이크로벤치 × 호출 수 추정이었다. 실측(활성 32 세션 `sample`)은 사이드바가 카드마다·프레임마다 여러 번(reproject 의 줄 수 재기 → `termGitBranch`→`termCwd`→`termCwdIsRemote`) 닿아 **앱 busy CPU 의 10 %** 였다. 그래서 `AppSession.tick` 머리에서 전진하는 프레임 도장으로 **그 프레임 안의 첫 조회만** 실제로 하고 나머지는 그 값을 쓴다 — 낡을 수 있는 폭은 한 프레임(16 ms)이고 다음 프레임엔 새 이름을 본다. 위 결함(수명 캐시가 바뀐 이름을 못 따라감)은 그대로 닫혀 있다. 도장이 0 이면(tick 없는 판정자·CLI) 예전처럼 매번 조회한다. A/B(활성 32 세션, 8쌍): 앱 CPU 중앙값 −7.2 %, 6/8 쌍에서 낮음. 판정자 `HN1` 이 «같은 프레임은 한 번 · **다음 프레임엔 바뀐 이름으로 판정이 뒤집힌다** · 제품 tick 이 도장을 넘긴다» 를 함께 고정한다(수명 캐시로 되돌리는 돌연변이가 빨갛다).
...
| provider 훅의 `cwd`(같은 payload 의 다른 필드 — 원격 pane 이면 **저쪽 기계**의 절대 경로) | 폴더줄·사이드바·창 제목, 그리고 **원격 SCM**(RS6) | **로컬 파일시스템에 안 댄다.** 표시는 그대로 그리고(§9.3 의 host 접두가 붙는다), 원격 SCM 은 그 경로를 **저쪽에서** `git -C` 로 연다. 값은 `(host, path)` 쌍으로만 산다 — 기계가 다르면 `git_ops.remoteCwd` 가 관측으로 되돌아간다(§9.5) |
...
- **훅이 그 구간을 메운다(2026-09-01, 에이전트 pane 한정).** 훅 payload 의 `cwd` 는 전면 TUI 가 붙어
  있는 동안에도 **매 턴** 오므로, 그 값을 관측 cwd 보다 우선한다. 규칙의 소유자는 `git_ops.remoteCwd`
  **한 함수**이고, 표시(`termCwdForDisplay`·창 제목 `currentCwd`)와 **원격 SCM**(`remoteScmTarget`)이
  그것을 함께 소비한다. 훅 모드를 벗어나면 값을 버려 「한 Term 두 소스」가 되지 않는다(관측 모드
  강등에서 `hook.tool` 과 같이 정리한다).
  ⚠️ **절대 경로만 담는다** — 상대 경로는 기준을 모르고, 잘린 절대 경로는 남의 디렉터리를 가리킨다.

  **소유자를 뽑기 전에는 그 「한 지점」이 사실이 아니었다(2026-09-05).** 앞 판은 이 문단에 「소비는 그
  한 지점」이라 적었지만 실제로는 `termCwdForDisplay` 만 그랬고, `remoteScmTarget` 과 `currentCwd` 는
  관측을 **직접** 읽었다. 그래서 원격 pane 에서 `claude` 를 띄우면 폴더줄은 진짜 경로를 그리는데
  소스 컨트롤 도크만 접속 직후의 홈을 대상으로 삼아 「저장소를 확인할 수 없습니다」로 떨어졌다
  (사용자 보고 2026-09-05). 규칙을 두 벌로 적으면 두 벌이 갈린다 — 이 저장소가 반복해서 당한 형태다.

- ⚠️ **훅 cwd 는 `(host, path)` 쌍이다**(`CwdLabel.host`). 훅 모드는 로그 파일이 한 번 생기면 세션
  내내 유지되므로(`agentHookMode` 의 «한 번 생긴 파일은 세션 중에 사라지지 않는다»), 로컬 에이전트가
  남긴 cwd 가 그 pane 이 `maru ssh` 로 들어간 뒤에도 **신선한 채로** 남는다. 시간만 보는 소비자는
  그것을 통과시키고, 그 값은 원격 `git -C` 의 인자가 되어 저쪽 기계의 **철자만 같은 남의 디렉터리**를
  연다. 그래서 값을 적을 때의 **기계**를 함께 담고, 지금 기계와 다르면 관측으로 되돌아간다
  ([remote-scm.md](plans/remote-scm.md) §2.1 「저장소 신원은 (host, path) 쌍이다」와 같은 규칙이다).
  그 기계를 정하는 것은 `git_ops.termMachineKey` 다 — **목적지가 있으면 그것, 없으면 OSC 7 authority**.
  맨 `ssh` 는 목적지가 비는데 로컬도 비므로, 목적지만 보면 그 둘이 같은 키가 되어 로컬 잔상이 통과한다.
