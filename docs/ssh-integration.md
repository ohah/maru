...
3. **드롭/paste 핸들러가 분기한다. ✅ 3·4단계 구현**
   ...
   - **pane 라우팅(공통)**: Swift `performDragOperation`이 `handleDrop` **직전에** 드롭 지점(backing px)을 ABI `maru_macos_app_session_focus_pane_at`(v115)로 넘겨 **떨어뜨린 pane을 활성으로** 만든다 → 뒤이은 삽입(paste_text/drop_files/drop_image)이 기존 경로 그대로 그 pane에 들어간다(예전엔 좌표를 버려 **어디에 떨어뜨려도 활성 pane**에만 삽입됐다). 대상 surface를 삽입 함수마다 넘기지 않고 포커스 이동으로 표현하는 이유: paste는 붙여넣기 보호 모달을 거쳐 **비동기로** 확정될 수 있어(`pasteNeedsConfirmation`) 대상을 모달 너머까지 들고 가면 확정 시점 대상이 흔들린다. pane rect는 탭 바를 포함하고, 사이드바·pane 밖이면 무동작(활성 pane 폴백).
