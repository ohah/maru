...
| 창 제목(`currentCwd` → Swift) | 그대로 쓴다 — 표시 전용이고 파일시스템에 닿지 않는다. host 접두는 붙지 않는다(후속) |
...
- ⚠️ **그래도 「보고자가 아예 없는」 원격은 못 메운다.** 위 우선은 `termCwdIsRemote` 분기 **안**에
  있는데 그 판정은 OSC 7 authority 하나로 서므로, 보고자가 없으면 §9.2 규칙상 **로컬로 떨어져** 훅 값에
  닿지 않는다. 그 pane 은 `ssh_remote_dest_present` 로 원격임을 이미 아는데도 그렇다.
  **고치려면 원격 판정을 그 신호까지 넓혀야 하고**, 그러면 표시에 쓸 host 도 `ssh_remote_dest` 에서
  끌어와야 한다(지금은 `cwd_host` 가 비어 `":<경로>"` 가 된다). 판정을 넓히는 것 자체는 §9.4 안전장치를
  **더 많은 pane 에 켜므로** 방향은 맞지만, 소비처가 넷이라 별도 슬라이스다.
  ⚠️ **로컬 해석기(`termCwd`)에 훅 값을 넣어 메우려 하면 안 된다** — 그 함수의 반환값은 호출자가
  **로컬 경로로** 쓴다(spawn·링크 resolve·git). §9.4 가 전수로 금지한 그것이 된다(적대적 검증에서
  실제로 그렇게 고쳤다가 되돌렸다).
