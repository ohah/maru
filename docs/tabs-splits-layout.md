...
- **에이전트 아이콘** — 그 Term 포그라운드가 claude/codex CLI인 동안만, 카드 **줄과 무관하게 슬롯 세로 중앙·좌측에 독립**
  ...
  - 심볼·색(베이스/결정): 브랜드 전용 유니코드가 없어 근사 글리프 — claude=`✶`(U+2736 6각별, Anthropic 선버스트 근사)·
    ...
    **상태줄**(running=**`▁▅▇▃ 진행중` codex식 이퀄라이저 스피너**, idle=`✓ {답변}`)이 구분한다 — running 스피너는 블록 문자
    ...
    진행한다. **위상은 스피너가 실제로 보일 때만 돈다**(`anyAgentRunning`): **(1) 탭바** 스피너는 활성 워크스페이스 상단이라
    **접힘·필터와 무관하게** 보이므로 활성 탭이 running이면 늘 돌고, **(2) 사이드바 카드** 스피너는 접힘이거나 검색 필터로 숨은 탭이면
    돌지 않는다(재투영 낭비 회피, code-review high #1). running 판정은 **탭 안 어느 pane/Term이든**(`tabHasRunningAgent`) — 활성
    Term만 보던 옛 게이트에서 확장(백그라운드 Term도 스피너·재투영 트리거). **브랜드색**(색칠 루프가 아이콘과 같은 패턴으로
    스피너 codepoint[블록 ▁~█]를 칠하되 **상태줄 row[카드 마지막 줄]로 좁혀** 이름/경로의 블록 글자가 안 오염, #2 / 색·kind는
    `tabAgentKind`로 running Term 우선)이라 "진행중"에 색 표현이 있다. **상단 탭 바**도 pane 라벨·running Term 탭 앞에 같은 파형을
    붙여(`spinnerPrefixedLabel` + `recolorSpinnerCells`) 사이드바와 표시를 통일한다. **아이콘은 항상 솔리드 브랜드색**(옛 `dimRgb` 밝기 펄스는
