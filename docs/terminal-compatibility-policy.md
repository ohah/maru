...
늘면** 이 모델이 나빠진다 — "어떤 OSC가 커질 수 있나"(`oscMayGrow` 허용 목록)와 "어떻게 처리하나"
(`dispatchOsc` 라우팅)가 두 곳으로 갈려 동기화 부담이 생긴다. 다만 **커지는 것이 한 시퀀스는 아니다**: 예로 드는
OSC 5522(kitty 클립보드)도 한 시퀀스는 4096바이트로 명세가 자르고, 큰 것은 청크 **재조립** 쪽이다.
...
이 토대 위에서 kitty 5522의 **청크 전송**은 거의 공짜로 따라온다 — 다만 5522가 함께 요구하는 것(읽기 권한
프롬프트·비밀번호·MIME 별칭·`CSI ? 5522 h` 모드·능력 질의·오류 코드 6종)은 청크와 별개라 **프로토콜 전체가
공짜인 것은 아니다**(2026-09-24 명세 확인 — 상세는 아래 단일 출처). **트리거는 "OSC 66/5522(또는 대용량 데이터)를
