...
GUI가 붙기 전에는 headless 경로가 v0의 첫 성공 기준이다. macOS 창과 renderer는 같은 계약을 소비하는 다음 단계로 붙인다.
...
이 기능들은 구조상 들어올 자리를 막지 않는다. 다만 v0에서는 interface와 문서상 경계만 유지하고, 실제 동작은 구현하지 않는다.

글로벌 핫키는 장기적으로 지원하되, v0에서는 [키 입력과 단축키 경계](key-input-and-shortcuts.md)에 적은 충돌 규칙만 유지한다.

## 성공 기준

- `mise run check`가 통과한다.
- v0 PTY/headless E2E가 자동화되어 있다.
