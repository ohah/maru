...
| 재부팅 뒤 부활(RB) — 새 셸 자동 실행·에이전트 대화 이어가기 | 계획(RB0 계약 문서만, 코드 0) | RB1: `boot-session` 쓰기/읽기·형식 손상=키 없음·재부팅 증명 판정(증명 없음·지금 값 못 읽음·값 같음 세 갈래), 증명된 복원이 host identity Term(live·ended·legacy)을 attach·probe·묘비 없이 저장 cwd로 새로 띄우는지, 증명 없는 복원은 기존 묘비 그대로인지, 알림 개수. RB2: `agent-resume` 캡처 조건(로컬·신원·비원격·비묘비)·읽기 검증, 대화 파일 끝 1 MiB에서 마지막 권한 모드·모델, 파일 없음=셸만, 도크 재개와 argv 동일, 재부팅 아닌 복원은 실행 0. 판정자별 변이 검사. 단일 출처는 [재부팅 뒤 부활 계획](plans/reboot-revival.md). | **실제 재부팅**은 헤드리스로 못 만든다 — 계획의 「손 테스트」 절차(파일의 `boot-session`을 다른 UUID로 바꾸고 host를 내린 뒤 실행)가 대신하고, 마지막에 사용자가 실제로 껐다 켜서 확인한다. |
