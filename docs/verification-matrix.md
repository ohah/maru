...
| macOS glyph text shader sampling smoke | `mise run test-macos-glyph-text-smoke`, `mise run macos-glyph-text-smoke` | `zig-out/maru-macos-glyph-text-smoke/glyph-text.summary.txt` | display가 있는 macOS에서 실제 AppKit 창 위 CAMetalLayer에 CoreText CPU glyph bitmap을 올린 Metal texture를 shader sampling으로 그린다. source glyph의 ink 픽셀 위치를 drawable 좌표로 매핑해 blit-readback하고, 선택한 모든 샘플이 clear 색이 아닐 때만 `glyph_texture_sampled=true`, `glyph_text=true`로 기록한다. 계약 테스트는 summary schema, status label, visible UI/pipeline/readback partial false case를 확인한다. 이 smoke는 제품 terminal renderer, atlas packing, screenshot artifact, cell-grid text layout을 아직 증명하지 않는다. |
