...
| 네이티브 편집기 — draw 저장소 몫 | 구현 전 | 계약은 [chrome-strategy.md](chrome-strategy.md) §5.4 「draw 저장소의 몫」과 [native-editor-visual-mapping.md](native-editor-visual-mapping.md) §4 「그릴 예산이 모자라면」(2026-09-24 사용자 결정). 헤드리스로 증명할 것: `frame.bufferSizes`·`diff_frame.bufferSizes`만큼만 잡은 저장소로 그리면 절단이 없고(`truncated = false`) 넉넉한 저장소로 그린 것과 op의 내용(종류·좌표·역할·run 글자)이 같다 — 속성 판정자는 **최악을 직접 만든 입력**(1칸 글자로 가득 찬 줄·열마다 번갈아 색·sticky 10줄)과 무작위 입력을 섞고, 접힘·진단 표식·인레이·위젯 행·비교 뷰를 넣어 **실제로 그려졌는지 따로 세며**, 소비를 **버퍼 최고 수위**로 잰다. sticky 몫·gutter `+ 3`·열 수를 줄이는 변이가 각각 빨개진다. op·run·글자 저장소를 **각각** 좁혀도 줄 번호와 sticky 머리줄이 남는다. `run 사용량 ≤ 몫` debug assert. | 제보된 결함(비교 뷰 한 열 run 640개 한도 — 68행 Zig 화면이 649개를 요구해 줄 번호가 통째로 사라짐)은 제품 경로 재현으로 확인했고 이 행이 닫는다. run 몫은 두 불변식(클러스터는 적어도 한 칸 · sticky 말고는 텍스트 op이 셀을 안 겹친다)에 기대며, 사전 검증 7,000회에서 위반 0·최대 98.3%였다. 글자 바이트는 결합 문자 때문에 묶는 불변식이 없어 기하 × 추정치와 절단을 남긴다. op 몫은 B2가 방식을 고른다. | 순서·선행(PR #3922)은 [plans/native-editor.md](plans/native-editor.md) 「draw 저장소 몫」(B1 run·글자 → B2 op)이 소유한다. |
