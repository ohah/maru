...
U5 restore activation 전 구간 failure matrix는 **구현 전**이다. 다음 gate
...
rollback 금지선 전 행을 한 source-host→same-PID exec 제품 fixture에서 전수 실행한다. rollback-safe 행은 canonical rollback exec 뒤
동일 PID·runtime·PTY I/O 생존을 MRSH로 다시 관측하고, manifest rollback이 불확정인 `AuthorityPoisoned` 행은 재귀 exec 없이
audit-required residue와 process fail-stop을 관측한다. 후속 `test-session-host-upgrade-restore-postcommit-failure-matrix`는 ready commit
