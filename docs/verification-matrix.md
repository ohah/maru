...
| 네이티브 편집기(등폭 GPU 뷰·편집·diff) | 구현 전 | 계약은 [native-editor.md](native-editor.md)가 소유한다(2026-08-09 사용자 결정 — `text` kind와 diff 본문을 CM6에서 Zig+Metal로 이관, 마크다운 렌더는 웹 유지). 헤드리스로 증명할 것: L2 편집 연산·selection 병합/primary 승계·undo 그룹핑과 복원·byte↔논리행·줄별 폭 합(탭스톱 포함)·lexer 상태 전이, L3 랩/접힘/가상줄 매핑과 **조건부** 좌표 왕복(§10 — 무조건 왕복은 성립하지 않는다). | **착수 전 gate: 현행 CM6 경로 성능 baseline 3축**(대형 파일 open, 스크롤 프레임타임 p50/p99, 타이핑→반영 지연)을 빌드 모드·기기와 함께 기록한다. 기록 없이는 이관 효과를 주장하지 않는다. **헤드리스 불가**: IME(특히 중간 caret 조합 — 현행 `NSTextInputClient`가 `replacementRange`를 무시하고 `markedRange().location`이 0 고정), 멀티 커서 × IME(정책은 "primary만 조합 → 확정 시 복제"로 확정됐고 실측은 그 검증이다), 커서 blink(ABI 단일 구간 전제 확장), 실제 GPU 출력. 터미널 IME 무회귀가 최대 리스크다. | 미정(구현 슬라이스에서 확정) |
