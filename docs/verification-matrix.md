...
| 에이전트 훅 통합(provider 훅 수신·모드) | **AH1 코어 구현**(순수 층), AH2~AH6 미구현 | [에이전트 훅 통합](agent-hooks.md)이 계약을, [단계 계획](plans/agent-hooks.md)이 AH1~AH6을 소유한다. **두 모드를 섞지 않는다** — 훅이 설치된 Term은 상태·알림·턴을 훅 payload에서만 얻고, 그 Term에서는 OSC 9/777 알림과 `agent_observer`의 화면·OSC title/progress·`output_active` 입력을 쓰지 않는다(OSC 7/133/52/11 등 무관 시퀀스와 `agent_kind` 판정용 프로세스 관측은 그대로 돈다). 훅이 없으면 [agent-session.md](agent-session.md)의 관측 모드가 그대로 돈다(그 문서는 2026-08-20 개정으로 관측 모드 전용). 이벤트 세트는 `SessionStart`·`UserPromptSubmit`·`Stop`·`PermissionRequest(*)`·`PreToolUse(*)`·`Notification` **6개**이고, Codex `trusted_hash`가 커맨드 바이트 해시라 **한 번에 확정**해야 한다. `PostToolUse`는 `originalFile`을 실어 큰 파일 편집에서 상한에 잘리므로 **뺐다**(계약 §3.1). **구현**: [`session/agent_hook_command.zig`](../src/session/agent_hook_command.zig)(인라인 커맨드·표식·legacy 식별·타임아웃)와 [`session/agent_hook_event.zig`](../src/session/agent_hook_event.zig)(ndjson 파서·tail 커서·재동기화·Codex 패치 경로). 실제 셸 게이트 `zig build check-agent-hook-command`가 계약 7개를 돌린다. **실측(2026-08-20)**: 훅 1회 12.27 ms(`sh` spawn 기저 8.04 ms, 백그라운드 분리는 14.28 ms로 더 느림), payload는 개행 없는 한 줄 JSON, Claude와 Codex의 `PreToolUse.tool_input` 모양이 다름(Codex는 `command` 하나뿐, 경로는 패치 텍스트 안), 셸 편집은 양쪽 다 훅에 안 잡힘, 동시 append 인터리브는 간헐적(24개 동시 쓰기에서 회차마다 0~2줄). | **platform 배선이 없다** — 파일 읽기·회전·정리·권한(0600/0700)은 AH2와 함께 붙인다. `resetIfRotated`가 요구하는 파일 아이덴티티(inode) 비교도 그쪽 몫이다. **`PermissionRequest`·`Notification`은 양 provider 모두 미검증** — 헤드리스에서 권한 거부를 유발해도 발화하지 않아 대화형 수동 검증이 AH4·AH5의 완료 조건이다. 비용 측정은 **샌드박스 안**에서 이뤄져 절대값은 상한으로 읽어야 한다(상대 비교인 "백그라운드가 더 느리다"는 환경 무관). 훅 모드에서는 **임의 프로그램의 OSC 알림도 사라진다**(발신자 구분 불가 — 감수하는 손해, 되찾으려면 훅을 끈다). MCP·커스텀 도구가 고친 파일은 `tool_input` 스키마가 제각각이라 소행 확정에서 빠진다(tree 비교가 잡으므로 유실은 아님). | **AH1(완료)**: 부분 라인·회전 경계·오프셋 전진·상한 절단·손상 라인 무시·형 변화 방어·재동기화 상한(뮤테이션 검증)·세트↔파서 드리프트(뮤테이션 검증)·구분자 comptime 가드(뮤테이션 검증)·상한 경계·빈 `out` assert·provider 이름 검증 = 단위 40개, 실제 셸 계약 7개(append 형식·가드 경로·stdin 드레인·상한 접기·디렉터리 부재 조용함·pane 식별자 경로 탈출·동시 append 유실 0). AH2: 사용자 훅이 있는 파일에서 순서·내용 보존, 설치/제거/재설치 멱등성, 게이트 off 시 완전 제거, Codex trust 동시 갱신, 과거 표식(`MARU_AGENT_MAP_HOOK_V2`) 불간섭, statusLine 훅 제거 후 세션 신원 유지. AH3: 모드별 소스가 정확히 하나(교차 오염 0), 강등 기록, 무관 OSC 동작, 두 모드 pane 공존. AH4: 상태 전이표, 조용히 오래 도는 셸 명령에서 진행중 유지, `AskUserQuestion`이 `PreToolUse`로만 올 때 입력 대기 + **대화형 수동 검증**. AH5: 같은 턴 중복 0, 자동 해소 승인에서 배너 0·배지 1, 포커스 억제 + **대화형 수동 검증**. AH6: 샌드박스 밖 비용 측정 artifact와 후퇴 설정에서 배지 유지. |
