...
| PTY/openpty controlled command | 환경 의존 | `mise run pty`가 macOS `openpty` backend로 controlled command, exit status, resize propagation, SurfaceRuntime routing, reader thread + bounded queue + RuntimeEventPump 경로를 검증한다. 아직 기본 `mise run check`에는 포함하지 않는다. | macOS가 아닌 환경이나 로컬 PTY 정책 문제는 기본 CI만으로 잡지 못한다. 대량 stdout backpressure stress와 interactive shell shutdown lifecycle은 아직 별도 검증 전이다. macOS window/app loop가 pump를 frame/input lifecycle에 연결하는 검증도 아직 없다. | [PTY 운영 모델](pty-operating-model.md)에 따라 `tests/integration/pty/` artifact를 남긴다. 다음 단계에서는 app host가 pump를 실제 window loop에서 호출하는 경로와 대량 stdout stress를 추가한다. |
