...
product exec와 real PTY/MRSH client gate로 검증했다. restore activation precommit 11행도 actual same-PID process와 real PTY로 검증했다. 최대치 근처 multi-runtime 제품 restore, postcommit failure injection,
...
exact audit-required residue와 process fail-stop을 관측한다. 후속 `test-session-host-upgrade-restore-postcommit-failure-matrix`는 ready commit
뒤 재귀 rollback 0과 fail-stop 또는 `promotion_failed` status-only 수렴을 별도로 닫는다. 제품 invocation·wire·ambient environment에는
