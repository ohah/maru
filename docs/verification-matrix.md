...
| --- | --- | --- | --- |
| CR3a-2c3c control facade | 부분 구현(C1·C2 완료, C3 미착수) | `test-session-host-2c3c-c2`가 C1의 raw extern DTO·closed error·authority·bounded encoder·canonical wire runtime 7과 boundary 1을 유지하면서, C2 runtime 5로 모든 15개 command projection, generation adapter의 typed admission, `Unsupported` consumed no-op, false/OOM/Busy retain, queue 64/65와 allocation fail-close를 Debug·ReleaseFast exact-count로 검증한다. boundary는 public declaration 10→12, raw method/JSON/stream/Client escape 0, generation nonblocking direct Client call 0, recovery resync baseline 1, legacy allowlist와 registry layout delta 0을 검사한다. C3은 blocking direct Client call을 제거한다. | Darwin socketpair에서 supported/unsupported wire 0, canonical frame-allocation OOM retain·무독성·exact-once retry, zero/partial/full pending progress, close hard-error retain, coalescing, `input prefix -> control -> input suffix` 순서를 통과했다. blocking OOM/flush는 C3에서 통과해야 전체 구현으로 승격한다. recovery resync, event, response decoder와 `RemoteRuntime.client` 제거는 이 행의 완료 범위가 아니다. |
