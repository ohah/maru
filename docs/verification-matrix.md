...
| P3-e4d-2a current-host foreground·consumer parity | 구현 전 | 별도 실제 daemon·generation-backed GUI connection·실제 forkpty에서 controlled `claude`와 `codex` process identity를 host sampler가 관측하고, 같은 owned observation을 AppSession의 `pollAgentKinds`, `termGitBranch`, file-drop SSH route가 제품 API로 소비해야 한다. 임시 Git repository와 OSC 7/5379를 사용하되 test가 observation/agent/branch/upload 결과를 직접 주입하지 않는다. provider·cwd·SSH 제거 뒤 absent 수렴과 Debug·ReleaseFast, source boundary를 함께 요구한다. | 실제 capability 없는 N-1 host executable 호환성은 e4d-2b다. raw cwd·SSH destination·argv·file payload artifact는 금지한다. |
