...
| --- | --- | --- | --- |
| P3-e4d-1 metadata isolation·reattach parity | 계약 고정, 제품 gate 구현 전 | `test-session-host-metadata-reattach-parity`가 별도 실제 daemon·하나의 GUI `Client`·두 forkpty runtime에서 A/B OSC 7/2/5379 event를 서로 격리하고, A detach 중 child가 게시한 최신 full-state가 같은 `host_id:runtime_id`의 다음 `attachExisting` 응답에서 추가 pump 없이 보이는지 Debug·ReleaseFast로 검증한다. trigger/marker가 순서를 소유하고, runtime identity·revision·owned cwd/title/SSH 문자열·sibling 불변·attachment/child/socket 정산을 단언한다. boundary gate는 test-only wire method/제품 분기와 민감한 metadata/argv artifact 기록을 금지한다. | controlled foreground process와 cwd→Git·agent·SSH 제품 소비자, capability 없는 실제 구 binary는 e4d-2 후속이다. 이 행만으로 P3-e4d 전체나 metadata parity 완료를 주장하지 않는다. |
