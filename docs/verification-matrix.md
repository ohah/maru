...
| 네이티브 편집기 — draw 저장소 몫 | 구현 전 | 계약은 [chrome-strategy.md](chrome-strategy.md) §5.4 「draw 저장소의 몫」과 [native-editor-visual-mapping.md](native-editor-visual-mapping.md) §4 「그릴 예산이 모자라면」(2026-09-24 사용자 결정). 헤드리스로 증명할 것: `editor_view.bufferSizes`·`diff_frame.bufferSizes`만큼만 잡은 저장소로 그리면 절단이 없고(`truncated = false`) 넉넉한 저장소로 그린 것과 op의 내용(종류·좌표·역할·run 글자)이 같다(색이 촘촘한 줄·랩 켬/끔·블록 caret·sticky·인레이·위젯 행·표식을 섞은 속성 판정자), op·run·글자 저장소를 **각각** 좁혀도 줄 번호와 sticky 머리줄이 남는다, 비교 뷰가 좌우를 몫대로 가른다. | 제보된 결함(비교 뷰 한 열 run 640개 한도 — 68행 Zig 화면이 649개를 요구해 줄 번호가 통째로 사라짐)은 제품 경로 재현으로 확인했고 이 행이 닫는다. 글자 바이트는 수 MB 한 줄 때문에 정확한 몫을 세지 않고 정책 상한과 절단을 남긴다. 폭 0 글자가 단독으로 색이 다르면 run 천장을 넘을 수 있으나 색만 번진다. | 순서·선행(PR #3922)은 [plans/native-editor.md](plans/native-editor.md) 「draw 저장소 몫」(B1 run·글자 → B2 op)이 소유한다. |
