...
| macOS PTY opt-in | `mise run pty` | `tests/artifacts/integration/pty/*.raw.txt`, `*.screen.txt`, `*.snapshot.txt` | macOS `openpty`가 controlled command stdout, process exit, resize를 실제 PTY로 전달하는지 확인한다. 환경 의존이라 기본 `check`에는 아직 넣지 않는다. |
...
| PTY/openpty controlled command | 환경 의존 | `mise run pty`가 macOS `openpty` backend로 controlled command, exit status, resize propagation을 검증한다. 아직 기본 `mise run check`에는 포함하지 않는다. | macOS가 아닌 환경이나 로컬 PTY 정책 문제는 기본 CI만으로 잡지 못한다. SurfaceRuntime reader thread/backpressure는 아직 별도 검증 전이다. | [PTY 운영 모델](pty-operating-model.md)에 따라 `tests/integration/pty/` artifact를 남긴다. SurfaceRuntime 연결 후 deterministic headless E2E에 일부를 편입한다. |
