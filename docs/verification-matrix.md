...
| CR3a-2c3d one-shot event facade | C1 구현·C2/C3 미구현 | C1은 GUI accepted/unknown header·payload·allocator admission seal, foreign allocator publication 전 거부, binding-entry checked-monotonic event generation과 same-address ABA burn, `.event` short permit, caller-final-address reusable `EventOwner`, `view()`, ordinary/idle/`ended_pending` take 및 sibling order/counter 보존을 구현했다. generation 제품 callsite는 아직 0이며 C1 테스트 cleanup만 별도 test-only settlement를 쓴다. | C1 focused Debug·ReleaseFast와 전체 `test-session-host`를 증거로 유지한다. C2에서 public `releaseEvent`, `ConnectionLease` cleanup pin, post-poison exact-once free, callback reentry, 4,096-slot/1 GiB trusted-mirror quarantine와 generic drop/deinit의 corrupt handoff를 닫는다. C3에서 `GenerationAttachment` inline owner/mirror, purge-first 제품 drain, actual socket revoked→fence→release, teardown Busy 및 generation raw Client event source-zero를 닫기 전에는 2c3d 완료를 주장하지 않는다. |
