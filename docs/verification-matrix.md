...
> **keep-alive 경로 CI 게이트(2026-08-27):** `session-host-keepalive-macos` 가
> `macos-session-host-recovery-smoke` 를 PR·푸시에서 돌린다 — **keep-alive 를 실제로 켜고 도는 유일한 CI
...
> 있고 통과한다(2분 14초, macOS 잡 중 최저). ⚠️ 덮지 않는 것: 강제 종료 경로, 멀티 윈도우 배치 복원(R7),
> GUI 부재 중 알림(OS 배너).
