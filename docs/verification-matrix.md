...
| font/layout/glyph atlas | 부분 구현, 환경 의존 | fake backend 기반 `DrawList -> GlyphRunList` 계약은 있다. 실제 CoreText resolve, fallback cache, glyph atlas bitmap/cache, device scale별 rasterization은 아직 없다. | 실제 한글/CJK/이모지 폰트 선택, fallback 성능, Retina glyph 선명도, font 설정 오류를 검증하지 못한다. | [폰트 전략](font-strategy.md)에 따라 실제 CoreText/screenshot/pixel smoke는 macOS opt-in으로 둔다. 다음 단계에서 glyph atlas cache key와 invalidation test를 확장한다. |
