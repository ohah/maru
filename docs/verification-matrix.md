...
| 에이전트 훅 통합(provider 훅 수신·모드) | 설계 확정, 미구현 | [에이전트 훅 통합](agent-hooks.md)이 계약을, [단계 계획](plans/agent-hooks.md)이 AH1~AH6을 소유한다. **두 모드를 섞지 않는다** — 훅이 설치된 Term은 상태·알림·턴을 훅 payload에서만 얻고, 그 Term에서는 OSC 9/777 알림과 `agent_observer`의 화면·OSC title/progress·`output_active` 입력을 쓰지 않는다(OSC 7/133/52/11 등 무관 시퀀스는 그대로 돈다). 훅이 없으면 [agent-session.md](agent-session.md)의 관측 모드가 그대로 돈다 — 그 문서는 2026-08-20 개정으로 관측 모드 전용이 됐다. 이벤트 세트는 `SessionStart`·`UserPromptSubmit`·`Stop`·`PermissionRequest(*)`·`PreToolUse(*)`·`PostToolUse(*)`·`Notification`이고, Codex `trusted_hash`가 커맨드 바이트 해시라 **한 번에 확정**해야 한다. payload는 양 provider 실측으로 확인했다(계약 §2). | 전달 채널이 없다 — 현행 `agent-sessions/<id>`는 `cat >` 덮어쓰기라 한 tick 안의 앞 이벤트가 사라지고, 소비 경로는 claude 전용 + 세션 id 문자열만 인정한다. 도구 훅 비용이 **미측정**이다(실측 세션 기준 도구 호출 3,585회 × 2 = 약 7,000회 spawn, `PostToolUse`에 `tool_response.stdout`이 통째로 실려 로그가 커진다). 훅 모드에서는 **임의 프로그램의 OSC 알림도 사라진다**(발신자 구분 불가 — 감수하는 손해, 되찾으려면 훅을 끈다). Codex `PostToolUse` payload 미검증. | AH1: 부분 라인·회전 경계·오프셋 전진·상한 절단·손상 라인 무시·동시 append. AH2: 사용자 훅이 있는 파일에서 순서·내용 보존, 설치/제거/재설치 멱등성, Codex trust 동시 갱신, statusLine 훅 제거 후 세션 신원 유지. AH3: 모드별 소스가 정확히 하나(교차 오염 0), 강등 기록, 무관 OSC 동작, 두 모드 pane 공존. AH4: 상태 전이표, 조용히 오래 도는 셸 명령에서 진행중 유지, `AskUserQuestion`이 `PreToolUse`로만 올 때 입력 대기. AH5: 같은 턴 중복 0, 자동 해소 승인에서 배너 0·배지 1, 포커스 억제. AH6: 비용 측정 artifact와 후퇴 설정에서 배지 유지. |
