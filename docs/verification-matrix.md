...
| P5b2b3 same-connection partial-batch pressure isolation | 계획 | 하나의 `connection_turn.Client`에 target/sibling 두 stream을 두고 target multi-frame batch의 첫 frame을 부분 전송한 상태에서 다음 producer batch가 soft cap을 넘는 정확한 경로를 먼저 red로 고정한다. 구현 뒤에는 target만 `drain_current_batch`로 들어가 새 screen/base admission 0, 현재 batch `end_stream` 전 invalidation notice 0, 종단 뒤 notice exact 1과 resync를 보장해야 한다. peer `Client`/`ScreenInbox`가 완성된 옛 target batch 뒤 invalidation을 받아 target partial/pending만 버리고 sibling batch를 적용하는지, malformed/no-end/timeout은 connection fail-close인지 검증한다. 실제 macOS socket gate는 기존 tiny-buffer 패턴으로 같은 GUI fd의 두 runtime stream 중 target에서 kernel short write와 후속 stall을 실제 관측하고, peer drain 재개 뒤 framing·batch 수렴, sibling marker와 `host.info` 왕복, teardown ledger 0을 Debug·ReleaseFast에서 고정한다. | 기존 P5b2b1은 서로 다른 slow fd의 격리와 partial connection fail-close를 증명하므로 이 같은-connection 문제를 덮지 않는다. frame 하나만 마친 뒤 같은 batch suffix를 purge하는 방식은 GUI에 종단 없는 `ScreenInbox.partial_batch`를 남겨 금지한다. screen pressure만 defer하며 detach/runtime-ended/protocol 오류의 기존 partial fail-close는 범위 밖이다. |
