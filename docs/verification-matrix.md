...
| P4 E3c GUI client idle-pump evidence | 구현 전 | ReleaseFast 별도 client process가 실제 독립 host와 generation-backed `RemoteTermBackend`의 `/bin/cat` runtime 1·10·15·100개를 사용한다. initial screen/metadata settle 뒤 60 idle frame의 client CPU와 selected owner, `pumpDelta`, timestamp seal, `ClientSlot` 전역 registry visit, socket read, applied event를 raw artifact에 기록하고 validator가 exact counter·round-robin·cleanup을 재계산한다. 100-runtime target marker는 sibling output 0과 기존 latency cap을 함께 판정한다. | 기존 E3a/E3b artifact는 host PID만 측정하므로 GUI 60Hz pump 원인을 증명하지 않는다. legacy/fake hook/microbenchmark나 정적 호출 수만으로 완료 처리하지 않으며, RED baseline 전에는 60→10Hz 또는 readable-event 전환을 선택하지 않는다. |
