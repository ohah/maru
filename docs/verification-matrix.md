...
| P5a1a accept hardening | 구현 | Debug/ReleaseFast `zig build test-session-host`가 listener/accepted fd의 명시적 nonblocking+CLOEXEC, empty accept, EINTR→success와 exact retry exhaustion, typed fd pressure와 owner retry/cadence predicate, same-UID 실제 socket, credential-provider seam의 other-UID rejection+peer EOF, frozen N-1/mismatched peer bounded readiness를 검증한다. macOS `zig build test-session-host-cross-uid-macos`는 실제 root `nc` 자식이 `0700` 디렉터리와 `0600` socket을 통과해 connect한 뒤 커널 peer UID에서 admission 전에 `.denied`되고 EOF로 정상 종료하는지 검증하며, PR의 `AppKit smokes macOS`가 이를 실행한다. | 이 gate는 passwordless `sudo`와 `/usr/bin/nc`가 있는 macOS runner를 요구하며, 별도 사용자 계정을 만들거나 앱의 실제 session-host 저장소를 읽거나 변경하지 않는다. P5a1a 당시 public CLI는 미구현이었고 read/admin CLI는 P5a2에서 열렸다. mutating CLI/attach TTY/SSH는 후속이다. |
...
| Workspace restore checkpoint backup re-arm | 구현 완료(R7-5) | 완전 복원 성공 뒤에만 secure parent와 held inode identity를 재검증해 stale `.bak`을 해제한다. 이후 첫 정상 background/final publication은 canonical 완전본을 `.workspace.v1.bak.tmp`에 복사·검증하고 `RENAME_EXCL`로 create-once `.bak`을 게시한 뒤 새 checkpoint를 원자 교체한다. Debug·ReleaseFast `test-workspace-checkpoint-file-adapter`의 17개 runtime과 source boundary가 성공/없음/재실행·symlink/non-regular/다른 UID predicate·fallible release phase, release 전후와 backup publish 전후 실제 child `SIGKILL`, stale temp 회수를 검증한다. `test-workspace-backup-rearm`은 성공 restore 배선과 공용 publication re-arm을 고정한다. restore-incomplete 실행의 background/final publication 차단은 유지한다. | 부분 모델을 canonical `workspace.v1`으로 승격하지 않으며, 불완전 실행 중 새 사용자 변경을 별도 복구하는 모델은 범위 밖이다. 전원 손실 durability는 주장하지 않는다. 단계와 완료 증거는 [Workspace restore 구현 계획](plans/workspace-restore.md#r7-5-checkpoint-backup-re-arm)이 소유한다. |
| Workspace restore 묘비 강등은 저장을 막지 않는다 | 구현 완료 | `zig build test-restore-accounting`이 두 축을 함께 잰다. ⑴ 살아 있던 handle 이 영구 부재로 강등되면 묘비가 되고 **`dropped` 래치는 서지 않는다**(강등 수는 따로 센다). ⑵ 이미 `runtime-state="ended"`인 파일만 열어도 래치가 안 서서 **그 실행이 저장할 수 있다** — 재부팅으로 저장된 세션이 전부 무효가 됐을 때 앱을 껐다 켤 때마다 배치가 사라지던 자기영속 교착(2026-09-19 실측)을 끊는다. 돌연변이 5(강등을 래치에 되더하기·강등을 안 세기·이미 묘비인 것도 강등으로 세기·래치를 항상 세우기·Gone 승격 제거) 전부 RED. |
| 재부팅 뒤 부활(RB) — 새 셸 자동 실행·에이전트 대화 이어가기 | **구현 완료**(RB1 재부팅 증명 + 새 셸 자동 실행, RB2 에이전트 대화 이어가기) | RB1(구현): `RB1-1`~`RB1-3`(`session/workspace.zig` — 왕복·형식이 틀린 값은 안 씀·깨진 값은 키 없음이고 창의 나머지는 보존·`rebootProven` 판정표), `RB1-4`~`RB1-7`(`app_session.zig` — keep-alive를 켠 채로 증명된 복원이 live·묘비·legacy bare 셋 다 attach·묘비 없이 새 셸로 띄우는지, 부활 수·checkpoint 표식·플래그 수명·새 capture의 `boot-session`, 증명 없는 세 모양(키 없음·같은 부팅·지금 값 못 읽음)은 지금처럼 묘비인지, apply 실패 롤백, 커널 `kern.bootsessionuuid` 형식), `RB1-8`(`app_host_abi.zig` — 무장이 표식을 한 번 소비해 첫 capture를 낸다). 변이 15(판정 함수 세 갈래·형식 불량 채택·형식 불량 쓰기·identity 안 비움·묘비 갈래 누락·묘비 짝맞춤 가드 제거·롤백 누락·무장이 표식 무시·플래그 안 내림·캡처 누락·알림이 안 비움·증명 게이트 제거·부활 수 안 셈) 전부 해당 판정자에서 RED. RB2(구현): `RB2-1`·`RB2-2`(`session/workspace.zig` — `agent-resume` 왕복·형식 불량은 안 씀·깨진 값 여덟 모양(P1이 지운 옛 `agent-kind`·`agent-session` 포함)은 키 없음이고 surface 나머지 보존), `RB2-3`·`RB2-4`(`agent_session_archive.zig` — 끝부분의 마지막 모드·모델, codex는 `session_meta` 없이, 도크와 부활이 같은 argv), `RB2-5`(`agent_transcript.readTail` — 잘린 첫 줄 버림·파일 전체면 지킴), `RB2-6`~`RB2-11`(`app_session.zig` — 저장 조건 다섯(종류·원격 훅 종류·원격 cwd·묘비·토큰)을 하나씩 깨면 없음, 캡처가 신원을 복사, 부활 요청이 claude·codex를 기록된 모드·모델로 셸 래핑하고 대화 파일 없음·저장 cwd 사라짐이면 셸만, 증명된 복원만 이어가고 수를 세며 증명 없으면 provider 실행 0, 종류 변화·훅 신원·관측 경로(상태줄) 신원이 checkpoint를 더럽힘). 변이 18 전부 해당 판정자에서 RED. 머지 뒤 코드 리뷰 반영(2026-09-24): `RB1-9`(같은 부팅 두 번 부활 금지 표식)·`RB1-10`(알림은 checkpoint 무장 뒤 한 번, host 실패면 생략)·`RB1-11`(창 둘 — 판정 한 번)·`RB2-12`(claude 대화 파일을 id 로)·`RB2-13`(codex 구분자)·`RB2-14`·`RB2-15`(보류 신원·종류 변화 시 신원 버림)·`RB2-16`(spawn 실패 시 그 칸만 묘비)·`RB2-17`(cwd 판정은 검색 권한), 그리고 `RB2-8`(slug 무관 폴더·2 MiB 뒤로 밀린 모드·대화 기록 cwd·`CODEX_HOME`·tcsh)과 `RB2-11`(옛 상태줄 파일 무시) 강화. 사이드바 대화 줄도 같은 id 찾기로 옮겼다(2026-09-24): `SB-1`(`app_session.zig` — slug 규칙과 다른 밑줄 폴더에서 찾음·신원 없으면 안 읽음·mtime 그대로면 안 읽음·새 신원의 파일이 없으면 옛 대화를 비움)과 `이미지 갤러리 … (IG6-b)`(claude 대체 경로 = 뿌리 + 캐시한 `<폴더>/<id>.jsonl`). 변이 5(id 찾기 무력화·옛 대화 남김·갤러리 뿌리 누락·mtime 게이트 제거·매번 재탐색) 전부 RED. 단일 출처는 [재부팅 뒤 부활 계획](plans/reboot-revival.md). | **실제 재부팅**은 헤드리스로 못 만든다 — 계획의 「손 테스트」 절차(파일의 `boot-session`을 다른 UUID로 바꾸고 host를 내린 뒤 실행)가 대신하고, 마지막에 사용자가 실제로 껐다 켜서 확인한다. |
...
| P5b2b1 product poll slow-observer isolation | 구현 | 서로 다른 controller/healthy observer/10개 slow fd의 실제 owner poll에서 non-writable queue stall을 기록하고, test-injected zero-prefix ledger pressure 중 가장 큰 stable tracker 하나만 invalidate한 뒤 healthy batch를 한 번 재시도한다. zero-prefix controller output도 lease를 유지한 채 reclaim할 수 있고 partial-prefix controller 또는 mixed controller/observer connection은 제외한다. 부분 전송된 같은-connection screen-pressure의 batch 종단 drain과 target-only 복구는 P5b2b3가 소유하며, 이 행은 서로 다른 fd의 zero-prefix 회수와 requester/sibling 진행을 소유한다. base reservation 부족 fixture는 요청량보다 작은 eligible victim 하나만 실제 회수한 뒤 재예약도 실패할 때 projector 0, requester valid/base/queue와 prepared 0, exact 1초 boundary 재시도를 검증한다. batch 부족 fixture는 base feasibility와 encoded batch 실패를 ledger 부등식으로 분리하고 projector 진입 시 prepared authority와 reclaim 0/1을 probe한다. victim 1회 뒤 재admission 실패에서 atomic prefix 0, semantic base/observation cadence/queue rollback, exact boundary의 두 번째 victim 뒤 실제 wire 7-chunk·7 MiB 전체 admission을 검증한다. 일반 output batch atomic preflight, base+queue 공통 turn당 1회 reclaim, mutation-time global/slot high-water, clients/producer/subscription/attachment/budget final 0을 fixture로 검증한다. 별도 10-controller product fixture는 eligible victim 0에서 requester state/base 보존, reclaim/projector 0, 정확한 1초 backoff 재시도를 검증한다. offset-zero purge deadline 초기화와 screen pressure 외 lifecycle 경로의 partial-prefix purge 거부는 slot 회귀로 고정한다. 제품 ACK fixture는 유효 ACK를 owner dispatch 시각에 한 번만 수락하고 같은 invalidation epoch의 중복 ACK가 deadline을 연장하지 않는지, ACK 뒤 정확한 1초 이전 projector 0과 경계의 2 MiB+17 byte snapshot 3-chunk atomic recovery, prepared base 16.25 MiB+encoded batch 17 MiB 결합 headroom 부족 시 projector 0, metadata 성공+snapshot 실패 뒤 prefix/revision rollback·notice/ACK 재발행 없이 `resync_pending`을 유지한 정확한 1초 재시도, 실제 peer wire의 metadata 1+snapshot 3 frame 순서·마지막 chunk만 `end_stream`, 마지막 byte 전 `.resync_draining`과 이후 `.valid`, 최종 global ledger 0을 함께 검증한다. | takeover rollback 보존은 P5b3 범위다. ACK wire의 epoch nonce와 cross-epoch replay 방지는 현재 MRSH v2 계약 밖의 차기 wire 백로그이며, socketpair 제품 adapter fixture는 실제 PTY drain이나 독립 host PID RSS를 증명하지 않고 그 gate는 P5b2b2다. |
| P5b2b3 same-connection partial-batch pressure isolation | 구현 완료 | `test-session-host-p5b2b3`가 Debug·ReleaseFast에서 실제 `connection_turn.Client`와 `SocketServer`+`poll_owner.Owner` 각각에 target/sibling 두 stream을 둔다. 실제 GUI socket에서 두 번째 initial attach의 oversized snapshot은 그 요청만 `payload_too_large`로 거부하고 기존 sibling tracker·connection·후속 ping을 보존한다. 별도 tiny socket buffer 행은 target multi-frame kernel short write와 후속 stall을 관측한다. pressure reclaim은 target만 `drain_current_batch`로 바꾸고 새 screen/base admission을 거부한다. peer drain 뒤 완전한 target `end_stream`, sibling marker, invalidation exact 1, 실제 resync ACK→fresh snapshot→`valid`, 후속 `host.info`를 검증한다. 제품 `Client`/`ScreenInbox`는 완성 target 뒤 invalidation을 받아 target pending/recovery만 정리하고 sibling inbox를 보존한다. teardown은 client/subscription/attachment와 queue/base/global ledger 0을 고정하며 종단 표식 없는 partial queue는 `PartialFrame` fail-close한다. | 기존 P5b2b1의 서로 다른 fd 격리와 별개다. frame 하나만 마치고 batch suffix를 purge하는 방식은 금지하며, screen pressure 외 detach/runtime-ended/protocol 오류의 partial fail-close는 유지한다. 독립 PTY/RSS 부하는 P5b2b2가 소유한다. |
...
> R7 전용 gate는 로컬 통과했고 PR의 `AppKit smokes macOS` job에 연결돼 있다.
...
| macOS app launch→first drawable L1 | `mise run macos-app-launch-first-drawable` | `tests/artifacts/perf/macos-app-launch-first-drawable.json` | 독립 parent가 ReleaseFast app executable의 timestamp를 닫힌 자식 환경에 게시하기 직전 monotonic 시각을 잡아 환경 entry 구성·`fork/exec` 준비까지 포함하고, 일반 제품 시작 경로의 첫 성공 `maru_metal_renderer_draw` 직후 timestamp와 exact latency를 5행 strict JSON으로 남긴다. validator는 schema·환경 지문·행 수·parent/Swift 시작 identity·순서·exact subtraction·첫 frame exact 1을 판정한다. 실행 전후 같은 경로의 SHA-256은 binary drift를 검출할 뿐 배포 provenance는 증명하지 않는다. `smokeMode`·build·shell·GPU completion·first visible pixel·first terminal content는 범위 밖이다. 상속 환경을 비운 뒤 기본 시스템 PATH와 독립 HOME/XDG config/cache·Codex/Claude config·TMPDIR·Maru config·session-host root·summary·측정 토큰만 사용해 실제 사용자 workspace와 host registry를 읽거나 지우지 않는다. 첫 슬라이스는 baseline-only라 hard cap을 두지 않는다. |
...
| 네이티브 편집기 — 구문 색 역할별 override | 구현 전 | [native-editor-ui.md](native-editor-ui.md) §9의 상속+override 계약대로 `editor.syntax.*` 메타·schema·GUI와 `syntax_theme.fromTheme` override 적용, 역할별 fallback, invalid 색 거부, reset/inheritance를 검증해야 한다. | 기본 파생 색과 터미널 ANSI 팔레트를 바꾸지 않는 opt-in 확장이다. 실제 Metal 픽셀·고대비 조합은 시각 gate가 별도로 필요하다. | 구현 slice에서 headless schema/round-trip과 Metal 시각 gate를 함께 확정한다. |
| Trace GUI inspector | 구현 전 | [trace-replay.md](trace-replay.md)의 별도 이니셔티브 계약대로 `maru trace inspect`가 기존 replay engine 결과만 자기완결 HTML로 투영하고, 입력 주입이나 별도 terminal-state 해석을 만들지 않는 fixture가 필요하다. | 웹 패널 완성과 독립적으로 외부 브라우저에서 열 수 있어야 한다. control-plane event 기록은 이 행의 범위가 아니다. | 구현 slice에서 CLI·HTML fixture와 실제 브라우저 read-only smoke를 확정한다. |
...
| 네이티브 편집기 — 괄호 판정 통일 | **구현**(2026-09-26) | `zig build test-editor`: `BRI1`~`BRI5`(공유 짝 질의), `SYN52`(언어별 추출), `BRP10`~`BRP12`(제품 동작·목록 도착), `BRC2`(OOM·재파싱)가 공통 목록·짝 결과에 대한 닿은 쌍/감싸는 쌍/다음 여는 괄호의 일치, `${` 전체 폭과 무색 쌍, Bash·Ruby·Kotlin의 언어별 제외, Markdown 탐색 유지·색 없음, HTML `(click)`·글 잎을 가로지르는 쌍을 검증한다. 제품 경로는 색 설정과 탐색의 독립, 처음 목록 도착 뒤 강조 갱신, 작성 중 편집·무효화 뒤 낡은 판 거부, 명시적 점프의 목록 완성, 커서 수 상한 부재를 검증한다. | 계약은 [문서 모델 §3.9c](native-editor-document-model.md#39c-괄호-짝으로-점프-2026-09-05)와 [시각 매핑 §5.1b·§5.1d](native-editor-visual-mapping.md). 기존 TextMate 색 오라클의 일치율을 새 점프·강조의 일치율로 주장하지 않는다. 구문 오류 복구·내장 언어 부재의 한계와 공통 깊이/긴 줄 제한은 남는다. 제품 Metal 캡처에서 TypeScript `${`의 두 칸 강조와 `}`의 한 칸 강조를 확인한다. | `zig build test-editor` 통과. 오라클·실측 범위와 전체 게이트 결과는 해당 PR의 검증 기록이 소유한다. |
| 네이티브 편집기 — draw 저장소 몫 | **B1 구현**(run·글자 — 단일·비교·병합·Windows 호스트) · B2(op) 구현 전 | 계약은 [chrome-strategy.md](chrome-strategy.md) §5.4 「draw 저장소의 몫」과 [native-editor-visual-mapping.md](native-editor-visual-mapping.md) §4 「그릴 예산이 모자라면」(2026-09-24 사용자 결정). 헤드리스로 증명할 것: `frame.bufferSizes`·`diff_frame.bufferSizes`만큼만 잡은 저장소로 그리면 절단이 없고(`truncated = false`) 넉넉한 저장소로 그린 것과 op의 내용(종류·좌표·역할·run 글자)이 같다 — 속성 판정자는 **최악을 직접 만든 입력**(1칸 글자로 가득 찬 줄·열마다 번갈아 색·sticky 10줄)과 무작위 입력을 섞고, 접힘·진단 표식·인레이·위젯 행·비교 뷰를 넣어 **실제로 그려졌는지 따로 세며**, 소비를 **버퍼 최고 수위**로 잰다. sticky 몫·gutter `+ 3`·열 수를 줄이는 변이가 각각 빨개진다(`RB1` — 실측: sticky 몫 빼기는 판정 실패, 나머지 둘은 debug assert). run·글자 저장소를 **각각** 좁혀도 줄 번호가 남는다(`RB2` — 프레임 몫 예약을 지우는 변이에 `RB2`·`RB3`가 빨개진다). 비교 뷰 제보 재현(`RB3`), 병합이 run·글자를 pane 몫대로 가른다(`RB4` — 넷 균등으로 되돌리는 변이에 빨개진다), 제품 경로 `appendPaneFrame` 비교 뷰 68행 zig 에서 좌우 번호가 행마다 선다(`DRB1`). 할당이 실패해도 호출자 저장소보다 작아지지 않는다(`RB6`). 글자가 한 칸 추정을 넘는 줄(탭 + ZWJ 이모지)에서도 번호 글자 몫이 남는다(`RB7` — 넉넉할 때 뒤 층 몫을 안 떼는 변이를 잡는다; run 에선 불변식 때문에 등가다). run 절단 보고는 모양마다 따로(`CTR1` 행 건너뛰기·`CTR2` 색 뭉개기·`CTR3` 위젯). **살아남는 변이는 등가 셋**이다 — 비교 뷰 run·글자 반반 복원(좌우 기하가 같아 몫이 같다), 병합 글자 균등 복원(글자 사용이 추정의 25~75% 라 균등 몫으로도 안 넘친다). op 축을 좁히는 판정은 B2 몫이다. `run 사용량 ≤ 몫` debug assert(본문·gutter·sticky). | 제보된 결함(비교 뷰 한 열 run 640개 한도 — 68행 Zig 화면이 649개를 요구해 줄 번호가 통째로 사라짐)은 제품 경로 재현으로 확인했고 이 행이 닫는다. run 몫은 두 불변식(클러스터는 적어도 한 칸 · sticky 말고는 텍스트 op이 셀을 안 겹친다)에 기대며, 사전 검증 7,000회에서 위반 0·최대 98.3%였다. 글자 바이트는 결합 문자 때문에 묶는 불변식이 없어 기하 × 추정치와 절단을 남긴다. op 몫은 B2가 방식을 고른다. | 순서·선행(PR #3922)은 [plans/native-editor.md](plans/native-editor.md) 「draw 저장소 몫」(B1 run·글자 → B2 op)이 소유한다. |
...
| 선택 영역을 에이전트에 보내기 | **네이티브 편집기 경로 완료**(NS1~NS5 + NSH — 2026-09-11). 파일 패널 웹 경로는 후속 | 편집기나 파일 패널에서 고른 부분을 **그 창에서 고른** 터미널 CLI에 붙여넣는다([send-selection-to-agent.md](send-selection-to-agent.md)). 경로가 둘이고 **네이티브 편집기가 1차**다 — 재료(선택·줄 인덱스·핀된 경로)가 이미 서 있고 층이 셋 적다(render iframe·신뢰 shell·브리지 epoch 검증이 없다). 슬라이스는 그 문서 §6.1(NS1~NS5)이 소유한다. **NS3·NS4 완료**(2026-08-30): 후보 순서·경로 접기(`session/agent_selection.zig`)와 **편집기 본문 우클릭 메뉴**가 섰다. 그 전에는 편집기 본문 우클릭이 터미널 본문 분기로 내려가 `input.right-click=paste` 기본값에서 **문서에** 클립보드를 붙여넣었다(PTY로 새지는 않았다 — `pasteText`가 편집기 Term을 알아본다). 그 자리를 가져오며 **`select_all`이 편집기만 빠져 있던 것**도 함께 고쳤다(주소창·커밋 상자는 분기하는데 편집기는 코어 큐로 보내고 있었고, 편집기 코어는 sentinel이라 ⌘A가 아무 일도 안 했다). **NS5 완료**(2026-08-30 — 메뉴에 보내기 구획이 서고 고르면 그 Term 에 `submitPaste` 로 붙는다. 이 줄은 한동안 "남은 것은 NS5" 로 낡아 있었다). **NSH 완료**(2026-09-11 — **고르면 뜨는 헬퍼**. 드래그·더블·트리플로 고르고 떼면 caret 한 줄 아래 한 줄짜리 상자가 뜨고, 누르면 후보가 하나면 바로 보내고 여럿이면 NS4 메뉴를 그 자리에 연다. 우클릭을 모르면 이 기능이 없는 것과 같다는 것이 근거다 — 사용자 결정. 모달이 아니라 키를 안 뺏고(`modalInputRole` = `.not_an_overlay`), 닫힘은 경로마다 심지 않고 **프레임마다 한 가지**를 다시 묻는다: 그 선택이 아직 있고 그 줄이 아직 그려졌는가. 새 UI 는 없다 — `context_menu` 의 다른 State 다. 판정자에서 **둘이 필수**임이 변이로 드러났다: **제품 경로**(`mouse` down→drag→up→클릭)가 없으면 배선이 통째로 빠져도 전부 초록이고, **프레임에 실리는지**(`buildChromeOverlayPrep`)가 없으면 "상태도 서고 클릭도 먹는데 화면에 없는" 변이가 통과한다). **적대적 검증 29 회가 뒤따랐고 여덟 회차가 결함을 냈다**(2026-09-11~12): ⑴ **§4 의 사슬이 끊겨 있었다** — 페이로드를 빚은 bracketed 모드와 인코딩 모드가 **두 번 읽혀** 그 사이 2004 가 꺼지면 여러 줄 인용이 감싸이지 않은 채 셸로 들어간다(`submitPasteShaped` 로 주입까지, `pending_paste_shaped` 로 **확인 모달 너머까지** 들고 간다 — 모달 쪽 창은 사람이 읽는 몇 초다). ⑵ **§7 종료 placeholder 가 보낼 곳으로 셌다**(`kind` 는 `.terminal` 인데 붙일 PTY 가 없어 바이트가 조용히 사라진다 — 묘비뿐인 창에서는 «후보가 하나» 라 메뉴도 없이 그리로 갔다). ⑶ 프레임 빌드 게이트(`overlayFrameNeeded`)가 헬퍼를 몰라 **영영 안 그려졌다**. ⑷ 후보가 하나인데 보내기가 실패하면 **눌러도 아무 일이 없었다**(이제 메뉴를 연다). **§4 는 이제 바이트로 잰다** — 큐 버퍼가 다 나갈 때까지 남는다는 것을 확인해, 잔여가 남는 크기로 넣고 `\x1b[200~` 이 실제로 붙었는지를 본다(NS5 가 «주입은 관측이 안 된다» 고 적어 조립을 갈라 둔 그 전제가 절반만 참이었다). **시각 회귀는 골든이 잡는다** — `context-menu-send-helper-single-row`(`mise run macos-dock-visual-golden`). **자동으로 못 잡는 것**: 앵커의 offset clamp 는 `lineAt` 의 Debug assert 를 위한 것이라, 판정자 바이너리에서 그 assert 가 사라져 변이를 죽음으로 못 잡는다(경로 도달과 정해진 결과까지만 잡힌다). 그리고 **실제 앱 손 확인**은 남아 있다. 선택 수집은 web, 정책·대상·주입은 Zig다 — web은 임의 경로나 대상 surface를 지정할 수 없다. 안전 계약의 핵심은 **페이로드 끝에 개행을 넣지 않는 것**과 bracketed paste가 꺼진 대상에는 여러 줄을 보내지 않는 것이다(터미널 주입은 명령 실행이 될 수 있다). 개행 부재는 macOS 스모크가 PTY에서 직접 확인한다. 읽기·소스 두 모드만 대상이고 리치는 원문 줄 역매핑이 없어 제외한다. |
...
| 에이전트 세션 기록 도크 (Codex·Claude, AS4-f·AS5·AS6 현재) | 구현 — UI/상호작용·스캔·정렬 계약이 모두 이행됐다. 남은 것은 아래 한계의 수동/잔여 gate뿐이다 | AS4-f의 `DockMetrics`는 28pt right-dock safety band, Session Dock switcher, root/header/scope/search/group/card/detail/action/scroll metric을 하나의 snapshot으로 resolve한다. 검색은 `OverlayInput { query, preedit }`와 `InputFocus.agent_session_search`를 통해 click·`/`·macOS committed text·한글 IME marked/commit·candidate caret를 같은 owner로 묶고, preedit는 표시만 하며 commit 뒤에만 immutable snapshot의 in-memory filter를 갱신한다. AppSession integration test가 committed text/IME preedit가 terminal PTY로 새지 않는 것을 고정한다. header refresh idle/busy는 같은 24pt registered-SVG slot을 쓰므로 refresh click이 1-cell Unicode glyph로 축소되지 않는다. `SessionDockUiZoom`은 Cmd font-size 비율을 750–1500 milli로 clamp해 backing scale과 한 번 합성하고, 그 resolved scale을 layout·CoreText request/fingerprint·paint·hit-test·scroll projection에 함께 전달한다. actual AppKit fixture는 기본 zoom의 isolated cold process 네 개(terminal font 14pt/24pt × injected render scale 1×/2×)에서 실제 Swift→ABI→Zig published tree의 header, scope, search, first/expanded card, resume/reveal border rect를 JSON으로 관측한다. 같은 기본 zoom의 두 font rect는 exact equality, 2× raw rect는 1×의 두 배를 요구한다. 별도 `font-zoom` AppKit process는 실제 `MaruMetalTerminalView.keyDown`의 Cmd+= → Cmd+- → Cmd+-를 보내, 스크롤에 잘리지 않는 published scope-row rect가 확대 → baseline 복귀 → 축소되고 terminal identity가 유지되는지를 확인한다. Zig integration test는 clamp/reset 경계를 보완한다. | **AS5가 스캔 갭 넷을 닫았다(2026-08-08 실측).** streaming parser로 파일 크기와 무관한 고정 64 KiB 버퍼가 됐고(이전 피크 463 MB), 그 전제를 방어하던 read cap이 사라져 **목록이 라운드마다 337개로 같다**(이전에는 69→272로 늘며 12라운드에도 미완). `partial`이 DTO로 올라와 헤더가 불완전을 말하며, 도크 첫 진입에도 스캔 요청이 나간다. **AS6가 정렬 갭을 닫았다.** 정렬 키가 transcript의 마지막 활동 시각이고(못 읽은 파일만 mtime 폴백) 카드의 `N분 전`이 같은 값을 읽는다 — 이전에는 실측 362개 중 257개(70%)가 제자리가 아니었고 Claude 쪽 최대 차이가 144시간이었다. header에 최신순/오래된순 토글이 있으며 방향은 표시 계층에서만 바뀐다. 골든 case `header-utility-row`가 `로컬`·토글·refresh의 배치를 픽셀로 판정한다 — 그 전까지는 **어떤 골든도 헤더를 보지 않아** utility를 통째로 지워도 전부 통과했다. exact-live는 macOS POC의 argv-only child 관측 한계로 provider 공식 payload→pane-bound mapping 설계 승인 전까지 차단이다. 실제 사용자 Codex/Claude resume도 사용자 승인 수동 gate다. **재개가 권한 모드와 모델을 되살리는 것**과 **provider가 끝나도 탭이 남는 것**은 `mise run macos-agent-session-archive-smoke`가 함께 고정한다 — fake provider가 모드·모델 플래그까지 포함한 정확한 argv일 때만 verdict를 쓰고(선택적 수용을 안 하므로 플래그가 빠진 회귀는 초록이 될 수 없다), 모드 매핑 자체는 `zig build test`의 왕복 판정자가 **허용된 값 전부**를 세며 모델 토큰 술어도 **허용된 바이트 65개를 센다**(금지 목록이 아니라 허용 개수라야 갈아입어도 안 샌다). 모델에는 거를 것이 실재한다 — Claude Code가 합성 줄에 적는 `<synthetic>`(실측 67건)을 안 거르면 카드와 argv가 함께 오염된다. 같은 규칙이 **세션 id 에도** 걸린다 — 인용은 셸로부터만 지켜 주고 `'-x'` 는 provider 의 인자 파서가 플래그로 읽으므로, 두 값이 하나의 토큰 규칙(`isSafeArgvToken`)을 지난다. 판정자는 **첫 자리 64 개 · 그 뒤 65 개**를 따로 세고(`-` 가 맨 앞에서만 빠지는 것을 값으로 고정), 못 가리키는 id 를 가진 세션이 목록에서 빠지는 것을 두 provider 각각으로 확인한다. provider를 끝낸 뒤 프롬프트가 남는 것은 실제 앱에서의 수동 확인 항목이다. **그 스모크는 2026-09-15까지 `font-scale-rects` 24pt에서 `timeout_enterAgentSessions`로 죽어 뒤 시나리오 넷(`reveal-recheck-pointer`·`claude-resume-pointer` 포함)이 통째로 안 돌고 있었다** — 도크 뷰 스위처 슬롯 폭이 터미널 폰트를 따라가 큰 폰트에서 스위처가 사라졌기 때문이다([file-explorer.md §3.5](file-explorer.md)). 슬롯 폭을 pt 토큰으로 옮긴 뒤 시나리오가 11개 → 15개로 늘었고, 위의 Claude 판정자가 비로소 실제로 돌아 `verdict=true`를 낸다. **그리고 그 스모크는 이제 CI 가 돈다**(`agent session archive macOS`) — 그 전까지는 `mise` 태스크로만 있어 손으로 안 돌리면 이 영역 회귀가 아무도 모르게 샜고, 위 사고가 정확히 그 형태였다. macOS 잡은 병렬이고 임계경로가 `file explorer`(실측 15분 41초)라 wall-clock 은 늘지 않는다(스모크 자체는 실측 40.5초). CI 에 얹으면서 그 스모크가 **기계 화면 크기에 답이 달려 있던 것**도 함께 닫았다 — fixture 창이 AppKit 의 화면 제약을 받아 러너에서 터미널 38열(로컬 135열)로 잘려 `timeout_observeLoading` 으로 죽었다. 이제 fixture 창만 `constrainFrameRect` 를 무시하고, 실제로 열린 크기를 `agent_session_archive_smoke_content_size` 로 summary 에 남겨 기계 독립을 값으로 증언한다. 같은 형태가 하나 더 있었다 — 합성 휠이 `CGEvent` 전역 좌표에 창 좌표를 그대로 넣어 **주 화면 높이만큼 어긋난 자리**에 떨어졌고(로컬은 그 자리가 우연히 도크 안이라 통과), 러너에서는 창 밖이라 도크가 휠을 못 받았다. 되돌아올 값이 창 좌표가 되도록 고치고 fixture 가 그 왕복을 스스로 단언한다(`agent_session_archive_smoke_scroll_point`)([plans/agent-session-list.md](plans/agent-session-list.md) AS4-c). Session Dock 검색의 실제 firstResponder/한글 candidate 위치는 AppKit smoke에 아직 독립 scenario가 없어 수동 확인 항목이다. 정렬 토글의 실제 pointer 왕복과 2× backing scale의 header utility rect JSON도 같은 성격의 잔여 gate이며, 정렬 결과를 실제 이력으로 대조하는 것은 개인 데이터라 CI에 넣을 수 없다(로컬 40개 표본을 독립 구현과 대조해 나노초까지 일치를 확인했다). | `zig build test`, `mise run test-macos-app-host-abi`, `mise run macos-app-host-swift-check`, `mise run macos-agent-session-archive-smoke`, `git diff --check`. |
...
| 긴 soak/제품 성능 예산 | 구현, 환경 의존 | `mise run perf`의 core 기준, CR6e-b 20배치/100회 actual-AppKit recovery soak와 CR6f 600초/60창 actual-host idle soak가 있다. CR6f는 동일 host/PTY identity, 창별 wake·observation·projector 0, CPU·FD·RSS raw 표본, 마지막 actual marker와 cleanup을 strict artifact로 판정하며 2026-09-11 실측을 통과했다. CR6e-c3c v2 actual-AppKit key→host-backed marker→보장된 Metal submit과 L1 launch→first drawable baseline도 실측했다. | c3c는 첫 visible pixel이 아니라 보수적 제출 상한이며 환경별 hard cap이 없는 baseline-only다. 장시간 soak는 로그인된 macOS runner가 필요한 opt-in이라 PR required check가 아니다. | c3c hard cap은 동일 환경 반복 baseline이 충분할 때 별도 확정한다. |
...
| U5 제품 | checksummed attempt ledger v3는 distinct target+canonical rollback image, 64 MiB two-copy preallocation, absolute deadline과 exact host/epoch/runtime set을 고정한다. 제품 daemon은 rollback self-image, **same-designated-requirement** codesign stager, controller, preflight와 strict pathname `execv`를 연결했다. quiesce 전 budget admission은 read-only codec preview, exact two-copy `F_PREALLOCATE`, 같은 파일의 incompressible durable probe와 4배 safety factor를 통과한 뒤에만 reader pause를 허용한다. authoritative capture의 membership/runtime set/길이는 reservation과 다시 대조하고, 예약 owner가 성공·in-process rollback·deadline에서 pathname/fd를 exact-once 정리하며 cleanup failure는 fail-stop한다. target/rollback bootstrap은 inherited PTY/owner/state FD와 role/image를 검증하고, `ready` commit→graph ownership→inherited fd close→reader release→rollback promotion→ledger→capability/admission 순서를 강제한다. crash residue sweep은 exact host owner lock 뒤 capability 광고 전에 실행하며 exact attempt/target/tomb 이름·UID·mode/type/link count·pinned dev/inode·closed child vocabulary를 전부 검증한 뒤 no-replace tomb를 거쳐 제거한다. drift/unknown/정리 실패는 keep-alive service를 보존하고 새 upgrade admission만 fail-close한다. zero-runtime product artifact, real-PTY old-side rollback, partial-prepare all-or-none, unexpected FD와 primary/backup/promotion failure component gate, valid/hostile residue의 실제 fork/socket daemon capability gate가 있다. `test-session-host-upgrade-product-rollback`은 corrupt target primary에서 canonical product rollback을 실제 exec하고 동일 peer PID의 listener에 MRSH client로 재접속해 old build/epoch, `rolled_back/restore_failed`, capability와 empty inventory를 단언한다. `test-session-host-upgrade-nonempty-rollback`은 supervised source-host가 직접 만든 PTY의 direct-child 관계, exact runtime identity, pre/post I/O와 exit-23 명령 뒤 host-owned reap/inventory 제거까지 한 process artifact로 검증한다. `test-session-host-upgrade-restore-precommit-failure-matrix`는 test-only enum 11행을 별도 root에서 실행한다. rollback-safe 10행은 fault exact-one 소비 뒤 canonical 제품 rollback을 같은 PID로 exec하고 old build/epoch·runtime/child·화면 marker·후속 입력·host-owned reap을 MRSH로 다시 관측한다. reader timeout은 실제 reader start gate 한 개를 미게시 상태로 둔 채 deadline을 밟는다. manifest poison 행은 재귀 rollback 없이 전용 nonzero exit, listener/activation 부재, terminal child 종료와 exact `rollback-current`/ready-temp/owner/manifest residue 및 inode를 기록한다. `test-session-host-upgrade-restore-postcommit-failure-matrix`는 ready commit 뒤 네 행을 별도 artifact와 root에서 실행한다. cleanup activation·inherited FD close·attempt report 실패는 재귀 rollback 없이 fail-stop하고 listener/owner/child/target/attempt residue 및 부모 FD delta를 관측한다. promotion 실패는 같은 PID·runtime·PTY를 살린 채 `committed/promotion_failed` status-only와 upgrade capability 철회를 MRSH로 확인한다. 실제 제품 binary bytes에는 fault option과 enum 이름이 없어야 한다. 각 행은 monotonic elapsed 표본도 출력한다. signed 성공 하네스도 복원 뒤 PTY 종료 marker를 보내 direct child 부재와 `runtime.list` 연속 2회 부재를 요구하며 `runtime.terminate` 단축 경로를 금지한다. exit status 숫자 자체는 wire 관측 증거로 주장하지 않는다. pathname 재-open은 kernel-loaded image pin이 아니며 pathname object identity+same-designated-requirement signer+same UID만 executable trust boundary다. | `connectOrLaunchDetailed`는 선택한 old host의 `upgraded`·`upgrade_busy`·`legacy_unavailable`·`upgrade_failed`를 bounded typed `UpgradeNotice`로 반환하고, AppSession은 최종 current-host 연결 성공 뒤 modal-free frame에서 정확히 한 번 표시한다. 로그와 UI는 같은 DTO를 쓰며 최종 host 연결 실패가 더 강한 notice로 우선한다. 이 자동 gate는 형식·보존·우선순위를 검증하지만 실제 앱 재실행 화면 증거는 아니다. signed 성공 하네스의 command·artifact·수명 검사는 구현됐지만 실제 immutable release manifest에 결속된 frozen N-1/current 서명 artifact로 아직 실행하지 않았다. 그 provenance-bound 실행, 1개·최대치 multi-runtime exact reattach, 실제 app relaunch/notice와 soak가 남았다. 이 gate 전에는 U5 완료·검증된 제품 migration 완료를 주장하지 않는다. 자동 upgrade 시도가 기본 connect 경로에 연결된 사실은 별도다. |

U5 restore activation failure matrix는 **precommit·postcommit 구현**이다.
`test-session-host-upgrade-restore-precommit-failure-matrix`는 `session-host-upgrade.md` §11.28의 closed test-only fault enum 중
rollback 금지선 전 11행을 한 source-host→same-PID exec 제품 fixture에서 전수 실행한다. rollback-safe 10행은 canonical rollback exec 뒤
동일 PID·runtime·PTY I/O 생존을 MRSH로 다시 관측하고, manifest rollback이 불확정인 `AuthorityPoisoned` 행은 재귀 exec 없이
exact audit-required residue와 process fail-stop을 관측한다. `test-session-host-upgrade-restore-postcommit-failure-matrix`는 ready commit
뒤 네 fault를 별도 artifact/root에서 전수 실행해 재귀 rollback 0과 fail-stop 또는 `promotion_failed` status-only 수렴을 닫는다. fail-stop 세 행은
listener·owner·terminal child·staged target과 exact host-dir inventory를 관측하며 cleanup activation 실패만 audit용 `rollback-current` 하나를
보존하고 나머지는 빈 inventory로 수렴한다. promotion 실패는 같은 PID/runtime/PTY I/O와 status-only controller를 확인한다. rollback
재진입 sentinel까지 넣은 최종 하네스의 Debug 표본은 행당 0.74~0.86초, ReleaseFast 표본은 0.21~0.26초였고 모두 parent FD delta 0이었다.
이 값은 로컬 APFS/process 진단 표본이지 성능 예산이 아니다.
제품 invocation·wire·ambient environment에는
fault 입력을 추가하지 않으며, signed frozen release provenance와 delayed-fsync kernel fault는 이 gate의 증거가 아니다.
...
U5 stage-5/6 bounded child owner는 `release_adapter_live_workflow_aggregate_child`가 contract argument를 먼저 parse해 active command tag를
유도하고, context 11개와 runner-authority 4개 SSOT의 closed environment만 canonical fixed storage로 재구성한 뒤 actual child observation을 reducer mapper에 직접
적용한다. observation 실패도 partial byte나 raw error를 게시하지 않고 synthetic incomplete observation을 거쳐 conservative
`cleanup_failed`가 된다. `test-session-host-release-adapter-live-workflow-aggregate-child`가 Debug·ReleaseFast actual process로 tuple,
signal·timeout·cap, wrong state/command/context와 payload·descriptor·state storage alias의 fork 전 거부, ambient secret 0과 FD delta 0을
검증한다. Actions step 간
...
U5 repository-local live release workflow caller는 **제품 배선 구현·로컬 검증 완료, protected baseline-A 시험 tag 실측 전**이다. `.github/actions/session-host-release-live/action.yml` 하나가
...
fixed five product executable build, local action exact-one, 단계별 최소 credential, fixed root/pathname과 candidate bundle의 exact-two·authored bundle의 exact-one consumer 및 command별 closed argv를
검증한다. candidate pinning과 command bridge의 existing actual-process gate는 harness-owned private root에서 20회 실행해 현재
candidate pinning median 10.012ms, draft 229.750ms, aggregate prepare 64.611ms, finalize 67.088ms, publication 67.866ms, cleanup 68.741ms,
failure/PID collision/FD delta/checkpoint·candidate residue 0을 기록했다. 이 source/local-process gate는 실제 GitHub attestation/release
service, Apple product, GitHub-issued timing이나 frozen signed U5 제품 E2E의 증거가 아니다.
...
local monotonic 진단은 GitHub-issued timestamp 증거가 아니다. 이 caller는 baseline-A 전용이며 frozen executable attestation만으로
...
이는 기존 leaf 증거의 누락 방지와 공통 실행 진입점이며, signed provenance나 후속
restore activation precommit·postcommit matrix의 완료를 이 gate 하나로 대신하지 않는다.
...
세션 컨트롤 플레인과 웹 패널은 [plans/control-plane.md](plans/control-plane.md) §11의 micro-slice를 기본 구현 단위로 삼는다. Phase 1~7은 제품 milestone이고, 구현 PR 하나가 통째로 한 Phase를 끝내는 것을 기본값으로 보지 않는다.
...
- **CR6e-c3c 반복 sample-set과 hard cap: 구현·환경 의존 반복 실측 완료.** 기존 v2 한 행에는 환경·binary 지문이 없으므로 현재 5회 숫자만
  hard cap으로 승격하지 않는다. opt-in 제품 gate가 한 번 빌드한 동일 ReleaseFast app/product executable로
  actual-AppKit v2를 20회 순차 실행하고, 실행마다 제품 허용 경로의 0700 HOME을 새로 만들며
  session-host root와 raw artifact를 분리한다.
  ...
  2026-09-11 `Mac16,9`·macOS `25.5.0`·16 logical CPU와 동일 app/product SHA-256의 독립 3묶음
  60회는 묶음별 median 20.604~21.272ms, p95 21.509~22.236ms, 전체 p95 22.199ms였고 모든 raw
  strict/cleanup 판정을 통과했다. 단발 max 73.833ms를 구조 회귀와 분리하기 위해 validator는
  20회 nearest-rank p95 ≤30ms와 개별 hang ≤100ms를 함께 판정한다. 두 상수는
  `sample_set_p95_cap_ns`·`sample_set_hang_cap_ns`가 소유한다.
  boundary test는 하나의 30ms 초과 tail은 통과하고 두 개의 초과는 p95 실패가 되며, 두 cap의 exact 값은
  통과하고 cap+1은 typed failure가 되는 경계를 고정한다.
...
- **N3-R2 provisioned product runner (R2a·R2b1·R2b2·R2b3·R2c 구현, provisioned 실행 대기):** R2a transaction owner는 candidate·app·helper·Aqua/permission
  ...
  `test-session-host-notification-helper`가 compile+runtime/source boundary로 검증한다. R2b2의 첫 leaf는 앱 callback이 받은
  canonical request와 parsed stable route, 실제 attach 결과(`bound` 또는 `recovered`)를 하나의 absolute deadline 아래
  ...
  **R2b3b2**는 이 값과 screen/input을 AppSession 상태머신 및 child transport에 연결하고, app receipt 뒤 두 번째 frame을
  ...
  substitution, signer/runtime drift, bind 후 byte drift를 fail-close하는지 검증한다. R2c composition은 이 authority를 R2a owner의
  두 scenario·최종 leaf 순서에 연결한다. 각 scenario의 성공 receipt는 상위 transaction이 관측값을
  소비한 뒤 exact held pathname으로 한 번만 제거하며, 제거 실패에는 같은 retry authority를 보존한다.
  composition은 caller가 만든 하나의 absolute deadline을 signer bind와 두 concrete scenario 및 모든 재검증에 그대로
  전달하며, concrete의 `u64`와 transaction clock의 `i128` 경계는 checked 동일값 변환만 허용한다. 첫 `not_provisioned`는
  일반 오류나 pass로 접지 않고 helper의 typed provisioning 원인을 transaction 결과에
  보존하며, 이 경우 최종 leaf를 만들거나 게시하지 않는다. 두 scenario가 모두 성공하면 canonical R1 bytes를 먼저
  파생하고 두 held scenario receipt를 역순으로 소비한 뒤, DMG/product authority 재검증 후에만 최종 경로를 배타 게시한다.
  최종 leaf의 durable publication은 mounted-candidate gate의 `publish` 단계가 소유하므로 execute 뒤 mount/product drift는
  publication 0이고, publication 뒤 detach 실패는 exact held leaf만 rollback한다. Debug·ReleaseFast
  `test-session-host-notification-candidate-gate`는 단일 deadline, typed `not_provisioned`, 입력 snapshot, clean retry,
  execute→publish candidate drift, occupied final leaf 비파괴와 canonical 최종 leaf를 검증한다. 실제 signed candidate와
  Notification Center를 쓰는 provisioned 실행은 구현된 R3 workflow를 실제 전용 runner에서 실행하는 외부 release gate로 남는다.
...
- **선결조건 상태: 부분 구현, 외부 gate 미준비.** G3 제품 config 코드는 provisioned `Session host product / default-on` runner와 immutable
  ...
  `release_adapter_profile_predecessor_authority.zig`는 profile에 exact 결속된 authenticated manifest/file을
  `github_tag_chain_transport.authenticateUntil`에 넘겨 GitHub ref/tag chain·immutable release·세 asset을 직접 검증하고,
  ...
  실측은 후속 gate다.
  Protected profile→signed execution은 `release_adapter_profile_upgrade_execution.zig`의 final-address owner 하나가 owned deadline 아래
  ...
  archive, copied/pre-owned owner와 allocation fail-index를 닫는다. Pinned GitHub CLI transport도 **구현**됐다.
  `release_adapter_live_timing_transport.zig`는 exact metadata GET 뒤 selected archive GET만 허용하고 두 child 전후 CLI authority를
  ...
  다음 remote Release gate의 첫 value owner는 **구현**됐다. current tag의 published immutable Release JSON을 protected Context와
  ...
  그중 current Release metadata 전후 fence는 **구현**됐다. `release_adapter_remote_release_fence.zig`가 같은
  `published_release` endpoint를 checkout 전 pinned CLI와 하나의 final-address deadline으로 작업 전후 조회하고, 각 child 전후 CLI
  재검증과 canonical role별 release/asset snapshot exact 일치 뒤에만 완료 capability를 게시한다.
  `test-session-host-release-adapter-remote-release-fence`는 Debug·ReleaseFast에서 exact argv/environment, 배열 순서 독립성,
  ...
  Current Release exact-ID private asset download는 **구현**됐다. `release_adapter_remote_release_assets.zig`가 begun fence의
  canonical 네 asset ID만으로 pinned `gh api`를 role 순서로 실행하고, shared deadline의 fresh remaining·child 전후
  CLI/fence 재검증·exact size/SHA·private 0400 held inode·failure cleanup retry를 소유한다. focused gate는
  Debug·ReleaseFast injected child와 actual clean-environment process/private filesystem에서 exact argv/order, short/long/digest/child/CLI/fence
  ...
  Filesystem→semantic bridge는 **구현**됐다. `release_adapter_remote_release_semantic_files.zig`가 exact-ID
  `Assets`의 held manifest/evidence descriptor를 bounded exact read·EOF·pre/post revalidation하고 begun fence metadata owner와
  함께 위 semantic owner에만 전달한다. focused Debug·ReleaseFast actual private filesystem gate가 baseline·upgrade
  publication, exact read, read/fence drift, pre-owned result의 I/O 전 거부, allocation unwind와 borrowed FD 0을 닫는다.
  ...
  Canonical 원격 pass record와 동일-run artifact 보존은 **구현됨**이다. final verdict에서만
  `maru.session-host-release-remote-pass.v1`을 투영·freeze하고 역순 cleanup 뒤 private APFS no-replace publication이 성공한 경우에만
  ...
  `errexit` 문맥과 무관하게 invalid tuple을 nonzero로 닫고 filesystem 존재, raw profile, token을 읽지 않는다.
  `test-session-host-release-adapter-profile-authored-attestation-fence`는 별도 fresh process가 current context/profile과 fixed pathname으로
  ...
  transport와 component parser는 `release_adapter_candidate_product.zig`·`release_adapter_dmg_authority.zig`와
  `release_adapter_pre_publish_product.zig`의 제품 조립 및 validator executable에 연결됐다. 실제 codesign·read-only DMG 관측,
  canonical summary publication과 live release workflow 배선도 구현됐다. 현재 일반 PR의 component/fixture green은 protected
  tag에서의 실제 Developer ID·GitHub service 실행을 대신하지 않는다. 일반 DMG release의 draft-first·no-clobber·재다운로드 byte
  equality는 연결됐고 release workflow의 third-party Action은 exact SHA로 고정했으며 임의 ref를 고를 수 있는 수동 실행은
  제거했다. session-host authored manifest/evidence attestation과 post-publish release attestation도 live workflow에 연결됐다.
  2026-09-13 GitHub API 재감사에서 repository self-hosted runner, deployment environment, tag ruleset은 각각 0개였고
  Actions Secret 목록 조회는 HTTP 500으로 판정 불가였다. 따라서 Developer ID 출하와 전용 Aqua 제품 gate는 현재
  `not_provisioned`다. repository-level release immutability는 공개 REST 응답으로
