...
| --- | --- | --- | --- |
| CR3a-2c3c control facade | 구현 전 | C1은 raw extern `RuntimeControl`과 `ValidatedRuntimeControl` exact 2 variants, `sendControl|sendControlNonBlocking`, closed `ControlError` 및 raw lifecycle/control tag 전수표를 Debug·ReleaseFast focused gate로 고정한다. C2/C3은 기존 `PendingControl.barrier` ordering을 보존하며 generation scroll/core의 nonblocking/blocking direct Client call을 차례로 0으로 만든다. boundary는 public declaration 10→12, raw method/JSON/stream/Client escape 0, recovery resync baseline 1, legacy allowlist와 registry layout delta 0을 검사한다. | Darwin socketpair에서 supported/unsupported, encode OOM, zero/partial/full pending write, close, 1/64/65 queue, coalescing, `input prefix -> control -> input suffix`를 모두 통과해야 구현으로 승격한다. recovery resync, event, response decoder와 `RemoteRuntime.client` 제거는 이 행의 완료 범위가 아니다. |
