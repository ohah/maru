...
- **C3-3b2b1 trusted preparation seal prerequisite(미구현):** focused gate `test-session-host-2c3d-c3-3b2b1`은 기존
  ...
  domain의 full 32-byte transcript/progress seal을 만든다. every-field/domain/role/order/attempt/progress replay, PID/ready/fork, golden vector,
  constant-time compare, generic writer/MAC·raw key·permit registry·persistent mirror 0과 source/import/caller inventory를 Debug·ReleaseFast로 고정한다.
