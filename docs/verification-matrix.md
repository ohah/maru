...
| macOS visible window smoke | `mise run test-macos-window-smoke`, `mise run macos-window-smoke` | `zig-out/maru-macos-window-smoke/window.summary.txt` | macOS에서 실제 AppKit 창을 띄운다(`visible_ui=true`). 계약 테스트는 summary schema를 확인하고, visible smoke는 실제 window server/AppKit lifecycle을 확인한다. 아직 Metal surface, font, terminal grid는 없으므로 화면 내용 검증이 아니다. |
| macOS Metal clear-frame smoke | `mise run test-macos-metal-smoke`, `mise run macos-metal-smoke` | `zig-out/maru-macos-metal-smoke/metal.summary.txt` | macOS에서 실제 AppKit 창 위 CAMetalLayer에 clear frame을 present한다(`metal_surface=true`). 계약 테스트는 summary schema를 확인하고, visible smoke는 Metal device/command queue/drawable/present 경로를 확인한다. 아직 font, glyph atlas, terminal grid는 없다. |
