...
| CR3a-2c3d one-shot event facade | 구현 전(doc-first) | caller-final-address `EventOwner`의 event-incarnation별 one-shot과 reusable inline storage, ClientNode binding-registry canonical `event_generation` SSOT와 attachment projection, 짧은 `.event` take/release permit, 기존 `ConnectionLease` 기반 node/slot cleanup pin, accepted/unknown admission seal, canonical Client allocator identity, purge-first `ended_pending`, connection poison 뒤 canonical release와 4,096-slot/1 GiB trusted-mirror no-free quarantine를 확정했다. reservation identity는 node incarnation+event generation+owner address이며 정상 release는 slot을 empty로 재사용한다. 제품 owner는 `GenerationAttachment` inline exact 1이며 attachment teardown은 owner lifecycle/generation을 별도 검사한다. C1은 seal·node generation·ordinary take, C2는 release·pin·quarantine·callback, C3는 제품 drain·ended priority·actual socket/source-zero를 소유한다. | 아직 제품 구현 증거가 없다. C1 gate부터 raw lifecycle/copy/move/owner+attachment same-address replay/stale authority/queue seal/allocator provenance를, C2에서 post-poison release·canonical-slot corruption trusted handoff·4,095/4,096/4,097 simultaneous cap·4,096 sequential reuse·event generation exhaustion rollback·callback exact-count를 Debug·ReleaseFast로 닫고, C3에서 generation direct raw Client event callsite 0과 revoked fence·ended priority·teardown Busy를 실제 socket으로 증명해야 구현으로 승격한다. |
