...
| Workspace restore checkpoint backup re-arm | 구현 전 | 현재 제품은 restore가 불완전하면 background checkpoint와 final checkpoint를 publication 전에 모두 건너뛰어 손상된 입력을 즉시 덮어쓰지 않는다. file adapter의 create-once secure `.bak`과 mode 축소 단위 gate는 존재하지만 이 정책에서는 호출되지 않는다. 별도 개선은 완전 복원 성공 뒤 stale `.bak`을 안전하게 해제하고, 이후 첫 publication에서 새 완전본을 다시 create-once backup으로 잡아야 한다. 성공/실패/재실행·symlink/non-regular/다른 UID·crash ordinal에서 원본과 backup 중 적어도 하나의 완전본을 보존하는 product gate가 필요하다. | 현재 보호는 안전 우선이라 restore-incomplete 실행에서 생긴 사용자 변경을 저장하지 않는다. 이 행은 P1~P5 일반 세션 연속성 완료 주장과 분리하며 구현 전에는 `.bak` 자동 회복을 주장하지 않는다. |
