...
| --- | --- | --- | --- |
| CR3a-2c3c control facade | 부분 구현(C1 완료) | `test-session-host-2c3c-c1`이 raw extern `RuntimeControl`과 `ValidatedRuntimeControl` exact 2 variants, `sendControl|sendControlNonBlocking`, closed `ControlError`, outer/nested raw tag 0...255, 모든 core-command 최대 payload의 bounded encoder 적합성, lifecycle·role·thread·fork·stale·revoke authority, allocator callback reentry permit, canonical capability·stream과 dedicated scroll/core frame을 Debug·ReleaseFast runtime 7+boundary 1 exact-count로 검증한다. C1은 product callsite 0을 유지한다. C2/C3은 기존 `PendingControl.barrier` ordering을 보존하며 generation scroll/core의 nonblocking/blocking direct Client call을 차례로 0으로 만든다. boundary는 public declaration 10→12, raw method/JSON/stream/Client escape 0, recovery resync baseline 1, legacy allowlist와 registry layout delta 0을 검사한다. | C1 Darwin socketpair는 supported/unsupported와 exact wire kind를 통과했다. encode OOM, zero/partial/full pending write, close, 1/64/65 queue, coalescing, `input prefix -> control -> input suffix` 제품 경로는 C2/C3에서 모두 통과해야 전체 구현으로 승격한다. recovery resync, event, response decoder와 `RemoteRuntime.client` 제거는 이 행의 완료 범위가 아니다. |
