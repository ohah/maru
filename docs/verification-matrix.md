...
| renderer DrawList 계약 | `mise run test`, `mise run check` | 없음 | `RenderSnapshot`이 GPU/Metal 없이 backend-neutral `DrawList`로 변환되는지 확인한다. 현재 dirty 모델은 row 범위이므로 dirty row만 draw command로 만들고, wide glyph continuation cell은 별도 draw command로 만들지 않는다. |
