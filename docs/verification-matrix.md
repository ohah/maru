...
| P5b2b3 same-connection partial-batch pressure isolation | 구현 1/2 | `test-session-host-p5b2b3`가 하나의 실제 `connection_turn.Client`에 target/sibling 두 stream을 두고 tiny socket buffer에서 target multi-frame batch의 kernel short write와 후속 stall을 관측한 뒤 다음 producer batch가 soft cap을 넘게 한다. target만 `drain_current_batch`로 들어가 새 screen admission을 거부하고, peer drain 뒤 완전한 target `end_stream`, sibling marker, invalidation notice exact 1과 후속 `host.info` 응답을 frame parser로 검증한다. 종단 표식 없는 partial queue는 `PartialFrame`으로 계속 fail-close한다. | peer 제품 `Client`/`ScreenInbox`의 target-only recovery, resync 완료, 실제 `SocketServer`+`poll_owner.Owner` teardown ledger 0은 2/2 제품 gate 전까지 미완료다. 기존 P5b2b1은 서로 다른 slow fd 격리만 증명한다. frame 하나만 마치고 batch suffix를 purge하는 방식은 금지하며, screen pressure 외 detach/runtime-ended/protocol 오류의 partial fail-close는 유지한다. |
