...
| 긴 soak/제품 성능 예산 | 부분 구현, 환경 의존 | `mise run perf`의 core 기준과 제품 경로 job에 더해 CR6f가 controller wire input→PTY→observer screen delta를, CR6e-c3c v2가 실제 AppKit input dispatch→host-backed marker 뒤 보장된 Metal submit을 raw ns로 잰다. 후자는 첫 visible pixel이 아니라 보수적 제출 상한이며 아직 baseline-only다. | **긴 soak와 앱 시작 시간은 여전히 측정하지 않는다.** AppKit 입력→프레임은 표본 분포와 기기 등급별 hard cap이 아직 없어 전역 회귀 예산으로는 못 잡는다. | c3c baseline을 반복 수집해 환경별 hard cap을 확정하고, startup 및 장시간 soak를 opt-in job으로 분리한다. |
