...
| macOS Metal clear-frame smoke | `mise run test-macos-metal-smoke`, `mise run macos-metal-smoke` | `zig-out/maru-macos-metal-smoke/metal.summary.txt` | display가 있는 macOS에서 실제 AppKit 창 위 CAMetalLayer에 clear frame을 present한다(`metal_surface=true`). 계약 테스트는 summary schema를 확인하고, visible smoke는 window visibility와 Metal device/command queue/drawable/present 경로를 확인한다. display가 없는 headless 세션(SSH, GUI 없는 CI)이나 window activation 실패에서는 `visible_ui=false`, `metal_surface=false`로 기록하고 non-zero로 실패한다. 아직 font, glyph atlas, terminal grid는 없다. |
