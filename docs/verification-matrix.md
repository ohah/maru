...
| CR3a-2c3d one-shot event facade | C1·C2·C3-1·C3-2 구현 완료, C3-3 confirmed poison primitive 구현·나머지 미구현 | C1은 GUI accepted/unknown header·payload·allocator admission seal, foreign allocator publication 전 거부, blocking generation RPC 임시 parser allocator에서 scope 이전 stable Client allocator로의 publication 전 exact-once canonicalization, process identity와 ledger final address에 봉인된 private registry scope authority와 current parser/Client installed-domain 재검증, binding-entry checked-monotonic event generation과 same-address ABA burn, `.event` short permit, caller-final-address reusable `EventOwner`, `view()`, ordinary/idle/`ended_pending` take 및 sibling order/counter 보존을 구현했다. C2 계약은 production-type facade exact 14, public `releaseEvent` exact 1, dedicated std/scalar quarantine registry exact 1, mutex 전 PID/canonical operation-thread 검사, callback 전 전수 검증·all-mutex release와 logical registered-node operation 유지, callback 뒤 lookup 없는 binding settlement 후 pin-last no-fail suffix, binding one-shot recovery permit+cleanup-only projection의 damaged-lease recovery를 고정한다. C3-2는 exact 15 facade의 무인자 purge, attachment wrapper와 `RemoteRuntime`의 release-pending→purge-first generation drain 및 `none|applied(EventDrain)|failed(ClientError)` pending outcome을 구현한다. C3-3 첫 slice는 exact-15 poison을 registered ClientSlot/node owner-thread admission과 blocking confirmed effect에 연결했다. | C1/C2/C3-1 gate를 상속한 `test-session-host-2c3d-c3-2`가 Debug·ReleaseFast attachment runtime 8+actual generation `RemoteRuntime` product drain 1+boundary 1을 실행한다. `test-session-host-2c3d-c3-3`은 actual socketpair에서 external typed invalid/exclusive Busy의 durable mutation 0, partial pending free exact 1, same-thread facade와 foreign-thread raw Client callback reentry Busy, first reason, deferred-open 수렴, fd=-1, peer EOF와 idempotency를 Debug·ReleaseFast로 검증한다. C3-3 후속은 ClientSlot membership으로 봉인한 single-mode Client, sealed queue latch→take-commit exact-receipt live row의 원자 handoff와 release consume, 다중 revoke·ABA/OOM 정산, canonical owner-thread fence 안의 queue+live-count 검사→allocation/offset/syscall no-yield admission과 재진입/foreign-thread mutation 0, confirmed poison effect 뒤 release, 열린-peer actual socket revoked→fence/effect→release, admitted unknown/semantic failure와 generation raw Client event source-zero를 검증한다. 공통 Client ingress cadence, immediate EOF/yield 집합, unread RX-first, socket ingress malformed/unknown cadence와 legacy/generation parity는 2c3e blocking gate이며 그 전에는 2c3 전체·제품 actual-socket parity·input→RPC/revoke ordering 완료를 주장하지 않는다. |
