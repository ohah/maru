...
| P5b2b2 real PTY/RSS slow-observer artifact | 구현 | `zig build test-session-host-slow-observer-macos`가 driver와 별도 fixture host를 모두 ReleaseFast로 빌드한다. fixture host는 제품 `RuntimeManager`·`SocketServer`·`poll_owner.Owner`와 wire `runtime.spawn`의 실제 forkpty child를 사용하고 controller·읽기를 멈춘 observer·healthy observer 세 connection을 붙인다. private inherited DGRAM command/report fd는 host 진입 직후 CLOEXEC로 바뀌며 PTY child가 fd 3·4 부재를 먼저 검사한다. READY 뒤 세 client stream을 모두 drain하면서 500ms foreground refresh를 넘는 600ms source-settle 창에서 observation/core-lock/output-wake 증분 0을 먼저 증명하고 bounded retry 안에 정착하지 않으면 실패한 뒤, 별도 250ms idle 구간을 측정한다. pressure 시작 직전에만 slow observer 읽기를 멈춘다. reset ACK 뒤 phase-local bytes/progress만 세고, 실제 slow connection의 pollout 부재를 관측한 뒤 controller nonce가 PTY reader raw-byte counter와 healthy screen에 도달해야 한다. driver가 pinned host/child process identity, `proc_pid_rusage:RUSAGE_INFO_V4` raw samples, canonical ledger peak/final, exact waitpid·client EBADF·socket ENOENT·directory cleanup을 flat typed JSON으로 기록한다. validator는 unknown/missing/duplicate field를 거부하고 raw median/max·sample gap·RSS analytic cap·logical cap·final zero를 독립 재계산한다. CI `session host slow observer macOS` job이 매 PR 실행하고 artifact를 `if-no-files-found:error`로 업로드한다. | 단일 runtime/3-client macOS gate이며 P5b3 controller takeover, 외부 raw TTY·SIGWINCH·public attach·SSH packaging은 각각 P5b3/P5c/P5d 범위다. allocator가 RSS를 즉시 OS에 반환하거나 post-drain RSS가 pressure peak보다 작다고 가정하지 않고, logical 회수는 final ledger 0으로 별도 판정한다. |
...
- **상태: N2a 구현, N2b 구현 전.** N1 journal은 `RuntimeManager` final owner, 실제 OSC core slot, 기존 GUI
  `runtime.notification` consume과 same-PID outer optional handoff에 연결됐다. product config/label metadata,
  stable-key GUI wire와 daemon-internal macOS sink는 아직 연결되지 않았다.
...
- **N2b 필수 자동 검증:** config off·label snapshot, controller-only metadata update, stable-key GUI response-admission ack,
  permission denied degraded와 transient OS adapter retry를 Debug·ReleaseFast에서 검증한다.
