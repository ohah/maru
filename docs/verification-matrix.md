...
| macOS PTY opt-in | `mise run pty` | `tests/artifacts/integration/pty/*.raw.txt`, `*.screen.txt`, `*.snapshot.txt`, `*.surface.txt`, `runtime-backpressure.summary.txt` | macOS `openpty`가 controlled command stdout, process exit, resize를 실제 PTY로 전달하는지 확인한다. `SurfaceRuntime` 경로는 실제 PTY output이 `PtyReader -> PtyEventQueue -> RuntimeEventPump -> SurfaceRuntime -> Surface -> TerminalCore`를 통과하고, surface metadata artifact에 live handle/env가 새지 않는지 확인한다. 대량 stdout은 queue capacity 1로 marker count와 output event 수를 검증한다. 환경 의존이라 기본 `check`에는 아직 넣지 않는다. |
...
| PTY/openpty controlled command | 환경 의존 | `mise run pty`가 macOS `openpty` backend로 controlled command, exit status, resize propagation, SurfaceRuntime routing, reader thread + bounded queue + RuntimeEventPump 경로를 검증한다. 대량 stdout은 capacity 1 queue로 drop 없이 marker가 보존되는지 검증한다. 아직 기본 `mise run check`에는 포함하지 않는다. | macOS가 아닌 환경이나 로컬 PTY 정책 문제는 기본 CI만으로 잡지 못한다. PTY backpressure의 RSS/latency/UI responsiveness 성능 예산과 interactive shell shutdown lifecycle은 아직 별도 검증 전이다. macOS window/app loop가 pump를 frame/input lifecycle에 연결하는 검증도 아직 없다. | [PTY 운영 모델](pty-operating-model.md)에 따라 `tests/integration/pty/` artifact를 남긴다. 다음 단계에서는 app host가 pump를 실제 window loop에서 호출하는 경로와 interactive shell shutdown lifecycle을 추가한다. |
...
| font/layout/glyph atlas | 구현 전, 환경 의존 | CoreText resolve, fallback cache, glyph atlas cache key, emoji/color glyph, device scale 처리가 아직 없다. | 한글/CJK/이모지 정렬, fallback 성능, Retina glyph 선명도, font 설정 오류를 검증하지 못한다. | [폰트 전략](font-strategy.md)에 따라 fake font backend 기반 DrawList test를 기본 CI에 넣고, 실제 CoreText/screenshot/pixel smoke는 macOS opt-in으로 둔다. |
