...
- **C3-3b2b immutable preparation(미구현):** focused gate `test-session-host-2c3d-c3-3b2b`는 production-source dormant orchestration을
  ...
  near-cap old observation과 공용 replace의 모든 owned slice `capacity == len`, non-exact cache staged-entry 거부,
  reduced failure, pristine `cwd_host|agent_progress`, queue structural/content guard, allocation pre/post-return callback drift, local+ClientSlot
  alias 범위, success/failure exclusive typed handoff를 고정한다. cleanup keyed transcript는 PID+nonce+domain, final owner/plan address,
  correlation/event/owner/source identity, lifecycle, prepare-attempt incarnation, ordered slot descriptor 전부와 progress ordinal/step/bitset을
  묶고 cross-owner·same-address new-attempt·slot reorder·stale-progress replay를 dereference/free 전에 거부한다. cleanup proof exact이면 prior canonical owner만 reverse exact-once 회수하고,
  proof loss는 remaining dereference/free 0, addressless evidence 0~1과 direct `_exit` subprocess로 닫는다. malformed/oversized/unsupported/stale,
  allocator null OOM은 `.failure(.out_of_memory)`, checked size/cap/counter exhaustion은
  `.failure(.local_resource_exhausted)`이며 둘의 `EffectRequest`를 `.poison(.local_resource_exhausted)`로 고정한다. 실제 settlement와
  `Busy` retry는 b3 gate가 소유한다. peer oversized/malformed는 protocol
  rejection/violation이다. compile-time exhaustive oracle은 closed failure tag가
  `out_of_memory|local_resource_exhausted|protocol_error|connection_closed`와 exact인지 고정한다. ordinary OOM/cap/Busy/callback-declared error의 fatal 호출은 0이다. allocator provenance는 exact allocator
  pointer+vtable, requested extent/alignment와 typed role의 일치를 뜻하며 arbitrary allocator 정직성을 증명한다고 주장하지 않는다.
  b2b는 per-owner pure budget만, b3는 same-owner Busy 3회, b4는
  `16 * 3 * protocol.max_control_json`/16-owner product fairness를 소유한다.
- **b2b dependency boundary:** `process_seal_service`가 소유하는 cleanup input은 pointer-free neutral scalar/digest struct와 fixed-domain
  writer뿐이다. service는 `RemoteRuntime|PendingEventOwner|Attachment|EventCorrelation` 구현, allocator, GUI/app type을 import하지 않는다.
