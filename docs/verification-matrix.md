...
| 내장 SSH 클라이언트(프로토콜 코어) | 부분 구현, **상호운용 미확인** | L2 `src/session/ssh/`(패킷·와이어·버전 교환·KEXINIT·KEX)가 sans-io 라 실서버 없이 바이트 열로 돌아간다. 공개 근거가 있는 것은 셋이다: **실서버 원문 벡터**(OpenSSH 10.2 의 버전 줄과 KEXINIT 패킷을 그대로 박아 파싱·협상을 고정), **RFC 7748 §6.1 X25519 벡터**, 그리고 **명세 상수 대조**(메시지 번호·블록 크기·패딩 하한·표시자 이름을 RFC 숫자/문자열과 직접 맞댄다 — 상수를 쓰는 쪽과 읽는 쪽에 함께 쓰면 자기충족이라 실제로 변이가 살아남았다). `tests/boundary/ssh_sans_io.zig` 가 이 층의 OS 호출 0 을 허용 목록 + 디렉터리 순회로 강제한다. | **교환 해시 `H` 와 키 유도는 손으로 조립한 값과 맞댄 것뿐이다** — SSH KEX 에는 공개된 `H` 벡터가 없어, 명세 **해석**이 틀렸다면 테스트도 같이 틀린다. 서명 검증·인증·채널이 아직 없어 **실서버에 붙여 본 적이 없다**. 즉 지금 코어만으로는 중간자에 무방비이고, 상호운용이 증명된 바 없다. | 서버가 `H` 에 서명하므로 그 서명이 검증되면(S5) `H` 가 서버 것과 같다는 뜻이다 — 그 왕복을 S8 의 `mise run ssh-smoke` 확장이 실서버로 돈다. 그때까지 이 층은 "아직 상호운용이 확인되지 않았다" 로 읽는다. 단일 출처: [SSH 클라이언트 계약](ssh-client.md), [구현 계획](plans/ssh-client.md). |
