...
11. **브라우저 프로필 유지와 Keychain(13 차 적대적 검증)** — 지금 WKWebView `.browser` 탭은 **공유 ephemeral 저장소**(`browserDataStore`, 위 「untrusted 패널 격리」)라 탭끼리는 로그인이 이어지지만 앱을 끄면 사라진다. **결정(사용자 2026-09-24): 로그인·쿠키는 재시작 뒤에도 유지한다** — 엔진 중립 계약이므로 WKWebView 쪽도 신뢰 저장소와 격리된 **영속** 저장소로 바꾼다. CEF 쪽 실측(154, 샌드박스 켬, 같은 프로필로 두 번 띄워 `document.cookie`·`localStorage` 를 비교):
   ...
   - **`root_cache_path` 는 maru 전용 경로여야 한다.** 비우면 CEF 기본값 `~/Library/Application Support/CEF/User Data` 를 쓰는데, 이 자리는 **기본값을 쓰는 다른 CEF 앱과 겹쳐** 프로세스 singleton(exit 24)에 걸린다. 같은 이유로 maru 앱 인스턴스(번들 ID)마다 프로필 경로를 나눈다.
