...
9. **JS 대화상자·파일 선택 — 필수 구현(9 차 적대적 검증에서 실측)** — 대화상자 핸들러 없이 창 없는 브라우저에서 `alert` 를 부르면 **페이지가 멈춘다**: 「alert 전」 뒤로 「alert 반환」이 오지 않고, 그리기가 정지하며(프레임 수 고정), 이후 클릭에도 반응이 없다. 기본 대화상자가 뜨지 못한 채 페이지를 붙잡는 것이다. 그래서 `on_jsdialog`(`alert`/`confirm`/`prompt`)·`on_before_unload_dialog` 를 구현해 maru chrome 모달로 받고 콜백을 불러 줘야 한다 — 모달 게이트(위 「호스트가 라우팅을 든다」)와도 맞는다. `confirm` 도 똑같이 멈췄다(10 차 실측). **파일 선택은 다르다(10 차 실측)**: `<input type=file>` 을 누르면 페이지의 `click` 은 발생하는데 `on_file_dialog` 가 **불리지 않고**, 열기 창도 뜨지 않으며, 페이지는 멈추지 않는다(타이머 계속) — 조용히 안 열린다. 원인은 가리지 못했다(사용자 활성화·`file://` 문맥·창 없는 모드 중 무엇인지 spike 필요).
