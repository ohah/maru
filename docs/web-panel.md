...
- **콘텐츠 번들러 = zntc** (dev-only, 확정). 웹 빌드 환경은 `web/` 하위 Bun workspace를 기본 방향으로 둔다. VoidZero 계열은 번들러 대안이 아니라 필요 시 테스트/보조 도구 후보일 뿐이며, Vite+ 전체 도입은 zntc와 역할이 겹치므로 기본값에서 제외한다. **CEF는 미래 코어 옵션**(plugin 아님 — §13).
