...
- **새 창/팝업 (`target=_blank`·`window.open`) — browser kind, 슬라이스 7f (계획)**: WKWebView는 페이지가 새 창을 요청하면 `WKUIDelegate.webView(_:createWebViewWithConfiguration:for:windowFeatures:)`를 **동기 호출**한다(§8 링크 라우팅 `decidePolicyForNavigationAction`은 **같은 뷰 내 top-level 이동**만 — 새 창은 이 **다른 경로**다). 현재 maru엔 `WKUIDelegate`가 없어 새 창이 **무동작**이다(7e 범위 밖 — 단일 패널 브라우징만).
  ...
  **분해**: **7f-0(Zig: Swift-first web term 등록 ABI + adopted-create 전이 — 헤드리스: term 삽입·surface_id 발급·adopted create 전이가 중복 생성 안 함)** · **7f-1(Swift: `WKUIDelegate.createWebViewWith` → 7f-0 ABI로 term 등록 + adopt init + 새 탭 활성화)** · **7f-2(opener/OAuth 왕복 + 팝업 정책 게이트 — 보안 테스트)**. WKWebView 팝업 생성·opener·포커스는 스모크 밖이라 **GUI 손 테스트 필수**(7e-2b/7e-3 동형; 스크린샷은 WKWebView 픽셀 못 잡음). ABI vNNN(신규 export만).

  **에이전트 제어와의 연결**: 팝업을 1급 surface(`surface_id`)로 adopt하는 것이 곧 **에이전트가 팝업까지 제어**할 수 있는 전제다 — §13의 host-mediated 브라우저 MCP가 각 surface(팝업 adopt 포함)를 `surface_id`로 navigate/eval/click/console/screenshot 제어한다.
