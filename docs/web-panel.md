...
| ⑤ | 접근성(VoiceOver) | 공사 | **대** |
| ⑥ | 크기 전파 | ⑦과 묶인 공사 | 중 |
| ⑦ | 소유 버퍼 링과 반납 | **PoC 로 확인** — 단일 버퍼 찢어짐 2.09% → 링 0% | 중 |
...
1. **설치 배치 — `.app` 번들 불필요(결정 2026-09-23)** — 배포는 Homebrew **formula** 로 정했고(위 「구조와 배포」), PoC 는 처음부터 **번들 없는 배치**(실행 파일 하나가 자기 자신을 helper 로 쓰는 `browser_subprocess_path = self`)로 돌았다 — 154 의 모든 실측(렌더러·GPU 프로세스, 입력, IME, 브라우저 N 개)이 이 배치다. formula 는 이 배치를 brew prefix 에 그대로 설치하면 되므로 `.app` 번들은 만들지 않는다(함정 6 의 「번들 layout 에서 렌더러가 안 뜸」은 번들을 쓸 때의 문제라 해당 없음). 남는 확인은 **권한 귀속** 하나다 — 웹페이지가 카메라·마이크를 요청할 때 macOS 권한 창은 앱 번들의 사용 목적 문구(Info.plist)를 요구하고, maru 가 spawn 한 sidecar 의 요청은 Maru.app 쪽으로 귀속될 가능성이 높다(그러면 Maru.app Info.plist 에 문구를 넣는다). 이 기계(Mac Studio)는 마이크·카메라가 없고 시험 환경에서는 maru 를 셸에서 띄워 귀속 대상이 제품과 달라 **재지 못했다.**
   - **샌드박스(12 차 적대적 검증에서 발견·해결 확인)**: 지금까지의 PoC 실측은 **전부 샌드박스를 끈 상태**였다(`settings.no_sandbox = 1` — 렌더러·GPU 가 모두 `sandbox_check` 0). `.browser` 는 신뢰할 수 없는 웹을 띄우는 자리라 이대로는 안 된다. 공식 요구는 「helper `main()` 첫머리에서 **프레임워크를 불러오기 전에** `cef_sandbox_initialize`」다. ⑴ PoC 처럼 프레임워크를 링크 시점에 붙이고 실행 파일 하나를 helper 로 겸용하면, 샌드박스 초기화는 성공하지만 GPU 프로세스가 exit 5 로 반복해 죽고 브라우저가 「GPU 프로세스를 쓸 수 없다」며 종료한다. ⑵ helper 를 **별도 실행 파일**로 두고 샌드박스를 켠 **뒤** 프레임워크를 `dlopen` 하면, 샌드박스가 프레임워크 경로 읽기를 막는다(`file system sandbox blocked open()`) — 번들이 없으면 **실행 파일 디렉터리가 번들 경로**로 쓰이고 그 밖은 못 읽는다. ⑶ **프레임워크를 실행 파일 디렉터리 아래로 옮기자 동작했다**: GPU·유틸리티·렌더러 helper 가 전부 `sandbox_check` 1 이고(브라우저 프로세스는 원래 샌드박스 밖), 페이지 그리기·클릭·입력, select 팝업, JS 대화상자, 파일 선택(샌드박스 안의 렌더러가 고른 파일의 내용까지 읽음), 한글 조합이 모두 그대로였다. 즉 **번들 없이도 샌드박스가 된다** — formula 는 `libexec/` 아래에 sidecar·helper·프레임워크를 함께 둔다(심볼릭 링크는 샌드박스가 실제 경로로 풀므로 실제 파일로 둔다).
...
2. **원격 세션호스트** — IOSurface mach port 는 같은 기계 안에서만 유효하다. 다만 **웹 pane 은 PTY 가 없는 pane 슬롯**이라 브라우저를 원격 host 에서 돌릴 이유가 없다. **결정(사용자 2026-09-23): 브라우저 sidecar 는 항상 maru 앱이 도는 기계에서 돈다.** 그러면 터미널 세션이 SSH 너머에 있어도 제로카피가 유지된다. 이 결정으로 남는 것:
