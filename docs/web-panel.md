...
- **divider 드래그·hover 커서**(↔/grip)도 본문 한정 + 드래그 중 통과로 처리.
  ...
  - **파일 도크 확장(FP8 부분 구현, FP9 정합 보강 계획)**: 같은 `seam_edges`를 도크 WKWebView에도 leaf 기하에서 계산해 right outer, editor-group 내부와 editor↔project-tree 경계의 left/right/bottom 비트를 준다. 다만 현재 Swift `MaruWebPanelView.dividerGrabBand=10pt`와 Zig hit rect가 같은 상수를 쓰지 않는다. 특히 group divider는 Zig `max(3px, dividerThicknessPx)`만 잡아 WebView가 통과시킨 10pt 중 일부가 아무 divider에도 맞지 않는 dead band가 된다. FP9는 `dock_layout.divider_grab_band_pt=10`을 현재 v130→v131 `WebSurfaceTransitionAbi.divider_grab_band_pt` 끝 필드로 전달해 Swift 독립 상수를 제거한다(선행 ABI 증가 시 다음 번호). producer는 `AppSession.collectWebSurfaces`, backing 변환은 `ceil(pt*scale_milli/1000)`, 0은 disabled다. hover/down/native pass-through/cursor는 같은 넓은 clamp grab rect를 쓰지만 render는 `layoutDividers`의 기존 1px/configured line rect를 유지한다. resize 동안 WebView는 숨기지 않고 surface diff `reframe`만 적용한다.
  - **hover cursor까지 terminal과 동형(FP9 계획)**: native pass-through만으로 WKWebView 자식의 arrow cursor가 남으면 최상단 chrome overlay가 ABI의 같은 grab rect에서만 resize cursor와 raw mouse 전달을 맡는다. overlay/Swift는 orientation·좌표를 전달할 뿐 resize target·offset·size commit을 판정하지 않고, FP9에서 신설할 Zig `PointerGestureOwner`와 divider action이 단일 정책을 유지한다. 따라서 seam overlay 배선은 분산 drag flag를 이 union으로 전환한 뒤에만 붙인다. 이로써 WebView 안쪽 band에서도 hover→down→drag→up이 terminal divider와 같은 rect·cursor·첫 delta를 쓴다. `split.divider-thickness=0`이면 ABI band도 0으로 내려 pass-through와 cursor를 함께 끈다.
...
**③ CSP(응답 헤더)**: 엄격 CSP를 응답에 항상 부착 — 외부 네트워크(`connect-src 'none'`)·임의 frame/worker·`<base>` 하이재킹·form-action exfil 차단. 문자열 단일 출처는 Zig의 host-role별 `app_csp_header`와 `render_csp_header`다. app은 exact same-origin worker를 위해 `worker-src 'self'`, render는 `worker-src 'none'`이고 scheme asset resolver도 `live-preview-worker.js`를 app host에만 제공한다. `script-src 'self'`, `frame-src maru-app://render`, `connect-src 'none'`, `base-uri 'none'`, `form-action 'none'`과 critical style SHA-256 핀은 유지한다. build integrity manifest가 shell/worker 두 bundle digest를 검증하고 HTML에는 shell SRI만 삽입한다. Swift는 Zig가 role별로 반환한 CSP를 붙일 뿐 host policy를 재판정하지 않는다. `unsafe-inline`·blob/data worker는 허용하지 않는다.
