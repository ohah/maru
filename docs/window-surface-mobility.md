...
| **M3d-2b Swift 창 수명 + 드래그 (범위 밖 — 손 테스트)** | M3d-2a 이동 API에 Swift 배선: NSWindow **생성**(창 밖 drop)·**focus** 착지·빈 source **close**(`source_window_closed` 신호 → `request_window_close`); palette/menu action; 드래그. control-plane 라이브 구독 재평가(movedOut/movedIn dispatch) | M3d-2a | cross-window 이동이 command/드래그로 동작; focus 착지·empty-source 창 close | **높음**(창 수명·control-plane 라이브 구독·AppKit 드래그) | **예** — palette로 창 간 workspace 이동·merge; 소스 창 auto-close | macos-app 실측 + `/code-review max` |
