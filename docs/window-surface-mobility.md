...
- **`LiveSurfaceRegistry`는 `LivePtySession`만 인스턴스화·소유한다.** `app_live_registry: LiveSurfaceRegistry(app.LivePtySession)`(app_session.zig:1604, 앱 전역 모듈-로컬 `var`, `smp_allocator`), Term은 `rt.live_pty: *LivePtySession`로 그 슬롯을 **참조만** 한다(M2b). Surface(=core)는 미포함.
- **`SurfaceRuntime`(입력/resize/명령 라우팅)은 per-window다.** `AppSession.runtime: app.SurfaceRuntime`(창마다 하나). 라우팅 표 `Link{surface_id, *Surface, pty_id, pty_io}`가 창별로 분리돼 있다. 앱 전역 라우팅 owner는 **없다**.
