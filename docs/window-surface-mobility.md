...
1. **M0a SurfaceIdAllocator**: 앱 인스턴스 전역 opaque u64 발급, 단조·비재사용 단위 테스트. 기존 per-session `next_id`(app_session.zig)는 외부 ID로 노출하지 않는다. 착수 결정(코드 대조로 확정):
   ...
   - **generation은 M0a 범위 밖**. surface_id 비재사용(단조)이 주 방어이고([control-plane.md] §3 defense-in-depth), generation은 보조 키다. **정책 소유자(정정, 적대적 리뷰 반영)**: M1(WindowGraph)·M2a(LiveSurfaceRegistry)는 generation을 **보존만** 하고 증가시키지 않는다(둘 다 완료). generation이 증가하는 유일 경로(crash-respawn으로 surface_id 유지+generation++, 새 런타임 인스턴스)는 아직 어느 slice에도 배정되지 않았다 — production `Term.live_pty` 소유를 옮기는 **M2b**가 respawn 연산과 함께 generation 증가 정책을 정한다(M1이 아니다). **타입**: generation은 `u64`다(`SurfaceDto.generation`·`LiveSurfaceRegistry.Entry.generation` 정합).
...
4. **M2 LiveSurfaceRegistry 분리**: terminal live runtime을 window 밖 owner로 이동. surface 이동 시 PTY/TerminalCore를 재시작하지 않음을 테스트. web panel state/WKWebView handle은 이 시점에 아직 없으므로 terminal runtime만 옮기고, web surface runtime은 Phase 4 이후 같은 registry에 합류한다.
   ...
   - **M2b(후속, production 마이그레이션)**: 현재 `TermRuntime.live_pty: LivePtySession`은 heap-pin `*Term` 안에 **inline value**로 소유되고(`createTerm` init → `destroyTerm`/창 close/`AppSession.deinit`가 직접 teardown), 앱 전역 `LivePtyRegistry`(비소유 close-index, 현재 smoke/test 전용)와는 별개다. M2b가 이 inline 소유를 `LiveSurfaceRegistry`(앱 전역 owner)로 올리고 `Term`은 `surface_id`로 참조하게 바꾼다. **주의(범위 판정 근거)**: (1) `TerminalCore`는 `LivePtySession`이 아니라 `Surface`(=`Term.surface`) 안에 있고 SplitTree leaf·렌더러·수백 곳의 `term.surface` 접근과 얽혀 있어, §3이 말하는 "registry가 TerminalCore를 소유"의 완성은 `Surface`를 `Term` 밖으로 빼는 대수술(M3+)을 요구한다 — `live_pty`만 옮기면 §3의 절반만 이루고 cross-window 이동도 못 하므로 reader-불변식이 걸린 35k줄 파일에 순 churn만 남는다. (2) reader thread가 `&live_pty.reader`와 `&surface.core`/`&surface.core_mutex`를 **교차 바인딩**하므로, 소유자를 registry/Term으로 쪼갤 때 teardown 순서(detach-before-deinit)를 정확히 지켜야 한다. 그래서 M2b는 실제 `app_session` 수술 + `zig build macos-app` 실행 검증을 동반하는 별도 slice다. registry teardown 계약(`Rt.deinit()`)에 detach 단계를 어떻게 실을지(coordinator가 `closeAndDetach` 선행 vs registry가 teardown ctx 수령)도 M2b가 정한다.
