...
**M3e 구현 정정(drift gate, 2026-07 — 코드 실측)**:
- **v1은 이미 이동 *배치*를 살렸다.** `saveWorkspace`가 각 세션의 라이브 트리를 위치 순서로 캡처하고 restore가 블록 순서로 창을 재생성하므로, M3d 이동으로 `*Tab`이 목적지 세션에 옮겨진 결과가 v1에서도 재시작 후 그 창에 남았다. 따라서 M3e의 진짜 델타는 "배치를 살림"이 아니라 **(a) 활성(key) 창 보존**(v1은 마커가 없어 마지막-생성 창이 key가 됐다)·**(b) window_id/window_kind를 포맷에 명시**(위치-암묵 → 명시)다.
- **활성 창 인코딩 = per-window `active-window=1` 플래그**(문서-레벨 라인 아님). 프로덕션 저장은 각 `AppSession`이 자기 창 블록을 내고 Swift가 헤더 하나로 모으는 **per-session 블록 조립**이라(AppRuntime가 전 창을 iterate하는 단일 지점이 없음 — §8A.6 라이브 미러 금지와 정합) 문서-레벨 라인을 쓸 곳이 없다. 각 세션이 자기 key 여부(`window?.isKeyWindow`)를 플래그로 자기서술하고, parse가 `workspace.activeWindowIndex`(첫 `active-window=1`)로 문서-레벨 인덱스를 파생한다. Swift가 `workspace_active_window` getter로 그 인덱스를 읽어 복원 loop 뒤 그 창을 다시 key로 올린다.
- **`window_id`는 포맷 완결성용**(재사용 안 함): 복원은 새 NSWindow에 새 토큰을 발급하므로 저장된 window_id를 재사용하지 않는다(surface_id 재채번과 동형). 미래 window-identity-stable 기능(control-plane 재연결)을 위한 필드다.
- **저장 파일 경로는 안정 유지**: `~/Library/Application Support/maru/workspace.v1`(파일명 `.v1`은 경로일 뿐, 포맷 버전은 내용 헤더 `maru.workspace.v2`). 경로를 바꾸면 옛 파일이 고아로 남고 v2 저장이 옛 v1 내용 파일을 in-place로 덮어쓰는 self-heal이 깨진다.
- **ABI**: `maru_macos_app_session_serialize_workspace`에 `window_id`(u64)·`window_kind`(u32)·`is_active`(u32) 인자 추가 + `maru_macos_app_session_workspace_active_window`(활성 창 인덱스, 없으면 -1) getter 신설. 헤더 상수는 `.h`/Zig 단일 출처(`MARU_WORKSPACE_HEADER`==`workspace.header`==`maru.workspace.v2`, ABI 테스트가 강제).
...
| **M3e 직렬화 → WindowGraph 포맷 (구현)** | `.v1`→`.v2`(window_id·window_kind·active-window을 `window` 라인에 명시; 활성=per-window `active-window=1` 플래그, parse가 `activeWindowIndex`로 파생). `serialize_workspace` ABI에 세 메타 인자·`workspace_active_window` getter 추가. Swift saveWorkspace가 세 값(토큰·normal·isKeyWindow)을 넘기고 restore가 활성 창을 다시 key로. 옛 v1 파일=BadHeader→조용한 폴백(v1 reader 없음), 저장 경로는 안정 유지(in-place self-heal). **정정**: v1은 이미 배치를 위치로 살렸으므로 M3e 델타=활성 창 보존+포맷 명시(위 §8A.6 drift 노트) | M3d-2 | **이동 결과가 재시작 후 살아남음**(배치+활성 창); 멀티-윈도우 round-trip | 중(포맷 변경·복원·멀티창) | **예** — 창 간 이동 후 ⌘Q·재실행, 배치(어느 창에 무엇)+활성 창 복원 확인 | round-trip 테스트 ✅ + v1 폴백·이동-재시작 유지 테스트 ✅ + gate green ✅ |
