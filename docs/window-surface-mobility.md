...
| **M3d-2 라이브 배선 + Swift (범위 밖 — 손 테스트)** | M3d-1 트랜잭션을 **라이브 per-window 트리 수술**에 배선: 기존 same-window 수술(`promotePaneToNewWorkspace`·`mergePaneIntoWorkspace`·`detachPaneFromTab`)을 `self→(src_session, dst_session)`로 일반화 = 두 `AppSession` 트리 재부모화 + **양-창 리프레시**(surface_ptrs·app_window.tabs·resize(dst cell metric)·rebuildSidebar·metal_dirty) + per-link `trace_recorder` 재지정. Swift NSWindow **생성**(창 밖 drop)·**focus** 착지·빈 source **close**(`request_window_close`); palette/menu action; 이동 통합 테스트 | M3d-1 | cross-window 이동이 command로 동작; focus 착지·empty-source 창 close·그룹 잔존/소멸 정확 | **높음**(양 트리 수술·양-창 렌더 리프레시·창 수명·control-plane 라이브 구독) | **예** — palette로 창 간 surface/pane/workspace 이동·merge; 소스 창 auto-close; 그룹 승계/소멸 | 두 controlled_smoke AppSession 이동 테스트 + macos-app 실측 + `/code-review max` |
| **M3e 직렬화 → WindowGraph 포맷** | `.v1`→`.v2`(window_id·window_kind·active-window); capture 출처 AppRuntime 뷰로; 복원 재조립; 옛 파일 조용한 폴백 | M3d-2 | **이동 결과가 재시작 후 살아남음**; 멀티-윈도우 round-trip | 중(포맷 변경·복원·멀티창) | **예** — 창 간 이동 후 ⌘Q·재실행, 배치(어느 창에 무엇) 복원 확인 | round-trip 테스트 + move-then-restart 실측 |
