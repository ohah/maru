...
- **복원 clamp**: 저장 frame이 **어떤** `NSScreen.visibleFrame`과도 충분히 교차하면 그대로(맞는 모니터·크기 보존), 아니면(모니터 분리·레이아웃 변경으로 화면 밖) main 화면 `visibleFrame`으로 재배치·clamp(Swift `clampFrameToVisibleScreens`). `constrainFrameRect` 참고하되 명시 clamp.
- **capture/복원**: `captureWorkspaceWindow`/`serializeWorkspaceWindow`에 `frame: ?Frame` 인자를 더하고, ABI `serialize_workspace(…, has_frame, frame_x/y/w/h)` + 창별 getter `workspace_window_frame(session, text, window_index, *out_x/y/w/h) i32`(1=있음/0=없음/-1=parse실패)를 추가. Swift `saveWorkspace`가 `window.frame`→점 정수로 저장, `restoreWorkspace`/`createTerminalWindow`가 창마다 getter→clamp→`setFrame`(없으면 현행 cascade 유지). primary(창0)·추가 창 둘 다. 단일 출처 저장 포맷·clamp 상세는 [workspace-restore.md](workspace-restore.md) "창 geometry 복원(M3f)".
