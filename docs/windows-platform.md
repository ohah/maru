...
- **W8.7a — 한 창에 둘이 뜬다.** 기하를 중립에서 받아 터미널을 `geometry.terminal` 에, 파일 트리를
  ...
  **⒜1 과 ⒜2 로 갈렸다**(실측 뒤). ⒜1 은 **창이 갈리는 것**까지 — 기하·격자·도크 배경·디바이더.
  ⒜2 가 파일 트리를 그 자리에 넣는데, 그것은 터미널과 **아틀라스를 나눠 써야** 해서(두 프레임이
  같은 `RendererState` 를 지나야 글리프 슬롯이 안 충돌한다) 별개의 배선이다.
...
**따로 본 것(보고, 안 고침).** 창 가장자리에 **글자 조각이 남는다** — 합성 전 스크린샷(오른쪽 끝)과
합성 후(왼쪽 끝) 둘 다에서 보인다. **합성이 만든 것이 아니라 그 전부터 있던 것**이고, 원인은 이
슬라이스 밖이다(스크롤백 재그리기 또는 클립). W8.7b 가 입력을 붙이며 다시 본다.
