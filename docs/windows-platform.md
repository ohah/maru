...
## 그 자리에서 본 것 — **제목 글자가 codepoint 하나씩 밀린다**(보고)

캡처를 찍다 발견했다. 에이전트 목록이 **비어 있고 훑는 중도 아닐 때**, 도크 제목이

```text
Agent session history   →   @f dm̀s rdrr hm̀ gfr sn qx
```

로 그려진다 — `A→@`, `g→f`, `e→d`, `n→m`, `t→s`. **모든 글리프가 앞 codepoint 로 한 칸씩 밀렸다.**
같은 화면의 다른 글자(`Local`·`Newest first`·`Workspace`·`Search sessions`)는 멀쩡하다.

`origin/main` 바이너리로 **재현했다** — 이 슬라이스가 만든 것이 아니다. 이 변경이 그 상태를 덜 보이게
만들 뿐이다(훑는 동안에는 `loading` 이라 다른 문구를 그린다), **빈 이력을 가진 사용자는 훑기가 끝난
뒤에도 그 제목을 본다.**

성질상 **글리프 → 아틀라스 슬롯 대응**이 한 칸 어긋나는 것으로 보이고(§2m.57 의 "두 font id 체계의
값 범위가 겹쳤다" 와 같은 부류), 제목만 겪는 이유는 아직 모른다. **별개 슬라이스로 보고만 한다.**
