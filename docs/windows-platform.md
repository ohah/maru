...
- **도크 막대는 안 쟀다.** 같은 헬퍼·같은 포인터 경로를 쓰지만 스모크의 트리가 **안 넘친다**
  (`dock_scroll=unjudgeable reason=content_fits`). 사이드바 쪽만 실측이다.
- **가로 스크롤바가 없다** — 긴 파일 이름은 여전히 잘린다.
- **트랙 클릭을 스모크가 안 밟았다**(`track_clicks=0`). 배선과 중립 계약은 있지만 실측은 thumb
  끌기뿐이다.
