...
**후보창 위치를 지정하지 않는다.** macOS는 조합 후보창을 커서 위치에 띄우는데, Windows에서 그렇게 하려면
`ImmSetCandidateWindow`/`CANDIDATEFORM`으로 좌표를 줘야 한다. 지금은 OS 기본 위치(창 좌상단 근처)다 —
셀 좌표를 IME에 넘기는 자리는 마우스·선택(W7.4d)과 같은 계층이라 그때 함께 정한다.
