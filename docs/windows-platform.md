...
**아직 하드코딩인 것**(정직하게 적는다): 테마 색·팔레트·`max_scrollback` 은 이 함수가 들고 있고,
...
### 2m.26 소스 컨트롤 표면이 Windows 화면에 뜬다 (W8.4, 실측 2026-08-23)

**새로 짠 것이 거의 없다.** 조각이 이미 다 있었다:

| 조각 | 어디 |
|---|---|
| git 프로세스 러너 | §2m.8·§2m.9 — `win32_process` 캡처 러너, `git_backend` 의 `comptime` 갈래 |
| 상태 모델 | `session/scm_view.build` — 중립 |
| 행 → `DrawList` | `buildDockScmDrawList` — 본문에 `coretext_*` 참조 **0 회**(§2m.6 과 같은 측정) |
| 창·표현 | §2m.20 의 공용 호스트 |

이 슬라이스는 그 넷을 잇는다. `maru win32-scm-draw-smoke` 가 **제품 진입점**으로 git 을 부르고
(`submitRepoStatus` → 백그라운드 스레드 → `takeRepoStatusResult`, §2m.9 와 같은 길) 저장소 자신의
변경 목록을 그린다.

```text
repo=D:/ohah/maru
branch=feat/w8-4-windows-scm-surface ahead=0 behind=0
model_rows=4 drawn_rows=5 empty=false
branch_drawn=true names_matched=3/3
d3d_cells=98 cells_digest=0x94EB2895ECDC9325
```

화면에는 브랜치 헤더, `∨ Changes 3` 섹션, 파일 세 줄(아이콘 + 파일명 + 디렉터리 + 상태 글자)이
그려지고 선택 행이 accent 로 강조된다.

**판정은 행 수가 아니라 내용이다** — 그려진 셀에서 글자를 도로 읽어 모델이 말한 **브랜치 이름**과
**파일 이름**이 실제로 그 행에 있는지 본다. 저장소 상태가 무엇이든 성립하는 판정이라 fixture 에 안 묶인다.

**그 판정이 처음엔 틀렸다 — 이 세션에서 세 번째다.** 경로 끝 12 바이트(`src/main.zig`)를 찾았는데
`0/1` 이 나왔다. 그려진 것을 찍어 보니:

```text
[scm miss] row=2 want-tail="src/main.zig" got="main.zigsrc/M"
```

그 컴포넌트는 **전체 경로를 한 덩어리로 안 그린다** — 아이콘 + **파일명** + 디렉터리 + 상태 글자
순이다(VS Code 의 SCM 목록과 같은 모양). 파일명으로 보게 고치자 `3/3` 이 됐다. **판정이 틀린 것이지
렌더가 틀린 것이 아니었다.**

> 이 세션에서 같은 부류를 셋 겪었다: 빈 파일의 `0/6`(잴 것이 없는 것을 실패로 셈), 클릭의
> `1822/5454`(caret 의미를 글자 의미로 오해), 여기 `0/1`(그리는 모양을 안 보고 판정을 지음).
> **판정이 낮게 나오면 구현보다 판정을 먼저 의심한다.**

**표본이 저장소 상태에 달렸다.** 변경 파일이 없으면 `names_matched=0/0` 이고 그것은 실패가 아니라
**판정 불가**다(§2m.23 이 같은 구별을 스크롤 대본에서 세웠다). 위 실측은 파일 셋을 만들어 잰 것이고,
깨끗한 트리에서는 `model_rows` 와 `empty` 가 그 사실을 말한다.

**아직 없는 것**: 클릭(행 선택·비교 열기)·스크롤·접기. 데이터와 그리기가 섰으니 그다음은 §2m.24 가
편집기에서 한 것과 같은 배선이다 — `scm_dock` 은 히트테스트를 자기 `view` 가 갖고 있다.
...
| **W8.4** | **소스 컨트롤** — ⒜ 프로세스 러너(**완료**, §2m.8·§2m.9 — `CreateProcessW` + 파이프) ⒝ 표면이 화면에 뜬다(**완료**, §2m.26) ⒞ 클릭·스크롤·접기(다음) | W8.1 |
