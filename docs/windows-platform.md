...
**채택 후보 — 오탐의 공통 모양은 하나였다: `\` 뒤가 *정규식 클래스 한 글자*(+선택적 수량자)다.**
`\d`·`\w`·`\s`·`\S`·`\n`·`\d+`·`\s*`·`\d{2,4}`·`\p{L}`가 전부 그 모양이고, 진짜 경로 세그먼트는
`src`·`build`·`Makefile`처럼 두 글자 이상이거나 클래스 글자가 아니다. 그래서 규칙은 **"세그먼트가
클래스 글자 하나이거나 클래스 글자+수량자면 거부"** 하나면 된다. POSIX 접두는 손대지 않아 회귀가 0이고,
...
| 갈래 | 상태 |
|---|---|
| `dot_relative`(`.\x`·`..\x`)·`home_path`(`~\x`) | **규칙이 있다.** 위 세그먼트 규칙으로 오탐 0·미검출 0 |
| `bare_relative`(`src\main.zig`) | **규칙을 못 찾았다.** 후보 네 계열이 전부 깨졌다 — 정책 결정이 필요하다 |
