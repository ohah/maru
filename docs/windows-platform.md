...
**수정마다 지키는 것이 다르다 — 대조군으로 확인했다.**

| 수정 | 지키는 것 | 되돌렸을 때 |
|---|---|---|
| W8.1 ⑴ 리프 열기 갈래 | 단위 테스트 | `zig build test` rc=1 |
| W8.1 ⑵ 루트 접두 판정 | 스모크 | rc=3 |
| W8.1 ⑶ 입구 정규화 | 단위 테스트 | `zig build test` rc=1 |
| W8.2 ⑴ `.iterate` | 스모크 | rc=1 |
| W8.2 ⑵ inode 채우기 | 스모크 | rc=1 |
