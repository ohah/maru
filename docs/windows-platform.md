...
### 3.2a 미해결 — OSC 9;9엔 authority가 없다

아래는 검토 과정을 그대로 남긴 것이다 — 결론만 적으면 다음 사람이 같은 오답을 다시 거친다.

**W1이 막히는 지점은 메커니즘이 아니라 위험 수용이다.** 아래 C(9;9은 host를 건드리지 않는다)는 비용이 0이고
A보다 엄격히 나으며 B·E를 나중에 얹는 것을 막지 않는다. 즉 **최소 메커니즘은 이미 정해져 있다.** 정해야 할
것은 *"C만으로 출시해도 되는가"* — 즉 **원격 Windows 케이스의 잔여 위험을 받아들일 것인가**다.
...
- **OSC 9;9의 host를 어떻게 볼 것인가**(§3.2a). 후보 A~E와 각각이 못 막는 것, 폐기한 논거, 결정 전에 재야 할
  것까지 §3.2a에 전부 적어 뒀다. **값싸면서 완전한 안이 없다** — 맨 `ssh` 세션에는 정보가 애초에 없다.
