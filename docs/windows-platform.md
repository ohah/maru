...
본다(앞 12 바이트 접두). 빈 줄은 셀이 없으므로 뺀다. 어느 파일이든 성립하는 판정이라 fixture 에 안 묶인다.
...
| **W8.3** | **에디터 표면** — ⒜ 본문·gutter 가 Windows 화면에 뜬다(**완료**, §2m.21) ⒝ 스크롤바·배경 사각 — `d3d11_cells` 가 예고한 **쿼드 파이프라인**이 여기서 필요하다(스크롤바 8px vs 셀 9px) ⒞ 입력 라우팅. 선행이던 셰이퍼 상한 둘은 §2m.19 로 걷었다 | W8.2 |
