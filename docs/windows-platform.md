...
### 2m.94 굴러가는데 막대가 안 보였다 — **폭 0 짜리 테두리** 때문에 quad 를 통째로 버리고 있었다 (실측 2026-08-31)

§2m.92 로 에이전트 목록이 굴러가기 시작했는데 **스크롤바가 화면에 없었다.** 오른쪽 34px 을 세어 보니
전부 배경이었다(캡처 픽셀 계량). 그런데 중립은 그 막대를 **발행하고 있었다**:

```text
DBGSB track=(466,210,8,307) thumb=(466,251,8,164)
```

자리는 정해져 있고 그리지만 않은 상태다 — 어떤 통계도 안 움직인다(`error_skip` 도 0 이다).

## 무엇이 버리고 있었나

op 을 찍어 보니 둘 다 **테두리 역할을 달고** 왔다:

```text
DBGQ bar quad rect=(466,210,8,307) grad=.solid border=.divider clip=… alpha=255
DBGQ bar quad rect=(466,251,8,165) grad=.solid border=.divider clip=… alpha=255
```

`appendChromeOps` 는 *"그라디언트·테두리는 이 셰이더에 없으므로 **세어서 남긴다**"* 는 규칙으로
`q.border_role == null` 이 아닌 quad 를 `dropped` 로 접는다. 그 규칙 자체는 옳다 — **판정이 틀렸다**:

`tree.PaintStyle` 은 기본 테두리 역할(`.divider`)을 **늘 싣고**, 그릴 것이 없으면 **폭을 0 으로** 둔다.
`scrollbarPaint` 가 정확히 그 모양이다(`border_widths_px = .{0,0,0,0}`). 즉 **그릴 테두리가 없는데도**
역할이 있다는 이유로 quad 전체가 버려졌다.

이제 **폭을 함께 본다**(`hasDrawableBorder`) — 역할이 있어도 네 폭이 전부 0 이면 테두리가 없는 것이다.
이 규칙은 편집기·SCM 이 함께 쓰는 자리라(그 함수 doc), 같은 이유로 사라지던 다른 quad 도 함께 살아난다.

## 판정 — **자리와 그림을 함께 잰다**

```text
agent_bar: track_drawn=true thumb_drawn=true track=(210,307) thumb_y 210->352.803 thumb_h=164.197 agent_bar_ok=true
```

- **⒜ 그렸나**: 발행된 rect 와 같은 자리에 **셀이 있는지** 본다. 발행 rect 만 보면 이 결함이 그대로
  통과한다 — 자리는 정해져 있었기 때문이다.
- **⒝ 끝이 끝을 가리키나**: offset 0 이면 thumb 이 track 맨 위, 상한이면 **맨 아래**여야 한다.
  가운데 값을 다시 계산해 견주면 **컴포넌트를 재구현**하는 꼴이라 끝 둘만 본다.

허용 오차는 **1.5px** 이다. 발행 rect 는 레이아웃의 실수값이고 draw op 은 정수 사각형이라 마지막에
반올림이 한 번 있다(실측: thumb `164.197 → 165`). 그보다 좁게 잡으면 **그렸는데 못 찾는다** — 처음에
0.6 으로 잡았다가 그렇게 빨개졌다.

| 뮤턴트 | 무엇이 빨개지나 |
|---|---|
| 테두리 **역할만** 보고 버린다(**옛 동작**) | `track_drawn=false thumb_drawn=false` |
| 목록 길이를 안 준다(`scroll_content_height_px = 0`) | 중립이 막대를 **발행조차 안 한다** → `track=(0,0)` |

## 아직 아닌 것

- **막대를 손으로 못 끈다.** 중립이 track/thumb 과 드래그 payload 를 발행하지만 Windows 는 그 액션을
  아직 안 받는다 — 지금은 휠뿐이다(계획서 W8.22).
- **`dropped` 는 여전히 조용하다.** 이번 결함이 그렇게 숨었다 — 그 수가 화면에서 무엇이 사라졌는지는
  말하지 않는다. 세는 것과 보이는 것을 잇는 자리는 아직 없다.

### 2m.95 막대를 잡아 끈다 — **드래그는 intent 가 아니라 `DragEvent` 로 온다** (W8.22, 실측 2026-08-31)

§2m.94 로 스크롤바가 보이게 됐는데 **잡히지는 않았다.** 눌러도 목록이 안 움직였다.

## 왜 안 잡혔나 — 표면이 그 값을 버리고 있었다

중립 `interaction.dispatch` 는 **셋**을 돌려준다: `action`(클릭 인텐트)·`dirty`·**`drag`**. 그 타입이
둘의 관계까지 적어 뒀다 — *"click action 과 배타적이지 않다: threshold 를 넘지 않은 up 은 `action` 만,
**넘은 up 은 `drag` 만** 낸다"*.

`win32_agent_surface.Routed` 는 `intent` 와 `dirty` 만 실었다. 그래서 끄는 동안 **아무것도 안 왔다** —
실측으로 down·move·up 셋 다 `intent=null` 이었고 목록은 그대로였다. 필드 하나를 더해 그대로 넘긴다.

## 잡은 지점은 **`down` 에서** 정해야 한다

처음에는 `began`(threshold 를 처음 넘은 순간)에서 `grab_dy = y - thumb_y` 를 쟀다. **그 순간은 이미
움직인 뒤다** — 40px 내린 자리에서 재니 "거기를 잡았다" 가 되어 thumb 이 **제자리**였다(판정이
`thumb 251.18 -> 251.18` 로 그것을 잡았다).

이제 `down` 좌표로 정한다. 잡았는지 판정하는 규칙은 중립이 갖고 있다 — `trackContains` 는 **보이는
막대가 아니라 거터 전체**로 판정한다(그 doc: 8px 막대를 정확히 찍어야만 잡히는 것은 의도가 아니다).
thumb 밖이면 `offsetForTrackClick` 으로 **그 자리로 뛰고**, 이어서 끄는 것은 thumb 을 잡은 것과 같다.

**up 에 오는 막대 인텐트는 안 쓴다.** 누르는 순간 이미 답했으므로 그때 다시 뛰면 **끌어서 놓은 자리를
누른 자리로 되돌린다.**

## 판정 — 끝만 보면 `grab_dy` 를 안 잰다

```text
agent_bar_drag: judgeable=true grabs=1 off 77->267(max 267) thumb 251.183->291.296(step 40) thumb_bottom=517 track_bottom=517 agent_bar_drag_ok=true
```

- **가운데를 잡고 딱 40px 내린다** — thumb 도 정확히 그만큼 내려가야 한다. 이것이 *"잡은 지점을
  유지한다"* 의 뜻이다. **끝(바닥)만 보면 clamp 때문에 `grab_dy` 가 틀려도 초록이 된다** — 실제로
  `grab_dy = 0` 뮤턴트가 그 상태에서 통과했다.
- 이어서 바닥까지 끌어 `offset == max` 와 `thumb 바닥 == track 바닥` 을 본다.
- 목표 offset 을 `offsetForPointer` 로 다시 계산해 견주지 않는다 — **같은 함수로 자기를 채점**하는
  꼴이다. 대신 **픽셀 이동량**(포인터 40 ↔ thumb 40)과 **끝**을 본다.

| 뮤턴트 | 무엇이 빨개지나 |
|---|---|
| 표면이 `drag` 를 안 넘긴다(**옛 동작**) | `off 77->77`, `thumb` 제자리 |
| 잡은 지점을 안 지킨다(`grab_dy = 0`) | `thumb 251.18->352.80` — 40 이 아니라 **101** 움직였다 |

## 아직 아닌 것

- **키보드(Page/Home/End)** 는 여전히 없다.
- **SCM 목록**은 그대로다(계획서 W8.22).
- ~~**hover 강조**: … Windows 는 아직 그 상태를 안 넘긴다~~ → **틀린 주장이었다**(§2m.104 에서 정정).
  포인터 이동이 이미 중립 `dispatch` 로 가 `hovered` 가 서고, `resolveCard` 가 그 자리에서 색을 바꾼다.

### 2m.96 막대 드래그 적대적 검증 5회 — **제스처의 주인**과 판정이 안 밟던 길들 (실측 2026-08-31)

§2m.95 가 초록으로 끝난 뒤 다섯 번 두들겼다. **가장 큰 것은 "잡은 것을 언제 놓는가"** 였다.

## 1회차 — 도크 밖에서 떼면 **계속 쥐고 있었다**

에이전트 포인터 블록은 `region == .dock_content` 안에 있다. 막대를 잡고 도크 밖으로 나가 떼면 그 up 이
이 블록에 **안 들어와** `agent_bar_drag` 가 남는다 — 다음에 도크 위를 지나가기만 해도 목록이 튄다.
**`capture_lost` 도 같은 구멍**이었다: 그 종류는 `kind` 매핑의 `else => continue` 로 조용히 버려졌다.

## 2회차 — 판정이 그 길을 **안 밟고 있었다**, 그리고 그러자 진짜 원인이 나왔다

1회차의 첫 고침(영역 조건에 `or agent_bar_drag != null` 을 더한 것)에 **뮤턴트를 걸었더니 초록이었다** —
판정이 도크 밖으로 나가지 않았기 때문이다. 밖으로 나갔다 떼는 단계를 넣자 그 고침이 **틀렸다**는 것도
같이 드러났다: 도크 밖 좌표는 **사이드바·터미널 핸들러가 먼저 먹는다**(그것들이 영역 판정 뒤에 있다).

그래서 막대 처리를 **영역 판정 앞**으로 옮겼다 — Windows 자체 스크롤바 블록이 같은 자리에 있는 이유와
같다(*"제스처는 시작한 곳이 소유한다"*). 옮기고 나서야 밖에서도 목록이 따라왔다(`267 → 155`).

```text
agent_bar_escape: judgeable=true off 267->155(밖에서) ->0(뗀 직후) ->0(지나간 뒤) agent_bar_escape_ok=true
```

**기대를 한 번 틀렸다.** 처음에는 "뗀 뒤 값 == 밖에서 끌던 값" 을 걸었는데, 뗀 자리(y=200)는 track 위쪽
이라 offset 0 이 **맞는 답**이다. 기준은 *"뗀 직후"* 여야 한다 — 그 뒤 지나가기만 했을 때 안 움직이는지가
이 판정이 묻는 것이다.

## 3회차 — **track 을 눌러 뛰는 길**에 판정이 없었다

드래그 판정은 thumb 을 잡는 길만 밟는다. `offsetForTrackClick` 이 통째로 죽어도 초록이었다. 판정을
세우며 **무장 함정 둘**을 지났다:

- 누르는 자리가 thumb **안**이면 잡기가 되어 `jumps=0` 이다.
- 너무 아래면 그 자리에 thumb 가운데를 놓으려다 **상한에 걸려** clamp 가 서고, 5.35px 어긋난다.

둘 사이(thumb 바로 아래)를 골라야 *"누른 자리에 thumb 가운데가 온다"* 를 잴 수 있다:

```text
agent_bar_jump: judgeable=true jumps=1 off 0->202 press_y=400.197 thumb_mid=400.137 agent_bar_jump_ok=true
```

## 4회차 — 잡은 채로 **뷰가 바뀌면** 그대로 남았다

뷰 스위처는 `dropWheelResidue()` 를 부르는데 쥔 막대는 안 놓았다. 그 막대는 화면에 없는데도 상태가
살아 있어, 돌아왔을 때 지나가기만 해도 튄다. 같은 자리에서 함께 놓는다.

## 5회차 — **이웃 제스처를 훔치는가**

영역 판정 앞에 블록을 하나 더 놓았으니 그 앞에 있던 것들이 상할 수 있다. 사이드바 막대·도크 막대·카드
클릭·터미널 선택이 전부 그대로 초록이다(`sb_bar_ok`·`dock_bar_ok`·`agent_click_ok`·`selections=1`).

**다만 `m.kind != .left_down` 가드에는 판정이 없다** — 그것을 빼도 아무 판정이 안 빨개졌다. 그 가드는
up 을 잃은 뒤 들어오는 down 이 삼켜지는 것(도크가 잠깐 안 눌리는 상태)을 막는데, 그 상태를 스모크로
만들려면 up 을 일부러 잃어야 한다. **보고만 한다.**

## 남은 자리

- ~~**hover·drag 강조**~~ → **틀린 주장이었다**(§2m.104 — 이미 동작한다: 포인터 이동이 중립
  `dispatch` 로 가 `hovered` 가 서고 `resolveCard` 가 색을 바꾼다).
...
### 2m.100 소스 컨트롤 목록도 굴러간다 — 그리고 **넘치는 상태는 사용자가 만든다** (W8.22, 실측 2026-09-01)

§2m.91 이 *"SCM 은 이번 시나리오에서 안 넘친다 — 배선은 같으니 길어지면 같은 일이 난다"* 로 남긴 자리를
갚는다. 에이전트 목록(§2m.92)과 **같은 규칙**을 그대로 쓴다.

## 높이 규칙은 이미 중립에 있었다

에이전트 때는 항목 높이 규칙을 새로 세워야 했는데(§2m.92 — `session_dock/scroll.zig`), 이쪽은
`types.DockMetrics.itemHeight` 가 **이미 소유**한다(머리 줄·섹션·파일 행·커밋 줄·커밋 상자…). 없던 것은
그 함수를 `scroll_area.project` 가 부를 수 있게 감싸는 것뿐이라, 그 어댑터만 중립에 세웠다
(`scm_dock/scroll.zig` — macOS 도 `app_session/scm_dock.zig` 에 같은 모양을 갖고 있어 그쪽이 옮겨 올 수 있다).

## 투영은 **표면 안에서** 한다

에이전트는 호스트가 목록을 만들어 넘기지만, SCM 은 표면이 `git status` 원문에서 항목을 짓는다. 그래서
투영도 그 안에서 한다 — 항목이 만들어진 **바로 다음**이다. 호스트는 offset 과 지난 뷰포트 높이를 주고,
표면이 상한과 **발행된 뷰포트 높이**를 돌려준다.

**뷰포트 높이를 손으로 빼지 않는다.** macOS 는 고정 chrome(탭 줄·요약 줄·브랜치 줄)을 빼서 구하는데 그
함수 주석이 *"하나라도 빠뜨리면 목록이 자기 자리보다 크다고 믿고 스크롤 범위가 어긋난다"* 고 적어 뒀다.
발행된 값(`build.scrollTextViewport`)을 되읽으면 그 산수의 주인이 하나로 남는다.

## 넘치는 상태는 **"모두 보기"** 뒤에 온다

판정을 세우다 알게 된 사실이다. 섹션은 기본 **10 행까지만** 보인다(중립 `default_section_rows` — 그 값의
doc: *"섹션이 스크롤 저 아래로 밀려 있는지조차 모르게 된다"*). 그래서 파일이 40 개여도 목록은 12 행이고
**안 넘친다**. 넘치는 상태는 사용자가 그 줄을 눌러 만드는 것이고, 판정도 그 길을 밟는다.

## 저장소를 안 건드리고 잰다

넘치는 목록이 필요한데 파일을 만들면 **사용자의 작업 트리가 바뀐다**. 그래서 `git status` 원문을
**메모리에서** 길게 지어(파일 40 개) 표면에 넘긴다 — 그 표면의 입력이 그 문자열 하나라서 가능하다.
디스크는 안 만진다.

```text
scm_scroll: judgeable=true max=531 off=190 first_y 58->-132 thumb_bottom=511 track_bottom=511 scrolls=2 scm_scroll_ok=true
```

- **`first_y` 가 핵심이다** — offset 만 보면 속 빈다. 발행된 첫 행이 정확히 그만큼(190) 올라갔다.
- **끝은 막대가 말한다.** 행 rect 로 재려다 두 번 헛디뎠다(엉뚱한 행을 집어 `-20`, 그다음엔 아무 id 도
  안 맞아 `-1`). 발행된 thumb·track 은 어느 목록이든 같은 사실을 말한다 — 끝까지 굴리면 thumb 바닥이
  track 바닥이다. 스크롤바가 그려지는지도 함께 재진다.

| 뮤턴트 | 무엇이 빨개지나 |
|---|---|
| 그린 자리에 offset 을 안 쓴다 | `first_y 58->58` — 상태만 움직인다 |
| SCM 뷰에서 휠을 안 받는다(**옛 동작**) | `off=0 scrolls=0 thumb_bottom=266 ≠ track_bottom=511` |

## 판정 하나가 **음수를 "없음" 으로 읽고** 있었다

`first_y` 를 `>= 0` 으로 "값이 있나" 판정했는데, **굴리면 첫 행은 뷰포트 위로 올라가 y 가 음수**다
(실측 `-132`). 정상값을 못 읽은 것으로 접어 판정이 빨갰다 — 본 적이 있는지를 **별도 bool** 로 든다.

## 아직 아닌 것

- **막대 드래그는 아직이다** — 에이전트에는 붙였지만(§2m.95) SCM 은 휠뿐이다.
- **키보드(Page/Home/End)** 도 아직이다.
- **탭마다 따로 기억하지 않는다** — 변경/히스토리 탭을 오가면 같은 offset 을 쓴다. macOS 는 탭별로
  기억한다(`rememberScrollExtent`).

### 2m.101 SCM 스크롤 적대적 검증 5회 — **같은 사실의 출처가 둘이었다** (실측 2026-09-01)

§2m.100 이 초록으로 끝난 뒤 다섯 번 두들겼다.

## 1회차 — 합성 status 가 나머지 판정으로 새는가 → **오늘은 안 샌다, 다만 덫이다**

판정이 `git status` 원문을 메모리에서 갈아 끼우는데, 그 뒤로 앱은 계속 그 가짜를 본다. 오늘은 상태바
항목이 프레임 루프 **앞에서 한 번** 만들어지고 그 뒤 SCM 판정이 없어 새지 않는다. 그래도 **판정 하나만
뒤에 붙어도 조용히 가짜 저장소를 읽는다** — 진짜 원문을 옆에 들고 있다가 판정이 끝나면 되돌려 창을 좁혔다.

## 2회차 — **같은 사실의 출처가 둘이었다**(진짜 결함)

`appendScmDockCells` 가 **게이트는 `inputs.status`**, **그리는 것은 `opts.status_text`** 를 봤다. 호출부가
하나만 갱신하면:

- `opts` 만 갱신 → 게이트가 옛 빈 값을 보고 **표면이 통째로 안 그려진다**.
- `status` 만 갱신 → **옛 목록이 남는다**.

이 슬라이스에서 실제로 둘을 함께 갱신해야 했다 — 그 순간이 이 결함의 증거다. 필드를 지우고 원문의
출처를 하나로 만들었다. 뮤턴트(`opts` 를 안 갱신)에서 `judgeable=false max=0` 으로 빨개진다.

## 3회차 — 다른 표면의 자리를 건드리는가

§2m.96 5회차가 에이전트에서 잡은 그 누수를 SCM 판정에도 세웠다 — 굴리는 동안 탐색기 트리와 에이전트
목록의 offset 이 그대로여야 한다.

```text
tree 641->641 agent 0->0          (뮤턴트: tree 0->641)
```

## 4회차 — 목록이 **짧아지면** 빈 바닥이 보이는가

파일이 스테이지되면 목록이 짧아진다. 끝까지 굴린 자리에서 status 를 짧게 갈아 끼워 그 상황을 만든다:

```text
scm_shrink: off 531->0 max_after=0 bar_after=false ok=true   (뮤턴트: off 531->531)
```

## 5회차 — 스크롤바가 **넘칠 때만** 나는가

안 넘치는데 막대가 남으면 없는 여백을 있다고 말하는 셈이다(그 자리 주석이 사용자 지적으로 그렇게
정해 뒀다). 여기서 **뮤턴트 둘이 각각으로는 안 걸렸다** — 그것이 이 자리의 성질을 드러낸다:

- `list_overflows = true` 만 → 막대 없음(길이가 짧아 thumb 이 안 선다).
- `content_h_px` 만 부풀림 → 막대 없음(거터가 0 이라 기하가 `null` 이다).
- **둘 다** 거짓말해야 막대가 뜬다 → `bar_after=true` 로 빨개진다.

즉 중립이 **두 입력이 모두 "넘친다" 라고 해야** 막대를 낸다. 판정은 그 결합을 재는 것이고, 한쪽만
틀리는 실수에는 애초에 막대가 안 뜬다.

> **함께 보고한다**: 그래서 `list_overflows`(거터 예약) 자체를 단독으로 재는 판정은 없다 — 화면에서
> 그 값만 틀리면 **12px 여백이 이유 없이 생기거나 사라진다**. 눈으로만 보이는 자리다.

## 남은 자리

- **SCM 막대 드래그**(에이전트에는 §2m.95 로 붙였다)·**키보드**·**탭별 offset 기억**.

### 2m.102 소스 컨트롤 막대도 잡힌다 — 에이전트에서 배운 셋을 그대로 (W8.22, 실측 2026-09-01)

§2m.100 으로 SCM 목록이 굴러가는데 **막대는 못 잡았다**. 에이전트에 이미 있는 배선(§2m.95)을 같은
모양으로 옮긴다 — 그리고 §2m.96 이 다섯 번 두들겨 얻은 **세 가지를 처음부터** 지킨다.

## 옮긴 것 — 그리고 처음부터 지킨 셋

1. **표면이 `DragEvent` 를 안 버린다.** 중립 `interaction.dispatch` 는 `action`·`dirty`·**`drag`** 셋을
   돌려주고, threshold 를 넘은 up 은 **`drag` 만** 낸다. `Routed` 가 앞의 둘만 실으면 끄는 동안
   **아무것도 안 온다**(에이전트가 정확히 그 상태였다).
2. **잡는 것은 `down` 이다.** `began` 은 threshold 를 넘은 **첫 이동**에 오므로 그때 `grab_dy` 를 재면
   이미 밀린 자리를 "잡은 자리" 로 적는다 — thumb 이 제자리가 된다.
3. **끄는 동안은 영역 판정 앞에서 답한다.** 뒤에 두면 도크 밖으로 나간 순간 사이드바·터미널이 그
   이벤트를 먼저 먹어 **막대를 쥔 채로 남는다**. `capture_lost` 도 같은 자리에서 놓는다.

thumb 밖을 누르면 `offsetForTrackClick` 으로 그 자리로 뛰고, 이어서 끄는 것은 thumb 을 잡은 것과 같다.

## 판정

```text
scm_bar_drag: judgeable=true grabs=1 thumb 58->98.05(step 40) off_end=531/531 thumb_bottom=511 track_bottom=511 ok=true
```

**가운데를 잡고 딱 40px** 내린다 — thumb 도 40.05px 갔다. 끝만 보면 clamp 때문에 `grab_dy` 가 틀려도
초록이라(§2m.95 가 그 함정을 지났다) **픽셀 이동량**을 먼저 본다. 목표 offset 을 `offsetForPointer` 로
다시 계산해 견주지 않는다 — 같은 함수로 자기를 채점하는 꼴이다.

| 뮤턴트 | 무엇이 빨개지나 |
|---|---|
| 표면이 `drag` 를 안 넘긴다(**옛 동작**) | `thumb 58->58`, `off_end=0`, `thumb_bottom=266 ≠ 511` |
| 잡은 지점을 안 지킨다(`grab_dy = 0`) | `thumb 58->202` — 40 이 아니라 **144** 움직였다 |

## 판정이 **인쇄 시점의 값**을 보고 있었다

`off_end == scm_scroll_max` 을 걸었는데, 인쇄는 훨씬 뒤에 하고 그 사이 §2m.101 의 "짧아지기" 단계가
목록을 줄여 상한이 **0** 이 된다(`off_end=531/0`). 살아 있는 값과 견주면 그 판정은 *"지금"* 을 재고
*"그때"* 를 못 잰다 — 잰 순간의 상한을 함께 챙긴다.

## 아직 아닌 것

- ~~**hover·drag 강조**~~ → **틀린 주장이었다**(§2m.104 — 이미 동작한다).
- **키보드(Page/Home/End)** · **탭별 offset 기억**.

### 2m.103 판정이 **개발자의 작업 트리**에 기대고 있었다 (실측 2026-09-01)

스모크 출력에 이 줄이 오래 있었다:

```text
scm_dock_click=unjudgeable reason=no_file_row
```

*"소스 컨트롤 행을 눌러 본다 — 뷰가 바뀌었는데 안 눌리면 죽은 컨트롤이다"* 라고 적힌 판정인데,
**변경 파일이 하나도 없으면 무장하지 않는다.** 커밋 직후에는 늘 그 상태다 — 즉 그 판정은 **개발자가
마침 손댄 파일을 갖고 있을 때만** 돌았다. §2m.88 이 `dock_scroll` 에서 본 것과 같은 종류(접힌 판정)이고,
이번에는 접힌 이유가 **환경**이라 더 조용했다.

## 저장소를 안 건드리고 결정적으로 만든다

파일을 만들면 **사용자의 작업 트리가 바뀐다**(이 저장소의 규율). §2m.100 이 스크롤 판정을 위해 쓴
도구를 함수로 뽑아 같이 쓴다 — `git status` 원문을 **메모리에서** 짓는다(`smokeScmStatus`). 그 표면의
입력이 그 문자열 하나라서 가능하다. 판정이 끝나면 **진짜 원문으로 되돌린다**(§2m.101 1회차의 덫).
...
```text
scm_dock_intents=2 scm_dock_redraws=5 scm_selected=null
```

인텐트는 둘 왔는데(`open_row{model_index=1}` 실측) **아무것도 안 골라졌다**. 결함이 아니라 **판정이
인쇄 시점의 상태를 읽고 있었다** — 인쇄는 스핀 12000 대에서 하고, 그 사이 목록이 바뀌며
`invalidateTree` 가 포인터 상태를 버린다. 누른 **직후**에 답을 챙긴다.

## 그리고 판정이 무엇을 재는지 못 말하고 있었다

`selected != null` 만 보면 *"눌리기는 한다"* 까지다. 두 가지를 더 건다:

- **누른 그 행**인가(`selected == 누른 index`) — 아무 행이나 골라도 초록이면 안 된다.
- **fixture 가 만든 행**인가(`row` 가 `smoke_` 로 시작) — 그래야 이 판정이 개발자의 트리와 **무관하게**
  같은 답을 낸다는 것을 판정 스스로 말한다.

```text
scm_dock_intents=2 scm_dock_redraws=5 scm_selected=1(want 1) row=smoke_0.zig scm_dock_click_ok=true
```

| 뮤턴트 | 무엇이 빨개지나 |
|---|---|
| fixture 를 안 깐다(**옛 동작**) | `row=main.zig` — **내 작업 트리**가 답을 만들었다는 것이 그대로 보인다 |
| 인쇄 시점 상태를 읽는다(**옛 동작**) | `scm_selected=null(want 1)` |

## 남은 자리

- **스모크에 `unjudgeable` 이 이제 0 이다**(실측). 다시 생기면 그것은 환경이 아니라 배선이 접힌 것이다.
- **탭은 그려지는데 안 바뀐다** — `select_tab` 은 Windows `State.apply` 가 `false` 를 낸다(그 자리 테스트가
  *"이 슬라이스 밖의 intent 는 false 를 낸다"* 로 못 박아 뒀다). 히스토리·에이전트 탭 내용이 선행이라
  이번 슬라이스에서 안 건드렸다 — **보고만 한다.**

### 2m.104 "막대 색이 안 변한다" — **재 보니 이미 변하고 있었다**(정정) (실측 2026-09-01)

§2m.95·§2m.102 의 "한계" 절과 계획서 W8.22 가 세 번에 걸쳐 이렇게 적고 있었다:

> **hover·drag 강조**: 중립이 상태별 색 규약을 갖고 있지만 Windows 는 아직 그 상태를 안 넘긴다 —
> 잡아도 막대 색이 안 변한다.

**그 주장은 재 보지 않고 적은 것이었고, 틀렸다.**

## 잰 것

스모크에서 SCM 막대의 thumb 한가운데로 포인터를 옮기고, 그 표면의 셀 지문과 `interaction.hovered` 를
앞뒤로 찍었다:

```text
DBGH before digest=d3c20a5fe0427cba hovered=5343102c   (다른 행)
DBGH after  digest=d8ed766510b67fbe hovered=53430005   (= NodeIds.scroll_thumb)
```

**호버가 서고 그림이 바뀐다.** 배선은 처음부터 있었다 — 포인터 이동이 `interaction.dispatch` 로 가
`state.hovered` 를 세우고, `ui_paint` 가 track·thumb 을 **card visual** 로 그리며(그 파일 주석:
*"눈에 보이는 것은 그 자식들과 track/thumb(각각 card visual)"*), `paint_style.resolveCard` 가 그 자리에서
색을 바꾼다 — capture 면 `tab_active_bg`, hover 면 `row_hover_bg`.

## 왜 틀린 채로 세 번 실렸나

에이전트 막대를 붙일 때(§2m.95) "안 해 본 것" 을 한계로 적으면서 **안 해 본 것과 안 되는 것을 뭉쳤다.**
그 문장이 다음 슬라이스(§2m.102)와 계획서로 **그대로 복사**됐다. 세 자리를 함께 정정한다.

> **규율**: 한계 절에 *"X 가 안 된다"* 를 적으려면 **X 를 한 번 재야 한다.** 안 재 봤으면 그렇게
> 적는다 — *"안 재 봤다"* 와 *"안 된다"* 는 다른 사실이고, 뒤엣것은 다음 사람에게 **없는 일감**을 만든다.

## 아직 아닌 것

- **막대 전용 색**은 없다 — 행과 같은 hover 색(`row_hover_bg`)을 쓴다. 그것이 의도인지 아닌지는
  디자인 결정이라 여기서 정하지 않는다.
- **판정은 없다.** 이 정정은 프로브로 잰 것이고, 상시 판정으로 세우려면 "호버하면 그 셀의 색이 바뀐다"
  를 재는 자리가 필요하다 — 지문만으로는 **무엇이** 바뀌었는지 말하지 못한다.

### 2m.105 목록이 **키보드로도** 굴러간다 — 그리고 그 넷의 주인이 정해져 있다 (W8.22, 실측 2026-09-01)

휠과 막대는 붙었는데 `PageUp`/`PageDown`/`Home`/`End` 는 그대로 셸로 갔다. 긴 목록에서 **끝으로
가는 길이 휠뿐**이면 마흔 번을 굴려야 한다.

## 규칙은 이미 정해져 있었다 — 세 문서가 이미 소유한다

여기서 새로 정한 것은 **없다.** 찾아보니 셋이 이미 답을 갖고 있었다:

| 무엇 | 어디 |
|---|---|
| 한 항목을 남기는 page step | `chrome.ui.scroll_area.pageStepPx` (*"Page 키는 일부러 카드 하나를 화면에 남긴다"*) |
| 그 넷의 **순서표** | [key-input-and-shortcuts.md](key-input-and-shortcuts.md) §목록 스크롤 키의 주인 |
| 소유권은 ScrollArea 밖 | [scroll-area.md](scroll-area.md) §4.5·§4.6 |

순서표가 정한 자리는 이렇다 — 검색 필드 → 오버레이 → **소스 컨트롤 도크 → 에이전트 도크** → 파일
탐색기(거기서는 **선택 이동**이다) → 터미널. Windows 도 그 순서를 그대로 밟는다.

**탐색기는 특히 받지 않는다.** scroll-area 문서의 그 정정(*"탐색기는 그 넷을 이미 쓰고 있고 의미가
다르다"*)이 그대로 여기에도 걸린다. Windows 에는 아직 그 선택 이동이 없으므로 **아무것도 안 하고
흘려보낸다** — 삼키지도 않는다.

## 없던 것은 「도크가 키를 들었는가」 하나였다

macOS 는 `agent_session_dock_key_focus` 로 그것을 든다(그 자리 doc: *"도크가 보이는 것만으로는
부족하고 사용자가 도크 안을 눌러 키보드를 넘겨준 상태여야 한다"*). Windows 에는 그 축이 아예
없었다. 같은 규칙으로 붙였다 — **도크 안 primary down 이 들고, 터미널·사이드바를 누르면 놓는다.**

**한 자리에서만 세운다.** 아래 분기들이 각자 `continue` 로 빠져나가므로 그 안에 흩어 놓으면 어떤
경로에서는 안 잡히고 어떤 경로에서는 안 놓인다 — "카드는 잡는데 빈 여백은 안 잡는다" 가 그 증상이고,
사용자에게는 **가끔 되는 키**로 보인다. macOS 가 이미 그 실패를 겪었다(그 자리 주석: *"⚠️ 아래 소스
컨트롤 분기는 `return` 하므로 이 줄을 그 뒤에 두면 소스 컨트롤에서는 영영 실행되지 않는다 — 실제로
그렇게 넣었다가 제품에서 키가 하나도 안 먹는 것을 사용자가 잡았다"*).

## 중립이 규칙을, 순수 함수가 판정을 갖는다

- **`scroll_area.applyKeyStep`** 을 새로 뒀다. 넷을 offset 변화로 옮기고 휠 잔여를 버린다.
  scroll-area 문서가 *"키보드 스크롤도 같은 offset 을 움직이는 동작이므로 ScrollArea 가 제공한다"*
  라고 이미 적어 둔 자리다. **반환값은 「픽셀이 움직였는가」이지 「키를 먹었는가」가 아니다** — 둘을
  뭉치면 맨 위에서 친 PageUp 이 뒤의 터미널로 새어 스크롤백이 감긴다.
- **`win32_keys.listScrollStep`** 이 VK 가 아니라 중립 `KeyEvent` 를 받아 그 넷인지 답한다.
  **수식키 가드도 그 안이다**(`⌘Home` 같은 조합은 다른 주인이 있다). 이벤트 루프에 인라인으로
  적으면 그 규칙을 잴 자리가 루프뿐인데, **합성 키는 수식키를 못 세운다**(창이 OS 키보드 상태를
  읽는다) — 즉 `Shift+PageUp` 을 목록이 삼켜도 판정이 없다. 함수로 빼서 단위 테스트가 잡는다.

## 판정 둘을 세웠다 — 그리고 「안 되는 것」을 먼저 잰다

```text
dock_keys: judgeable=true focus false->true off 531(blurred 531) home=0(again 0) page=429(want 429)
           end=531/531 past 2->3->3 scrolls=6 consumed=7 dock_keys_ok=true
agent_keys: judgeable=true search true->false(off 267) focus false->true off 267(blurred 267)
           home=0 page=221(want 221) end=267/267 scm 0->0 agent_keys_ok=true
```

- **놓았을 때 안 굴러가는가**(`blurred 531`) — 이 자리가 없으면 *"늘 굴러간다"* 는 뮤턴트가 그대로
  초록이고, 제품에서는 터미널에 치던 PageUp 이 목록을 흔든다.
- **검색이 먼저인가**(`search true->false(off 267)`) — 도크 검색이 키를 든 동안 친 Home 은 목록을
  안 움직여야 한다. 순서표 1번이 3·4번보다 앞이라는 그 사실이다.
- **키가 도크를 지나갔는가**(`past 2->3->3`) — 이 대목에 마침 파일 뷰가 떠 있어, 도크를 못 지나간
  키는 그 다음 가드가 센다. 놓았을 때는 **늘고**(게이트가 죽은 코드가 아니다), 들었을 때는 **안
  늘어야** 한다(굴리고 또 흘리면 같은 키가 두 번 일한다). 이 둘이 **순서까지** 말한다.
- **경계에서도 먹는가**(`home=0(again 0)`, `consumed=7 > scrolls=6`) — 맨 위에서 한 번 더 친 Home 은
  픽셀을 못 움직이지만 도크가 먹는다.
- **갈래를 잘못 안 타는가**(`scm 0->0`) — 에이전트를 굴리는 동안 SCM offset 이 안 움직인다.

**기대값을 `pageStepPx` 로 다시 계산하지 않는다** — 같은 함수로 자기를 채점하는 꼴이다(§2m.95 의
규율). 뷰포트 높이와 항목 높이에서 판정이 **직접** 뺀다.

| 뮤턴트 | 무엇이 빨개지나 |
|---|---|
| 소유권 게이트를 없앤다 | `blurred 0` — 놓았는데도 굴러간다 |
| page step 에서 한 항목을 안 남긴다 | `page=453(want 429)`, 그리고 **중립 단위 테스트 2 개** |
| 본문을 눌러도 키를 안 넘긴다 | `focus false->false`, `home=531`, `scrolls=0` |
| 수식키 가드에서 `shift` 를 흘린다 | `listScrollStep` 단위 테스트 |
| 삼키지 않고 아래로 흘린다 | `past 5->6->9` |

## 아직 아닌 것

- **탐색기 뷰**는 그 넷을 안 받는다(위). 선택 이동이 서면 그때 같은 자리에 붙는다.
...
### 2m.107 도크를 한 번 다시 그리는 데 **17 ms** 가 들고 있었다 — 그 중 15 ms 가 폰트 폴백 (실측 2026-09-01)

W8.22 의 남은 항목 **가상화**를 하려던 참이었다. 그 전에 재 봤다(*"안 재 봤다" 와 "느리다" 는 다른
사실이다* — §2m.104 의 규율).

## 재 보니 가상화 문제가 아니었다

에이전트 뷰에서 도크를 세 번 다시 짓고 잰다(ReleaseFast):

```text
DBGV agent items=8 records=12 entries=24 us=16994 / 16668 / 16514
```

**항목이 여덟인데 17 ms 다.** 즉 비용이 목록 길이에 안 붙어 있다 — 가상화로 줄일 것이 아니었다.
단계를 갈라 재니 한 자리였다:

```text
DBGT build=34 view=24 prep=12  shape=16552  resolve=34 runs=16 raster=229 upload=0   (µs)
```

셰이핑 안을 다시 갈랐다:

```text
DBGS calls=54  format=127µs  fallback=15093µs  layout=836µs
```

**`attachFallback` 이 54 번에 15 ms 다** — 런 하나에 ~280µs. 그리고 그 함수는 매 런마다
`CreateFontFallbackBuilder` → `AddMapping` × 후보 → `GetSystemFontFallback` → **`CreateFontFallback`**
을 처음부터 다시 했다. 마지막 것이 시스템 폰트 목록을 훑는다.

시작할 때 쌓인 값은 더 크다 — 첫 측정 창에서 **`calls=4002`, fallback 누계 1.09 초**였다.

## 고친 것 — 목록을 한 번만 짓는다

폴백 목록이 무엇이 되는지는 **`(주 face 이름, 폴백 CSV)`** 에서만 나온다(`fallbackCandidates` 가 그
둘만 본다). 크기·굵기는 안 들어간다 — 그것들은 format 이 갖는다. 그래서 그 짝을 키로 `Shaper` 에
하나 캐시한다. 제품에서 그 짝은 사실상 하나이므로 **한 칸이면 족하다**.

**키에 구분자를 넣는다**(`primary ++ 0 ++ csv`). 그냥 이어 붙이면 `("ab","c")` 와 `("a","bc")` 가 같은
키가 되어 **폰트를 바꿨는데 옛 폴백을 계속 쓰는** 상태가 된다 — 화면에서는 *"글꼴을 바꿨는데 일부
글자만 안 바뀐다"* 로 보인다. 그 성질만 순수 함수로 떼어 단위 테스트를 세웠다.

**키를 못 담으면 캐시하지 않고 예전처럼 짓고 놓는다** — 느려질 뿐 답은 같다. 그리고 **옛것은 새 키를
담는 데 성공한 뒤에** 놓는다: 먼저 놓고 할당이 실패하면 캐시가 빈 채로 남아 다음 런이 또 짓는다.

## 결과

두 뷰를 각각 세 번 다시 짓고 잰다(ReleaseFast, 같은 기계·같은 스핀):

| 뷰 | 캐시 없음(**옛 동작**) | 캐시 |
|---|---|---|
| 소스 컨트롤 | 6435 · 5913 · 5830 µs | **743 · 579 · 555 µs** |
| 에이전트(항목 8) | 16994 · 16668 · 16514 µs | **2465 · 1997 · 1922 µs** |

**한 프레임 예산이 16.7 ms 인데 에이전트 도크 한 번 다시 그리는 것이 그것을 통째로 먹고 있었다** —
휠 한 눈금, 호버 한 번, 검색 글자 하나가 모두 그 값이었다.

> **남은 2 ms 가 무엇인지는 안 쟀다.** 폴백이 아니라는 것까지가 이번에 잰 것이다.

## 판정은 **시간이 아니라 횟수**를 잰다

시간은 기계마다 흔들리지만 *"다시 그릴 때마다 또 짓는가"* 는 안 흔들린다. `CreateFontFallback` 을
실제로 부른 횟수를 세고, 도크를 세 번 더 지어 그 수가 **안 늘어야** 한다:

```text
shaper_fallback: judgeable=true redraws=3 shapes 4488->4545 builds 1->1 shaper_fallback_ok=true
```

**무장 조건이 공허하면 안 된다** — 셰이핑이 실제로 더 돌았는지(`shapes`)를 함께 본다. 그것 없이
`builds` 만 보면 **아무것도 안 그린 프레임**도 초록이다. 그리고 `builds_first > 0` 도 건다: 한 번도
안 지었으면 캐시가 도는 것이 아니라 이 경로가 안 밟힌 것이다.

**그런데 횟수만 재면 절반이다.** 키가 틀려 **엉뚱한 목록**을 내줘도 그 자리는 초록이다 — 오히려
`builds` 는 더 안 는다. 그래서 둘째 판정이 *"캐시가 내주는 것이 방금 지었을 것과 같은가"* 를 잰다:
**처음 보는 face 이름**으로 두 번 셰이핑해 ⑴ 첫 번은 짓고 ⑵ 둘째 번은 안 짓고 ⑶ 글리프가 한 글자도
안 다른지 본다.

```text
shaper_cache_same: judgeable=true glyphs=3 built true/false equal=true via_fallback=true shaper_cache_same_ok=true
```

**폴백이 실제로 갈리는 글자를 골라야 한다.** `Consolas` 에 한글이 없어 다른 face 에서 나온다(실측:
`MalgunGothic`) — 그 자리가 안 서면 이 판정은 폴백 목록을 아예 안 지나간 셈이라 공허하다. 그리고
**`GlyphRecord.fallback` 플래그로는 그것을 못 잰다**: 실측으로 `false` 였다(그 값은 주 face 를 열어
견주는 자리에서 나오는데 이 경로에서는 그 face 가 안 열린다). 나온 face **이름**이 부탁한 이름과
다른지로 잰다.

| 뮤턴트 | 무엇이 빨개지나 |
|---|---|
| 캐시를 안 본다(**옛 동작**) | `builds 4488->4545` — **셰이핑 수와 정확히 같다**(런마다 한 번) |
| 키를 안 본다(늘 적중) | `built false/false` — 처음 보는 face 인데 안 지었다 · **키 단위 테스트** |
| 키 구분자를 뺀다 | 키 단위 테스트 세 줄 |

## 아직 아닌 것

- **format 은 안 캐시한다** — 실측 2.4µs/런이라 지금은 값이 없다. 나중에 이 자리가 다시 보이면 그때.
- **가상화는 여전히 안 한다**(W8.22). 이 측정이 말한 것은 *"지금 병목이 거기가 아니다"* 이지
  *"길어도 괜찮다"* 가 아니다 — SCM 목록은 `rows_buf` 가 256 에서 끊고 그 끝을 «더 있다» 행이 말하지만
  (중립 모델의 규약), 에이전트 목록에는 그런 상한이 없다.
- **여러 스레드에서 셰이핑하면 이 캐시는 안전하지 않다.** 지금은 `sharedShaper()` 자체가 잠금 없는
  프로세스 전역이라 **이미 단일 스레드 전제**다 — 캐시가 그 전제를 새로 만들지는 않는다.
- **키가 바뀌는 길은 제품에 없다.** 판정이 그 길을 억지로 만들어 밟지만, 제품에서 폰트를 런타임에
  바꾸는 자리는 아직 없다.
- **둘째 판정은 이 기계의 폰트에 기댄다.** `Consolas` 에 한글이 없고 시스템에 한글 face 가 있어야
...
### 2m.110 셸 게이트를 Zig 로 옮기기 시작한다 — 넷 중 하나 (실측 2026-09-02)

§2m.109 가 남긴 물음이 *"Windows 에서 어떤 셸을 지원하는가"* 였다. 사용자 결정은 **셸 의존을 없앤다**
쪽이다 — 그래서 게이트가 셸로 나가는 자리를 하나씩 Zig 로 옮긴다.

## 다른 터미널은 어떻게 하나 — 읽어 봤다

| 프로젝트 | 정확성 게이트(빌드·테스트) | POSIX 스크립트 |
|---|---|---|
| **Alacritty** | Windows 잡이 기본 셸에서 `cargo test` 를 그대로 돌린다 | CI 한 단계의 `grep`·`sed` 곁다리뿐 |
| **WezTerm** | Windows 잡이 `cmd` 로 `cargo build`·`nextest run` | 패키징 하나 — **인터프리터를 이름으로 부른다**(`bash ci/deploy.sh`) |

**패턴은 하나다: 정확성 게이트는 빌드 도구 안에 있고, POSIX 스크립트는 곁다리이며 인터프리터를
명시한다.** maru 가 다른 지점은 스크립트가 있다는 것이 아니라 **게이트 자신이 셸로 나간다**는 것이다.

## 넷을 세어 보니 둘로 갈렸다

`zig build test` 에 매달린 `sh` 단계는 넷이었다. 각각 **무엇을 재는가**로 갈린다:

| 스크립트 | 재는 것 | Zig 로 |
|---|---|---|
| `test-session-host-release-workflow.sh` (44줄) | `release.yml` **텍스트 대조만** | **된다 — 이번에 옮겼다** |
| `test-github-release-publication.sh` (101줄) | 다른 스크립트 **소스를 grep**(`gh …` 는 전부 grep 패턴이다) | 된다 — `test -x` 하나가 결정 자리 |
| `test-release-version.sh` (90줄) | `sh "$renderer" render …` 를 **12 번 실제로 돌린다** | **안 된다** — 셸을 몰아야 뜻이 산다 |
| `check-agent-hook-command.sh` (236줄) | *"실제 셸을 돌려야만 안다"* 가 그 파일의 존재 이유다 | **안 된다** — 옮기면 재는 것이 사라진다 |

> **「Zig 로 옮긴다」가 「셸을 없앤다」는 아니다.** 아래 둘은 셸의 **동작**을 재는 판정이라, 옮기면
> 판정이 남고 재는 것이 사라진다. 그 둘은 다른 답이 필요하다(§2m.109 의 ㉡ — 인터프리터를 빌드가
> 찾아 준다).

## 옮긴 것 — `tests/release_workflow/authority_capture.zig`

원본의 `test` 줄과 **일대일**이다. 판정 넷으로 갈라 어디가 깨졌는지 이름이 말하게 했다(원본은
`set -eu` 라 첫 실패에서 멈추고 무엇이었는지는 줄 번호로만 남았다).

**셸 의미를 그대로 옮기느라 걸린 자리 둘**(둘 다 처음에 틀렸고 테스트가 잡았다):

- **`grep -c` 는 일치 **횟수**가 아니라 일치한 **줄 수**다.** 그래서 `countMatchingLines` 다.
- **`$(...)` 는 끝 개행을 지운다.** 트리거 블록 비교에서 `permissions:` 앞의 빈 줄이 원본에서는
  안 보였는데 옮긴 쪽에서는 보였다 — 끝을 털어 맞췄다.

그리고 **내가 새로 넣은 결함 하나**: `uses:` 앞을 검사하는 루프에서 라벨 없는 `continue` 를 썼다.
그것은 안쪽 `for` 를 이을 뿐 바깥 줄 루프를 안 이어서, 걸러야 할 줄이 그대로 내려갔다. 라벨을 달았다.

**빈 판정을 막는 줄도 하나 넣었다** — Action 을 하나도 못 찾으면(형식이 바뀌어 파서가 헛돌면) 위
루프는 조용히 통과한다. `seen > 0` 을 건다.

## 적대적 검증이 잡은 것 — **옮긴 판정 둘이 원본보다 느슨했다**

처음 올린 포트는 *"원본의 `test` 줄과 일대일"* 이라고 적었는데, 하나씩 맞춰 보니 둘이 달랐다:

| 원본 | 처음 옮긴 것 | 새던 것 |
|---|---|---|
| `grep -c '\$canonical.*\n.*\$canonical.*\r'` **= 1** | 존재하면 통과 | 그 가드가 **둘로 늘어도** 초록 |
| `^[^@[:space:]]+@[0-9a-f]{40}$` | `lastIndexOf('@')` 로 잘랐다 | **`actions/checkout@evil@<40자리>` 가 통과** |

둘째가 특히 나쁘다 — **Action 핀 검사인데 `@` 를 하나 더 끼우면 빠져나간다.** 고친 뒤 그 입력이
`unpinned Action (more than one @)` 로 빨개진다.

**줄바꿈에도 안 흔들리게 했다.** `core.autocrlf=true` 로 받은 트리에서는 이 파일이 CRLF 인데,
`"on:
"` 을 바늘로 쓰면 블록을 **아예 못 찾는다**(실측 `TriggerBlockMissing`). 줄 머리를 먼저 찾고
자르며, 비교 전에 CR 을 턴다. LF·CRLF 양쪽에서 `EXIT=0` 을 확인했다(원본 스크립트도 같은 약점이었다).

## 뮤턴트 — 판정에 빈 곳이 없다

단언이 걸린 줄을 **하나씩 지워** 열둘을 쓸었고 **전부 `EXIT=1`** 이다. 값을 바꾸는 뮤턴트도 넷:

| 뮤턴트 | 결과 |
|---|---|
| 신뢰 획득 단계를 체크아웃 **뒤로** 옮긴다 | 판정 2 개 빨강 |
| `GITHUB_OUTPUT` 하나를 `GITHUB_ENV` 로 | 1 개 |
| checkout 을 `@v4` 태그로 못박는다 | 2 개 |
| 트리거에 `workflow_dispatch` 를 더한다 | 1 개 |
| `actions/checkout@evil@<40자리>` | 2 개 — **고치기 전에는 통과했다** |
| 개행 가드를 한 줄 더 복사한다 | 1 개 — **고치기 전에는 통과했다** |

> ### 쓸면서 세 번 헛다리를 짚었다 — 전부 «재는 쪽» 이었다
>
> 처음 쓸었을 때 셋(`realpath`·`stat`·`shasum`)이 *"안 잡힌다"* 로 나왔다. 셋 다 **판정 문제가
> 아니었다**: ⑴ 결과를 `grep "[0-9]+ failed"` 로 읽어 하드 실패의 다른 출력 형식을 놓쳤고,
> ⑵ 되돌릴 원본을 이미 오염된 파일에서 떠 왔고, ⑶ **Git Bash 가 `/usr/bin/realpath` 를
> `C:/Program Files/Git/usr/bin/realpath` 로 다시 써서**(MSYS 경로 변환) 뮤턴트가 아예 안 걸렸다.
>
> **뮤턴트는 「걸렸는지」부터 확인해야 한다.** 안 걸린 뮤턴트의 초록은 「판정이 없다」와 글자가 같다.
> 지금은 치환이 실제로 일어났는지 단언한 뒤에 잰다(`MSYS_NO_PATHCONV=1` 과 함께).

## 남은 셋

~~`zig build test` 에 아직 무방비로 매달린 `sh` 단계는 **셋**이다.~~ → **그 수가 틀렸다**(적대적 검증
3 회차). 세는 카운터가 `"sh"` 만 매칭해서 **`bash` 로 들어온 단계를 통째로 못 봤다**:

| | 실제 |
|---|---|
| 이 절의 포트 **전** | `sh` 4 + `bash` 1 = **5** |
| 포트 **후** | `sh` 3 + `bash` 1 = **4** |

옮길 수 있는 것은 `test-github-release-publication` 하나뿐이고, 나머지 셋
(`check-agent-hook-command`·`test-release-version`·`test-session-host-release-attestation-action`)은
셸 **동작**을 재는 판정이라 **다른 답**을 정해야 한다.

> **같은 자리에 여섯 번째로 걸렸다.** 게이트 결과든 개수든, **공간을 안 덮는 패턴으로 세면 없는 것이
> 없다고 나온다.** 종료 코드로 판정하고(§2m.109), 셀 때는 **셀 대상의 정의부터 넓게 잡는다.**

### 2m.111 게이트를 고치는 속도보다 **새로 깨지는 속도**가 문제다 (실측 2026-09-03)

§2m.110 을 머지하고 main 에서 게이트를 돌렸더니 **다시 `EXIT=1`** 이었다.

```text
test
+- run bash failure          ← tools/test-session-host-release-attestation-action.sh
```

## 원인 — 모르는 OS 면 그냥 닫는다

그 스크립트가 부르는 `pin-subject.sh` 가 이렇게 갈린다:

```sh
case $(/usr/bin/uname -s) in
    Darwin) … ;;
    Linux)  … ;;
    *) return 1 ;;          # 이 호스트는 MINGW64_NT-10.0-19045
esac
```

**계약이 깨진 것이 아니라 「이 호스트를 모른다」인데, 게이트에는 똑같이 빨강으로 보인다.** §2m.108 이
훅 스크립트에서 본 것과 같은 모양이고, 그때 세운 규율(*"안 됐다" 가 아니라 "못 쟀다" 로 적는다*)이
여기에는 아직 안 걸려 있다.

## 언제부터 — **22 커밋 동안 아무도 몰랐다**

| | |
|---|---|
| 들여온 커밋 | `85197d22` (2026-09-03) |
| 그 뒤 main 에 쌓인 커밋 | **22** |
| 그동안 Windows `zig build test` | **계속 빨강** |

내 커밋 직전(`804f47b3`)을 워크트리로 떠서 돌려 확인했다 — **내 변경이 아니다.**

## 세어 보니 흐름이 있다

`build.zig` 에 셸 단계가 들어온 커밋을 훑으면 지난 2 주에 **무방비로 넷**이 기본 `test` 그래프에
붙었다(08-20, 08-27 둘, 09-03). 나는 그 사이 **하나**를 뺐다.

> **속도 다툼이 요점이 아니다.** 요점은 **가장 최근 것이 들어온 그날 게이트를 깼고 22 커밋 동안
> 아무도 못 봤다**는 것이다. Windows 러너가 없으니 볼 사람이 없고, 그러면 **하나씩 옮기는 방식은
> 이미 들어온 것만 고친다.**

## 그래서 무엇이 답인가 — 내가 처음 말한 것은 답이 아니었다

처음에는 *"§2m.108 의 `posix_host_tests` 같은 구조적 게이트"* 라고 적었는데 **그것은 이것을 안
막는다**: 그 술어는 session_host **테스트 실행**에 걸린 것이고, 이번 단계는 `test_step` 에 **직접**
매달렸다.

**저장소가 이미 답의 모양을 갖고 있다** — `build.zig` 를 읽어 훑는 경계 판정이 셋 있다
(`tests/boundary/imports.zig` 두 자리, `wake_latency_budget.zig`). 그 자리에 이 규칙을 넣으면
**다음에 들어오는 것에도 자동으로 걸린다**:

> 셸 인터프리터를 부르는 `addSystemCommand` 가 **가드 없이** `test_step` 에 붙으면 빨강.

## 넣었다 — 그런데 「전부 가린다」는 답이 아니었다

규칙을 세우기 전에 넷을 **하나씩 돌려 봤다**(Windows/Git Bash):

| 스크립트 | |
|---|---|
| `check-agent-hook-command.sh` | `EXIT=0` — 돈다 |
| `test-release-version.sh` | `EXIT=0` — 돈다 |
| `test-github-release-publication.sh` | `EXIT=0` — 돈다 |
| `test-session-host-release-attestation-action.sh` | **`EXIT=1`** — 이것 하나다 |

**셋은 지금도 잘 돈다.** 「셸 단계를 전부 가린다」로 규칙을 세웠으면 **도는 계약 셋을 그냥 버릴 뻔했다.**

> 이 측정에서도 한 번 헛다리를 짚었다. 처음 돌린 루프가 넷 다 `EXIT=0` 이라고 했는데,
> `printf "…" "$(basename $s)" … "$?"` 의 **명령 치환이 `$?` 를 덮어썼다.** 상태는 즉시 변수에
> 담아야 한다 — 이 세션에서 「재는 쪽」에 걸린 일곱 번째다.

## 그래서 금지가 아니라 **원장**이다

`tests/boundary/shell_gate_ledger.zig` 가 `build.zig` 를 훑어 기본 `test` 그래프에 붙은 셸 단계를
걷고 **명단과 대조**한다. 새로 붙이면 원장을 함께 고쳐야 하고, 그 순간 이 질문을 읽게 된다 —
*"이 스크립트가 모든 개발 호스트에서 도는가?"*

- **돈다** → `.every_host`(근거는 그 호스트에서 한 번 돌려 본 것)
- **안 돈다** → `posix_host_tests` 로 가리고 `.posix_only`
- **못 재겠다** → 「안 됐다」가 아니라 **「못 쟀다」**로 적는 길이 스크립트 쪽에 있다(§2m.108 의 `SKIP`)

**이 게이트는 셸을 금지하지 않는다 — 결정을 강제한다.** 지금 원장은 **여섯**이고 그중 `.every_host` 가
셋이다. 실패하던 하나는 `.posix_only` 로 가렸다(Windows 에서 그 계약은 안 재진다 — 아는 채로 적었다).

| 뮤턴트 | |
|---|---|
| 새 셸 단계를 원장 없이 붙인다 | `EXIT=1` |
| `.posix_only` 인데 가드를 뺀다 | `EXIT=1` |
| `.every_host` 인데 가드를 씌운다 | `EXIT=1` |
| 원장에 있는데 `build.zig` 에서 사라진다 | `EXIT=1` |

**스캐너가 비지 않는지도 판정한다** — 픽스처로 `python3` 단계를 안 걷는 것과 `macos_only_test_step`
을 기본 그래프로 안 읽는 것을 고정했다(접미로 매칭하면 원장이 엉뚱하게 커진다).

## 첫날에 하나를 잡았다 — 예고편이 아니라 실물이다

이 원장을 넣는 PR 이 열려 있는 **그 사이에** main 에 여섯째 셸 단계가 들어왔다
(`892938b8`, 같은 날). 내 브랜치에는 없었고 **CI 가 머지 결과에서 잡았다**:

```text
build.zig: `tests/session-host-baseline-child-paths.sh` 가 기본 `test` 그래프에 새로 붙었다 — 원장에 없다.
```

원장이 시키는 절차를 그대로 밟았다 — **그 호스트에서 돌려 봤다**:

```text
mkdir: cannot change permissions of '/tmp/…/success': Permission denied
```

`mkdir -m` 으로 권한을 잡는데 **NTFS 에 그 개념이 없다.** 훅 스크립트의 `0600`(§2m.108)·
`pin-subject.sh` 의 `uname`(위)과 **같은 계열**이라 `.posix_only` 로 적고 가렸다.

> **이 사건이 §2m.110(하나씩 Zig 로 옮기기)의 한계를 그대로 보여 준다.** 내가 하나를 옮기는 동안
> 둘이 들어왔다. 옮기는 것은 **이미 들어온 것**을 고치고, 원장은 **들어오는 것**을 잡는다.

**이 게이트가 못 막는 것.** `.every_host` 가 참인지는 못 본다 — 사람이 한 번 돌려 보고 적는 값이고,
이 판정은 *"누가 그 질문에 답했는가"* 까지만 강제한다. `.mise.toml` 의 셸 의존도 못 본다(별도 자리).

### 2m.112 편집기 구문 색 — 규칙은 **중립이 갖는다** (실측 2026-09-03)
...
| 자리 | 무엇 | 왜 거기인가 |
|---|---|---|
| `chrome/components/editor_view/syntax_colors.zig` | 바이트 구간 → 줄별 `ColorSpan` | chrome 타입만 쓴다. 두 호스트가 **같은 것**을 부른다 |
| `src/syntax_colors.zig` | 캡처 이름 → `ColorRole` | 입력이 session, 출력이 chrome 인데 **chrome 은 session 을 import 하지 않는다**(`chrome/tokens.zig` 가 그 자리에 이유를 적어 뒀다). 그래서 최상위다 — `chrome_theme.zig`(config↔chrome)·`scm_items.zig`(session↔chrome)와 **같은 모양**이다 |
| `platform/{macos,windows}` | 재료를 그 낱말로 옮기는 일 | 호스트마다 다른 것은 이것뿐이다 |

**호스트마다 두지 않는 이유.** 위 문서의 *"옮기는 일은 platform 이 한다"* 는 **「chrome 안에서 하지
마라」**는 뜻이지 「호스트 수만큼 복사하라」가 아니다. 그렇게 두면 11 갈래 switch 가 두 벌이 되고
한쪽에 이름이 늘 때 **조용히 어긋난다**. 대신 `Role` 과 `ColorRole` 의 이름이 1:1 이라는 사실을
**컴파일 타임에 못 박았다** — `Role` 에 갈래를 하나 넣으면 `Role 과 ColorRole 의 이름이 갈렸다: …`
로 빌드가 죽는다(적대적 검증에서 확인).

**반사(`@field`)로 유도하지 않는다.** 그러면 그 파일이 경계 원장의 «외부 반사» 목록에 올라가고,
그 뒤로 고칠 때마다 digest 갱신 도구를 돌려야 하는데 그 도구가 `python3` 를 박아 두어 Windows 에서
안 돈다(§2m.109). 손으로 쓴 갈래 + 컴파일 타임 검사로 같은 보장을 얻는다.

#### 줄 경계는 **진짜 문서 오프셋**이어야 한다

`OpenFile.line_starts` 는 CR 을 벗긴 뒤 만든 **가상** 오프셋이다(선택 영역이 그 축을 쓴다). 그대로
tree-sitter 에 주면 CRLF 파일에서 축이 어긋나 색이 줄마다 밀린다 — 줄 슬라이스가 문서 버퍼 안을
가리키므로 **포인터 차이**로 진짜 오프셋을 얻는다. 만들기 전에 적대적 검증이 잡았고, 판정으로 물어
뒀다(`ab
cd
ef` 의 둘째 줄이 `[4,6)` 이지 `[3,5)` 가 아니다).

#### `line_colors` 는 **문서 줄**로 색인된다 — 그리고 그 비용이 있었다

중립 프레임이 `props.line_colors[props.first_line + n]` 으로 읽는다. 즉 창 앞의 줄에도 **자리가
있어야** 하고, 호스트는 `leading_empty = first_line` 을 준다.

처음에는 그 앞자리를 **프레임마다 다시 채웠다**. 재 보니 `first_line = 200_000` 에서 60 프레임에
**336,657 µs — 프레임당 5.6 ms** 였다(Debug). 색 하나 없는 칸을 3.2 MB 씩 다시 쓰는 값이다.
창 앞 구간은 **이미 무색**이므로 어디까지 무색인지 기억하고 늘어난 만큼만 채운다 → **2,104 µs**.

> 위험한 자리는 창이 **아래로** 갈 때다. 지난 프레임이 색을 쓴 칸이 이번 프레임에는 창 앞이 되고,
> 그 칸을 안 덮으면 옛 색이(그것도 매달린 슬라이스로) 남는다. 판정으로 물어 뒀다 —
> 덮기를 없앤 뮤턴트가 빨개진다.

#### 스모크가 못 재는 축이 있다

스모크에는 **편집기가 스크롤된 채로 그려지는 프레임이 없다.** 그래서 `leading_empty` 를 0 으로
바꿔도 판정 72 개가 **전부 초록**이었다(적대적 검증 7 회차). 화면 판정이 못 가는 자리라 단위 판정으로
물었다 — 창을 3 줄 내린 상태에서 색이 `[3]` 에 붙는지 본다.

**스모크 판정도 두 번 거짓 초록이었다.** ⑴ *"기본 글자색이 아니다"* 로 셌더니 **거터 회색**이 걸려
구문 색이 하나도 없는데 `591/594` 였고, ⑵ 역할별로 더했더니 한 칸이 여러 번 세어져 `629/525`(전체보다
크다)가 나왔다. 지금은 칸마다 **테마의 `syntax_*` 색 열하나와 대조**해 한 번씩만 센다.

#### `check-targets` 에 `syntax` 가 안 물려 있었다

`main.zig` 가 `@import("syntax")` 를 하는데 크로스 타깃 exe 모듈에는 그 import 가 없어
`no module named 'syntax'` 로 게이트가 빨개졌다 — **본 빌드만 초록이라 한동안 안 보였다.**
C 원본은 안 싣는다: 이 게이트는 `-fno-emit-bin` 이라 링크까지 안 가고, grammar 열여덟 벌을 타깃마다
컴파일하면 얻는 것 없이 게이트만 길어진다. `@cImport` 가 볼 헤더와 쿼리 임베드만 물린다.

### 2m.113 화면 증거를 만들 수 있게 한다 — `--hold-editor-ms` (실측 2026-09-05)

§2m.112 는 **화면 증거 없이** 머지됐다. 편집기가 색으로 그려지는지 눈으로 못 봤다는 뜻이다.

**"스모크가 편집기를 안 그린다" 가 아니었다.** 재 보니 편집기는 **334 프레임** present 된다. 안 된 것은
**캡처가 그 위에 앉는 것**이다. 셋이 겹쳤다:

| 무엇 | 실측 |
|---|---|
| 스모크가 얼마나 긴가 | **88 초** — 나는 1~6 초에 여섯 번 찍고 "안 그린다" 고 결론지었다 |
| 편집기 국면이 언제 오는가 | **약 70 초** 지점 |
| 한 프레임이 얼마나 머무는가 | 16 ms — 하네스가 겨냥할 수 없는 폭이다 |

그래서 `win32-terminal-smoke` 에 **`--hold-editor-ms <n>`** 을 뒀다. **기본은 0 이라 게이트의 시간은 안
변한다** — 증거를 만들 때만 켠다.

```console
$ maru win32-terminal-smoke --hold-editor-ms 20000 > hold.log &
$ # 표식이 나올 때까지 기다렸다가 찍는다 — 시간을 짐작하지 않는다
$ until grep -q "editor_hold:" hold.log; do sleep 1; done
$ powershell -File tools/windows/capture-window.ps1 -OutPath editor.png
CAPTURE-OK ... distinct_colors=155        # 터미널 판일 때는 51 이었다
$ grep editor_hold hold.log
editor_hold: ms=20000 colored=473/525 roles=7
editor_hold_measured_ms=20012
```

**표식을 자기 전에 찍고, 잰 값을 깨어나서 찍는다.** 앞의 것은 하네스의 방아쇠이고 뒤의 것은 *"정말
멈췄는가"* 다 — 처음에 그것을 안 찍었더니 «스모크 전체 시간이 88 초 대 84 초» 라는 관측이 나와
**멈추지 않는다고 오해**했다(실제로는 멈춘다. 자는 동안 시간에 매인 다른 국면이 함께 정리된다).

**`esyn_judgeable` 만 보고 멈추면 안 된다.** 그 값은 **색칠 수와 무관하게** 선다 — 색이 하나도 없는
프레임에서 멈춰 **거짓 증거**를 만든다. 그래서 `editor_syntax` 가 초록이라고 말하는 그 문턱을 그대로
쓴다(색칠된 칸이 있고 역할이 둘 이상). 적대적 검증 2 회차가 잡았고, 색칠을 0 으로 만든 뮤턴트에서
hold 가 **한 번도 안 걸린다**.

**선택지 해석은 순수 함수로 뺐다**(`terminalSmokeOpts`). 디스패치 안에서는 인자 반복자와 stderr 가
얽혀 판정을 걸 수 없어서, 이 계약은 한동안 **손으로 세 번 돌려 본 것**이 전부였다. 그리고 그 자리에
진짜 결함이 있었다 — 안내문을 `stderr` 에 쓰고 **비우지 않은 채** 오류를 돌려주어, 사용자에게는
`error: InvalidArgs` 와 스택 트레이스만 갔다.

**이것이 못 하는 것.** 스크롤하지 않는다 — §2m.112 가 남긴 «창 앞의 빈 줄» 축은 여전히 화면 판정이
못 잰다(단위 판정이 물고 있다).

### 2m.114 굴린 편집기의 색을 잰다 — 판정이 보고도 안 잡던 자리 (실측 2026-09-06)

§2m.112 가 구멍을 하나 적어 두고 닫았다: `leading_empty` 를 0 으로 바꾼 뮤턴트가 **스모크 판정 72 개를
전부 통과**했다. 단위 판정으로 물어 뒀지만 화면 쪽은 비어 있었다.

**"스모크가 편집기를 안 굴린다" 가 아니었다.** 굴린다 — 102 줄까지. 재 보니 이랬다:

```text
PROBE first_line=102 syntax=false path=D:/ohah/maru/.github/workflows/cache-cleanup.yml
PROBE max_first_line=102 max_first_line_with_syntax=0
```

**휠이 가는 파일에 파서가 없었다.** `cache-cleanup.yml` 이고 YAML 은 번들 grammar 열여덟에 없다
(정상이다). 그래서 «굴린 프레임»과 «색 있는 프레임»이 **한 번도 안 겹쳤다.** 색 있는 파일
(`build.zig`)은 스핀 772~1165 동안 화면에 있는데 그동안 한 번도 안 굴렀다.

이제 색 있는 파일을 직접 굴리고 그 프레임을 잰다(`editor_syntax_scrolled`).

**굴린 뒤에는 되돌린다.** 이 뒤로도 300 스핀 넘게 판정이 이어지는데 굴려 둔 채 넘기면 그것들이 보는
화면이 달라진다 — 판정을 세우면서 판정을 흔드는 꼴이다. 굴리는 자리도 기존 스크롤 판정(790·792)과
가로 스크롤 판정이 **끝난 뒤**(스핀 810)로 잡았다.

#### 문턱은 «색칠된 칸이 있다» 가 아니라 «색이 화면에 퍼져 있다»

처음엔 첫 화면 판정과 같은 문턱(`colored > 0`)을 썼다. **뮤턴트가 그것을 통과했다:**

| | 성한 것 | `leading_empty = 0` |
|---|---|---|
| 색칠된 칸 | 370/431 | **21**/431 |
| 색이 실린 **행** | **28/31** | **1/31** |
| 그 문턱의 판정 | 초록 | **초록** ← 보고도 안 잡았다 |

축이 틀어지면 색이 **사라지는 것이 아니라 한 줄에 몰린다.** 칸 수만 보면 21 도 «0 보다 크다» 다.
그래서 **행 퍼짐**을 문턱으로 삼는다 — 절반은 위 두 실측(28/31 대 1/31) 사이에서 고른 값이고
양쪽에 큰 여유가 있다. 첫 화면 판정에도 같은 잣대를 걸었다(실측 26/30).

> **이것이 "판정이 없다" 와 다른 실패다.** 판정은 있었고, 값도 정확히 움직였다(370→21). 다만
> **묻는 문장이 그 움직임을 통과시켰다.** 그래서 뮤턴트는 판정이 *움직이는지*가 아니라
> **빨개지는지**로 읽어야 한다.

**세는 일은 한 함수가 갖는다**(`countSyntaxPaint`). 판정이 둘이 되는 순간 셈법을 베끼면 한쪽만
고쳐진다 — 이 셈법은 이미 두 번 틀렸던 자리다(§2m.112 의 `591/594`·`629/525`). 단위 판정도 함께 걸어
«구문 색이 아닌 칸은 안 센다»·«같은 역할 두 칸은 갈래 하나다» 를 못 박았다.

#### 판정 하나가 **다른 호스트의 컴파일을 죽였다**

`countSyntaxPaint` 의 서명에 `d3d11_cells.Cell` 을 박았다. **그 타입은 Windows 밖에 없다**(다른
호스트에서는 스텁이다). 로컬 `mise run check` 는 초록이었고 CI 의 Linux 컴파일이 죽었다:

```text
src/main.zig:5323: error: struct 'maru.d3d11_cells__struct_41926' has no member named 'Cell'
```

**`SkipZigTest` 로는 못 막는다** — 런타임 건너뛰기는 컴파일을 안 막는다(이 저장소가 이미 다른 자리에
그 함정을 적어 뒀다). 그래서 셀 타입을 `anytype` 으로 받는다. 필요한 것은 `fg` 와 `rect` 둘뿐이고,
그러면 이 판정이 **모든 호스트에서 돈다** — 건너뛰는 것보다 낫다.

**그리고 로컬 게이트가 이것을 볼 방법이 없었다.** `check-targets` 는 `main.zig` 를 **실행 파일로만**
크로스 컴파일한다 — `test` 블록은 안 실린다(§2m.2 가 «테스트 빌드는 test 가 참조하는 것만 분석한다»
를 적어 둔 그 짝이다: 이번엔 반대로 **실행 파일이 test 를 안 본다**). 이제 크로스 타깃마다
`addProjectTest` 도 함께 건다. 되돌린 뮤턴트에서 `check-targets` 가 **EXIT=1** 이다.

#### 재 보니 근거가 없던 것 둘 — 그래도 남긴다

굴린 뒤 **되돌리기**와 굴리는 자리의 **스핀 바닥(810)**. 주석에는 *"안 되돌리면 뒤따르는 300 스핀의
판정이 보는 화면이 달라진다"* 고 **단언**해 뒀는데, 지워 보니 판정 75 개가 **전부 그대로**였다.
바닥을 0 으로 내려도 같았다.

남긴 이유는 **지금의 배치가 우연이기 때문**이다 — 기존 스크롤 판정(790)이 보는 파일이 마침
`cache-cleanup.yml` 이라 내가 `build.zig` 를 굴려도 안 겹친다. 파일 순서가 바뀌면 그 우연이 사라진다.
**「지금 안 걸린다」와 「걸릴 수 없다」는 다르다** — 다만 주석이 앞엣것을 뒤엣것처럼 말하면 안 된다.

#### 재 봤지만 아니었던 것 — 줄 번호는 안 세어진다

*"0.5 회색이 구문 색과 겹치니 473/525 에 줄 번호가 섞였을 것"* 이라고 의심했다. **아니다.**
줄 번호는 `muted_fg = 184,184,184` 이고 가장 가까운 구문 색은 `syntax_punctuation`(178,178,178),
거리 18/255 로 문턱(0.01) **밖**이다. 겹친 것은 0.5 회색과 `syntax_comment`(128,128,128)였고
그건 정상이다 — 주석은 원래 회색이다.

**다만 그 실측이 다른 것을 보여 줬다.** 기본 팔레트에서 `keyword`=`attribute`=255,0,255,
`property`=`function`, `tag`=`invalid` 다 — **11 역할이 8 색**이다. 그래서 판정이 찍는 `roles=` 는
역할 수가 아니라 **색 갈래 수**이고, 역할 수의 **하한**이다(색이 겹치면 두 역할이 하나로 보인다).
`>= 2` 문턱에는 안전한 방향이지만 이름이 값보다 크게 말한다.

> **단위 판정의 테마도 함정이었다.** 처음엔 `Tokens.rich(zeroes(ThemeColors))` 를 썼는데 그러면
> 열한 색이 **전부 같아서** «같은 역할 두 칸은 갈래 하나» 가 **동어반복**이 된다 — 역할별로 세는
> 코드도 그대로 통과한다. 스모크가 쓰는 진짜 토큰으로 바꿨다.

#### 행 상한이 **조용하면 거짓 초록**이 된다

행은 `[512]f32` 두 벌로 센다(색칠된 행·전체 행). **따로 찬다** — 색 없는 행이 상한을 먼저 채우면
그 뒤의 색칠된 행은 **분자에만** 들어가고 분모에는 안 들어간다. 참값이 300/812 인데 300/512 로
통과하는 모양이 만들어진다. 지금 편집기 프레임은 서른 행 남짓이라 안 닿지만, **상한에 닿은 것과
다 센 것이 똑같이 초록으로 보이면 안 된다** — §2m.2 가 깊이 상한에서 배운 그것이다.

그래서 상한에 닿으면 `capped` 를 세우고 판정은 **«못 쟀다» 로 접는다**. 단위 판정이 그 모양을
그대로 만들어 «비율로는 통과하는데 `capped` 가 서 있다» 를 못 박는다.

> **그 자리에 죽은 안전장치가 하나 있었다.** 색칠된 행 쪽에도 상한 보고를 달았는데, 색칠된 행은
> 전체 행의 **부분집합**이라 먼저 찰 수가 없다 — 지운 뮤턴트가 **초록이었다**(적대적 검증 11 회차).
> 도달할 수 없는 안전장치는 «막아 준다» 는 착각만 남기므로 지우고, 그 관계를 `assert` 로 적었다.

#### 가로로 굴러간 프레임을 재면 **거짓 빨강**이 된다

스모크가 그리는 편집기 프레임 전부에서 최저 비율을 재 봤더니 **0/30 인 프레임**이 있었다.
결함이 아니었다 — 스핀 980 은 **가로 막대를 끄는** 판정이고, 본문이 화면 오른쪽 밖으로 나가면
줄 번호만 남는다(편집기 셀 **462 → 52**). 그 화면에 색이 없는 것은 그 화면의 성질이다.

문제는 **거리**였다. 색 판정이 무장하는 자리는 스핀 977 이고 그 스핀은 980 — **셋 차이**다.
파일이 조금만 늦게 열리면 이 판정은 가로로 굴러간 화면을 재고 빨개진다. 그래서 `first_col == 0`
일 때만 잰다. 무장 자리를 그 구간으로 강제해 견줘 보면 차이가 그대로 나온다:

| | 결과 |
|---|---|
| `first_col == 0` 가드 있음 | `rows=27/30 ok=true` — 본문이 보이는 프레임까지 기다린다 |
| 가드 없음 | `rows=0/30` **`ok=false`** — 거짓 빨강 |

**판정은 세 번 돌려 값이 같다**(`colored=370/431 rows=28/31`) — 흔들리지 않는다.

#### `--hold-editor-ms` 는 이제 **두 번** 잡는다

`kind=first`(첫 화면)와 `kind=scrolled`(굴린 뒤). 둘은 서로 다른 것을 보여 준다 — 앞은 «색이 나는가»,
뒤는 «굴려도 색이 제자리인가» 다. 하네스는 기다릴 표식을 `kind=` 로 고른다.

```console
$ until grep -q "kind=scrolled" hold.log; do sleep 1; done   # 약 100 초 지점
$ powershell -File tools/windows/capture-window.ps1 -OutPath scrolled.png
CAPTURE-OK ... distinct_colors=149
```
