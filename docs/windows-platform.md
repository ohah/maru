...
| **W8.3** | **에디터 표면** — ⒜ 본문·gutter(**완료**, §2m.21) ⒝ 배경·스크롤바(**완료**, §2m.22 — 파이프라인이 아니라 셰이더였다) ⒞1 스크롤(**완료**, §2m.23) ⒞2 클릭 → 문서 offset — macOS `hitTestBody` 의 중립 알맹이를 `editor_view` 로 빼는 것이 선행이다(다음) | W8.2 |
