...
`zig build test` 에 아직 무방비로 매달린 `sh` 단계는 **셋**이다(`check-agent-hook-command`·
`test-release-version`·`test-github-release-publication`). 그중 옮길 수 있는 것은 하나뿐이고,
나머지 둘은 셸 동작을 재는 판정이라 **다른 답**을 정해야 한다.
