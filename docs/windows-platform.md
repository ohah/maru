...
미루어 뒀던 이유가 "매-mouseMove 비용"이었으므로 그 비용부터 쟀다(Windows 10, `GetFileAttributesW`, D: 로컬 SSD):

| 대상 | 1회 | 비고 |
|---|---|---|
| 로컬 실재 파일·디렉터리 | **7.9 µs** | |
| 로컬 없는 파일 | **11.4 µs** | 오탐이 걸리는 쪽이 더 비싸지만 같은 자릿수다 |
| 없는 드라이브(`Z:\…`) | **0.8 µs** | 드라이브 테이블에서 즉시 거절 |
| 링크 후보 50개(뷰포트 1화면분) | **0.61 ms** | 60fps 예산 16.7 ms의 **3.6 %** |

느려지는 경우도 찾았다 — UNC 죽은 호스트 **755 ms**, 라우팅 불가 IP **11 s**. 그런데 `isDetectableAbsoluteFor`가
`\\`로 시작하는 토큰을 감지에서 이미 떨어뜨리므로 stat까지 **도달하지 않는다**. 원격 세션은 `linkScopesForTerm`이
네 경로 scope를 전부 끄므로 host 왕복(`collectViewportLinks`)에도 새 비용이 붙지 않는다. **남은 미지수**: 매핑돼
있으나 연결이 끊긴 네트워크 드라이브(`Y:` → 죽은 서버)는 재지 못했다. 그 경로는 로컬 드라이브 문자로 보이므로
감지를 통과한다.

불변식은 테스트가 지킨다 — `src/terminal/core.zig`의 *"hover와 클릭이 같은 답을 낸다 — 존재검증까지"* 가 실재
경로와 없는 경로를 두 OS 형태로 넣고 hover 결과와 클릭 결과가 어긋나지 않는지 단언한다.
