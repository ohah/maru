...
- **이어가기(재부팅이 증명됐을 때만)**:
  1. 대화 파일을 찾는다 — claude는 `<claude 설정 디렉터리>/projects/<cwd slug>/<id>.jsonl`, codex는
     ...
     「못 이어간 수」에 센다.
  2. 파일 **끝부분 최대 1 MiB**만 읽어 마지막 권한 모드와 모델을 얻는다. 끝부분 읽기와 「중간에서 잘라 읽었으면 첫
  ...
  3. 도크 재개와 **같은 셸 래핑**으로 띄운다 — `<shell> -l -i -c "<provider argv>; exec <shell> -l -i"`, cwd는 spawn
     ...
     셸로 돌아온다.
  4. **저장 cwd 디렉터리가 사라졌으면 claude는 이어가지 않는다.** claude는 대화를 작업 디렉터리별로 찾으므로 기본
     자리에서 띄우면 「대화 없음」으로 끝난다. 판정은 디렉터리를 실제로 열어 본다 — `usableRestoreCwd`는 형식만 보는
     이른 필터라 없는 디렉터리도 통과시킨다. codex는 세션 id로 찾으므로 이 조건이 없다.
