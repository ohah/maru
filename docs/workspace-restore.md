...
**문제**: SSH 세션에서 만든 문서를 저쪽에 저장하면(U3) 그 문서의 신원은 **호스트와 원격 경로**다 — 로컬 경로도 dock entry 도 없어 위 두 절의 어느 record 에도 들지 않는다. 그래서 재시작 뒤 그 탭이 사라지고, 미저장 편집의 백업 레코드는 **아무도 소비하지 않는다**.

**결정(2026-09-22 사용자 결정)**: 그 문서는 **이름 없는 dirty 문서로** 되살리고 어디서 왔는지 알린다 — 재시작 시점에는 control socket 이 없어 저쪽 신원을 다시 세울 수 없고, 조용히 로컬 파일로 만들면 **다른 곳에 쓰는 것**이 된다([문서 모델](native-editor-document-model.md) §3.10·§3.11).

### 포맷 — 위 두 절과 같은 규칙

`pane` 줄의 반복 필드 `remote-doc-term="<insert-after>:<dest-byte-len>:<dest>:<path-byte-len>:<path>"`. `insert_after` 의 뜻과 downgrade 근거는 브라우저 절 그대로이고, 값의 자기-구획(길이를 앞에) 규칙은 `file-term` 그대로다.

- **신원을 싣는다**(호스트·원격 경로) — 백업 파일 이름은 그 신원에서 **파생**되므로 여기 적지 않는다(이름 없는 문서 절과 같은 이유).
- **되살린 문서는 «새» 이름 없는 문서다**: 번호를 발급기에서 새로 받는다. 그 문서는 「지난 실행에서 이름이 없던 것」이 아니라 **신원을 잃은 것**이라, 옛 번호가 없다.
- **옛 레코드는 소비하고 지운다** — 안 지우면 매 실행마다 같은 문서가 또 되살아난다.
- **구버전은 이 필드를 모르고 건너뛴다** → 그 창은 그 탭만 잃는다(지금과 같다).
...
하고 종료는 그대로 진행한다 — `captureWorkspaceSnapshot`에서 `nil`을 돌려주면 체크포인트 상태 기계가 그것을
「캡처 실패」로 읽어 `CANCEL_QUIT`을 내고, `allowsFailure=false`인 keep-alive 종료가 **취소돼 앱이 안 닫힌다**(실측).
「안 쓰기로 한 것」과 「쓰지 못한 것」은 다른 사건이므로 다른 자리에서 말해야 한다.

**이것은 v144 → 「백업 후 저장」 → 다시 「차단」으로 돌아온 것이고, 되돌린 이유는 설계가 틀려서가 아니라 그 설계가
실제로 동작한 적이 없어서다.** 「백업 후 저장」은 `ensureBackup`이 덮어쓰기 직전에 마지막 완전본을 `workspace.v1.bak`으로
남긴다는 전제 위에 섰다. 그런데 `ensureBackup`은 `O_EXCL`이라 **`.bak`이 이미 있으면 아무것도 하지 않고 성공을 반환한다.**
그 `O_EXCL` 자체는 의도였다 — 연속 불완전 실행이 가장 완전한 첫 사본을 밀어내지 않게. **빠진 것은 복원이 «성공»했을 때
`.bak`을 풀어 주는 경로다.** 불변식을 다시 무장하는 단계가 없으니 한 번 생긴 `.bak`이 영구히 눌러앉는다.

2026-09-13 실측: `.bak`은 **7월 25일 4614 B**에 멈춰 있었다(7주). 그 상태에서 복원이 실패한 종료 저장이 원본을
**7421 B(창 2·탭 12) → 341 B(탭 1)** 로 덮었고, 되돌릴 사본이 없었다. **하루에 다섯 번.**

**차단의 알려진 대가 — v144가 되돌려진 이유는 아직 유효하다.** 저장을 막으면 stale 파일이 그대로 남아 다음 실행이 같은
drop을 재생산하고(자기영속 루프), 그 사이 사용자가 만든 창·탭·split·pane rename·창 위치는 매 종료마다 사라진다. 무기한
차단은 데이터 손실 방지가 아니라 데이터 손실 그 자체가 될 수 있다(code-review max). 지금 차단을 택한 것은 **그 대가가
"복원이 계속 실패하는 동안"에만 발생하는 반면, 백업 없는 덮어쓰기는 «첫 번째 실패에서 즉시» 되돌릴 수 없는 손실**이기
때문이다. 비용이 비대칭이다.

**별도 개선 계약.** 아래 경로를 채택했다. 단계·진행 상태는
[Workspace restore 구현 계획](plans/workspace-restore.md#r7-5-checkpoint-backup-re-arm), 현재 검증 상태는
...
1. **복원이 완전히 성공한 실행에서 `.bak`을 해제한다.** 그 시점의 `workspace.v1`은 신뢰할 수 있으므로 「마지막 완전본」
   ...
   보존되면서 `.bak`이 썩지 않는다.
2. 정상 background/final publication은 해제 뒤 첫 저장에서 현재 완전본을 create-once `.bak`으로 다시 잡는다.
   불완전 복원 실행의 publication 차단은 유지해 부분 모델을 canonical 파일로 승격하지 않는다.
3. 불완전 실행 중 새로 만든 layout까지 별도 복구하려면 그 모델을 `workspace.v1`이 아닌 별도 파일에 적는
   후속 정책이 필요하다. R7-5는 기존 완전본 보존을 우선하며 그 별도 모델을 도입하지 않는다.
...
- **왜 부팅 시각이 아니라 UUID인가.** `kern.boottime`은 벽시계 값이라 시계 조정에 흔들린다. UUID는 같음·다름만
  말하고 그 밖의 해석이 없다.
- **같은 부팅에서 두 번 부활하지 않는다.** 부활한 실행은 캐시 디렉터리에 「이번 부팅에 부활함」 표식
  (`~/.cache/maru/reboot-revived`, 내용은 부팅 UUID)을 **곧바로** 쓰고, 다음 실행은 표식이 지금 부팅과 같으면 증명으로
  치지 않는다. 부활 뒤 checkpoint가 새 `boot-session`을 실으면 그것으로 충분하지만, 복원이 불완전하면(explorer root
  하나가 사라짐 등) **모든 저장이 막혀** 파일이 옛 부팅 값을 그대로 든다 — 표식이 없으면 같은 부팅의 다음 실행이 또
  부활해 같은 셸과 같은 `claude --resume`을 겹쳐 띄운다. 판정은 **프로세스당 한 번** 내린다(창마다 내리면 첫 창이 쓴
  표식을 둘째 창이 읽고 증명을 거둔다).
...
- **host identity가 없는 터미널 Term**(in-process로 만든 것): 원래 새 셸로 뜬다. `agent-resume`이 적혀 있으면 위와
  **같은 부활 경로**로 보내 대화를 이어가고 부활 수에도 센다. 그래서 이 절은 keep-alive 설정과 무관하게 **한
  규칙**이다 — keep-alive를 끈 사용자도 재부팅 뒤 에이전트가 이어진다.
- **원격 cwd**는 원래 저장하지 않으므로(빈 값) 기본 자리에서 뜬다. 파일·브라우저·이름 없는 문서 Term은 각자의
  기존 규칙 그대로다.
- **부활 칸의 spawn이 실패하면 그 칸만 묘비로 강등한다**(full `runtime-handle`이 있을 때 — `⏎`로 다시 시작). 부활은
  모든 칸을 새로 띄우므로 host의 runtime 상한·fd 고갈 하나가 창 전체를 잃게 해서는 안 된다(재부팅 전이었다면 같은
  파일이 묘비로 복원됐다). legacy bare handle·in-process 칸은 묘비로 표현할 수 없어 오류를 그대로 올린다.
- **복원이 다 끝난 뒤(checkpoint 무장 뒤) 앱에 알림을 한 번 띄운다**(창마다가 아니다 — 추가 창 apply 사이에도 tick이
  돈다). host 연결 실패 알림이 있으면 이 알림은 생략한다(알림 자리가 하나라 「유지 안 됨」과 그 이유가 이긴다) — 「재부팅으로 이전 터미널이 끝나 N개를 새로 시작했습니다」, 이어간
...
- **저장**: `surface` 줄의 scalar `agent-resume="<provider>:<session-id>"`. provider는 `claude`·`codex`. 다음을
  **모두** 만족할 때만 싣고, 아니면 키를 생략한다.
  ...
  - Term이 원격이 아니다(ssh 채널이 열린 Term, 원격 cwd, **원격 훅이 종류를 세운** Term은 에이전트가 **저쪽 기계**에서
    돈다 — 여기서 이어가면 엉뚱한 기계에서 연다). 원격 훅의 종류 래치는 ssh를 빠져나오면 푼다 — 안 풀면 그 칸의 로컬
    에이전트가 영영 저장되지 않는다.
  - 신원이 세션 기록 도크의 argv 토큰 규칙(`isResumableSessionId`)을 통과한다. 이 값은 실행 인자가 되므로 쓰는
    쪽과 읽는 쪽이 **같은 규칙**을 쓴다.
  - 묘비 Term은 싣지 않는다(그 안에서 도는 것이 없다).
- **신원의 수명**: 에이전트가 끝나거나 다른 종류로 바뀌면 그 Term의 신원을 버린다 — 신원 캐시는 종류가 바뀌어도
  남도록 설계됐는데, 그러면 같은 칸에서 새로 띄운 세션이 **옛 세션의 id**로 저장돼 재부팅 뒤 엉뚱한 대화를 이어간다.
  없음 → 에이전트는 버리지 않는다(훅의 `SessionStart`가 프로세스 판정보다 먼저 올 수 있다). 거둬들인 상태줄 훅이 남긴
  신원 파일은 **이 실행이 시작한 뒤에 쓰인 것만** 믿는다(파일 이름이 실행마다 1부터 다시 세는 surface id다).
- **보류 신원**: 복원은 surface의 `agent-resume`을 Term에 보류 신원으로 옮기고, 캡처는 **살아 있는 신원이 없고 종류가
  같을 때만** 그것을 싣는다. 이것이 없으면 재접속(같은 부팅의 재실행)이나 부활 직후 훅이 신원을 다시 알려 주기 전에
  찍힌 checkpoint가 파일이 알던 값을 지운다. 에이전트가 끝나거나 종류가 바뀌면 함께 버린다.
...
- **이어가기(재부팅이 증명됐을 때만)**:
  1. 대화 파일을 **세션 id로** 찾는다 — claude는 `<claude 설정 디렉터리>/projects/*/<id>.jsonl`, codex는
     `$CODEX_HOME/sessions`(없으면 `~/.codex/sessions`) 아래 `rollout-*-<id>.jsonl`(id 앞이 구분자일 때만). claude의
     폴더 이름(slug)을 cwd로 다시 만들지 않는다 — claude는 `realpath(cwd)`의 **영숫자 아닌 모든 문자**를 `-`로 바꾸고
     200자를 넘으면 해시를 붙여(2026-09-24 설치본 2.1.280 코드에서 확인) 그 규칙을 흉내 내면 밑줄·공백·한글·심볼릭
     링크·긴 경로에서 있는 파일을 못 찾는다. **파일이 없으면 이어가지 않는다** — provider가 거절할 세션을 열어 오류 화면을 남기지 않고, 셸만 띄운 뒤 알림의
     「못 이어간 수」에 센다.
  2. 파일 **끝부분**만 읽어 마지막 권한 모드와 모델을 얻는다 — 1 MiB부터, 모드를 못 찾으면 4 MiB·16 MiB로 넓힌다
     (마지막 턴의 큰 도구 출력이 모드 줄을 밀어내는 일이 실제로 있다 — 실측 codex 끝 300개 중 6개가 1.2~2.9 MiB. 그때
     플래그 없이 열면 provider 기본값이 기록보다 넓은 권한일 수 있다). 값이 있을 수 있는 줄(claude `permissionMode`·
     `model`, codex `turn_context`)만 JSON으로 연다. 끝부분 읽기와 「중간에서 잘라 읽었으면 첫
     줄을 버린다」는 사이드바 대화 줄과 **같은 `agent_transcript.readTail`**이 소유한다. 세션 기록 도크의 재개와
     **같은 파서·같은 argv 조립(`resumeArgvFor`)**을 쓴다 — 「기록된 대로 되살린다」는 규칙이 두 자리에서 갈리지 않게([에이전트 세션
  ...
  3. 도크 재개와 **같은 셸 래핑**으로 띄운다 — `<shell> -l -i -c "<provider argv>; exec <shell> -l -i"`, cwd는 spawn
     작업 디렉터리로만 넘긴다. 조립은 도크와 부활이 공유하는 `AgentResumeLaunch` 하나다. 에이전트를 끝내면 그 Term은
     셸로 돌아온다. **그 래핑을 받는 셸(zsh·bash·sh·fish·dash·ksh)에서만** 이어간다 — tcsh·csh는 `-l`을 단독 인자로만
     받아 곧바로 죽고 칸이 닫힌다. 그 밖의 셸이면 셸만 띄우고 「못 이어간 수」에 센다(도크 재개는 실패 알림).
  4. **작업 디렉터리는 대화가 기록한 곳이 먼저다**(claude는 줄마다 cwd를 싣는다) — 에이전트가 실제로 쓴 디렉터리이고,
     셸이 마지막으로 알린 cwd는 그와 다를 수 있다(`cd x && claude`, OSC 7을 안 내는 셸). 없으면 저장 cwd, 둘 다
     chdir할 수 없으면 기본 자리다. **cwd 때문에 이어가기를 포기하지는 않는다** — 대화는 id로 찾았다. 판정은
     `access(X_OK)`(검색 권한)다 — 디렉터리를 열어 보면 읽기 권한이 없는 디렉터리를 「사라졌다」로 읽는다.
- **알려진 한계**: 훅이 꺼져 있고 자식 env로도 신원을 못 잡은 세션은 이어가지 않는다(셸만). `CLAUDE_CONFIG_DIR`는
  **앱 프로세스의** 환경에서 읽는다 — `.zshrc`에서만 설정하고 Dock으로 띄우면 기본 위치를 찾는다. 모드 줄이 끝 16 MiB
  밖이면 플래그 없이 연다. 도크 재개의 셸 래핑은 사용자의 `shell.args`를 싣지 않는다.

상태 표시는 [에이전트 상태 감지](agent-session.md)가 단일 출처다. host/runtime이 끝난 뒤 provider 경로로 대화를
이어가는 것은 위 재부팅 증명 복원 하나뿐이다.
