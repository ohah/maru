 ...
 */
import { Terminal, themes as PRESETS } from "/core/dist/index.js";
...
const COMMANDS = {
  ...
  links: () => "\x1b]8;;https://ziglang.org\x07Zig\x1b]8;;\x07 · \x1b]8;;https://github.com/ohah/maru\x07maru\x1b]8;;\x07",
  theme: () => `현재 테마: ${$("o-theme").value} (${Object.keys(allThemes).length}종)`,
  clear: () => "\x1b[2J\x1b[H",
};
...
async function onBytes(bytes) {
  ...
  if (s === "\x1b[D" || s === "\x1bOD") { if (pos > 0) { pos--; await redraw(); } return; }
  if (s === "\x1b[C" || s === "\x1bOC") { if (pos < gs().length) { pos++; await redraw(); } return; }
  if (s === "\x1b[H" || s === "\x1bOH" || s === "\x01") { pos = 0; await redraw(); return; }
  if (s === "\x1b[F" || s === "\x1bOF" || s === "\x05") { pos = gs().length; await redraw(); return; }
  ...
  if (s === "\x1b[A" || s === "\x1bOA") {
    if (histIdx + 1 < history.length) { histIdx++; line = history[histIdx]; pos = graphemes(line).length; await redraw(); }
    return;
  }
  if (s === "\x1b[B" || s === "\x1bOB") {
    if (histIdx > 0) { histIdx--; line = history[histIdx]; } else { histIdx = -1; line = ""; }
  ...
  if (s === "\x03") { term.write("^C"); prompt(); return; }
  if (s === "\x0c") { term.write("\x1b[2J\x1b[H" + PROMPT + line); return; }
...
async function mount() {
  ...
  $("host").innerHTML = "";
  const workerOpt = $("o-worker").value === "false" ? false : "full";
  // 격자를 명시하면 그 크기를 지키고, "자동"이면 컨테이너를 따라간다(ResizeObserver).
  const g = $("o-grid").value;
  const fixed = g === "auto" ? null : g.split("x").map(Number);
  term = new Terminal({
    worker: workerOpt,
    loadFont: $("o-jet").checked ? "jetendard" : false,
    ...(fixed ? { cols: fixed[0], rows: fixed[1] } : {}),
    ...($("o-font").value ? { fontFamily: $("o-font").value } : {}),
    fontSize: Number($("o-size").value),
    lineHeight: Number($("o-line").value),
    cursorShape: $("o-cursor").value,
    scrollback: Number($("o-sb").value),
    ambiguousWide: $("o-amb").checked,
    ligatures: $("o-lig").checked,
    theme: allThemes[$("o-theme").value] ?? Object.values(allThemes)[0],
  });
  term.onData(onBytes);
  ...
  prompt();
  watchIme();
  globalThis.__term = term; // 콘솔에서 터미널을 만져볼 수 있게 노출한다
}

/* ── 옵션 배선 ──────────────────────────────────────────────────────── */
const live = (id, read, apply, view) => {
  const el = $(id);
  const fire = () => {
    const v = read(el);
    if (view) view(v);
  ...
  el.addEventListener(el.type === "range" ? "input" : "change", fire);
};

live("o-size", (e) => Number(e.value), (v) => term?.setOptions({ fontSize: v }), (v) => ($("v-size").textContent = v));
live("o-line", (e) => Number(e.value), (v) => term?.setOptions({ lineHeight: v }), (v) => ($("v-line").textContent = v.toFixed(2)));
live("o-font", (e) => e.value, (v) => v && term?.setOptions({ fontFamily: v }));
$("o-font").addEventListener("change", (e) => { if (!e.target.value) void mount(); }); // 기본으로 되돌리려면 재마운트
live("o-cursor", (e) => e.value, (v) => term?.setOptions({ cursorShape: v }));
live("o-sb", (e) => Number(e.value), (v) => term?.setOptions({ scrollback: v }), (v) => ($("v-sb").textContent = v));
live("o-amb", (e) => e.checked, (v) => term?.setOptions({ ambiguousWide: v }));
live("o-lig", (e) => e.checked, (v) => term?.setOptions({ ligatures: v }));
live("o-theme", (e) => e.value, (v) => term?.setTheme(allThemes[v]));
$("o-worker").addEventListener("change", mount); // 모드는 재마운트가 필요하다
$("o-grid").addEventListener("change", mount); // 격자 고정/자동 전환도 마찬가지
$("o-jet").addEventListener("change", mount); // 폰트 로드는 마운트 시점에 정해진다
...
$("o-theme-q").addEventListener("input", (e) => fillThemes(e.target.value));
...
function watchIme() {
  const ta = document.querySelector("#host textarea");
  if (!ta) return;
  const log = [];
  const show = (s) => {
    log.push(s);
  ...
  for (const name of ["compositionstart", "compositionupdate", "compositionend"])
    ta.addEventListener(name, (e) => show(`${name.slice(11)}(${JSON.stringify(e.data ?? "")})`));
  ta.addEventListener("input", (e) => show(`input[${e.inputType ?? "?"}]="${ta.value}"`));
}
...
fillThemes();
$("o-theme").value = Object.keys(allThemes).includes("dark") ? "dark" : Object.keys(allThemes)[0];
await mount();
