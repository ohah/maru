...
/// 슬롯 하나가 차지하는 셀 수. 아이콘 1셀 + 좌우 여백 1셀씩이라 3셀이면 인접 슬롯이 붙어 보이지 않는다.
pub const slot_cols: u32 = 3;
...
test "슬롯은 셀 정렬 폭으로 좌측부터 이어 붙는다" {
    ...
    try testing.expectEqual(@as(u32, 24), a.w); // 3셀 × 8px
...
test "바가 접혔거나 폭이 모자라면 슬롯을 하나도 그리지 않는다" {
    ...
    try testing.expect(slotRect(.{ .x = 0, .y = 0, .w = 40, .h = 18 }, 8, 0) == null);
    try testing.expect(slotRect(.{ .x = 0, .y = 0, .w = 48, .h = 18 }, 8, 0) != null); // 2슬롯 × 24px = 48
...
test "hit-test는 슬롯 안에서만 자리를 돌려준다" {
    ...
    try testing.expectEqual(@as(usize, 1), slotAtPoint(bar, 8, 40, 10).?);
    // 슬롯 오른쪽 여백·바 위아래는 no-op이다.
    try testing.expect(slotAtPoint(bar, 8, 120, 10) == null);
