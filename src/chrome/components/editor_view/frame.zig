...
/// **이것은 기본값이고 진짜 값은 설정이다**(`editor.max-columns` — `TermRuntime.editor_max_columns`).
///
/// **예전에는 스크롤 상한(`max_first_col = 10,000`)에서 파생된 상수였다.** 그 상한의 근거는 *"렌더
...
/// 지우고 **세는 쪽 하나만** 남겼고, 그것을 설정으로 냈다.
///
/// **기본 100,000 의 근거는 실제 파일이다** — 이 저장소에서 편집기가 열 수 있는(UTF-8) 파일의 가장 긴
/// 줄이 33,061자다. VSCode·Monaco 기본값(`editor.stopRenderingLineAfter` = 10,000)의 **열 배**이고,
/// 그쪽은 그 너머를 *"Rendering paused for long line"* 으로 알리고 멈춘다 — 관례는 따르고 값만 넉넉히.
pub const default_max_columns: u32 = 100_000;
...
test "CRT7 스크롤된 화면에서도 caret 은 제 줄·제 열에 선다 (사용자 제보 2026-09-12)" {
    ...
    //
    // 이 파일은 같은 배열을 **두 가지로** 인덱싱하고 있었다: 띠·검색·마크는 `first_line + v.line`,
    // caret 만 `v.line`. `VisualRow.line` 은 **뷰포트 첫 줄로부터의 상대 인덱스**이고(그 필드 doc)
    // `carets`·`lines` 는 **절대** 배열이므로, 스크롤되면 caret 이 **다른 줄의 자료**로 그려진다.
    // 고친 뒤에는 **덧셈이 한 자리뿐이다** — `VisualRow.docIndex` 가 그 축 변환을 소유하고
    // gutter 까지 그것을 부른다(두 자리에 같은 덧셈이 있으면 한쪽이 또 낡는다).
    //
