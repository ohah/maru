...
///
/// **4,000,000 으로도 재 봤고 되돌렸다** — 1MB 가 전부 NUL 인 적대적 입력(§3.8 표기로 8백만 열)에서
/// 처음 밀린 프레임이 **226ms** 라 「재앙 감지선」 200ms 를 넘었다. 그 비용은 체크포인트를 §2.1 의
/// 점진 계수처럼 **프레임에 나눠** 세우면 풀리지만 그것은 따로 할 조각이다. 그때 이 값을 올린다.
///
