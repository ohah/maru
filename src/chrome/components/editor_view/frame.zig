...
/// 체크포인트를 **몇 열마다** 둘지. 전개가 힌트에서 시작해도 최대 이만큼은 걷는다 — 그래서 이 값이
/// 「남는 걸음」의 상한이고, 동시에 메모리의 분모다(문서 전체 열 수 ÷ 이 값 × 8byte).
///
/// 1,024 의 근거: 화면 폭(수백 열)과 같은 자릿수라 남는 걸음이 프레임당 한 화면 몫을 안 넘고,
/// 20만열 줄이 195개(1.5KB)로 끝난다. 더 잘게 두면 메모리가, 크게 두면 남는 걸음이 늘어난다.
pub const col_mark_every: u32 = 1_024;

/// 체크포인트를 **어느 길이부터** 만들지. 이보다 짧은 줄은 처음부터 걸어도 화면 폭 수준이라 만들 몫이
/// 없다 — 만들면 대부분의 줄에 빈 몫만 늘어난다.
pub const col_mark_min_cols: u32 = 2_048;

/// 체크포인트를 **한 프레임에 몇 걸음까지** 세울지(§2.1 점진 계수와 같은 결).
///
/// **줄 단위로는 못 나눈다** — 이 조각이 겨냥하는 최악은 줄 **하나**가 수백만 열인 minified 파일이라,
/// 줄로 쪼개면 그 한 줄이 통째로 한 프레임에 들어온다. 그래서 `stepColumn` 걸음 수로 나누고 줄
/// **중간에서 이어 걷는다**(`columnCheckpointsFrom` — 재개점이 곧 마지막 마크다).
///
/// 400,000 의 근거: 실측(ReleaseFast) 걸음당 약 12.6ns 라 프레임당 약 5ms 이고 60fps 예산(16.7ms)
/// 안에 든다. 560만 열 한 줄이면 약 14프레임(≈70ms)에 정확해진다 — 그동안은 마크가 없는 구간만
/// 전개가 처음부터 걷고 **답은 같다**.
pub const col_mark_chunk_steps: usize = 400_000;

/// 가장 긴 줄을 셀 때 **여기까지만 센다**(§3.8 — *"초장문·극단 입력에서 기능을 줄인다"*).
///
/// **예전에는 스크롤 상한(`max_first_col = 10,000`)에서 파생된 값이었다.** 그 상한의 근거는 *"렌더
/// 비용이 밀린 거리에 비례한다"* 였는데, 줄마다 열→byte 체크포인트(`col_mark_every`)가 들어오면서
/// **그 비례가 사라졌다** — 실측(ReleaseFast, 화면을 20만열 줄로 채운 판, `first_col` 60,000):
/// 프레임당 **45.0ms → 0.68ms**(66배)이고 `first_col` 0 과 같은 자릿수다. 그래서 스크롤 상한을
/// 지우고, 여기 **세는 쪽 가드 하나만** 남긴다.
///
/// 1,000,000 의 근거는 **한 번 치르는 비용**이다. 실측(ReleaseFast, 6.3MB 한 줄 · 560만열):
/// 폭 세기 70.7ms · 체크포인트 71.8ms. 백만 열이면 그 둘이 각각 **13ms 쯤**이고, 문서를 열 때(폭)와
/// 처음 가로로 밀 때(체크포인트) 한 번씩만 치른 뒤 캐시가 산다.
///
/// **올려 보려다 되돌렸다 — 두 번.** 1MB 가 전부 NUL 인 적대적 입력(§3.8 표기로 8백만 열)에서 처음
/// 밀린 프레임이 「재앙 감지선」 200ms 를 넘는다:
///
///  · 4,000,000 · 한 번에 세우기 → **226ms**(내 기기 ReleaseFast). 그래서 점진 구축을 넣었다.
///  · 2,000,000 · 점진 구축 → 내 기기 Debug 111ms 로 통과했는데 **CI Debug 에서 308ms** 였다.
///    **감지선이 강제되는 곳은 내 기기가 아니라 CI 러너다** — 거기가 약 2.8배 느리다.
///
/// 그래서 값은 1,000,000 에 둔다. 점진 구축은 남는다 — 전이 구간을 고르게 만들고, **다음 조각**
/// (지금 보이는 줄의 `first_col` 까지를 먼저 세우는 우선순위)이 이 값을 올릴 여지를 만든다.
///
/// **그 너머는 못 간다** — 가로 위치의 상한이 `max_cols - 보이는 열`이고 `max_cols` 가 여기서 멈춘다.
/// 즉 이 값이 §3.8 의 「줄인 기능」이 서는 **유일한 자리**다.
pub const max_cols_count_limit: u32 = 1_000_000;
