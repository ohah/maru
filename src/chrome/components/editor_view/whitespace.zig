...
/// `line` 에서 기호를 세울 글자의 byte 자리를 `out` 에 오름차순으로 쓰고 그 수를 돌려준다(넘치면 앞에서부터 담은 만큼). `selections` 는 그 줄의
/// 선택 범위(오름차순 · 겹치지 않음) — `selection` 모드에서만 읽는다.
pub fn collect(line: []const u8, mode: Mode, selections: []const Range, out: []align(1) u32) usize {
...
test "WSR1 모드 다섯 — 앞뒤 공백 · 사이의 탭 · 홀로 선 공백 · 선택 · 뒤 공백 (§5.1e, VS Code _applyRenderWhitespace)" {
    ...
    const line = "  a b  c\t d  ";
    // all — 공백·탭 전부
    try testing.expectEqualSlices(u32, &.{ 0, 1, 3, 5, 6, 8, 9, 11, 12 }, buf[0..collect(line, .all, &.{}, &buf)]);
    // boundary — 사이의 홀로 선 공백(3)만 빠진다; 탭(8)과 그 뒤 공백(9 — 앞이 공백이었다)은 선다
    try testing.expectEqualSlices(u32, &.{ 0, 1, 5, 6, 8, 9, 11, 12 }, buf[0..collect(line, .boundary, &.{}, &buf)]);
    // trailing — 뒤 공백만
    try testing.expectEqualSlices(u32, &.{ 11, 12 }, buf[0..collect(line, .trailing, &.{}, &buf)]);
    // selection — [2, 7) 안만; 선택이 없으면(커서만) 없다
    try testing.expectEqualSlices(u32, &.{ 3, 5, 6 }, buf[0..collect(line, .selection, &.{.{ .start = 2, .end = 7 }}, &buf)]);
    try testing.expectEqual(@as(usize, 0), collect(line, .selection, &.{}, &buf));
    // 공백뿐인 줄 — trailing 이면 줄 전체
    try testing.expectEqualSlices(u32, &.{ 0, 1, 2 }, buf[0..collect(" \t ", .trailing, &.{}, &buf)]);
    // NBSP 는 공백이 아니다(글자다)
    try testing.expectEqual(@as(usize, 0), collect("a\u{a0}b", .all, &.{}, &buf));
    try testing.expectEqual(@as(usize, 0), collect("x", .none, &.{}, &buf));
}
...
test "WSR2 VS Code 와 대조 — monaco 0.56 renderViewLine2 가 낸 줄 80 × 모드 다섯(기호 871)과 같다 (§5.1e)" {
    ...
    for (parsed.value.cases) |c| {
        ...
        const got = buf[0..collect(c.l, mode, sels[0..c.s.len], &buf)];
