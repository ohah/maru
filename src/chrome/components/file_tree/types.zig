...
pub const Props = struct {
    ...
    /// 도메인 목록의 **전체** 행 수. 접근성 서술자의 `set_size` 가 이 값을 쓴다.
    ///
    /// `rows.len` 으로는 안 된다 — 그건 가상화된 **창**의 크기라, 스크린 리더가 400줄짜리 트리를
    /// 늘 "N / 8" 로 읽게 된다(`ui/semantics.zig` 의 `position_in_set` 주석이 그 이유를 소유한다).
    /// 0 은 "모른다"이고 그때는 집합 정보를 아예 안 싣는다.
    total_rows: u32 = 0,
};
