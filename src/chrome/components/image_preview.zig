...
/// 그림과 테두리 사이 여백.
///
/// ⚠️ **0이다. 여백을 둘 수 없다** — 그 영역을 채울 자리가 렌더 순서에 없기 때문이다. 렌더러는
/// `bottom quad(layer 2) → 셀 → 이미지(pass>=2) → under(0)·over(1) quad` 순으로 그리므로, **셀보다
/// 위이면서 이미지보다 아래인 quad 패스가 존재하지 않는다.** 2를 쓰면 터미널 글자가 배경 위로
/// 올라오고, 0·1을 쓰면 배경이 그림을 덮는다(둘 다 사용자 제보로 확인).
///
/// 그래서 배경을 깔지 않고 **테두리만 4변 스트립으로** 그린다 — 그림 위에 얹혀도 액자로 읽힌다.
/// 여백이 필요해지면 그때 렌더러에 「셀 위·이미지 아래」 패스를 하나 여는 것이 정공법이다.
