...
pub const TurnFileOrigin = enum {
    ...
    ai_edit,
    /// 에이전트의 **셸 명령이** 고쳤다 — provider 가 명령 전후의 작업트리 diff 로 검증했다(`Entry.shell_diff`,
    /// AT3b-2). 편집 도구와 **같은 급의 근거**라 화면에서는 같은 기호(`✎`)를 쓴다(사용자 결정 2026-09-20 — 내용
    /// diff 로 검증됐다). 값을 가르는 이유는 테스트·툴팁·뒷날의 표기 분리다. 다른 점 하나: 명령이 도는 몇 초
    /// 사이의 사용자 저장이 섞일 수 있다(계약 §8).
    shell_edit,
    /// 턴 구간에 바뀌었으나 편집 도구·셸 diff 소행이 **아니다** — 사용자·다른 세션·gitignore 파일 등.
    turn_change,
};
