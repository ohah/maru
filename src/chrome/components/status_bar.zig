...
pub const ItemId = enum(u64) {
    ...
    editor_cursor = 10,
    /// 이 문서에 쓰는 문법(`status-bar.md` 「언어 항목」). **표시 전용**이다.
    editor_language = 14,
    /// 언어 서버 상태(editor-surface-tooling.md §8.2a) — 「없음 — 설치」·「묻는 중」·「시작 중」·「연결」·「실패 — 다시」·
    /// 「거부됨 — 다시 묻기」. 저하 계열이라 `editor_degraded` 바로 뒤에 더한다(앞쪽 — 오래 살아남는다). **누를 수 있다.**
    editor_lsp = 15,
