...
pub const VisualRow = struct {
    ...
    /// 비례한다).
    ///
    /// **`start_byte`의 열은 `start_col`과 다를 수 있다** — 그때 쓰는 것이 `start_byte_col`이다.
    ///
    ...
    /// `start_byte`가 있는 **절대 열**. 보통 `start_col`과 같지만, 시작 열에 걸친 cluster를 렌더가
    /// **잘라 그리는** 경우(탭·§3.8 표기)에는 더 작다 — 그 cluster가 화면 0열보다 앞에서 시작하기
    /// 때문이다.
    ///
    /// **§3.8 표기 때문에 필요하다.** 표기는 `<U+202E>`처럼 폭이 고정(8칸)이라, 걸려 잘린 채
    /// 그려지면 그 cluster의 시작 열이 화면 0열보다 `F − c0`만큼 앞이고 **그 차가 끝까지 남는다**.
    /// 걸음을 화면 0열에서 시작하면 그만큼 영구히 밀린다.
    ///
    /// **탭에서는 구조적으로 불필요하다**(적대적 검증 5회차가 증명했다): 걸린 탭의 시작 열 `c0`와
    /// 화면 0열 `F`는 늘 **같은 탭스톱 구간**에 있어(`c0 < F < ((c0/tw)+1)*tw`) 어느 쪽에서
    /// 시작하든 첫 걸음 뒤 열이 즉시 수렴한다. 4차가 "탭스톱 때문"이라 적었던 것은 틀렸다 — 그때
