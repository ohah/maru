...
pub const Action = union(enum) {
    ...
    toggle_editor_wrap,
    /// 파일 안 다음/이전 충돌 구간으로 caret 을 옮긴다(docs/editor-merge-conflicts.md S5). 편집기 컨텍스트 기본키
    ...
    trigger_suggest,
    /// code action 목록(tooling §8.2h — `⌘.`, VS Code `editor.action.quickFix`). caret/선택 자리의 진단을 문맥으로 서버에 묻고 메뉴로 낸다.
    quick_fix,
...
pub fn parseAction(value: []const u8) ?Action {
    ...
    if (std.mem.eql(u8, value, "quick_fix")) return .quick_fix;
