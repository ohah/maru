...
pub const SidebarConfig = struct {
    ...
    /// 건드리던 옛 경로와 달리, 이쪽은 턴 경계·상태·알림을 통째로 훅에서 받는 **소스 추가**라 대가가
    /// 더 크다(docs/agent-hooks.md §1 — 훅은 그 Term 상태 판정의 **주 소스**가 된다).
    ///
...
pub const EditorConfig = struct {
    ...
    /// **괄호 쌍 색**(§5.1d). VS Code `editor.bracketPairColorization.enabled` 와 같은 기본(켬).
...
pub const Config = struct {
    ...
    /// 비치고, 명시 배경색 셀·텍스트는 그 위에 그려진다. **PNG 전 변종**(wuffs — JPEG 등 다른 포맷은 후속).
