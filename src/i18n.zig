...
const en: Table = .{
    ...
    .mob_servers = "Servers",
    .mob_servers_empty = "No servers yet",
    .mob_sessions_loading = "Loading sessions...",
    .mob_sessions_none = "No sessions on this machine",
    .mob_control_off_timeout = "No answer - maru may not be running there",
    .mob_control_off_noise = "Unexpected output - the server runs a forced command",
    .mob_control_off_protocol = "Version mismatch - update maru on that machine",
    .mob_control_off_frame = "The answer was too large",
    .mob_control_off_open = "Could not open the session channel - the terminal still works",
    .mob_control_off_missing = "maru is not on that machine - set the maru path in the server settings",
    .mob_control_off_failed = "maru quit right away on that machine",
    .mob_server_incomplete = "Address or user missing - cannot connect",
    .mob_server_first_connect = "First connect - fingerprint will be shown",
    ...
    .mob_conn_unreachable = "Could not reach the server - check address and port",
    .mob_conn_auth = "Neither the key nor the password was accepted",
    .mob_conn_hostkey = "The server key differs from the one you accepted - did not connect",
    .mob_conn_rejected = "You declined the fingerprint",
    .mob_conn_timeout = "No answer - gave up waiting",
    .mob_conn_closed = "The server closed the session",
    .mob_conn_ended = "Disconnected - go back and in to reconnect",
...
const ko: Table = .{
    ...
    .mob_settings = "설정",
    .mob_servers = "서버",
    .mob_servers_empty = "등록된 서버가 없다",
    .mob_sessions_loading = "세션을 받는 중",
    .mob_sessions_none = "이 기계에 세션이 없다",
    .mob_control_off_timeout = "답이 없다 — 그 기계에 maru 가 안 떠 있을 수 있다",
    .mob_control_off_noise = "엉뚱한 출력이 온다 — 그 서버는 명령을 강제한다",
    .mob_control_off_protocol = "버전이 다르다 — 그 기계의 maru 를 올려야 한다",
    .mob_control_off_frame = "답이 너무 크다",
    .mob_control_off_open = "세션 목록 채널을 못 열었다 — 터미널은 그대로 쓸 수 있다",
    .mob_control_off_missing = "그 기계에 maru 가 없다 — 서버 설정에 maru 경로를 적는다",
    .mob_control_off_failed = "그 기계의 maru 가 바로 끝났다",
    .mob_server_incomplete = "주소나 사용자가 없다 — 접속할 수 없다",
    .mob_server_first_connect = "처음 붙는 서버 — 지문을 확인하게 된다",
    ...
    .mob_keyboard = "자판",
    .mob_disconnect = "끊기",
    .mob_hostkey_title = "처음 붙는 서버",
    .mob_hostkey_hint = "이 지문이 서버의 것과 같은지 확인한다. 승인하면 저장된다.",
    .mob_hostkey_ok = "지문이 맞다 — 접속",
    .mob_hostkey_cancel = "취소",
    .mob_conn_connecting = "붙는 중...",
    .mob_conn_ready = "붙었다",
    .mob_conn_unreachable = "서버에 닿지 못했다 — 주소와 포트를 확인한다",
    .mob_conn_auth = "키도 비밀번호도 안 먹었다",
    .mob_conn_hostkey = "서버 키가 승인한 것과 다르다 — 붙지 않았다",
    .mob_conn_rejected = "지문을 거절했다",
    .mob_conn_timeout = "답이 없어 기다리기를 그만뒀다",
    .mob_conn_closed = "서버가 세션을 끊었다",
    .mob_conn_ended = "끊었다 — 다시 붙으려면 뒤로 나갔다 들어온다",
    .mob_conn_failed = "붙지 못했다",
    .mob_appearance = "모양",
