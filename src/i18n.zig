...
const Table = struct {
    ...
    /// 판 위 줄의 「둘 다」 — 순서를 말한다(S3b-3c). Current 판에서는 현재 것이 먼저다.
    editor_conflict_accept_both_current_first: [:0]const u8,
    editor_conflict_accept_both_incoming_first: [:0]const u8,
    editor_conflict_markers_remain: [:0]const u8,
    editor_untitled_no_base: [:0]const u8,
    editor_untitled_bad_name: [:0]const u8,
    editor_untitled_already_open: [:0]const u8,
    editor_untitled_name_not_file: [:0]const u8,
    ...
    app_reboot_revived: [:0]const u8,
    app_reboot_revived_agents: [:0]const u8,
    app_reboot_revived_agents_missed: [:0]const u8,
    ...
    fp_remote_delete_confirm: [:0]const u8,
    ...
    /// 에이전트 활동 도크(활동 뷰 계약 §2 · 이미지 종류는 docs/agent-image-gallery.md §2). **넷을 가른다** — 「에이전트가 없다」·
    ...
    agent_activity_result_lines_suffix: [:0]const u8,
    /// 그 호출이 **실패로 끝났다**. provider 가 적은 것만 근거로 쓴다(계약 §2.3) — 우리가 명령이나
    /// 출력을 해석해 판정하지 않는다.
    agent_activity_result_failed: [:0]const u8,
    ...
    agent_activity_count_suffix: [:0]const u8,
    agent_activity_activity_count_suffix: [:0]const u8,
    agent_activity_too_narrow: [:0]const u8,
    ...
    /// 않는다. 사용자가 붙여넣었으면 라벨은 그가 쓴 문장이고, 에이전트가 읽었으면 파일명인데 — 그것은
    /// 규약이 아니라 우연한 상관이라, 사용자가 경로를 적어 보낸 순간 틀린다.
    ///
    /// 도크 폭이 좁아 **짧아야 한다**(자리를 못 얻으면 통째로 빠진다). 아이콘이 아니라 글자인 이유는
    /// 폭 계산이 정직하기 때문이다 — PUA 합성 아이콘은 렌더가 2 칸을 쓰는데 폭 계산은 1 칸으로 봐서
    /// 라벨이 밀린다.
    agent_activity_filter_images: [:0]const u8,
    agent_activity_filter_execs: [:0]const u8,
    agent_activity_filter_reads: [:0]const u8,
    agent_activity_filter_all: [:0]const u8,
    agent_activity_origin_sent: [:0]const u8,
    /// 에이전트가 도구로 **읽은** 이미지(Claude `Read` · Codex `view_image`).
    agent_activity_origin_read: [:0]const u8,
    ...
    cfg_editor_minimap: [:0]const u8,
    cfg_editor_minimap_width: [:0]const u8,
    cfg_editor_sticky_scroll: [:0]const u8,
    cfg_editor_sticky_scroll_max_lines: [:0]const u8,
    ...
    /// 구현·타입 정의·선언(§8.2m) — 없음·프롬프트, 그리고 서버가 그 종류를 지원하지 않는다(`{0}` = 명령 이름).
    impl_none: [:0]const u8,
    impl_prompt: [:0]const u8,
    typedef_none: [:0]const u8,
    typedef_prompt: [:0]const u8,
    decl_none: [:0]const u8,
    decl_prompt: [:0]const u8,
    nav_unsupported: [:0]const u8,
    /// 문서 포맷(§8.2e) — 요청 뒤 문서가 바뀌어 결과를 버렸다 / 결과가 겹치거나 모양이 틀려 거부했다.
    fmt_stale: [:0]const u8,
    fmt_rejected: [:0]const u8,
    /// 심볼 이름 바꾸기(§8.2f). `{0}` = 파일 수 / 서버 message / 경로.
    rn_done: [:0]const u8,
    rn_error: [:0]const u8,
    rn_stale: [:0]const u8,
    rn_outside_root: [:0]const u8,
    rn_rejected: [:0]const u8,
    ...
    cfg_font_follow_system: [:0]const u8,
    ...
    /// 재시작 뒤 되살린 턴 기록의 tree 가 이미 gc 되어 **통째로 접었다**(AT7 — 계약 §6.3). «밀림» 과 다른 사실.
    scm_turns_expired: [:0]const u8,
    ...
    scm_turns_other_machine: [:0]const u8,
    ...
    fp_remote_tree_unreadable: [:0]const u8,
    fp_remote_watch_gave_up: [:0]const u8,
    fp_remote_watch_no_repo: [:0]const u8,
    scm_remote_watch_no_repo: [:0]const u8,
    fp_remote_tree_loading: [:0]const u8,
    fp_remote_file_read_failed: [:0]const u8,
    fp_remote_file_too_large: [:0]const u8,
    fp_remote_file_busy: [:0]const u8,
    fp_remote_create_failed: [:0]const u8,
    fp_remote_delete_failed: [:0]const u8,
    fp_remote_delete_not_empty: [:0]const u8,
    fp_remote_rename_failed: [:0]const u8,
    fp_remote_rename_stale: [:0]const u8,
    fp_remote_rename_unsupported: [:0]const u8,
    ...
    mob_a11y_select_from: [:0]const u8,
    mob_a11y_select_to: [:0]const u8,
    mob_a11y_selected_lines: [:0]const u8,
    mob_a11y_on: [:0]const u8,
    mob_a11y_off: [:0]const u8,
    mob_keyboard: [:0]const u8,
    mob_disconnect: [:0]const u8,
    mob_conn_reconnected: [:0]const u8,
    mob_switch_title: [:0]const u8,
    mob_switch_now: [:0]const u8,
    mob_switch_to: [:0]const u8,
    mob_switch_ok: [:0]const u8,
    mob_switch_cancel: [:0]const u8,
    ...
    web_dialog_title: [:0]const u8,
    /// 출처가 불투명할 때(`about:blank`·파일 등).
    web_dialog_title_page: [:0]const u8,
    web_dialog_ok: [:0]const u8,
    web_dialog_cancel: [:0]const u8,
    /// 떠나기 확인 — 페이지 글이 아니라 maru 가 정한 문구다(CEF 도 영어 한 줄을 준다).
    web_dialog_leave_title: [:0]const u8,
    web_dialog_leave_message: [:0]const u8,
    web_dialog_leave: [:0]const u8,
    /// 파일 선택 열기 창의 안내.
    web_file_pick_message: [:0]const u8,
    /// 저장 창의 안내 — 페이지에 쓰기를 맡기는 선택이라 올리기 문구와 가른다.
    web_file_save_message: [:0]const u8,
    /// 보이는데 초점이 없는 pane 의 웹 페이지가 대화상자 답을 기다린다(한 번).
    web_dialog_waiting: [:0]const u8,
    /// 이동 없이 두 번째 대화상자부터 보이는 선택(Chrome 과 같다 — `while(1) alert()` 의 탈출구).
    web_dialog_suppress: [:0]const u8,
    /// 폴더 올리기 확인 — `{0}` 은 파일 수(Chrome 과 같다 — 폴더를 통째로 올리기 전에 한 번 묻는다).
    web_folder_upload_title: [:0]const u8,
    web_folder_upload: [:0]const u8,
    /// `{0}` 은 상한(4096).
    web_folder_too_many: [:0]const u8,
    /// 권한 요청 sheet(W5b) — `{0}` 은 출처.
    web_permission_title: [:0]const u8,
    web_permission_title_page: [:0]const u8,
    web_permission_allow: [:0]const u8,
    web_permission_block: [:0]const u8,
    ...
    web_permission_open_settings: [:0]const u8,
    /// 화면 기록은 macOS 가 앱을 다시 띄워야 반영한다 — `{0}` 은 항목 이름.
    web_permission_macos_blocked_restart: [:0]const u8,
    ...
    /// 권한 이름(청한 목록의 한 줄).
    perm_camera: [:0]const u8,
    perm_microphone: [:0]const u8,
    perm_screen: [:0]const u8,
    perm_screen_audio: [:0]const u8,
    perm_geolocation: [:0]const u8,
    perm_notifications: [:0]const u8,
    perm_midi_sysex: [:0]const u8,
    perm_clipboard: [:0]const u8,
    perm_local_fonts: [:0]const u8,
    perm_window_management: [:0]const u8,
    perm_storage_access: [:0]const u8,
    perm_idle_detection: [:0]const u8,
    perm_camera_pan_tilt_zoom: [:0]const u8,
    perm_keyboard_lock: [:0]const u8,
    perm_pointer_lock: [:0]const u8,
    perm_protected_media_identifier: [:0]const u8,
    perm_register_protocol_handler: [:0]const u8,
    perm_multiple_downloads: [:0]const u8,
    perm_file_system_access: [:0]const u8,
    perm_local_network: [:0]const u8,
    perm_loopback_network: [:0]const u8,
    perm_sensors: [:0]const u8,
    perm_ar_session: [:0]const u8,
    perm_vr_session: [:0]const u8,
    perm_hand_tracking: [:0]const u8,
    perm_identity_provider: [:0]const u8,
    perm_web_app_installation: [:0]const u8,
    perm_disk_quota: [:0]const u8,
    perm_captured_surface_control: [:0]const u8,
...
const en: Table = .{
    ...
    .editor_conflict_accept_both_current_first = "Both (Current First)",
    .editor_conflict_accept_both_incoming_first = "Both (Incoming First)",
    .editor_conflict_markers_remain = "Conflict markers are still in this file",
    .editor_untitled_no_base = "There is no folder to save into. Open a folder in the explorer first.",
    .editor_untitled_bad_name = "Use a name inside the current folder.",
    .editor_untitled_already_open = "That file is already open in another tab.",
    .editor_untitled_name_not_file = "That name is not a file. Choose a different name.",
    ...
    .app_reboot_revived = "The Mac restarted, so the previous sessions ended — started {0} terminals fresh in their last folders.",
    .app_reboot_revived_agents = "The Mac restarted, so the previous sessions ended — started {0} terminals fresh and resumed {1} agent conversations.",
    .app_reboot_revived_agents_missed = "The Mac restarted, so the previous sessions ended — started {0} terminals fresh and resumed {1} agent conversations ({2} could not be found, so only a shell was opened).",
    ...
    .fp_remote_delete_confirm = "Delete the selected item on the remote? There is no Trash on a remote, so this cannot be undone.",
    ...
    .scm_turns_expired = "Saved turn history expired (its snapshots were garbage-collected)",
    .scm_turns_other_machine = "These agent turns were recorded on another machine",
    ...
    .fp_remote_tree_unreadable = "Could not read the remote folder. Check the SSH connection and the remote helper.",
    .fp_remote_watch_gave_up = "This remote cannot watch for changes. The tree stops refreshing on its own — collapse and expand a folder to re-read it.",
    .fp_remote_watch_no_repo = "This folder is not a Git repository, so changes are not watched. Pick a repository folder, or refresh manually.",
    .scm_remote_watch_no_repo = "This folder is not a Git repository, so changes are not watched. Open the repository root, or refresh manually.",
    .fp_remote_tree_loading = "Reading the remote folder\u{2026}",
    .fp_remote_file_read_failed = "Could not read the remote file. Check the SSH connection.",
    .fp_remote_create_failed = "Could not create it on the remote. Check the SSH connection and permissions.",
    .fp_remote_delete_failed = "Could not delete on the remote. Check the SSH connection and permissions.",
    .fp_remote_delete_not_empty = "That folder is not empty. Maru does not delete folder trees on a remote — empty it first.",
    .fp_remote_rename_failed = "Could not rename on the remote. Check the SSH connection and permissions.",
    .fp_remote_rename_stale = "That is no longer the same item on the remote. The tree is being re-read — try again.",
    .fp_remote_rename_unsupported = "This remote cannot rename without replacing an existing file, so the rename was not attempted.",
    .fp_remote_file_too_large = "The remote file is too large to view (over 4 MiB).",
    .fp_remote_file_busy = "Another remote file is still loading.",
    ...
    .mob_a11y_select_from = "select from here",
    .mob_a11y_select_to = "select to here",
    .mob_a11y_selected_lines = "lines selected",
    .mob_a11y_on = "on",
    .mob_a11y_off = "off",
    .mob_keyboard = "kbd",
    .mob_disconnect = "disc",
    .mob_conn_reconnected = "The connection dropped and was re-established",
    .mob_switch_title = "Switch servers?",
    .mob_switch_now = "Connected to",
    .mob_switch_to = "Switch to",
    .mob_switch_ok = "Disconnect and switch",
    .mob_switch_cancel = "Cancel",
    ...
    .web_dialog_title_page = "This page says",
    .web_dialog_ok = "OK",
    .web_dialog_cancel = "Cancel",
    .web_dialog_leave_title = "Leave site?",
    .web_dialog_leave_message = "Changes you made may not be saved.",
    .web_dialog_leave = "Leave",
    .web_file_pick_message = "Choose files for this page",
    .web_file_save_message = "Choose where this page saves the file",
    .web_dialog_waiting = "A web page in another pane is waiting for your answer. Click that pane to see the dialog.",
    .web_dialog_suppress = "Prevent this page from creating additional dialogs",
    .web_folder_upload_title = "Upload {0} files to this site?",
    .web_folder_upload = "Upload",
    .web_folder_too_many = "This folder has more than {0} files. Choose a smaller folder.",
    .web_permission_title = "{0} wants to",
    .web_permission_title_page = "This page wants to",
    .web_permission_allow = "Allow",
    ...
    .perm_screen = "Share your entire screen",
    .perm_screen_audio = "Share your system audio",
    .perm_geolocation = "Know your location",
    .perm_notifications = "Show notifications",
    .perm_midi_sysex = "Control and reprogram your MIDI devices",
    .perm_clipboard = "See text and images copied to the clipboard",
    .perm_local_fonts = "Use fonts installed on this device",
    .perm_window_management = "Manage windows on all your displays",
    .perm_storage_access = "Use cookies and site data it saved on other sites",
    .perm_idle_detection = "Know when you are actively using this device",
    .perm_camera_pan_tilt_zoom = "Move your camera",
    .perm_keyboard_lock = "Take over your keyboard",
    .perm_pointer_lock = "Lock and use your mouse pointer",
    .perm_protected_media_identifier = "Identify your device to play protected content",
    .perm_register_protocol_handler = "Open certain kinds of links",
    .perm_multiple_downloads = "Download multiple files",
    .perm_file_system_access = "Edit files on this device",
    .perm_local_network = "Find and connect to devices on your local network",
    .perm_loopback_network = "Connect to apps running on this device",
    .perm_sensors = "Use your motion and light sensors",
    .perm_ar_session = "Use augmented reality",
    .perm_vr_session = "Use virtual reality",
    .perm_hand_tracking = "Track your hands",
    .perm_identity_provider = "Sign you in with an identity provider",
    .perm_web_app_installation = "Install web apps",
    .perm_disk_quota = "Store more data on this device",
    .perm_captured_surface_control = "Scroll and zoom the tab you share",
    ...
    .agent_activity_too_narrow = "Not enough room to show the list",
    ...
    .agent_activity_filter_all = "All",
    .agent_activity_origin_sent = "Sent",
    .agent_activity_origin_read = "Read",
    ...
    .cfg_editor_minimap = "Show the minimap next to the editor body",
    .cfg_editor_minimap_width = "Minimap width in cells",
    .cfg_editor_sticky_scroll = "Pin the enclosing scopes' header lines at the top while scrolling",
    .cfg_editor_sticky_scroll_max_lines = "Maximum number of pinned header lines",
    ...
    .impl_none = "No implementation found",
    .impl_prompt = "{0} implementations",
    .typedef_none = "No type definition found",
    .typedef_prompt = "{0} type definitions",
    .decl_none = "No declaration found",
    .decl_prompt = "{0} declarations",
    .nav_unsupported = "This language server does not support {0}",
    ...
    .rn_error = "Cannot rename — {0}",
    .rn_stale = "Document changed — rename result discarded",
    .rn_outside_root = "Not applied — a file is outside the workspace root: {0}",
    .rn_rejected = "Not applied — edit cannot be applied: {0}",
    ...
    .cfg_font_follow_system = "Follow the system text size (accessibility)",
    ...
    .cfg_browser_engine = "Web browser tab engine (applies after restarting maru)",
    ...
    .cfg_notif_web = "Web notifications from Chromium browser tabs",
...
const ko: Table = .{
    ...
    .editor_conflict_accept_both_current_first = "둘 다 (현재 것 먼저)",
    .editor_conflict_accept_both_incoming_first = "둘 다 (들어온 것 먼저)",
    .editor_conflict_markers_remain = "이 파일에 충돌 표시가 남아 있습니다",
    .editor_untitled_no_base = "저장할 폴더가 없습니다. 탐색기에서 폴더를 먼저 여세요",
    .editor_untitled_bad_name = "지금 폴더 안의 이름을 쓰세요",
    .editor_untitled_already_open = "그 파일은 다른 탭에 이미 열려 있습니다",
    .editor_untitled_name_not_file = "그 이름은 파일이 아닙니다. 다른 이름을 쓰세요",
    ...
    .app_reboot_revived = "맥이 재시작돼 이전 세션이 끝났습니다 — 터미널 {0}개를 마지막 위치에서 새로 시작했습니다.",
    .app_reboot_revived_agents = "맥이 재시작돼 이전 세션이 끝났습니다 — 터미널 {0}개를 새로 시작하고 에이전트 대화 {1}개를 이어갔습니다.",
    .app_reboot_revived_agents_missed = "맥이 재시작돼 이전 세션이 끝났습니다 — 터미널 {0}개를 새로 시작하고 에이전트 대화 {1}개를 이어갔습니다({2}개는 대화 파일을 못 찾아 셸만 열었습니다).",
    ...
    .fp_remote_delete_confirm = "선택한 항목을 원격에서 삭제할까요? 원격에는 휴지통이 없어 되돌릴 수 없습니다.",
    ...
    .scm_turns_expired = "저장된 턴 기록이 오래되어 사라졌습니다 (스냅샷이 정리됨)",
    .scm_turns_other_machine = "이 세션의 턴은 다른 기계에서 기록됐습니다",
    ...
    .fp_remote_tree_unreadable = "원격 폴더를 읽지 못했습니다. SSH 연결과 원격 도우미를 확인해주세요.",
    .fp_remote_watch_gave_up = "이 원격은 변경을 감시하지 못합니다. 트리가 스스로 갱신되지 않으니 폴더를 접었다 펴서 다시 읽어주세요.",
    .fp_remote_watch_no_repo = "이 폴더는 Git 저장소가 아니라 변경을 감시하지 않습니다. 저장소 폴더를 열거나, 새로고침해서 다시 읽어주세요.",
    .scm_remote_watch_no_repo = "이 폴더는 Git 저장소가 아니라 변경을 감시하지 않습니다. 저장소 루트를 열거나, 직접 새로고침하세요.",
    .fp_remote_tree_loading = "원격 폴더를 읽는 중\u{2026}",
    .fp_remote_file_read_failed = "원격 파일을 읽지 못했습니다. SSH 연결을 확인해주세요.",
    .fp_remote_create_failed = "원격에 만들지 못했습니다. SSH 연결과 권한을 확인해주세요.",
    .fp_remote_delete_failed = "원격에서 삭제하지 못했습니다. SSH 연결과 권한을 확인해주세요.",
    .fp_remote_delete_not_empty = "그 폴더가 비어 있지 않습니다. 원격에서는 폴더를 통째로 지우지 않습니다 — 먼저 비워주세요.",
    .fp_remote_rename_failed = "원격에서 이름을 바꾸지 못했습니다. SSH 연결과 권한을 확인해주세요.",
    .fp_remote_rename_stale = "원격에서 그 항목이 더 이상 같은 것이 아닙니다. 트리를 다시 읽는 중이니 다시 시도해주세요.",
    .fp_remote_rename_unsupported = "이 원격은 기존 파일을 덮어쓰지 않는 이름 변경을 못 해서, 바꾸지 않았습니다.",
    .fp_remote_file_too_large = "원격 파일이 너무 커서 볼 수 없습니다 (4 MiB 초과).",
    .fp_remote_file_busy = "다른 원격 파일을 여는 중입니다.",
    ...
    .mob_a11y_select_from = "여기서부터 선택",
    .mob_a11y_select_to = "여기까지 선택",
    .mob_a11y_selected_lines = "줄 선택됨",
    .mob_a11y_on = "켜짐",
    .mob_a11y_off = "꺼짐",
    .mob_keyboard = "자판",
    .mob_disconnect = "끊기",
    .mob_conn_reconnected = "연결이 끊겨 다시 연결했습니다",
    .mob_switch_title = "다른 서버로 갈까요?",
    .mob_switch_now = "지금 붙어 있는 서버",
    .mob_switch_to = "갈 서버",
    .mob_switch_ok = "끊고 전환",
    .mob_switch_cancel = "취소",
    ...
    .web_dialog_title_page = "이 페이지의 메시지",
    .web_dialog_ok = "확인",
    .web_dialog_cancel = "취소",
    .web_dialog_leave_title = "사이트에서 나가시겠습니까?",
    .web_dialog_leave_message = "변경한 내용이 저장되지 않을 수 있습니다.",
    .web_dialog_leave = "나가기",
    .web_file_pick_message = "이 페이지에 올릴 파일을 고르세요",
    .web_file_save_message = "이 페이지가 파일을 저장할 자리를 고르세요",
    .web_dialog_waiting = "다른 pane 의 웹 페이지가 답을 기다립니다. 그 pane 을 누르면 대화상자가 뜹니다.",
    .web_dialog_suppress = "이 페이지가 대화상자를 더 띄우지 못하게 하기",
    .web_folder_upload_title = "파일 {0}개를 이 사이트에 올릴까요?",
    .web_folder_upload = "올리기",
    .web_folder_too_many = "폴더의 파일이 {0}개를 넘습니다. 더 작은 폴더를 고르세요.",
    .web_permission_title = "{0}에서 권한을 요청합니다",
    .web_permission_title_page = "이 페이지가 권한을 요청합니다",
    .web_permission_allow = "허용",
    ...
    .perm_screen = "화면 전체 공유",
    .perm_screen_audio = "시스템 소리 공유",
    .perm_geolocation = "위치 확인",
    .perm_notifications = "알림 표시",
    .perm_midi_sysex = "MIDI 기기 제어·재설정",
    .perm_clipboard = "클립보드에 복사한 글·그림 보기",
    .perm_local_fonts = "이 기기에 설치된 글꼴 사용",
    .perm_window_management = "모든 화면의 창 관리",
    .perm_storage_access = "다른 사이트에 저장한 쿠키·사이트 데이터 사용",
    .perm_idle_detection = "이 기기를 쓰고 있는지 알기",
    .perm_camera_pan_tilt_zoom = "카메라 움직이기",
    .perm_keyboard_lock = "키보드 가져가기",
    .perm_pointer_lock = "마우스 포인터 잠그고 쓰기",
    .perm_protected_media_identifier = "보호된 콘텐츠 재생을 위해 기기 식별",
    .perm_register_protocol_handler = "특정 종류의 링크 열기",
    .perm_multiple_downloads = "여러 파일 내려받기",
    .perm_file_system_access = "이 기기의 파일 편집",
    .perm_local_network = "로컬 네트워크의 기기 찾기·연결",
    .perm_loopback_network = "이 기기에서 도는 앱에 연결",
    .perm_sensors = "동작·빛 센서 사용",
    .perm_ar_session = "증강 현실 사용",
    .perm_vr_session = "가상 현실 사용",
    .perm_hand_tracking = "손 추적",
    .perm_identity_provider = "ID 공급자로 로그인",
    .perm_web_app_installation = "웹 앱 설치",
    .perm_disk_quota = "이 기기에 데이터 더 저장",
    .perm_captured_surface_control = "공유한 탭 스크롤·확대",
    ...
    .agent_activity_too_narrow = "목록을 그릴 자리가 없습니다",
    ...
    .agent_activity_filter_all = "전체",
    .agent_activity_origin_sent = "첨부",
    .agent_activity_origin_read = "읽음",
    ...
    .cfg_editor_minimap = "편집기 본문 옆에 미니맵을 표시",
    .cfg_editor_minimap_width = "미니맵 폭(칸)",
    .cfg_editor_sticky_scroll = "스크롤할 때 감싸는 스코프의 머리줄을 위에 고정",
    .cfg_editor_sticky_scroll_max_lines = "고정할 머리줄의 최대 수",
    ...
    .impl_none = "구현을 찾지 못했습니다",
    .impl_prompt = "구현 {0}개",
    .typedef_none = "타입 정의를 찾지 못했습니다",
    .typedef_prompt = "타입 정의 {0}개",
    .decl_none = "선언을 찾지 못했습니다",
    .decl_prompt = "선언 {0}개",
    .nav_unsupported = "이 언어 서버는 {0}을(를) 지원하지 않습니다",
    .fmt_stale = "문서가 바뀌어 포맷 결과를 버렸습니다",
    .fmt_rejected = "포맷 결과를 거부했습니다 — 겹치거나 모양이 틀립니다",
    .rn_done = "{0}개 파일에서 이름을 바꿨습니다",
    .rn_error = "이름을 바꿀 수 없습니다 — {0}",
    .rn_stale = "문서가 바뀌어 이름 바꾸기 결과를 버렸습니다",
    .rn_outside_root = "루트 밖 파일이 있어 적용하지 않습니다 — {0}",
    .rn_rejected = "적용할 수 없는 편집입니다 — {0}",
    ...
    .cfg_font_follow_system = "시스템 글자 크기 따라가기(접근성)",
    ...
    .cfg_browser_engine = "웹 브라우저 탭 엔진(maru 를 다시 시작하면 적용)",
    ...
    .cfg_notif_web = "Chromium 브라우저 탭의 웹 알림",
