...
/// `maru win32-scm-draw-smoke` — W8.4. **소스 컨트롤 목록이 Windows 화면에 뜬다.**
///
/// 데이터는 #2496·#2501 이 뚫었고 투영은 원래 중립이었다(§2m.6). 남은 것은 그 `DrawList` 를 Windows 가
/// 이미 가진 셀 파이프라인에 먹이는 일뿐이라, 파일 트리 그리기(§2m.6)와 **같은 네 단계**를 쓴다.
///
/// **저장소 자신을 연다.** fixture 를 만들면 화면이 늘 같아 "그럴듯한 그림" 과 "실제로 도는 것" 이 안
/// 갈린다. 진짜 작업 트리 상태가 떠야 한다.
fn runWin32ScmDrawSmoke(io: std.Io, allocator: std.mem.Allocator, stdout: *std.Io.Writer, stderr: *std.Io.Writer) !void {
    ...
    var raster = dwrite_font.Rasterizer.create(allocator, cfg.font.family, cfg.font.fallback, cfg.font.size) catch |err| {
        try stderr.print("maru win32-scm-draw-smoke: could not set up the font({s})\n", .{@errorName(err)});
        try stderr.flush();
        return error.UnknownCommand;
    };
    defer raster.destroy();
    ...
    const scratch_px = try allocator.alloc(u8, win32_text.NeutralRasterizer.scratchSizeFor(cell_w * 2, cell_h));
    defer allocator.free(scratch_px);
    const shaper = win32_text.Shaper{ .raster = raster };
    const rasterizer = win32_text.NeutralRasterizer{ .raster = raster, .scratch = scratch_px };

    const title = std.unicode.utf8ToUtf16LeStringLiteral("maru (W8.4 source control)");
    var window = win32_window.Window.create(allocator, title, 760, 560) catch |err| {
        try stderr.print("maru win32-scm-draw-smoke: could not create the window({s}, Win32 error {d})\n", .{ @errorName(err), win32_window.last_create_error });
        try stderr.flush();
        return error.UnknownCommand;
    };
    defer window.destroy();
    ...
    const initial = window.clientSize() orelse win32_window.ClientSize{ .width_px = 760, .height_px = 560 };
    var present = d3d11_present.Present.create(allocator, window.hwnd, initial.width_px, initial.height_px) catch |err| {
        try stderr.print("maru win32-scm-draw-smoke: could not set up the present path({s})\n", .{@errorName(err)});
        try stderr.flush();
        return error.UnknownCommand;
    };
    defer present.destroy();
    ...
    var atlas_h = renderer_state.atlas.config.atlas_height_px;
    var pipeline = d3d11_cells.CellPipeline.createEmptyAtlas(allocator, present.device, present.context, atlas_w, atlas_h) catch |err| {
        try stderr.print("maru win32-scm-draw-smoke: could not set up the cell pipeline({s})\n", .{@errorName(err)});
        try stderr.flush();
        return error.UnknownCommand;
    };
    defer pipeline.destroy();
    ...
    while (rounds < 6000) : (rounds += 1) {
        if (backend.takeRepoStatusResult()) |r| {
            taken = r;
            break;
        }
        io.sleep(.fromMilliseconds(1), .awake) catch {};
    ...
    // `scm_dock.zig`). 여기서 채우면 제품이 안 밟는 자리를 재게 된다.
    const scm_view = maru.session.scm_view;
    const row_capacity = 512;
    const rows_buf = try allocator.alloc(scm_view.Row, row_capacity);
    defer allocator.free(rows_buf);
    const path_scratch = try allocator.alloc(u8, 256 * 1024);
    defer allocator.free(path_scratch);
    const collapsed = [_]bool{ false, false };
    const expanded = [_]bool{ true, true };
    const model = scm_view.build(status.text, "", "", "", collapsed, expanded, false, rows_buf, path_scratch);

    // ── 투영 ─────────────────────────────────────────────────────────────────────────────────
    const grid = win32_window.cellsForClient(initial.width_px, initial.height_px, cell_w, cell_h) orelse
        maru.terminal.Size{ .cols = 80, .rows = 24 };
    const fg: maru.terminal.Color = .{ .rgb = .{ .r = 0xD8, .g = 0xE0, .b = 0xF0 } };
    const muted: maru.terminal.Color = .{ .rgb = .{ .r = 0x7A, .g = 0x86, .b = 0x99 } };
    const accent: maru.terminal.Color = .{ .rgb = .{ .r = 0x8A, .g = 0xB4, .b = 0xF8 } };
    const collapsed_flags = try allocator.alloc(bool, model.rows.len);
    defer allocator.free(collapsed_flags);
    @memset(collapsed_flags, false);

    var draw_list = try coretext_frame_builder.buildDockScmDrawList(
        allocator,
        grid.cols,
        grid.rows,
        model.head,
        model.rows,
        collapsed_flags,
        null, // 선택 없음 — 띠는 파일 트리 슬라이스가 이미 다뤘다(§2m.7)
        0,
        fg,
        muted,
        accent,
    ...
    // ── 화면 ─────────────────────────────────────────────────────────────────────────────────
    //
    // **소유권은 프레임으로 넘어간다** — 여기서 또 해제하면 double free 다(§2m.6 에서 실측했다).
    var frame = renderer_state.buildFrameFromDrawListWithRasterizer(allocator, draw_list, shaper, rasterizer) catch |err| {
        draw_list.deinit(allocator);
        return err;
    };
    ...
    // ── 판정: 내용이다 ───────────────────────────────────────────────────────────────────────
    //
    // 프레임 수나 셀 수만 세면 **빈 목록도 초록**이다. 그려진 셀에서 글자를 도로 읽어 브랜치 이름이
    // 실제로 있는지 본다 — 이 저장소는 늘 브랜치 위에 있으므로 fixture 없이 성립하는 판정이다.
    var line: std.ArrayList(u8) = .empty;
    defer line.deinit(allocator);
    for (frame.draw_list.cells) |cell| {
    ...
    const branch = model.head.branch orelse "";
    const branch_drawn = branch.len > 0 and std.mem.indexOf(u8, line.items, branch) != null;

    const clear = d3d11_present.clearColorFromArgb(0xFF1E2430);
    var frames: usize = 0;
    var close_requested = false;
    while (frames < 120 and !window.quit_requested and !close_requested) : (frames += 1) {
        for (window.poll()) |ev| switch (ev) {
            .close_requested => close_requested = true,
            .resized => |r| try present.resize(r.width_px, r.height_px),
            else => {},
    ...
    try stdout.print("repo={s}\n", .{repo});
    try stdout.print("branch={s} model_rows={d} empty={}\n", .{ branch, model.rows.len, model.empty });
    try stdout.print("grid={d}x{d} d3d_cells={d}\n", .{ grid.cols, grid.rows, cells.items.len });
    try stdout.print("branch_drawn={}\n", .{branch_drawn});
    try stdout.print("frames_presented={d}\n", .{frames});
    try maru.renderer.writeRenderFrameStats(stdout, "renderer_", stats);
    try stdout.flush();

    if (!stats.prepared() or cells.items.len == 0 or !branch_drawn) {
        try stderr.print("maru win32-scm-draw-smoke: not drawn as expected (prepared={} cells={d} branch_drawn={})\n", .{ stats.prepared(), cells.items.len, branch_drawn });
        try stderr.flush();
        return error.UnknownCommand;
    }
}
