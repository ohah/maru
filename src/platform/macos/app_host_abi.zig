...
// Reset to Defaults 메뉴 — resetAllSettings(통합 리셋)에 위임한다: 모든 config를 내장 기본값으로 되돌리고 config 파일을
// 삭제해 런타임·영속 모두 공장 기본값으로 만든다(커맨드 팝업 "Reset All Settings to Defaults"와 같은 동작). 항상
