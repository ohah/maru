...
pub const AppSession = struct {
    ...
    find_nav: bool = false,
    // 타이핑(글자 입력) 중 마우스 커서 숨김 1회성 신호(config input.mouse-hide-while-typing). handleKeyEvent가
    ...
    /// macOS Option을 Meta(Alt)로 쓸지(config input.option-as-meta 캐시). handleKeyEvent가 encodeKey에 넘겨 Option
    ...
    pub fn handleKeyEvent(self: *AppSession, event: terminal.KeyEvent) !FrameSummary {
        ...
        const result = try self.frame_loop.handleKeyEvent(self.loaded_config.keyBindingResolver(), key_event, self.option_as_meta);
        switch (result) {
    ...
    fn startUpload(self: *AppSession, ctl: []const u8, dest: []const u8, path: []const u8) !void {
        ...
        const name = maru.cli.ssh.sanitizeDropFilename(self.allocator, path) catch |e| {
    ...
    /// 자동(별도 unhide 불요). take_bell과 같은 1회성 패턴이라 한 tick에 여러 글자를 쳐도 hide 호출은 한 번이다.
    pub fn takeMouseHide(self: *AppSession) bool {
    ...
    pub fn runAction(self: *AppSession, action_key: []const u8) bool {
        ...
        // 가로채면 안 된다. anyOverlayOpen 단일 출처로 판정(settings·context_menu 포함 — 예전엔 일부 모달을 누락해 세팅
        // 중 ⌘T가 샜다). 모달 자신의 키는 chrome_host.handleInput·녹음 캡처가 따로 처리하므로 이 경로를 안 거친다.
...
test "activateSurfaceById: surface.id로 (탭·split panel·가로탭)을 역조회해 그 자리로 활성화한다" {
    ...
    // 중요한 이유: 알림은 "지금 안 보는 터미널"이 보내므로, 클릭이 정확히 그 화면을 되살리지 못하면 기능이 무의미하다.
    if (builtin.os.tag != .macos) return error.SkipZigTest; // splitActivePane/newTab/newTermInActivePane = 실 PTY/CoreText
    const allocator = std.testing.allocator;
    const session = try allocator.create(AppSession);
    defer allocator.destroy(session);
    try session.init(std.Io.Threaded.global_single_threaded.io(), allocator, .{
    ...
    defer session.deinit();
    session.backing_width_px = session.sidebar_width_px + 800; // split/resize가 의미 있으려면 backing 필요
...
test "setSystemAppearance: follow-system on이면 light/dark 프리셋으로 교체, off면 무시 (F2-9)" {
    ...
    const light_bg = config_mod.theme.presetColors(.solarized_light).background;
    const dark_bg = config_mod.theme.presetColors(.gruvbox_dark).background;

    // follow-system OFF(기본): setSystemAppearance가 테마를 안 바꾼다.
    ...
    session.setSystemAppearance(false); // 라이트라고 알려도
    try std.testing.expectEqualStrings(config_mod.theme.presetColors(.maru).background, session.loaded_config.config.theme.background); // maru 유지

    // follow-system ON: preset-light/dark로 교체.
    ...
    session.setSystemAppearance(false); // 라이트 → solarized-light
    try std.testing.expectEqualStrings(light_bg, session.loaded_config.config.theme.background);
    session.setSystemAppearance(true); // 다크 → gruvbox-dark
...
// 기본값으로 되돌리는지. 파일 덮어쓰기·테마 프리셋은 별도 통합 테스트가 다루므로, 여기선 런타임 상태
// (appearance·cell 메트릭·behavior 캐시·loaded_config)가 기본값이 되는지 + 완료 notice가 뜨는지(회귀 가드) 본다.
...
test "focusedTermCwd/workspaceRootCwd/newSurfaceCwd: 포커스 cwd 상속 + inherit 토글 폴백" {
    ...
    var buf: [std.fs.max_path_bytes]u8 = undefined;
    // OSC 7 보고 전(셸 통합 없음·첫 프롬프트 전) → 상속할 포커스 cwd 없음(null). focusedTermCwd는 lockCore 아래
    // currentCwd를 buf로 복사해 돌려준다(reader 스레드 OSC 7 free와의 레이스 차단) — 빈 cwd는 그대로 null.
    ...
    try session.activeSurface().core.write("\x1b]7;file://h/tmp/proj\x07");
    try std.testing.expectEqualStrings("/tmp/proj", session.focusedTermCwd(&buf).?);
...
// 버린다(config 안 건드림). config 파일을 덮어쓰는 파괴적 동작이라 무확인 즉시 실행을 막는 게이트를 close-confirm과 같은
// 키 경로(handleKeyEvent → chrome 라우팅 → confirm_accept/cancel)로 증명한다.
...
// 렌더러가 이 값/1000을 화면 clear color alpha에 곱해 default 배경만 투명해진다(명시적 배경색 셀은 셀 quad라 불변).
test "window.opacity → metalFrame.window_opacity_milli ×1000 (F1-1)" {
    if (builtin.os.tag != .macos) return error.SkipZigTest;
    const allocator = std.testing.allocator;
    const session = try allocator.create(AppSession);
    defer allocator.destroy(session);
    ...
    // 기본(불투명) → 1000.
    try std.testing.expectEqual(@as(u32, 1000), session.metalFrame().window_opacity_milli);

    // 0.6 → 600(반올림).
    session.appearance.window_opacity = 0.6;
    try std.testing.expectEqual(@as(u32, 600), session.metalFrame().window_opacity_milli);

    // 0.0(완전 투명) → 0.
    session.appearance.window_opacity = 0.0;
    try std.testing.expectEqual(@as(u32, 0), session.metalFrame().window_opacity_milli);
}
