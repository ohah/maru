...
void maru_macos_chrome_lab_smoke_render(
    ...
    const BOOL drew = maru_metal_renderer_draw(
        ...
        NULL,
        0,
        /* 캡처의 "아무도 안 그린 자리" 색. **편집기 배경(`surface_bg` = 20,20,20)과 달라야 한다** —
           같으면 편집기 뷰 경계가 캡처에서 보이지 않고, 뷰포트가 화면보다 작을 때 "화면이 안 찼는데
           스크롤바가 있다"처럼 읽힌다(§4.1b). 창 배경이 편집기보다 조금 어둡다는 자연스러운 관계로
           둔다 — 픽스처가 제품에 없는 색을 지어내면 리뷰어를 오도한다. */
        0xFF0A0A0Au,
        0,
        1000,
        0,
