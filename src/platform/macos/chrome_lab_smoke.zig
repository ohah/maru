...
pub fn main(init: std.process.Init) !void {
    ...
    if (scenario_id == .editor_real_file or scenario_id == .editor_typescript or scenario_id == .editor_minimap) {
        ...
        const ts = scenario_id == .editor_typescript or scenario_id == .editor_minimap;
...
fn scenarioFromEnvValue(raw: []const u8) ?lab.ScenarioId {
    ...
    if (std.mem.eql(u8, raw, "editor-conflict")) return .editor_conflict;
    ...
    if (std.mem.eql(u8, raw, "editor-minimap")) return .editor_minimap;
    ...
    if (std.mem.eql(u8, raw, "editor-merge-scrolled")) return .editor_merge_scrolled;
    if (std.mem.eql(u8, raw, "editor-merge-hscrolled")) return .editor_merge_hscrolled;
    if (std.mem.eql(u8, raw, "editor-merge-caret")) return .editor_merge_caret;
...
fn artifactName(id: lab.ScenarioId) []const u8 {
    return switch (id) {
        ...
        .editor_merge_scrolled => "editor-merge-scrolled",
        .editor_merge_hscrolled => "editor-merge-hscrolled",
        .editor_merge_caret => "editor-merge-caret",
