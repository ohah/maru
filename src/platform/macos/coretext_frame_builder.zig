...
pub fn buildFilePanelHeaderDrawList(
    ...
    if (cols >= 1) {
        if (dock_layout.headerCellLayout(cols, dirty, external_change)) |header| {
            ...
            // **`> 1` 과 `> 0` 은 같은 화면을 낸다**(그 변이가 살아남는 것이 정상이다 — 적대적
            // 검증 2026-09-09 Q12): `control_start == 1` 이면 아래가 `start_col = 1`·
            // `end_col = 1` 인 **빈 범위**로 불려 아무 셀도 안 나온다. 그래도 `> 1` 인 것은
            // **뜻**이다 — "그릴 칸이 있나" 를 여기서 묻고, 없는데 부르지 않는다.
