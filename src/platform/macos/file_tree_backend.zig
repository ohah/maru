...
/// 멀티루트에서 원격이 슬롯 넷을 다 물어 **로컬 트리가 멎는다.**
///
/// **재측정 끝났다 — 2 를 유지한다**(2026-09-06, 계획 §10.16). 상한을 정하는 것은 처리량이 아니라
/// **ssh 세션 예산**이다. 실측: 같은 control master 에 동시 8 이면 TCP 연결이 안 늘고(전부 다중화),
/// 동시 16 이면 **4 → 16 으로 늘었다**. 즉 `MaxSessions`(기본 10)를 넘겨도 **오류가 안 나고
/// 조용히 새 연결을 연다** — 그 연결마다 TCP + 인증이 붙고, 비밀번호 호스트면 **묻는 자식이
/// 붙박인다**(§2.2 ⑸ 가 판정에서 막은 그 함정을 실행 단계가 다시 여는 것이다).
///
/// 그 예산을 이미 쓰는 것들: pane 마다 터미널 1 · 호스트마다 에이전트 채널 1 · 감시자 1, 그리고
/// SCM 읽기·파일 읽기·변경이 그때그때. 같은 호스트에 pane 이 둘만 돼도 여유가 얇다.
