...
pub const session_host_coordinator = if (builtin.os.tag == .macos)
    ...
    struct {};
pub const reconnect_worker_owner = if (builtin.os.tag == .macos)
    @import("session_host/reconnect_worker_owner.zig")
else
    struct {};
pub const reconnect_worker_issuer = if (builtin.os.tag == .macos)
    @import("session_host/reconnect_worker_issuer.zig")
else
    struct {};
pub const reconnect_worker_runtime = if (builtin.os.tag == .macos)
...
test {
    ...
    // 같은 이유로 resolver 도 명시로 끈다 — `pub const` 는 refAllDecls 가 얕게만 보므로 그 안의 test 가
    // 안 돌았다. 죽은 host 줄이 상한을 넘겨 attach 가 통째로 막히던 것을 재는 test 가 여기 있다.
    if (builtin.os.tag == .macos) _ = @import("session_host/attach_resolver.zig");
    comptime {
