...
/// Strict CLI adoption intentionally stores `preflight = null`; c3b classifies its complete FIFO.
pub const BufferedEvent = struct {
    header: protocol.Header,
    payload: []u8,
    preflight: ?runtime_event_wire.Verdict = null,
    admission_seal: ?AdmissionSeal = null,
...
pub const ValidatedGenerationEventTake = struct {
    event_index: u32,
    payload: []const u8,
    preflight: runtime_event_wire.Verdict,
    payload_digest: runtime_event_wire.Digest,
...
pub const GenerationEventHeldError = error{ Corrupt, Terminal, InvalidPrepared };
...
pub const PreparedEndedPurgeCommit = struct {
    const Lifecycle = enum(u8) { pristine, prepared, finalization_pending, consumed };
    ...
    batch_plan: ended_purge_transaction.QueuePlan = .{},
    stream_plan: ended_purge_transaction.QueuePlan = .{},
    event_plan: ended_purge_transaction.QueuePlan = .{},
    partial_plan: ended_purge_transaction.QueuePlan = .{},
    batch_cleanup_ordinal: usize = 0,
    stream_cleanup_ordinal: usize = 0,
    event_cleanup_ordinal: usize = 0,
    partial_cleanup_ordinal: usize = 0,
    finalization_seal: owner_seal.Digest = [_]u8{0} ** 32,
...
fn externalFrameDescriptor(frame: anytype) ExternalFrameDescriptor {
    return .{
        .header = frame.header,
...
pub const RegisteredOperationExecutionMintReceipt = operation_thread_identity.MintReceipt;

pub const RegisteredOperationExecutionCapability = struct {
    self_addr: usize = 0,
    client_addr: usize = 0,
    ...
    publication_identity: std.atomic.Value(u64) = .init(0),

    fn digest(self: *const @This()) owner_seal.Digest {
        var writer = owner_seal.Writer.init("maru.registered-operation-execution-capability.v1");
        ...
        writer.writeU64(self.lease_identity);
        writer.writeU64(self.owner_process_id);
        writer.writeU64(self.owner_process_nonce);
        writer.writeU64(self.owner_thread_id);
        writer.writeU64(self.owner_thread_incarnation);
        writer.writeU64(self.operation_identity);
        writer.writeU8(@intFromBool(self.live));
        return writer.finish();
    }

    pub fn pristineExact(self: *const @This()) bool {
        return self.self_addr == 0 and self.client_addr == 0 and self.fence_generation == 0 and
            self.fence_incarnation == 0 and self.lease_identity == 0 and
            self.owner_process_id == 0 and self.owner_process_nonce == 0 and
            self.owner_thread_id == 0 and self.owner_thread_incarnation == 0 and
            self.operation_identity == 0 and !self.live and
            std.mem.allEqual(u8, &self.seal, 0) and
            self.publication_identity.load(.acquire) == 0;
    }

    fn resetPristine(self: *@This()) void {
        self.self_addr = 0;
        self.client_addr = 0;
        self.fence_generation = 0;
        self.fence_incarnation = 0;
        self.lease_identity = 0;
        self.owner_process_id = 0;
        self.owner_process_nonce = 0;
        self.owner_thread_id = 0;
        self.owner_thread_incarnation = 0;
        self.operation_identity = 0;
        self.live = false;
        self.seal = [_]u8{0} ** 32;
        self.publication_identity.store(0, .release);
    }
};

pub const RegisteredOperationExecutionHandle = operation_thread_identity.CapabilityHandle;

const PinnedExecutionCapabilityGuard = struct {
    capability: ?*const RegisteredOperationExecutionCapability = null,
    pin: operation_thread_identity.CapabilityPin = .{},

    fn deinit(self: *@This()) void {
        if (!self.pin.live) return;
        if (!operation_thread_identity.unpinCapability(&self.pin))
            @panic("execution capability registry unpin failed");
    }

    fn close(self: *@This()) bool {
        return operation_thread_identity.closeCapability(&self.pin);
    }
};

fn initRegisteredOperationExecutionCapabilityGuard(
    out: *PinnedExecutionCapabilityGuard,
    handle: RegisteredOperationExecutionHandle,
    expected_client_addr: usize,
) bool {
    out.* = .{};
    if (!operation_thread_identity.pinCapability(handle, &out.pin)) return false;
    out.capability = @ptrFromInt(out.pin.fields.capability_addr);
    var transfer_pin = false;
    defer if (!transfer_pin) out.deinit();
    const capability = out.capability.?;
    const published_identity = capability.publication_identity.load(.acquire);
    if (published_identity == 0 or published_identity != out.pin.fields.publication_identity) return false;
    if (builtin.is_test and execution_capability_read_test_hook.armed.swap(false, .acq_rel)) {
        execution_capability_read_test_hook.reached.store(true, .release);
        if (!ExecutionCapabilityPublicationTestHook.waitUntil(
            &execution_capability_read_test_hook.proceed,
        )) @panic("execution capability reader test hook timed out");
    }
    const current_pid = operation_thread_identity.currentProcessId();
    const owner_thread_id: u64 = @intCast(std.Thread.getCurrentId());
    if (!(capability.self_addr == @intFromPtr(capability) and capability.live and
        capability.client_addr == expected_client_addr and
        capability.client_addr == out.pin.fields.client_addr and
        capability.fence_generation == out.pin.fields.fence_generation and
        capability.fence_incarnation == out.pin.fields.fence_incarnation and
        current_pid != 0 and capability.owner_process_id == current_pid and
        capability.owner_process_id == out.pin.fields.owner_process_id and
        capability.owner_process_nonce == out.pin.fields.owner_process_nonce and
        capability.lease_identity == published_identity and
        capability.owner_thread_id == owner_thread_id and
        capability.owner_thread_id == out.pin.fields.owner_thread_id and
        capability.owner_thread_incarnation == out.pin.fields.owner_thread_incarnation and
        operation_thread_identity.matches(capability.owner_thread_incarnation) and
        capability.operation_identity == out.pin.fields.operation_identity and
        std.mem.eql(u8, &capability.seal, &capability.digest()))) return false;
    transfer_pin = true;
    return true;
}
...
pub const ClientOperationFence = struct {
    ...
    execution_capability_addr: std.atomic.Value(usize) = .init(0),
    execution_capability_identity: std.atomic.Value(u64) = .init(0),
    execution_owner_thread_id: std.atomic.Value(u64) = .init(0),
    execution_owner_thread_incarnation: std.atomic.Value(u64) = .init(0),
    next_execution_capability_identity: std.atomic.Value(u64) = .init(1),
    ...
    pub fn initInPlace(
        ...
        if (client_addr == 0 or owner_process_id == 0 or owner_process_nonce == 0 or slot_incarnation == 0 or
    ...
    fn issueExecutionCapabilityIdentity(self: *ClientOperationFence) ?u64 {
        var observed = self.next_execution_capability_identity.load(.acquire);
        while (true) {
            if (observed == 0 or observed == std.math.maxInt(u64)) return null;
            if (self.next_execution_capability_identity.cmpxchgWeak(
                observed,
                observed + 1,
                .acq_rel,
                .acquire,
    ...
    fn executionCapabilityMatches(
        self: *const ClientOperationFence,
        client_addr: usize,
        fence_generation: u64,
        capability: *const RegisteredOperationExecutionCapability,
    ) bool {
        if (generationAllocatorCallbackActive()) return false;
        const published_identity = capability.publication_identity.load(.acquire);
        if (published_identity == 0) return false;
        const thread_id: u64 = @intCast(std.Thread.getCurrentId());
        return capability.self_addr == @intFromPtr(capability) and capability.live and
            capability.client_addr == client_addr and
            capability.owner_process_id == self.owner_process_id and
            capability.owner_process_nonce == self.owner_process_nonce and
            capability.fence_generation == fence_generation and
            capability.fence_incarnation == self.fence_incarnation and
            capability.lease_identity != 0 and capability.owner_thread_id == thread_id and
            capability.owner_thread_incarnation != 0 and capability.operation_identity != 0 and
            operation_thread_identity.matches(capability.owner_thread_incarnation) and
            std.mem.eql(u8, &capability.seal, &capability.digest()) and
            self.executionLeaseHeld(client_addr, fence_generation) and
            self.execution_capability_addr.load(.acquire) == @intFromPtr(capability) and
            published_identity == capability.lease_identity and
            self.execution_capability_identity.load(.acquire) == published_identity and
            self.execution_owner_thread_id.load(.acquire) == thread_id and
            self.execution_owner_thread_incarnation.load(.acquire) ==
                capability.owner_thread_incarnation;
    }
...
pub const ExecutionCapabilityPublicationTestHook = struct {
    armed: std.atomic.Value(bool) = .init(false),
    reached: std.atomic.Value(bool) = .init(false),
    proceed: std.atomic.Value(bool) = .init(false),

    pub fn waitUntil(flag: *const std.atomic.Value(bool)) bool {
        const deadline = std.Io.Clock.awake.now(std.testing.io).nanoseconds +
            10 * std.time.ns_per_s;
        while (!flag.load(.acquire)) {
            if (std.Io.Clock.awake.now(std.testing.io).nanoseconds >= deadline) return false;
            std.Thread.yield() catch {};
        }
        return true;
    }
};
pub var execution_capability_publication_test_hook: if (builtin.is_test)
    ExecutionCapabilityPublicationTestHook
else
    void = if (builtin.is_test) .{} else {};
pub var execution_capability_read_test_hook: if (builtin.is_test)
    ExecutionCapabilityPublicationTestHook
else
    void = if (builtin.is_test) .{} else {};
...
test "CR3a B3b-F Client rejects rebind and same-address generation ABA" {
    ...
    fence.initInPlace(@intFromPtr(&client), pid, 5, 39, 40, 41);
...
test "CR3a B3b-F fork child rejects an inherited fence before atomic state access" {
    ...
    fence.initInPlace(@intFromPtr(&client_storage), pid, 5, 79, 83, 89);
...
pub const Client = struct {
    ...
    /// schema bit.
    attachment_capabilities: AttachmentCapabilities = .{},
    /// Runtime metadata schema support is immutable hello provenance, not observation
    /// availability. Frozen N-1 is always unsupported; a current peer is supported only when its
    /// ACK advertises the capability we requested.
    metadata_support: runtime_metadata_wire.MetadataSupport = .unsupported,
    /// Optional parse observability used by deterministic product-path gates. It does not receive
    /// payload bytes and cannot alter classification; normal clients leave it null.
    event_parse_observer: ?runtime_event_wire.ParseObserver = null,
    ...
    fn finishHello(
        ...
        self.attachment_capabilities.negotiated_controller_transfer =
            ...
            self.attachment_capabilities.peer_attach_generation;
        self.metadata_support = if (profile.kind == .current and
            payloadHasCapability(ack.payload, "runtime_metadata_v1"))
            .supported
        else
            .unsupported;
    ...
    fn callWithIo(
        ...
        const operation_fence_held = try self.requireBlockingMode();
        defer if (operation_fence_held) self.endPublicMutation();
        return self.callGuardedWithIo(method, params_json, io, false);
    }

    fn callGuardedWithIo(
        self: *Client,
        method: []const u8,
        params_json: ?[]const u8,
        io: RpcIo,
        ...
        try self.flushPendingOutboundBlockingMode(execution_lease_held);
        var prepared: PreparedBlockingRpc = .{};
        self.prepareBlockingRpcGuarded(&prepared, method, params_json) catch |err| return switch (err) {
    ...
    pub fn callUnderRegisteredOperationExecutionLease(
        self: *Client,
        ...
        return self.callGuardedWithIo(method, params_json, .blocking, true) catch |err| switch (err) {
            error.DeadlineExceeded => unreachable,
            else => |client_err| client_err,
        };
    }
    ...
    fn prepareBlockingRpc(
        ...
        const operation_fence_held = try self.requireBlockingMode();
        defer if (operation_fence_held) self.endPublicMutation();
        return self.prepareBlockingRpcGuarded(out, method, params_json);
    }

    fn prepareBlockingRpcGuarded(
        self: *Client,
        out: *PreparedBlockingRpc,
        method: []const u8,
        params_json: ?[]const u8,
    ...
    fn executePreparedBlockingRpcClassified(
        ...
        const response = self.executePreparedWithIo(prepared, .blocking, true, null, null, false) catch |err| {
    ...
    fn flushPendingOutboundForPreparedExecution(
        ...
        return self.flushPendingOutboundBlockingWithOps(lease, .posix_ops, true);
    }
        ...
        while (self.pending_outbound) |*pending| {
            ...
            switch (result) {
                ...
                .would_block, .zero, .failed => {
                    ...
                    if (lease != null or execution_lease_held)
                        self.poisonWhilePreparedExecutionHeld(.outbound_write_ambiguous)
                    else
    ...
    fn executePreparedWithIo(
        ...
        if (!prepared.validFor(self) or !prepared.frameIntact() or
            prepared.request_id != self.next_request_id)
            return error.InvalidPreparedRpc;
        if (flush_pending) try self.flushPendingOutboundBlockingMode(execution_lease_held);
            ...
            self.bufferEventWithAllocator(frame, allocator) catch |err| {
                if (execution_lease_held and self.first_poison_reason == null)
                    self.poisonMutationIo(switch (err) {
                        error.OutOfMemory => .local_resource_exhausted,
                        error.EventQueueFull => .event_queue_overflow,
    ...
    pub fn commitGenerationEventTake(
        ...
        defer if (operation_fence_held) self.endPublicMutation();
        if (checkedAllocatorReentry(self)) return error.Busy;
        const validated = try self.validateGenerationEventTakeCore(prepared, out);
        self.commitValidatedGenerationEventTakeCore(prepared, out, validated);
    }

    fn validateGenerationEventTakeCore(
        self: *Client,
        prepared: *PreparedGenerationEventTake,
        out: *?OwnedGenerationEvent,
    ) GenerationEventHeldError!ValidatedGenerationEventTake {
    ...
    fn commitValidatedGenerationEventTakeCore(
        self: *Client,
        prepared: *PreparedGenerationEventTake,
        out: *?OwnedGenerationEvent,
        validated: ValidatedGenerationEventTake,
    ) void {
        const index: usize = validated.event_index;
        const event = self.pending_events.items[index];
        if (@intFromPtr(event.payload.ptr) != @intFromPtr(validated.payload.ptr) or
            event.payload.len != validated.payload.len or
    ...
    pub fn validateGenerationEventTakeUnderRegisteredOperationExecutionLease(
        self: *Client,
        capability: RegisteredOperationExecutionHandle,
        prepared: *PreparedGenerationEventTake,
        out: *?OwnedGenerationEvent,
    ) GenerationEventHeldError!ValidatedGenerationEventTake {
        self.requireRegisteredOperationExecutionCapability(capability) catch
            return error.InvalidPrepared;
        return self.validateGenerationEventTakeCore(prepared, out);
    }

    pub fn commitGenerationEventTakeUnderRegisteredOperationExecutionLease(
        self: *Client,
        capability: RegisteredOperationExecutionHandle,
        prepared: *PreparedGenerationEventTake,
        out: *?OwnedGenerationEvent,
        validated: ValidatedGenerationEventTake,
    ) GenerationEventHeldError!void {
        const current = try self.validateGenerationEventTakeUnderRegisteredOperationExecutionLease(
            capability,
            prepared,
            out,
        );
        if (current.event_index != validated.event_index or
            @intFromPtr(current.payload.ptr) != @intFromPtr(validated.payload.ptr) or
            current.payload.len != validated.payload.len or
            !std.mem.eql(u8, &current.payload_digest, &validated.payload_digest))
            return error.Corrupt;
        self.commitValidatedGenerationEventTakeCore(prepared, out, current);
    }

    pub fn abortGenerationEventTakeDuringRegisteredOperation(
        self: *const Client,
        prepared: *PreparedGenerationEventTake,
    ) void {
        self.abortGenerationEventTake(prepared);
    }

    pub fn abortGenerationEventTakeUnderRegisteredOperationExecutionLease(
        self: *Client,
        capability: RegisteredOperationExecutionHandle,
        prepared: *PreparedGenerationEventTake,
    ) void {
        self.requireRegisteredOperationExecutionCapability(capability) catch
            @panic("generation event held abort requires exact execution capability");
    ...
    fn endedPurgeCompleteOwnerSeal(
        ...
        if (operation_generation == 0 or prepared.self_addr != 0 or
            ...
            !std.mem.allEqual(u8, &prepared.finalization_seal, 0) or
    ...
    fn publishEndedPurgeCompaction(
        self: *Client,
        frozen: *const EndedPurgeScratch,
        prepared: *const PreparedEndedPurgeCommit,
    ) void {
        const batch_plan = prepared.batch_plan.rawPlannedScalars() catch
            @panic("ended purge batch plan state drifted");
        var batch_survivor: usize = 0;
        ...
        var event_survivor: usize = 0;
        for (0..event_plan.source_count) |source| {
            if (frozen.event_targets.isSet(source)) continue;
            if (event_survivor != source)
                self.pending_events.items[event_survivor] = self.pending_events.items[source];
            event_survivor += 1;
        }
    ...
    fn cleanupEndedPurgeTargetDirect(
        ...
        ordinal.* += 1;
        if (len == 0) return;
        if (checked_allocator_owner_addr != 0)
            @panic("ended purge allocator callback latch was already held");
        checked_allocator_owner_addr = @intFromPtr(self);
        allocator.free(externalOwnedBytes(address, len));
        checked_allocator_owner_addr = 0;
    }

    fn endedPurgePostValidate(
        self: *const Client,
        operation_generation: u64,
        frozen: *const EndedPurgeScratch,
        inventory: *const PreparedEndedPurgeInventory,
        ...
        const build_id = self.build_id orelse &.{};
        const build_descriptor = ExternalArrayDescriptor{
            .address = externalOwnedSliceAddress(u8, build_id),
            .len = build_id.len,
            .capacity = build_id.len,
        };
        const lifecycle_descriptor = ExternalArrayDescriptor{
            .address = externalOwnedSliceAddress(u8, self.lifecycle),
            .len = self.lifecycle.len,
            .capacity = self.lifecycle.len,
        };
        const pending_descriptor: ?EndedPurgeScratch.PendingOutboundDescriptor =
            if (self.pending_outbound) |pending| .{
                .frame_address = externalOwnedSliceAddress(u8, pending.frame),
                .frame_len = pending.frame.len,
                .stream_id = pending.stream_id,
        ...
        if (!std.meta.eql(build_descriptor, frozen.build_id) or
            !std.meta.eql(lifecycle_descriptor, frozen.client_lifecycle) or
            !std.meta.eql(pending_descriptor, frozen.pending_outbound))
            return false;

        // Rebuild the complete scalar/owner transcript after callbacks. This comparison binds the
        // original allocator, parser, socket dev/inode/type and every retained complete-graph
        // descriptor without dereferencing a freed target payload.
        var current_complete: PreparedEndedPurgeCommit = .{};
        self.endedPurgeCompleteOwnerSeal(
            operation_generation,
            frozen,
            inventory,
            &current_complete,
        ...
        if (current_complete.captured_fd != prepared.captured_fd or
            ...
            !std.mem.eql(
                u8,
                &current_complete.complete_owner_seal,
                &prepared.complete_owner_seal,
            ))
            return false;

        var survivors = owner_seal.Writer.init("maru.ended-purge.pre-callback-survivor.v1");
        survivors.writeU64(operation_generation);
        survivors.writeBytes(build_id);
        survivors.writeBytes(self.lifecycle);
        survivors.writeUsize(frozen.parser_backing.address);
        survivors.writeUsize(frozen.parser_backing.len);
        survivors.writeUsize(frozen.parser_backing.capacity);
        survivors.writeBytes(self.parser.buf.items);
        if (self.pending_outbound) |pending| {
            survivors.writeBool(true);
            survivors.writeU64(pending.stream_id);
        ...
        if (!frozen.partial_target) if (self.partial_batch) |partial| {
            ...
            if (!std.meta.eql(current, descriptor)) return false;
            writeEndedPurgePartialSeal(&survivors, descriptor, partial.bytes.items);
        };
        const digest = survivors.finish();
        return std.mem.eql(u8, &digest, &prepared.pre_callback_survivor_seal);
    }

    fn tombstoneEndedPurgeOwnedGraph(self: *Client) void {
        self.fd = -1;
        self.build_id = null;
        self.lifecycle = &.{};
    ...
    fn endedPurgeFinalizationSeal(
        ...
        const fence = self.operation_fence orelse
            @panic("ended purge finalization seal lost its fence");
        var writer = owner_seal.Writer.init("maru.ended-purge.finalization.v1");
        writer.writeUsize(prepared.self_addr);
        writer.writeUsize(prepared.client_addr);
        writer.writeUsize(prepared.scratch_addr);
        writer.writeUsize(prepared.inventory_addr);
        writer.writeU64(prepared.target_stream);
        writer.writeU64(@bitCast(@as(i64, prepared.captured_fd)));
        writer.writeUsize(prepared.complete_owned_extent_bytes);
        writer.writeBytes(&prepared.complete_owner_seal);
        writer.writeBytes(&prepared.pre_callback_survivor_seal);
        inline for (.{
            prepared.batch_plan,
            prepared.stream_plan,
            prepared.event_plan,
            prepared.partial_plan,
        }) |plan| {
            writer.writeU8(plan.state);
            writer.writeUsize(plan.scalars.source_count);
            writer.writeUsize(plan.scalars.target_count);
            writer.writeUsize(plan.scalars.survivor_count);
            writer.writeUsize(plan.scalars.source_bytes);
            writer.writeUsize(plan.scalars.target_bytes);
            writer.writeUsize(plan.scalars.survivor_bytes);
        }
        inline for (.{
            prepared.batch_cleanup_ordinal,
            prepared.stream_cleanup_ordinal,
            prepared.event_cleanup_ordinal,
            prepared.partial_cleanup_ordinal,
        }) |ordinal| writer.writeUsize(ordinal);
        writer.writeU8(@intFromEnum(prepared.lifecycle));
        writer.writeU64(operation_generation);
        writer.writeUsize(fence.self_addr);
        writer.writeUsize(fence.client_addr);
        writer.writeU64(fence.owner_process_id);
        writer.writeU64(fence.slot_incarnation);
        writer.writeU64(fence.node_incarnation);
        writer.writeU64(fence.fence_generation);
        writer.writeU64(self.operation_fence_generation);
        return writer.finish();
    }

    fn endedPurgeRawFinalizerStateValid(
        prepared: *const PreparedEndedPurgeCommit,
    ) bool {
        const lifecycle_tag = @as(*const u8, @ptrCast(&prepared.lifecycle)).*;
        return lifecycle_tag == @intFromEnum(
            PreparedEndedPurgeCommit.Lifecycle.finalization_pending,
        ) and prepared.batch_plan.state == ended_purge_transaction.QueuePlan.planned_state and
            prepared.stream_plan.state == ended_purge_transaction.QueuePlan.planned_state and
            prepared.event_plan.state == ended_purge_transaction.QueuePlan.planned_state and
            prepared.partial_plan.state == ended_purge_transaction.QueuePlan.planned_state;
    }
    ...
    pub fn commitEndedPurgePrepared(
        ...
        const fence = self.operation_fence orelse @panic("ended purge commit lost its fence");
        if (operation_generation == 0 or
            prepared.lifecycle != .prepared or
            prepared.self_addr != @intFromPtr(prepared) or
            prepared.client_addr != @intFromPtr(self) or
            prepared.scratch_addr != @intFromPtr(scratch) or
            prepared.inventory_addr != @intFromPtr(inventory) or
            prepared.batch_cleanup_ordinal != 0 or
            prepared.stream_cleanup_ordinal != 0 or
            prepared.event_cleanup_ordinal != 0 or
            prepared.partial_cleanup_ordinal != 0 or
            scratch.lifecycle != .commit_frozen or
            !inventory.validPreparedAtFinalAddress() or
            inventory.client_addr != @intFromPtr(self) or
            inventory.scratch_addr != @intFromPtr(scratch) or
            fence.state.load(.acquire) != ClientOperationFence.exclusive_bit |
                ClientOperationFence.publication_bit)
            @panic("ended purge commit authority drifted before no-fail suffix");
        ...
        validateEndedPurgeOuterRanges(
            ...
            scratch,
            inventory,
            externalRangeOfValue(prepared),
        ) catch @panic("ended purge outer owner graph drifted before cleanup");
        _ = validateEndedPurgeOwnerRanges(
            self,
            scratch,
            inventory,
        ...
        for (self.pending_stream.items, 0..) |frame, index| {
            ...
            writeEndedPurgeFrameSeal(&stream_writer, descriptor, frame.payload);
        }
        for (self.pending_events.items, 0..) |event, index| {
            const descriptor = externalFrameDescriptor(event);
        ...
        if (self.partial_batch) |partial| {
            ...
            if (!std.meta.eql(frozen.partial, descriptor))
                @panic("ended purge partial descriptor drifted before cleanup");
            writeEndedPurgePartialSeal(&partial_writer, descriptor, partial.bytes.items);
        } else if (frozen.partial != null) {
            @panic("ended purge partial owner disappeared before cleanup");
        }
        const current_target_map_seal = endedPurgeTargetMapSeal(frozen);
        const current_batch_seal = batch_writer.finish();
        const current_stream_seal = stream_writer.finish();
        const current_event_seal = event_writer.finish();
        const current_partial_seal = partial_writer.finish();
        if (!std.mem.eql(u8, &current_batch_seal, &inventory.batch_seal) or
            !std.mem.eql(u8, &current_stream_seal, &inventory.stream_seal) or
            !std.mem.eql(u8, &current_event_seal, &inventory.event_seal) or
            !std.mem.eql(u8, &current_partial_seal, &inventory.partial_seal) or
            !std.mem.eql(u8, &current_target_map_seal, &inventory.target_map_seal))
            @panic("ended purge sealed queue graph drifted before cleanup");
        var current_complete: PreparedEndedPurgeCommit = .{};
        self.endedPurgeCompleteOwnerSeal(
            operation_generation,
            frozen,
        ...
        if (current_complete.captured_fd != prepared.captured_fd or
            ...
            @panic("ended purge complete owner seal mismatch before cleanup");
        const batch_plan = prepared.batch_plan.rawPlannedScalars() catch
            @panic("ended purge batch plan state drifted");
        const stream_plan = prepared.stream_plan.rawPlannedScalars() catch
            @panic("ended purge stream plan state drifted");
        const event_plan = prepared.event_plan.rawPlannedScalars() catch
            @panic("ended purge event plan state drifted");
        const partial_plan = prepared.partial_plan.rawPlannedScalars() catch
            @panic("ended purge partial plan state drifted");
        var batch_cursor = ended_purge_transaction.DispositionCursor(
            protocol.max_client_screen_items,
        ).init(batch_plan, &frozen.batch_targets) catch
            @panic("ended purge batch plan drifted");
        while (batch_cursor.next() catch @panic("ended purge batch cursor drifted")) |_| {}
        var stream_cursor = ended_purge_transaction.DispositionCursor(
            protocol.max_client_screen_items,
        ).init(stream_plan, &frozen.stream_targets) catch
            @panic("ended purge stream plan drifted");
        while (stream_cursor.next() catch @panic("ended purge stream cursor drifted")) |_| {}
        var event_cursor = ended_purge_transaction.DispositionCursor(
            max_pending_event_count,
        ).init(event_plan, &frozen.event_targets) catch
            @panic("ended purge event plan drifted");
        while (event_cursor.next() catch @panic("ended purge event cursor drifted")) |_| {}
        var partial_targets = std.StaticBitSet(1).initEmpty();
        if (frozen.partial_target) partial_targets.set(0);
        var partial_cursor = ended_purge_transaction.DispositionCursor(1).init(
            partial_plan,
            &partial_targets,
        ) catch @panic("ended purge partial plan drifted");
        while (partial_cursor.next() catch @panic("ended purge partial cursor drifted")) |_| {}

        scratch.lifecycle = .consumed;
        self.publishEndedPurgeCompaction(frozen, prepared);
        var expected_cleanup_ordinal: usize = 0;
        for (0..batch_plan.source_count) |source| if (frozen.batch_targets.isSet(source)) {
            if (prepared.batch_cleanup_ordinal != expected_cleanup_ordinal)
                @panic("ended purge batch cleanup ordinal drifted");
            const descriptor = frozen.batches[source];
            ...
            expected_cleanup_ordinal += 1;
        };
        if (expected_cleanup_ordinal != batch_plan.target_count)
            @panic("ended purge batch cleanup count drifted");
        expected_cleanup_ordinal = 0;
        for (0..stream_plan.source_count) |source| if (frozen.stream_targets.isSet(source)) {
            if (prepared.stream_cleanup_ordinal != expected_cleanup_ordinal)
                @panic("ended purge stream cleanup ordinal drifted");
            const descriptor = frozen.stream[source];
            self.cleanupEndedPurgeTargetDirect(
                cleanup_allocator,
                descriptor.payload_address,
        ...
        if (expected_cleanup_ordinal != stream_plan.target_count)
            @panic("ended purge stream cleanup count drifted");
        expected_cleanup_ordinal = 0;
        for (0..event_plan.source_count) |source| if (frozen.event_targets.isSet(source)) {
            if (prepared.event_cleanup_ordinal != expected_cleanup_ordinal)
        ...
        if (expected_cleanup_ordinal != event_plan.target_count)
            @panic("ended purge event cleanup count drifted");
        expected_cleanup_ordinal = 0;
        if (frozen.partial_target) {
            const descriptor = frozen.partial orelse
                @panic("ended purge partial target lost its descriptor");
            self.cleanupEndedPurgeTargetDirect(
                cleanup_allocator,
                descriptor.bytes.address,
                descriptor.bytes.capacity,
        ...
        if (expected_cleanup_ordinal != partial_plan.target_count)
            @panic("ended purge partial cleanup count drifted");

        if (self.endedPurgePostValidate(
            operation_generation,
            frozen,
            inventory,
            prepared,
        ...
        self.tombstoneEndedPurgeOwnedGraph();
        prepared.lifecycle = .finalization_pending;
        prepared.finalization_seal = self.endedPurgeFinalizationSeal(
            operation_generation,
            prepared,
        );
        return .drift_pending_finalize;
    }

    fn publishEndedPurgeNoFreePoison(self: *Client) void {
        self.ownership = .quarantined_no_free;
        self.markPoisonedForDeferredCleanup(.external_transfer_quarantined);
    }
    ...
    pub fn finalizeEndedPurgeNoFreePoison(
        ...
        if (actual_pid == 0 or quarantine_proof.process_id != actual_pid)
            @panic("ended purge finalizer process domain mismatch");
        if (!endedPurgeRawFinalizerStateValid(prepared))
            @panic("ended purge finalizer raw state mismatch");
        const fence = self.operation_fence orelse
            @panic("ended purge finalizer lost its fence");
        const current_finalization_seal = self.endedPurgeFinalizationSeal(
            operation_generation,
            prepared,
        );
        if (operation_generation == 0 or
            prepared.lifecycle != .finalization_pending or
            prepared.self_addr != @intFromPtr(prepared) or
            prepared.client_addr != @intFromPtr(self) or
            prepared.complete_owned_extent_bytes == 0 or
            std.mem.allEqual(u8, &prepared.finalization_seal, 0) or
            self.ownership != .standalone or self.first_poison_reason != null or
            fence.owner_process_id != actual_pid or
            !fence.identityMatches(@intFromPtr(self), self.operation_fence_generation) or
            fence.state.load(.acquire) != ClientOperationFence.exclusive_bit |
                ClientOperationFence.publication_bit or
            !quarantine_proof.matches(
                actual_pid,
                fence.node_incarnation,
                operation_generation,
                prepared.complete_owned_extent_bytes,
            ) or
            !std.mem.eql(
                u8,
                &prepared.finalization_seal,
                &current_finalization_seal,
            ))
            @panic("ended purge finalizer authority mismatch");

        self.publishEndedPurgeNoFreePoison();
        prepared.lifecycle = .consumed;
        if (!fence.commitExclusiveTerminal(
            @intFromPtr(self),
            self.operation_fence_generation,
        )) @panic("ended purge finalizer terminal publication failed");
    }
    ...
    fn bufferEventWithAllocator(
        ...
        const verdict = runtime_event_wire.preflightEventObserved(
            frame.payload,
            .{},
            self.event_parse_observer,
        );
        const accepted_event: ?runtime_event_wire.Event = switch (verdict) {
        ...
        const resized: ?resize_wire.Event = if (accepted_event) |event| switch (event) {
            .resized => |value| value,
            else => null,
        } else null;
        const runtime_ended = if (accepted_event) |event| event == .ended else false;
        if (accepted_event) |event| switch (event) {
            .revoked => |revoked| {
                // The header is already sufficient routing evidence at ingress. Queueing a
        ...
        for (self.pending_events.items, 0..) |old, i| {
            if (metadata_revision != null and
                old.header.stream_id == frame.header.stream_id)
            {
                const old_metadata: ?runtime_event_wire.MetadataView = if (old.preflight) |old_verdict| switch (old_verdict) {
                    .accepted => |old_accepted| switch (old_accepted.event) {
                ...
                if (old_metadata) |old_value| {
                    ...
                    if (metadata_revision.? == old_value.revision) {
                        const next_value = switch (accepted_event.?) {
                            .metadata => |metadata| metadata,
                            else => unreachable,
                        };
                        if (!runtime_event_wire.metadataSemanticEqlExact(
                            old.payload,
                            &old_value,
                            buffered.payload,
                            &next_value,
            ...
            if (resized != null and old.header.stream_id == frame.header.stream_id) {
                const old_resize: ?resize_wire.Event = if (old.preflight) |old_verdict| switch (old_verdict) {
                    .accepted => |old_accepted| switch (old_accepted.event) {
                        .resized => |value| value,
                        else => null,
                    },
    ...
    pub fn sendInput(self: *Client, stream_id: u64, bytes: []const u8) ClientError!void {
        const operation_fence_held = try self.requireBlockingMode();
        defer if (operation_fence_held) self.endPublicMutation();
        return self.sendInputGuarded(stream_id, bytes, false);
    }

    fn sendInputGuarded(self: *Client, stream_id: u64, bytes: []const u8, execution_lease_held: bool) ClientError!void {
        ...
        while (offset < bytes.len) {
            ...
            socket_server.writeAll(self.fd, frame_bytes) catch {
                ...
                self.poisonMutationIo(.outbound_write_ambiguous, execution_lease_held);
                return error.WriteFailed;
            };
            self.allocator.free(frame_bytes);
            offset = end;
    ...
    pub fn sendInputUnderRegisteredOperationExecutionLease(
        self: *Client,
        capability: RegisteredOperationExecutionHandle,
        stream_id: u64,
        bytes: []const u8,
        ...
        try self.requireRegisteredOperationExecutionCapability(capability);
        return self.sendInputGuarded(stream_id, bytes, true);
    }
    ...
    pub fn sendInputNonBlocking(self: *Client, stream_id: u64, bytes: []const u8) ClientError!usize {
        const operation_fence_held = try self.requireBlockingMode();
        defer if (operation_fence_held) self.endPublicMutation();
        return self.sendInputNonBlockingGuarded(stream_id, bytes, false);
    }

    fn sendInputNonBlockingGuarded(self: *Client, stream_id: u64, bytes: []const u8, execution_lease_held: bool) ClientError!usize {
        // 기존 frame을 먼저 밀어 FIFO를 지킨다. 여전히 막혔으면 새 payload는 caller가 계속 소유한다.
        if (!(try self.pumpPendingOutputGuarded(execution_lease_held))) return 0;
    ...
    pub fn sendInputNonBlockingUnderRegisteredOperationExecutionLease(
        self: *Client,
        capability: RegisteredOperationExecutionHandle,
        stream_id: u64,
        bytes: []const u8,
        ...
        try self.requireRegisteredOperationExecutionCapability(capability);
        return self.sendInputNonBlockingGuarded(stream_id, bytes, true);
    }
    ...
    pub fn sendScrollToBottomNonBlocking(self: *Client, stream_id: u64) ClientError!bool {
        const operation_fence_held = try self.requireBlockingMode();
        defer if (operation_fence_held) self.endPublicMutation();
        return self.sendScrollToBottomNonBlockingGuarded(stream_id, false);
    }

    fn sendScrollToBottomNonBlockingGuarded(self: *Client, stream_id: u64, execution_lease_held: bool) ClientError!bool {
    ...
    pub fn sendScrollToBottomNonBlockingUnderRegisteredOperationExecutionLease(
        self: *Client,
        capability: RegisteredOperationExecutionHandle,
        stream_id: u64,
    ) ClientError!bool {
        try self.requireRegisteredOperationExecutionCapability(capability);
        return self.sendScrollToBottomNonBlockingGuarded(stream_id, true);
    }
    ...
    pub fn sendResyncNonBlockingUnderRegisteredOperationExecutionLease(
        self: *Client,
        capability: RegisteredOperationExecutionHandle,
        stream_id: u64,
    ) ClientError!bool {
        try self.requireRegisteredOperationExecutionCapability(capability);
        return self.sendResyncNonBlockingGuarded(stream_id, true);
    }
    ...
    pub fn sendCoreCommandNonBlocking(self: *Client, stream_id: u64, payload: []const u8) ClientError!bool {
        const operation_fence_held = try self.requireBlockingMode();
        defer if (operation_fence_held) self.endPublicMutation();
        return self.sendCoreCommandNonBlockingGuarded(stream_id, payload, false);
    }

    fn sendCoreCommandNonBlockingGuarded(self: *Client, stream_id: u64, payload: []const u8, execution_lease_held: bool) ClientError!bool {
    ...
    pub fn sendCoreCommandNonBlockingUnderRegisteredOperationExecutionLease(
        self: *Client,
        capability: RegisteredOperationExecutionHandle,
        stream_id: u64,
        payload: []const u8,
    ...
    pub fn sendScrollToBottom(self: *Client, stream_id: u64) ClientError!void {
        const operation_fence_held = try self.requireBlockingMode();
        defer if (operation_fence_held) self.endPublicMutation();
        return self.sendScrollToBottomGuardedWithOps(stream_id, .posix_ops, false);
    }
    ...
    fn sendScrollToBottomWithOps(
        ...
        const operation_fence_held = try self.requireBlockingMode();
        defer if (operation_fence_held) self.endPublicMutation();
        return self.sendScrollToBottomGuardedWithOps(stream_id, ops, false);
    }
    ...
    fn sendScrollToBottomGuardedWithOps(
        ...
        try self.writeBlockingStreamFrameWithOps(frame, ops, execution_lease_held);
    }

    pub fn sendScrollToBottomUnderRegisteredOperationExecutionLease(
        self: *Client,
        capability: RegisteredOperationExecutionHandle,
        stream_id: u64,
    ) ClientError!void {
        try self.requireRegisteredOperationExecutionCapability(capability);
        return self.sendScrollToBottomGuardedWithOps(stream_id, .posix_ops, true);
    }
    ...
    pub fn sendCoreCommand(self: *Client, stream_id: u64, payload: []const u8) ClientError!void {
        const operation_fence_held = try self.requireBlockingMode();
        defer if (operation_fence_held) self.endPublicMutation();
        return self.sendCoreCommandGuardedWithOps(stream_id, payload, .posix_ops, false);
    }
    ...
    fn sendCoreCommandWithOps(
        ...
        const operation_fence_held = try self.requireBlockingMode();
        defer if (operation_fence_held) self.endPublicMutation();
        return self.sendCoreCommandGuardedWithOps(stream_id, payload, ops, false);
    }
    ...
    pub fn sendCoreCommandUnderRegisteredOperationExecutionLease(
        self: *Client,
        capability: RegisteredOperationExecutionHandle,
        stream_id: u64,
        payload: []const u8,
        ...
        try self.requireRegisteredOperationExecutionCapability(capability);
        return self.sendCoreCommandGuardedWithOps(stream_id, payload, .posix_ops, true);
    }
    ...
    fn flushPendingOutboundBlocking(self: *Client) ClientError!void {
        return self.flushPendingOutboundBlockingMode(false);
    }

    fn flushPendingOutboundBlockingMode(
        self: *Client,
        execution_lease_held: bool,
    ) ClientError!void {
        return self.flushPendingOutboundBlockingWithOps(null, .posix_ops, execution_lease_held);
    }
    ...
    pub fn pumpPendingOutput(self: *Client) ClientError!bool {
        const operation_fence_held = try self.requireBlockingMode();
        defer if (operation_fence_held) self.endPublicMutation();
        return self.pumpPendingOutputGuarded(false);
    }

    fn pumpPendingOutputGuarded(self: *Client, execution_lease_held: bool) ClientError!bool {
        while (self.pending_outbound) |*pending| {
            const remaining = pending.frame[pending.offset..];
            switch (try self.sendDontWait(remaining, execution_lease_held)) {
    ...
    pub fn pumpPendingOutputUnderRegisteredOperationExecutionLease(
        self: *Client,
        capability: RegisteredOperationExecutionHandle,
    ) ClientError!bool {
        try self.requireRegisteredOperationExecutionCapability(capability);
            ...
            switch (verdict) {
                .accepted => |accepted| {
                    switch (accepted.event) {
                        .revoked => |revoked| {
                            // Header/payload disagreement is itself authority-ambiguous and must
                            // close every mutation latch. Otherwise global sees every revoke while
                            // a stream-local query closes for either exact routing identity.
                            if (revoked.stream_id != frame.header.stream_id) return true;
                            if (stream_id) |expected| {
                                if (frame.header.stream_id == expected) return true;
                            } else return true;
                        },
                        else => {},
                    }
    ...
    fn sendDontWait(self: *Client, bytes: []const u8, execution_lease_held: bool) ClientError!NonblockingSend {
    ...
    fn requireRegisteredOperationExecutionCapability(
        self: *const Client,
        handle: RegisteredOperationExecutionHandle,
    ) ClientError!void {
        // TLS and caller-final token checks precede every mutable Client graph read. The fence
        // then proves that this exact address/thread/identity owns the active execution lease.
        if (generationAllocatorCallbackActive()) return error.AdminBusy;
        var guard: PinnedExecutionCapabilityGuard = .{};
        if (!initRegisteredOperationExecutionCapabilityGuard(
            &guard,
            handle,
            @intFromPtr(self),
        )) return error.ConnectionClosed;
        ...
        if (!fence.executionCapabilityMatches(
            @intFromPtr(self),
            self.operation_fence_generation,
            capability,
        )) return error.ConnectionClosed;
    ...
    pub fn beginRegisteredOperationExecutionLease(
        self: *Client,
        capability: *RegisteredOperationExecutionCapability,
        receipt: *RegisteredOperationExecutionMintReceipt,
    ) ClientError!RegisteredOperationExecutionHandle {
        const owner_thread_id: u64 = @intCast(std.Thread.getCurrentId());
        if (generationAllocatorCallbackActive() or !capability.pristineExact())
            return error.AdminBusy;
        const authorization = operation_thread_identity.consumeMintReceipt(
            receipt,
            @intFromPtr(self),
        ) orelse return error.AdminBusy;
        const fence = self.operation_fence orelse return error.ConnectionClosed;
        fence.upgradeSingleSharedToExecutionLease(
            @intFromPtr(self),
            self.operation_fence_generation,
        ...
        errdefer if (!fence.downgradeExecutionLeaseToSingleShared(
            @intFromPtr(self),
            self.operation_fence_generation,
        )) @panic("registered operation execution fence rollback failed");
        const lease_identity = fence.issueExecutionCapabilityIdentity() orelse
            return error.ConnectionClosed;
        if (owner_thread_id == 0 or fence.execution_capability_addr.load(.acquire) != 0)
            return error.ConnectionClosed;
        capability.self_addr = @intFromPtr(capability);
        capability.client_addr = @intFromPtr(self);
        capability.fence_generation = self.operation_fence_generation;
        capability.fence_incarnation = fence.fence_incarnation;
        capability.lease_identity = lease_identity;
        capability.owner_process_id = authorization.owner_process_id;
        capability.owner_process_nonce = authorization.owner_process_nonce;
        capability.owner_thread_id = owner_thread_id;
        capability.owner_thread_incarnation = authorization.owner_thread_incarnation;
        capability.operation_identity = authorization.operation_identity;
        capability.live = true;
        capability.seal = capability.digest();
        var capability_committed = false;
        errdefer if (!capability_committed) capability.resetPristine();
        fence.execution_capability_identity.store(lease_identity, .monotonic);
        fence.execution_owner_thread_id.store(owner_thread_id, .monotonic);
        fence.execution_owner_thread_incarnation.store(authorization.owner_thread_incarnation, .monotonic);
        fence.execution_capability_addr.store(@intFromPtr(capability), .monotonic);
        var publication_committed = false;
        errdefer if (!publication_committed) {
            capability.publication_identity.store(0, .release);
            fence.execution_owner_thread_incarnation.store(0, .release);
            fence.execution_owner_thread_id.store(0, .release);
            fence.execution_capability_identity.store(0, .release);
            fence.execution_capability_addr.store(0, .release);
        };
        if (builtin.is_test and
            execution_capability_publication_test_hook.armed.swap(false, .acq_rel))
        {
            execution_capability_publication_test_hook.reached.store(true, .release);
            if (!ExecutionCapabilityPublicationTestHook.waitUntil(
                &execution_capability_publication_test_hook.proceed,
            )) @panic("execution capability publication test hook timed out");
        }
        capability.publication_identity.store(lease_identity, .release);
        const handle = operation_thread_identity.publishCapability(.{
            .capability_addr = @intFromPtr(capability),
            .publication_identity = lease_identity,
            .operation_identity = authorization.operation_identity,
            .client_addr = @intFromPtr(self),
            .fence_generation = self.operation_fence_generation,
            .fence_incarnation = fence.fence_incarnation,
            .owner_process_id = authorization.owner_process_id,
            .owner_process_nonce = authorization.owner_process_nonce,
            .owner_thread_id = owner_thread_id,
            .owner_thread_incarnation = authorization.owner_thread_incarnation,
        }) catch return error.AdminBusy;
        publication_committed = true;
        capability_committed = true;
        return handle;
    }

    pub fn setNextExecutionCapabilityIdentityForTest(self: *Client, value: u64) void {
        if (!builtin.is_test) unreachable;
        const fence = self.operation_fence orelse @panic("missing operation fence");
        fence.next_execution_capability_identity.store(value, .release);
    }

    pub fn nextExecutionCapabilityIdentityForTest(self: *const Client) u64 {
        if (!builtin.is_test) unreachable;
        const fence = self.operation_fence orelse @panic("missing operation fence");
        return fence.next_execution_capability_identity.load(.acquire);
    }
    ...
    pub fn endRegisteredOperationExecutionLease(
        ...
        if (!initRegisteredOperationExecutionCapabilityGuard(
            ...
            handle,
            @intFromPtr(self),
        )) @panic("registered operation execution capability unpublished or foreign");
        defer guard.deinit();
        const capability = @constCast(guard.capability.?);
        const fence = self.operation_fence orelse
        ...
        if (!fence.executionCapabilityMatches(
            ...
            self.operation_fence_generation,
            capability,
        )) @panic("registered operation execution capability drifted");
        if (!guard.close())
            @panic("registered operation execution capability close failed");
        if (capability.publication_identity.cmpxchgStrong(
            capability.lease_identity,
            0,
            .acq_rel,
            .acquire,
        ) != null) @panic("registered operation execution capability consume failed");
        fence.execution_owner_thread_incarnation.store(0, .release);
        fence.execution_owner_thread_id.store(0, .release);
        fence.execution_capability_identity.store(0, .release);
        fence.execution_capability_addr.store(0, .release);
        // The registry has retired this publication and drained all pinned readers. The body may
        // now leave scope; publication_identity == 0 remains the local consumed marker.
    ...
    pub fn beginConfirmedGenerationPoisonExclusive(self: *Client) ClientError!void {
        const fence = self.operation_fence orelse return error.ConnectionClosed;
        return fence.upgradeSingleSharedToExecutionLease(
            @intFromPtr(self),
            self.operation_fence_generation,
    ...
    pub fn endConfirmedGenerationPoisonExclusive(self: *Client) void {
        const fence = self.operation_fence orelse
            @panic("confirmed poison execution fence disappeared");
        if (!fence.downgradeExecutionLeaseToSingleShared(
            @intFromPtr(self),
            self.operation_fence_generation,
        )) @panic("confirmed poison execution fence release failed");
    }
...
test "B3-3 caller-final execution lease blocks mutation and preserves clean zero progress" {
    ...
    var fence: ClientOperationFence = undefined;
    const pid: u32 = @intCast(c.getpid());
    fence.initInPlace(@intFromPtr(&client), pid, 5, 43, 47, 53);
...
test "B3-3 execution lease releases only its frozen fence across pointer ABA" {
    ...
    inline for (.{
        ...
        const pid: u32 = @intCast(c.getpid());
        canonical_fence.initInPlace(@intFromPtr(&client), pid, 5, 71, 73, 79);
        foreign_fence.initInPlace(@intFromPtr(&client), pid, 5, 83, 89, 79);
...
test "B3-3 injected pending partial then failure preserves prior ambiguity" {
    ...
    try std.testing.expectError(
        error.WriteFailed,
        client.flushPendingOutboundBlockingWithOps(&lease, probe.ops(), true),
    );
...
test "client metadata events coalesce by stream and preserve other streams" {
    ...
    const latest = (try client.takeEventForStream(7)) orelse return error.TestUnexpectedResult;
    defer latest.deinit(allocator);
    try std.testing.expectEqualStrings(rev2, latest.payload);
    const other = (try client.takeEventForStream(8)) orelse return error.TestUnexpectedResult;
    defer other.deinit(allocator);
    try std.testing.expectEqualStrings(future, other.payload);
    try std.testing.expectEqual(@as(usize, 0), client.pending_event_bytes);
}

test "client metadata coalescing rejects same-revision equivocation and stale preflight bytes" {
    const allocator = std.testing.allocator;
    const original =
        \\{"event":"runtime.metadata","metadata_revision":2,"metadata":{"cwd":"/one","window_title":"work","ssh_remote_dest":null,"semantic_state":0,"alt_active":false,"app_cursor_keys":false,"alternate_scroll":true,"observer_generation":1,"title_generation":1,"cols":80,"rows":24,"foreground_available":false,"foreground_pgid":null,"processes":[]}}
    ;
    const equivocation =
        \\{"event":"runtime.metadata","metadata_revision":2,"metadata":{"cwd":"/two","window_title":"work","ssh_remote_dest":null,"semantic_state":0,"alt_active":false,"app_cursor_keys":false,"alternate_scroll":true,"observer_generation":1,"title_generation":1,"cols":80,"rows":24,"foreground_available":false,"foreground_pgid":null,"processes":[]}}
    ;
    {
        var client = Client{
            .allocator = allocator,
...
test "client admitted event seal rejects parsed scalar and header mutation before routing" {
    const allocator = std.testing.allocator;
    const metadata =
        \\{"event":"runtime.metadata","metadata_revision":2,"metadata":{"cwd":"/one","window_title":"work","ssh_remote_dest":null,"semantic_state":0,"alt_active":false,"app_cursor_keys":false,"alternate_scroll":true,"observer_generation":1,"title_generation":1,"cols":80,"rows":24,"foreground_available":false,"foreground_pgid":null,"processes":[]}}
    ;
    ...
    }
    {
        ...
        try std.testing.expectError(error.ProtocolError, client.takeEventForStream(7));
        try std.testing.expect(client.unusable);
        try std.testing.expectEqual(@as(usize, 0), client.pending_events.items.len);
    }
}

test "client rejects cross-stream revoke before either authority latch can skip it" {
    const allocator = std.testing.allocator;
    var client = Client{
        .allocator = allocator,
        .fd = -1,
...
test "client admitted revoke header mutation closes global and both stream latches" {
    const allocator = std.testing.allocator;
    var client = Client{
        .allocator = allocator,
        .fd = -1,
...
test "CR3a-2c2b3b prepare commit freezes a final-address blocking owner graph" {
    ...
    var fence: ClientOperationFence = undefined;
    const pid: u32 = @intCast(c.getpid());
    fence.initInPlace(@intFromPtr(&client), pid, 5, 101, 103, 107);
    ...
    try std.testing.expectEqual(
        ended_purge_transaction.QueuePlan.planned_state,
        prepared.batch_plan.state,
    );
    try std.testing.expectEqual(
        ended_purge_transaction.QueuePlan.planned_state,
        prepared.stream_plan.state,
    );
    try std.testing.expectEqual(
        ended_purge_transaction.QueuePlan.planned_state,
        prepared.event_plan.state,
    );
    try std.testing.expectEqual(
        ended_purge_transaction.QueuePlan.planned_state,
        prepared.partial_plan.state,
    );
    try std.testing.expect(!client.endedPurgeFenceIntruded());
    try std.testing.expect(client.releaseEndedPurgeExclusiveClean());
}

test "CR3a-2c2b3b commit stably compacts every queue and consumes clean authority" {
    if (builtin.os.tag != .macos) return error.SkipZigTest;
    const allocator = std.testing.allocator;
    var fds: [2]c.fd_t = undefined;
    try std.testing.expectEqual(
    ...
    try client.pending_stream.append(allocator, .{
        .header = .{
            .kind = .delta_chunk,
            .stream_id = 10,
            .payload_len = @intCast(sibling_stream_payload.len),
            .flags = protocol.Flags.end_stream,
        },
    ...
    // Use a fully valid admitted sibling. A partial metadata object is dropped at ingress and an
    // unknown future event is intentionally rejected by the ended-purge inventory classifier, so
    // neither would prove stable compaction of a real retained owner.
    ...
    try client.prepareEndedPurgeInventory(9, hint, &scratch, &inventory);
    var fence: ClientOperationFence = undefined;
    fence.initInPlace(@intFromPtr(&client), @intCast(c.getpid()), 5, 139, 149, 151);
    try std.testing.expect(client.bindOperationFence(&fence, 151));
    try client.tryAcquireEndedPurgeExclusive();
    var prepared: PreparedEndedPurgeCommit = .{};
    try client.prepareEndedPurgeCommit(9, 157, &scratch, &inventory, &prepared);
    const event_plan = try prepared.event_plan.rawPlannedScalars();
    try std.testing.expectEqual(@as(usize, 2), event_plan.source_count);
    try std.testing.expectEqual(@as(usize, 1), event_plan.target_count);
    try std.testing.expectEqual(@as(usize, 1), event_plan.survivor_count);

    try std.testing.expectEqual(
        EndedPurgeClientCommitOutcome.clean,
        client.commitEndedPurgePrepared(157, &scratch, &inventory, &prepared),
    );
    try std.testing.expectEqual(EndedPurgeScratch.Lifecycle.consumed, scratch.lifecycle);
    try std.testing.expectEqual(PreparedEndedPurgeCommit.Lifecycle.consumed, prepared.lifecycle);
    try std.testing.expectEqual(@as(usize, 1), prepared.batch_cleanup_ordinal);
...
test "CR3a-2c2b3b allocator drift tombstones then consumed proof finalizes terminal last" {
    if (builtin.os.tag != .macos) return error.SkipZigTest;
    var probe = PreparedRpcAllocatorDriftProbe{
    ...
    var fence: ClientOperationFence = undefined;
    const pid: u32 = @intCast(c.getpid());
    fence.initInPlace(@intFromPtr(&client), pid, 5, 163, 167, 173);
    try std.testing.expect(client.bindOperationFence(&fence, 173));
    try client.tryAcquireEndedPurgeExclusive();
    var prepared: PreparedEndedPurgeCommit = .{};
    try client.prepareEndedPurgeCommit(9, 179, &scratch, &inventory, &prepared);
    const captured_fd = prepared.captured_fd;
    probe.armed = true;

    try std.testing.expectEqual(
        EndedPurgeClientCommitOutcome.drift_pending_finalize,
        client.commitEndedPurgePrepared(179, &scratch, &inventory, &prepared),
    );
    try std.testing.expect(probe.drifted);
    ...
    try std.testing.expectEqual(@as(usize, 0), client.pending_events.items.len);
    try std.testing.expect(client.first_poison_reason == null);
    try std.testing.expectEqual(ClientOwnership.standalone, client.ownership);
    try std.testing.expect(!std.mem.allEqual(u8, &prepared.finalization_seal, 0));

    const SealSensitivity = struct {
        fn expectChanged(
            seal_client: *const Client,
            operation_generation: u64,
            baseline: owner_seal.Digest,
            candidate: *const PreparedEndedPurgeCommit,
        ) !void {
            const changed = seal_client.endedPurgeFinalizationSeal(
                operation_generation,
                candidate,
            );
    ...
    const baseline_finalization_seal = client.endedPurgeFinalizationSeal(179, &prepared);
    try std.testing.expectEqualSlices(
        u8,
        &prepared.finalization_seal,
        &baseline_finalization_seal,
    );
    inline for (.{
        "self_addr",
        "client_addr",
        ...
        "target_stream",
        "captured_fd",
        "complete_owned_extent_bytes",
        "batch_cleanup_ordinal",
        "stream_cleanup_ordinal",
        "event_cleanup_ordinal",
        "partial_cleanup_ordinal",
    }) |field_name| {
        var candidate = prepared;
        @field(candidate, field_name) +%= 1;
        try SealSensitivity.expectChanged(&client, 179, baseline_finalization_seal, &candidate);
    }
    inline for (.{ "complete_owner_seal", "pre_callback_survivor_seal" }) |field_name| {
        var candidate = prepared;
        @field(candidate, field_name)[0] ^= 1;
        try SealSensitivity.expectChanged(&client, 179, baseline_finalization_seal, &candidate);
    }
    inline for (.{ "batch_plan", "stream_plan", "event_plan", "partial_plan" }) |plan_name| {
        var state_candidate = prepared;
        @field(state_candidate, plan_name).state ^= 1;
        try SealSensitivity.expectChanged(
            &client,
            179,
            baseline_finalization_seal,
            &state_candidate,
        ...
        inline for (.{
            ...
            var scalar_candidate = prepared;
            @field(@field(scalar_candidate, plan_name).scalars, scalar_name) +%= 1;
            try SealSensitivity.expectChanged(
                &client,
                179,
                baseline_finalization_seal,
    ...
    var lifecycle_candidate = prepared;
    lifecycle_candidate.lifecycle = .prepared;
    try SealSensitivity.expectChanged(
        &client,
        179,
        baseline_finalization_seal,
    ...
    const original_operation_fence_generation = client.operation_fence_generation;
    client.operation_fence_generation +%= 1;
    try SealSensitivity.expectChanged(&client, 179, baseline_finalization_seal, &prepared);
    client.operation_fence_generation = original_operation_fence_generation;
    inline for (.{
        "self_addr",
        "client_addr",
        "owner_process_id",
        ...
        @field(fence, field_name) +%= 1;
        try SealSensitivity.expectChanged(&client, 179, baseline_finalization_seal, &prepared);
        @field(fence, field_name) -%= 1;
    }
    const changed_operation_seal = client.endedPurgeFinalizationSeal(181, &prepared);
    try std.testing.expect(!std.mem.eql(
        u8,
        &baseline_finalization_seal,
        &changed_operation_seal,
    ));
    ...
    const FinalizerRejectRunner = struct {
        ...
        fn run(
            ...
            const rejected_child = c.fork();
            try std.testing.expect(rejected_child >= 0);
            if (rejected_child == 0) {
                _ = c.close(stderr_pipe[0]);
                if (c.dup2(stderr_pipe[1], 2) < 0) c._exit(126);
                _ = c.close(stderr_pipe[1]);
                const child_pid: u32 = @intCast(c.getpid());
                if (raw_lifecycle) |raw|
                    @as(*u8, @ptrCast(&rejected_prepared.lifecycle)).* = raw;
                rejected_client.finalizeEndedPurgeNoFreePoison(
                    operation_generation,
                    rejected_prepared,
                    .{
                        .reservation_generation = 1,
                        .process_id = if (raw_lifecycle == null) parent_pid else child_pid,
                        .node_incarnation = node_incarnation,
                        .operation_generation = operation_generation,
                        .bytes = rejected_prepared.complete_owned_extent_bytes,
                    },
                );
                c._exit(0);
            }
    ...
    // An inherited proof is rejected before any inherited Client/fence/tag state is touched.
    try FinalizerRejectRunner.run(
        &client,
        &prepared,
        179,
        pid,
        167,
        null,
        "ended purge finalizer process domain mismatch",
    );
    // Exercise the product preflight over the full raw byte domain without paying for 255 crash
    // subprocesses; a representative invalid byte below proves the panic branch itself.
    var raw_lifecycle: u16 = 0;
    while (raw_lifecycle <= std.math.maxInt(u8)) : (raw_lifecycle += 1) {
        const raw: u8 = @intCast(raw_lifecycle);
        var raw_candidate = prepared;
        @as(*u8, @ptrCast(&raw_candidate.lifecycle)).* = raw;
        try std.testing.expectEqual(
            raw == @intFromEnum(PreparedEndedPurgeCommit.Lifecycle.finalization_pending),
            Client.endedPurgeRawFinalizerStateValid(&raw_candidate),
        );
    }
    inline for (.{ "batch_plan", "stream_plan", "event_plan", "partial_plan" }) |plan_name| {
        var raw_state: u16 = 0;
        while (raw_state <= std.math.maxInt(u8)) : (raw_state += 1) {
            const raw: u8 = @intCast(raw_state);
            var raw_candidate = prepared;
            @field(raw_candidate, plan_name).state = raw;
            try std.testing.expectEqual(
                raw == ended_purge_transaction.QueuePlan.planned_state,
                Client.endedPurgeRawFinalizerStateValid(&raw_candidate),
            );
        }
    }
    try FinalizerRejectRunner.run(
        ...
        167,
        std.math.maxInt(u8),
        "ended purge finalizer raw state mismatch",
    );
    try std.testing.expectEqual(PreparedEndedPurgeCommit.Lifecycle.finalization_pending, prepared.lifecycle);
    try std.testing.expectEqual(ClientOwnership.standalone, client.ownership);
    try std.testing.expect(client.first_poison_reason == null);
    ...
    if (child == 0) {
        ...
        const child_pid: u32 = @intCast(c.getpid());
        fence.owner_process_id = child_pid;
        prepared.finalization_seal = client.endedPurgeFinalizationSeal(179, &prepared);
        prepared.target_stream +%= 1;
        client.finalizeEndedPurgeNoFreePoison(179, &prepared, .{
            .reservation_generation = 1,
            .process_id = child_pid,
            .node_incarnation = 167,
            .operation_generation = 179,
            .bytes = prepared.complete_owned_extent_bytes,
        });
        c._exit(0);
    }
    _ = c.close(panic_pipe[1]);
    ...
    try std.testing.expectEqual(PreparedEndedPurgeCommit.Lifecycle.consumed, prepared.lifecycle);
    try std.testing.expectEqual(ClientOwnership.quarantined_no_free, client.ownership);
    try std.testing.expectEqual(
    ...
    try std.testing.expect(client.unusable);
    try std.testing.expectEqual(
        ClientOperationFence.exclusive_bit |
            ClientOperationFence.publication_bit |
            ClientOperationFence.terminal_bit,
        fence.state.load(.acquire),
    );
    _ = c.close(captured_fd);
}
...
test "CR3a-2c2b3b prepare commit requires exclusive and preserves destinations" {
    ...
    try client.prepareEndedPurgeInventory(9, hint, &scratch, &inventory);
    var fence: ClientOperationFence = undefined;
    fence.initInPlace(@intFromPtr(&client), @intCast(c.getpid()), 5, 113, 127, 131);
...
test "CR3a-2c2b3b complete owner extent accepts exact cap and rejects cap plus one" {
    ...
    const Case = struct {
        fn run(extra_bytes: usize, expect_success: bool) !void {
            ...
            try client.prepareEndedPurgeInventory(9, hint, &scratch, &inventory);
            var fence: ClientOperationFence = undefined;
            fence.initInPlace(@intFromPtr(&client), @intCast(c.getpid()), 5, 277, 281, 283);
...
test "CR3a-2c2b3b prepare commit rejects occupied forged and aliased evidence" {
    ...
    try client.prepareEndedPurgeInventory(9, hint, &scratch, &inventory);
    var fence: ClientOperationFence = undefined;
    fence.initInPlace(@intFromPtr(&client), @intCast(c.getpid()), 5, 163, 167, 173);
    try std.testing.expect(client.bindOperationFence(&fence, 173));
    try client.tryAcquireEndedPurgeExclusive();
...
test "CR3a-2c2b3b allocator callback reentry rejects prepare before graph traversal" {
    ...
    try client.prepareEndedPurgeInventory(9, hint, &scratch, &inventory);
    var fence: ClientOperationFence = undefined;
    fence.initInPlace(@intFromPtr(&client), @intCast(c.getpid()), 5, 229, 233, 239);
...
test "CR3a-2c2b3b fence intrusion prevents prepare authority publication" {
    ...
    try client.prepareEndedPurgeInventory(9, hint, &scratch, &inventory);
    var fence: ClientOperationFence = undefined;
    fence.initInPlace(@intFromPtr(&client), @intCast(c.getpid()), 5, 251, 257, 263);
...
test "CR3a-2c2b3b fork child rejects prepare commit before inherited state mutation" {
    ...
    try client.prepareEndedPurgeInventory(9, hint, &scratch, &inventory);
    var fence: ClientOperationFence = undefined;
    fence.initInPlace(@intFromPtr(&client), @intCast(c.getpid()), 5, 199, 211, 223);
...
const strict_cli_inherited_event_payloads = [_][]const u8{
    ...
    "{\"event\":\"runtime.metadata\",\"metadata_revision\":1,\"metadata\":{}}",
    "{\"event\":\"runtime.metadata\",\"metadata_revision\":2,\"metadata\":\"malformed\"}",
    "{\"event\":\"runtime.resized\",\"data\":{\"runtime_id\":\"a\"}}",
    "{\"event\":\"runtime.resized\",\"data\":{\"runtime_id\":\"b\"}}",
    "{\"event\":\"future.required\"}",
    "{\"event\":\"runtime.ended\"}",
};

fn strictCliAttachPeer(fd: c.fd_t, ok: *bool) void {
    defer _ = c.close(fd);
    const allocator = std.heap.page_allocator;
    var raw_header: [protocol.header_size]u8 = undefined;
    readExactFd(fd, &raw_header) catch return;
    const request = protocol.Header.decode(&raw_header) catch return;
    if (request.kind != .request or request.request_id != 1 or
        request.payload_len > protocol.max_control_json)
        return;
    const request_payload = allocator.alloc(u8, request.payload_len) catch return;
    defer allocator.free(request_payload);
    readExactFd(fd, request_payload) catch return;
    if (std.mem.indexOf(u8, request_payload, "\"method\":\"runtime.attach\"") == null)
        return;

    for (strict_cli_inherited_event_payloads) |payload| {
...
test "client cli attach socket demux preserves inherited events in exact raw FIFO adoption inventory" {
    if (builtin.os.tag != .macos) return error.SkipZigTest;
    const allocator = std.testing.allocator;
    var fds: [2]c.fd_t = undefined;
    try std.testing.expectEqual(
    ...
    var peer_ok = false;
    const peer = try std.Thread.spawn(.{}, strictCliAttachPeer, .{ fds[1], &peer_ok });
    var client = makeConnectedTestClient(allocator, fds[0]);
    defer client.deinit();
    ...
    peer.join();
    try std.testing.expect(peer_ok);
    try std.testing.expectEqualStrings("{\"result\":{\"stream_id\":7}}", response);

    var expected_bytes: usize = 0;
    for (strict_cli_inherited_event_payloads) |payload| {
        expected_bytes += payload.len;
    }
    try std.testing.expectEqual(strict_cli_inherited_event_payloads.len, client.pending_events.items.len);
    try std.testing.expectEqual(expected_bytes, client.pending_event_bytes);
    for (strict_cli_inherited_event_payloads, client.pending_events.items) |expected, actual| {
        try std.testing.expectEqual(protocol.version_major, actual.header.major);
        try std.testing.expectEqual(@as(u64, 7), actual.header.stream_id);
        try std.testing.expectEqual(@as(u32, @intCast(expected.len)), actual.header.payload_len);
        try std.testing.expectEqualStrings(expected, actual.payload);
    }
    ...
    try std.testing.expectEqual(strict_cli_inherited_event_payloads.len, inventory.event_count);
    try std.testing.expectEqual(expected_bytes, inventory.event_payload_bytes);
    try std.testing.expectEqual(client.pending_events.items.len, inventory.event_descriptors.len);
    for (client.pending_events.items, inventory.event_descriptors) |frame, descriptor| {
        try std.testing.expect(std.meta.eql(frame.header, descriptor.header));
        try std.testing.expectEqual(@intFromPtr(frame.payload.ptr), descriptor.payload_address);
        try std.testing.expectEqual(frame.payload.len, descriptor.payload_len);
    }
}

test "client cli attach raw FIFO fails closed at exact count and byte caps" {
    const allocator = std.testing.allocator;
    var count_client = Client{
        .allocator = allocator,
        .fd = -1,
        .host_id = 0,
        .connection_profile = .cli_attach,
    ...
    defer count_client.parser.deinit();
    defer {
        for (count_client.pending_events.items) |frame| frame.deinit(allocator);
    ...
    try std.testing.expectError(error.EventQueueFull, count_client.bufferLegacyEventForTest(.{
        .header = .{
            .major = protocol.version_major,
            .kind = .event,
            .stream_id = max_pending_event_count + 1,
            .payload_len = 0,
        },
        .payload = try allocator.dupe(u8, ""),
    }));
    try std.testing.expect(count_client.unusable);
    try std.testing.expectEqual(max_pending_event_count, count_client.pending_events.items.len);

    var bytes_client = Client{
        .allocator = allocator,
        .fd = -1,
        .host_id = 0,
        .connection_profile = .cli_attach,
    ...
    defer bytes_client.parser.deinit();
    defer {
        for (bytes_client.pending_events.items) |frame| frame.deinit(allocator);
        bytes_client.pending_events.deinit(allocator);
    }
    const frame_count = protocol.max_client_queue / protocol.max_control_json;
    try std.testing.expectEqual(@as(usize, 0), protocol.max_client_queue % protocol.max_control_json);
    for (0..frame_count) |index| {
        ...
        try bytes_client.bufferLegacyEventForTest(.{
            .header = .{
                ...
                .kind = .event,
                .stream_id = @intCast(index + 1),
                .payload_len = protocol.max_control_json,
            },
            .payload = payload,
        });
    }
    ...
    try std.testing.expectError(error.EventQueueFull, bytes_client.bufferLegacyEventForTest(.{
        .header = .{
            .major = protocol.version_major,
            .kind = .event,
            .stream_id = @intCast(frame_count + 1),
            .payload_len = 1,
        },
        .payload = try allocator.dupe(u8, "x"),
    }));
    try std.testing.expect(bytes_client.unusable);
    try std.testing.expectEqual(protocol.max_client_queue, bytes_client.pending_event_bytes);
}

test "client gui rejects unnegotiated metadata without publishing or coalescing" {
    const allocator = std.testing.allocator;
    var client = Client{
        .allocator = allocator,
        .fd = -1,
...
test "client same-generation resize equivocation preserves the admitted full state and closes" {
    const allocator = std.testing.allocator;
    var client = Client{
        .allocator = allocator,
        .fd = -1,
    ...
    try std.testing.expect(client.unusable);
    try std.testing.expectEqual(@as(usize, 1), client.pending_events.items.len);
    try std.testing.expectEqualStrings(admitted, client.pending_events.items[0].payload);
}
...
test "client event queue overflow poisons every runtime sharing the connection" {
    ...
    try std.testing.expect((try client.takeEventForStream(1)) == null);
...
test "client ended event replaces same-stream metadata at exact event cap" {
    ...
    try std.testing.expect(!client.unusable);
    try std.testing.expectEqual(@as(usize, 256), client.pending_events.items.len);
    const ended = (try client.takeEventForStream(1)) orelse return error.TestUnexpectedResult;
    defer ended.deinit(allocator);
    try std.testing.expectEqualStrings("{\"event\":\"runtime.ended\"}", ended.payload);
    const sibling = (try client.takeEventForStream(256)) orelse return error.TestUnexpectedResult;
    defer sibling.deinit(allocator);
    try std.testing.expectEqual(
        @as(?u64, 1),
        try observation_wire.eventRevision(allocator, sibling.payload, .supported),
    );
}
...
test "external adoption inventory seals exact client state and disarms owned queues" {
    ...
    const event_count_before_profile_flip = client.pending_events.items.len;
    const event_bytes_before_profile_flip = client.pending_event_bytes;
    client.metadata_support = .supported;
    try std.testing.expect(!client.validateExternalAdoptionPlan(&plan));
    try std.testing.expectError(error.StaleClient, client.sealExternalAdoption(&plan));
    try std.testing.expectEqual(event_count_before_profile_flip, client.pending_events.items.len);
    try std.testing.expectEqual(event_bytes_before_profile_flip, client.pending_event_bytes);
    try std.testing.expectEqual(ClientOwnership.external_pump, client.ownership);
    client.metadata_support = .unsupported;
    try std.testing.expect(client.validateExternalAdoptionPlan(&plan));
    var copied = plan;
...
fn checkFinishHelloAllocation(allocator: std.mem.Allocator) !void {
    ...
    try std.testing.expectEqual(runtime_metadata_wire.MetadataSupport.unsupported, client.metadata_support);
    client.deinit();
}

test "finish hello seals current metadata capability separately from availability" {
    const payload =
        \\{"version":2,"host_id":"00000000000000000000000000000001","build_id":"build","lifecycle":"ready","upgrade_epoch":1,"screen_codec_version":2,"capabilities":["host_manifest_v1","runtime_metadata_v1"]}
    ;
    var client = Client{
        .allocator = std.testing.allocator,
        .fd = -1,
    ...
    try client.finishHello(
        .cli_attach,
        compatibility.profileForMajor(protocol.version_major).?,
        ack,
    );
    try std.testing.expectEqual(runtime_metadata_wire.MetadataSupport.supported, client.metadata_support);
    client.deinit();
}

test "finish hello keeps frozen N-1 metadata unsupported despite peer advertisement" {
    const payload =
        \\{"version":1,"host_id":"00000000000000000000000000000001","capabilities":["screen_stream_v1_current_body","runtime_metadata_v1"]}
    ;
    var client = Client{
        .allocator = std.testing.allocator,
        .fd = -1,
        .host_id = 0,
        .wire_major = 1,
        .parser = framing.FrameParser.initForMajor(std.testing.allocator, 1),
    };
    const ack: framing.Frame = .{
        .header = .{ .major = 1, .kind = .hello_ack, .payload_len = payload.len },
        .payload = @constCast(payload),
    };
    try client.finishHello(
        .cli_attach,
        compatibility.profileForMajor(1).?,
        ack,
    );
    try std.testing.expectEqual(runtime_metadata_wire.MetadataSupport.unsupported, client.metadata_support);
    client.deinit();
...
test "client: sibling attachment converges after explicit terminate and natural exit" {
    ...
    defer allocator.free(still_live);
    try testing.expect(std.mem.indexOf(u8, still_live, "\"runtime_count\":0") != null);
    const ended_event = (try sibling.takeEventForStream(sibling_stream)) orelse
        return error.TestUnexpectedResult;
    ...
    const first_ended = (try first.takeEventForStream(first_stream)) orelse
        return error.TestUnexpectedResult;
    ...
    const natural_ended = (try sibling.takeEventForStream(natural_stream)) orelse
        return error.TestUnexpectedResult;
