...
pub fn HostPool(comptime Adapter: type) type {
    return struct {
        ...
        /// 없고 따라서 실패 지점도 없다. `runtime_refs`와 `spawn_host_id`도 그대로 유지된다 — 교체는
        /// 참조 관계를 바꾸지 않는다.
        ///
        /// 돌려받은 옛 adapter는 **재부착을 모두 끝낸 뒤** `destroyReplaced`로 해제한다. 먼저 해제하면
        /// 재부착 대상이 참조하던 `Client` 주소가 dangling된다.
...
test "host pool: adapter 원자적 교체가 lease와 spawn host를 보존한다" {
    // 연결이 무효화된 host를 다시 붙일 때 `remove` + `addOwned`로 쪼개면, 그 사이 addOwned 실패가 host 슬롯을
    // 통째로 잃게 하고 그 host의 runtime들은 이미 해제된 Client를 가리킨 채 남는다(use-after-free).
    // 교체는 참조 관계를 바꾸지 않으므로 lease와 spawn host가 그대로 살아 있어야 한다.
    ...
    // lease가 살아 있어야 재부착 후 release 균형이 맞는다. spawn host가 비면 새 pane spawn이 막힌다.
