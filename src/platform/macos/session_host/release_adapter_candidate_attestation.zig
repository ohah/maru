...
fn validateInputs(context: context_mod.Context, paths: Paths, executable: []const u8, token: []const u8, output: []u8, result: *CandidateAttestation) !void {
    ...
    for ([_][]const u8{ result_bytes, context.repository.owner, context.repository.name, context.tag, context.source_commit, context.build.workflow_ref, paths.dmg, paths.frozen_executable, executable, token }) |authority_bytes| {
