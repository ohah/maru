...
pub const Role = enum(u1) { evidence, manifest };
...
pub const Value = struct { directory: []const u8, entries: [role_count]Entry };

pub const DurablePreparation = struct {
    owner: ?*DurablePreparation = null,
    ...
    pub fn value(self: *const @This()) ?Value {
        if (self.owner != self or self.phase != .open or !validStorage(self) or
    ...
    fn revalidateDirectory(self: *const @This()) Error!void {
        ...
        if (c.fstat(self.directory_fd, &held) != 0 or
            ...
            !validOwnedDirectory(held) or !validOwnedDirectory(named) or
            @as(u64, @intCast(held.dev)) != self.directory_device or
            @as(u64, @intCast(held.ino)) != self.directory_inode or
            held.dev != named.dev or held.ino != named.ino)
    ...
    fn cleanupCore(self: *@This(), fail_after_unlinks: ?usize) TestError!void {
        ...
        const leaf: [:0]const u8 = self.directory_leaf[0..self.directory_leaf_len :0];
        if (!sameDirectoryAt(self.parent_fd, leaf, self.directory_fd) or
            c.unlinkat(self.parent_fd, leaf.ptr, posix.AT.REMOVEDIR) != 0 or c.fsync(self.parent_fd) != 0)
            return error.CleanupFailed;
...
pub fn promote(allocator: std.mem.Allocator, sources: Sources, destination: [:0]const u8, result: *DurablePreparation) Error!void {
...
pub fn promoteFailingForTest(allocator: std.mem.Allocator, sources: Sources, destination: [:0]const u8, result: *DurablePreparation, checkpoint: TestCheckpoint) TestError!void {
...
const Working = struct {
    ...
    fn cleanupChecked(self: *@This()) Error!void {
        ...
        if (!self.renamed) {
            ...
            if (!sameDirectoryAt(self.parent_fd, leaf, self.directory_fd) or
                c.unlinkat(self.parent_fd, leaf.ptr, posix.AT.REMOVEDIR) != 0 or c.fsync(self.parent_fd) != 0) failed = true;
        }
...
fn promoteCore(allocator: std.mem.Allocator, sources: Sources, destination: [:0]const u8, result: *DurablePreparation, injector: anytype) TestError!void {
    try validateInputs(sources, destination, result);
    const observations = try sourceObservations(sources);
    if (observations[0].size > evidence_mod.max_evidence_bytes or observations[1].size > manifest_mod.max_manifest_bytes)
    ...
    if (c.fstat(directory_fd, &directory_stat) != 0 or !validOwnedDirectory(directory_stat)) {
    ...
    parent_owned = false;
    completePromotion(allocator, sources, observations, semantic, destination, parent.leaf, result, injector, &working) catch |err| {
...
fn completePromotion(allocator: std.mem.Allocator, sources: Sources, observations: [role_count]files.ExecutableObservation, semantic: Semantic, destination: [:0]const u8, final_leaf: [:0]const u8, result: *DurablePreparation, injector: anytype, working: *Working) TestError!void {
    ...
    for (0..role_count) |index| {
        ...
        var input = source.file.readHeldAlloc(allocator, source.path, caps[index]) catch |err| switch (err) {
            error.OutOfMemory => return error.OutOfMemory,
            error.TooLarge => return error.TooLarge,
            else => return error.SourceChanged,
        };
        defer input.deinit(allocator);
        if (!sameObservation(observations[index], observation(input))) return error.SourceChanged;
        try writeLeaf(working.directory_fd, working.names[index][0..name.len :0], input.bytes);
        working.present[index] = true;
        try injector.hit(if (index == 0) .evidence_written else .manifest_written);
        try files.pinReleaseFileObserved(&working.files[index], path, false, caps[index]);
        const copied = working.files[index].value().?;
        if (sameIdentity(observations[index].identity, copied.identity) or observations[index].size != copied.size or
            !std.mem.eql(u8, &observations[index].sha256, &copied.sha256)) return error.FileChanged;
        try injector.hit(if (index == 0) .evidence_copied else .manifest_copied);
    }
    var identities: [role_count]files.Identity = undefined;
    for (&working.files, 0..) |*file, index| identities[index] = file.value().?.identity;
    try files.requireDistinct(&identities);
    try exactInventory(working.directory_fd, .{ role_names[0], role_names[1] });
    ...
    const final_sources = try sourceObservations(sources);
    for (final_sources, observations) |current, initial| if (!sameObservation(current, initial)) return error.SourceChanged;
    try validateFinalResult(result);
    result.phase = .open;
    result.seal = metadataSeal(result);
}

fn adoptAfterRename(working: *Working, destination: [:0]const u8, final_leaf: [:0]const u8, result: *DurablePreparation) void {
    var adopted: DurablePreparation = .{
    ...
    result.* = adopted;
    result.owner = result;
    result.phase = .cleanup_required;
    for (&result.files) |*file| file.owner = file;
    result.seal = metadataSeal(result);
    working.adopted = true;
}

fn validateInputs(sources: Sources, destination: [:0]const u8, result: *const DurablePreparation) Error!void {
    if (!pristine(result)) return error.InvalidOwner;
    if (!canonicalAbsolute(destination)) return error.InvalidPath;
    const destination_parent = std.fs.path.dirname(destination) orelse return error.InvalidPath;
    if (destination_parent.len + 1 + ".maru-preparation-ffffffffffffffff".len >= std.fs.max_path_bytes) return error.InvalidPath;
    const result_bytes = std.mem.asBytes(result);
    var identities: [role_count]files.Identity = undefined;
    for (0..role_count) |index| {
        const source = sources.at(@enumFromInt(index));
        const observed = source.file.value() orelse return error.InvalidOwner;
        identities[index] = observed.identity;
        if (!canonicalAbsolute(source.root) or !canonicalAbsolute(source.path) or !directChild(source.root, source.path) or
            sameOrDescendant(source.root, destination)) return error.InvalidPath;
        if (overlaps(result_bytes, std.mem.asBytes(source.file)) or overlaps(result_bytes, source.root) or
            overlaps(result_bytes, source.path) or overlaps(result_bytes, destination)) return error.InvalidOwner;
        for (0..index) |prior| {
...
fn validateSemantic(allocator: std.mem.Allocator, sources: Sources, observations: [role_count]files.ExecutableObservation) Error!Semantic {
    ...
    if (!sameObservation(observations[0], observation(evidence_input)) or !sameObservation(observations[1], observation(manifest_input)))
    ...
    for (authored.assets) |asset| switch (asset.role) {
        .universal_dmg => {
            if (dmg_asset != null) return error.InvalidBinding;
            dmg_asset = asset;
        },
        .frozen_product_executable => {
            if (frozen_asset != null) return error.InvalidBinding;
            frozen_asset = asset;
        },
        .evidence_summary => {
            if (evidence_asset != null) return error.InvalidBinding;
...
fn validateFinalResult(result: *const DurablePreparation) Error!void {
    if (result.owner != result or result.phase != .cleanup_required or !validStorage(result) or
        !std.mem.eql(u8, &result.seal, &metadataSeal(result))) return error.InvalidOwner;
    try result.revalidateDirectory();
    try exactInventory(result.directory_fd, .{
        result.names[0][0..result.name_lens[0]],
        result.names[1][0..result.name_lens[1]],
    });
    var identities: [role_count]files.Identity = undefined;
    for (0..role_count) |index| {
        const initial = result.files[index].value() orelse return error.InvalidOwner;
        const path: [:0]const u8 = result.paths[index][0..result.path_lens[index] :0];
        const current = result.files[index].revalidate(path) catch return error.FileChanged;
        if (!sameObservation(initial, current)) return error.FileChanged;
        identities[index] = current.identity;
    }
    try files.requireDistinct(&identities);
}

fn exactInventory(directory_fd: c.fd_t, expected: [role_count][]const u8) Error!void {
    const scan_fd = c.openat(directory_fd, ".", posix.O{ .ACCMODE = .RDONLY, .DIRECTORY = true, .CLOEXEC = true });
    if (scan_fd < 0) return error.FileChanged;
    const directory = c.fdopendir(scan_fd) orelse {
        _ = c.close(scan_fd);
        return error.FileChanged;
    };
    defer _ = c.closedir(directory);
    var found: [role_count]bool = @splat(false);
    var count: usize = 0;
    c._errno().* = 0;
    while (c.readdir(directory)) |entry| {
        ...
        count += 1;
        if (count > role_count) return error.FileChanged;
        var matched = false;
        for (expected, 0..) |wanted, index| if (std.mem.eql(u8, name, wanted)) {
            if (found[index]) return error.FileChanged;
            found[index] = true;
            matched = true;
            break;
        };
        if (!matched) return error.FileChanged;
    }
    if (c._errno().* != 0 or count != role_count) return error.FileChanged;
    for (found) |present| if (!present) return error.FileChanged;
}

fn tombstoneAndClose(self: *DurablePreparation) Error!void {
    ...
    self.parent_fd = -1;
    var failed = false;
    for (descriptors[0 .. count + 2]) |fd| {
        if (fd >= 0 and c.close(fd) != 0) failed = true;
    }
...
fn createStage(parent_fd: c.fd_t, storage: *[std.fs.max_name_bytes:0]u8) Error![:0]const u8 {
    for (0..8) |_| {
        ...
        const name = std.fmt.bufPrintZ(storage, ".maru-preparation-{x}", .{nonce}) catch return error.CreateFailed;
...
fn cleanupName(storage: *[std.fs.max_name_bytes:0]u8) Error![:0]const u8 {
    var nonce: u64 = undefined;
    c.arc4random_buf(std.mem.asBytes(&nonce).ptr, @sizeOf(u64));
    return std.fmt.bufPrintZ(storage, ".maru-preparation-cleanup-{x}", .{nonce}) catch error.CleanupFailed;
}

fn unlinkExact(parent_fd: c.fd_t, name: [:0]const u8, held_fd: c.fd_t) bool {
    var held: posix.Stat = undefined;
    var named: posix.Stat = undefined;
    return c.fstat(held_fd, &held) == 0 and c.fstatat(parent_fd, name.ptr, &named, posix.AT.SYMLINK_NOFOLLOW) == 0 and
        held.dev == named.dev and held.ino == named.ino and c.unlinkat(parent_fd, name.ptr, 0) == 0;
}
...
fn sameDirectoryAt(parent_fd: c.fd_t, name: [:0]const u8, held_fd: c.fd_t) bool {
    ...
    var named: posix.Stat = undefined;
    return c.fstat(held_fd, &held) == 0 and c.fstatat(parent_fd, name.ptr, &named, posix.AT.SYMLINK_NOFOLLOW) == 0 and
        posix.S.ISDIR(held.mode) and posix.S.ISDIR(named.mode) and held.dev == named.dev and held.ino == named.ino;
}

fn pristine(value: *const DurablePreparation) bool {
    if (value.owner != null or value.phase != .pristine or value.parent_fd >= 0 or value.directory_fd >= 0 or
        value.destination_len != 0 or value.directory_leaf_len != 0) return false;
    for (value.files) |file| if (file.owner != null or file.fd >= 0 or file.parent_fd >= 0) return false;
    return true;
}

fn validStorage(value: *const DurablePreparation) bool {
    if (value.parent_fd < 0 or value.directory_fd < 0 or value.destination_len == 0 or value.destination_len >= value.destination.len or
        ...
        !std.mem.eql(u8, std.fs.path.basename(value.destination[0..value.destination_len]), value.directory_leaf[0..value.directory_leaf_len])) return false;
    for (0..role_count) |index| {
        if (value.path_lens[index] == 0 or value.path_lens[index] >= value.paths[index].len or
            value.name_lens[index] == 0 or value.name_lens[index] >= value.names[index].len or
...
fn metadataSeal(value: *const DurablePreparation) [32]u8 {
    ...
    for (0..role_count) |index| {
        ...
        const observed = value.files[index].value() orelse continue;
        hasher.update(std.mem.asBytes(&observed.identity.device));
        hasher.update(std.mem.asBytes(&observed.identity.inode));
...
fn observation(input: anytype) files.ExecutableObservation {
...
fn sameRepository(left: evidence_mod.Repository, right: manifest_mod.Repository) bool {
    return left.id == right.id and std.mem.eql(u8, left.owner, right.owner) and std.mem.eql(u8, left.name, right.name);
}
fn sameRelease(left: evidence_mod.Release, right: manifest_mod.Release) bool {
    return left.id == right.id and std.mem.eql(u8, left.tag, right.tag) and std.mem.eql(u8, left.version, right.version);
}
fn sameSource(left: evidence_mod.Source, right: manifest_mod.Source) bool {
    return std.mem.eql(u8, left.commit, right.commit) and std.mem.eql(u8, left.tree, right.tree);
}
fn sameBuild(left: evidence_mod.Build, right: manifest_mod.Build) bool {
    return left.run_id == right.run_id and left.run_attempt == right.run_attempt and std.mem.eql(u8, left.workflow_ref, right.workflow_ref);
}
...
fn directChild(root: []const u8, child: []const u8) bool {
    if (root.len == 1 and root[0] == '/') return child.len > 1 and std.mem.indexOfScalar(u8, child[1..], '/') == null;
    return child.len > root.len + 1 and std.mem.startsWith(u8, child, root) and child[root.len] == '/' and
        std.mem.indexOfScalar(u8, child[root.len + 1 ..], '/') == null;
}
