//! Fetches one current-attempt timing artifact through an already-pinned GitHub CLI.
//!
...
const archive_leaf: [:0]const u8 = "timing-artifact.zip";
...
pub fn fetch(
    ...
    fetchUntil(io, allocator, cli, token, expected, workspace_path, metadata_buffer, &deadline, result) catch |err| {
...
pub fn fetchUntil(
    ...
    return fetchWith(&operations, allocator, cli, token, expected, workspace_path, metadata_buffer, deadline, result);
}
...
fn fetchWith(
    ...
    try validateInputs(cli, token, expected, workspace_path, metadata_buffer, deadline, result);

    var workspace: Workspace = .{};
    try workspace.prepare(workspace_path);
    fetchPrepared(operations, allocator, cli, token, expected, metadata_buffer, deadline, result, &workspace) catch |err| {
...
fn fetchPrepared(
    ...
    try artifact.selectMetadata(allocator, metadata_bytes, expected, &metadata);
    ...
    try artifact.bindArchive(allocator, &metadata, archive_bytes, result);
}
...
fn listPlan(storage: *PlanStorage, expected: artifact.Expected) !Plan {
    ...
    const name = std.fmt.bufPrint(&storage.name, "session-host-release-live-timing-{d}", .{expected.run_attempt}) catch
...
fn validateInputs(
    ...
    if (cli.path.len < 2 or cli.path[0] != '/' or std.mem.indexOfScalar(u8, cli.path, 0) != null or
        ...
        expected.repository_id == 0 or expected.run_id == 0 or expected.run_attempt == 0 or !lowerHex(expected.source_sha, 40) or
    ...
    if (overlaps(output, token) or overlaps(output, cli.path) or overlaps(output, workspace_path) or
        overlaps(output, expected.source_sha) or overlaps(output, metadata_buffer) or
        ...
        overlaps(metadata_buffer, expected.source_sha) or overlaps(metadata_buffer, std.mem.asBytes(deadline)) or
