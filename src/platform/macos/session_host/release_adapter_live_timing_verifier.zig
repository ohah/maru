//! Product composition for verifying one current protected-run live timing artifact.
...
pub fn parse(args: []const []const u8) !Command {
    ...
    if (!std.mem.eql(u8, args[0], "verify")) return error.InvalidCommand;
...
pub fn verify(
    ...
    transport.fetchUntil(io, allocator, .{ .path = cli_path, .pinned = &pinned }, token, expected(trusted), workspace_path, metadata_buffer, &deadline, &provenance) catch |err| {
    ...
    if (provenance.repository_id != trusted.repository.id or value.timing.run_id != trusted.build.run_id or
        value.timing.run_attempt != trusted.build.run_attempt or
        !std.mem.eql(u8, value.timing.source_sha, trusted.source_commit))
    {
        provenance.deinit() catch {};
        deadline.deinit() catch {};
        return error.ContextMismatch;
    }
    provenance.deinit() catch |err| {
        deadline.deinit() catch {};
        return err;
    };
    try deadline.deinit();
...
fn expected(trusted: context_mod.Context) artifact.Expected {
    return .{
        .repository_id = trusted.repository.id,
        .run_id = trusted.build.run_id,
        .run_attempt = trusted.build.run_attempt,
        .source_sha = trusted.source_commit,
    };
}
