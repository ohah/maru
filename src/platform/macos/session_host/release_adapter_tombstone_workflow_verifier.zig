//! Filesystem and remote-authority composition for the protected durable-tombstone verdict.
...
pub const final_name = "session-host-tombstone-workflow-pass.json";

pub const Command = struct { cli_path: [:0]const u8, cli_sha256: []const u8, evidence_path: [:0]const u8, bundle_path: [:0]const u8, output_path: [:0]const u8 };

pub fn verify(io: std.Io, allocator: std.mem.Allocator, context: context_mod.Context, command: Command, token: []const u8, budget_ns: i128) !void {
...
pub fn verifyWith(allocator: std.mem.Allocator, context: context_mod.Context, command: Command, remote: anytype) !void {
    ...
    if (!exactOutput(command.output_path) or !pairwiseDistinct(.{ command.cli_path, command.evidence_path, command.bundle_path, command.output_path })) return error.InvalidPath;
    var evidence_file: files.PinnedReleaseFile = .{};
    try files.pinReleaseFileObserved(&evidence_file, command.evidence_path, false, evidence.max_bytes);
    ...
    if (evidence_observation.identity.device == bundle_observation.identity.device and evidence_observation.identity.inode == bundle_observation.identity.inode) return error.PathAlias;
    var input = try evidence_file.readHeldAlloc(allocator, command.evidence_path, evidence.max_bytes);
    defer input.deinit(allocator);
    var leaf = try evidence.parse(allocator, input.bytes);
    defer leaf.deinit();
...
const Remote = struct {
    ...
    fn authenticate(self: *@This()) !record.Authority {
        ...
        try current.authenticateProfilePhaseUntil(self.io, self.allocator, self.context, self.cli, self.token, &self.response, self.deadline, .tombstone_product, .completed, &result);
    ...
    fn attest(self: *@This(), evidence_path: []const u8, bundle_path: []const u8, name: []const u8, sha256: []const u8) !record.Attestation {
        ...
        try cli_authority.revalidate(self.allocator, self.cli.path, self.cli.pinned);
        return .{ .verified = observed.verified, .run_id = observed.run_id, .run_attempt = observed.run_attempt, .subject_name = observed.subject_name, .subject_sha256 = observed.subject_sha256, .self_hosted = true };
    }
