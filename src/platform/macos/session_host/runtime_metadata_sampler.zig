...
fn testSource(observer: u64, title: u32, foreground_generation: u64) Source {
    return .{
...
test "P4 E3b sampler is runtime-scoped, stale-safe, and checked across rollover" {
    var record = Record.init(testSource(1, 2, 3), 100);
    try std.testing.expectEqual(Token{ .incarnation = 1, .revision = 1 }, record.token);
    try std.testing.expectEqual(Outcome.unchanged, try record.sample(testSource(1, 2, 3), 101));
    try std.testing.expectEqual(Outcome.stale, try record.sample(testSource(9, 9, 9), 100));
    try std.testing.expectEqual(Token{ .incarnation = 1, .revision = 1 }, record.token);

    try std.testing.expectEqual(Outcome.changed, try record.sample(testSource(2, 2, 3), 101));
    ...
    try std.testing.expectEqual(Outcome.changed, try record.sample(testSource(2, 3, 3), 102));
    ...
    const before = record;
    try std.testing.expectError(error.TokenExhausted, record.sample(testSource(2, 3, 4), 103));
