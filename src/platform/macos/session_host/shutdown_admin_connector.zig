...
test "C3-3b6 실제 이전 wire 기준은 ambiguous 뒤 destructive retry를 하지 않는다" {
    ...
    if (c.getenv(short_endpoint.legacy_shared_fixture_env) == null) return error.SkipZigTest;
    try short_endpoint.claimEmptySharedLegacyFixture();
    defer short_endpoint.releaseEmptySharedLegacyFixture();
    ...
    const socket_path = try short_endpoint.sharedLegacySocketPathIn(&socket_buf, host_id);
    const child = try launcher.spawnSessionHostSupervisedLegacySharedForTest(
        allocator,
        baseline_z,
        session_dir,
        socket_path,
