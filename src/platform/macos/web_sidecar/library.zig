...
/// 필드 이름에 `cef_` 를 붙인 심볼을 찾는다. 타입은 헤더 선언에서 그대로 가져와 서명이 어긋나면 컴파일이 멈춘다.
...
pub fn load(framework_binary: [*:0]const u8) LoadError!Api {
    ...
    inline for (std.meta.fields(Api)) |field| {
        const symbol = std.c.dlsym(handle, "cef_" ++ field.name) orelse return error.SymbolMissing;
        @field(api, field.name) = @ptrCast(@alignCast(symbol));
    }
    return api;
}
