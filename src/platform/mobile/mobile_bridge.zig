...
///
/// **곱한 결과를 `size` 와 같은 범위로 자른다.** 접근성 배율은 3배도 넘을 수 있는데(iOS 의 큰
/// 글씨 접근성 크기), 셀 상한(`MARU_ATLAS_CELL_MAX`) 위로 올라가면 어차피 다시 확대돼 흐려진다 —
/// 범위는 스키마가 소유한다(두 곳에 숫자를 적으면 갈린다).
///
/// **`size` 자체는 이미 그 범위 안이다.** 「켜고 끄는 것만으로 사용자가 적은 값이 바뀌는 것 아니냐」를
/// 의심해 봤는데(파일을 손으로 고치면 범위 밖 값이 들어올 수 있으니까), 파일 파싱이 `schema.tryParse`
/// 라 **거기서 이미 거절한다**(`font.size = 200` 은 기본값 22 로 떨어진다 — 판정자로 확인했다).
/// 데스크톱은 `parse_range` 로 파일 범위를 GUI 보다 넓게 여는데 모바일 `font.size` 에는 그것이
/// 없어서 둘이 같다. 그래서 여기 경계를 사용자 값으로 넓히는 것은 **헛방어**라 안 넣는다.
...
///
/// **셀에 안 넘치게 자른다.** 아틀라스 슬롯은 host 가 알린 격자(`atlas_cell_w/h`)라 그보다 큰
...
/// 그리고 **아틀라스는 둘이다** — 커버리지(R8)와 이모지(RGBA8)가 같은 격자로 서므로 넓이 하나에
/// 픽셀당 5바이트가 든다. 96px 셀이면 1152×3072 이라 3.4 + 13.5 ≈ **16.9 MiB** 다(32px 셀이던
/// 때는 1.9 MiB). 그 위는 폰에 부담이다. 상한을 넘는 크기는 다시 확대가 되지만, 그때도
/// 지금(2.8배)보다는 훨씬 낫다.
///
...
fn resetAtlasIfBakeSizeChanged() void {
    ...
    atlas_baked_px = px;
    atlas_n = 0;
    // **컬러(이모지) 등록부도 같이 비운다.** host 는 다시 구울 때 두 텍스처를 **둘 다** 새로
    // 만든다(같은 격자라 크기가 함께 바뀐다) — 여기서 글자 등록부만 비우면 이모지는 「등록은
    // 있는데 그림은 없는」 칸을 영영 가리킨다. 다시 굽는 목록에도 안 올라오므로 앱을 껐다 켤
    // 때까지 이모지가 빈칸이다. 셀이 상수였을 때는 이 경로가 아예 안 돌아서 안 드러났다.
    color_n = 0;
}
...
fn buildUi(width: u32, height: u32, tk: *const tokens.Tokens) !void {
    ...
    // 탭 셋과 카드 다섯이 **눌러도 아무 일이 안 났다**(핸들러가 아예 없었다) — U3 이 걷어냈다.
    // 사이드바는 폰 세로에서 **폭의 1/3 을 상시 점유**했다.
    //
    // **세션을 바꾸는 길은 그래서 「덮개」다**(사용자 확정 — [UX §3](../../../docs/mobile-ux.md)):
    // 뒤로 나가 목록에서 고른다. 새 손짓도 새 chrome 도 안 만든다. 화면들과 세션을 여럿 드는
    // 일은 U2a~c 가 세웠다 — 여기서 가짜 탭을 그리지 않는 이유가 그것이다(UX §2.4).
