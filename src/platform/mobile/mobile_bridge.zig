...
const term_feed =
    ...
    // 이모지 한 줄. BMP 밖 글자(서러게이트 쌍)와 **컬러 아틀라스**가 서 있는지가 화면에서
    // 드러난다 — 이 줄이 없으면 이모지가 빈칸이던 것도 화면으로는 못 봤다(M4a6).
    // 뒤의 둘은 **host 굽기 경로의 시각 판정자**다(계약 테스트가 못 닿는 자리라 이 줄이 유일한
    // 판정자다): `\u{2764}` 는 번들 폰트에 **없는** 기호라 시스템 폰트 폴백이 빠지면 빈칸이 되고,
    // `\u{1D400}` 는 **BMP 밖 비컬러** 글자라 서러게이트 쌍 조립이 빠지면 엉뚱한 글자가 된다.
    // 둘 다 컬러 이모지와 **다른 경로**를 타므로 이모지만 보고는 회귀를 못 잡는다.
    "emoji \u{1F600}\u{1F389}\u{1F680} + 한글 가나 \u{2764} \u{1D400}\r\n";
