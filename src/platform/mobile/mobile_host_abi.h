...
/// **이제 기본값일 뿐이다** — 실제로 구울 셀은 아래 두 함수가 답한다(코어가 화면 셀 × 기기
/// 배율로 정한다). 상수로 구우면 그 그림을 늘려 써서 흐려진다: 실측(갤럭시 S25 FE, 배율 2.8)에서
/// 기본 설정으로도 22px 그림을 62px 자리에 늘리고 있었고, 같은 화면의 시스템 글자보다 가장자리가
/// 한 픽셀 더 번졌다. 이 값들은 텍스처 상한을 어림잡는 자리에만 남는다.
#define MARU_ATLAS_CELL_W 24
#define MARU_ATLAS_CELL_H 32
/// 지금 구워야 하는 셀 크기(기기 픽셀). **굽기 직전에 묻는다** — `font.size` 나 기기 배율이
/// 바뀌면 값이 달라지고, 그때 host 는 텍스처를 그 크기로 **다시 만들어** 다시 구워야 한다.
