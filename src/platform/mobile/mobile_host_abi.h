...
///
/// **상수가 아니라 «묻는 값» 이다.** 예전에는 `MARU_ATLAS_CELL_W/H` 라는 `#define` 이었는데,
/// 상수로 구우면 그 그림을 늘려 써서 흐려진다: 실측(갤럭시 S25 FE, 배율 2.8)에서 기본 설정으로도
/// 22px 그림을 62px 자리에 늘리고 있었고, 같은 화면의 시스템 글자보다 가장자리가 한 픽셀 더
/// 번졌다. 그 두 `#define` 은 **지웠다** — 남겨 두면 굽는 자리가 다시 그것을 집는다.
///
/// **굽기 직전에 묻는다** — `font.size` 나 그리는 배율이 바뀌면 값이 달라지고, 그때 host 는
/// 텍스처를 그 크기로 **다시 만들어** 다시 구워야 한다. 반대로 **이미 서 있는 텍스처의 슬롯을
/// 계산할 때는 이 값을 쓰지 않는다** — 그때는 host 가 `maru_mobile_atlas_geometry` 로 알린
/// 「실제로 구운」 격자다(둘이 갈리면 슬롯이 이미지 밖을 가리킨다).
...
/// 3배 드로어블에 그린다(그 확대는 NDC 변환에서 일어난다). host 는 첫 프레임에 이것을 알린다.
void maru_mobile_set_render_scale(unsigned int scale_milli);
...
/* **스크린 리더가 누를 때 쓰는 손가락 id.** 진짜 손가락은 플랫폼이 주는 번호(0 부터)를 쓰므로,
   ...
   (host 마다 고르면 한쪽만 고쳐도 안 드러난다). 브리지는 id 를 첨자가 아니라 검색 키로 쓰므로
   값이 커도 안전하다. */
#define MARU_MOBILE_POINTER_ID_A11Y 0xFFFFFFFFu
