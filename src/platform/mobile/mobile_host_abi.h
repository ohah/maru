...
/// **스크롤 델타를 받는 자리는 없다.** 전에는 `maru_mobile_scroll(float)` 이 있었고 host 의
/// 관성 루프가 그것을 불렀다 — 관성이 코어로 오면서 **부르는 host 가 하나도 남지 않았다**.
/// 지금 플랫폼이 넣는 것은 **원시 포인터 이벤트뿐**이고 델타는 코어가 만든다(§3.1).
/// 마우스 휠·트랙패드(`ACTION_SCROLL`)를 붙일 때 델타 진입점이 다시 필요해지는데, 그때는
/// 손가락과 입력원이 달라 서명도 달라진다(가로 성분·노치 단위) — 그 슬라이스에서 새로 정한다.
