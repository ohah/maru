...
pub const SpawnRequest = struct {
    ...
    pane_id: ?u64 = null,
    /// 훅 로그 경로의 **인스턴스 칸**. 셸에 `MARU_HOOK_INSTANCE=<이 값>` 으로 주입하고, 에이전트 훅이
    /// 로그 경로의 첫 칸으로 쓴다(docs/agent-hooks.md §4).
    ///
    /// **왜 pane 칸만으로는 부족한가**: 훅 로그 디렉터리는 사용자 캐시 하나뿐인데 `surface.id` 는
    /// **프로세스마다 1 부터** 발급된다(`SurfaceIdAllocator`). 그래서 maru 를 두 개 띄우면 두 인스턴스의 첫
    /// pane 이 같은 파일 이름을 갖고, 서로의 이벤트를 읽거나 시작 시 정리가 남의 살아 있는 로그를 지운다.
    ///
    /// **이미 포맷된 토큰을 받는다** — 만드는 곳은 `session.agent_hook_command`(계약 SSOT)의
    /// `formatGuiInstance`(GUI 소유 = 그 프로세스 pid) 또는 `formatHostInstance`(host 소유 = `host_<hex>`)다.
    /// 여기서 다시 숫자로 조립하지 않는 이유: 소유자가 둘이고 모양이 다른데 그 규칙을 두 곳에 적으면
    /// «훅이 받는 이름 ≠ maru 가 읽는 이름» 이 조용히 성립한다.
    ///
    /// null 이면 주입하지 않는다 — 그러면 훅은 조용히 나가고 로그를 안 남긴다(fail-closed).
    /// 호출자 소유 슬라이스이며 spawn 이 끝날 때까지만 살아 있으면 된다(env 는 spawn 시점에 굳는다).
    hook_instance: ?[]const u8 = null,
    /// 훅 로그 경로의 **pane 칸**(`MARU_HOOK_PANE`). `formatSurfacePane`(GUI 소유 = process-local surface id)
    /// 또는 `formatRuntimePane`(host 소유 = 32 hex `runtime_id`)이 만든다.
    ///
    /// **`pane_id` 와 갈라 둔 이유**: `MARU_PANE_ID` 는 control-plane `auth.self` selector 이고 GUI
    /// process-local 이라 host 가 띄운 자식에 실을 수 없다. 한 변수가 두 일을 하면 «훅 신원을 host 소유
