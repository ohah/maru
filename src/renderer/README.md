...
`FontId`가 "안정적"이려면 registry가 **atlas와 같은 수명**이어야 한다. `FontId`는 face가 registry에 처음 등장한 순서로 매기는 지역 순번이라, registry를 frame·pane마다 새로 만들면 같은 순번이 frame마다 다른 face를 가리킨다. glyph atlas는 frame 사이에 살아남고 cache key에 그 `FontId`를 담으므로, per-frame registry면 이전 frame이 한 face용으로 구운 atlas slot을 다음 frame의 **다른** face가 같은 `(FontId, glyph_id)`로 오인 HIT해 엉뚱한 glyph를 그린다(순번이 뒤집힐 때만 터져 간헐적 — 예: 조합 중 '놔'에 번개). 그래서 registry는 `RendererState`가 atlas와 나란히 소유하는 **단일 공유 registry**로 두고, 모든 pane의 shape가 여기 intern해 같은 PostScript name이 앱 수명 내내 같은 `FontId`를 받게 한다. 멀티 페인이 여러 `ShapedPane`을 동시에 들고 있어도 shape/rasterize는 렌더 스레드 단일 실행([I/O–렌더 스레딩](../../docs/io-render-threading.md) §3)이라 이 공유 registry에 락이 필요 없다.
...
`RendererState`는 frame 사이에 살아남는 renderer 소유 상태다. 현재는 `GlyphAtlas`와 `FontIdentityRegistry`를 오래 들고 가며 `RenderSnapshot -> DrawList -> GlyphRunList -> GlyphFrame -> GlyphQuadFrame -> GlyphRasterFrame`을 한 `RenderFrame`으로 준비한다. `FontIdentityRegistry`를 atlas와 같은 수명으로 함께 두는 이유는 위 `FontIdentityRegistry` 절 참고 — atlas cache key의 `FontId`가 frame 간 안정돼야 하기 때문이다. app host와 future Metal backend는 이 facade만 호출해야 하고, atlas reuse, font layout, UV 변환, raster upload byte 정책을 각자 다시 구현하면 안 된다.

CoreText 같은 제품 shaper는 `RendererState.buildFrame`의 fake-friendly `shape(cell)` 계약에 맞추지 않는다. 대신 `DrawList` 전체를 shape해 `ShapedGlyphRecord -> GlyphRunList`를 만든 뒤 `RendererState.buildFrameFromGlyphRunList`로 들어온다. 이 entrypoint는 성공하면 `DrawList` ownership을 반환된 `RenderFrame`으로 옮기고, 실패하면 caller가 여전히 `DrawList`를 정리하거나 artifact로 남길 수 있게 한다. macOS `coretext_shaper.zig`는 이미 shape된 CoreText glyph record를 이 entrypoint가 소비할 `GlyphRunList`로 바꾸는 제품 후보 경계와, 실제 `TerminalCore -> DrawList` cell 배열을 native CoreText bridge로 shape하는 `CoreTextDrawListShaper` 경계를 함께 가진다. CoreText smoke와 Metal smoke는 `renderer_input=terminal_core_draw_list`/`drawlist_input=terminal_core_draw_list`로 이 제품 shaper 경계를 검증한다.
