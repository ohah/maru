...
`GlyphQuadFrame`은 `GlyphFrame` 다음 단계다. atlas slot의 pixel rect를 shader가 쓰는 normalized UV로 바꾼다. 이 계산을 renderer domain에 두면 Metal/WebGPU backend가 각자 texture bounds check와 UV 나눗셈을 다시 만들지 않는다.
