...
// octant에서 건너뛰는 26개 패턴 — 이미 별도 코드포인트로 인코딩된 것들(space·반칸·4분면·full block).
// 이 26개를 제외한 230개를 마스크 오름차순으로 U+1CD00부터 배정(Unicode 표준 순서). skip만 데이터로.
const octant_skip = [_]u8{
    0, // space
    1, 2, 3, // 이미 인코딩(부분 칸 조합)
    5, 10, // ▘▝ 위 4분면
    15, // ▀ upper half
    20,
    40,
    ...
    90, 95, // ▞ / ▙
    128, // ▗ 아래우 4분면
    160, 165, 170, 175, // ▗·▚·▐ right half·▜
    192, // ▄ lower half
    240, 245, 250, 252, // 하부 조합·▟
    255, // █ full
};
