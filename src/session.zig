...
pub const remote_shell = @import("session/remote_shell.zig"); // 원격 로그인 셸 규율(PATH 처방·`sh` 껍데기) — 세 사용처의 단일 출처
pub const remote_watch_install = @import("session/remote_watch_install.zig"); // 원격 감시자 설치 계약(RW2 — uname 판별·멱등 확인·심기)
...
pub const remote_activity_wire = @import("session/remote_activity_wire.zig"); // 원격 활동 wire(RAV1 — 헬퍼가 저쪽에서 훑은 Hit+라벨을 실어 나른다)
pub const remote_file_mutation = @import("session/remote_file_mutation.zig"); // 원격 변경 결과 wire(RF6a — 저쪽 한 프로세스의 stat→비교→비대체 rename 결말)
...
pub const editor = struct {
    ...
    pub const lsp = struct {
        ...
        /// semantic tokens 2층(tooling §8.2i · visual-mapping §5) — legend 매핑·relative 풀기·편집 밀기.
