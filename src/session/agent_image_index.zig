...
pub fn scanResultLine(line: []const u8, line_offset: u64, has_image: bool) ?ResultRecord {
    ...
    if (std.mem.indexOf(u8, head, claude_tool_result_marker)) |m| {
        ...
        if (!body.parsed) {
            ...
            //
            // ⚠️ **`failed` 는 안 채운다 — 못 채운다**(적대적 EB3). `"is_error":true` 는 `content`
            // **뒤**에 오므로 본문의 끝을 알아야 안전하게 찾는데, 배열의 끝을 찾으려면 JSON 을 실제로
            // 파싱해야 한다(문자열 안의 `]` 때문에). 끝을 모른 채 마커 뒤에서 찾으면 **본문에 담긴
            // 그 글자**를 실패로 읽는다 — 도구 출력에 JSON 이 실리는 일은 흔하다.
            // 실측(코퍼스 154 건)에서 이 형태의 실패는 **0 건**이지만, 0 이 「없다」는 아니므로
            // 한계로 적어 둔다: 그런 결과가 오면 화면은 「실패」 대신 「완료」라고 적는다.
