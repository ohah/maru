...
///
/// **스피너 프레임은 회전한다.** 아래 여섯은 전부 실측이다. 마지막 관측은 claude pane 전체를
/// 0.3 초 간격 900 회(≈5 분) 훑은 것이고(2026-09-05), `…` 로 끝나는 줄의 첫 문자 분포는 이렇다:
/// `✽`(U+273D) 932 · `·`(U+00B7) 896 · `✢`(U+2722) 817 · `✻`(U+273B) 741 · `✳`(U+2733) 591 ·
/// `✶`(U+2736) 564. **그 표본에서 목록 밖 프레임은 하나도 안 나왔다.**
...
///
/// ⚠️ **목록은 원리상 불완전하다.** 적대적 검증에서 `·`(U+00B7) 가 뒤늦게 나왔다 — 그런데 다시 재 보니
/// **896 회로 두 번째로 흔한 프레임**이었다. 드물어서 놓친 게 아니라 **표본이 모자랐다**(150 회 관측에서
/// 0 회). 표본을 6 배로 늘린 뒤에야 실제 분포가 보였으니, 「다 모았다」고 여기지 않는다. 목록에 없는 프레임이 오면 규칙이
/// 조용히 안 걸리고 증상이 그대로 재발하므로, `MARU_DEBUG` 의 `screen_rule=` 로 무엇이 걸렸는지 보고
/// (계약 §1.7) 새 프레임을 **실측으로** 더한다.
///
/// `·` 는 흔한 문자지만 **줄 시작 + `…`** 라는 두 조건이 함께 걸리므로 안전하다 — 그 모양으로 시작하는
/// 진행 줄 아닌 사례는 전 pane 관측에서 0 건이었다. 대화 본문은 공백으로 시작하고(같은 관측 15 건),
/// 답변 블록은 `⏺` 로 시작한다(2 건) — 둘 다 이 목록에 없어 걸리지 않는다.
