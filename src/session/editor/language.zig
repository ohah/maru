...
//! - **상태바 언어 표시** — 칸이 섰다(2026-09-07). 배치는 `status-bar.md` 「언어 항목」이 소유하고,
//!   **쓰는 것은 `Language` 가 아니라 `Grammar` 다**: 이 열거는 주석 문법으로 묶은 것이라 `c_like` 가
//!   `"C-like"` 를 내고 `Makefile`·`Dockerfile` 을 `shell` 로 묶는다 — 주석 토글에는 그것이 옳지만
//!   상태바에 적으면 거짓이다. **그래서 표시 이름은 `Grammar` 옆에 둔다**(아래).
//!
