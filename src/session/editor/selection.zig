...
pub const ColumnAnchor = struct {
    ...
    // **행은 문서 기준이다 — 뷰포트 기준이 아니다**(§3.2a). 이 저장소는 그 구분을 비교 뷰 선택에서
    // 이미 겪었다(`VisualRow.line` 이 뷰포트 상대인데 절대로 써서 굴린 뒤 클릭이 100% 어긋났다).
    // 열 선택은 드래그 중 **자동 스크롤**이 붙으므로 뷰포트 기준으로 들면 화면이 굴러갈 때마다
    // 사각형이 문서에서 미끄러진다. 그래서 스크롤은 이 값을 무효화하지 않는다.
};
