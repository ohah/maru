...
const config_overrides = [_][]const u8{
    ...
    //
    // ⚠️ **어느 파일이 그 이벤트를 냈는지는 확정하지 못했다.** `.git/index` 재기록은 실측으로
    // 확인했지만(maru3: 일반 status 는 다시 쓰고 플래그를 붙이면 안 쓴다) 하니스 저장소에서는 index 가
    // 안 바뀌는데도 깨움이 2 였다 — 잠금 파일이 순간적으로 생겼다 사라지는 쪽으로 보이나 50 ms 폴링
    // 으로는 못 잡았다. **기록은 관측한 것까지만 한다.**
    //
    // 실측으로 갈랐다 — 감시자를 띄워 두고:
    //   일반 status              → 깨움 2
    //   --no-optional-locks      → 깨움 0
    //   진짜 파일 변경(대조군)    → 깨움 1
    //
