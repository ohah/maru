...
pub const ScanFlags = struct {
    ...
    /// 저쪽이 **이번에 실제로 읽은** 바이트. 화면이 쓰지는 않지만 **진단이 이 값으로 「정말 훑었나」를
    /// 본다**.
    ///
    /// ⚠️ **파일 크기가 아니다.** 통째 스캔이면 체인 **전체의 합**이고, 이어읽기(RAV7b-3)면 자국
    /// 뒤의 **머리 파일 꼬리**뿐이다 — 부모는 아예 안 열기 때문이다. 신선도도 이어읽기 판정도
    /// `head_bytes` 를 봐야 한다(RAV7a 적대적 S1 이 그 혼동으로 재개 세션 58% 의 신선도를 껐다).
    scanned_bytes: u64 = 0,
