...
///
/// **둘이다: 스테이지된 변경 · 변경 사항.** 에이전트 탭의 `마지막 턴 이후` 줄이 여기에 세 번째 섹션을
/// 만들지 않는 것은 설계다 — 그 줄을 누르면 **이 탭으로 온다**([dock §3.5.4](../../docs/editor-surface-dock.md)).
/// 그 비교(마지막 스냅샷 ↔ 작업트리)를 위해 `DiffBase.turn` 이 선언돼 있었으나 **대입하는 코드가 끝내
/// 안 생겨** 2026-08-25 에 죽은 가지로 걷어냈다. 완료된 턴은 `DiffBase.turn_range`(스냅샷 둘)가 소유한다.
