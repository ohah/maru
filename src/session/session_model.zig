...
pub fn Model(comptime Rt: type) type {
    return struct {
        ...
        pub const Tab = struct {
            ...
            /// 카드 하위 **Term 목록**이 접혔는가(docs/sidebar-agent-list.md §4). 토글 행이 생기는 카드에만
            /// 의미가 있다(에이전트가 있으면 Term 1개부터, 없으면 Term 2개 이상 — agent.appendAgentRows).
            /// workspace.v1 영속(agents-collapsed). 처음엔 "파생 상태 비영속"으로 안 저장했는데, 목록 대상이 에이전트
            /// 전수에서 Term 전수로 넓어진 뒤로 행 구성은 같은 파일이 복원하는 Term들이라 그 근거가 사라졌다
            /// (§4 정정 2026-09-13). 여기 기본값 false(펼침)는 **실행 중 새로 만든 워크스페이스**의 기본(§1)이고,
            /// **파일 기본값은 접힘**이다(키 없는 옛 줄 = 사용자 의도 없음 → 접어 둔다 — workspace.zig Tab 주석).
