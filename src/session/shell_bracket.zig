...
//!
//! **무엇을 답하나**: 「이 턴에서 에이전트의 셸이 돌던 구간들은 언제였나」. **소비자는 AT3b-3(시간 창 폴백)이다** —
//! AT3b-2 는 provider 의 셸 diff(`bashEditDiff`, Claude bypass 모드)로 귀속하므로 이 구간을 읽지 않는다. 구간이
//! 필요한 곳은 그 diff 가 없는 자리(Codex·`unavailable`·gitignore 파일)이고, 그때 이 구간 안에 `ctime` 이 떨어진
//! 파일을 «셸이 고쳤다(추정)» 로 올린다. 그때까지는 테스트만 읽는다 — 죽은 코드가 아니라 다음 단계의 재료다. 구간은 **턴 단위 합집합**으로 쓴다 — 어느 명령의
//! 소행인지는 묻지 않는다(계획 ⑷). 그래서 겹친 구간은 무해하고, 여기서 합치지도 않는다.
//!
