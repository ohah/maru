...
여기서 "참고"는 공개 명세와 아키텍처 경계를 배우는 것이지, 레퍼런스 소스의 코드 표현(자료구조 레이아웃, 함수/이터레이터 구조)을 그대로 옮기는 것이 아니다. Maru의 VT 코어는 ECMA-48과 vt100.net DEC ANSI state machine 같은 공개 명세에서 유도한다. copyleft(LGPL/GPL) 레퍼런스는 최종 화면 동작 비교(오라클)로만 쓰고 구현 유도를 위해 소스를 읽지 않는다. 자세한 규칙은 [필수 프로젝트 규칙](docs/project-rules.md)을 따른다.
...
터미널 호환성/보안 기본값은 docs/terminal-compatibility-policy.md를 따른다.
```
