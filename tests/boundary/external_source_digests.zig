...
pub const inventory = [_]Proof{
    ...
    // 숫자 입력(M10e)이 `outOfRange` 를 더한다 — 스키마의 range 메타를 훑어 화면이 받아들일 값을
    // 판정한다(범위를 화면이 따로 적으면 파일 파싱과 GUI 가 다른 값을 받는다). 반사 자리가 그만큼
    // 늘어 count 가 8 → 9 다.
    // 적대적 검증이 낸 셋을 고치며 바뀐다 — 프리셋 되짚기가 배경 하나가 아니라 네 색을 다 보고,
    // "고른 것 없음"(preset_none)을 낸다. 반사 자리는 안 늘었다(count 8 그대로) — 비교를 늘렸을
    // 뿐 `@field` 로 도는 곳은 그대로다.
    ...
    // 등록·수정·삭제(S9b-2b-2)의 `withServers` 가 붙어 또 바뀐다 — 목록을 통째로 다시 적는
    ...
    // Client 구성·receiver 집합을 안 건드린다.
    // P3d-②에서 또 움직인다: 목록이 **저장소 머리 줄부터** 만들어지고(그 아래에 그 저장소의 줄들),
    // 접힘 상태가 세션에 산다. 테스트 픽스처가 Term cwd를 `/tmp`로 고정하는 것도 여기 포함 —
    // 목록이 터미널 cwd에서 나오므로 그러지 않으면 테스트가 도는 위치에 따라 목록이 갈린다.
    // count는 2 그대로다.
    //
    ...
    //
    // 공백 든 경로(계약 §5.2 ⒝)에서 또 움직인다: hover 상태에 `hover_url_end`가 붙고, 밑줄이 그 끝을
    // 쓰도록 `hoverLinkSpanFor`가 한 줄 갈린다. count는 2 그대로다 — 새 `@field` 반사가 없다.
    ...
    // 창 안으로 끌어오는 신호(`scm_commit_reveal`)와 접힘 시 blur가 붙어 다시 움직인다. count는 2 그대로다.
    ...
    // 스크롤바 자리를 **넘칠 때만** 주도록 바꾸며 또 움직인다(2026-08-20 사용자 결정): 마지막 프레임이
    // 그 자리를 비웠는지를 세션이 들고(`scm_list_overflows`), 커밋 상자의 랩 계산이 그 값을 읽는다 —
    // 조건을 두 곳에서 판정하면 host가 2줄이라 믿는 상자에 view가 3줄을 그린다(테스트가 그걸 잡았다).
    // count는 2 그대로다 — 필드 하나와 테스트만 늘었다.
    ...
    // 소스 컨트롤 도크의 스크롤 상한을 투영 하나로 모으며 세션 필드(`scm_scroll_extent`)가 하나 늘고
    // 회귀 둘이 붙어 또 움직인다. count는 2 그대로다 — 필드·테스트만 늘었고 `@field` 반사와 밖에서
    // 부르는 자리는 그대로다.
    ...
    // count는 1 그대로다.
    // 편집 화면(S9b-2b-2)이 키를 더 넣고 안 쓰게 된 하나(`mob_servers_empty_hint`)를 빼서 또
    ...
    // fetch 칩이 글자에서 아이콘이 되며 문구 둘(`scm_fetch`·`scm_fetching`)을 **뺐다**(2026-08-20 사용자
    // 결정). 화면에 없는 문구를 표에 남기지 않는다. count는 1 그대로다 — 표에서 항목이 준 것이지
