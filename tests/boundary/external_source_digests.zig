...
pub const inventory = [_]Proof{
    ...
    //
    // hover 존재검증(계약 §8)에서 또 한 줄 움직인다: hover가 `urlAnchorAt`(분류만) 대신
    // `openableLinkAnchorAt`(클릭과 같은 resolve+stat)을 부른다. count는 2 그대로다.
    .{ .path = "src/platform/macos/app_session.zig", .count = 2, .digest_hex = "ec7a866134fd1ee2d8bbb32132b9029da960740b002dcef8a5b2741ceedf4a35" },
