...
const inventory = [_]Entry{
    ...
    //
    // 41 = 35 + **설치 재시도 배선의 진단 여섯**(2026-09-14). 설치 실패를 굳히지 않고 백오프로 바꾸며
    // (`spawnRemoteHookInstall` 의 두 `catch`), 다시 띄우는 자리에서 `HOME`·control socket 경로·목적지
    // 이름의 실패 사유를 남긴다 — 전부 표시가 아니라 **로그**다.
    //
    // 53 = 51 + **grid shaping 단계·run 반복률 두 줄**(2026-09-15, §10.7). run 캐시를 만들기 전에 «어느 워크로드에서
    // 적중하나」 를 재는 진단이다(폭포 8%, 페이저 줄 스크롤 89%). 로그다.
    //
    // 51 = 50 + **1초 요약의 `빈자리=`**(2026-09-15, §13.8 정정) — placement 는 있는데 이미지 view 가 없어 비운
    // 프레임 수. 플리커의 직접 신호(수정 전 초당 2~6, 수정 뒤 0). 로그다.
    //
    // 50 = 47 + **코어 락 호출 지점 히스토그램 세 줄**(2026-09-15, §12.9) — 창 요약·지점 줄·`밖` 표시. «tick당 26~41회」가 어느 지점 몇 번인지
    // 세는 진단이고, 빈 출력 신호마다 잡던 락(전체의 60%)을 이걸로 찾았다. 로그다.
    //
    // 47 = 46 + **kitty 이미지 전송이 락 아래에서 쓴 시간을 단계별로 뱉는 한 줄**(2026-09-15, §13.7).
    // 양보 뒤에도 남는 «긴 보유»의 정체(inflate 5.6MB)를 가른 계측이다. 로그다.
    //
    // 46 = 44 + **코어 락 경합을 두 방향으로 재는 `.frametime` 진단 두 줄**(2026-09-15). 메인 `lockCore`
    // 대기(합·최대·횟수·최대 지점)와 리더 `core.write` 보유/대기 — 텍스트 폭포에서 tick 의 85% 가 «락을
    // 기다린 시간» 이었고, 보유가 짧은데 대기가 긴 «기아» 를 이 두 줄의 대조로 읽는다(plans §13).
    // `MARU_DEBUG` 뒤에만 나오고 **표시가 아니라 로그**다. 단계 이름이 한국어인 이유는 위 44 와 같다.
    //
    // 44 = 42 + **프레임 비용을 단계별로 뱉는 `.frametime` 요약 두 줄**(2026-09-14). 터미널 브라우저가
    // 60Hz 를 무너뜨린 원인을 가르는 데 쓴 계측이다 — chrome 을 카드·상태바·헤더로, assemble 을
    // 픽셀복사·조립·replace 로 쪼개 어느 단계가 예산을 먹는지 한 줄로 읽는다(docs §10.6). `MARU_DEBUG`
    // 뒤에만 나오고 **표시가 아니라 로그**다. 단계 이름을 한국어로 둔 이유는 이 로그를 읽는 사람이
    // 그 문서를 함께 읽기 때문이고, 영어로 적으면 문서의 절 이름과 갈려 대조가 끊긴다.
    //
    // 34 = 33 + **침묵으로 스트리머를 다시 띄운다는 사유 하나**(2026-09-10). 좀비 자식은 EOF 를 안 줘
    // 재시작 트리거가 없었다 — 그 자리를 알리는 로그다(표시 아님).
    //
    // 33 = 31 + **control socket 실패를 두 갈래로 가른 진단 둘**(2026-09-08). 그 함수는 경로 길이 초과
    // (영원히)와 할당 실패(지금만)로 갈리는데 한 `catch` 가 둘을 뭉개, 메모리가 잠깐 모자랐다는 이유로
    // 그 목적지가 앱 수명 내내 죽었다(계획 §RA5-b 「두 실패를 반드시 가른다」). 표시가 아니라 **로그**다.
    //
    ...
    // 38~45: `scm-turn-badges` 시나리오(AT3b-2, 2026-09-20)의 턴 줄 픽스처 — 위치 이름(`마지막 턴`·`2턴 전`)·요약
    // (`4개 파일 · ✎2`·`1개 파일`)·시각(`2분 전`·`9분 전`)·응답 두 줄. `scm-blocker` 와 같은 부류다: **제품이 내는
    // 문자열을 그대로 적어야** 캡처가 제품을 예고한다(`i18n.t` 로 부르면 키 값이 바뀔 때 골든이 조용히 따라가
    // 배지 자리·색 계약이 아니라 문구를 재게 된다). 응답 두 줄은 한글 폭이 요약·시각을 밀어내지 않는지를 담는다.
    .{ .path = "src/platform/macos/chrome/lab.zig", .count = 45 },
