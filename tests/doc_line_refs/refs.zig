...
test "문서가 좌표로 지목한 심볼이 아직 그 자리에 있다" {
    ...
    //
    // **실측 6건이므로 6으로 못 박는다**(build.zig의 `--maru-expect-tests`와 같은 규율). 하한을 넉넉히
    // 낮춰 두면 인용이 조용히 사라져도 초록이라, 게이트가 지키는 범위가 줄어든 것을 아무도 모른다.
    // 숫자가 틀렸다고 나오면 먼저 **어느 인용이 사라졌는지** 확인하고, 정당한 증감일 때만 갱신한다.
    //
    // 2026-09-14: 여섯에서 다섯으로 줄었다. `plans/io-render-threading.md` 의
    // `buildDrawListWithUnfocused`(draw_list.zig:93) 가 줄끝 trim 이 그 파일에 판정 함수와 주석을
    // 더하면서 26 줄 밀렸다 — 렌더 경로의 진입점이라 그 자리는 **앞으로도 밀린다**. 게이트가 권하는
    // 대로 줄을 떼어 파일·심볼만 남겼다.
    //
    // 2026-08-30: 일곱에서 여섯으로 줄었다. `file-explorer.md` 의 `termRect`(app_session.zig:6295) 가
    // 이미지 갤러리 축이 그 파일에 줄을 더하면서 30 줄 밀렸다 — `app_session.zig` 는 모두가 만지는
    // 7만 줄 파일이라 그 자리는 **앞으로도 밀린다**. 게이트가 권하는 대로 줄을 떼어 파일·심볼만 남겼다.
    //
    // 2026-08-29: 여덟에서 일곱으로 줄었다. `sidebar-groups.md` 의 `Tab`(session_model.zig:184) 이 원격
    // 축(RA5)이 그 구조체에 필드를 더하면서 28 줄 밀렸고, 그 자리는 **앞으로도 밀린다** — 게이트가 권하는
    // 대로 좌표에서 줄을 떼어 파일·심볼만 남겼다(그러면 grep 한 번이고 다시 안 썩는다).
    //
    // 5건은 문서가 가진 좌표 전부가 아니다 — 세 관문을 통과한 것만이다(문서가 심볼 없이 좌표만 적거나
    // 심볼의 정의가 여럿이면 판정하지 않는다). 관문을 넓히는 것은 후속이며, 넓힐 때마다 오탐이 0인지를
    // 먼저 확인한다.
    try std.testing.expectEqual(@as(usize, 5), judged);
}
