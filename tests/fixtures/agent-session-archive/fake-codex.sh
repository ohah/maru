...
# Codex 는 축이 셋(승인 정책 · 샌드박스 · 모델)이라 셋 다 요구한다. 한 축만 봐도 나머지가 조용히
# 빠지는 회귀가 통과한다.
if [ "$#" -eq 8 ] && [ "$1" = "resume" ] && [ "$2" = "fixture-codex-session" ] &&
    [ "$3" = "--ask-for-approval" ] && [ "$4" = "never" ] &&
    [ "$5" = "--sandbox" ] && [ "$6" = "danger-full-access" ] &&
    [ "$7" = "--model" ] && [ "$8" = "gpt-fixture" ]; then
