...
const cases = [_]Case{
    ...
    // 쓰고 읽어 그린다.
    //
    // 여덟 행을 잡는 이유는 **세 계약이 서로 다른 줄에 있기** 때문이다: 1행은 BOM이 유령 글자로
    // 서지 않는지(파일은 BOM으로 시작한다), 1~3행은 CRLF의 `\r`이 줄 끝에 남지 않는지(§3.8이
    // 켜져 있으면 `<U+000D>`로 그려져 즉시 보인다), 5~6행은 탭이 전개되어 들여쓰기가 맞는지.
    // 마지막 8행은 파일이 개행으로 끝나 생긴 빈 줄이라, 줄 수를 하나 더 세거나 덜 세면 어긋난다.
    .{
