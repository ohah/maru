...
const cases = [_]Case{
    ...
    //
    // fixture가 줄마다 같은 열에 `|`를 두었으므로 **폭이 틀리면 그 줄의 막대만 어긋난다** — 어느 규칙이
    // 깨졌는지까지 캡처가 말한다. 헤드리스 단언으로는 "폭 계산 함수가 2를 돌려준다"까지만 확인되고,
    // 그 값이 실제 배치에 반영됐는지는 픽셀만이 답한다(폭과 배치가 갈리던 것이 바로 이 회귀였다).
    .{
        .name = "editor-wide-glyph-width",
        .capture = "editor-wide-glyph.ppm",
        .contract = "이모지·가족·스킨톤·국기·VS16·동그란 번호가 각 2칸을 차지해 줄 끝 | 가 한 열에 선다",
        .rect = .{ .x = 56, .y = 0, .w = 300, .h = 180 },
    },
};
