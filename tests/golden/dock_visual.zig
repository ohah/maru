...
const cases = [_]Case{
    ...
    .{
        ...
        // 세로 정책만 다른데(`below_clamp` 대 `at_anchor`), 그 차이를 보는 그림이 없었다. 이 crop 이
        // 보는 것은 **목록이 control 아래에 차례로 서는가** 하나다.
        //
        ...
        //
        // ⚠️ **그래서 「상자가 control 폭보다 좁아지지 않는가」(`@max(box_w, anchor.w)`)는 이 그림이
        // 답하지 못한다**(적대 2회차 실측). 그 `@max` 를 지우는 뮤테이션을 넣어도 골든은 **통과한다** —
        // 상자 폭은 배경으로만 보이고, 항목 줄을 채우는 우측 공백은 글리프가 없다. 처음에 이 crop 의
        // 계약에 그 주장을 적었다가 뮤테이션이 거짓임을 보여 뺐다. 그 계약은 `dropdown.zig` 의
        // `popupRect` 판정자가 지킨다 — **골든이 못 보는 것을 골든이 본다고 적으면 안 된다.**
        .name = "dropdown-open-below-control",
        .capture = "dropdown-open.ppm",
        .contract = "control 아래에 항목 다섯 줄이 차례로 서고, 각 줄이 좌패딩 한 칸만큼 들여진다",
        // control 글자(y≈68)부터 마지막 항목(y≈158)까지 + 사방 여백. **아래로 한 행을 더 담는다**
        // (적대 10회차): 마지막 항목 아래 10px 만 남기면 항목이 하나 늘어도 그 글자가 crop 밖이라
        // 「여기서 끝난다」를 이 그림으로 물을 수 없다. 한 행(16px)을 더 담아 **빈 줄이 보이게** 한다.
        //
