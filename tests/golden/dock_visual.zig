...
const cases = [_]Case{
    ...
    .{
        // §3.5 목업의 바닥: 브랜치·원격 줄 → 커밋 입력 → 커밋 버튼. 이 crop이 없으면 상자가 사라지거나
        // 브랜치 줄과 자리가 바뀌어도 아무 검사가 말하지 않는다.
        .name = "scm-commit-box",
        .capture = "scm-rows.ppm",
        .contract = "브랜치 줄 아래에 커밋 입력(빈 상자는 안내 문구)과 커밋 버튼이 이 순서로 선다",
        .rect = .{ .x = 0, .y = 645, .w = 480, .h = 75 },
    },
    .{
