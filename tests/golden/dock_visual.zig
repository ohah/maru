...
const cases = [_]Case{
    ...
    // 변환이 어긋나면 오른쪽 끝이 흐트러져 바로 드러난다. 폭을 80px로 좁게 잡아 본문 영역의 변화가
    // 이 판정을 흔들지 않게 한다(gutter는 8셀 = 64px이므로 여유 한 칸만 포함한다).
    .{
        .name = "editor-gutter-line-numbers",
        .capture = "editor-gutter.ppm",
        .contract = "줄 번호가 우측 정렬로 같은 오른쪽 끝에 서고, 9→10 자릿수 변화에도 정렬이 유지된다",
        .rect = .{ .x = 0, .y = 0, .w = 80, .h = 200 },
    },
    ...
    // N1 §4.1 — **배치의 셀 비례성**. 셀을 1.5배로 주면 gutter가 함께 커져 본문이 64px가 아니라
    // 96px에서 시작한다. gutter가 상수 픽셀을 쓰기 시작하면 여기서 바로 드러난다.
    //
    // **폰트 크기 연동은 이 case가 증명하지 못한다.** 실측하면 글리프가 두 캡처 모두 8×10px로 같다 —
    // 편집기 텍스트가 measured로 그려져 폰트 크기가 chrome role 토큰(pt)에 고정돼 있기 때문이다
    // (native-editor.md §2.0의 ⚠️). 셀 렌더 경로가 배선되면 이 case의 계약을 연동까지 넓힌다.
    .{
        .name = "editor-font-large",
        .capture = "editor-font-large.ppm",
        .contract = "셀을 키우면 배치가 비례해 커진다(본문 시작이 셀 폭에 비례해 밀린다). 폰트 크기 연동 자체는 아직 미검증 — native-editor.md §2.0의 ⚠️",
