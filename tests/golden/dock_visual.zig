...
const cases = [_]Case{
    ...
    .{
        ...
        // **y는 탭 줄 높이를 따라간다**(2026-08-16에 28 → 34로 늘렸다) — 고정 chrome이 자라면 아래
        // 모든 crop이 함께 내려간다. 눈으로 확인하지 않고 숫자만 두면 crop이 엉뚱한 띠를 정답으로 굳힌다.
        .rect = .{ .x = 0, .y = 34, .w = 480, .h = 62 },
    },
    .{
