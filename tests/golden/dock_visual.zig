...
const cases = [_]Case{
    ...
    .{
        ...
        //
        // 이 시나리오는 도크 뷰포트를 상태바 높이만큼 줄여 세우고 그 아래에 띠를 심는다. 목록이 자기
        // 뷰포트에서 멈추지 않으면 행이 띠를 덮어 이 crop 이 빨개진다. 띠는 **under 층**이라 도크가
        // 새는 순간 그 위에 그려진다 — 덮이면 보이지 않는 것이 아니라 **다른 그림**이 된다.
        .name = "dock-stops-above-status-bar",
        .capture = "dock-over-status-bar.ppm",
        .contract = "스크롤된 도크 목록이 상태바 띠를 덮지 않고 자기 뷰포트에서 끝난다",
