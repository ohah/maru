...
const cases = [_]Case{
    ...
    .{
        ...
        .contract = "스크롤로 밀린 행은 목록 뷰포트 위(요약 줄·탭 줄)에 그려지지 않고 그 경계에서 잘린다",
    ...
    .{
        // **목록이 자기 뷰포트를 넘지 않는다 — 위쪽 경계.** 스크롤된 목록에서 첫 행은 뷰포트 위로 반쯤
        // 잘려 있고, 그 clip 이 풀리면 잘린 절반이 위쪽 고정 chrome 위에 그려진다. 사용자 캡처로만
        // 드러났던 2026-08-21 결함이 정확히 그 그림이었다.
        //
        // 이 crop 은 **변이로 증명했다**: `ui/tree.zig` 의 scroll_area `.overflow = .clip` 을
        // `.visible` 로 바꾸면 이 구간(y 51~57, 전폭)에 979 픽셀이 새로 그려진다.
