...
const cases = [_]Case{
    ...
    .{
        // **우·하 edge_gap 에는 그림이 없었다.** 위 crop 들은 전부 앵커 (24, 24) 라 **좌·상만** 증언한다 —
        // 오른쪽·아래로 넘칠 때 상자를 당기는 경로는 판정자만 있었고, 그 둘이 뒤바뀌거나 한쪽이 0 이
        // 되어도 골든은 조용했다. 네 방향으로 넓힌 슬라이스(#3734)의 **절반이 시각 증거 없이** 있었다.
        //
        // 실측(2026-09-15, 이 crop 을 세우며): 상자 120×32 가 x[1072..1191] y[672..703] 에 서고
        // **우측 여백 8px · 하단 여백 16px** — 각각 셀 한 칸이다. 그 값이 곧 계약이다.
        //
        // 창이 1200px 인 것도 계약의 일부다(`ScenarioId.context_menu_bottom_right` 주석 ⑵). 좁은
        // 프레임에서는 같은 8px 이 폭의 1.7% 라 과장돼 읽힌다 — 「넓은 창에서 어색한가」를 사람
        // 기억이 아니라 그림으로 묻는 자리다.
        .name = "context-menu-bottom-right-gap",
        .capture = "context-menu-bottom-right.ppm",
        .contract = "우하단 모서리에서 연 메뉴가 오른쪽·아래 경계에서 각각 한 셀 떨어져 선다(우 8px·하 16px)",
        // 상자 + 네 방향 여백. 오른쪽·아래는 **프레임 끝까지** 담는다 — 여백이 crop 밖으로 나가면
        // 「닿지 않는가」를 이 그림으로 물을 수 없다.
        .rect = .{ .x = 1056, .y = 656, .w = 144, .h = 64 },
    },
    .{
        // **드롭다운에는 골든이 하나도 없었다.** `context_menu` 와 **같은 `popup_box.place`** 를 쓰면서
        // 세로 정책만 다른데(`below_clamp` 대 `at_anchor`), 그 차이를 보는 그림이 없었다. 이 crop 은
        // 평범한 자리에서의 계약 둘을 본다 — 목록이 **control 아래**에 서는가, 그리고 상자가
        // **control 폭보다 좁아지지 않는가**(`@max(box_w, anchor.w)`).
        //
        // ⚠️ **행 배경은 이 그림에 없다.** 드롭다운은 행 칠을 `.fill` op 으로 내는데 Lab 의 lowering
        // 은 `.quad` 만 내린다(컨텍스트 메뉴 시나리오가 선택 강조에서 이미 겪은 한계 — 랩의 한계이지
        // 제품 결함이 아니다). 그래서 이 골든이 고정하는 것은 **글자의 자리와 존재**다.
        .name = "dropdown-open-below-control",
        .capture = "dropdown-open.ppm",
        .contract = "control 아래에 항목 다섯 줄이 차례로 서고, 각 줄이 좌패딩 한 칸만큼 들여진다",
        // control 글자(y≈68)부터 마지막 항목(y≈158)까지 + 사방 여백.
        //
        // ⚠️ **「좌단이 같다」가 아니다**(골든을 눈으로 보다 고친 문구). 상자 좌단은 control 과 같지만
        // `viewPopup` 이 항목마다 좌패딩 한 칸을 넣어 **글자는 한 칸 들어간다** — 배경이 안 그려지는
        // 이 그림에서는 글자만 보이므로, 계약을 상자 기준으로 적으면 그림과 어긋난다.
        .rect = .{ .x = 56, .y = 56, .w = 96, .h = 112 },
    },
    .{
        // **`below_clamp` 의 본체다.** 아래 공간이 모자라면 목록을 **위로 당기되 뒤집지 않는다** —
        // 뒤집으면 목록이 control 위의 다른 행을 덮어 「저 행의 목록인가」로 읽힌다
        // (chrome-strategy.md §5.4).
        //
        // **이 시나리오가 없으면 그 정책은 그림으로 증명되지 않는다.** 점 앵커에서는 `below_clamp` 와
        // `at_anchor` 가 수학적으로 같아(`popup_box` A44) 평범한 캡처로는 구분이 안 된다 — 앵커에
        // **두께가 있고**(control 한 줄) 아래가 모자랄 때만 갈린다.
        //
        // 그림에서 마지막 항목이 control 과 **같은 줄에 겹쳐** 보이는 것이 곧 「당겼다」의 증거다.
        // 제품에서는 행 배경이 불투명해 control 을 가린다(위 crop 의 `.fill` 주석 참조).
        .name = "dropdown-bottom-clamp-not-flipped",
        .capture = "dropdown-bottom-clamp.ppm",
        .contract = "아래 공간이 모자라면 목록이 위로 당겨지고, control 위로 뒤집히지 않는다(마지막 항목이 control 줄에 겹친다)",
        // 당겨진 목록 첫 줄(y≈628)부터 **프레임 바닥까지** — 아래 여백이 crop 밖으로 나가면
        // 「경계에 닿지 않는가」를 못 묻는다.
        .rect = .{ .x = 56, .y = 616, .w = 96, .h = 104 },
    },
    .{
        // **선택 헬퍼 한 줄**(NSH — send-selection-to-agent.md §6.2). 고르고 손을 떼면 caret 아래 뜨는
        // 그 상자다. 위 셋과 **같은 컴포넌트·다른 State** 라 폭 계약도 같은 축인데, 항목이 하나뿐이라
        // 「가장 긴 줄」이 곧 유일한 줄이다 — 한글(EAW Wide) 폭 계산이 한 칸만 틀려도 여기서 드러난다.
        //
        // **골든이 없으면 캡처는 회귀 게이트가 아니다.** 아래 `scm-list-rows` 가 그 대가를 적어 두었다
        // (골든 없이 나간 «도크가 통째로 비는» 결함). 새 시각 표면을 더하면서 캡처만 붙이고 골든을
        // 안 두면 같은 자리를 다시 밟는다(적대적 29 회차).
        .name = "context-menu-send-helper-single-row",
        .capture = "context-menu-send-helper.ppm",
        .contract = "한 줄짜리 상자가 라벨을 담고, 그 줄 뒤에 우측 패딩이 남아 테두리에 닿지 않는다",
        // 보내기 메뉴와 **같은 좌우 범위**로 잘라 두 캡처를 나란히 읽을 수 있게 한다. 높이는 한 줄이다.
        .rect = .{ .x = 0, .y = 16, .w = 480, .h = 40 },
    },
    .{
    ...
    .{
        // 호버하지 **않은** 충돌 행. 동작 버튼은 호버할 때만 그려지므로 여기서는 상태 문자 `U`만 선다 —
        // 평소 목록에서 충돌 행이 조용한지를 본다.
        //
        // ⚠️ **이 crop 하나로는 충돌 행의 동작을 증언할 수 없다.** 그 사실을 S1에서 실측했다: 이 case는
        // `scm-row-hover.ppm`을 쓰는데 그 캡처가 호버하는 것은 **다른 행**(`changed.zig`)이라, 충돌 행의
        // `action` 값을 무엇으로 바꿔도 이 crop은 똑같다. 「충돌 행에는 동작이 없다」를 이것으로 지켰다고
        // 믿은 동안 그 축은 비어 있었다 — 아래 `scm-conflict-hover-resolve`가 그 자리를 맡는다.
        .name = "scm-conflict-row-quiet",
        .capture = "scm-row-hover.ppm",
        .contract = "호버하지 않은 충돌 행에는 버튼이 없다(상태 문자 `U`만 오른쪽 끝에 선다)",
        .rect = .{ .x = 240, .y = 254, .w = 240, .h = 22 },
    },
    .{
        // **충돌 행을 호버한 상태**(S1 — docs/editor-merge-conflicts.md §5). 여기서 갈리는 것이 계약이다:
        // 그 자리에 `+`가 서면 누르는 순간 `git add`가 돌아 충돌 표시가 든 파일이 "해결됨"이 되고,
    ...
    // **막힌 이유의 자리와 색**(2026-08-31 사용자 제보). 크롭이 커밋 버튼부터 두 번째 저장소 머리
    // 줄까지를 담는다 — 「버튼 바로 아래」와 「중립 안내와 색이 갈린다」와 「다음 저장소 밑엔 없다」가
    // 한 사각에서 전부 보여야 이 계약이 픽셀로 잠긴다.
    .{
        .name = "scm-blocker-under-its-repo",
        .capture = "scm-blocker.ppm",
