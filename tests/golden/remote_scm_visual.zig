...
const cases = [_]Case{
    ...
    // 원격 에이전트 탭, **에이전트 세션이 없는** pane. 한 줄로 끝나는 것이 계약이다 — 로컬 세션의 턴이
    // 그 자리에 남으면 「이 기계의 기록」으로 읽힌다(§18.6). AT3c(2026-09-21) 뒤 원격에서도 턴이 서므로
    // 이 장은 「아직 안 된다」가 아니라 **「이번 실행에서 관측한 턴이 없다」**를 고정한다 — 어디에도 기록이
    // 없는데 «다른 기계의 기록» 이라 하지 않는다.
    .{ .name = "agent-remote-no-turns", .rect = dock_column },
};
