...
test "2c4 경계는 RuntimeConnection 하나만 mode 권위로 허용한다" {
    ...
    const fields = between(product, "pub const RemoteRuntime = struct {", "fn generationConnection(") orelse
        return error.TestUnexpectedResult;
    try std.testing.expectEqual(@as(usize, 1), count(fields, "connection: RuntimeConnection,"));
    try std.testing.expectEqual(@as(usize, 0), count(fields, "client: *client_mod.Client,"));
