...
test "CR3b R2a 경계는 callback 없는 placeholder와 detached tombstone만 게시한다" {
    ...
    // prepare and exact-validation halves for the same terminal attachment/adapter identity.
    try std.testing.expectEqual(@as(usize, 5), count(runtime, ".generation => |current_adapter| if (current_adapter != adapter)"));
    try std.testing.expectEqual(
        @as(usize, 0),
        try countProductSourcesExceptTwo(
            allocator,
