//! CR5a canonical multi-runtime ledger contract and product caller-zero boundary.

const std = @import("std");
const posixWalk = @import("support/posix_walk.zig").posixWalk;
const max_source_bytes = 16 * 1024 * 1024;

test "CR5a 경계는 CR2e enum을 재사용한 canonical runtime-set contract와 제품 caller 0을 고정한다" {
    ...
    inline for (.{
        ...
        "inputAllowed",
        "host_reconnect_runtime_ledger.zig",
    ...
    const gate = between(build, "const session_host_cr5a_step =", "const b3_1_boundary_tests =") orelse
