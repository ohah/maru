...
test "CR6d 경계는 exact recovered screen probe와 actual AppKit input smoke만 연다" {
    ...
    try std.testing.expectEqual(@as(usize, 1), count(app, "pub const abi_version: u32 = 187;"));
    try std.testing.expectEqual(@as(usize, 1), count(abi, "expectEqual(@as(u32, 187), abi_version)"));
    try std.testing.expectEqual(@as(usize, 1), count(header, "#define MARU_MACOS_APP_HOST_ABI_VERSION 187u"));
