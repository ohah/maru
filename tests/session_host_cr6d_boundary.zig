...
test "CR6d 경계는 exact recovered screen probe와 actual AppKit input smoke만 연다" {
    ...
    // ABI 173 retains the read-only record. The record exposes four scalar observations and no
    // input handle, runtime pointer, or action token that Swift could use to bypass NSEvent.
    try std.testing.expectEqual(@as(usize, 1), count(app, "pub const abi_version: u32 = 173;"));
    try std.testing.expectEqual(@as(usize, 1), count(abi, "expectEqual(@as(u32, 173), abi_version)"));
    try std.testing.expectEqual(@as(usize, 1), count(header, "#define MARU_MACOS_APP_HOST_ABI_VERSION 173u"));
