...
test "CR3a-2c3d C1 event facade remains closed and product-unwired" {
    ...
    // EventCorrelation, 두 CR0b capture request, CR3c2 retired receipt, CR5b-2c shared-client
    // retention preflight receipt, registered-operation test seam과 R2a의 단일 namespace alias만 열고,
    // ClientSlot의 event take/release registry 직접 호출은 계속 닫는다.
    try std.testing.expectEqual(@as(usize, 9), count(runtime, "client_slot_mod"));
