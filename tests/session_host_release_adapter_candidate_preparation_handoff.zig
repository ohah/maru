...
const Fixture = struct {
    ...
    fn replaceWithUpgrade(self: *@This(), manifest_requirement: []const u8) !void {
        ...
        defer std.testing.allocator.free(bytes);
        try self.tmp.dir.writeFile(std.testing.io, .{ .sub_path = "manifest-root/Maru-1.2.3-session-host-release.json", .data = bytes });
        try files.pinReleaseFileObserved(&self.owners[1], self.manifestPath(), false, manifest.max_manifest_bytes);
    }
};
...
test "upgrade B evidence and role-B manifest cross the same durable handoff" {
    var fixture: Fixture = undefined;
    try fixture.init();
    defer fixture.deinit();
