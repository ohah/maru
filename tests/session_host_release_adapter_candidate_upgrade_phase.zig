...
test "success uses one deadline and preserves three evidence outputs plus predecessor copy" {
    var steps = Steps{};
    ...
    try std.testing.expectEqual(@as(usize, 9), steps.deadline_uses);
}
...
test "every operation failure cleans only attempted outputs in reverse order" {
    for (0..10) |fail_index| {
        ...
        const expected = switch (fail_index) {
            ...
            3, 4 => &[_]Event{ .cleanup_one, .cleanup_predecessor },
            5, 6 => &[_]Event{ .cleanup_near_max, .cleanup_one, .cleanup_predecessor },
            7, 8, 9 => &[_]Event{ .cleanup_evidence, .cleanup_near_max, .cleanup_one, .cleanup_predecessor },
            else => unreachable,
        };
...
test "cleanup is best effort and cleanup failure outranks execution failure" {
    inline for (.{ CleanupFailure.evidence, .near_max, .one, .predecessor }) |cleanup_failure| {
        var steps = Steps{ .fail_index = 7, .cleanup_fail = cleanup_failure };
        try std.testing.expectError(error.CleanupFailed, phase.runWith(&steps));
        try std.testing.expectEqual(@as(usize, 8), steps.operation_index);
        try std.testing.expectEqualSlices(Event, &.{ .cleanup_evidence, .cleanup_near_max, .cleanup_one, .cleanup_predecessor }, steps.cleanupEvents());
    }
}

test "final deadline failure cleans every attempted output" {
    var steps = Steps{ .fail_index = 9 };
    try std.testing.expectError(error.InjectedFailure, phase.runWith(&steps));
    try std.testing.expectEqual(@as(usize, 9), steps.deadline_uses);
    try std.testing.expectEqualSlices(Event, &.{ .cleanup_evidence, .cleanup_near_max, .cleanup_one, .cleanup_predecessor }, steps.cleanupEvents());
}

const CleanupFailure = enum { none, evidence, near_max, one, predecessor };
...
const Steps = struct {
    ...
    fn cleanupEvents(self: *const @This()) []const Event {
        for (self.events[0..self.event_count], 0..) |event, index| switch (event) {
            .cleanup_evidence, .cleanup_near_max, .cleanup_one, .cleanup_predecessor => return self.events[index..self.event_count],
            else => {},
        };
        return &.{};
    }
