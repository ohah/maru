...
const Authority = struct {
    calls: usize = 0,
    fail_on: usize = 0,
    fn revalidate(self: *@This(), _: std.mem.Allocator, path: [:0]const u8) !void {
...
const Publisher = struct {
    fail: bool = false,
    fn publish(self: *@This(), result: *draft.DraftAuthority) !void {
...
test "malformed successful response becomes remote state unknown without retry" {
    ...
    var output: [4096]u8 = undefined;
    var result: draft.DraftAuthority = .{};
    try std.testing.expectError(error.ReleaseMismatch, draft.createWith(&authority, &executor, &publisher, &deadline, std.testing.allocator, context(), "/usr/local/bin/gh", "secret", &output, &result));
