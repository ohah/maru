...
fn candidate() manifest.Manifest {
    return .{ .schema = manifest.schema, .role = .a, .repository = .{ .id = 12345, .owner = "ohah", .name = "maru" },
        .release = .{ .id = 77, .tag = "v1.2.3", .version = "1.2.3" }, .source = .{ .commit = commit, .tree = "1111111111111111111111111111111111111111" },
        ...
        .assets = &.{ .{ .role = .universal_dmg, .name = "a", .sha256 = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", .size = 1 }, .{ .role = .frozen_product_executable, .name = "b", .sha256 = "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb", .size = 1 }, .{ .role = .evidence_summary, .name = "c", .sha256 = "cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc", .size = 1 } },
        .evidence = .{ .test_uuid = "123e4567-e89b-12d3-a456-426614174000", .summary_name = "c", .summary_sha256 = "cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc", .result = "passed" } };
}

const Authenticated = struct { value_storage: manifest.Manifest, pub fn value(self: *const @This()) ?*const manifest.Manifest { return &self.value_storage; } };
const Clock = struct { now_value: i128 = 100, step: i128 = 1, pub fn now(self: *@This()) i128 { const value = self.now_value; self.now_value += self.step; return value; } };
const Authority = struct { calls: usize = 0, fail_at: usize = 0, pub fn revalidate(self: *@This(), _: std.mem.Allocator, _: [:0]const u8) !void { self.calls += 1; if (self.calls == self.fail_at) return error.ExecutableChanged; } };
...
const Executor = struct {
    ...
    pub fn capture(self: *@This(), _: []const u8, args: []const []const u8, _: []const []const u8, output: []u8, budget: i128) ![]const u8 {
        self.budgets[self.calls] = budget; self.calls += 1;
        ...
        if (std.mem.indexOf(u8, endpoint, "/git/ref/tags/") != null) {
            ...
            var index: usize = 0; while (index < shas.len and std.mem.indexOf(u8, endpoint, shas[index]) == null) : (index += 1) {}
            if (index == shas.len) return error.UnexpectedEndpoint;
            const last = index + 1 == self.depth;
            const kind = if (last) "commit" else "tag";
...
const Sink = struct {
    calls: usize = 0, expected_depth: usize,
    pub fn preflight(_: *@This(), result: *const usize) !void {
        if (result.* != 0) return error.InvalidOwner;
    }
    pub fn authenticate(self: *@This(), _: anytype, _: anytype, _: std.mem.Allocator, _: anytype, ref: anytype, tags: anytype, _: [:0]const u8, _: []const u8, _: [:0]const u8, _: []u8, _: i128, result: *usize) !void {
        self.calls += 1; try std.testing.expectEqual(self.expected_depth, tags.len); try std.testing.expectEqualStrings("v1.2.3", ref.tag);
...
fn run(depth: usize) !void {
    var auth = Authenticated{ .value_storage = candidate() }; var authority = Authority{}; var executor = Executor{ .depth = depth }; var clock = Clock{}; var sink = Sink{ .expected_depth = depth };
    var response: [65536]u8 = undefined; var result: usize = 0;
    ...
    try std.testing.expectEqual(depth + 1, result); try std.testing.expectEqual(depth + 1, authority.calls); try std.testing.expectEqual(depth + 1, executor.calls);
...
test "lightweight one-hop and eight-hop chains reach downstream exactly once" { try run(0); try run(1); try run(8); }

test "ninth annotated object is rejected before its request and downstream" {
    var auth = Authenticated{ .value_storage = candidate() }; var authority = Authority{}; var executor = Executor{ .depth = 9 }; var clock = Clock{}; var sink = Sink{ .expected_depth = 9 };
    var response: [65536]u8 = undefined; var result: usize = 0;
    ...
    try std.testing.expectEqual(@as(usize, 0), sink.calls); try std.testing.expectEqual(@as(usize, 9), executor.calls);
}

test "cycle and CLI replacement publish nothing" {
    var auth = Authenticated{ .value_storage = candidate() }; var response: [65536]u8 = undefined; var result: usize = 0;
    var authority = Authority{}; var executor = Executor{ .depth = 1, .cycle = true }; var clock = Clock{}; var sink = Sink{ .expected_depth = 1 };
    result = 1;
    ...
    try std.testing.expectEqual(@as(usize, 0), authority.calls); try std.testing.expectEqual(@as(usize, 0), executor.calls); try std.testing.expectEqual(@as(usize, 0), sink.calls);
    result = 0;
    ...
    authority = .{ .fail_at = 1 }; executor = .{ .depth = 0 }; clock = .{};
...
test "single absolute deadline expires without starting a late child" {
    var auth = Authenticated{ .value_storage = candidate() }; var authority = Authority{}; var executor = Executor{ .depth = 1 }; var clock = Clock{ .step = 60 }; var sink = Sink{ .expected_depth = 1 };
    var response: [65536]u8 = undefined; var result: usize = 0;
    ...
    try std.testing.expectEqual(@as(usize, 1), executor.calls); try std.testing.expectEqual(@as(usize, 0), sink.calls);
}
