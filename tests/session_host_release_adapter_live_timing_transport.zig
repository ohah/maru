...
const c = std.c;
const transport = @import("release_adapter_live_timing_transport");
...
test "deadline expiry after a valid download revokes provenance after cleanup" {
    ...
    try std.testing.expectError(error.TimedOut, transport.fetchWithForTest(&fixture.ops, std.testing.allocator, fixture.cli(), "token", expected(), fixture.workspace(), &fixture.metadata_buffer, &fixture.deadline, &fixture.result));
    try std.testing.expect(fixture.result.value() == null);
    try std.testing.expect(!try pathExists(fixture.workspace()));
}
...
test "undersized metadata storage is rejected before filesystem and child access" {
    ...
    try std.testing.expectError(error.InvalidInput, transport.fetchWithForTest(&fixture.ops, std.testing.allocator, fixture.cli(), "token", expected(), fixture.workspace(), &short, &fixture.deadline, &fixture.result));
    try std.testing.expectEqual(@as(usize, 0), fixture.ops.revalidations);
    try std.testing.expect(!try pathExists(fixture.workspace()));
}
...
test "production transport runs an exact clean-environment child into the held archive fd" {
    ...
    try transport.fetch(std.testing.io, std.testing.allocator, .{ .path = cli_path, .pinned = &pinned }, "token", expected(), fixture.workspace(), &fixture.metadata_buffer, 10 * std.time.ns_per_s, &fixture.result);
    try std.testing.expectEqual(@as(u64, 987), fixture.result.value().?.artifact_id);
    try std.testing.expect(!try pathExists(fixture.workspace()));
}
...
test "transport source has no shell PATH token persistence or generic downloader" {
    const source_text = try std.Io.Dir.cwd().readFileAlloc(std.testing.io, "src/platform/macos/session_host/release_adapter_live_timing_transport.zig", std.testing.allocator, .limited(128 * 1024));
...
const FakeOperations = struct {
    ...
    pub fn capture(self: *@This(), _: [:0]const u8, token: []const u8, args: []const []const u8, output: []u8, _: i128) ![]const u8 {
        ...
        self.list_shape_ok = std.mem.eql(u8, token, "token") and args.len == 10 and
            ...
            std.mem.eql(u8, args[9], "repos/ohah/maru/actions/runs/333/artifacts?per_page=100&name=session-host-release-live-timing-2");
        if (self.metadata.len > output.len) return error.OutputTooLarge;
        @memcpy(output[0..self.metadata.len], self.metadata);
        return output[0..self.metadata.len];
    }
    ...
    pub fn download(self: *@This(), _: [:0]const u8, token: []const u8, args: []const []const u8, fd: c.fd_t, expected_size: u64, _: i128) !@import("bounded_process").Digest {
        ...
        self.download_shape_ok = std.mem.eql(u8, token, "token") and args.len == 10 and
            std.mem.eql(u8, args[9], "repos/ohah/maru/actions/artifacts/987/zip");
...
const Fixture = struct {
    ...
    fn run(self: *@This()) !void {
        ...
        try transport.fetchWithForTest(&self.ops, std.testing.allocator, self.cli(), "token", expected(), self.workspace(), &self.metadata_buffer, &self.deadline, &self.result);
    }
...
fn expectProductionFailure(mode: ProcessFailure, expected_error: anyerror, budget_ns: i128) !void {
    ...
    const hex = std.fmt.bytesToHex(digest, .lower);
    const pinned = try cli_authority.pin(std.testing.allocator, cli_path, &hex);
    try std.testing.expectError(expected_error, transport.fetch(std.testing.io, std.testing.allocator, .{ .path = cli_path, .pinned = &pinned }, "token", expected(), fixture.workspace(), &fixture.metadata_buffer, budget_ns, &fixture.result));
    try std.testing.expect(fixture.result.value() == null);
    try std.testing.expect(!try pathExists(fixture.workspace()));
}

fn expected() artifact.Expected {
    return .{ .repository_id = 1257870483, .run_id = 333, .run_attempt = 2, .source_sha = source };
}
