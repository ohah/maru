//! session-host release adapter 판정자 73 개를 **한 바이너리로** 모은다 — macOS 에서만(`if (target.result.os.tag == .macos)` 안 — 이 가족들은 원래 그 가드 안에 있었다)에서 이 파일이 쓰인다.
...
test {
    ...
    _ = @import("session_host_release_adapter_candidate_aggregate_retention.zig");
    _ = @import("session_host_release_adapter_candidate_aggregate_cleanup_recovery.zig");
    ...
    _ = @import("session_host_release_adapter_candidate_resume_publication_command.zig");
    ...
    _ = @import("session_host_release_adapter_live_workflow_aggregate_child.zig");
