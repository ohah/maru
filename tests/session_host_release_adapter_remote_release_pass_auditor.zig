//! Product boundary for one protected-run remote pass audit.

const std = @import("std");
const verifier = @import("release_adapter_remote_release_pass_auditor");
...
test "closed verifier command accepts only pinned CLI digest and absent absolute workspace" {
    const parsed = try verifier.parse(&.{ "audit", "/opt/hostedtoolcache/gh", "0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef", "/private/tmp/maru-remote-pass-audit-verifier" });
    try std.testing.expectEqualStrings("/opt/hostedtoolcache/gh", parsed.cli_path);
    try std.testing.expectError(error.InvalidArguments, verifier.parse(&.{"audit"}));
    ...
    try std.testing.expectError(error.InvalidPath, verifier.parse(&.{ "audit", "gh", "0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef", "/private/tmp/x" }));
    try std.testing.expectError(error.InvalidSha256, verifier.parse(&.{ "audit", "/gh", "ABC", "/private/tmp/x" }));
}
...
test "product verifier derives current identity and consumes an actual pinned child transport" {
    ...
    try verifier.audit(std.testing.io, std.testing.allocator, context(), runner(), .{ .cli_path = cli_path, .cli_sha256 = &hex, .workspace_path = workspace }, "token", &metadata_buffer, 10 * std.time.ns_per_s);
...
test "context and runner mismatch fail before CLI or workspace access" {
    ...
    try std.testing.expectError(error.UnprotectedRef, verifier.audit(std.testing.io, std.testing.allocator, bad_context, runner(), command, "token", &metadata_buffer, std.time.ns_per_s));
    var foreign = runner();
    foreign.workflow_sha[0] = 'f';
    try std.testing.expectError(error.ContextMismatch, verifier.audit(std.testing.io, std.testing.allocator, context(), foreign, command, "token", &metadata_buffer, std.time.ns_per_s));
}

test "auditor source owns no output or mutable GitHub operation" {
    const source_text = try std.Io.Dir.cwd().readFileAlloc(std.testing.io, "src/platform/macos/session_host/release_adapter_remote_release_pass_auditor.zig", std.testing.allocator, .limited(64 * 1024));
...
test "product CLI reads one token after trusted context and runner and wipes its bounded copy" {
    const source_text = try std.Io.Dir.cwd().readFileAlloc(std.testing.io, "tools/session-host/release_remote_pass_auditor_cli.zig", std.testing.allocator, .limited(64 * 1024));
    defer std.testing.allocator.free(source_text);
    ...
    const verify_at = std.mem.indexOf(u8, source_text, "auditor.audit(") orelse return error.MissingVerifierCall;
