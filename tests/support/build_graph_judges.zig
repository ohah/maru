...
test "표도 등록이다 — 루프 한 줄 뒤의 스무 건을 세어 둔다" {
    ...
    // 판정자 스물두 개가 `boundary_scans` 표에 있고, 그 표를 도는 루프가 전부 매단다.
    // 호출만 보는 눈에는 「매다는 줄 하나」로 보이므로, 개수는 여기서 지킨다.
    try std.testing.expectEqual(@as(usize, 22), g.countTableRows("boundary_scans"));
    ...
    try std.testing.expectEqual(@as(usize, 21), default_optimize);

    // 모듈을 주입받는 행 다섯. 이름이 `boundary_scan_modules` 에 없으면 빌드가 죽으므로
    ...
    try std.testing.expectEqual(@as(usize, 5), with_deps);
}
