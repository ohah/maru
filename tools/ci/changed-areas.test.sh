...
expect "code=false web=false docs=true" docs/architecture.md
expect "code=false web=false docs=true" AGENTS.md .claude/settings.json LICENSE

# Zig 변경은 Zig 게이트만 — web:check는 이 경로를 읽지 않는다.
expect "code=true web=false docs=false" src/core.zig
expect "code=true web=false docs=false" build.zig tests/boundary/imports.zig tools/perf/core.zig

# web 변경은 web 게이트만 — Zig 게이트는 이 경로를 읽지 않는다.
expect "code=false web=true docs=false" web/src/main.ts
expect "code=false web=true docs=false" web/package.json web/bun.lock
...
expect "code=true web=true docs=false" tools/ci/changed-areas.sh
...
if [ "${noargs% }" = "code=true web=true docs=true" ]; then
...
if [ "${badsha% }" = "code=true web=true docs=true" ]; then
