...
# (게이트가 조용히 열린다), 과대 판정은 문서 한 줄에 macOS 러너 세 대를 태운다. 특히 fail-safe 경로
...
#
# 이 축이 없으면 문서 경로를 가리키는 주석 한 줄에 macOS 러너 세 대가 돈다. 반대로 판정이 느슨해
# 코드 변경을 주석으로 오인하면 검증 없이 머지되므로, 아래 네 케이스로 경계를 못박는다.

# doc comment만 고쳤다 — 문서 분할에서 실제로 반복된 모양이다.
expect_edit "code=true runtime=false web=false packages=true docs=false" src/mod.zig \
	'/// docs/old.md §3 참조.
pub const x = 1;
' \
	'/// docs/new.md §3 참조.
...
expect_edit "code=true runtime=true web=false packages=true docs=false" src/mod.zig \
	'// 설명.
pub const x = 1;
' \
	'// 설명을 고쳤다.
...
expect_edit "code=true runtime=false web=false packages=true docs=false" src/platform/macos/Host.swift \
	'// docs/old.md §2.
let x = 1
' \
	'// docs/new.md §2.
...
expect_edit "code=true runtime=true web=false packages=true docs=false" terminfo/maru.src \
	'a
' \
	'b
'
...
expect "code=false runtime=false web=false packages=false docs=true" docs/architecture.md
expect "code=false runtime=false web=false packages=false docs=true" AGENTS.md .claude/settings.json LICENSE

# Zig 변경은 Zig 게이트만 — web:check는 이 경로를 읽지 않는다.
expect "code=true runtime=true web=false packages=true docs=false" src/core.zig
expect "code=true runtime=true web=false packages=true docs=false" build.zig tests/boundary/imports.zig tools/perf/core.zig

# web 변경은 web 게이트만 — Zig 게이트는 이 경로를 읽지 않는다.
expect "code=false runtime=false web=true packages=false docs=false" web/src/main.ts
expect "code=false runtime=false web=true packages=false docs=false" web/package.json web/bun.lock
...
# packages 는 켜지 않는다 — 이 문서는 config 스키마에만 박히고, wasm 코어 클로저(terminal·width·
# grapheme·color·path_shape)에는 config 가 들어가지 않는다.
expect "code=true runtime=true web=false packages=false docs=false" docs/configuration.md

# 파이프라인 정의는 어느 축에 영향을 줄지 파일 안을 봐야 알므로 둘 다 켠다.
expect "code=true runtime=true web=true packages=true docs=false" .mise.toml
expect "code=true runtime=true web=true packages=true docs=false" .github/workflows/ci.yml
expect "code=true runtime=true web=true packages=true docs=false" tools/ci/changed-areas.sh

# 분류되지 않은 새 경로는 fail-safe로 전부 실행한다 — 목록을 갱신하지 않아도 게이트가 열리지 않는다.
expect "code=true runtime=true web=true packages=true docs=false" newtop/thing.txt

# 비ASCII 경로. git 기본 core.quotePath는 `"docs/\355\225\234.md"`로 내보내 앞의 따옴표가 docs/* 매칭을
# 깨뜨린다 — 문서 하나에 macOS 러너가 전부 도는 과대 판정이 된다.
expect "code=false runtime=false web=false packages=false docs=true" "docs/한글문서.md"
...
if [ "${renamed% }" = "code=true runtime=true web=false packages=true docs=true" ]; then
...
if [ "${noargs% }" = "code=true runtime=true web=true packages=true docs=true" ]; then
...
if [ "${badsha% }" = "code=true runtime=true web=true packages=true docs=true" ]; then
