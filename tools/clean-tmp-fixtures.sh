...
# - **쓰는 프로세스가 살아 있으면 남긴다**(`ps` 로 본다 — 이름을 믿기 전에 이것이 먼저다).
# - 이름 끝의 pid 가 **살아 있으면 남긴다**. 지금 돌고 있는 테스트의 자리일 수 있다.
#   ⚠️ 그 검사는 **이 프로세스가 아는 것만** 본다 — 다른 러너가 동시에 돌면서 그 **자식**이 이미
#   죽었고 부모가 아직 그 자리를 검사 중이면, 여기서는 「죽었다」로 읽혀 지운다. 손으로 부르던
#   시절에는 드물었지만 러너가 매번 부르면서 빈도가 올라갔다(`run-test-shards.sh`). 고치려면
#   자리마다 잠금이 필요해 지금은 **알고 받아들인다** — 실패하면 그 실행만 다시 돌리면 된다.
...
# 🔥 **이름의 숫자를 pid 로 믿으면 살아 있는 자리를 지운다**(적대적 F1, 2026-09-11).
#
# `maru-product-test-230343823-cr6e-c3c` 같은 이름이 있다. 그 `230343823` 은 pid 가 아니라 다른
# 번호인데 앞에서부터 훑는 규칙이 그것을 pid 로 읽고, 그런 pid 가 없으니 **「죽었다」로 판정해
# 지운다** — 실제로 그 자리를 쓰는 `maru __session-host` 가 **살아 있는 채로 고아가 됐다**(실측:
# 디렉터리는 사라졌는데 프로세스는 `ps` 에 그대로 있었다).
#
# 그래서 이름을 믿기 전에 **실제로 쓰는 프로세스가 있는지** 본다. `ps` 를 한 번만 찍어 명령줄에
# 나온 `/tmp/maru-*` 뿌리를 모으고, 그 목록에 있으면 무조건 남긴다. 이름 규칙(`kill -0`)은 그
# 뒤의 보조 판정이다.
live_roots=$(
    ps -eo args= 2>/dev/null |
        tr ' ' '\n' |
        sed -n -e 's|^/private/tmp/|/tmp/|' -e 's|^\(/tmp/maru-[^/]*\).*|\1|p' |
        sort -u
)
live_list=$(printf '\n%s\n' "$live_roots")
...
for path in /tmp/maru-*; do
  ...
  # **쓰는 프로세스가 살아 있으면 무조건 남긴다** — 이름 규칙보다 이것이 먼저다.
  case "$live_list" in
    *"
$path
"*) kept_alive=$((kept_alive + 1)); continue ;;
  esac
  ...
  # ⚠️ **`maru-t<pid>` 는 하이픈이 없어 아래 칸 훑기가 못 읽는다.** 그 모양은 session host 의
  # **테스트 루트**다(`short_endpoint.zig` — 테스트 빌드가 공용 `maru-<uid>` 를 절대 안 쓰도록
  # pid 로 가른 자리). 못 읽으면 「pid 를 못 읽으면 남긴다」 규칙에 걸려 **영영 안 지워진다** —
  # 실측 2026-09-11 에 남은 자리의 **94%(2,710 개)** 가 이 모양이었고 그 안에 `session-host/` 와
  # `sh/` 가 든 채였다. pid 재사용이 그 죽은 manifest 를 **살아 있는 host** 로 읽는 바로 그 자리다.
  #
  # 이름 **전체**가 `maru-t<숫자>` 일 때만 읽는다 — `maru-tomb-1234` 처럼 하이픈이 있는 태그는
  # 아래 칸 훑기가 이미 옳게 다룬다.
  case "$name" in
    maru-t*)
      rest=${name#maru-t}
      case "$rest" in
        ''|*[!0-9]*) ;;
        *) if [ "$rest" -ge 2 ] 2>/dev/null; then pid=$rest; fi ;;
      esac
      ;;
  esac

  cand=$name
  while [ -z "$pid" ] && [ -n "$cand" ]; do
    head=${cand%%-*}
    case "$head" in
      ''|*[!0-9]*) ;;
