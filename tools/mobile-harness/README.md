...
**누르고 있는 제스처도 스크립트로 만들 수 있다** — `sim_input.swift hold x y ms` 를 넣었다
(`mouseDown` → 기다린다 → `mouseUp`, 2px 만 흔든다). `idb` 가 못 하는 것이지 CGEvent 가 못 하는
것이 아니었다. 길게 누름은 프레임에서 판정되므로 누른 채 기다리기만 하면 된다.

**다만 시뮬레이터가 합성 마우스를 아예 안 받는 상태가 된다.** 한 세션 안에서 탭·드래그가 잘
되다가, 앱을 다시 설치한 뒤로 **앱은 물론 OS 홈 제스처까지** 무반응이 됐다(2026-08-17 실측:
커서는 창 위 정확한 지점으로 움직이는데 화면은 픽셀 하나 안 바뀐다 — posting 은 정상이고
시뮬레이터가 안 받는 것이다). 기기 재부팅·Simulator 재시작으로도 안 돌아왔다. **판정 방법**:
홈 제스처(`drag 200 870 200 500`)를 보내 화면이 안 바뀌면 그 세션에서는 CGEvent 로 아무것도
검증할 수 없다 — 앱 결함으로 오해하지 말 것. 사람이 창을 한 번 클릭해 주면 대개 풀린다.
