...
# **M12 의 「미착수」를 숫자로 못 박는다.** 계획서가 「브리지가 `UiActionId` 를 한 번도 안 쓴다」
# 를 그 슬라이스의 «남은 일»의 근거로 삼는다 — 그 말이 참이 아니게 되는 순간(= M12 를 시작하는
# 순간) 여기가 붉어져 계획서를 함께 고치게 된다. 산문 속 숫자는 아무도 안 본다.
ck "브리지가 UiActionId 를 안 쓴다" 0 "$(grep -c 'UiActionId' src/platform/mobile/mobile_bridge.zig)"
# **계약이 그 반대를 «사실»로 적고 있었다**(적대적 검증 9회차): 「chrome 이 UiActionId +
# snapshot_generation 계약을 갖고 있고 **모바일도 그 경로를 쓴다**」. 코드는 0건인데 아무도
# 안 물었다. 위 판정자와 **짝으로** 묶는다 — 브리지가 안 쓰는 동안 계약이 쓴다고 말하면 붉어지고,
# M12 가 실제로 태우면 위 판정자가 먼저 붉어져 이 자리도 함께 고치게 된다.
# **위양성은 「이력 서술」이다** — 옛 문구를 그대로 인용하면 문다(실제로 처음에 그렇게 물렸다).
# 이력은 인용하지 말고 풀어 쓴다.
ck "계약이 모바일도 그 경로를 쓴다고 안 적는다" 0 "$(sed '/<!--/,/-->/d' docs/mobile-platform.md | grep -c '모바일도 그 경로를 쓴다')"
# **문단이 통째로 사라지는 것도 막는다** — 위 판정자만 두면 그 절을 지워도 초록이다.
ck "계약이 안 쓴다고 «말한다»" 1 "$(grep -c '한 번도 안 쓴다' docs/mobile-platform.md)"
