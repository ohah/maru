...
# **M12 는 닫혔다 — 그리고 그것을 못 잡은 판정자를 여기서 고친다.**
#
# 옛 판정자 셋은 이렇게 짜여 있었다: ① 브리지에 `UiActionId` 라는 **글자**가 0 건인지 세고,
# ② 계약이 그 반대를 말하면 물고, ③ 계약이 「한 번도 안 쓴다」고 **말하는지**(=1) 물었다.
# 뜻은 「M12 를 시작하는 순간 붉어져 문서를 함께 고치게 한다」였는데 **한 번도 안 붉었다.**
#
# 두 가지가 어긋났다. **㉠ ①이 «이름» 을 셌다** — 브리지는 표를 세우면서 자기 이름(`UiTable`·
# `registerAction`)을 썼고 chrome 의 타입 이름은 안 적었다. 그래서 M12a~c 가 다 끝나도 0 이었다.
# **㉡ ③이 낡은 문장을 «있어야 한다» 로 못 박았다** — 코드가 바뀐 뒤에도 그 문장을 지우면
# 붉어지므로, 판정자가 **문서를 낡은 채로 붙들고 있었다.**
#
# 그래서 이름이 아니라 **하는 일**을 센다(오늘 M2-f2 가 배운 것과 같은 규칙).
# **맞히는 자리**는 위 §3.1 의 「맞히는 자리는 하나다」가 이미 센다. 여기서는 **예외**를 센다 —
# 좌표로 고르는 자리가 표 말고 하나(선택 팝업)뿐임을 박아, 둘째가 생기면 붉어지게 한다.
# **컬렉션 이름이 아니라 «자리의 수» 를 센다.** 처음에는 팝업 배열의 이름을 셌는데, 같은 배열에
# 두 번째 히트 테스트를 붙이는 변이가 **그대로 통과했다**(적대적 검증). 좌표로 맞히는 호출을
# 통째로 세면 새 자리가 하나라도 늘 때 붉어진다. 지금 일곱이고 그 내역은 이렇다:
#   · `hitAction` 의 표 훑기 1 — **제품이 버튼을 고르는 유일한 자리**
#   · 선택 팝업 1 — 계약이 적어 둔 의도적 예외(뗄 때 손가락 밑의 항목)
#   · 조회 5 — `settingsRowAt` 1 · `terminalBackHitAt` 1 · `terminalChromeHitAt` 3.
#     전부 판정자가 쓰고 제품 포인터 경로는 안 부른다.
# **줄이 아니라 «발생» 을 센다** — 한 줄에 둘을 쓰면 줄 세기로는 못 본다(적대적 검증에서 샜다).
ck "좌표로 맞히는 자리는 일곱이다" 7 "$(sed 's,//.*,,' src/platform/mobile/mobile_bridge.zig | grep -v 'fn setHit' | grep -o 'setHit(' | wc -l | tr -d ' ')"
# **계약이 지금 사실을 «말한다».** 문단이 통째로 사라지는 것도 막는다.
ck "계약이 같은 표를 쓴다고 말한다" 1 "$(grep -c '모바일도 chrome 과 «같은 기계»를 쓴다' docs/mobile-platform.md)"
ck "계약이 그 예외 둘을 적는다" 1 "$(grep -c '좌표로 버튼을 고르는 자리는 둘뿐이다' docs/mobile-platform.md)"
# **낡은 말이 안 남았다.** ③ 이 「있어야 한다」로 붙들던 바로 그 문장이다 — 방향을 뒤집는다.
ck "계약이 낡은 말을 안 한다" 0 "$(grep -c '한 번도 안 쓴다' docs/mobile-platform.md)"
...
# **커서는 터미널 셀 비율이다** — 이 수가 갈리면 「블록 커서」가 그냥 사각형이 된다. 실제로
# 정사각에 가깝게 눕혀 봤더니 커서로 안 읽혔다(그 판은 버렸다). 문서와 코드를 묶어 둔다.
ck "커서가 셀 비율이다" 1 "$(grep -c 'ch / 1.9' $G)"
ck "그 비율이 문서에도 있다" 1 "$(grep -c '1:1.9' docs/distribution.md)"
# **프롬프트 셰브론이 있다** — 「마」 느낌을 좇다가 이걸 버리면 터미널이라는 뜻이 통째로 빠진다.
...
echo "UX 계약이 «내린 결정» 을 든다 (U0)"
UXD=docs/mobile-ux.md
# **사용자가 내린 결정은 계약이 현재형으로 들어야 한다.** 「덮개」는 2026-09-04 에 정해졌는데
# 계약은 그 뒤로도 **「안 정했다」** 를 들고 있었고, 그 자리가 「지금 구현은 탭 줄」이라는 **거짓**
# 까지 함께 말하고 있었다(U3 이 그 탭 줄을 걷어낸 뒤였다). 결정이 계획서에만 있으면 누군가
# — 나 자신을 포함해 — 그것을 **다시 연다**.
ck "계약이 덮개를 현재형으로 든다" 1 "$(grep -c '세션을 바꾸는 길은 목록 하나다' $UXD)"
# **되살아나는 것도 막는다** — 「안 정했다」가 다시 적히면 붉어진다(방향을 양쪽으로 건다).
ck "계약이 그것을 «안 정했다» 고 안 한다" 0 "$(grep -c '세션 전환의 손짓\*\*을 안 정했다' $UXD)"
# **없어진 것을 「있다」고 말하지 않는다.** 상단 탭·좌측 사이드바는 U3 이 걷어냈다.
ck "브리지에 탭 줄이 없다" 0 "$(sed 's,//.*,,' src/platform/mobile/mobile_bridge.zig | grep -cE 'tab_bar|drawTabs|tab_rects')"
ck "계약도 그렇게 적는다" 1 "$(grep -c '탭·사이드바는 \*\*없어졌다\*\*' $UXD)"

echo "계획서의 «상태» 가 서술절·코드와 맞는가"
PLAN=docs/plans/mobile-platform.md
# **한 문서 안에서 상태가 두 곳에 산다** — 표의 상태 칸과 아래 서술절. 측정은 서술절에 쌓이는데
# 사람이 먼저 읽는 것은 표라, 표만 낡으면 **끝난 일을 다시 하러 간다**(실제로 M12·U2 에서 그랬다:
# U2 행은 「남은 것은 실기 확인뿐」인데 서술절에는 그 실기 측정표가 2026-09-04 자로 있었다).
#
# 산문끼리의 모순은 grep 으로 못 잡는다. **코드로 잴 수 있는 것만** 못 박는다.
#
# **① 없어진 것을 「미달」이라고 말하지 않는다.** 상단 탭 줄은 U3 이 걷어냈다(브리지에 0건).
ck "계획서가 «탭 34 미달» 을 안 말한다" 0 "$(grep -c '탭 34는 여전히 미달' $PLAN)"
# **② bounce 는 들어왔다** — 코드에 `overshoot_px` 축이 있고, 계획서가 그것을 「남았다」고 말하면 안 된다.
ck "bounce 축이 코드에 있다" 1 "$(grep -c 'overshoot_px: i32' src/chrome/ui/scroll_area.zig)"
ck "계획서가 bounce 를 «남았다» 고 안 한다" 0 "$(grep -c 'bounce\` 가 남았다' $PLAN)"
# **③ U2 의 실기 확인은 끝났다** — 서술절이 측정표를 든다. 행이 그 반대를 말하면 붉어진다.
ck "계획서가 U2 를 «실기 확인뿐» 이라 안 한다" 0 "$(grep -c '남은 것은 실기 확인뿐' $PLAN)"
...
echo "계획서가 «열린 것» 을 한곳에 든다"
# **상태가 두 곳에 살면 한쪽이 낡는다** — 표의 상태 칸과 아래 서술절. 그 드리프트가 하루에 세 번
# 났고(M12·U2·U1) 그중 하나는 「다음에 뭘 할까」 판단을 틀리게 만들었다. 행이 스스로 «열림»·«막힘»
# 을 선언하게 하고, 그 집합을 머리말 목록과 대조한다 — 산문끼리는 못 대조해도 **이것은 된다.**
"$PY" "$(dirname "$0")/plan_open_slices.py" > /tmp/maru_open.$$ 2>&1
ck "열린 것 ↔ 머리말 목록" 0 "$?"
grep -E "^  (어휘|누락|잉여)" /tmp/maru_open.$$ || true
rm -f /tmp/maru_open.$$
