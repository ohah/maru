...
//
// **이 게이트가 실제로 잡는 크기**(적대적 검증 실측 — 제품 코드에 진짜 회귀를 주입해 쟀다).
// `buildGpuImages` 의 배치당 루프에 여분 할당(64B alloc+free)을 넣고 `kitty_image_pipeline` 을 봤다:
//
//   기하 계산 3배            324 -> 330ms (+2%)   — 기하는 이 벤치의 2% 다
//   배치당 할당 1회          324 -> 553ms (1.7배) — **안 잡힌다**(CI 환산 49%)
//   배치당 할당 4회          324 -> 1057ms (3.3배) — CI 환산 94%, 경계
//
// 즉 감지 한계는 **약 3.5배**다. 주석이 「alloc 구조가 퇴화하면 잡는다」고 말하지만, 할당 **하나**가
// 늘어나는 정도는 못 잡는다 — 그건 러너 변동을 흡수하려고 둔 여유의 대가다. 더 촘촘히 잡으려면
// 예산을 조일 게 아니라 기준선 대비 비교 같은 다른 장치가 필요하다(별건).
