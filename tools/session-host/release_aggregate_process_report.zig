...
pub const schema = "maru.session-host-release-aggregate-process-perf.v2";
...
pub fn render(buffer: []u8, value: Report) ![]const u8 {
    return std.fmt.bufPrint(buffer, "{{\"schema\":\"{s}\",\"profile\":\"{s}\",\"iterations\":{d},\"successful_pairs\":{d},\"distinct_pid_pairs\":{d}," ++
        ...
        value.schema,             value.profile,            value.iterations,      value.successful_pairs,      value.distinct_pid_pairs,
        value.prepare_ns.median,  value.prepare_ns.p95,     value.prepare_ns.max,  value.handoff_gap_ns.median, value.handoff_gap_ns.p95,
        value.handoff_gap_ns.max, value.finalize_ns.median, value.finalize_ns.p95, value.finalize_ns.max,       value.total_ns.median,
        value.total_ns.p95,       value.total_ns.max,       value.failures,        value.parent_fd_delta,       value.aggregate_residue,
        value.staging_residue,
    });
}
...
pub fn parseCanonical(allocator: std.mem.Allocator, bytes: []const u8) !std.json.Parsed(Report) {
    ...
    if (!std.mem.eql(u8, value.schema, schema) or
        (!std.mem.eql(u8, value.profile, "baseline_a") and !std.mem.eql(u8, value.profile, "upgrade_b")) or
        (!std.mem.eql(u8, value.profile, "baseline_a") and !std.mem.eql(u8, value.profile, "upgrade_b")) or
        value.iterations == 0 or value.iterations > max_iterations or
