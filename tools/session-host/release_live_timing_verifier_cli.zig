//! Zero-output product entrypoint for one protected-run live timing verification.
...
fn mainFallible(init: std.process.Init) !void {
    ...
    try verifier.verify(init.io, init.gpa, trusted, runner, command, token_storage[0..token.len], &metadata_buffer, 5 * std.time.ns_per_min);
}
