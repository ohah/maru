...
pub const schema = "maru.session-host-release-workflow-command-process-perf.v2";
...
pub fn render(buffer: []u8, value: Report) ![]const u8 {
    return std.fmt.bufPrint(buffer, "{{\"schema\":\"{s}\",\"iterations\":{d},\"successful_runs\":{d}," ++
        ...
        "\"failures\":{d},\"profile_failures\":{d},\"child_pid_collisions\":{d},\"parent_fd_delta\":{d}," ++
        "\"checkpoint_residue\":{d},\"profile_checkpoint_residue\":{d}}}", .{
        value.schema,
        value.iterations,
        value.successful_runs,
        value.profile_successful_runs,
        value.draft_authoring_ns.median,
        value.draft_authoring_ns.p95,
        value.draft_authoring_ns.max,
        value.profile_draft_authoring_ns.median,
        value.profile_draft_authoring_ns.p95,
        value.profile_draft_authoring_ns.max,
...
pub fn parseCanonical(allocator: std.mem.Allocator, bytes: []const u8) !std.json.Parsed(Report) {
    ...
    const profile_accounted = std.math.add(u64, value.profile_successful_runs, value.profile_failures) catch return error.InvalidReport;
    if (!std.mem.eql(u8, value.schema, schema) or value.iterations == 0 or value.iterations > max_iterations or
        accounted != value.iterations or profile_accounted != value.iterations or
        !validTimes(value.draft_authoring_ns) or !validTimes(value.profile_draft_authoring_ns) or !validTimes(value.aggregate_prepare_ns) or
