//! Fresh-process product smoke for the protected-run remote pass auditor.
...
pub fn main(init: std.process.Init) !void {
    ...
    const template = try std.fmt.bufPrintZ(&root_storage, "/private/tmp/maru-remote-pass-auditor.XXXXXX", .{});
...
fn run(io: std.Io, allocator: std.mem.Allocator, executable: []const u8, cli_path: []const u8, digest: []const u8, workspace: []const u8, environment: *const std.process.Environ.Map, want_success: bool) !void {
    const result = try std.process.run(allocator, io, .{
        .argv = &.{ executable, "audit", cli_path, digest, workspace },
