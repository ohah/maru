...
test "$(printf '%s\n' "$third_party_uses" | sed '/^$/d' | wc -l | tr -d ' ')" = 30
...
test "$(printf '%s\n' "$action_uses" | grep -Fxc 'actions/checkout@34e114876b0b11c390a56381ad16ebd13914f8d5')" = 8
test "$(printf '%s\n' "$action_uses" | grep -Fxc 'jdx/mise-action@c37c93293d6b742fc901e1406b8f764f6fb19dac')" = 8
test "$(printf '%s\n' "$action_uses" | grep -Fxc 'actions/upload-artifact@ea165f8d65b6e75b540449e92b4886f43607fa02')" = 8
test "$(printf '%s\n' "$action_uses" | grep -Fxc 'actions/download-artifact@d3f86a106a0bac45b974a628896c90dbdf5c8093')" = 4
test "$(printf '%s\n' "$action_uses" | grep -Fxc 'actions/attest-build-provenance@43d14bc2b83dec42d39ecae14e916627a18bb661')" = 2
test "$(grep -c 'persist-credentials: false' "$workflow")" = 8
