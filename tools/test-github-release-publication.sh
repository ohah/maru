...
test "$(printf '%s\n' "$third_party_uses" | sed '/^$/d' | wc -l | tr -d ' ')" = 6
...
test "$(printf '%s\n' "$action_uses" | grep -Fxc 'actions/checkout@34e114876b0b11c390a56381ad16ebd13914f8d5')" = 2
test "$(printf '%s\n' "$action_uses" | grep -Fxc 'jdx/mise-action@c37c93293d6b742fc901e1406b8f764f6fb19dac')" = 2
test "$(printf '%s\n' "$action_uses" | grep -Fxc 'actions/upload-artifact@ea165f8d65b6e75b540449e92b4886f43607fa02')" = 2
test "$(grep -c 'persist-credentials: false' "$workflow")" = 2
