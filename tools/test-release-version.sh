...
sh "$renderer" render "$template" "$tmp/Info.plist" 1.2.3
test "$(grep -c '<string>1\.2\.3</string>' "$tmp/Info.plist")" = 1
! grep -q '@@MARU_VERSION@@' "$tmp/Info.plist"

if sh "$renderer" render "$template" "$tmp/bad.plist" '1.2.3/escape' >/dev/null 2>&1; then
    echo 'error: invalid version was accepted' >&2
    exit 1
fi
if sh "$renderer" render "$template" "$tmp/prerelease.plist" '1.2.3-beta.1' >/dev/null 2>&1; then
    echo 'error: prerelease version invalid for CFBundleShortVersionString was accepted' >&2
    exit 1
fi
if sh "$renderer" render "$template" "$tmp/leading-zero.plist" '01.2.3' >/dev/null 2>&1; then
...
if sh "$renderer" render "$tmp/duplicate.in" "$tmp/duplicate.plist" 1.2.3 >/dev/null 2>&1; then
    echo 'error: duplicate plist placeholder was accepted' >&2
    exit 1
fi

sed 's/@@MARU_VERSION@@/1.2.3/' "$template" > "$tmp/missing.in"
if sh "$renderer" render "$tmp/missing.in" "$tmp/missing.plist" 1.2.3 >/dev/null 2>&1; then
