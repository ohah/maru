...
def main() -> int:
    ...
    for path, old, new in updated:
        # **새 해시는 전체를 찍는다.** 예전에는 양쪽을 16자로 줄여 보여 줬는데, 그 출력이 복사해 붙이기
        # 좋아 보여서 실제로 **잘린 해시를 커밋할 뻔한 사례**가 나왔다(다른 세션 보고). 원장은 이 도구가
        # 이미 고쳐 놓았으므로 손으로 옮길 일이 없지만, 리베이스 충돌을 손으로 풀 때는 이 값을 그대로
        # 쓰게 된다 — 그때 잘린 값이면 게이트가 다시 빨개지고 원인이 안 보인다.
        # 옛 해시는 "무엇이 바뀌었나" 를 눈으로 잡는 용도라 줄인 채로 둔다.
        print(f"  {path}\n    {old[:16]}… → {new}")
    print(
